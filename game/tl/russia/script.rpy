# TODO: Translation updated at 2021-10-09 21:10

# game/script.rpy:1565
translate russia checkStatusAgents_7bd604a9:

    # "You have [capturedAgents] captured nano controlled agents and [freedAgents] freed agents ready for duty."
    "У вас есть [capturedAgents] захваченных агентов под контролем наноботов и [freedAgents] освобождённых агентов, готовых к работе."

# game/script.rpy:1567
translate russia checkStatusAgents_e2d69fb9:

    # "It is [timeToGrab] more days until the next Landgrab."
    "До следующего захвата тереторий осталось [timeToGrab] дней."

# game/script.rpy:1836
translate russia landgrabDeny_0e9509db:

    # y "Maybe not during a landgrab."
    y "Может, не во время захвата территории."

# game/script.rpy:2210
translate russia landgrabPlanning_a7c7f887:

    # "The Aces are currently at [acesStrength]%% strength. How many agents do you wish to send?"
    "Сила Тузов в настоящее время составляет [acesStrength]%%. Сколько агентов вы хотите отправить?"

# game/script.rpy:2214
translate russia landgrabPlanning_96de2b2d:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Вперёд! Вперёд! Вперёд!"

# game/script.rpy:2225
translate russia landgrabPlanning_96de2b2d_1:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Гоу! Гоу Гоу!"

# game/script.rpy:2236
translate russia landgrabPlanning_96de2b2d_2:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Погнали!"

# game/script.rpy:2256
translate russia landgrabPlanning_3264f385:

    # "Aces Lieutenant identified: Melody von Guggen."
    "Лейтенант Тузов идентифицирован: Мелоди фон Гугген."

# game/script.rpy:2266
translate russia landgrabPlanning_5137f2a6:

    # "Aces Leader identified: Margaret N."
    "Лидер Тузов идентифицирован: Маргарет Н."

# game/script.rpy:2285
translate russia landgrabPlanning_30bfb4cc:

    # "You lost [randLosses] agent(s) to nanobot control during the last attack."
    "[randLosses] агента(ов) попали под контроль наноботов во время последней атаки."

# game/script.rpy:2289
translate russia landgrabPlanning_6a13e552:

    # "The Drift Punk are currently at [punkStrength]%% strength. How many agents do you wish to send?"
    "Сила Дрифт Панков в настоящее время составляет [punkStrength]%%. Сколько агентов вы хотите отправить?"

# game/script.rpy:2293
translate russia landgrabPlanning_96de2b2d_3:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Поехали!"

# game/script.rpy:2304
translate russia landgrabPlanning_96de2b2d_4:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Вперёд! Вперёд! Вперёд!"

# game/script.rpy:2315
translate russia landgrabPlanning_96de2b2d_5:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Гоу! Гоу! Гоу!"

# game/script.rpy:2335
translate russia landgrabPlanning_a7dd2fa8:

    # "Drift Punk Lieutenant identified: Carla Wong."
    "Лейтенант Дрифт Панков идентифицирован: Карла Вонг."

# game/script.rpy:2345
translate russia landgrabPlanning_d01b0340:

    # "Drift Punk Lieutenant identified: Talia H."
    "Лейтенант Дрифт Панков идентифицирован: Талия Х"

# game/script.rpy:2355
translate russia landgrabPlanning_9964e6a8:

    # "Drift Punk Lieutenant identified: ???"
    "Лейтенант Дрифт Панков идентифицирован: ???"

# game/script.rpy:2378
translate russia landgrabPlanning_30bfb4cc_1:

    # "You lost [randLosses] agent(s) to nanobot control during the last attack."
    "[randLosses] агента(ов) попали под контроль наноботов во время последней атаки."

# game/script.rpy:2382
translate russia landgrabPlanning_8a9ce5bc:

    # "The Outsiders are currently at [outsidersStrength]%% strength. How many agents do you wish to send?"
    "Сила Аутсайдеров в настоящее время составляет [outsidersStrength]%%. Сколько агентов вы хотите отправить?"

# game/script.rpy:2386
translate russia landgrabPlanning_96de2b2d_6:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Погнали!"

# game/script.rpy:2397
translate russia landgrabPlanning_96de2b2d_7:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Поехали!"

# game/script.rpy:2408
translate russia landgrabPlanning_96de2b2d_8:

    # "Agents" "Go! Go! Go!"
    "Агенты" "Вперёд! Вперёд! Вперёд!"

# game/script.rpy:2428
translate russia landgrabPlanning_7b3dd031:

    # "Outsiders Lieutenant identified: Muffy Peprich."
    "Лейтенант Аутсайдеров идентифицирован: Маффи Пеприч"

# game/script.rpy:2438
translate russia landgrabPlanning_6a21a8a0:

    # "Outsiders Lieutenant identified: Felicity Fences."
    "Лейтенант Аутсайдеров идентифицирован: Фелисити Фэнсэс"

# game/script.rpy:2448
translate russia landgrabPlanning_46c244e2:

    # "Outsiders Leader identified: The Great Kandinsky."
    "Лидер Аутсайдеров идентифицирован: Великий Кандинский."

# game/script.rpy:2471
translate russia landgrabPlanning_30bfb4cc_2:

    # "You lost [randLosses] agent(s) to nanobot control during the last attack."
    "[randLosses] агента(ов) попали под контроль наноботов во время последней атаки."

# game/script.rpy:2475
translate russia landgrabPlanning_bb46a66a:

    # "A small group of civillians is scavenging for supplies while the gangs are preoccupied."
    "Пока банды заняты, небольшая группа мирных жителей собирает припасы."

# game/script.rpy:2524
translate russia donateMenu1_34943d18:

    # "The thankful civilians give you $ [donateReward] as a reward."
    "В качестве благодарности они дают вам [donateReward]$."

# game/script.rpy:2528
translate russia donateMenu1_f92daa31:

    # "They also give you {color=#ffeda6}Medkit{/color} x3."
    "Они также дают вам {color=#ffeda6}Аптечка{/color} x3."

# game/script.rpy:2532
translate russia donateMenu1_b71a34d2:

    # "They also give you {color=#ffeda6}Computer Chip{/color} x3."
    "Они также дают вам {color=#ffeda6}Компьютерный чип{/color} x3."

# game/script.rpy:2536
translate russia donateMenu1_c1802a98:

    # "They also give you a {color=#ffeda6}Nanobot Injector{/color}."
    "Они также дают вам {color=#ffeda6}Инжектор наноботов{/color}."

# game/script.rpy:2541
translate russia donateMenu1_bb46a66a:

    # "A small group of civillians is scavenging for supplies while the gangs are preoccupied."
    "Пока банды заняты, небольшая группа мирных жителей собирает припасы."

# game/script.rpy:2590
translate russia donateMenu2_34943d18:

    # "The thankful civilians give you $ [donateReward] as a reward."
    "В качестве благодарности они дают вам [donateReward]$."

# game/script.rpy:2594
translate russia donateMenu2_927cc992:

    # "They also give you {color=#ffeda6}Magazine{/color} x3."
    "Они также дают вам {color=#ffeda6}Журнал{/color} x3."

# game/script.rpy:2598
translate russia donateMenu2_a920facc:

    # "They also give you {color=#ffeda6}Saltpeter{/color} x3."
    "Они также дают вам {color=#ffeda6}Селитра{/color} x3."

# game/script.rpy:2602
translate russia donateMenu2_c1802a98:

    # "They also give you a {color=#ffeda6}Nanobot Injector{/color}."
    "Они также дают вам {color=#ffeda6}Инжектор наноботов{/color}."

# game/script.rpy:2607
translate russia donateMenu2_bb46a66a:

    # "A small group of civillians is scavenging for supplies while the gangs are preoccupied."
    "Пока банды заняты, небольшая группа мирных жителей собирает припасы."

# game/script.rpy:2656
translate russia donateMenu3_34943d18:

    # "The thankful civilians give you $ [donateReward] as a reward."
    "В качестве благодарности они дают вам [donateReward]$."

# game/script.rpy:2660
translate russia donateMenu3_e8686378:

    # "They also give you {color=#ffeda6}Intel{/color} x1."
    "Они также дают вам {color=#ffeda6}Информация{/color} x1."

# game/script.rpy:2664
translate russia donateMenu3_23b2929c:

    # "They also give you {color=#ffeda6}Valuables{/color} x1."
    "Они также дают вам {color=#ffeda6}Ценности{/color} x1."

# game/script.rpy:2668
translate russia donateMenu3_c1802a98:

    # "They also give you a {color=#ffeda6}Nanobot Injector{/color}."
    "Они также дают вам {color=#ffeda6}Инжектор наноботов{/color}."

# game/script.rpy:2754
translate russia workBonusMenu_179f5a7a:

    # sM "[playerName]? I got something here..."
    sM "[playerName]? У меня тут кое-что есть..."

# game/script.rpy:2755
translate russia workBonusMenu_bf70d271:

    # sM "I found someone who can give me more intel, but wants something in return."
    sM "Я нашла человека, который может дать мне больше сведений, но он хочет кое-что взамен."

# game/script.rpy:2756
translate russia workBonusMenu_a4a5a1a8:

    # sM "................."
    sM "................."

# game/script.rpy:2757
translate russia workBonusMenu_778ec03e:

    # sM "He wants me to show my naked breasts..."
    sM "Он хочет, чтобы я показала свою обнаженную грудь..."

# game/script.rpy:2761
translate russia workBonusMenu_c2a629e3:

    # y "Well what are you waiting for?! Show them!"
    y "Чего же ты ждёшь?! Вперёд!"

# game/script.rpy:2762
translate russia workBonusMenu_deadfe83:

    # sM "..........."
    sM "..........."

# game/script.rpy:2763
translate russia workBonusMenu_489ecc25:

    # sM "{b}*Sigh*{/b} Yes sir."
    sM "{b}*Вздох*{/b} Да, сэр."

# game/script.rpy:2772
translate russia workBonusMenu_9330ff72:

    # s "Well... here you go...."
    s "Что ж... вот, пожалуйста...."

# game/script.rpy:2773
translate russia workBonusMenu_96fdcbe3:

    # "Gangster" "Oh wow you actually did it...!"
    "Гангстер" "Ух, ты действительно это сделала...!"

# game/script.rpy:2774
translate russia workBonusMenu_a6826a00:

    # "Gangster" "A deal's a deal. Here's the intel."
    "Гангстер" "Сделка есть сделка. Вот всё что я знаю."

# game/script.rpy:2786
translate russia workBonusMenu_93175713:

    # s "You wanted to see boobs? Then you'll get boobs..."
    s "Ты хотел увидеть сиськи? Вот тебе сиськи..."

# game/script.rpy:2787
translate russia workBonusMenu_03613af9:

    # "Gangster" "Oh my God! You have the nicest rack in the Aces!"
    "Гангстер" "Боже мой! У тебя самая красивая грудь из Тузов!"

# game/script.rpy:2788
translate russia workBonusMenu_1f4e8432:

    # s "About the intel...."
    s "На счёт сведений...."

# game/script.rpy:2789
translate russia workBonusMenu_dad6cdc7:

    # "Gangster" "Oh of course! Here it is."
    "Гангстер" "Ах точно! Вот."

# game/script.rpy:2801
translate russia workBonusMenu_2436547e:

    # s "Feast your eyes on these puppies!"
    s "Полюбуйся на эти буффера!"

# game/script.rpy:2802
translate russia workBonusMenu_5a03c004:

    # "Gangster" "The tits of a Goddess~....!"
    "Гангстер" "Грудь богини~....!"

# game/script.rpy:2803
translate russia workBonusMenu_82d4a18a:

    # "Gangster" "Here's the intel. I'll come find you if I find anymore!"
    "Гангстер" "Вот информация. Свяжусь с тобой, если найду ещё!"

# game/script.rpy:2804
translate russia workBonusMenu_c5a0fa39:

    # s "Sure, I'll be waiting~...."
    s "Конечно, буду ждать~...."

# game/script.rpy:2818
translate russia workBonusMenu_78a15204:

    # y "Don't be a floozy, Sam."
    y "Не будь шлюхой, Сэм."

# game/script.rpy:2819
translate russia workBonusMenu_c0faf73a:

    # sM "But I thought...!"
    sM "Но я думала...!"

# game/script.rpy:2820
translate russia workBonusMenu_47bebeb5:

    # sM "Erm... well all right then. I'll tell him no."
    sM "Эм... Ну тогда ладно. Я откажу ему."

# game/script.rpy:2823
translate russia workBonusMenu_910f432a:

    # sM "Hey [playerName]..?"
    sM "Эй [playerName]..?"

# game/script.rpy:2824
translate russia workBonusMenu_c2fc04f4:

    # y "What's up, Sam?"
    y "Как дела, Сэм?"

# game/script.rpy:2825
translate russia workBonusMenu_36074754:

    # sM "Well I think I can bring back a little more money, but~..."
    sM "Думаю, что смогу заработать немного больше денег, но~..."

# game/script.rpy:2826
translate russia workBonusMenu_23784e12:

    # sM "I'll have to do some sexy dancing for it..."
    sM "Мне придётся исполнить несколько сексуальных танцев для этого..."

# game/script.rpy:2830
translate russia workBonusMenu_6f0a59f2:

    # y "More is money! Time to lose your clothes!"
    y "Деньги есть деньги! Раздевайся!"

# game/script.rpy:2832
translate russia workBonusMenu_2047d4c1:

    # sM "I guess you're right... I'm not happy about it though."
    sM "Думаю, ты прав... Я тоже не в восторге от этого."

# game/script.rpy:2840
translate russia workBonusMenu_9330ff72_1:

    # s "Well... here you go...."
    s "Что ж... вот, пожалуйста...."

# game/script.rpy:2841
translate russia workBonusMenu_96fdcbe3_1:

    # "Gangster" "Oh wow you actually did it...!"
    "Гангстер" "Ого, ты действительно это сделала...!"

# game/script.rpy:2842
translate russia workBonusMenu_71274c40:

    # "Gangster" "Okay a deal's a deal. Here's the intel."
    "Гангстер" "Сделка есть сделка. Вот всё что у меня есть."

# game/script.rpy:2856
translate russia workBonusMenu_93175713_1:

    # s "You wanted to see boobs? Then you'll get boobs..."
    s "Ты хотел увидеть сиськи? Вот тебе сиськи..."

# game/script.rpy:2857
translate russia workBonusMenu_03613af9_1:

    # "Gangster" "Oh my God! You have the nicest rack in the Aces!"
    "Гангстер" "Боже мой! У тебя самая красивая грудь в Тузе!"

# game/script.rpy:2858
translate russia workBonusMenu_1f4e8432_1:

    # s "About the intel...."
    s "На счёт сведений...."

# game/script.rpy:2859
translate russia workBonusMenu_dad6cdc7_1:

    # "Gangster" "Oh of course! Here it is."
    "Гангстер" "Ах точно! Вот."

# game/script.rpy:2873
translate russia workBonusMenu_2436547e_1:

    # s "Feast your eyes on these puppies!"
    s "Полюбуйся на эти буффера!"

# game/script.rpy:2874
translate russia workBonusMenu_5a03c004_1:

    # "Gangster" "The tits of a Goddess~....!"
    "Гангстер" "Грудь богини~....!"

# game/script.rpy:2875
translate russia workBonusMenu_82d4a18a_1:

    # "Gangster" "Here's the intel. I'll come find you if I find anymore!"
    "Гангстер" "Вот информация. Свяжусь с тобой, если найду ещё!"

# game/script.rpy:2876
translate russia workBonusMenu_c5a0fa39_1:

    # s "Sure, I'll be waiting~...."
    s "Конечно, буду ждать~...."

# game/script.rpy:2883
translate russia workBonusMenu_2a87d55a:

    # y "You can't go showing off your naked body!"
    y "Нельзя выставлять напоказ своё обнаженное тело!"

# game/script.rpy:2884
translate russia workBonusMenu_611fd901:

    # sM "Really? But... {i}'you'{/i} saw me naked."
    sM "Правда? Но... {i}'ты'{/i} видел меня голой."

# game/script.rpy:2885
translate russia workBonusMenu_c4094011:

    # sM "I mean.. If you don't want me to, then I approve. I'd rather not, but I expected you to be all for it."
    sM "Я имею в виду... Если ты не хочешь, то я только за. Сама я не очень хотела бы, но думала что ты не упустишь такой шанс."

# game/script.rpy:2886
translate russia workBonusMenu_b8effcc6:

    # sM "I'll refuse the offer then."
    sM "Тогда я откажусь от предложения."

# game/script.rpy:2889
translate russia workBonusMenu_fc320f6a:

    # sM "This is Sam, reporting in."
    sM "Это Сэм, докладываю."

# game/script.rpy:2890
translate russia workBonusMenu_1c78490b:

    # y "Sam? Did something happen?"
    y "Сэм? Что-то случилось?"

# game/script.rpy:2891
translate russia workBonusMenu_fc625359:

    # sM "No, just..."
    sM "Нет, просто..."

# game/script.rpy:2892
translate russia workBonusMenu_b205668f:

    # sM "..........................."
    sM "..........................."

# game/script.rpy:2893
translate russia workBonusMenu_0efa3e7f:

    # sM "They dared me to make out with another girl."
    sM "Они пытаются заставить меня целоваться с другой девушкой."

# game/script.rpy:2894
translate russia workBonusMenu_ac17dec1:

    # y "What?"
    y "Что?"

# game/script.rpy:2895
translate russia workBonusMenu_b1047d09:

    # sM "They want me to kiss one of the girls. I won't get anything for it, but it'll boost my reputation a bit."
    sM "Они хотят, чтобы я поцеловала одну из девушек. Я ничего за это не получу, но это немного поднимет мою репутацию."

# game/script.rpy:2899
translate russia workBonusMenu_14d7a52a:

    # y "H-hot... lesbian action?"
    y "Г-горячо... Лесбийские игры?"

# game/script.rpy:2900
translate russia workBonusMenu_8c054575:

    # sM "[playerName]..."
    sM "[playerName]..."

# game/script.rpy:2901
translate russia workBonusMenu_3c06de5d:

    # y "YES!"
    y "ДА!"

# game/script.rpy:2902
translate russia workBonusMenu_4b4fe901:

    # y "I mean... erm... yes, I think you totally should."
    y "Я... ээмм... да, думаю, тебе стоит."

# game/script.rpy:2903
translate russia workBonusMenu_82e29b2d:

    # sM "I'm not sure I like how quickly you agreed to this, but whatever."
    sM "Мне не нравиться что ты так быстро согласился на это, но неважно."

# game/script.rpy:2904
translate russia workBonusMenu_1899a86b:

    # sM "It's a fine day to start doubting my sexuality..."
    sM "Сегодня прекрасный день, чтобы начать сомневаться в своей сексуальности..."

# game/script.rpy:2910
translate russia workBonusMenu_db80d5b6:

    # y "You can't just start kissing everyone you see, Sam."
    y "Ты не можешь просто начать целоваться с каждым встречным, Сэм."

# game/script.rpy:2911
translate russia workBonusMenu_9e3a0699:

    # sM "No, I know, but..."
    sM "Нет, я знаю, но..."

# game/script.rpy:2912
translate russia workBonusMenu_1e167edd:

    # y "I know kissing girls is hot, but please suppress your urges."
    y "Я знаю, что целоваться с девушкой это очень сексуально, но, пожалуйста, подави свои порывы."

# game/script.rpy:2913
translate russia workBonusMenu_8c054575_1:

    # sM "[playerName]..."
    sM "[playerName]..."

# game/script.rpy:2916
translate russia workBonusMenu_6d3a91a6:

    # sM "Hey [playerName], it's me, Sam."
    sM "Привет [playerName], это я, Сэм."

# game/script.rpy:2917
translate russia workBonusMenu_defb9c19:

    # y "Hello [samNick]. Anything to report?"
    y "Привет [samNick]. Есть что сообщить?"

# game/script.rpy:2918
translate russia workBonusMenu_1ef893c2:

    # sM "....................."
    sM "....................."

# game/script.rpy:2919
translate russia workBonusMenu_1a295c3e:

    # sM "Someone wants a blowjob...."
    sM "Кто-то хочет минет...."

# game/script.rpy:2920
translate russia workBonusMenu_db832180:

    # y "........"
    y "........"

# game/script.rpy:2921
translate russia workBonusMenu_ac17dec1_1:

    # y "What?"
    y "Что?"

# game/script.rpy:2922
translate russia workBonusMenu_9c4f3726:

    # sM "One of the gangsters promised me $100 bucks for a blowjob."
    sM "Один из бандитов пообещал мне 100 долларов за минет."

# game/script.rpy:2926
translate russia workBonusMenu_3daa469b:

    # y "I don't see the downside to this."
    y "Не вижу в этом ничего плохого."

# game/script.rpy:2927
translate russia workBonusMenu_e3073794:

    # sM "But it's a blowjob! That's pretty serious!"
    sM "Но это же минет! Это довольно серьезно!"

# game/script.rpy:2928
translate russia workBonusMenu_46f95e79:

    # y "No it's not. Better get sucking!"
    y "Нет, это не так. Лучше приступая сосать!"

# game/script.rpy:2929
translate russia workBonusMenu_dfaba66e:

    # y "Start polishing that girthy dickmeat until it glistens and explodes with the furiosity of a vulcano! Raining down on your soft and gentle pale face like the mana from heaven, in your greatest time of need!"
    y "Начинай полировать этот член, пока он не заблестит и не взорвётся словно вулкана! Капая на твое мягкое и нежное лицо, как манна с небес, в самое трудное для тебя время!"

# game/script.rpy:2930
translate russia workBonusMenu_14808151:

    # sM "Okay jeeze! Fine! I'll suck his dick!"
    sM "Отлично, черт возьми! Ладно! Я отсосу его член!"

# game/script.rpy:2931
translate russia workBonusMenu_a45e4779:

    # sM "God!"
    sM "Боже!"

# game/script.rpy:2938
translate russia workBonusMenu_5043ff6c:

    # "Gangster" "Worth every dollar..."
    "Гангстер" "Это стоит каждого цента..."

# game/script.rpy:2939
translate russia workBonusMenu_f2dff589:

    # s "...................."
    s "...................."

# game/script.rpy:2954
translate russia workBonusMenu_4e7bc82a:

    # y "And have you walk around the base with penisbreath? I don't think so."
    y "Чтобы ты ходила по базе и от тебя воняло пенисами? Не думаю."

# game/script.rpy:2955
translate russia workBonusMenu_89ff92c0:

    # sM "Okay good, cause I wasn't really feeling lik-..."
    sM "Отлично, потому что я не особо хотела-..."

# game/script.rpy:2956
translate russia workBonusMenu_18467801:

    # y "Just imagining you suckle down on somebody else's meat makes me sick!"
    y "Я даже представлять не хочу, что ты будешь сосать кому-то кроме меня!"

# game/script.rpy:2957
translate russia workBonusMenu_dd887596:

    # sM "Er..."
    sM "Эм..."

# game/script.rpy:2958
translate russia workBonusMenu_1eab1582:

    # y "I couldn't even look at you! {size=+4}You whore!{/size} {size=+8}You floozy!{/size} {size=+12}You common street tart!{/size}"
    y "Как я после этого должен буду смотреть тебе в глаза! {size=+4}Шлюха!{/size} {size=+8}Потаскуха!{/size} {size=+12}Обычная уличная шмара!{/size}"

# game/script.rpy:2959
translate russia workBonusMenu_0c166f64:

    # sM "Okay okay! Jeeze... calm down. No blowjobs, got it."
    sM "Хорошо! Боооже... успокойся. Никаких минетов, я поняла."

# game/script.rpy:2962
translate russia workBonusMenu_5cd5fc61:

    # sM "[playerName] report in."
    sM "[playerName] приём."

# game/script.rpy:2963
translate russia workBonusMenu_111e0d25:

    # y "Reporting in! What's up [samNick]"
    y "Приём! Чё каво [samNick]?"

# game/script.rpy:2964
translate russia workBonusMenu_f4a51d52:

    # sM "{b}*Ahem*{/b} Well..."
    sM "{b}*Кхм*{/b} Нуу..."

# game/script.rpy:2965
translate russia workBonusMenu_290397ae:

    # sM "..................."
    sM "..................."

# game/script.rpy:2966
translate russia workBonusMenu_fe42ecf0:

    # y "What~...?"
    y "Что~...?"

# game/script.rpy:2967
translate russia workBonusMenu_4f025f8d:

    # sM "Wet... wet t-shirt contest..."
    sM "Конк-... конкурс мокрых футболок..."

# game/script.rpy:2968
translate russia workBonusMenu_c545d660:

    # y "Oh... Oooh!"
    y "Оо... Ооох!"

# game/script.rpy:2969
translate russia workBonusMenu_555c4c3d:

    # sM "Should I?"
    sM "Стоит?"

# game/script.rpy:2977
translate russia workBonusMenu_3c06de5d_1:

    # y "YES!"
    y "ДА!"

# game/script.rpy:2978
translate russia workBonusMenu_fd2c4cdd:

    # y "Wait do we get anything out of it?"
    y "Подожди, а мы что-нибудь за это получим?"

# game/script.rpy:2979
translate russia workBonusMenu_3b7eeff8:

    # sM "It's a friendly competition between gangs. It just increases my reputation with them a bit."
    sM "Это дружеское соревнование между бандами. Конкурс немного поднимет мою репутацию среди банд."

# game/script.rpy:2980
translate russia workBonusMenu_31d2c39c:

    # y "Good enough, wet t-shirt contest!"
    y "Отлично, конкурс мокрых майек!"

# game/script.rpy:2981
translate russia workBonusMenu_c770b903:

    # sM "As you wish..."
    sM "Как скажешь..."

# game/script.rpy:2986
translate russia workBonusMenu_72a6ede8:

    # "Aces" "Go Sam, go Sam, go Sam!"
    "Тузы" "Вперёд Сэм, давай Сэм!"

# game/script.rpy:3001
translate russia workBonusMenu_e91a7261:

    # y "And have them see your perfectly shaped breasts clinging through your wet t-shirt as your gentle nipples poke through the fabric?"
    y "И, удалось всем лицезреть твою прекрасную, пышную грудь, с выпирающими от низкой температуры сосками?"

# game/script.rpy:3002
translate russia workBonusMenu_ca7034d8:

    # sM "Er... yeah pretty much."
    sM "Эмм... да, думаю все всё разглядели."

# game/script.rpy:3003
translate russia workBonusMenu_65abada1:

    # y "Never! Only I get to behold such majesty!"
    y "Никогда! Только я имею право созерцать такое величие!"

# game/script.rpy:3004
translate russia workBonusMenu_577cf4cd:

    # sM "No? Well okay then."
    sM "Нет? Ну и хорошо."

# game/script.rpy:3007
translate russia workBonusMenu_150c4e32:

    # y "Sam?"
    y "Сэм?"

# game/script.rpy:3008
translate russia workBonusMenu_71e36e5e:

    # sM "I think I got something here..."
    sM "Думаю, что я что-то нашла..."

# game/script.rpy:3009
translate russia workBonusMenu_b458cd44:

    # sM "I just overheard a couple of girls talking, but they headed into a sauna."
    sM "Я только что подслушала разговор нескольких девушек, но они направились в сауну."

# game/script.rpy:3010
translate russia workBonusMenu_41e92603:

    # sM "It sounded important, but it's... a mixed gender sauna."
    sM "Разговор звучал очень важно, но это... смешанная сауна."

# game/script.rpy:3014
translate russia workBonusMenu_014c8d83:

    # sM "Maybe it's just girls in there..."
    sM "Может быть там только девушки..."

# game/script.rpy:3015
translate russia workBonusMenu_580eeef5:

    # sM "Okay, I'm going in!"
    sM "Ладно, я вхожу!"

# game/script.rpy:3016
translate russia workBonusMenu_579b3821:

    # "...................................."
    "...................................."

# game/script.rpy:3017
translate russia workBonusMenu_893f74a7:

    # y "There's also guys in there, aren't there?"
    y "Там есть парни, не так ли?"

# game/script.rpy:3018
translate russia workBonusMenu_912fc811:

    # s "{b}*Whines*{/b}"
    s "{b}*Скулит*{/b}"

# game/script.rpy:3024
translate russia workBonusMenu_226ec59d:

    # y "Too bad. Don't follow them."
    y "Жаль. Не стоит их преследовать."

# game/script.rpy:3025
translate russia workBonusMenu_1789899c:

    # sM "Really? But it sounds like a good way to get some intel."
    sM "Серьёзно? Но это похоже отличный шанс получить немного полезной информации."

# game/script.rpy:3026
translate russia workBonusMenu_8d91907f:

    # y "At the cost of you exposing yourself to strangers!"
    y "За счёт того, что ты выставишь себя на показ перед незнакомыми!"

# game/script.rpy:3027
translate russia workBonusMenu_77935314:

    # sM "........................"
    sM "........................"

# game/script.rpy:3030
translate russia workBonusMenu_b0f8508d:

    # "EVENT NOT VISIBLE TO PLAYER"
    "СОБЫТИЕ НЕ ВИДНО ИГРОКУ"

# game/script.rpy:3032
translate russia workBonusMenu_92c0efff:

    # sM "Sam reporting in. I... may have an opportunity."
    sM "Сэм докладывает. Может... я смогу."

# game/script.rpy:3033
translate russia workBonusMenu_81df9bf0:

    # sM "Just got a request to show my tits. Should I?"
    sM "Только что попросили показать сиськи. Стоит ли?"

# game/script.rpy:3037
translate russia workBonusMenu_b07458d2:

    # y "Will you get anything out of it?"
    y "Ты что-нибудь получишь за это?"

# game/script.rpy:3038
translate russia workBonusMenu_001d2bd9:

    # sM "A little bit of money. They also promised me some intel if I did it."
    sM "Немного денег. Они также обещали мне достать для меня кое-какую информацию, если я это сделаю."

# game/script.rpy:3039
translate russia workBonusMenu_2fb1ec00:

    # y "Then go right ahead."
    y "Ну тогда, вперёд."

# game/script.rpy:3040
translate russia workBonusMenu_bc0fffb9:

    # sM "All right... naked tits seem to have a way of making guys talk..."
    sM "Лааадно... Похоже, что голые сиськи, смогут заставить говорить даже мёртвого парня..."

# game/script.rpy:3048
translate russia workBonusMenu_13c5d017:

    # s "Here you go you guys~...."
    s "Вот, наслаждайтесь ребятки~...."

# game/script.rpy:3049
translate russia workBonusMenu_6194ec65:

    # "Gangsters" "Wooo! Go Sammy!"
    "Гангстеры" "Вооо! Давай Сэммии!"

# game/script.rpy:3069
translate russia workBonusMenu_c0d7e84b:

    # y "Nah, but give 'em a slight hint that you {i}'might'{/i} do in the future and leave them a false sense of hope."
    y "Неет, но намекни им, что {i}'может быть'{/i} в будущем им больше повезёт, оставь им ложное чувство надежды."

# game/script.rpy:3070
translate russia workBonusMenu_4847fbb1:

    # sM "That's evil..."
    sM "Это злобно..."

# game/script.rpy:3078
translate russia workBonusMenu_4a36bb80:

    # cM "Clover here."
    cM "Клевер здесь."

# game/script.rpy:3079
translate russia workBonusMenu_a270a1da:

    # y "Coming in loud and clear."
    y "Слышу тебя громко и чётко."

# game/script.rpy:3080
translate russia workBonusMenu_b9ecd3b6:

    # cM "This one guy promised me some intel in return for..."
    cM "Один парень обещал мне кое-какие сведения в обмен на..."

# game/script.rpy:3081
translate russia workBonusMenu_e08e4da7:

    # cM "............."
    cM "............."

# game/script.rpy:3082
translate russia workBonusMenu_e11aa16e:

    # cM "{b}*Sighs*{/b} Motorboating my tits."
    cM "{b}*Вздох*{/b} Мне нужно потрясти сиськами перед ним."

# game/script.rpy:3088
translate russia workBonusMenu_0b160287:

    # y "We need all the intel we can get! Let him!"
    y "Информации много не бывает! Вперёд!"

# game/script.rpy:3089
translate russia workBonusMenu_cdf4a6e9:

    # cM "Ugh... {i}'so'{/i} not cool. Fine whatever, I'll let him..."
    cM "Угх... {i}'круто'{/i}. Ладно, неважно, я это сделаю..."

# game/script.rpy:3094
translate russia workBonusMenu_5683ff12:

    # c "{b}*Scoffs*{/b} Happy now?"
    c "{b}*Насмехаться*{/b} Ну что, доволен?"

# game/script.rpy:3095
translate russia workBonusMenu_ffdb5ea8:

    # "Gangster" "I've never been so happy in my life!"
    "Гангстер" "Я никогда в жизни не был так счастлив!"

# game/script.rpy:3108
translate russia workBonusMenu_d6752c12:

    # y "What a weirdo."
    y "Вот чудак."

# game/script.rpy:3109
translate russia workBonusMenu_27f75419:

    # cM "That's what I thought."
    cM "Я так и подумала."

# game/script.rpy:3110
translate russia workBonusMenu_56b76953:

    # y "Tell him no. Your tits are not for motorboating."
    y "Откажи ему. Твоя грудь слишком хороша для этого."

# game/script.rpy:3111
translate russia workBonusMenu_c37a0ac3:

    # cM "Unless it's you...?"
    cM "Если только это не ты...?"

# game/script.rpy:3112
translate russia workBonusMenu_2bac6c11:

    # y "Exactly!"
    y "В точку!"

# game/script.rpy:3115
translate russia workBonusMenu_688d7c2b:

    # cM "I got an opportunity here."
    cM "У меня есть возможность."

# game/script.rpy:3116
translate russia workBonusMenu_4a132c05:

    # cM "An amateur photographer wannabe wants to photograph me in lingerie for money."
    cM "Фотограф-любитель хочет сфотографировать меня в нижнем белье за деньги."

# game/script.rpy:3125
translate russia workBonusMenu_294bc26e:

    # y "Hot."
    y "Горячо."

# game/script.rpy:3126
translate russia workBonusMenu_1071bd75:

    # cM ".................."
    cM ".................."

# game/script.rpy:3127
translate russia workBonusMenu_30fc3695:

    # y "Bring home the money!"
    y "Принеси деньги домой!"

# game/script.rpy:3128
translate russia workBonusMenu_6cc98eb0:

    # cM "Fine..."
    cM "Лаадно..."

# game/script.rpy:3133
translate russia workBonusMenu_7e5869e9:

    # "Gangster" "Yes, you are rocking it!"
    "Гангстер" "Да, ты в ударе!"

# game/script.rpy:3134
translate russia workBonusMenu_a592b42b:

    # c "Did we have to do it outside...?"
    c "Мы должны были делать это на улице...?"

# game/script.rpy:3154
translate russia workBonusMenu_be506a49:

    # y "Hey, nobody is allowed to photograph you in your underwear but me!"
    y "Эй, никому, кроме меня, нельзя фотографировать тебя в нижнем белье!"

# game/script.rpy:3155
translate russia workBonusMenu_f5bfed0a:

    # cM "I figured you'd say that. All right, I'll tell him no."
    cM "Так и думала, что ты это скажешь. Хорошо, я откажу ему."

# game/script.rpy:3158
translate russia workBonusMenu_4a1aeb1b:

    # y "Clover?"
    y "Кловер?"

# game/script.rpy:3159
translate russia workBonusMenu_aa2d2e59:

    # cM "So er..."
    cM "Нуу эмм..."

# game/script.rpy:3160
translate russia workBonusMenu_d823f314:

    # cM "I accidentally spilled some milk over my top and it's becoming see through..."
    cM "Я случайно пролила немного молока на свой топ, и он стал просвечивать..."

# game/script.rpy:3161
translate russia workBonusMenu_29eb2a13:

    # y "Nice."
    y "Класс."

# game/script.rpy:3162
translate russia workBonusMenu_d9c4d6fd:

    # cM "Should I quickly go change and walk around like this for a while?"
    cM "Стоит ли мне быстро переодеться или походить так некоторое время?"

# game/script.rpy:3163
translate russia workBonusMenu_fae88dae:

    # cM "It might raise my reputation around town."
    cM "Это может поднять мою репутацию в городе."

# game/script.rpy:3169
translate russia workBonusMenu_3a2ef091:

    # y "Don't be silly. Don't change and be sure to show off a bit!"
    y "Не глупи. Не переодевайся и не забудьте немного покрасоваться!"

# game/script.rpy:3170
translate russia workBonusMenu_dc8d1092:

    # cM "All right, I'm already drawing some attention."
    cM "Отлично, потому что я уже начала привлекать внимание."

# game/script.rpy:3175
translate russia workBonusMenu_6616a39c:

    # "Gangster" "WOAH CLOVER!"
    "Гангстер" "ВАУ КЛОВЕР!"

# game/script.rpy:3176
translate russia workBonusMenu_1893f391:

    # "Gangster" "Niiiice...!"
    "Гангстер" "Клаааас...!"

# game/script.rpy:3194
translate russia workBonusMenu_3c3cca0f:

    # y "Have you no shame! Of course you're gonna go change!"
    y "Тебе не стыдно! Конечно иди и переоденься!"

# game/script.rpy:3195
translate russia workBonusMenu_9de2ba72:

    # cM "But..."
    cM "Но..."

# game/script.rpy:3196
translate russia workBonusMenu_75c194a3:

    # y "Change! Can't have you walk around half decent!"
    y "Переоденься! Не может быть, чтобы ты ходила в полупрозрачном топе!"

# game/script.rpy:3197
translate russia workBonusMenu_6cc98eb0_1:

    # cM "Fine..."
    cM "Лаадно..."

# game/script.rpy:3200
translate russia workBonusMenu_711b2bb8:

    # cM "Clover here. Someone is offering me $120 for a boobjob."
    cM "Это Кловер, приём. Кто-то предлагает мне 120$ за пайзури."

# game/script.rpy:3201
translate russia workBonusMenu_73d39b8f:

    # cM "..............."
    cM "..............."

# game/script.rpy:3202
translate russia workBonusMenu_06773d24:

    # cM "Should I...?"
    cM "Стоит...?"

# game/script.rpy:3209
translate russia workBonusMenu_e603f4a2:

    # y "Money for lying down and doing nothing. Go for it!"
    y "Деньги за то, чтобы лечь и ничего не делать. Конечно!"

# game/script.rpy:3210
translate russia workBonusMenu_72526a76:

    # cM "I can't believe I'm doing this..."
    cM "Не могу поверить, что я это делаю..."

# game/script.rpy:3215
translate russia workBonusMenu_edcc3b84:

    # c "Ew...."
    c "Фуу...."

# game/script.rpy:3216
translate russia workBonusMenu_b67f5c6d:

    # "Gangster" "Wow Clover. I never knew you were such a whore!"
    "Гангстер" "Ого Кловер. Я и подумать не мог, что ты такая шлюха!"

# game/script.rpy:3217
translate russia workBonusMenu_2d13b75c:

    # c "................"
    c "................"

# game/script.rpy:3218
translate russia workBonusMenu_6ddceada:

    # y "What a jerk..."
    y "Ну и придурок..."

# game/script.rpy:3220
translate russia workBonusMenu_85da164c:

    # y "You should visit him more often!"
    y "Тебе нужно посещать его почаще!"

# game/script.rpy:3221
translate russia workBonusMenu_a5a59e0a:

    # c r38 "..........."
    c r38 "..........."

# game/script.rpy:3235
translate russia workBonusMenu_d931fd05:

    # y "Well you 'are' the boobjob queen."
    y "Ну ты же 'королева' пайзури."

# game/script.rpy:3236
translate russia workBonusMenu_11ea47a6:

    # y "But we can't have you wasting your talents on just anyone! Refuse the offer."
    y "Но мы не можем растрачивать твой талант на кого угодно! Откажись."

# game/script.rpy:3237
translate russia workBonusMenu_724c7a3a:

    # cM "All right, I will."
    cM "Хорошо, откажусь."

# game/script.rpy:3240
translate russia workBonusMenu_089e8b87:

    # cM "Hey [playerName]. I got something here, but...."
    cM "Эй [playerName]. Кажется я кое-что нашла, но...."

# game/script.rpy:3241
translate russia workBonusMenu_6943c65f:

    # cM "Erm... there's a video chat between the 3 biggest gangs in the city... "
    cM "Эvм... тут есть видеочат между 3мя крупнейшими бандами в городе..."

# game/script.rpy:3242
translate russia workBonusMenu_86edd8ea:

    # y "And?"
    y "И?"

# game/script.rpy:3243
translate russia workBonusMenu_09f27e39:

    # cM "Well I was just thinking..."
    cM "Ну, я просто подумала..."

# game/script.rpy:3244
translate russia workBonusMenu_3d25d5eb:

    # cM "What if I flashed my tits on livestream?"
    cM "Что если, засветить свои сиськи в прямом эфире?"

# game/script.rpy:3248
translate russia workBonusMenu_d675b2fb:

    # y "That's a great idea!"
    y "Отличная идея!"

# game/script.rpy:3249
translate russia workBonusMenu_413c4980:

    # y "That has got to increase your reputation with the gangs a bit!"
    y "Это должно немного повысить твою репутацию среди банд!"

# game/script.rpy:3250
translate russia workBonusMenu_80bdc99d:

    # cM "Okay... Okay, I'll do it!"
    cM "Ладно... Хорошо, я это сделаю!"

# game/script.rpy:3257
translate russia workBonusMenu_8ec0bcea:

    # y "No! No tit flashing, bad Clover!"
    y "Нет! Никаких прямых эфиров, плохая Кловер!"

# game/script.rpy:3258
translate russia workBonusMenu_8bdd87d4:

    # cM "But it will be some quick easy reputat-..."
    cM "Но это довольно легкий рост репут-..."

# game/script.rpy:3259
translate russia workBonusMenu_f9dfc410:

    # y "You don't want to have a reputation of being quick and easy! Bad Clover!"
    y "Ты не хочешь иметь репутацию быстрой и легкой! Плохая Кловер!"

# game/script.rpy:3260
translate russia workBonusMenu_454984ce:

    # cM "All right, if you say so."
    cM "Лаадно, как скажешь."

# game/script.rpy:3263
translate russia workBonusMenu_6fff9bd6:

    # cM "I think I can score a little extra intel."
    cM "Думаю, что у меня получится заполучить немного дополнительной информации."

# game/script.rpy:3264
translate russia workBonusMenu_fbb57ff3:

    # y "That's good. What do you have?"
    y "Это хорошо. Что у тебя там?"

# game/script.rpy:3265
translate russia workBonusMenu_7a44ef99:

    # cM "I'm at a spa and just overheard some gangsters drop some info."
    cM "Я сейчас не далеко от спа-салона и только что услышала, как пару гангстеров обсуждают что-то важное."

# game/script.rpy:3266
translate russia workBonusMenu_c9ec8b04:

    # cM "I think I can get some extra intel out of them if I offer a massage~..."
    cM "Думаю, что смогу разузнать больше, если предложу им массаж~..."

# game/script.rpy:3270
translate russia workBonusMenu_acad6de6:

    # y "Sounds like an easy way to get intel. Do it."
    y "Звучит как простой способ раздобыть сведения. Вперёд."

# game/script.rpy:3271
translate russia workBonusMenu_a4e4f194:

    # cM "Understood."
    cM "Поняла."

# game/script.rpy:3277
translate russia workBonusMenu_b584513f:

    # y "Leave them for now. I'm sure another chance will come around."
    y "Оставьте их пока. Я уверен, что шанс ещё появится."

# game/script.rpy:3278
translate russia workBonusMenu_74598e77:

    # cM "If you say so."
    cM "Как скажешь."

# game/script.rpy:3281
translate russia workBonusMenu_1d2576fb:

    # cM "Er..."
    cM "Эмм..."

# game/script.rpy:3282
translate russia workBonusMenu_f233c925:

    # y "Yeah?"
    y "Что?"

# game/script.rpy:3283
translate russia workBonusMenu_a75fb888:

    # cM "I've been offered $1.000 to star in a bondage show~...."
    cM "Мне предложили $1000 за то, чтобы я снялась в бандаж-шоу~...."

# game/script.rpy:3284
translate russia workBonusMenu_f91d5a34:

    # cM "They're gonna undress me and tie me up."
    cM "Они собираются раздеть меня и связать."

# game/script.rpy:3285
translate russia workBonusMenu_faf8387d:

    # y "Aaaand fuck you?"
    y "Аааа трахнуть тебя?"

# game/script.rpy:3286
translate russia workBonusMenu_f66c9cc4:

    # cM "I have no idea..."
    cM "Я не знаю..."

# game/script.rpy:3290
translate russia workBonusMenu_26ab5298:

    # y "Sounds like easy money. Go be a play toy for a while."
    y "Звучит как лёгкие деньги. Иди притворись игрушкой на некоторое время."

# game/script.rpy:3291
translate russia workBonusMenu_ff8bdc94:

    # cM "All right then."
    cM "Хорошо."

# game/script.rpy:3296
translate russia workBonusMenu_c0297aa7:

    # y "Sounds like you're about to get gangbanged. Refuse."
    y "Звучит, как дешёвое предложение на групповушку. Отказывать."

# game/script.rpy:3297
translate russia workBonusMenu_0d0f949d:

    # cM "Might be for the best."
    cM "Может быть и к лучшему."

# game/script.rpy:3300
translate russia workBonusMenu_b53f49e4:

    # cM "Clover here. Would it be okay if I showed my tits off a bit today?"
    cM "Это Кловер, приём. Ничего, если я сегодня немного посвечу своими сиськами?"

# game/script.rpy:3301
translate russia workBonusMenu_1bb1cc56:

    # y "Okay...?"
    y "Лаадно...?"

# game/script.rpy:3302
translate russia workBonusMenu_9984a3a5:

    # cM "There's a good atmosphere here today. I bet I could get some loose lips talking~..."
    cM "Сегодня здесь царит хорошая атмосфера. Готова поспорить, что смогу разговорить шаловливые языки~..."

# game/script.rpy:3308
translate russia workBonusMenu_408734f9:

    # y "If you think it will help, go for it."
    y "Если думаешь, что это поможет, действуй."

# game/script.rpy:3309
translate russia workBonusMenu_cd63707e:

    # cM "All right, got it."
    cM "Хорошо, поняла."

# game/script.rpy:3315
translate russia workBonusMenu_3423b36f:

    # c "Check this out boys~...."
    c "Как дела мальчики~...."

# game/script.rpy:3316
translate russia workBonusMenu_79e097d8:

    # "Gangsters" "Clover! Clover! Clover!"
    "Гангстеры" "Кловер! Кловер! Кловер!"

# game/script.rpy:3331
translate russia workBonusMenu_1dc8433d:

    # y "Nnnnnnnnnnnnnnnnnope."
    y "Неееееееееееееееееет."

# game/script.rpy:3332
translate russia workBonusMenu_31e8a9eb:

    # cM "No?"
    cM "Нет?"

# game/script.rpy:3333
translate russia workBonusMenu_d3b3b939:

    # y "Nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn-....{nw}"
    y "Нннннннннннннннннннннннннннннннннннннннннннннннннннннннн-....{nw}"

# game/script.rpy:3334
translate russia workBonusMenu_f7cd42f3:

    # cM "Ugh... weirdo."
    cM "Угх... чудак."

# game/script.rpy:3343
translate russia workBonusMenu_1c2e0e2e:

    # aM "Alex here!"
    aM "Алекс на месте!"

# game/script.rpy:3344
translate russia workBonusMenu_e47a015b:

    # y "Alex? What's up?"
    y "Алекс? Что случилось?"

# game/script.rpy:3345
translate russia workBonusMenu_a62362b4:

    # aM "So erm..."
    aM "Нуу эмм..."

# game/script.rpy:3346
translate russia workBonusMenu_320d5a07:

    # aM "There's this one really sexy swimsuit for sale at the mall and I've just been dared to try it on."
    aM "В торговом центре продается один очень сексуальный купальник, и мне только что предложили его примерить."

# game/script.rpy:3347
translate russia workBonusMenu_5de1222a:

    # aM "In return the guy promised to share some intel."
    aM "Взамен парень пообещал поделиться кое-какими сведениями."

# game/script.rpy:3354
translate russia workBonusMenu_cbfff146:

    # y "Go for it. What kind of swimsuit?"
    y "Давай, вперёд. А что за купальник?"

# game/script.rpy:3355
translate russia workBonusMenu_8ce3470c:

    # aM "Erm... A one piece thong swimsuit..."
    aM "Эмм... Цельный купальник со стрингами..."

# game/script.rpy:3356
translate russia workBonusMenu_ce895a18:

    # y "..........."
    y "..........."

# game/script.rpy:3357
translate russia workBonusMenu_24800f17:

    # y "Not gonna lie, I'm kinda jealous that I'm not there to see it."
    y "Не буду врать, я немного завидую, что меня там не будет."

# game/script.rpy:3358
translate russia workBonusMenu_722d46be:

    # aM "Check the camera, I'll go put it on!"
    aM "Смотри по камерам, пойду переоденусь!"

# game/script.rpy:3364
translate russia workBonusMenu_d0dfca67:

    # a "So how do I look?"
    a "И, как я выгляжу?"

# game/script.rpy:3365
translate russia workBonusMenu_b6db739b:

    # "Gangster" "Nghhhh~...!"
    "Гангстер" "Нгххх~...!"

# game/script.rpy:3379
translate russia workBonusMenu_46e5a597:

    # y "Swimsuits are for swimming and I don't think you'll be doing any swimming."
    y "Купальники предназначены для плавания, а я не думаю, что вы собираетесь поплавать сегодня."

# game/script.rpy:3380
translate russia workBonusMenu_c47f0590:

    # y "So no. Don't put on the swimsuit."
    y "Так что нет. Не надевай купальник."

# game/script.rpy:3381
translate russia workBonusMenu_73306339:

    # aM "Okay, if you say so, [playerName]."
    aM "Хорошо, как скажешь, [playerName]."

# game/script.rpy:3384
translate russia workBonusMenu_1ed23f83:

    # aM "Hey [playerName]. There's this one super sketchie guy who says he'll pay me $50 if he can smell my feet!"
    aM "Привет [playerName]. Есть один суперхитрый парень, который говорит, что заплатит мне $50, если сможет понюхать мои ноги!"

# game/script.rpy:3385
translate russia workBonusMenu_f358cb0c:

    # y "................."
    y "................."

# game/script.rpy:3386
translate russia workBonusMenu_ac17dec1_2:

    # y "What?"
    y "Что?"

# game/script.rpy:3387
translate russia workBonusMenu_d5217612:

    # aM "It's weird, but...{w} It's still 50 bucks."
    aM "Это странно...{w} Но это 50 баксов как-никак."

# game/script.rpy:3391
translate russia workBonusMenu_30be5a73:

    # y "Why couldn't it be something normal..."
    y "Почему это не может быть чем-то нормальным..."

# game/script.rpy:3392
translate russia workBonusMenu_ad007f89:

    # y "Fine, go for it. We'll need the money and just never speak of it again."
    y "Ладно, действуйте. Нам понадобятся деньги, мы просто никогда больше не будем об этом говорить."

# game/script.rpy:3393
translate russia workBonusMenu_c2ca5c15:

    # aM "Okay~..."
    aM "Окей~..."

# game/script.rpy:3400
translate russia workBonusMenu_0be5ecda:

    # y "Alex that's super weird. Get out of there now."
    y "Алекс, это очень странно. Убирайся оттуда сейчас же."

# game/script.rpy:3401
translate russia workBonusMenu_5d6302f6:

    # aM "No feet smelling?"
    aM "Никаких нюханий?"

# game/script.rpy:3402
translate russia workBonusMenu_c20d9d5d:

    # y "No feet smelling."
    y "Никаких нюханий."

# game/script.rpy:3405
translate russia workBonusMenu_d1635291:

    # aM "What is... 'twerking'?"
    aM "Что такое... 'тверк'?"

# game/script.rpy:3406
translate russia workBonusMenu_ac17dec1_3:

    # y "What?"
    y "Что?"

# game/script.rpy:3407
translate russia workBonusMenu_53cce76b:

    # aM "I've been asked to do some twerking for a diplomatic missions between the gangs."
    aM "Меня попросили потверкать для дипломатической миссии между бандами."

# game/script.rpy:3411
translate russia workBonusMenu_04fb88f0:

    # y "I have no idea what it is, but I'm sure you can look it up online."
    y "Я понятия не имею, что это такое. Но уверен, что ты найдёшь что-нибудь в Интернете."

# game/script.rpy:3412
translate russia workBonusMenu_8b797804:

    # y "So do it."
    y "Так что вперёд."

# game/script.rpy:3413
translate russia workBonusMenu_d746607d:

    # aM "Aw... okay...~"
    aM "Оу... ладно...~"

# game/script.rpy:3421
translate russia workBonusMenu_7c15a18b:

    # y "Twerking...? That sounds painful. Best refuse."
    y "Тверк...? Звучит болезненно. Лучшие отказать."

# game/script.rpy:3422
translate russia workBonusMenu_73e3c6db:

    # a "All right."
    a "Хорошо."

# game/script.rpy:3425
translate russia workBonusMenu_28261612:

    # aM "You know those buttjobs we've been practising?"
    aM "Помнишь, что мы практиковали с моей задницей?"

# game/script.rpy:3426
translate russia workBonusMenu_2b3e284d:

    # y "Yeees...?"
    y "Дааа...?"

# game/script.rpy:3427
translate russia workBonusMenu_aa9ae938:

    # aM "Someone here is willing to pay for one."
    aM "Кое-кто готов заплатить за это."

# game/script.rpy:3428
translate russia workBonusMenu_4ad2fe2d:

    # y "Really? How much is he offering?"
    y "Правда? Сколько он предлагает?"

# game/script.rpy:3429
translate russia workBonusMenu_28c189e5:

    # aM "A hundred bucks."
    aM "100 баксов."

# game/script.rpy:3433
translate russia workBonusMenu_e603f4a2_1:

    # y "Money for lying down and doing nothing. Go for it!"
    y "Деньги за то, чтобы лечь и ничего не делать. Вперёд!"

# game/script.rpy:3434
translate russia workBonusMenu_38f1a7cf:

    # aM "But... for a stranger...? I'll do it [playerName], but I don't like it!"
    aM "Но... для незнакомца...? Я сделаю это [playerName], но мне это не нравится!"

# game/script.rpy:3435
translate russia workBonusMenu_3e931dec:

    # aM "......................"
    aM "......................"

# game/script.rpy:3436
translate russia workBonusMenu_5dbadf11:

    # aM "Okay maybe I like it a little!{w} But just a little bit!"
    aM "Ладно, может быть, мне это немного и нравится!{w} Но совсем чуть-чуть!"

# game/script.rpy:3447
translate russia workBonusMenu_2b7dab5d:

    # y "Buttjobs are a rare treasure, not meant to be given out all willy nilly!"
    y "Твоя попка - это редкое сокровище, оно не предназначено для всех подряд!"

# game/script.rpy:3448
translate russia workBonusMenu_a2389b69:

    # a "{b}*Giggle*{/b} You said willy!"
    a "{b}*Хихикает*{/b} Что правда, то правда!"

# game/script.rpy:3449
translate russia workBonusMenu_cf8849f9:

    # y "Turn him down, Alex. We'll make money some other way."
    y "Откажи ему, Алекс. Мы заработаем деньги другим способом."

# game/script.rpy:3452
translate russia workBonusMenu_f3be3149:

    # aM "I get to be a waitress!"
    aM "Я стану официанткой!"

# game/script.rpy:3453
translate russia workBonusMenu_c43f77c7:

    # y "You already are a waitress."
    y "Ты уже работаешь официанткой."

# game/script.rpy:3454
translate russia workBonusMenu_c82e1d4f:

    # aM "But now I get to do it at a party!"
    aM "Но теперь я буду работать на вечеринке!"

# game/script.rpy:3455
translate russia workBonusMenu_77618331:

    # aM "Hand out cocktails, cigarettes and snacks!"
    aM "Раздавать коктейли, сигареты и закуски!"

# game/script.rpy:3456
translate russia workBonusMenu_8a3117c9:

    # y "Okay.... what's the catch?"
    y "Окей.... а в чём подвох?"

# game/script.rpy:3457
translate russia workBonusMenu_72b29711:

    # aM "Oh... right, I have to do it naked."
    aM "Охх... точно, я буду делать это голой."

# game/script.rpy:3463
translate russia workBonusMenu_85fc9225:

    # y "Hot. I'd say, do it."
    y "Жарко. Я не против, вперёд."

# game/script.rpy:3464
translate russia workBonusMenu_ebdcdd79:

    # aM "I don't get anything out of it really, except a reputation boost."
    aM "Я не получу за это ничего, кроме повышения репутации."

# game/script.rpy:3465
translate russia workBonusMenu_9d0436af:

    # y "That's worth it. Go for it."
    y "Это того стоит. Действуй."

# game/script.rpy:3471
translate russia workBonusMenu_7ca26e7a:

    # a "Sigars! Cigarettes!"
    a "Сигары! Сигареты!"

# game/script.rpy:3472
translate russia workBonusMenu_794bcc77:

    # "Gangster" "Hey hottie... are you on the menu?"
    "Гангстер" "Эй, красотка... А тебя можно заказать?"

# game/script.rpy:3473
translate russia workBonusMenu_58577f6c:

    # a "Tony is that you? How's your girlfriend?"
    a "Тони, это ты? Как там твоя девушка?"

# game/script.rpy:3474
translate russia workBonusMenu_fa310edf:

    # "Gangster" "A-Alex?! Er.. you never saw me here, okay?"
    "Гангстер" "А-Алекс?! Эм... ты никогда меня здесь не видела, хорошо?"

# game/script.rpy:3485
translate russia workBonusMenu_cce20a9b:

    # y "Naked waitresses?!"
    y "Голая официантка?!"

# game/script.rpy:3486
translate russia workBonusMenu_6ba420a9:

    # y "How sleezy!"
    y "Как подло!"

# game/script.rpy:3487
translate russia workBonusMenu_8740b2b4:

    # aM "But [playerName]...? Aren't we-..."
    aM "Но [playerName]...? Разве нам не-..."

# game/script.rpy:3488
translate russia workBonusMenu_1299c7d2:

    # y "The answer is no."
    y "Мой ответ нет."

# game/script.rpy:3491
translate russia workBonusMenu_e5a29ff4:

    # aM "Stuffed mouths don't talk."
    aM "С набитым ртом не разговаривают."

# game/script.rpy:3492
translate russia workBonusMenu_b5732a78:

    # y "....................."
    y "....................."

# game/script.rpy:3493
translate russia workBonusMenu_6fad1870:

    # y "What are you, a philosopher?"
    y "Ты кто, философ?"

# game/script.rpy:3494
translate russia workBonusMenu_0a24eeb5:

    # aM "I heard some bigshots say that. They're looking for some girls to give them blowjobs as they talk about their upcoming heist."
    aM "Я слышала, как кто-то сверху говорил так. Они ищут девушек, которые будут делать им минет, пока они будут обсуждать предстоящие ограбление."

# game/script.rpy:3495
translate russia workBonusMenu_fb1029f8:

    # y "Oooh..."
    y "Оохх..."

# game/script.rpy:3501
translate russia workBonusMenu_be33b7cf:

    # y "Get into that meeting. If you have to suck some dick for it, do it."
    y "Попади на эту встречу. Даже если для этого нужно отсосать у кого-то."

# game/script.rpy:3502
translate russia workBonusMenu_cda9cedb:

    # aM "Are you sure...?"
    aM "Ты уверен...?"

# game/script.rpy:3503
translate russia workBonusMenu_0ee77766:

    # y "Gaining valuable intel in return a 5 minutes blowjob? I'd say that's worth it."
    y "Ценные сведения в обмен на 5-минутный минет? Думаю, что это того стоит."

# game/script.rpy:3504
translate russia workBonusMenu_23874321:

    # aM "If you say so..."
    aM "Как скажешь..."

# game/script.rpy:3509
translate russia workBonusMenu_3c6c52b4:

    # "Gangster" "That's a good girl, Alex."
    "Гангстер" "Хорошая девочка, Алекс."

# game/script.rpy:3510
translate russia workBonusMenu_1f72a48a:

    # "Gangster" "Best keep this up. We've got loads more of jummy cum for you~..."
    "Гангстер" "Продолжай в том же духе. У нас ещё много вкусной спермы для тебя~..."

# game/script.rpy:3511
translate russia workBonusMenu_5758066b:

    # a "........................."
    a "........................."

# game/script.rpy:3523
translate russia workBonusMenu_600befb5:

    # y "It can't be that big of a heist if they let sluts into their secret meetings. Don't do it."
    y "Это не может быть таким уж большим ограблением, раз они пускают шлюх на тайные встречи. Не стоит этого делать."

# game/script.rpy:3524
translate russia workBonusMenu_cd8fed92:

    # aM "Phew~... Okay!"
    aM "Фух~... Хорошо!"

# game/script.rpy:3527
translate russia workBonusMenu_b0f8508d_1:

    # "EVENT NOT VISIBLE TO PLAYER"
    "СОБЫТИЕ НЕ ВИДНО ИГРОКУ"

# game/script.rpy:3529
translate russia workBonusMenu_123ca587:

    # aM "Boobies on display!"
    aM "Сиськи на показ!"

# game/script.rpy:3530
translate russia workBonusMenu_d1642ce7:

    # y "Huh?"
    y "Хух?"

# game/script.rpy:3531
translate russia workBonusMenu_9b76bbc3:

    # aM "It's hot and some of the other girls are also doing it."
    aM "Это горячо, и некоторые другие девушки тоже так делают."

# game/script.rpy:3532
translate russia workBonusMenu_1877f2cb:

    # aM "I've been dared to take off my top and flash my tits for a bit of money and intel."
    aM "Мне предложили снять топ и показать свои сиськи за небольшие деньги и информацию."

# game/script.rpy:3536
translate russia workBonusMenu_408734f9_1:

    # y "If you think it will help, go for it."
    y "Если ты считаешь, что это поможет, действуй."

# game/script.rpy:3537
translate russia workBonusMenu_d52ed590:

    # aM "Bouncing boobies on display!"
    aM "Прыгающие сиськи!"

# game/script.rpy:3544
translate russia workBonusMenu_ec8af175:

    # a "Bounce bounce bounce!"
    a "Прыг-скок прыг-скок!"

# game/script.rpy:3545
translate russia workBonusMenu_ee741be6:

    # "You hear a crowd of cheering gangsters in the backgrounds."
    "На заднем плане слышны возгласы бандитов."

# game/script.rpy:3562
translate russia workBonusMenu_3e07cf67:

    # y "Seeing a good pair of tits is like having a fine wine."
    y "Увидеть хорошую пару сисек - это как выпить хорошего вина."

# game/script.rpy:3563
translate russia workBonusMenu_2a67dbc0:

    # y "You can't just whip them out whenever!"
    y "Ты не можешь просто взять и трясти своими сиськами когда тебе вздумается!"

# game/script.rpy:3564
translate russia workBonusMenu_bfd22ea4:

    # a "So... No boobies on display then?"
    a "Значит... никаких сисек?"

# game/script.rpy:3565
translate russia workBonusMenu_ca96356c:

    # y "Not today!"
    y "Никаких сисек!"

# game/script.rpy:5153
translate russia statusSetSup_9d1ae406:

    # "Set Sam's nanobot suppression levels."
    "Установите уровень подавления наноботов для Сэм."

# game/script.rpy:5157
translate russia statusSetSup_5951c8f5:

    # y "Make sure to keep your nanobots suppressed as much as possible."
    y "Убедись, что твои наноботы будут подавлены настолько, насколько это возможно."

# game/script.rpy:5158
translate russia statusSetSup_93490517:

    # sM "Understood."
    sM "Поняла."

# game/script.rpy:5162
translate russia statusSetSup_f8fad612:

    # y "Keep the nanobots suppressed, but don't overdo it."
    y "Подави своих наноботов, но не переусердствуй."

# game/script.rpy:5163
translate russia statusSetSup_2e033cb5:

    # sM "Okay, got it."
    sM "Хорошо, поняла."

# game/script.rpy:5167
translate russia statusSetSup_7b1a98a6:

    # y "I want to up your nanobot levels a bit. Go easy on the nanobot suppression."
    y "Я хочу немного повысить твой нано-уровень. Полегче с подавлением наноботов."

# game/script.rpy:5168
translate russia statusSetSup_dbf013db:

    # sM "Okay if you're sure."
    sM "Хорошо, если ты уверен."

# game/script.rpy:5171
translate russia statusSetSup_cc7b1e61:

    # "Your spies nano levels will slowly rise over time."
    "Нано-уровень шпионок будет со временем медленно повышаться."

# game/script.rpy:5172
translate russia statusSetSup_f9eb9899:

    # "High nano levels mean your spies will more easily agree to doing sexual actions and will have less of a mood penalty for all actions. However, spies may become more rebellious, steal or even disappear for a few days."
    "Высокий нано-уровень означает, что ваши шпионы будут легче соглашаться на совершение сексуальных действий, штраф за настроение при любых действиях также будут меньше. Однако шпионки могут стать более непослушными, украсть или даже исчезнуть на несколько дней."

# game/script.rpy:5173
translate russia statusSetSup_3cadc971:

    # "Low nano levels means that your spy will make a more steady amount of money, but will take greater mood penalties when asked to preform sex or illigal acts."
    "Низкий нано-уровень означает, что ваши шпионы будет зарабатывать более стабильно. Штрафы за настроение, когда их попросят совершить сексуальные или противозаконные действия, будут больше."

# game/script.rpy:5174
translate russia statusSetSup_42609b72:

    # "Medium nano levels is a steady balance between the two."
    "Средний нано-уровень, это устойчивый баланс между этими двумя показателями."

# game/script.rpy:5175
translate russia statusSetSup_50f5ef9a:

    # "Nano levels can be reduced by having your spies preform sexual acts. Or they can be boosted by injecting them with more nanobots. A spy with low suppression will increase her nanobot levels much faster than one with high suppression as well."
    "Нано-уровень можно снизить, если заставить ваших шпионов совершать сексуальные действия. Так же нано-уровень можно усилить, введя в шпионов больше наноботов. Шпионка с низким уровнем подавления будет увеличивать нано-уровень гораздо быстрее, чем шпионка с высоким уровнем подавления."

# game/script.rpy:5178
translate russia statusSetSup_cdf2984f:

    # "Set Clover's nanobot suppression levels."
    "Установите уровень подавления наноботов для Кловер."

# game/script.rpy:5182
translate russia statusSetSup_5951c8f5_1:

    # y "Make sure to keep your nanobots suppressed as much as possible."
    y "Убедись, что твои наноботы будут подавлены настолько, насколько это возможно."

# game/script.rpy:5183
translate russia statusSetSup_edc3fddc:

    # cM "Get my naughty on and suppress the nanobots. Got it."
    cM "Возбудиться и подавить наноботов. Поняла."

# game/script.rpy:5187
translate russia statusSetSup_f8fad612_1:

    # y "Keep the nanobots suppressed, but don't overdo it."
    y "Подави своих наноботов, но не переусердствуй."

# game/script.rpy:5188
translate russia statusSetSup_373e6867:

    # cM "Okay, I can do that."
    cM "Хорошо, я могу."

# game/script.rpy:5192
translate russia statusSetSup_7b1a98a6_1:

    # y "I want to up your nanobot levels a bit. Go easy on the nanobot suppression."
    y "Я хочу немного повысить твой нано-уровень. Полегче с подавлением наноботов."

# game/script.rpy:5193
translate russia statusSetSup_f0c85779:

    # cM "Risky, but all right."
    cM "Рискованно, но ладно."

# game/script.rpy:5196
translate russia statusSetSup_cc7b1e61_1:

    # "Your spies nano levels will slowly rise over time."
    "Нано-уровень шпионок будет со временем медленно повышаться."

# game/script.rpy:5197
translate russia statusSetSup_f9eb9899_1:

    # "High nano levels mean your spies will more easily agree to doing sexual actions and will have less of a mood penalty for all actions. However, spies may become more rebellious, steal or even disappear for a few days."
    "Высокий нано-уровень означает, что ваши шпионы будут легче соглашаться на совершение сексуальных действий, штраф за настроение при любых действиях также будут меньше. Однако шпионки могут стать более непослушными, украсть или даже исчезнуть на несколько дней."

# game/script.rpy:5198
translate russia statusSetSup_3cadc971_1:

    # "Low nano levels means that your spy will make a more steady amount of money, but will take greater mood penalties when asked to preform sex or illigal acts."
    "Низкий нано-уровень означает, что ваши шпионы будет зарабатывать более стабильно. Штрафы за настроение, когда их попросят совершить сексуальные или противозаконные действия, будут больше."

# game/script.rpy:5199
translate russia statusSetSup_42609b72_1:

    # "Medium nano levels is a steady balance between the two."
    "Средний нано-уровень, это устойчивый баланс между этими двумя показателями."

# game/script.rpy:5200
translate russia statusSetSup_8cd84526:

    # "Nano levels can be reduced by having your spies preform sexual acts. Or they can be boosted by injecting them with more nanobots. A spy with low suppression will increase her nanobot levels much faster than one with high suppression."
    "Нано-уровень можно снизить, если заставить ваших шпионов совершать сексуальные действия. Так же нано-уровень можно усилить, введя в шпионов больше наноботов. Шпионка с низким уровнем подавления будет увеличивать нано-уровень гораздо быстрее, чем шпионка с высоким уровнем подавления."

# game/script.rpy:5203
translate russia statusSetSup_9fd8fdea:

    # "Set Alex's nanobot suppression levels."
    "Установите уровень подавления наноботов для Алекс."

# game/script.rpy:5207
translate russia statusSetSup_5951c8f5_2:

    # y "Make sure to keep your nanobots suppressed as much as possible."
    y "Убедись, что твои наноботы будут подавлены настолько, насколько это возможно."

# game/script.rpy:5208
translate russia statusSetSup_8e95353a:

    # aM "Oh, I gets to have some fuuuun~...!."
    aM "Ох, похоже у меня намечается немного весееееелия~...!"

# game/script.rpy:5212
translate russia statusSetSup_f8fad612_2:

    # y "Keep the nanobots suppressed, but don't overdo it."
    y "Подави своих наноботов, но не переусердствуй."

# game/script.rpy:5213
translate russia statusSetSup_927fcf44:

    # aM "Okidokie."
    aM "Оки-доки."

# game/script.rpy:5217
translate russia statusSetSup_7b1a98a6_2:

    # y "I want to up your nanobot levels a bit. Go easy on the nanobot suppression."
    y "Я хочу немного повысить твой нано-уровень. Полегче с подавлением наноботов."

# game/script.rpy:5218
translate russia statusSetSup_d42c948d:

    # aM "Hm okay, but only if you're really really sure."
    aM "Хм, хорошо, если ты действительно уверены."

# game/script.rpy:5221
translate russia statusSetSup_cc7b1e61_2:

    # "Your spies nano levels will slowly rise over time."
    "Нано-уровень шпионок будет со временем медленно повышаться."

# game/script.rpy:5222
translate russia statusSetSup_f9eb9899_2:

    # "High nano levels mean your spies will more easily agree to doing sexual actions and will have less of a mood penalty for all actions. However, spies may become more rebellious, steal or even disappear for a few days."
    "Высокий нано-уровень означает, что ваши шпионы будут легче соглашаться на совершение сексуальных действий, штраф за настроение при любых действиях также будут меньше. Однако шпионки могут стать более непослушными, украсть или даже исчезнуть на несколько дней."

# game/script.rpy:5223
translate russia statusSetSup_3cadc971_2:

    # "Low nano levels means that your spy will make a more steady amount of money, but will take greater mood penalties when asked to preform sex or illigal acts."
    "Низкий нано-уровень означает, что ваши шпионы будет зарабатывать более стабильно. Штрафы за настроение, когда их попросят совершить сексуальные или противозаконные действия, будут больше."

# game/script.rpy:5224
translate russia statusSetSup_42609b72_2:

    # "Medium nano levels is a steady balance between the two."
    "Средний нано-уровень, это устойчивый баланс между этими двумя показателями."

# game/script.rpy:5225
translate russia statusSetSup_8cd84526_1:

    # "Nano levels can be reduced by having your spies preform sexual acts. Or they can be boosted by injecting them with more nanobots. A spy with low suppression will increase her nanobot levels much faster than one with high suppression."
    "Нано-уровень можно снизить, если заставить ваших шпионов совершать сексуальные действия. Так же нано-уровень можно усилить, введя в шпионов больше наноботов. Шпионка с низким уровнем подавления будет увеличивать нано-уровень гораздо быстрее, чем шпионка с высоким уровнем подавления."

# game/script.rpy:5365
translate russia mallNotAvailable_20cd6e9e:

    # "Shopping not yet available. Work in progress."
    "Покупки пока недоступны. В процессе."

# game/script.rpy:7976
translate russia purchaseButton_0fa6d9a9:

    # y "I don't have enough money to buy this."
    y "У меня недостаточно денег, чтобы купить это."

# game/script.rpy:12197
translate russia shopNoCash_5da029c9:

    # y "I don't have enough money for that."
    y "У меня не хватает на это денег."

# game/script.rpy:13322
translate russia jobTrickOrTreat_00e1b435:

    # sM "Trick or Treat!"
    sM "Кошелёк или жизнь!"

# game/script.rpy:13327
translate russia jobTrickOrTreat_ae6ded83:

    # cM "Trick or Treat!"
    cM "Сладость или гадость!"

# game/script.rpy:13332
translate russia jobTrickOrTreat_c577c79c:

    # aM "Trick or Treat!"
    aM "Кошелёк или жизнь!"

translate russia strings:

    # game/script.rpy:1569
    old "Skip to the next landgrab"
    new "Перейти к следующему захвату территории"

    # game/script.rpy:1569
    old "Never mind"
    new "Неважно"

    # game/script.rpy:2211
    old "Send 5 agents"
    new "Отправить 5 агентов"

    # game/script.rpy:2211
    old "Send 10 agents"
    new "Отправить 10 агентов"

    # game/script.rpy:2211
    old "Send 20 agents"
    new "Отправить 20 агентов"

    # game/script.rpy:2211
    old "Back"
    new "Назад"

    # game/script.rpy:2478
    old "Donate food ([resourceFood])"
    new "Пожертвовать продукты ([resourceFood])"

    # game/script.rpy:2478
    old "Donate water ([resourceWater])"
    new "Пожертвовать воду([resourceWater])"

    # game/script.rpy:2478
    old "Donate toys ([resourceToys])"
    new "Пожертвовать игрушки ([resourceToys])"

    # game/script.rpy:2478
    old "Donate clothes ([resourceClothes])"
    new "Пожертвовать одежду ([resourceClothes])"

    # game/script.rpy:2478
    old "Donate tools ([resourceTools])"
    new "Пожертвовать инструменты ([resourceTools])"

    # game/script.rpy:2478
    old "Finish donating"
    new "Закончить пожертвование"

    # game/script.rpy:2758
    old "Show them"
    new "Показать им"

    # game/script.rpy:2758
    old "Don't show them"
    new "Не показывать им"

    # game/script.rpy:2827
    old "Strip for money"
    new "Раздеться за деньги"

    # game/script.rpy:2827
    old "Refuse"
    new "Отказать"

    # game/script.rpy:2896
    old "Kiss the girl"
    new "Поцеловать девушку"

    # game/script.rpy:2896
    old "Reject"
    new "Отказать"

    # game/script.rpy:2923
    old "What are you waiting for?"
    new "Чего ты ждёшь?"

    # game/script.rpy:2923
    old "Turn him down"
    new "Отказать ему"

    # game/script.rpy:2970
    old "Hell yeah!"
    new "Чёрт возьми, ДА!"

    # game/script.rpy:3011
    old "Who cares, go in"
    new "Кого еб*т, заходи"

    # game/script.rpy:3011
    old "Don't follow"
    new "Не следовать"

    # game/script.rpy:3034
    old "Go for it"
    new "Действуй"

    # game/script.rpy:3083
    old "Do it"
    new "Вперёд"

    # game/script.rpy:3083
    old "Don't do it"
    new "Не делай этого"

    # game/script.rpy:3117
    old "Agree to photoshoot"
    new "Согласиться на фотосессию"

    # game/script.rpy:3164
    old "Don't change"
    new "Не (переодеваться)"

    # game/script.rpy:3164
    old "Change"
    new "(Переодеться)"

    # game/script.rpy:3203
    old "Yes"
    new "Да"

    # game/script.rpy:3245
    old "Do it!"
    new "Давай, сделай это!"

    # game/script.rpy:3267
    old "Don't"
    new "Нет, не делай этого!"

    # game/script.rpy:3388
    old "Agree"
    new "Согласен"

    # game/script.rpy:3496
    old "Give them a blowjob"
    new "Сделай им минет"

    # game/script.rpy:3496
    old "Don't give them a blowjob"
    new "Не делай им минет"

    # game/script.rpy:5154
    old "High Suppression"
    new "Высокий уровень подавления"

    # game/script.rpy:5154
    old "Medium Suppression"
    new "Средний уровень подавления"

    # game/script.rpy:5154
    old "Low Suppression"
    new "Низкий уровень подавления"

    # game/script.rpy:5154
    old "Info"
    new "Инфо"

# TODO: Translation updated at 2021-10-17 19:03

# game/script.rpy:2430
translate russia landgrabPlanning_1e7bdbc2:

    # "Muffy Peprich is now available for capture. Send your spies on a mission to The Carnival to capture her."
    "Маффи Пеприч теперь доступна для захвата. Отправьте своих шпионов на Карнавал, чтобы захватить её."

translate russia strings:

    # game/script.rpy:13343
    old "Gadgets"
    new "Гаджеты"

translate russia strings:

    # game/script.rpy:970
    old "{font=fonts/freshmarker.ttf}Lore{/font}"
    new "{font=fonts/freshmarker.ttf}Лор{/font}"

translate russia strings:

    # game/script.rpy:967
    old "{font=fonts/freshmarker.ttf}Quests{/font}"
    new "{font=fonts/freshmarker.ttf}Квесты{/font}"

    # game/script.rpy:972
    old "{font=fonts/freshmarker.ttf}Status{/font}"
    new "{font=fonts/freshmarker.ttf}Статус{/font}"

    # game/script.rpy:989
    old "{font=fonts/freshmarker.ttf}Gadgets{/font}"
    new "{font=fonts/freshmarker.ttf}Гаджеты{/font}"

    # game/script.rpy:992
    old "{font=fonts/freshmarker.ttf}Outfits{/font}"
    new "{font=fonts/freshmarker.ttf}Одежда{/font}"

    # game/script.rpy:994
    old "{font=fonts/freshmarker.ttf}Items{/font}"
    new "{font=fonts/freshmarker.ttf}Вещи{/font}"
# TODO: Translation updated at 2022-01-01 19:06

translate russia strings:

    # game/script.rpy:337
    old "Gained [randIntel] INT"
    new ""

    # game/script.rpy:346
    old "Gained $[randMoney]"
    new ""

    # game/script.rpy:355
    old "Lore gathered"
    new ""

    # game/script.rpy:364
    old "Rep gained"
    new ""

    # game/script.rpy:373
    old "Item(s) gained"
    new ""

    # game/script.rpy:382
    old "Crew gained"
    new ""

    # game/script.rpy:393
    old "Quest photo added"
    new ""

    # game/script.rpy:614
    old "Spy: [greenName] \nLocation: [greenLocation] \nExpertise: Support"
    new ""

    # game/script.rpy:624
    old "Spy: [redName] \nLocation: [redLocation] \nExpertise: Distraction"
    new ""

    # game/script.rpy:634
    old "Spy: [yellowName] \nLocation: [yellowLocation] \nExpertise: Infiltration"
    new ""

    # game/script.rpy:869
    old "{font=fonts/freshMarker.ttf}{size=-4}Noteriety{/size}{/font}"
    new ""

    # game/script.rpy:880
    old "{font=fonts/freshMarker.ttf}{size=-4}Cash{/size}{/font}"
    new ""

    # game/script.rpy:891
    old "{font=fonts/freshMarker.ttf}{size=-4}Intel{/size}{/font}"
    new ""

    # game/script.rpy:903
    old "{font=fonts/freshMarker.ttf}{size=-4}Agents{/size}{/font}"
    new ""

    # game/script.rpy:908
    old "{font=fonts/freshMarker.ttf}{size=-4}Day: [daysPlayed]{/size}{/font}"
    new ""

    # game/script.rpy:935
    old "Sam{#tbs}"
    new ""

    # game/script.rpy:938
    old "???{#tbs}"
    new ""

    # game/script.rpy:941
    old "Clover{#tbs}"
    new ""

    # game/script.rpy:947
    old "Alex{#tbs}"
    new ""

    # game/script.rpy:1101
    old "{font=fonts/freshmarker.ttf}{size=+18}Aces{/size}{/font}"
    new ""

    # game/script.rpy:1153
    old "{font=fonts/freshmarker.ttf}{size=+12}Drift Punk{/size}{/font}"
    new ""

    # game/script.rpy:1201
    old "{font=fonts/freshmarker.ttf}{size=+14}Outsiders{/size}{/font}"
    new ""

    # game/script.rpy:2163
    old "{font=fonts/freshmarker.ttf}{size=+18}Drift Punk{/size}{/font}"
    new ""

    # game/script.rpy:2182
    old "{font=fonts/freshmarker.ttf}{size=+18}Outsiders{/size}{/font}"
    new ""

    # game/script.rpy:4907
    old "{font=fonts/freshmarker.ttf}{size=+5}Angelic{/size}{/font}"
    new ""

    # game/script.rpy:4909
    old "{font=fonts/freshmarker.ttf}{size=+5}Heroic{/size}{/font}"
    new ""

    # game/script.rpy:4911
    old "{font=fonts/freshmarker.ttf}{size=+5}Good{/size}{/font}"
    new ""

    # game/script.rpy:4913
    old "{font=fonts/freshmarker.ttf}{size=+5}Dubious{/size}{/font}"
    new ""

    # game/script.rpy:4915
    old "{font=fonts/freshmarker.ttf}{size=+5}Moralless{/size}{/font}"
    new ""

    # game/script.rpy:4917
    old "{font=fonts/freshmarker.ttf}{size=+5}Evil{/size}{/font}"
    new ""

    # game/script.rpy:4973
    old "{font=fonts/freshmarker.ttf}{size=+40}SAM{/size}{/font}"
    new ""

    # game/script.rpy:4975
    old "{font=fonts/freshmarker.ttf}Healthy{/font}"
    new ""

    # game/script.rpy:4978
    old "{font=fonts/freshmarker.ttf}Hurt{/font}"
    new ""

    # game/script.rpy:4981
    old "{font=fonts/freshmarker.ttf}}Badly hurt{/font}"
    new ""

    # game/script.rpy:4985
    old "{font=fonts/freshmarker.ttf}Prudish{/font}"
    new ""

    # game/script.rpy:4987
    old "{font=fonts/freshmarker.ttf}Flirty{/font}"
    new ""

    # game/script.rpy:4989
    old "{font=fonts/freshmarker.ttf}}Promiscuous{/font}"
    new ""

    # game/script.rpy:4991
    old "{font=fonts/freshmarker.ttf}}Slutty{/font}"
    new ""

    # game/script.rpy:4993
    old "{font=fonts/freshmarker.ttf}}Whorish{/font}"
    new ""

    # game/script.rpy:4996
    old "{font=fonts/freshmarker.ttf}Depressed{/font}"
    new ""

    # game/script.rpy:4998
    old "{font=fonts/freshmarker.ttf}Very low{/font}"
    new ""

    # game/script.rpy:5000
    old "{font=fonts/freshmarker.ttf}Low{/font}"
    new ""

    # game/script.rpy:5002
    old "{font=fonts/freshmarker.ttf}Normal{/font}"
    new ""

    # game/script.rpy:5004
    old "{font=fonts/freshmarker.ttf}High{/font}"
    new ""

    # game/script.rpy:5006
    old "{font=fonts/freshmarker.ttf}Very high{/font}"
    new ""

    # game/script.rpy:5017
    old "{font=fonts/freshmarker.ttf}Medium{/font}"
    new ""

    # game/script.rpy:5031
    old "{font=fonts/freshmarker.ttf}{size=+40}CLOVER{/size}{/font}"
    new ""

    # game/script.rpy:5090
    old "{font=fonts/freshmarker.ttf}{size=+40}ALEX{/size}{/font}"
    new ""

    # game/script.rpy:12353
    old "{font=fonts/freshMarker.ttf}ACES{/font}"
    new ""

    # game/script.rpy:12355
    old "{font=fonts/freshMarker.ttf}DRIFT PUNK{/font}"
    new ""

    # game/script.rpy:12357
    old "{font=fonts/freshMarker.ttf}OUTSIDERS{/font}"
    new ""

    # game/script.rpy:12359
    old "{font=fonts/freshMarker.ttf}LES EPINES{/font}"
    new ""

    # game/script.rpy:12361
    old "{font=fonts/freshMarker.ttf}HUNTERS{/font}"
    new ""

    # game/script.rpy:12363
    old "{font=fonts/freshMarker.ttf}GLIMMERS{/font}"
    new ""

    # game/script.rpy:12365
    old "{font=fonts/freshMarker.ttf}EXCHANGE{/font}"
    new ""

    # game/script.rpy:12367
    old "{font=fonts/freshMarker.ttf}WOOHP{/font}"
    new ""

    # game/script.rpy:12369
    old "{font=fonts/freshMarker.ttf}MISC.{/font}"
    new ""

    # game/script.rpy:12372
    old "{font=fonts/freshMarker.ttf}{size=+34}ACES{/size}{/font}"
    new ""

    # game/script.rpy:12374
    old "{font=fonts/freshMarker.ttf}{size=+25}DRIFT PUNK{/size}{/font}"
    new ""

    # game/script.rpy:12376
    old "{font=fonts/freshMarker.ttf}{size=+30}OUTSIDERS{/size}{/font}"
    new ""

    # game/script.rpy:12378
    old "{font=fonts/freshMarker.ttf}{size=+25}LES EPINES{/size}{/font}"
    new ""

    # game/script.rpy:12380
    old "{font=fonts/freshMarker.ttf}{size=+30}HUNTERS{/size}{/font}"
    new ""

    # game/script.rpy:12382
    old "{font=fonts/freshMarker.ttf}{size=+30}GLIMMERS{/size}{/font}"
    new ""

    # game/script.rpy:12384
    old "{font=fonts/freshMarker.ttf}{size=+30}EXCHANGE{/size}{/font}"
    new ""

    # game/script.rpy:12386
    old "{font=fonts/freshMarker.ttf}{size=+36}WOOHP{/size}{/font}"
    new ""

    # game/script.rpy:12388
    old "{font=fonts/freshMarker.ttf}{size=+33}MISC.{/size}{/font}"
    new ""

    # game/script.rpy:12391
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 1{/size}{/font}"
    new ""

    # game/script.rpy:12394
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 2{/size}{/font}"
    new ""

    # game/script.rpy:12397
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 3{/size}{/font}"
    new ""

    # game/script.rpy:12400
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 4{/size}{/font}"
    new ""

    # game/script.rpy:12403
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 5{/size}{/font}"
    new ""

    # game/script.rpy:12406
    old "{font=fonts/freshMarker.ttf}{size=+3}Entry 6{/size}{/font}"
    new ""

