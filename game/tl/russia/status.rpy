# TODO: Translation updated at 2021-10-09 21:10

# game/status.rpy:13
translate russia skills_1ef9fb4c:

    # y "I better get busy exploring."
    y "Пора бы заняться исследованием."

# game/status.rpy:17
translate russia skills_31ca5265:

    # "Not yet in"
    "В разработке"

# game/status.rpy:26
translate russia status_e9d3e274:

    # y "I'll check this out later."
    y "Проверю это позже."

# game/status.rpy:48
translate russia lore_6ee1951e:

    # y "I don't have time for that right now."
    y "Сейчас у меня нет на это времени."

# game/status.rpy:57
translate russia itemsScreen_31ca5265:

    # "Not yet in"
    "В разработке"

# game/status.rpy:59
translate russia itemsScreen_df3de95a:

    # "Show items screen here. NOT IN YET"
    "Show items screen here. NOT IN YET"

# game/status.rpy:67
translate russia exploreMap_343cfe65:

    # y "I should probably send my spy out to bring back some intel first."
    y "Думаю сначала нужно отправить шпионку, чтобы она принесла кое-какие сведения."

# game/status.rpy:72
translate russia exploreMap_73971457:

    # y "I'll wait until tomorrow morning to explore the city."
    y "Подожду до завтрашнего утра, чтобы осмотреть город."

# game/status.rpy:119
translate russia quests_e9d3e274:

    # y "I'll check this out later."
    y "Проверю это позже."

# game/status.rpy:341
translate russia task2_3c18f6c6:

    # y "Okay [greenName]. Today I'm sending you to..."
    y "Ладно [greenName]. Сегодня я отправляю тебя..."

# game/status.rpy:342
translate russia task2_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:343
translate russia task2_d752491c:

    # y "[greenName]?"
    y "[greenName]?"

# game/status.rpy:345
translate russia task2_0665c773:

    # s "Hey [playerName]. Sorry I'm just feeling a bit distracted."
    s "Привет [playerName]. Извини, я просто немного задумалась."

# game/status.rpy:346
translate russia task2_2c996790:

    # y "It better not be the nanobots acting up again! You {i}'have'{/i} been suppressing them, right?"
    y "Надеюсь что это не наноботы на тебя так влияют! Ты же {i}'подавила'{/i} их сегодня, да?"

# game/status.rpy:347
translate russia task2_86be8d2d:

    # s g18 "Yeah... well sort of."
    s g18 "Да... ну, типа того."

# game/status.rpy:348
translate russia task2_5f07a172:

    # y "Sort of...?"
    y "Типа того...?"

# game/status.rpy:349
translate russia task2_c223a092:

    # s "Anyways, that's not why I was distracted. I was wondering if my friends are doing all right."
    s "В любом случае я задумалась не из-за этого. Мне интересно, всё ли в порядке у моих друзей."

# game/status.rpy:350
translate russia task2_664c7ab3:

    # y "The other spies?"
    y "Другие шпионки?"

# game/status.rpy:351
translate russia task2_4590d3aa:

    # s g38 "Uhuh. I haven't seen them in days and I'm worried for them..."
    s g38 "Угу. Я не видела их несколько дней. Я волнуюсь за них..."

# game/status.rpy:352
translate russia task2_389f9982:

    # s g14 "Are you sure we can't bring them back here?"
    s g14 "Ты уверен, что мы не можем привести их сюда?"

# game/status.rpy:353
translate russia task2_ca8c8953:

    # y "We could... But we'd risk them exposing our plans."
    y "Мы можем... Но мы рискуем, ведь они могут раскрыть наши планы."

# game/status.rpy:354
translate russia task2_2d308e10:

    # s g32 "Not if we break their nanobot control!"
    s g32 "Нет, если мы нарушим работу их наноботов!"

# game/status.rpy:355
translate russia task2_82822f89:

    # y "Well we could use an extra pair of hands around here. Fine, I'll set up a mission and you can go and find them."
    y "Ну, нам бы не помешала лишняя пара рук. Хорошо я подготовлю всё для мисси и ты сможешь пойти и найти их."

# game/status.rpy:356
translate russia task2_7e4379b0:

    # y "Any idea where they're at?"
    y "Есть идеи, где они могут быть?"

# game/status.rpy:357
translate russia task2_908252ce:

    # s g37 "Well... Clover is probably still at the school.{w} I don't know of Alex her whereabouts."
    s g37 "Ну... Кловер, вероятно, всё ещё в школе.{w} А где Алекс я не знаю."

# game/status.rpy:358
translate russia task2_44e42824:

    # y "We'll worry about her later. For now let's go for the blonde one."
    y "Займёмся ею позже. Пока давай найдём блондинку."

# game/status.rpy:360
translate russia task2_3b2bef4b:

    # y "I'll set up a mission and you can go after her."
    y "Я подготовлю миссию, а ты сможешь отправиться за ней."

# game/status.rpy:361
translate russia task2_98a8f8f5:

    # s g1 "Understood!"
    s g1 "Поняла!"

# game/status.rpy:401
translate russia task2_ad7688f9:

    # "Agent" "There's no sign of her. Should we keep looking?"
    "Агент" "Никаких следов. Нам продолжать поиск?"

# game/status.rpy:403
translate russia task2_1bc7a919:

    # "Agent" "We've spotted her! She has to be around here somewhere!"
    "Агент" "Мы нашли её следы! Она должна быть где-то здесь!"

# game/status.rpy:413
translate russia task2_19ada25d:

    # c r11 "Oh she's here all right~...."
    c r11 "Ох, она здесь, отлично~...."

# game/status.rpy:414
translate russia task2_252c932d:

    # c r23 "She might even be listening to us as we speak..."
    c r23 "Возможно, она даже подслушивает разговор..."

# game/status.rpy:415
translate russia task2_2e79719a:

    # c "...................."
    c "...................."

# game/status.rpy:416
translate russia task2_e0b46ce8:

    # c r10 "Stay on your guard. She'll be back."
    c r10 "Будьте начеку. Она скоро объявиться."

# game/status.rpy:417
translate russia task2_bed730d2:

    # "Agent" "Yes ma'am!"
    "Агент" "Да мэм!"

# game/status.rpy:428
translate russia task2_7d59a55f:

    # sM "I spotted her, [playerName]."
    sM "Я нашла её, [playerName]."

# game/status.rpy:429
translate russia task2_0922fb03:

    # y "Yeah I saw. Try getting her back, but stay on your guard."
    y "Да, я видел. Попробуй схватить её, но будь начеку."

# game/status.rpy:430
translate russia task2_ea181510:

    # sM "Right."
    sM "Конечно."

# game/status.rpy:465
translate russia task2_e4ceec1b:

    # c "You don't have to hide anymore... I know you're there."
    c "Хватит прятаться... Я знаю, что ты там."

# game/status.rpy:474
translate russia task2_8d2be2bf:

    # s "Clover, listen I've come to get you out of here."
    s "Кловер, послушай, я пришла, чтобы спасти тебя. "

# game/status.rpy:475
translate russia task2_44c21185:

    # c r22 "Oh... I certainly like to see you try."
    c r22 "Ох... хотела бы я посмотреть как у тебя это получиться."

# game/status.rpy:476
translate russia task2_1cb29714:

    # c "Guards!"
    c "Стража!"

# game/status.rpy:492
translate russia task2_d9a5559f:

    # c r12 "Deal with her."
    c r12 "Разберитесь с ней."

# game/status.rpy:522
translate russia task2_2d13b75c:

    # c "................"
    c "................"

# game/status.rpy:524
translate russia task2_7f589553:

    # s "Willing or not, you're coming with me, Clover!"
    s "Хочешь или нет, но ты пойдешь со мной, Кловер!"

# game/status.rpy:525
translate russia task2_6d49ed0d:

    # c "Fine, I'll deal with you myself..."
    c "Ладно, я сам с тобой разберусь..."

# game/status.rpy:526
translate russia task2_9e308b2a:

    # s g8 "Listen to me, I don't want to hurt you!"
    s g8 "Послушай, я не хочу причинить тебе боль!"

# game/status.rpy:527
translate russia task2_c2a41c6a:

    # y "Now's your chance, [greenName]. Knock her out and bring her back to the base."
    y "Это твой шанс, [greenName]. Выруби её и верни на базу."

# game/status.rpy:528
translate russia task2_10735e11:

    # s g29 "But she's my friend! I can't fight her!"
    s g29 "Но она моя подруга! Я не могу с ней сражаться!"

# game/status.rpy:529
translate russia task2_5757f912:

    # c r22 "Remember that one time you left your diary open and said all those embarrassing things?{w} I told everyone about that..."
    c r22 "Помнишь, когда ты оставила свой дневник на столе?{w} Я всё прочитала и всем всё этом рассказала..."

# game/status.rpy:530
translate russia task2_68ef665f:

    # s g28 "Oh that bitch....!"
    s g28 "Ах, ты сука...!"

# game/status.rpy:531
translate russia task2_04193e49:

    # y "[greenName] there are more Agents on their way. This is the only chance you'll get. Now knock out that blonde and that's an order!"
    y "[greenName], на подходе ещё больше Агентов. Это твой единственный шанс. Просто выруби эту блондинку, это приказ!"

# game/status.rpy:532
translate russia task2_4b69142c:

    # s "B-but...!"
    s "Н-но...!"

# game/status.rpy:544
translate russia task2_a51a4e7e:

    # s "I obey..."
    s "Т-так точно..."

# game/status.rpy:596
translate russia task2_b64245e0:

    # "Sam wrestles Clover to the ground with ferosity!"
    "Сэм жёстко кидает Кловер на пол!"

# game/status.rpy:597
translate russia task2_e7bd5ac4:

    # "She grabs her wrists and pins her to the ground."
    "Она хватает её запястья и прижимает к полу."

# game/status.rpy:598
translate russia task2_3accbe0c:

    # sM "Target subdued sir!"
    sM "Цель захвачена сэр! "

# game/status.rpy:599
translate russia task2_7e979830:

    # y "Awesome!"
    y "Отлично!"

# game/status.rpy:600
translate russia task2_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:601
translate russia task2_de231507:

    # y "Now kiss."
    y "Теперь целуй."

# game/status.rpy:602
translate russia task2_57f79a69:

    # sM "Yes sir."
    sM "Так точно."

# game/status.rpy:603
translate russia task2_df202442:

    # "Without hesitation Sam deeply kisses the blonde spy!"
    "Без колебаний Сэм целует шпионку в красном!"

# game/status.rpy:606
translate russia task2_ba7be20c:

    # c "Mhmp!!"
    c "Мфмп!!"

# game/status.rpy:607
translate russia task2_605b8e18:

    # c "Mphm~..."
    c "Мфмп~..."

# game/status.rpy:608
translate russia task2_d6e712c6:

    # c "......................"
    c "......................"

# game/status.rpy:623
translate russia task2_5cbf3310:

    # c "Sam...."
    c "Сэм..."

# game/status.rpy:624
translate russia task2_fcb4412e:

    # c "What happened....?"
    c "Что случилось...?"

# game/status.rpy:625
translate russia task2_e4fcdb85:

    # s "I don't know... I just..."
    s "Я не знаю... Я просто..."

# game/status.rpy:626
translate russia task2_092a8874:

    # y "Woah you actually did it! Damn [greenName] I didn't expect that from you."
    y "Ты действительно это сделала! Чёрт [greenName], не ожидал от тебя такого."

# game/status.rpy:627
translate russia task2_201979c6:

    # s g28 "Y-you...! It's not what it seems!"
    s g28 "Т-ты...! Это не то, чем кажется!"

# game/status.rpy:628
translate russia task2_9c069237:

    # y "Save it for later girls, you two need to get out of there now!"
    y "Оставьте это на потом, вам двоим нужно убираться оттуда сейчас же!"

# game/status.rpy:643
translate russia task2_a9ac52b0:

    # "Agent" "I sure love going on patrol!"
    "Агент" "Мне очень нравится патрулировать!"

# game/status.rpy:644
translate russia task2_57f9c0af:

    # "Agent" "Wait you're that intruder!"
    "Агент" "Стоп, вы ты самые злоумышленник!"

# game/status.rpy:645
translate russia task2_c0fa3489:

    # y "Get to the entrance of the school, I'll pull the two of you out of there!"
    y "Идите ко входу в школу, я вытащу вас оттуда!"

# game/status.rpy:648
translate russia task2_cd01d10f:

    # sM "On our way!"
    sM "Бежим!"

# game/status.rpy:682
translate russia task2_906674d3:

    # s "It looks clear."
    s "Тут чисто."

# game/status.rpy:683
translate russia task2_cffa2e20:

    # c "Hang on..."
    c "Подожди..."

# game/status.rpy:686
translate russia task2_d96542b9:

    # c "It looks clear."
    c "Вроде чисто."

# game/status.rpy:687
translate russia task2_b378f1a6:

    # a "Hang on..."
    a "Постой..."

# game/status.rpy:690
translate russia task2_6b967c6c:

    # a "It looks clear."
    a "Чисто."

# game/status.rpy:691
translate russia task2_f767c7cf:

    # s "Hang on..."
    s "Стой..."

# game/status.rpy:702
translate russia task2_0cbed71b:

    # "Agent" "I'm guarding this exit like a chump!"
    "Агент" "Стою и охраняю этот выход как дурак! "

# game/status.rpy:704
translate russia task2_e1bb11f5:

    # s "Woah!"
    s "Ого!"

# game/status.rpy:705
translate russia task2_f254eff0:

    # c "Yeah he's gonna be tough to slip past. We're gonna have to fight him."
    c "Да, мимо него будет трудно проскользнуть. Нам придётся сражаться с ним."

# game/status.rpy:706
translate russia task2_f14075c4:

    # c "I'm gonna find a more advantages spot. Use your compowder to call in backup and I'll help you fight it."
    c "Пойду найду более выгодную позицию. Используй коммуникатор, чтобы вызвать подкрепление. "

# game/status.rpy:708
translate russia task2_6bbc5d98:

    # c "Woah!"
    c "Ого!"

# game/status.rpy:709
translate russia task2_5d71356e:

    # a "Yeah he's gonna be tough to slip past. We're gonna have to fight him."
    a "Ага, мы тут явно не пройдём. Придётся сражаться с ним."

# game/status.rpy:710
translate russia task2_a12e81c3:

    # a "I'm gonna find a more advantages spot. Use your compowder to call in backup and I'll help you fight it."
    a "Пойду найду более выгодную позицию. Используй коммуникатор, чтобы вызвать подкрепление. "

# game/status.rpy:712
translate russia task2_65274cc2:

    # a "Woah!"
    a "Ого!"

# game/status.rpy:713
translate russia task2_d75eae06:

    # s "Yeah he's gonna be tough to slip past. We're gonna have to fight him."
    s "Похоже тут без боя не пройти."

# game/status.rpy:714
translate russia task2_e91b70fc:

    # s "I'm gonna find a more advantages spot. Use your compowder to call in backup and I'll help you fight it."
    s "Пойду найду более выгодную позицию. Используй коммуникатор, чтобы вызвать подкрепление. "

# game/status.rpy:755
translate russia task2_0c09e814:

    # s "We're back."
    s "Мы вернулись."

# game/status.rpy:756
translate russia task2_b043f768:

    # y "So you are..."
    y "И так..."

# game/status.rpy:760
translate russia task2_fd36e74f:

    # c "Ngh~...."
    c "Нгх~...."

# game/status.rpy:761
translate russia task2_6f1c54a8:

    # y "And this must be Clover."
    y "А это, должно быть, Кловер."

# game/status.rpy:762
translate russia task2_728e24f1:

    # c r30 "Wha-.... Who are you?"
    c r30 "Чт-.... Кто ты?"

# game/status.rpy:763
translate russia task2_62555e06:

    # s g11 "It's a long story..."
    s g11 "Это долгая история..."

# game/status.rpy:767
translate russia task2_23f3cb21:

    # "You quickly bring Clover up to speed."
    "Вы быстро вводите Кловер в курс дела."

# game/status.rpy:772
translate russia task2_0a3db6ab:

    # c "So like... It's just us left?"
    c "Типа... Остались только мы?"

# game/status.rpy:773
translate russia task2_0e7d2b5d:

    # s "Looks like it."
    s "Получается так."

# game/status.rpy:774
translate russia task2_e6f0edb0:

    # c r11 "That's like... so not cool. I hate working overtime. It gets in the way of my beauty sleep..."
    c r11 "Это типа... не круто. Я ненавижу работать сверхурочно. Это нарушает мой дневной сон..."

# game/status.rpy:775
translate russia task2_641d3323:

    # y "I think we've got more important things to worry about than your sleep schedule."
    y "Думаю, у нас есть вещи поважнее, чем твой режим сна."

# game/status.rpy:776
translate russia task2_0f0d66fe:

    # y "Like the fact that you're pumped full of nanobots and just being here is a risk to this operation."
    y "Например, тот факт, что ты накачана наноботами до краёв и просто находясь здесь - мы рискуем операцией."

# game/status.rpy:777
translate russia task2_82179cea:

    # c r38 "{b}*Scoffs*{/b} Oh please, Sam seemed to have...{w} {i}'fixed'{/i} that issue. I feel absolutely fine!"
    c r38 "{b}*Насмешка*{/b} Умоляю тебя, Сэм же вроде всё...{w} {i}'исправила'{/i}. Я чувствую себя абсолютно нормально!"

# game/status.rpy:778
translate russia task2_398cda84:

    # y "Oh can't I? {w}Sam, I 'order' you to put Clover in a cell."
    y "А может...? {w}Сэм, я приказываю тебе посадить Кловер в камеру."

# game/status.rpy:779
translate russia task2_e62b3e18:

    # s g28 "Wha-...!"
    s g28 "Чт-...!"

# game/status.rpy:792
translate russia task2_a51a4e7e_1:

    # s "I obey..."
    s "Так точно..."

# game/status.rpy:798
translate russia task2_674cf94e:

    # "Sam jumped on Clover!"
    "Сэм прыгнула на Кловер!"

# game/status.rpy:799
translate russia task2_3565ce4a:

    # c r28 "Ow, Sammy~....!"
    c r28 "Оу, Сэмми~....!"

# game/status.rpy:800
translate russia task2_4a1a3be0:

    # "After a short struggle, Sam drags Clover to one of the unoccupied cells and shoves her in."
    "После недолгой борьбы Сэм подтаскивает Кловер к одной из незанятых камер и запихивает её внутрь."

# game/status.rpy:802
translate russia task2_84dad62c:

    # s "Orders complete."
    s "Приказ выполнен."

# game/status.rpy:803
translate russia task2_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:804
translate russia task2_435a9ad6:

    # s g15 "........"
    s g15 "........"

# game/status.rpy:805
translate russia task2_dd2d2c99:

    # s g28 "{b}*Blinks*{/b} What just happened...?"
    s g28 "{b}*Моргает*{/b} Что только что произошло...?"

# game/status.rpy:806
translate russia task2_f8fb41b7:

    # y "I just tried something...."
    y "Я просто кое-что попробовал..."

# game/status.rpy:807
translate russia task2_a75ded36:

    # y "Remember when I ordered you to fight Clover back at the university?"
    y "Помнишь, когда я приказал тебе сразиться с Кловер ещё в школе?"

# game/status.rpy:808
translate russia task2_a829b3d4:

    # s g16 "Er...."
    s g16 "Эм..."

# game/status.rpy:809
translate russia task2_10eafe8e:

    # y "And just now... I ordered you to put her in a cell."
    y "А только что... Я приказал тебе посадить её в камеру."

# game/status.rpy:810
translate russia task2_e01d25cb:

    # s "You're saying....?"
    s "Ты хочешь сказать...?"

# game/status.rpy:811
translate russia task2_47410732:

    # y "That I can control your nanobots!"
    y "Я могу управлять твоими наноботами!"

# game/status.rpy:812
translate russia task2_b519742b:

    # y "And although I'm ecstatic at the thought that I can make you do my laundry. I am a little curious about something~...."
    y "Мне очень нравиться мысль, что я могу заставить тебя стирать моё бельё. Мне интересно кое-что ещё~..."

# game/status.rpy:813
translate russia task2_337e6906:

    # y "Saaaam~....."
    y "Сээээм~....."

# game/status.rpy:814
translate russia task2_bf6c141c:

    # s g11 "..................."
    s g11 "..................."

# game/status.rpy:815
translate russia task2_c71dabcc:

    # y "Have you kept up with the supression of your nanobots~....?"
    y "Ты продолжаешь подавлять своих наноботов~....?"

# game/status.rpy:816
translate russia task2_e3b41f07:

    # s g12 "{size=-10}I....{w} I may have forgotten about it....{/size}"
    s g12 "{size=-10}Я....{w} Возможно, я забыла об этом.....{/size}"

# game/status.rpy:817
translate russia task2_65852b82:

    # y "You forgot about it...."
    y "Ты забыла об этом..."

# game/status.rpy:818
translate russia task2_54ee9166:

    # s g40 "I was embarrassed, okay?! Plus I was feeling fine, I didn't think I was still being controlled!"
    s g40 "Мне неловко, понятно?! К тому же я чувствовала себя прекрасно, я не думала, что меня всё ещё контролируют!"

# game/status.rpy:819
translate russia task2_9fdcccb4:

    # y "Oh no, I can totally understand that. No hard feelings."
    y "Да я всё понимаю. Без обид."

# game/status.rpy:820
translate russia task2_5680bdcd:

    # y "Now I order you to do a handstand."
    y "А теперь встань на руки, это приказ."

# game/status.rpy:821
translate russia task2_0e549475:

    # s g28 "N-no wait...!"
    s g28 "Н-нет стой...!"

# game/status.rpy:824
translate russia task2_89064dc8:

    # "In the blink of a second, Sam's eyes turn red again as she gets down to do a handstand!"
    "В мгновение ока глаза Сэм снова становятся красными и она встаёт на руки!"

# game/status.rpy:831
translate russia task2_89aad6f2:

    # y "Yeah you're right Sam. I don't see what could {i}'possibly'{/i} go wrong."
    y "Да, ты права Сэм. Не думаю, что что-то {i}'может'{/i} пойти не так."

# game/status.rpy:832
translate russia task2_e97762bd:

    # s "I'm sorry~....! I'll keep them suppressed from now on. I promise!"
    s "Мне жаль~....! Впредь я буду их подавлять. Я обещаю!"

# game/status.rpy:835
translate russia task2_e3af48b8:

    # y "You promise?"
    y "Обещаешь?"

# game/status.rpy:836
translate russia task2_f6b8c043:

    # s "I do!"
    s "Конечно!"

# game/status.rpy:837
translate russia task2_99d52356:

    # y "I'm pretty sure you {i}'promised'{/i} last time."
    y "В прошлый раз ты тоже {i}'обещала'{/i}."

# game/status.rpy:838
translate russia task2_1cfaba43:

    # s "It's different now, really!"
    s "В этот раз всё по-другому, честно!"

# game/status.rpy:839
translate russia task2_c3d7abb3:

    # s "..................."
    s "..................."

# game/status.rpy:840
translate russia task2_76b11851:

    # s "Can I please stand up now? I'm getting dizzy."
    s "Можно мне уже встать? У меня кружится голова."

# game/status.rpy:841
translate russia task2_8e902243:

    # y "First go make me a sandwich."
    y "Сначала сделай мне сэндвич."

# game/status.rpy:842
translate russia task2_508258a2:

    # s "On my hands?!"
    s "На руках?!"

# game/status.rpy:843
translate russia task2_0c9f6b4f:

    # y "Yes, and then go clean the tables in the milkshake bar."
    y "Да, а потом иди и убери со столов в баре."

# game/status.rpy:844
translate russia task2_4b69142c_1:

    # s "B-but...!"
    s "Н-но...!"

# game/status.rpy:845
translate russia task2_aa3a3846:

    # y "Unless you want me to order you?"
    y "Если только ты не хочешь, чтобы я тебе приказал?"

# game/status.rpy:846
translate russia task2_65a57a27:

    # s "N-no...!"
    s "Н-нет...!"

# game/status.rpy:849
translate russia task2_51cc1714:

    # "Gracefully, Sam walks out of the room on her hands."
    "Грациозно, на руках Сэм выходит из комнаты."

# game/status.rpy:850
translate russia task2_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:851
translate russia task2_b21a347f:

    # "You go play your gameboy for a while until she returns."
    "Вы идёте играть в свой геймбой, пока она не вернётся."

# game/status.rpy:862
translate russia task2_587930bc:

    # s "Your... sandwich is ready..."
    s "Твой... сэндвич готов..."

# game/status.rpy:863
translate russia task2_ebee29aa:

    # y "Hang on, I just gotta finish this level first."
    y "Подожди, уровень до пройду."

# game/status.rpy:864
translate russia task2_ec24d075:

    # s "[playerName] please!"
    s "[playerName] пожалуйста!"

# game/status.rpy:866
translate russia task2_13a1b321:

    # y "Well I guess this is an uncoventional request. Still I need to know that you won't turn on me at any moment."
    y "Ну, думаю это необычная просьба. И всё же мне нужно знать, что ты не отвернешься от меня в любой момент."

# game/status.rpy:867
translate russia task2_ff53beb8:

    # y "I am going to preach to you for a while though."
    y "Я собираюсь прочитать тебе пропаведь."

# game/status.rpy:868
translate russia task2_bdaf23b4:

    # s "Is that really necessary...?"
    s "Это действительно необходимо...?"

# game/status.rpy:869
translate russia task2_b2be3207:

    # "You grab the WOOHP Guidelines and begin to read."
    "Вы берёте Руководство WOOHP и начинаете читать."

# game/status.rpy:870
translate russia task2_4dccc829:

    # y "{b}*Ahem*{/b} "
    y "{b}*Кхем*{/b}"

# game/status.rpy:871
translate russia task2_432cd7cb:

    # y "{i}'It is important to remember your betters. When you're in distress or confused, look towards those outranking you and follow their orders to the letter.'{/i}"
    y "{i}'Важно помнить о своих вышестоящих. Когда вы в беде или в замешательстве, выполняйте приказы вышестоящих коллег в точности с их указаниями.'{/i}"

# game/status.rpy:872
translate russia task2_e83a2e9f:

    # s "[playerName] please... the blood is rushing to my head."
    s "[playerName] пожалуйста... кровь приливает к моей голове."

# game/status.rpy:873
translate russia task2_8e9a8ab8:

    # y "{i}'There may come a time when you think to yourself: 'I am not sure if I'm okay with commiting these warcrimes.'{w} Just remember that your superiors know what they are doing and that there is only a small chance that you will be scapegoated for these event.'{/i}"
    y "{i}'Может наступить момент, когда вы будете не уверены в совершении тех или иных военных преступлений.'{w} Просто помните, что ваше начальство знает, что делает. Существует лишь небольшой шанс, что вы станете козлом отпущения за данный инцидент.'{/i}"

# game/status.rpy:874
translate russia task2_8ae476f3:

    # y "(Warcrimes...?)"
    y "(Военные преступления...?)"

# game/status.rpy:875
translate russia task2_73d8f6d7:

    # y "{i}'When you are setting fire to an orphanage, take comfort in the fact that your officer made an informed decision to do so. Afterall... Any one of these orphans could grow up to become Hitler! '{/i}"
    y "{i}'Поджигаете сиротский приют, помните, что ваш офицер принял взвешенное решение. В конце концов... любой из этих сирот может вырасти и стать Гитлером!'{/i}"

# game/status.rpy:876
translate russia task2_bb423893:

    # y "Wow, WOOHP is kind of messed up."
    y "Ого, как в WOOHP всё запутано."

# game/status.rpy:877
translate russia task2_57155588:

    # s "[playerName] I'm getting dizzy~...!"
    s "[playerName], у меня кружится голова~...!"

# game/status.rpy:883
translate russia task2_ab9da557:

    # y "No Sam! You have betrayed my trust and put the entire world in danger!"
    y "Нет Сэма! Ты предала моё доверие и подвергла опасности весь мир!"

# game/status.rpy:885
translate russia task2_0eb84f9f:

    # y "Your transgression is so vile, it can no longer be judged by mortal means!"
    y "Твой проступок настолько мерзок, что его уже нельзя судить обычными способами!"

# game/status.rpy:886
translate russia task2_86dbd400:

    # y "All you had to do is keep yourself in check and spare the world a horrible fate of being plundged into chaos."
    y "Всё что от тебя требовалось - это следить за своими наноботами и избавить мир от ужасной участи быть погружённым в хаос."

# game/status.rpy:887
translate russia task2_867717c9:

    # y "Yet you, thinking only about yourself, disregarded this responsibility and followed your own selfish instincts!"
    y "Но ты думаешь только о себе, пренебрегаешь этой ответственностью и следуешь своим эгоистичным инстинктам!"

# game/status.rpy:888
translate russia task2_18ecf054:

    # y "I therefor summon you! You mighty! You infinite! You, master of all time! {w}Of all space!"
    y "Я вызываю тебя! Ты могуч! Ты бесконечен! Мастер всех времён! {w}Всего космоса!"

# game/status.rpy:892
translate russia task2_bef96fb3:

    # y "You the bringer of the endtimes! {w}The all-father!{w} You who's name is only said in whispers!"
    y "Ты - вестник конца времен! {w}Всеотец!{w} Ты, чье имя произносят только шёпотом!"

# game/status.rpy:893
translate russia task2_0d642afd:

    # y "ZUKKERTHROTH, DESTROYER OF MOONS! I CALL UPON YOU TO JUDGE THIS FEEBLE MORTAL WOMAN!"
    y "ЦУККЕРТРОТ, РАЗРУШИТЕЛЬ ЛУН! Я ПРИЗЫВАЮ ТЕБЯ СУДИ ЭТУ СЛАБУЮ СМЕРТНУЮ ЖЕНЩИНУ!"

# game/status.rpy:898
translate russia task2_594efb42:

    # s "..........."
    s "..........."

# game/status.rpy:899
translate russia task2_ba081319:

    # s "Erm... Can I please stand up now?"
    s "Эмм... Можно я уже встану?"

# game/status.rpy:903
translate russia task2_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:904
translate russia task2_eaa24d5f:

    # y "Wait..."
    y "Подожди..."

# game/status.rpy:905
translate russia task2_ebcbe9ee:

    # y "Are you not seeing Zukkerthroth, immortal being of infinite wisdom and lubed tentacles?"
    y "Ты не видишь Цуккертрота, бессмертное существо бесконечной мудрости и смазанных щупалец?"

# game/status.rpy:909
translate russia task2_83bd80f7:

    # s "No....~?"
    s "Нет...~?"

# game/status.rpy:913
translate russia task2_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:917
translate russia task2_ef689093:

    # s "Please, the blood is rushing to my head~...."
    s "Пожалуйста, кровь приливает к моей голове~...."

# game/status.rpy:921
translate russia task2_a85b3cf0:

    # y "(Sorry Zukkerthroth, she's an unbeliever. {w}Better luck next time, eh?)"
    y "(Извините, Зуккертрот, она неверующая. {w}В следующий раз повезёт, да?)"

# game/status.rpy:922
translate russia task2_489d20e0:

    # "Zukkerthroth, destroyer of moons" "{b}*Dejected whining of the infinite*{/b}"
    "Цуккертрот, разрушитель лун" "{b}*Мрачное скуление бесконечности*{/b}"

# game/status.rpy:928
translate russia task2_7e4d75a7:

    # y "Okay fine, you can stand up again."
    y "Ладно, хорошо, можешь снова встать."

# game/status.rpy:934
translate russia task2_3de2c37d:

    # y "Still, the fact I can order you around probably means that the other agents can do it too."
    y "Тем не менее тот факт, что я могу приказывать тебе, означает, что и другие агенты тоже могут это делать."

# game/status.rpy:935
translate russia task2_92bace41:

    # y "Do you want that, Sam? Being ordered around? Be somebody else's puppet?"
    y "Ты хочешь этого, Сэм? Чтобы тебе приказывали? Быть чьей-то марионеткой?"

# game/status.rpy:936
translate russia task2_19d52778:

    # s g11 "No~..."
    s g11 "Нет~..."

# game/status.rpy:937
translate russia task2_96eadbfb:

    # y "To be somebody's plaything and commit acts of tyranny and chaos?"
    y "Быть чьей-то игрушкой и совершать акты тирании и хаоса?"

# game/status.rpy:938
translate russia task2_ba52a4e9:

    # s g11 "No I don't~...."
    s g11 "Нет я не хочу~..."

# game/status.rpy:939
translate russia task2_0f749f5c:

    # y "To steal from old ladies? Beat up old ladies? Help old ladies cross the street{w} only to run away half way through, leaving them behind in the middle of the crosswalk as the cars menacingly rev their engines?"
    y "Воровать у старушек? Избивать старушек? Помогать старушкам переходить улицу{w} только для того, чтобы убежать на полпути, оставив их посреди дороги?"

# game/status.rpy:940
translate russia task2_4ea5c101:

    # s g14 "Okay, okay I get it. No, I don't want that, [playerName]!"
    s g14 "Ладно, ладно, я поняла. Нет, я этого не хочу, [playerName]!"

# game/status.rpy:941
translate russia task2_b84345fb:

    # y "THEN GO FUCK YOURSELF!"
    y "ТОГДА ИДИ И ОТЫМЕЙ СЕБЯ ПО ПОЛНОЙ!"

# game/status.rpy:942
translate russia task2_2dfbc557:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:943
translate russia task2_9d7a47be:

    # y "Literally in this case."
    y "В данном случае буквально."

# game/status.rpy:944
translate russia task2_edcf56d5:

    # s g15 "I will! All right? I'm sorry, I made a mistake!"
    s g15 "Обязательно! Ладно? Прости, я совершила ошибку!"

# game/status.rpy:945
translate russia task2_19e5eec2:

    # s "It won't happen again..."
    s "Этого больше не повторится..."

# game/status.rpy:946
translate russia task2_2630f67f:

    # y "Well let's make sure of that."
    y "Что ж, давай убедимся в этом."

# game/status.rpy:947
translate russia task2_e8d692c6:

    # s g18 "...?"
    s g18 "...?"

# game/status.rpy:948
translate russia task2_8efa8d4e:

    # y "I'm going to need a way to measure your oxytocin levels. WOOHP might have something in their database."
    y "Мне нужен способ следить за твоим уровнем окситоцина. В базе данных WOOHP может быть что-то полезное."

# game/status.rpy:949
translate russia task2_88fafd96:

    # s "Ah okay. Well you can always check the Equipment tab at your workbench."
    s "Аа, ладно. Ты всегда сможешь проверить вкладку \"Оборудование\" на своём верстаке."

# game/status.rpy:950
translate russia task2_30d7b58c:

    # y "I think I'll do that.{w} Oh and Sam...."
    y "Думаю, я так и сделаю.{w} Ох, и Сэм..."

# game/status.rpy:951
translate russia task2_d25bc4fc:

    # s "Yeah...?"
    s "Да...?"

# game/status.rpy:952
translate russia task2_793a7edb:

    # y "I order you to...."
    y "Я приказываю тебе..."

# game/status.rpy:953
translate russia task2_f18b93b9:

    # s g28 "No no no no!"
    s g28 "Нет нет нет нет!"

# game/status.rpy:954
translate russia task2_b16d67d2:

    # y "Handstand!"
    y "Стой на руках!"

# game/status.rpy:955
translate russia task2_a153141f:

    # s "Aaah!"
    s "Ааах!"

# game/status.rpy:965
translate russia task2_4b5c48bf:

    # s "[playerName]!"
    s "[playerName]!"

# game/status.rpy:1002
translate russia task2_30285868:

    # y "Let's see what what we can make here..."
    y "Посмотрим, что тут у нас..."

# game/status.rpy:1003
translate russia task2_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1004
translate russia task2_968eba08:

    # "Error: Database Protected."
    "Ошибка: База данных защищена."

# game/status.rpy:1005
translate russia task2_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:1006
translate russia task2_cb3c8129_1:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1007
translate russia task2_968eba08_1:

    # "Error: Database Protected."
    "Ошибка: База данных защищена."

# game/status.rpy:1008
translate russia task2_75b11a2d:

    # "{b}*Bleep* *Bleep* *Bleep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:1009
translate russia task2_968eba08_2:

    # "Error: Database Protected."
    "Ошибка: База данных защищена."

# game/status.rpy:1010
translate russia task2_762461b6:

    # y "Yeah this isn't going anywhere."
    y "Так ничего не получиться."

# game/status.rpy:1011
translate russia task2_c6a92703:

    # y "(I'm going to have to find a way to restore the database.)"
    y "(Мне нужно найти способ восстановить базу данных.)"

# game/status.rpy:1013
translate russia task2_b12ba1a2:

    # y "(Perhaps doing some exploring around will help with this.)"
    y "(Возможно, исследование окрестностей поможет в этом.)"

# game/status.rpy:1015
translate russia task2_01372a4d:

    # y "(Maybe Mathias will be able to help me with this.)"
    y "(Может Матиас сможет мне помочь.)"

# game/status.rpy:1028
translate russia task2_2afb661e:

    # y "Hey Mathias, could you hack into a database for me?"
    y "Эй, Матиас, не мог бы ты взломать для меня одну базу данных?"

# game/status.rpy:1029
translate russia task2_0070893d:

    # mat "I could. Whadda ya need?"
    mat "Могу. Что тебе нужно?"

# game/status.rpy:1030
translate russia task2_8f2fc1c4:

    # y "Acces to secured files in the WOOHP network."
    y "Доступ к защищённым файлам в сети WOOHP."

# game/status.rpy:1031
translate russia task2_9ecee2c6:

    # mat "The WOOHP Network?!{w} That's risky man..."
    mat "Сеть WOOHP?!{w} Это рискованно чувак..."

# game/status.rpy:1032
translate russia task2_e7115b96:

    # y "You can't do it?"
    y "Ты не сможешь?"

# game/status.rpy:1033
translate russia task2_42733636:

    # mat "Of course I can do it, but it'll take time and I got other priorities right now."
    mat "Конечно я смогу, но на это потребуется время, а у меня сейчас другие приоритеты."

# game/status.rpy:1034
translate russia task2_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:1035
translate russia task2_fe21e8ab:

    # y "Okay, how much?"
    y "Ладно, сколько?"

# game/status.rpy:1036
translate russia task2_f4ad5989:

    # mat "How much what?"
    mat "Сколько чего?"

# game/status.rpy:1037
translate russia task2_f5d2ed94:

    # y "Money. How much money do you want for it?"
    y "Денег. Сколько денег ты хочешь?"

# game/status.rpy:1038
translate russia task2_352fa0f6:

    # mat "Hah! I'll do it for free my dude, but you gotta be patient."
    mat "Хах! Я сделаю это бесплатно, чувак, но тебе нужно потерпеть."

# game/status.rpy:1039
translate russia task2_a2ecadc9:

    # y "Patience is not my strongsuit. Is there something else you want?{w} Some dirty mags maybe?"
    y "Терпение - не моя сильная сторона. Есть ли что-то ещё, чего ты хочешь?{w} Как насчёт развратных журналов?"

# game/status.rpy:1040
translate russia task2_3526b50d:

    # mat "I got the internet y'know...."
    mat "У меня же есть интернет..."

# game/status.rpy:1041
translate russia task2_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:1042
translate russia task2_c3cd132a:

    # mat "If you can get a bikini model to show up at my front door, I promise to move you to the priority list. Else you're gonna have to be patient."
    mat "Если ты сможешь заставить модель в бикини появиться у моей входной двери, я обещаю переместить тебя в список приоритетов. Иначе придётся подождать."

# game/status.rpy:1043
translate russia task2_db832180:

    # y "........"
    y "........"

# game/status.rpy:1044
translate russia task2_95d31cdf:

    # y "A bikini model?{w} I could probably convince my spy to wear a bikini..."
    y "Модель в бикини?{w} Может у меня получиться убедить шпионок надеть бикини..."

# game/status.rpy:1045
translate russia task2_e00e42df:

    # mat "....?"
    mat "....?"

# game/status.rpy:1046
translate russia task2_fe1994d8:

    # y "He doesn't have to know that I'm not actually part of WOOHP and that we're trying to take back the city to restore order."
    y "Ему не обязательно знать, что я на самом деле не являюсь частью WOOHP и то что мы пытаемся спасти город и восстановить порядок."

# game/status.rpy:1047
translate russia task2_bd7f8a7a:

    # mat "Err...."
    mat "Эмм..."

# game/status.rpy:1048
translate russia task2_d1d62467:

    # y "Yeah, that sounds like a great idea! I'll have my spy show up here wearing a bikini, have him hack into the WOOHP mainframe and get those database entries!"
    y "Да, звучит как отличная идея! Я попрошу своих шпионок прийти сюда в бикини, попрошу его взломать главный компьютер WOOHP и получить эти записи из базы данных!"

# game/status.rpy:1049
translate russia task2_6d474c54:

    # mat "Dude..."
    mat "Чувак..."

# game/status.rpy:1050
translate russia task2_42f38d2b:

    # y "Meanwhile keeping him completely in the dark! No need to drag this kid into a war between the forces of good and evil!"
    y "При этом держа его в полном неведении! Не нужно втягивать этого ребёнка в войну между силами добра и зла!"

# game/status.rpy:1051
translate russia task2_f5d256e5:

    # mat "You know I can hear everything you're saying, right?"
    mat "Ты же в курсе, что я тебя слышу, чувак?"

# game/status.rpy:1052
translate russia task2_1984a35a:

    # y "....."
    y "....."

# game/status.rpy:1053
translate russia task2_37797c55:

    # y "Did I say all of that out loud?"
    y "Я что, сказал всё это вслух?"

# game/status.rpy:1054
translate russia task2_1d71b3f3:

    # mat "Pretty much."
    mat "Вроде да."

# game/status.rpy:1055
translate russia task2_97757901:

    # y "Damn it. I need to be more careful in the future.{w} I might make this kid an accomplice in our borderline illegal plot to re-take the city!"
    y "Блять. Нужно быть более осторожным в будущем.{w} Я могу сделать этого парня соучастником нашего незаконного заговора по захвату города!"

# game/status.rpy:1056
translate russia task2_22e318bd:

    # mat "Still talking out loud....."
    mat "Я всё ещё тебя слышу..."

# game/status.rpy:1057
translate russia task2_5adef63f:

    # y "(CRAP!)"
    y "(ЧЁРТ!)"

# game/status.rpy:1058
translate russia task2_cc845f3d:

    # y "Okay... You get busy hacking the WOOHP network. If I can get you a bikini model, you'll speed things up?"
    y "Ладно... займись взломом WOOHP. Если я смогу найти тебе модель в бикини, ты ускоришь процесс?"

# game/status.rpy:1059
translate russia task2_4408e138:

    # mat "You got a deal, see ya dude."
    mat "Договорились, увидимся чувак."

# game/status.rpy:1060
translate russia task2_2e5a2c94:

    # y "Laters."
    y "Увидимся."

# game/status.rpy:1070
translate russia task2_4fd76e28:

    # y "(As long as I got some bikinis, I could send someone over to Mathias for a research boost.)"
    y "(До тех пор, пока у меня есть бикини, я могу отправлять шпионок к Матиасу для ускорения прогресса.)"

# game/status.rpy:1071
translate russia task2_0e9c2d64:

    # y "Girls, could you come out here for a second?"
    y "Девочки, не могли бы вы подойти на секундочку?"

# game/status.rpy:1078
translate russia task2_dc34dc58:

    # y "Soooooo~....."
    y "Тааааак~....."

# game/status.rpy:1080
translate russia task2_9ae2f597:

    # s "[playerName]~.....?"
    s "[playerName]~.....?"

# game/status.rpy:1081
translate russia task2_850ef2d0:

    # s g18 "It sounds like you're about to ask us to do something."
    s g18 "Звучит так, будто ты собираешься попросить нас о чём-то."

# game/status.rpy:1082
translate russia task2_1db49ca3:

    # y "........."
    y "........."

# game/status.rpy:1083
translate russia task2_ec5b0ddf:

    # y "Put on a bikini."
    y "Надевай бикини."

# game/status.rpy:1084
translate russia task2_c38eabb7:

    # s g28 "Huh? You... want me to...? What?"
    s g28 "Хух? Ты... хочешь, чтобы я...? Что?"

# game/status.rpy:1085
translate russia task2_a0dcf827:

    # y "Put on a bikini and head to this address.{w} Don't ask why."
    y "Надевай бикини и шуруй по этому адресу.{w} Не спрашивай зачем."

# game/status.rpy:1086
translate russia task2_1d138bf8:

    # s "Are you kidding?! Of course I'm gonna ask why!"
    s "Ты шутишь?! Конечно я спрошу!"

# game/status.rpy:1087
translate russia task2_552e28d6:

    # y "There's this guy who promised to hack into the WOOHP network, but only if he gets a bikini model."
    y "Есть один парень, который обещал взломать сеть WOOHP, но только если он получит модель в бикини."

# game/status.rpy:1088
translate russia task2_4ed17b4e:

    # s g9 "....................."
    s g9 "....................."

# game/status.rpy:1089
translate russia task2_5a5f72d4:

    # s g40 "And what about the fact that I'm not a model..."
    s g40 "А как насчёт того, что я не модель..."

# game/status.rpy:1090
translate russia task2_ba6a1b68:

    # y "Are you joking? Anyone can be a model now-a-days! Just put on a bikini and take a few pictures of yourself!"
    y "Ты шутишь? В наше время моделью может стать каждый! Просто надень бикини и сделай несколько фотографий!"

# game/status.rpy:1091
translate russia task2_6b0c5c54:

    # s g8 "[playerName] I don't appreciate you renting me out like this!"
    s g8 "[playerName] мне не нравится, что ты меня так сдаёшь!"

# game/status.rpy:1092
translate russia task2_f5b34892:

    # c r12 "I got excited for a moment there when you talked about being a model..."
    c r12 "Я на мгновение обкрадывалась, когда ты заговорил о моделях..."

# game/status.rpy:1093
translate russia task2_adc3844f:

    # c r14 "Being a stripper for some sweaty nerd to ogle at is {i}'so'{/i} not glamorous!"
    c r14 "Но быть стриптизершей для какого-то потного ботана, это {i}'так'{/i} не гламурно!"

# game/status.rpy:1094
translate russia task2_298dab44:

    # s "What are we even suppose to do once we get there?"
    s "Что нам там вообще делать?"

# game/status.rpy:1095
translate russia task2_7f372ff3:

    # y "I dunno. Stand around looking hot, do some poses, compliment the guy on his collection of harddrives. Get creative!"
    y "Не знаю. Постойте в разных позах, выглядите сексуально, постройте глазки, похвалите парня за его коллекцию жестких дисков. Будьте изобретательны!"

# game/status.rpy:1096
translate russia task2_9c64dbd8:

    # s g38 "Well I appreciate the suggestions, but I will not be doing it."
    s g38 "Я благодарю за предложение, но я не буду этого делать."

# game/status.rpy:1097
translate russia task2_7cf11cda:

    # s "I've already been embarrassed enough over these last few days, I don't want to turn into a sex object for people to gawk at."
    s "Я уже достаточно засмущалась за последние несколько дней, я не хочу превращаться в сексуальный объект, на который люди будут глазеть."

# game/status.rpy:1098
translate russia task2_692be6fa:

    # y "............"
    y "............"

# game/status.rpy:1100
translate russia task2_75fbe4bf:

    # c "[playerName]?"
    c "[playerName]?"

# game/status.rpy:1101
translate russia task2_bfb73fe2:

    # c r18 "What's this all about...?"
    c r18 "Что всё это значит...?"

# game/status.rpy:1102
translate russia task2_1db49ca3_1:

    # y "........."
    y "........."

# game/status.rpy:1103
translate russia task2_ec5b0ddf_1:

    # y "Put on a bikini."
    y "Надевай бикини."

# game/status.rpy:1104
translate russia task2_6c2292ac:

    # cr28 "A bikini? This better involve some hot guys, [playerName]!"
    cr28 "Бикини? Надеюсь это будет как-то связано с горячими парнями, [playerName]!"

# game/status.rpy:1105
translate russia task2_a0dcf827_1:

    # y "Put on a bikini and head to this address.{w} Don't ask why."
    y "Надевай бикини и шуруй по этому адресу.{w} Не спрашивай зачем."

# game/status.rpy:1106
translate russia task2_154e4fc5:

    # c "........."
    c "........."

# game/status.rpy:1107
translate russia task2_24847d4b:

    # c "That's like... totally sketchy."
    c "Это типа... как-то странно."

# game/status.rpy:1108
translate russia task2_552e28d6_1:

    # y "There's this guy who promised to hack into the WOOHP network, but only if he gets a bikini model."
    y "Есть один парень, который обещал взломать сеть WOOHP, но только если он получит модель в бикини."

# game/status.rpy:1109
translate russia task2_36908da6:

    # c r9 "Ew, a hacker? Those are usually totally growdy nerds!"
    c r9 "Фу, хакер? Обычно это совершенно шизанутые гики!"

# game/status.rpy:1110
translate russia task2_bb132e75:

    # c r8 "Are you just going to rent us out like that...?!"
    c r8 "Ты что, собираешься вот так просто сдать нас в \"аренду\"...?!"

# game/status.rpy:1111
translate russia task2_ca618152:

    # a "I hoped that we were going to the beach..."
    a "А я надеялась, что мы пойдём на пляж..."

# game/status.rpy:1112
translate russia task2_fe08bb87:

    # a "Going to some hacker's underground basement sounds like the opposite of that."
    a "Но поход в подвал какому-то хакеру, звучит как полная противоположность этому."

# game/status.rpy:1113
translate russia task2_02c18273:

    # c "What are we even suppose to do once we get there?"
    c "Что нам там вообще делать?"

# game/status.rpy:1114
translate russia task2_7f372ff3_1:

    # y "I dunno. Stand around looking hot, do some poses, compliment the guy on his collection of harddrives. Get creative!"
    y "Не знаю. Постойте в разных позах, выглядите сексуально, постройте глазки, похвалите парня за его коллекцию жестких дисков. Будьте изобретательны!"

# game/status.rpy:1115
translate russia task2_88ea27cd:

    # c r38 "Okay... first of all that's not how you get a guy interested in you. And second of all my youthful beauty is wasted on geeks like that."
    c r38 "Ладно... во-первых, это точно не правильный способ привлечь внимание парня. А во-вторых, моя красота напрасно тратится на таких чудаков."

# game/status.rpy:1116
translate russia task2_16cce8ce:

    # a "But I thought you liked being ogled and admired, Clover?"
    a "Но я думала, тебе нравится, когда на тебя глазеют, Кловер?"

# game/status.rpy:1117
translate russia task2_b9603689:

    # c "Only by hot guys!"
    c "Только если это горячие парни!"

# game/status.rpy:1118
translate russia task2_692be6fa_1:

    # y "............"
    y "............"

# game/status.rpy:1120
translate russia task2_b065bc07:

    # a "[playerName]?"
    a "[playerName]?"

# game/status.rpy:1121
translate russia task2_ca3f63cd:

    # a y18 "What can we do for you?"
    a y18 "Чем мы можем помочь?"

# game/status.rpy:1122
translate russia task2_1db49ca3_2:

    # y "........."
    y "........."

# game/status.rpy:1123
translate russia task2_ec5b0ddf_2:

    # y "Put on a bikini."
    y "Надевай бикини."

# game/status.rpy:1124
translate russia task2_da3511ed:

    # c r28 "A bikini? Are we going to the beach?"
    c r28 "Бикини? Мы идём на пляж?"

# game/status.rpy:1125
translate russia task2_01619e95:

    # y "No.{w} Put on a bikini and head to this address.{w} Don't ask why."
    y "Нет.{w} Надевай бикини и шуруй по этому адресу.{w} Не спрашивай зачем."

# game/status.rpy:1126
translate russia task2_b2b821d1:

    # a "........."
    a "........."

# game/status.rpy:1127
translate russia task2_046e8bfc:

    # a "W-...{w}waterpark then?"
    a "Тогда...{w} аквапарк?"

# game/status.rpy:1128
translate russia task2_552e28d6_2:

    # y "There's this guy who promised to hack into the WOOHP network, but only if he gets a bikini model."
    y "Есть один парень, который обещал взломать сеть WOOHP, но только если он получит модель в бикини."

# game/status.rpy:1129
translate russia task2_62e56469:

    # a y9 "{b}*Disappointed*{/b} No waterpark then...."
    a y9 "{b}*Разочарована*{/b} Никакого аквапарка тогда..."

# game/status.rpy:1130
translate russia task2_8c93966a:

    # s g8 "Are you just going to rent us out like that...?!"
    s g8 "Ты что, собираешься вот так просто сдать нас в \"аренду\"...?!"

# game/status.rpy:1131
translate russia task2_298dab44_1:

    # s "What are we even suppose to do once we get there?"
    s "Что нам там вообще делать?"

# game/status.rpy:1132
translate russia task2_7f372ff3_2:

    # y "I dunno. Stand around looking hot, do some poses, compliment the guy on his collection of harddrives. Get creative!"
    y "Не знаю. Постойте в разных позах, выглядите сексуально, постройте глазки, похвалите парня за его коллекцию жестких дисков. Будьте изобретательны!"

# game/status.rpy:1133
translate russia task2_78adfc99:

    # a "Stand around and look hot...?"
    a "Стоять и выглядеть сексуально...?"

# game/status.rpy:1134
translate russia task2_f7941638:

    # a "I dunno [playerName]. I totally saw this documentary about a group of people who got kidnapped like that once!"
    a "Не знаю [playerName]. Я точно видела один документальный фильм о группе людей, которых однажды похитили подобным образом!"

# game/status.rpy:1135
translate russia task2_692be6fa_2:

    # y "............"
    y "............"

# game/status.rpy:1159
translate russia vibMenuJump_f7b302db:

    # y "Sam... he's going to help us break into the WOOHP Database.{w} With his help, we'll have acces to new gadgets and equipment."
    y "Сэм... он поможет нам взломать базу данных WOOHP.{w} С его помощью мы получим доступ к новым гаджетам и оборудованию."

# game/status.rpy:1160
translate russia vibMenuJump_28e1205a:

    # y "It'll make missions a lot easier to have some extra tech on our side."
    y "Если на нашей стороне будет дополнительная техника, это значительно облегчит выполнение миссий."

# game/status.rpy:1161
translate russia vibMenuJump_a608601e:

    # s "{b}*Hmph*{/b}~..."
    s "{b}*Хмпх*{/b}~..."

# game/status.rpy:1162
translate russia vibMenuJump_dbade75c:

    # s "So you expect me to entertain some nerdy loner by shaking my ass a bit?"
    s "То есть ты хочешь чтобы я, слегка покачивая задницей, развлекала какого-то зануду-одиночку?"

# game/status.rpy:1163
translate russia vibMenuJump_48c03327:

    # s "I find this very disrespectful, [playerName]!"
    s "Это очень неуважительно, [playerName]!"

# game/status.rpy:1164
translate russia vibMenuJump_3356ef62:

    # c r38 "You said it, Sammy!"
    c r38 "Так его, Сэмми!"

# game/status.rpy:1165
translate russia vibMenuJump_31dc26fd:

    # s g11 "................"
    s g11 "................"

# game/status.rpy:1166
translate russia vibMenuJump_cc4caae8:

    # s "But...."
    s "Но..."

# game/status.rpy:1167
translate russia vibMenuJump_ad9a3669:

    # s g12 "But if it'll help us take back WOOHP.... I guess I'll do it."
    s g12 "Но если это поможет нам вернуть WOOHP.... Думаю, я готова это сделать."

# game/status.rpy:1168
translate russia vibMenuJump_f4769d74:

    # c r11 "Hmph~...."
    c r11 "Хмпф~...."

# game/status.rpy:1169
translate russia vibMenuJump_f6b8de2f:

    # s "Just think about it Clover. What if they start bringing out their big guns? We need gadgets to stand a chance."
    s "Просто подумай об этом, Кловер. Что если они начнут использовать большие пушки? Нам нужны гаджеты, чтобы иметь шанс."

# game/status.rpy:1170
translate russia vibMenuJump_68590e87:

    # s "I don't like it, [playerName]. But for the safety of the world. I'll do it."
    s "Мне не нравится это, [playerName]. Но ради безопасности мира. Я сделаю это."

# game/status.rpy:1171
translate russia vibMenuJump_fa395d2b:

    # c "............."
    c "............."

# game/status.rpy:1172
translate russia vibMenuJump_4c7bad17:

    # c r38 "Gah! Fine!{w} As long as he doesn't take picture and upload them to his blog or something. I can't be seen around people like him, it would ruin my reputation!"
    c r38 "Гах! Ладно!{w} До тех пор, пока он не будет фотографировать и загружать их в свой блог или что-то в этом роде. Я не могу находиться рядом с такими людьми, как он, это испортит мою репутацию!"

# game/status.rpy:1173
translate russia vibMenuJump_9ee2fb09:

    # y "That's the spirit! I might send you out tomorrow."
    y "Вот это дух! Я могу отправить вас завтра."

# game/status.rpy:1174
translate russia vibMenuJump_47fa8604:

    # y "Oh and Sam...."
    y "Оо и Сэм..."

# game/status.rpy:1175
translate russia vibMenuJump_40a334cc:

    # s g10 "Hm?"
    s g10 "Хм?"

# game/status.rpy:1176
translate russia vibMenuJump_704bfe09:

    # y "It wouldn't hurt to smile once you get there."
    y "Когда будешь там, не забудь улыбнуться, лишним не будет."

# game/status.rpy:1177
translate russia vibMenuJump_124c684d:

    # s g33 "{b}*Sighs*{/b} Fine..."
    s g33 "{b}*Вздох*{/b} Хорошо..."

# game/status.rpy:1195
translate russia vibMenuJump_aa050ddd:

    # y "You wouldn't even do it for Mr. Benjamin...?"
    y "Ты бы не сделала этого даже ради Мистера. Бенджамина...?"

# game/status.rpy:1196
translate russia vibMenuJump_8440ceeb:

    # c r7 "!!!"
    c r7 "!!!"

# game/status.rpy:1197
translate russia vibMenuJump_a91541ba:

    # s g8 "[playerName]! I am {b}'not'{/b} a prostitute!"
    s g8 "[playerName]! Я {b}'не'{/b} проститутка!"

# game/status.rpy:1198
translate russia vibMenuJump_18a65fd2:

    # s "I can't believe that you think so little of me! I am not going and that's final!"
    s "Не могу поверить, что ты так низко обо мне думаешь! Я не пойду, и это точка!"

# game/status.rpy:1199
translate russia vibMenuJump_387cd099:

    # y "But...!"
    y "Но...!"

# game/status.rpy:1200
translate russia vibMenuJump_472acdb9:

    # c r40 "I can't believe you're trying to bribe us with one Benjamin!"
    c r40 "Не могу поверить, что ты пытаешься подкупить нас одним Бенджамином!"

# game/status.rpy:1201
translate russia vibMenuJump_1b0a83c0:

    # y "How about two?"
    y "Как насчёт двух?"

# game/status.rpy:1202
translate russia vibMenuJump_45127467:

    # c r23 "Well....~"
    c r23 "Нууу...~"

# game/status.rpy:1203
translate russia vibMenuJump_10ea6829:

    # s "CLOVER, NO!"
    s "КЛОВЕР, НЕТ!"

# game/status.rpy:1225
translate russia vibMenuJump_3656d585:

    # y "Sam... Clover..."
    y "Сэм... Кловер..."

# game/status.rpy:1226
translate russia vibMenuJump_13fab0f0:

    # s g18 "Hm?"
    s g18 "Хм?"

# game/status.rpy:1227
translate russia vibMenuJump_0a7a2e56:

    # y "I {b}'order'{/b} you two to put on a bikini and head to Mathias."
    y "Я {b}'приказываю'{/b} вам двоим надеть бикини и отправиться к Матиасу."

# game/status.rpy:1228
translate russia vibMenuJump_df778033:

    # s g28 "W-wait...!"
    s g28 "С-стой...!"

# game/status.rpy:1229
translate russia vibMenuJump_53b6fb1e:

    # c r29 "What?!"
    c r29 "Что?!"

# game/status.rpy:1242
translate russia vibMenuJump_a2d05249:

    # s g39 "Yes sir! Seduction mission is a go!"
    s g39 "Так точно сэр! Миссия соблазнения начата!"

# game/status.rpy:1243
translate russia vibMenuJump_729bb4bd:

    # c r8 "Shedding some clothes for the glory of WOOHP is but a small price to pay!"
    c r8 "Сбросить немного одежды во славу WOOHP - это лишь малая цена!"

# game/status.rpy:1244
translate russia vibMenuJump_0ae37512:

    # y "That's better! I might send you two out during the day to visit Mathias."
    y "Так-то лучше! Я отправлю вас двоих днём навестить Матиаса."

# game/status.rpy:1245
translate russia vibMenuJump_1246abe5:

    # "Both girls" "Yes sir!"
    "Обе девушки" "Так точно!"

# game/status.rpy:1248
translate russia vibMenuJump_caea8601:

    # "You can now send girls to Mathias during the day to boost his research. Only spies that have a swimsuit available can go."
    "Теперь вы можете посылать девушек к Матиасу в течение дня, чтобы ускорить его исследования. Только шпионки, у которых в наличии есть купальник, могут пойти."

# game/status.rpy:1249
translate russia vibMenuJump_737105da:

    # "This is entirely optional however. Mathias will continue to work on hacking the database, even without the spies visiting him."
    "Однако это совершенно необязательно. Матиас будет продолжать работать над взломом базы данных даже без посещения его шпионками."

# game/status.rpy:1263
translate russia vibMenuJump_b325af99:

    # "The Vitals Indication Bug aka V.I.B. unlocked!"
    "Вибро-индикатор-вредителей, он же V.I.B. разблокирован!"

# game/status.rpy:1264
translate russia vibMenuJump_e8f77e01:

    # "The V.I.B. will messure a spy's oxytocin levels and nanobots control via the STATUS screen."
    "V.I.B. будет проверять уровень окситоцина шпионок и поможет в управлении наноботами через экран СТАТУС."

# game/status.rpy:1265
translate russia vibMenuJump_edb884fc:

    # "It can also be used during missions to make a nanocontrolled spy come to her senses."
    "V.I.B. можно использовать во время миссий, он заставит шпионку прийти в себя если она вдруг окажется под контролем наноботов."

# game/status.rpy:1266
translate russia vibMenuJump_9072e430:

    # "Careful however! Using the V.I.B. during missions can give away the spies location."
    "Однако будьте осторожны! Использование V.I.B. во время миссий может выдать местоположение шпионок."

# game/status.rpy:1267
translate russia vibMenuJump_6815855a:

    # y "I bet the girls are gonna love this..."
    y "Уверен, девочкам это понравится..."

# game/status.rpy:1268
translate russia vibMenuJump_73c37ba1:

    # y "Once I have a moment, I should go talk to them."
    y "Как только появится свободная минутка, пойду и поговорю с ними."

# game/status.rpy:1293
translate russia vibMenuJump_a5377adb:

    # y "{b}*Deep breath*{/b}"
    y "{b}*Глубокий вдох*{/b}"

# game/status.rpy:1294
translate russia vibMenuJump_cbf0d555:

    # y "Okay, how am I going to tell them...."
    y "Ладно, как мне им это сказать..."

# game/status.rpy:1295
translate russia vibMenuJump_a6c5d69b:

    # "Inner Voice" "Be honest and straight forward with them."
    "Внутренний голос" "Будь честным и откровенным с ними."

# game/status.rpy:1296
translate russia vibMenuJump_a3aa9132:

    # y "I know I should, voice in my head. But you know how they're going to react."
    y "Я знаю, голос в моей голове. Но ты знаешь, как они отреагируют."

# game/status.rpy:1297
translate russia vibMenuJump_e8cb0137:

    # "Inner Voice" "They will probably say some nasty things to you, but just remember:{w} {i}I am a strong independent man and my ego is strong enough to handle some mean remarks by teenage girls.{/i}"
    "Внутренний голос" "Скорее всего они будут говорить тебе неприятные вещи, но помни:{w} {i}Я сильный независимый мужчина и мое эго достаточно сильно, чтобы выдержать несколько грубых замечаний со стороны девочек-подростков.{/i}"

# game/status.rpy:1298
translate russia vibMenuJump_a62ec5b5:

    # y "Yeah... Thanks voice."
    y "Ага... Спасибо голос."

# game/status.rpy:1299
translate russia vibMenuJump_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:1300
translate russia vibMenuJump_e0b06210:

    # y "By the way....{w} Are you suppose to be my conscious or something?"
    y "Кстати...{w} Ты моя совесть или что-то в этом роде?"

# game/status.rpy:1301
translate russia vibMenuJump_a421e21b:

    # "Inner Voice" "Also if the girls refuse and say mean things to you, you should strap them down to a bed and preform Chinese water torture on them."
    "Внутренний голос" "Если девушки откажутся и будут говорить тебе гадости, тебе всего лишь нужно будет пристегнуть их к кровати и пытать водой, известный китайский способ."

# game/status.rpy:1302
translate russia vibMenuJump_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:1303
translate russia vibMenuJump_e080c181:

    # y "Definitely not my conscious."
    y "Ты точно не моя совесть."

# game/status.rpy:1308
translate russia vibMenuJump_830b9aac:

    # s g18 "[playerName]? Did I hear you talking to someone...?"
    s g18 "[playerName]? Ты с кем-то разговаривал...?"

# game/status.rpy:1310
translate russia vibMenuJump_dad853f9:

    # y "Girls, I figured out a solution to our little nanobots problem."
    y "Девочки, я нашёл решение проблемы с наноботами."

# game/status.rpy:1311
translate russia vibMenuJump_feaae1c6:

    # y "I present you with...{w} THE V.I.B.!"
    y "Предстовляю вам...{w} V.I.B.!"

# game/status.rpy:1312
translate russia vibMenuJump_e113d65e:

    # "You hold up two toys to the girls."
    "Вы протягиваете две игрушки девочкам."

# game/status.rpy:1313
translate russia vibMenuJump_5bebad92:

    # c r18 "Is that...?"
    c r18 "Это...?"

# game/status.rpy:1314
translate russia vibMenuJump_59d8f64b:

    # s g16 "A toy? I don't see how this is going to help us, [playerName]."
    s g16 "Игрушка? Я не понимаю, как она должна нам помочь, [playerName]."

# game/status.rpy:1315
translate russia vibMenuJump_0199c4bb:

    # c r35 "Er... Sam...."
    c r35 "Эм... Сэм..."

# game/status.rpy:1316
translate russia vibMenuJump_2046dd3e:

    # "Sam picks up the toy and inspects it."
    "Сэм берёт игрушку и осматривает её."

# game/status.rpy:1317
translate russia vibMenuJump_d9812ee5:

    # s g41 "So is this like a gadg-...?"
    s g41 "Так, и это типа гадже-...?"

# game/status.rpy:1319
translate russia vibMenuJump_ee3829f6:

    # "{b}*Vibrates*{/b}"
    "{b}*Вибрирует*{/b}"

# game/status.rpy:1320
translate russia vibMenuJump_afbe56cc:

    # s g16 "Why is it vibrating...?"
    s g16 "Почему он вибрирует...?"

# game/status.rpy:1321
translate russia vibMenuJump_d6e712c6:

    # c "......................"
    c "......................"

# game/status.rpy:1322
translate russia vibMenuJump_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:1323
translate russia vibMenuJump_a88aa66e:

    # s "................"
    s "................"

# game/status.rpy:1324
translate russia vibMenuJump_5049bd8b:

    # s g28 "!!!"
    s g28 "!!!"

# game/status.rpy:1325
translate russia vibMenuJump_c31cc4fc:

    # s g30 "NO! NO WAY!"
    s g30 "НЕТ! НИЗА ЧТО!"

# game/status.rpy:1326
translate russia vibMenuJump_56303348:

    # s "Is this...?! We're suppose to...?! You expect us to...?!"
    s "Это...?! Мы нужно...?! Ты думаешь, что мы...?!"

# game/status.rpy:1327
translate russia vibMenuJump_f889c820:

    # s "I... I...! Clover! Say something!"
    s "Я... я...! Кловер! Скажи что-нибудь!"

# game/status.rpy:1328
translate russia vibMenuJump_2f46071a:

    # c r23 "Oooh~... it has five higher settings~...."
    c r23 "Ооох~... тут целых пять режимов~..."

# game/status.rpy:1329
translate russia vibMenuJump_de3b2ca0:

    # s g8 "Clover!"
    s g8 "Кловер!"

# game/status.rpy:1330
translate russia vibMenuJump_b637dee8:

    # c r3 "Oh Sam, calm down. It's just a vibrator."
    c r3 "Ох, Сэм успокойся. Это просто вибратор."

# game/status.rpy:1331
translate russia vibMenuJump_f48177a5:

    # s g4 "B-but...!"
    s g4 "Н-но...!"

# game/status.rpy:1332
translate russia vibMenuJump_74d05b1c:

    # s "{size=-10}But it's naughty...!{/size}"
    s "{size=-10}Но это же плохо...!{/size}"

# game/status.rpy:1333
translate russia vibMenuJump_5aaa9430:

    # c r38 "Listen Sam..."
    c r38 "Послушай Сэм..."

# game/status.rpy:1334
translate russia vibMenuJump_b3fcbf70:

    # c "We're not little girls anymore. We're full blown spies, working for one of the most powerful organizations on earth."
    c "Мы уже не маленькие девочки. Мы - полноценные шпионки, работающие на одну из самых могущественных организаций на Земле."

# game/status.rpy:1335
translate russia vibMenuJump_de278b87:

    # c "If anything, I'm surprised we haven't run into anything like this before."
    c "Если честно, я удивлена, что мы не сталкивались с подобным раньше."

# game/status.rpy:1336
translate russia vibMenuJump_105dcbd5:

    # s "{size=-10}But...{/size}"
    s "{size=-10}Но...{/size}"

# game/status.rpy:1337
translate russia vibMenuJump_61dd5f58:

    # c r12 "I mean, don't get me wrong...{w} Did you have to go with such a gaudy pink, [playerName]? That's like... {i}'so'{/i} not fashionable!"
    c r12 "Не пойми меня неправильно...{w} Обязательно было выбирать такой безвкусный розовый цвет, [playerName]? Это типа... {i}'вообще'{/i} немодно!"

# game/status.rpy:1338
translate russia vibMenuJump_2ff55530:

    # s g11 "............"
    s g11 "............"

# game/status.rpy:1339
translate russia vibMenuJump_44f933fb:

    # c "Just deal with it Sam. It's not like he's going to ask us to wear it to missions."
    c "Просто смирись с этим Сэм. Он же не собирается просить нас надевать его на миссии."

# game/status.rpy:1340
translate russia vibMenuJump_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:1341
translate russia vibMenuJump_fe9409bd:

    # c r16 "Right?"
    c r16 "Ведь так?"

# game/status.rpy:1342
translate russia vibMenuJump_9cb97b48_1:

    # y "........................"
    y "........................"

# game/status.rpy:1343
translate russia vibMenuJump_c33807f4:

    # c "Right....~?"
    c "Ведь так...~?"

# game/status.rpy:1344
translate russia vibMenuJump_3de565b1:

    # s g28 "[playerName]...! This is not funny!"
    s g28 "[playerName]...! Это не смешно!"

# game/status.rpy:1345
translate russia vibMenuJump_e14b7ca6:

    # y "Not only will you wear it on missions."
    y "Вы будете носить его не только на заданиях."

# game/status.rpy:1346
translate russia vibMenuJump_7c46ba01:

    # y "You will also wear it when going undercover."
    y "Вы также будете носить его, во время работы под прикрытием."

# game/status.rpy:1347
translate russia vibMenuJump_2dfbc557:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:1348
translate russia vibMenuJump_b6a26306:

    # c r30 "!!!"
    c r30 "!!!"

# game/status.rpy:1349
translate russia vibMenuJump_93706d17:

    # y "And when you're at the base."
    y "И когда вы на базе."

# game/status.rpy:1350
translate russia vibMenuJump_d393a302:

    # c "T-that's...!"
    c "Эт-это...!"

# game/status.rpy:1351
translate russia vibMenuJump_0d08de86:

    # y "And not only that..."
    y "И не только его..."

# game/status.rpy:1352
translate russia vibMenuJump_36152e15:

    # s "There's more...?"
    s "Есть ещё...?"

# game/status.rpy:1353
translate russia vibMenuJump_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1355
translate russia vibMenuJump_cf090b36:

    # c r29 "...............!"
    c r29 "...............!"

# game/status.rpy:1356
translate russia vibMenuJump_6455b90a:

    # s g28 "It's remote controlled...?!"
    s g28 "Он управляется дистанционно...?!"

# game/status.rpy:1357
translate russia vibMenuJump_a5fe66b2:

    # y "Whenever I suspect any of you to turn traitorous, I can use this to snap you out of it."
    y "Как только я заподозрю, что кто-то из вас переходит под контроль наноботов, с помощью этого гаджета я смогу подавить их активность."

# game/status.rpy:1358
translate russia vibMenuJump_014b7428:

    # c r11 "......"
    c r11 "......"

# game/status.rpy:1359
translate russia vibMenuJump_0f4dd8a9:

    # c r12 "He has a point..."
    c r12 "Он прав..."

# game/status.rpy:1360
translate russia vibMenuJump_af2430b1:

    # s g9 "Clover! Don't give in so easily!"
    s g9 "Кловер! Не поддавайся так легко!"

# game/status.rpy:1361
translate russia vibMenuJump_313f7455:

    # c "It's either this or helping in overthrowing the world."
    c "Либо это, либо помощь в свержении мира."

# game/status.rpy:1362
translate russia vibMenuJump_67b329ea:

    # s g40 "There has to be another way!"
    s g40 "Должен быть другой способ!"

# game/status.rpy:1363
translate russia vibMenuJump_0db45d84:

    # y "I'm sure there is, but we're sorta on a tight deadline here."
    y "Я уверен что есть, но у нас вроде как не так много времени."

# game/status.rpy:1364
translate russia vibMenuJump_89bfd39f:

    # s g31 "Well we could...! {w} How about instead we...{w} I'm sure that if we..."
    s g31 "Ну, мы могли бы...! {w}Как насчёт...{w} Я уверена, что если мы..."

# game/status.rpy:1365
translate russia vibMenuJump_375c59fb:

    # s g33 ".............................."
    s g33 ".............................."

# game/status.rpy:1366
translate russia vibMenuJump_82aae0cd:

    # c r22 "Gimme one of those..."
    c r22 "Дай мне одну..."

# game/status.rpy:1368
translate russia vibMenuJump_9f6b27a2:

    # "Clover takes one of the V.I.B.s and heads into a nearby room."
    "Кловер берёт один V.I.B. и направляется в соседнюю комнату."

# game/status.rpy:1369
translate russia vibMenuJump_9cb97b48_2:

    # y "........................"
    y "........................"

# game/status.rpy:1370
translate russia vibMenuJump_af5fad4e:

    # s "........................"
    s "........................"

# game/status.rpy:1371
translate russia vibMenuJump_cf3aa3c1:

    # s g38 "This is stupid, you can't just order us around like this."
    s g38 "Это глупо, ты не можешь вот так просто приказывать нам."

# game/status.rpy:1372
translate russia vibMenuJump_7d08e1ff:

    # y "I'm your boss, I'm pretty sure I can."
    y "Я твой босс. Я уверен, что могу."

# game/status.rpy:1373
translate russia vibMenuJump_7178d12a:

    # s g40 "No, you can't! We're not your slaves!"
    s g40 "Нет, не можешь! Мы не твои рабы!"

# game/status.rpy:1374
translate russia vibMenuJump_307ed6f3:

    # c "We might as well be at this point."
    c "На данный момент мы вполне на них похожи."

# game/status.rpy:1382
translate russia vibMenuJump_3536dc7f:

    # s g29 "C-clover...!"
    s g29 "К-кловер...!"

# game/status.rpy:1383
translate russia vibMenuJump_e9418ac4:

    # c "If we're going to save this world. Like it or not we need him."
    c "Если мы хотим спасти мир. Нравится нам это или нет, но он нам нужен."

# game/status.rpy:1384
translate russia vibMenuJump_c0a38de1:

    # s g30 "But..."
    s g30 "Но..."

# game/status.rpy:1385
translate russia vibMenuJump_e4e73519:

    # c "And he knows it. He can ask us to do whatever he wants."
    c "И он это знает. Он может просить нас делать всё, что захочет."

# game/status.rpy:1386
translate russia vibMenuJump_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:1387
translate russia vibMenuJump_5751b93f:

    # s "You... you wouldn't do that!"
    s "Ты... ты бы этого не сделал!"

# game/status.rpy:1388
translate russia vibMenuJump_10fe9619:

    # s "............."
    s "............."

# game/status.rpy:1389
translate russia vibMenuJump_fa5a4671:

    # s g29 "Would you...?"
    s g29 "Или...?"

# game/status.rpy:1392
translate russia vibMenuJump_f03eb316:

    # y "I just wanted to let you two know that we're all in the same boat."
    y "Просто хотел чтобы вы знали, мы всё в одной лодке."

# game/status.rpy:1393
translate russia vibMenuJump_f001385f:

    # y "I know that things look bleak right now, but you can trust me."
    y "Знаю, что сейчас всё выглядит мрачновато, но вы можете мне верить."

# game/status.rpy:1394
translate russia vibMenuJump_b424cc06:

    # s g1 "We can...?"
    s g1 "Мы можем...?"

# game/status.rpy:1395
translate russia vibMenuJump_9500a0b4_1:

    # y "......................"
    y "......................"

# game/status.rpy:1396
translate russia vibMenuJump_da8257de:

    # y "Okay you can {i}'probably'{/i} trust me."
    y "Ладно, {i}'вероятно'{/i} вы можете мне доверять."

# game/status.rpy:1397
translate russia vibMenuJump_b892467f:

    # y "Most likely."
    y "С большой вероятностью."

# game/status.rpy:1398
translate russia vibMenuJump_6bb725a0:

    # s g10 "................"
    s g10 "................"

# game/status.rpy:1399
translate russia vibMenuJump_0e3fb1f9:

    # y "Potentially."
    y "Потенциально."

# game/status.rpy:1400
translate russia vibMenuJump_f2cde862:

    # y "Possibly..."
    y "Возможно..."

# game/status.rpy:1401
translate russia vibMenuJump_34d5447b_1:

    # y "......................."
    y "......................."

# game/status.rpy:1402
translate russia vibMenuJump_3003834f:

    # y "I'd say about 50/50."
    y "Я бы сказал, где-то 50 на 50."

# game/status.rpy:1403
translate russia vibMenuJump_226fd1c0:

    # s g12 "Great, that {i}'really'{/i} set my mind at ease..."
    s g12 "Класс, теперь я {i}'точно'{/i} спокойна..."

# game/status.rpy:1406
translate russia vibMenuJump_72eaa01c:

    # y "YES! I got bitches!"
    y "ДА! У меня есть сучки!"

# game/status.rpy:1407
translate russia vibMenuJump_f8b64d02:

    # y "Time to feed me my grapes slaves~.... ♥♥♥"
    y "Время поесть виноград, мои рабы~.... ♥♥♥"

# game/status.rpy:1408
translate russia vibMenuJump_624577be:

    # c "Told ya."
    c "Говорила же."

# game/status.rpy:1410
translate russia vibMenuJump_8e7a7096:

    # s g28 "W-what! No! I'm nobody's slave!"
    s g28 "Ч-что! Нет! Я не рабыня!"

# game/status.rpy:1411
translate russia vibMenuJump_0d27074d:

    # c "All right Sam, then how about you leave and join the rest of the mindcontrolled masses?"
    c "Ладно Сэм, тогда как насчёт того, чтобы ты присоединилась к остальным зомбифицированным?"

# game/status.rpy:1412
translate russia vibMenuJump_7473093f:

    # s "But... but...!"
    s "Но... но...!"

# game/status.rpy:1413
translate russia vibMenuJump_2d651986:

    # "Sam looks around desperately."
    "Сэм в отчаянии оглядывается по сторонам."

# game/status.rpy:1414
translate russia vibMenuJump_02fbc625:

    # s g9 "..................."
    s g9 "..................."

# game/status.rpy:1415
translate russia vibMenuJump_d317ae0d:

    # s g15 "There's really no alternative. Is there...?"
    s g15 "Альтернативы действительно нет. Может...?"

# game/status.rpy:1416
translate russia vibMenuJump_16768da9:

    # c "Now you're getting it."
    c "Теперь ты это понимаешь."

# game/status.rpy:1418
translate russia vibMenuJump_e9e7518b:

    # y "I could order you to do all kinds of sick things!"
    y "Я могу приказать тебе сделать всё, что угодно!"

# game/status.rpy:1419
translate russia vibMenuJump_2dfbc557_1:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:1420
translate russia vibMenuJump_3a928ac8:

    # y "I mean I wouldn't, but I could."
    y "Конечно я бы не стал, но я могу."

# game/status.rpy:1421
translate russia vibMenuJump_6f5c2768:

    # s g29 "Oh..."
    s g29 "Ох..."

# game/status.rpy:1422
translate russia vibMenuJump_8a2fe986:

    # y "At least I probably wouldn't...."
    y "По крайней мере, наверное я бы не стал..."

# game/status.rpy:1423
translate russia vibMenuJump_496509c2:

    # s g31 "[playerName]..."
    s g31 "[playerName]..."

# game/status.rpy:1424
translate russia vibMenuJump_b07a2ecc:

    # y "Nah, okay real talk. I would {i}'totally'{/i} make you do all the weird stuff."
    y "Ну ладно, поговорим начистоту. Я бы {i}'точно'{/i} заставил тебя делать все эти странные вещи."

# game/status.rpy:1426
translate russia vibMenuJump_8068af10:

    # y "I could turn your cell into a dungeon..."
    y "Я могу превратить твою камеру в темницу..."

# game/status.rpy:1428
translate russia vibMenuJump_81153c8b:

    # s g28 "No!"
    s g28 "Нет!"

# game/status.rpy:1429
translate russia vibMenuJump_2225256b:

    # y "Or more likely, just have you do all my boring stuff, like file my taxes."
    y "Или, что более вероятно, просто попрошу тебя делать все мои скучные дела, например, подавать мою налоговую декларацию."

# game/status.rpy:1430
translate russia vibMenuJump_4dbc1507:

    # s g10 "............"
    s g10 "............"

# game/status.rpy:1431
translate russia vibMenuJump_d5e2634e:

    # s g12 "I still don't like it..."
    s g12 "Мне это всё ещё не нравится..."

# game/status.rpy:1435
translate russia vibMenuJump_f006c2d0:

    # c r38 "Like it or not. We're at his whims and it's going to be up to him to decide how he treats us."
    c r38 "Нравится тебе это или нет. Мы подчиняемся его прихотям, он сам будет решать, как с нами обращаться."

# game/status.rpy:1436
translate russia vibMenuJump_d17a6201:

    # c r11 "[playerName].... You got a lot of power over us now, okay?"
    c r11 "[playerName].... У тебя теперь много власти над нами, ясно?"

# game/status.rpy:1437
translate russia vibMenuJump_c8b02a21:

    # c "But we're still people. Forcing us to do things we don't want to, will have a negative impact. Plus, treating us like slaves and over-working us will affect our mood."
    c "Но мы всё ещё люди. Заставлять нас делать то, чего мы не хотим, будет иметь негативные последствия. Кроме того, обращение с нами как с рабами и чрезмерная занятость повлияют на наше настроение."

# game/status.rpy:1438
translate russia vibMenuJump_0a2f0c5f:

    # s g29 "Now hang on a minute! You don't {i}have{/i} to treat us like slaves."
    s g29 "Подождите минутку! Тебе не {i}нужно{/i} обращаться с нами как с рабами."

# game/status.rpy:1439
translate russia vibMenuJump_09b43c65:

    # s g1 "I bet we could become great friends if you don't abuse your power too much."
    s g1 "Уверена, мы можем стать отличными друзьями, если ты не будешь слишком злоупотреблять своей властью."

# game/status.rpy:1440
translate russia vibMenuJump_953d88b1:

    # y "I guess we'll see. For now, go put on the V.I.B."
    y "Посмотрим. А пока возьми V.I.B."

# game/status.rpy:1441
translate russia vibMenuJump_d284a606:

    # s g11 "Fine...."
    s g11 "Ладно..."

# game/status.rpy:1444
translate russia vibMenuJump_07cc5ad1:

    # c "............"
    c "............"

# game/status.rpy:1445
translate russia vibMenuJump_e4f07db3:

    # c "So you found this thing in the WOOHP database?"
    c "Так ты нашёл эту штуку в базе данных WOOHP?"

# game/status.rpy:1446
translate russia vibMenuJump_34fa9179:

    # y "Yup."
    y "Ага."

# game/status.rpy:1447
translate russia vibMenuJump_aeccb3c9:

    # c r23 "Always knew that Jerry was secretly a perv."
    c r23 "Всегда знала, что Джерри извращенец."

# game/status.rpy:1448
translate russia vibMenuJump_ac5c2703:

    # y "Jerry is the founder of WOOHP, right?"
    y "Джерри - основатель WOOHP, верно?"

# game/status.rpy:1449
translate russia vibMenuJump_e9608076:

    # c r49 "Yeah. The fact that we haven't heard anything about him makes me sorta worried."
    c r49 "Да. Меня беспокоит то что мы ничего о нём не знаем. Ни что с ним стало, ни где он сейчас."

# game/status.rpy:1450
translate russia vibMenuJump_12ede94b:

    # c "I hope that nothing bad happened to him..."
    c "Надеюсь, что с ним ничего не случилось..."

# game/status.rpy:1458
translate russia vibMenuJump_77bab620:

    # s "................."
    s "................."

# game/status.rpy:1459
translate russia vibMenuJump_abd3fc4b:

    # y "Got it sorted?"
    y "Разобрались?"

# game/status.rpy:1460
translate russia vibMenuJump_7ccc9528:

    # s "What do you think...?"
    s "Что думаешь...?"

# game/status.rpy:1461
translate russia vibMenuJump_7b71e744:

    # y "Good! Let's test these things out!"
    y "Отлично! Давайте проверим эту штуку!"

# game/status.rpy:1462
translate russia vibMenuJump_cb3c8129_1:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1464
translate russia vibMenuJump_5049bd8b_1:

    # s g28 "!!!"
    s g28 "!!!"

# game/status.rpy:1465
translate russia vibMenuJump_15c6cd00:

    # c r28 "!!!"
    c r28 "!!!"

# game/status.rpy:1466
translate russia vibMenuJump_1db49ca3:

    # y "........."
    y "........."

# game/status.rpy:1467
translate russia vibMenuJump_8153e9fe:

    # y "Judging by your reaction, it looks like they work."
    y "Судя по вашей реакции, они работают."

# game/status.rpy:1468
translate russia vibMenuJump_74d0f13e:

    # c r19 "A-and... these will measure our vital signs?"
    c r19 "И-и... он будет следить за нашими жизненными показателями?"

# game/status.rpy:1469
translate russia vibMenuJump_6005cbb6:

    # y "Yup. I got them right here!"
    y "Ага. Я вижу их прямо сейчас!"

# game/status.rpy:1470
translate russia vibMenuJump_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:1471
translate russia vibMenuJump_60eeee89:

    # y "It says you're aroused."
    y "Тут говориться о том, что вы возбуждены."

# game/status.rpy:1472
translate russia vibMenuJump_2d13b75c:

    # c "................"
    c "................"

# game/status.rpy:1473
translate russia vibMenuJump_3726d72e:

    # s g30 "[playerName]...! Can you pl-please turn it off now?!"
    s g30 "[playerName]...! Не мог бы ты п-пожалуйста, выключить его сейчас?!"

# game/status.rpy:1474
translate russia vibMenuJump_a7d7794e:

    # y "Oh right, sorry."
    y "Ох да, простите."

# game/status.rpy:1475
translate russia vibMenuJump_cb3c8129_2:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1476
translate russia vibMenuJump_e41bb355:

    # y "All right, well done girls."
    y "Отлично, молодцы девочки."

# game/status.rpy:1477
translate russia vibMenuJump_3f1a6a12:

    # y "Head back to your cells for now. We got a busy day tomorrow."
    y "Возвращайтесь пока в свои камеры. Завтра нас ждёт тяжёлый день."

# game/status.rpy:1478
translate russia vibMenuJump_075abb38:

    # "The girls nod and take their leave."
    "Девушки кивают и уходят."

# game/status.rpy:1499
translate russia vibMenuJump_8c648976:

    # y "Good morning Clover. How are you liking sleeping in an asylum?"
    y "Доброе утро, Кловер. Как тебе спиться в психушке?"

# game/status.rpy:1500
translate russia vibMenuJump_767d6a1e:

    # c r42 "Not as bad as I thought, but like... HEL-LO, hang up a mirror or something."
    c r42 "Не так плохо, как я думала, но типа... АЛ-ЛО, повесьте хотя бы зеркало или ещё что-нибудь."

# game/status.rpy:1501
translate russia vibMenuJump_3e5dc14b:

    # c "I simply can't operate without my hair looking in peak condition."
    c "Я не могу работать с беспорядком на голове."

# game/status.rpy:1502
translate russia vibMenuJump_fc4b0728:

    # y "Noted."
    y "Вас понял."

# game/status.rpy:1503
translate russia vibMenuJump_90b343b3:

    # c r18 "So what do you have planned for today?"
    c r18 "Так что ты запланировано на сегодня?"

# game/status.rpy:1504
translate russia vibMenuJump_ae79206e:

    # y "Well I've been sending Sam undercover at the different gangs around town."
    y "Ну обычно я отправляю Сэм под прикрытием в разные банды по всему городу."

# game/status.rpy:1505
translate russia vibMenuJump_d6436199:

    # y "I figured you'd do the same."
    y "Я думал что ты займёшься тем же самым."

# game/status.rpy:1507
translate russia vibMenuJump_ea20975a:

    # c r1 "Ooooh! I can't wait to go undercover with the Aces!"
    c r1 "Оооо! Не могу дождаться, когда буду работать под прикрытием у Тузов!"

# game/status.rpy:1508
translate russia vibMenuJump_f02d9388:

    # c "All those fashionable clothes, the fast cars, the big boats!"
    c "Вся эта модная одежда, быстрые машины, большие яхты!"

# game/status.rpy:1509
translate russia vibMenuJump_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:1510
translate russia vibMenuJump_4576da83:

    # y "Actually I was thinking of sending you undercover at the Drift Punks."
    y "Вообще-то я думал отправить тебя под прикрытием к Дрифт Панкам."

# game/status.rpy:1513
translate russia vibMenuJump_38a72463:

    # c r17 "W-what?! Those nerds?!"
    c r17 "Ч-что?! Эти гики?!"

# game/status.rpy:1514
translate russia vibMenuJump_6c5c6d72:

    # c r5 "Please [playerName] have mercy! I can't be seen around those kinds of people. It will ruin my reputation!"
    c r5 "Пожалуйста, [playerName] смилуйся! Я не могу находиться рядом с такими людьми. Это разрушит мою репутацию!"

# game/status.rpy:1515
translate russia vibMenuJump_bcf2dbe7:

    # y "They seem to have a thing for blondes. I bet you'd fit right in."
    y "Судя по всему им очень нравится блондинки. Ты идеально туда впишешься."

# game/status.rpy:1516
translate russia vibMenuJump_c87b44bf:

    # c "Please just send me anywhere but there!"
    c "Пожалуйста, отправь меня куда угодно, только не туда!"

# game/status.rpy:1517
translate russia vibMenuJump_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:1518
translate russia vibMenuJump_1a8a8888:

    # y "I'll think about it."
    y "Я подумаю об этом."

# game/status.rpy:1520
translate russia vibMenuJump_ea20975a_1:

    # c r1 "Ooooh! I can't wait to go undercover with the Aces!"
    c r1 "Оооо! Не могу дождаться, когда буду работать под прикрытием у Тузов!"

# game/status.rpy:1521
translate russia vibMenuJump_f02d9388_1:

    # c "All those fashionable clothes, the fast cars, the big boats!"
    c "Вся эта модная одежда, быстрые машины, большие яхты!"

# game/status.rpy:1522
translate russia vibMenuJump_6f323c89_1:

    # y "............."
    y "............."

# game/status.rpy:1523
translate russia vibMenuJump_87c2e8e6:

    # y "It's too bad we don't have a contact within the Drift Punks yet. I was hoping to send you there."
    y "Жаль что у нас пока нет контака в Дрифт Панках. Я надеялся отправить тебя туда."

# game/status.rpy:1526
translate russia vibMenuJump_38a72463_1:

    # c r17 "W-what?! Those nerds?!"
    c r17 "Ч-что?! Эти гики?!"

# game/status.rpy:1527
translate russia vibMenuJump_6c5c6d72_1:

    # c r5 "Please [playerName] have mercy! I can't be seen around those kinds of people. It will ruin my reputation!"
    c r5 "Пожалуйста, [playerName] смилуйся! Я не могу находиться рядом с такими людьми. Это разрушит мою репутацию!"

# game/status.rpy:1528
translate russia vibMenuJump_644e34b2:

    # c "Send me anywhere but there!"
    c "Пожалуйста, отправь меня куда угодно, только не туда!"

# game/status.rpy:1529
translate russia vibMenuJump_6f362d57_1:

    # y "..............."
    y "..............."

# game/status.rpy:1530
translate russia vibMenuJump_1a8a8888_1:

    # y "I'll think about it."
    y "Я подумаю об этом."

# game/status.rpy:1542
translate russia vibMenuJump_1ef581d3:

    # c "I'm back from working at Drift Punk..."
    c "Я вернулась с работы у Дрифт Панков..."

# game/status.rpy:1543
translate russia vibMenuJump_993bb9e8:

    # y "Let me guess. It was hell and you're never going back there."
    y "Дай угадаю. Это был ад, и ты никогда туда не вернёшься."

# game/status.rpy:1544
translate russia vibMenuJump_66aa0964:

    # c r11 "{b}*Scoffs*{/b} It wasn't {i}'that'{/i} bad. It was just really boring."
    c r11 "{b}*Насмешка*{/b} На самом деле было {i}'не так'{/i} плохо. Было очень скучно."

# game/status.rpy:1545
translate russia vibMenuJump_2daad5ed:

    # y "Boring?"
    y "Скучно?"

# game/status.rpy:1546
translate russia vibMenuJump_572bbec8:

    # c r12 "Yeah... Turns out they make most of their money hacking into bank accounts. Which... isn't exactly the most exciting way to commit a crime..."
    c r12 "Даа... Оказывается, они зарабатывают большую часть своих денег, взламывая банковские счета. Это... не самый захватывающий вид преступления..."

# game/status.rpy:1547
translate russia vibMenuJump_15d20f7c:

    # c "They tried to teach me, but I didn't really end up accomplishing much. I guess they were nice enough about it though. Even gave me this for my efforts."
    c "Они пытались меня чему-то научить, но у меня не особо получилось. Они были достаточно любезны и даже подарили мне это за мои старания."

# game/status.rpy:1551
translate russia vibMenuJump_9740aac9:

    # c "But what's there to say really... It's a place full of nerds, committing nerd crimes."
    c "Но что тут можно сказать на самом деле... Это место, полное ботаников и гиков, которые совершают странные преступления."

# game/status.rpy:1552
translate russia vibMenuJump_de28918c:

    # y "Any hint of who's running the operation?"
    y "Есть какие-нибудь намёки на то, кто руководит операцией?"

# game/status.rpy:1553
translate russia vibMenuJump_95c61931:

    # c r10 "No, not yet, but I feel like they wouldn't let me in on that kind of data on the first day."
    c r10 "Нее, пока что нет. Мне кажется, что в первый день они не допустили бы меня к такой информации."

# game/status.rpy:1554
translate russia vibMenuJump_f885de06:

    # y "All right well done. I'll be sure to send you back from time to time."
    y "Отличная работа. Я обязательно буду время от времени отправлять тебя туда."

# game/status.rpy:1555
translate russia vibMenuJump_0b26c9d7:

    # c r12 "I still rather go to the Aces..."
    c r12 "Я всё ещё предпочитаю работать у Тузов..."

# game/status.rpy:1556
translate russia vibMenuJump_55519da1:

    # y "Yeah yeah, less complaining."
    y "Да-да, хватит ныть."

# game/status.rpy:1557
translate russia vibMenuJump_1f106f37:

    # y "Head back to your cell for now."
    y "Возвращайся пока в свою камеру."

# game/status.rpy:1559
translate russia vibMenuJump_b5a4d5d4:

    # "Clover shrugs and heads back to her cell."
    "Кловер пожимает плечами и возвращается в свою камеру."

# game/status.rpy:1568
translate russia task3_1346f0d5:

    # "INACCESIBLE, PLAYERS CAN'T SEE THIS."
    "НЕДОСТУПНО, ИГРОКИ НЕ МОГУТ ЭТОГО ВИДЕТЬ."

# game/status.rpy:1574
translate russia task3_441218c7:

    # y "All right, head to the Aces today."
    y "Отлично, отправляйся сегодня к Тузам."

# game/status.rpy:1575
translate russia task3_de7d0c1f:

    # sM "............................"
    sM "............................"

# game/status.rpy:1576
translate russia task3_6e167fe1:

    # y "No...?"
    y "Нет...?"

# game/status.rpy:1577
translate russia task3_64677eb1:

    # sM "No it's fine. It's just that they're having a party today."
    sM "Нет, всё в порядке. Просто у них сегодня вечеринка."

# game/status.rpy:1578
translate russia task3_d393bf9e:

    # y "Oh that doesn't sound so bad."
    y "Ох, звучит не так уж и плохо."

# game/status.rpy:1579
translate russia task3_580f263d:

    # sM "No no, it's not, but..."
    sM "Нет, нет, это не так. Но..."

# game/status.rpy:1580
translate russia task3_f139af01:

    # sM "Well... I don't normally go to parties.{w} I prefer to stay at home and do some reading."
    sM "Ну... обычно я не хожу на вечеринки.{w} Я предпочитаю оставаться дома и заниматься чтением."

# game/status.rpy:1581
translate russia task3_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:1582
translate russia task3_d0441335:

    # sM "So er... Got any tips for me?"
    sM "Так что... Есть какие-нибудь советы для меня?"

# game/status.rpy:1583
translate russia task3_04053a51:

    # y "Look at you, being a social shutin."
    y "Посмотри на себя, ты - социально замкнутый человек."

# game/status.rpy:1584
translate russia task3_0fc3fbef:

    # sM "[playerName]... I'm serious."
    sM "[playerName]... Я серьезно."

# game/status.rpy:1585
translate russia task3_01447778:

    # y "Just relax and have a good time. Do some socializing. It's much easier to gather intel from someone while they're drunk."
    y "Просто расслабься и хорошо проведи время. Пообщайся. Гораздо легче получить информацию от человека, когда он пьян."

# game/status.rpy:1586
translate russia task3_6ed368fe:

    # sM "And what if someone tries to make a move on me?"
    sM "А что, если кто-то попытается ко мне подкатить?"

# game/status.rpy:1587
translate russia task3_d1642ce7:

    # y "Huh?"
    y "Хмм?"

# game/status.rpy:1588
translate russia task3_5e57db67:

    # sM "You know... What if someone tries to get touchy or tries to... y'know...{w} kiss..."
    sM "Аа... Что делать, если кто-то начнёт распускать руки или попытается... ну знаешь...{w} поцеловать мня..."

# game/status.rpy:1590
translate russia task3_57cef088:

    # y "(Hm...)"
    y "(Хм...)"

# game/status.rpy:1591
translate russia task3_706961ed:

    # y "(On one hand, being slutty might make things easier for us. Sam is a bit of a prude and getting her more in touch with her slutty side could lead to more money and intel in the future.)"
    y "(С одной стороны, распущенность может облегчить нам жизнь. Сэм немного сжата, но если она начнёт раскрывать свою распутную сторону, то в будущем это может привести к большим деньгам и сведениям.)"

# game/status.rpy:1592
translate russia task3_49b865b0:

    # y "(On the other hand, do I really want my spies to go sleeping around with the rest of the world...?)"
    y "(С другой стороны, действительно ли я хочу, чтобы мои шпионки спали со всем миром...?)"

# game/status.rpy:1597
translate russia task3_ae7f22e8:

    # y "Well..."
    y "Нуу..."

# game/status.rpy:1598
translate russia task3_cd2608ff:

    # y "Being a little flirty might loose some lips."
    y "Лёгкая кокетливость может развязать некоторым парням язык."

# game/status.rpy:1599
translate russia task3_c5f3c506:

    # sM "But I wouldn't want to go too far of course."
    sM "Но я, конечно не хотела бы заходить слишком далеко."

# game/status.rpy:1600
translate russia task3_d32f8ecc:

    # y "Of course."
    y "Конечно."

# game/status.rpy:1601
translate russia task3_fb1c4d10:

    # sM "I'll give it a try... I'll just flutter my eyelashes, smile coyly at them and have them buy me a drink."
    sM "Я попробую... Я просто поиграю ресничками, мило поулыбаюсь и они угостят меня выпивкой."

# game/status.rpy:1602
translate russia task3_565cb9c0:

    # sM "Then when I got what I need, I'll figure out an excuse to get out of there.{w} I've seen Clover do it plenty of times."
    sM "Потом, когда я получу то, что мне нужно, я придумаю предлог, чтобы уйти оттуда.{w} Я много раз видела как Кловер это делала."

# game/status.rpy:1604
translate russia task3_a5c6491e:

    # y "Absolutely not! No promiscuity!"
    y "Ни в коем случае! Никакой распущенности!"

# game/status.rpy:1605
translate russia task3_c29fe303:

    # sM "Oh! I'm surprised. I figured you'd encourage it."
    sM "Ох! Ты меня удивил, я думала, что ты будешь только за."

# game/status.rpy:1606
translate russia task3_86501249:

    # sM "Well okay then. I'll make sure to keep my guard up."
    sM "Ну тогда ладно. Я постараюсь быть начеку."

# game/status.rpy:1607
translate russia task3_ccee84cd:

    # sM "Okay, got it! Thanks for the advice, [playerName]."
    sM "Хорошо, поняла! Спасибо за совет, [playerName]."

# game/status.rpy:1609
translate russia task3_e1571174:

    # "Sam leaves for the Aces party."
    "Сэм уходит на вечеринку Тузов."

# game/status.rpy:1622
translate russia task3_9564c702:

    # "Incoming transmission."
    "Входящий звонок."

# game/status.rpy:1623
translate russia task3_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:1624
translate russia task3_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:1630
translate russia task3_be82cfa6:

    # s "[playerName]..? Are you there?"
    s "[playerName]...? Ты тут?"

# game/status.rpy:1631
translate russia task3_02438e9c:

    # y "Hey Sam, enjoying the party?"
    y "Привет Сэм, наслаждаешься обстановкой?"

# game/status.rpy:1632
translate russia task3_dfebfaba:

    # s g34 "I'm in Spain."
    s g34 "Я в Испании."

# game/status.rpy:1634
translate russia task3_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:1635
translate russia task3_ab0bd2ce:

    # y "SPAIN?!"
    y "ИСПАНИЯ?!"

# game/status.rpy:1636
translate russia task3_00d9cad0:

    # y "AS IN THE COUNTRY?!"
    y "ТИПА В СТРАНЕ?!"

# game/status.rpy:1637
translate russia task3_bf9bdd8b:

    # s "Yes."
    s "Да."

# game/status.rpy:1638
translate russia task3_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:1639
translate russia task3_794a8240:

    # y "WHAT ARE YOU DOING IN SPAIN?!"
    y "что ты делаешь в испании?!"

# game/status.rpy:1640
translate russia task3_0871c314:

    # s g37 "So it turns out these guys don't do things half way... big surprise."
    s g37 "Оказывается, эти парни ко всему относятся серьёзно... неожиданно."

# game/status.rpy:1641
translate russia task3_ab01d04f:

    # s g32 "I'm in a privately owned castle off the coast of Valencia. They flew me out here to come party with them."
    s g32 "Я нахожусь в частном замке у побережья Валенсии. Мы прилетели сюда на вечеринку."

# game/status.rpy:1642
translate russia task3_05912b67:

    # s "I can't talk for long, I'll be back tomorrow with a full report.{w} Sam out."
    s "Не могу долго говорить, я вернусь завтра с полным отчётом.{w} Отбой."

# game/status.rpy:1650
translate russia task3_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:1651
translate russia task3_8e89dc9b:

    # y "A private castle in Spain...{w} Those rich bastards..."
    y "Частный замок в Испании...{w} Богатые ублюдки..."

# game/status.rpy:1667
translate russia task3_c600aea3:

    # s "Hola."
    s "Хола."

# game/status.rpy:1668
translate russia task3_7c9ed468:

    # y "Hola to you too.{w} How was the Mediterranean?"
    y "Хола.{w} Как там на Средиземноморье?"

# game/status.rpy:1669
translate russia task3_80796498:

    # s g28 "It...{w} was...{w} AMAZING!"
    s g28 "Это...{w} было...{w} ВЕЛИКОЛЕПНО!"

# game/status.rpy:1670
translate russia task3_339c0fe1:

    # y "Oh, you liked the party?"
    y "Оо, тебе понравилась вечеринка?"

# game/status.rpy:1671
translate russia task3_aec3ecb4:

    # s g29 "Oh right, I forgot about the party. No I mean Valencia was amazing!"
    s g29 "Ах да, я забыла о вечеринке. Нет, я имею в виду, что Валенсия была потрясающей!"

# game/status.rpy:1672
translate russia task3_e9fd7707:

    # s "All the little nooks and crannies, the architecture, the culture!"
    s "Все маленькие закоулки и укромные уголки, архитектура, культура!"

# game/status.rpy:1673
translate russia task3_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:1674
translate russia task3_36ee8c75:

    # s "They had museums and the castle I was staying at dated back to the 13th century!"
    s "Там есть музеи, а замок, в котором я остановилась, датируется 13-м веком!"

# game/status.rpy:1675
translate russia task3_40da0333:

    # s g28 "They had a painting there that was over 300 years old!!!"
    s g28 "У них там была картина, которой было более 300 лет!!!"

# game/status.rpy:1676
translate russia task3_45a9abef:

    # y "Okay... But did you learn anything?"
    y "Хорошо... Но узнала ли ты что-нибудь?"

# game/status.rpy:1677
translate russia task3_40a92bd2:

    # s g31 "Huh? Oh right..."
    s g31 "Хмм? Ах, точно..."

# game/status.rpy:1678
translate russia task3_c677d6e5:

    # s "Yes, I did."
    s "Да, узнала."

# game/status.rpy:1679
translate russia task3_27bb7f32:

    # s "Whoever was hosting the party is a bigshot within the Aces. He finances them and invites their top brass out to Spain on a regular basis."
    s "Тот что устроил вечеринку - большая шишка в Тузах. Он финансирует их и регулярно приглашает высшее руководство Тузов в Испанию."

# game/status.rpy:1680
translate russia task3_ff2178d5:

    # y "Any idea who it is?"
    y "Есть идеи, кто это?"

# game/status.rpy:1681
translate russia task3_eb12e5e7:

    # s g32 "No... I wasn't allowed to go see him, but I did see an old familiar face..."
    s g32 "Нет... Меня не пустили к нему, но я увидела одно знакомое лицо..."

# game/status.rpy:1682
translate russia task3_b32c1e0f:

    # s "Maggie T."
    s "Мэгги Т."

# game/status.rpy:1683
translate russia task3_8220a558:

    # y "Maggie T?"
    y "Мэгги Т.?"

# game/status.rpy:1684
translate russia task3_a54f0c5a:

    # s "We've fought her before. She's just an interior designer, but got famous after trying to assassinate the US President."
    s "Мы уже сражались с ней раньше. Она всего лишь дизайнер интерьера, но прославилась после покушения на президента США."

# game/status.rpy:1685
translate russia task3_0c067791:

    # s "She's been rolling in money ever since. The Aces are probably offering her a position on their gang in return for funding."
    s "С тех пор она зарабатывает деньги. Скорее всего, Тузы предлагают ей место в своей банде в обмен на финансирование."

# game/status.rpy:1686
translate russia task3_eaf3a6bb:

    # s "Meanwhile I had other things to worry about."
    s "Тем временем у меня были другие заботы."

# game/status.rpy:1687
translate russia task3_0200f59a:

    # s g37 "Namely boys..."
    s g37 "А именно мальчики..."

# game/status.rpy:1688
translate russia task3_1da9ae52:

    # y "Boys? Sam, I am disappointed in you. To prioritize boys over your mission!"
    y "Мальчики? Сэм, я разочарован в тебе. Мальчики в приоритете над твоей миссии!"

# game/status.rpy:1689
translate russia task3_93fa6d77:

    # y "I mean Clover? Sure, I can see her doing it, but you?"
    y "Конечно я могу понять, как Кловер это делает, но ты?"

# game/status.rpy:1690
translate russia task3_bed553de:

    # s g28 "N-no! It's not what you think. I was busy keeping them off of me!"
    s g28 "Н-нет! Это не то о чём ты подумал. Я была занят тем, что не подпускал их к себе!"

# game/status.rpy:1691
translate russia task3_d1642ce7_1:

    # y "Huh?"
    y "Хм?"

# game/status.rpy:1693
translate russia task3_bd646bed:

    # s g37 "You know how you suggested to be more flirty?"
    s g37 "Помнишь, ты предлагал быть более кокетливой?"

# game/status.rpy:1694
translate russia task3_7bca4ac1:

    # s "Well I tried. Which by the way...{w} I'm not very good at."
    s "Ну, я пыталась. Что, кстати...{w} у меня не очень хорошо получается."

# game/status.rpy:1695
translate russia task3_02c8bf3b:

    # s "But anyways I flirted around with a few guys, then more and more started showing up!"
    s "Но так или иначе, я флиртовала с несколькими парнями, а потом их стало появляться всё больше и больше!"

# game/status.rpy:1696
translate russia task3_db17d438:

    # s g15 "I spent most my time hiding! How does Clover do it...?!"
    s g15 "Я почти всё время пряталась! Как Кловер это делает...?!"

# game/status.rpy:1697
translate russia task3_435f9fc6:

    # s g1 "I mean it wasn't all bad. I gathered some good intel...{w}{size=-10} And the attention was kind of nice...{/size}"
    s g1 "Всё было не так уж и плохо. Я собрала много информации...{w}{size=-10} Да и внимание было довольно приятным...{/size}"

# game/status.rpy:1698
translate russia task3_3396197f:

    # s "Here... this is what I've learned."
    s "Вот... вот что я нашла."

# game/status.rpy:1702
translate russia task3_7b75c1e3:

    # s g38 "As you suggested, I wouldn't be flirty and kept everyone who approached me at arm's length."
    s g38 "Как ты и советовал, я не кокетничала и держала всех, на расстоянии вытянутой руки."

# game/status.rpy:1703
translate russia task3_30985439:

    # s "But they just saw that as playing hard-to-get!"
    s "Но они только посчитали это игрой в недотрогу!"

# game/status.rpy:1704
translate russia task3_637eb2e5:

    # s "It wasn't made any easier by some jealous girls who thought I was hitting on their men!"
    s "Было только сложнее из-за ревнивых девушек, которые думали, что я пристаю к их мужчинам!"

# game/status.rpy:1705
translate russia task3_eca464a7:

    # s g10 "Eventually I just ended up hiding for most of the party instead of gathering intel."
    s g10 "В итоге я просто пряталась большую часть вечеринки вместо того, чтобы собирать информацию."

# game/status.rpy:1706
translate russia task3_671d53cf:

    # y "Well it's good to have you back State side. I might send you girls out to do some missions at the castle from now on."
    y "Хорошо, что ты вернулась домой. Может теперь я буду посылать вас, девочки, выполнять задания в замке."

# game/status.rpy:1707
translate russia task3_46e47727:

    # s g1 "{b}*Nods*{/b} I was thinking the same thing. Let's catch the creep who's supplying them money!"
    s g1 "{b}*Кивает*{/b} Я думала о том же. Давайте поймаем мерзавца, который снабжает их деньгами!"

# game/status.rpy:1713
translate russia task3_458112c4:

    # "You've unlocked {color=#ffeda6}Castle Fiebre del Oro{/color}. You can now sent your spies there for missions."
    "Вы разблокировали {color=#ffeda6}Замок \"Золотая лихорадка\"{/color}. Теперь вы можете отправлять туда своих шпионов для выполнения заданий."

# game/status.rpy:1714
translate russia task3_bcda24a5:

    # "New clothing options for Sam are availabe at the Mall."
    "Новые варианты одежды для Сэм можно приобрести в торговом центре."

# game/status.rpy:1737
translate russia task3_9cd5115a:

    # sM "Nowhere left to run, Maggie!"
    sM "Бежать больше некуда, Мэгги!"

# game/status.rpy:1738
translate russia task3_308f58a7:

    # mag "You pesky girls!{w} Guards get them!"
    mag "Как же вы надоели!{w} Стража забирает их!"

# game/status.rpy:1739
translate russia task3_8c231e0b:

    # mag ".............."
    mag ".............."

# game/status.rpy:1740
translate russia task3_b243112a:

    # mag "Guards?"
    mag "Стража?"

# game/status.rpy:1741
translate russia task3_c3e6f5e4:

    # y "We got her n-..."
    y "Мы взяли её н-..."

# game/status.rpy:1742
translate russia task3_946ab664:

    # y "Wait...{w} Her eyes are red."
    y "Подождите...{w} У неё красные глаза."

# game/status.rpy:1743
translate russia task3_a890e2fc:

    # cM "Is she... under nanobot control aswell?"
    cM "Она... тоже под контролем наноботов?"

# game/status.rpy:1744
translate russia task3_0e0695f3:

    # y "Only one way to find out. Take her in and we'll interrogate her at the base."
    y "Есть только один способ узнать это. Заберите её, мы допросим её на базе."

# game/status.rpy:1745
translate russia task3_0954199e:

    # sM "Got it!"
    sM "Хорошо!"

# game/status.rpy:1746
translate russia task3_3c1f179c:

    # mag "Uh oh..."
    mag "Ой-ой..."

# game/status.rpy:1765
translate russia task3_b01a3762:

    # c "We're back."
    c "Мы вернулись."

# game/status.rpy:1766
translate russia task3_cdf122ae:

    # y "Good, just put her in one of the empty cells. I'll go talk to her when I'm ready."
    y "Отлично, посадите её в одну из пустых камер. Я поговорю с ней, когда буду готов."

# game/status.rpy:1767
translate russia task3_054afdb1:

    # c "Understood. Don't wait too long before questioning her."
    c "Хорошо. Только не затягивай с допросом."

# game/status.rpy:1782
translate russia task3_3deac0a5:

    # y "Well well well... If it isn't the infamous Maggie T. IF THAT IS YOUR REAL NAME!"
    y "Так-так-так... Если это не печально известная Мэгги Т. если это твоё настоящее имя!"

# game/status.rpy:1783
translate russia task3_e607b329:

    # mag "................."
    mag "................."

# game/status.rpy:1784
translate russia task3_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:1785
translate russia task3_e6edf295:

    # y "Odd, I expected an evil monologue or something."
    y "Странно, я ожидал злобного монолога или чего типа того."

# game/status.rpy:1787
translate russia task3_f3308085:

    # s "Hey, [playerName]... There's something off about her. See her eyes?"
    s "Привет, [playerName]... С ней что-то не так. Видел её глаза?"

# game/status.rpy:1788
translate russia task3_2546c64d:

    # y "They're red... Does that mean...?"
    y "Они красные... Значит ли это...?"

# game/status.rpy:1789
translate russia task3_333a55f6:

    # s "She's being controlled by nanobots aswell!"
    s "Она тоже под контролем наноботов!"

# game/status.rpy:1790
translate russia task3_bf417a25:

    # mag "Soon the world will fall under the control of the mastermind, and there's nothing you can d-...."
    mag "Вскоре весь мир окажется под контролем Организатора, вы ничего не сможете сделать-..."

# game/status.rpy:1791
translate russia task3_bafa6853:

    # "{b}*Zip*{/b}"
    "{b}*Зип*{/b}"

# game/status.rpy:1792
translate russia task3_c23daca7:

    # mag "Why are you unzipping your pants...?!"
    mag "Почему ты расстёгиваешь штаны...?!"

# game/status.rpy:1793
translate russia task3_d1eb730d:

    # s g28 "[playerName]! You can't do that against her will! We're the good guys, remember?"
    s g28 "[playerName]! Ты не можешь сделать это против её воли! Мы хорошие, помнишь?"

# game/status.rpy:1794
translate russia task3_3c44df1a:

    # y "Who said anything about doing it against her will?"
    y "Кто сказал, что это делается против её воли?"

# game/status.rpy:1795
translate russia task3_14f83683:

    # y "Hey Maggie, you're currently being controlled by nanobots, but we can break you out of it."
    y "Эй Мэгги, в настоящее время ты под контролем наноботов, но мы можем вывести тебя из этого состояния."

# game/status.rpy:1796
translate russia task3_75034df7:

    # mag "What?! Nobody is controlling the great Maggie Trendset!"
    mag "Что?! Никто не можнет контролировать великую Мэгги Трэндсет!"

# game/status.rpy:1797
translate russia task3_13ff2d95:

    # y "We can prove it, but we'll have to break you out of it with a quick fuc-.... {w}Wait the T. stands for Trendset?"
    y "Я легко могу это доказать, но нам придётся вывести тебя из этого состояния с помощью быстрого тра-... {w}Подождите Т. означает Трэндсет?"

# game/status.rpy:1798
translate russia task3_82b398bc:

    # y "That's a stupid name."
    y "Какое глупое имя."

# game/status.rpy:1799
translate russia task3_f51bb873:

    # mag "Hey!"
    mag "Эй!"

# game/status.rpy:1800
translate russia task3_552fe57e:

    # mag "Fine... I've been feeling more... murderous than usual. If you say a quick shag will break me out of it, then I'm willing to give it a try..."
    mag "Прекрасно... Чувствую себя более... кровожадно, чем обычно. Если ты говоришь, что быстрый трах избавит меня от этого, то я готова попробовать..."

# game/status.rpy:1801
translate russia task3_b1abc812:

    # s g33 "I don't wanna watch this...."
    s g33 "Я не хочу на это смотреть..."

# game/status.rpy:1807
translate russia task3_c3ad511c:

    # mag "Oh! Oh! Oh!"
    mag "Ох! Аах! Ааа!"

# game/status.rpy:1808
translate russia task3_01dff492:

    # mag "Oooh yes, I'm feeling better already!"
    mag "Ооох да, я уже чувствую себя лучше!"

# game/status.rpy:1809
translate russia task3_8bca739a:

    # mag "In my {i}'what'{/i}?! Well just this once then~... ♥"
    mag "{i}'К-куда'{/i}?! Ну, тогда только в этот раз~... ♥"

# game/status.rpy:1810
translate russia task3_6e99e0a0:

    # mag "Ahh! Ahhhhhh! ♥♥♥"
    mag "Аахх! АаААааахХХ! ♥♥♥"

# game/status.rpy:1811
translate russia task3_07309111:

    # y "Say cheese..."
    y "Скажи сыр..."

# game/status.rpy:1821
translate russia task3_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:1826
translate russia task3_d84fe841:

    # mag "I... I can't believe they were controlling me..."
    mag "Я... Я не могу поверить, что они контролировали меня..."

# game/status.rpy:1827
translate russia task3_db1efe85:

    # y "You've been working as a lieutenant for a gang here in town all this time."
    y "Всё это время ты работала лейтенантом в банде здесь, в городе."

# game/status.rpy:1828
translate russia task3_78f7ead8:

    # mag "To use me like a puppet! I'll get them for this!"
    mag "Использовать меня как марионетку?! Я их достану!"

# game/status.rpy:1829
translate russia task3_4ba12349:

    # mag "I WILL TEACH THEM NOT TO MESS WITH MAGGIE TRENDSET!"
    mag "Я НАУЧУ ИХ НЕ СВЯЗЫВАТЬСЯ С МЭГГИ ТРЭНДСЕТ!"

# game/status.rpy:1830
translate russia task3_721e8921:

    # y "Okay okay, calm down. If it's revenge you want, we can help you with that, but first you need to tell us everything you know."
    y "Ладно, ладно, успокойся. Если хочешь отомстить, мы можем помочь тебе, но сначала тебе нужно рассказать нам всё что знаешь."

# game/status.rpy:1831
translate russia task3_b760d0e3:

    # mag "Well..."
    mag "Нуу..."

# game/status.rpy:1848
translate russia task3_f5e2949c:

    # mag "The Aces, like most big gangs around town have two lieutenants. Only one now that you've freed me from the nanobot control."
    mag "У Тузов, как и у большинства крупных банд в городе, есть два лейтенанта. У Тузов теперь остался только один."

# game/status.rpy:1853
translate russia task3_1f9a001f:

    # mag "If you want to bring down the gang, you're going to have to capture the other officer and then confront the gangleader."
    mag "Если вы хотите уничтожить банду, вам придётся захватить второго офицера, а затем противостоять главарю банды."

# game/status.rpy:1854
translate russia task3_01af6333:

    # y "And how do we do that?"
    y "И как нам это сделать?"

# game/status.rpy:1855
translate russia task3_a410b898:

    # mag "The lieutenants will stay hidden for the most part. You need to draw them out."
    mag "Лейтенанты по большей части остаются в тени. Вам прийдётся их выманить."

# game/status.rpy:1868
translate russia task3_f0a7402c:

    # mag "Every 30 days, there is something called a {color=#ffeda6}'Landgrab'{/color}.{w} The gangs will fight amongst each other for control over different neighborhoods."
    mag "Каждые 30 дней происходит то, что называется {color=#ffeda6}'Войной за территорию'{/color}.{w} Банды будут бороться между собой за контроль над различными районами."

# game/status.rpy:1879
translate russia task3_bdf88f06:

    # mag "You need to capture Agents to fight for you during the landgrab."
    mag "Вам необходимо захватить агентов, которые будут сражаться за вас во время войны за территорию."

# game/status.rpy:1880
translate russia task3_b5b0aa81:

    # y "We can make {color=#ffeda6}Hypno Earrings{/color} and use those to capture Agents during missions."
    y "Мы можем сделать {color=#ffeda6}Гипно-Серьги{/color} и использовать их для захвата агентов во время миссий."

# game/status.rpy:1881
translate russia task3_61849d80:

    # mag "Once a gang begins to lose a lot of ground, their lieutenants will reveal themselves. If your reputation in the gang is high enough, you might get close enough to capture them."
    mag "Как только банда начинает терять свои позиции, её лейтенанты раскрываются. Если ваша репутация в банде достаточно высока, вы можете подобраться достаточно близко, чтобы схватить их."

# game/status.rpy:1882
translate russia task3_3b266849:

    # y "So keep sending my spies undercover and once every 30 days fight in a Landgrab?"
    y "Значит я продолжаю посылать шпионок под прикрытием и раз в 30 дней принимать участие в войне за территорию?"

# game/status.rpy:1883
translate russia task3_4b6e2935:

    # y "Do I have to wait that long?"
    y "Мне придётся ждать так долго?"

# game/status.rpy:1884
translate russia task3_5e15dc98:

    # mag "You can skip to the next Landgrab via the Agents button."
    mag "Ты можешь перейти к следующей Войне с помощью кнопки Agents."

# game/status.rpy:1888
translate russia task3_99d96ada:

    # mag "You're going to have to build up an army though. These Landgrabs can get pretty hairy and you'll loose agents to nanobot control."
    mag "Однако тебе придётся создать армию. Война за территорию может стать довольно кровопролитной, и вы потеряете своих агентов."

# game/status.rpy:1889
translate russia task3_001f5c38:

    # y "Gather more WOOHP agents and break them out of their nanobot control in the basement. Got it."
    y "Просто захватим больше агентов и выведем их из-под контроля наноботов в подвале. Всё схвачено."

# game/status.rpy:1898
translate russia task3_dba2607f:

    # mag "......................"
    mag "......................"

# game/status.rpy:1899
translate russia task3_2736c9d1:

    # mag "So......"
    mag "Иии..."

# game/status.rpy:1900
translate russia task3_6171fd1e:

    # mag "I'd say I helped you out quite a bit...{w} Any chance of letting me go?"
    mag "Я вроде как довольно сильно тебе помогла...{w} Есть шанс, что ты меня отпустишь?"

# game/status.rpy:1901
translate russia task3_3afcf255:

    # y "Not a chance."
    y "Без шансов."

# game/status.rpy:1902
translate russia task3_d175e19b:

    # mag "Curses...."
    mag "Проклятие..."

# game/status.rpy:1911
translate russia task3_c77c3148:

    # "Landgrabs happen ever 30 days. Currently it's [timetograb] more days until the next one."
    "Война за территорию происходит каждые 30 дней. До следующей войны осталось [timetograb] дней."

# game/status.rpy:1912
translate russia task3_b22b20b4:

    # "During a Landgrab, you can send freed WOOHP agents to fight for you in the streets and try to bring enemy lieutenants out of hiding."
    "Во время Войны вы можете посылать освобожденных агентов WOOHP сражаться за вас на улицах и попытаться вывести вражеских лейтенантов из укрытия."

# game/status.rpy:1913
translate russia task3_fba4425f:

    # "However, you can lose agents during the fight. Agents can be captured by planning missions and using the {color=#ffeda6}Hypno Earring{/color} gadget."
    "Однако вы можете потерять агентов во время боя. Агенты могут быть захвачены вовремя миссий при использовании {color=#ffeda6}Гипно-Серьг{/color}."

# game/status.rpy:1914
translate russia task3_3dd9a308:

    # "After they've been captured and send to the base, you can break them out of their nanobot control by throwing down dirty magazines into the basement."
    "После их захвата и отправления на базу, вы сможете сломить контроль наноботов, сбросив в подвал грязные журналы."

# game/status.rpy:1916
translate russia task3_0baf7e97:

    # "You can see when the next landgrab is by clicking on the 'Agents' button at the top of the screen. Here you can also choose to skip to the next Landgrab in case you don't want to wait."
    "Вы можете узнать, когда будет следующая война за территорию, нажав на кнопку \"Agents\" в верхней части экрана. Здесь вы также можете выбрать переход к следующей Войне, если вы не хотите ждать."

# game/status.rpy:1917
translate russia task3_6626d1fc:

    # "Spies send undercover during a Landgrab earn a reputation boost, but risk getting hurt on the job."
    "Шпионки, работающие под прикрытием во время войны за территории, повышают репутацию, но рискуют пострадать во время работы."

# game/status.rpy:1934
translate russia task3_00040114:

    # y "Company meeting!"
    y "Общее собрание!"

# game/status.rpy:1939
translate russia task3_30687c86:

    # y "Today I wanted to talk abou-..."
    y "Сегодня я хочу поговорить о-..."

# game/status.rpy:1940
translate russia task3_d361245c:

    # "{b}*Ting* *Ting*{/b}"
    "{b}*Дзынь* *Дзынь*{/b}"

# game/status.rpy:1941
translate russia task3_29bc5049:

    # "???" "Hello?"
    "???" "Есть кто?"

# game/status.rpy:1942
translate russia task3_aa785824:

    # y "Sorry, we're not open ye-...."
    y "Извините, мы ещё закры-..."

# game/status.rpy:1958
translate russia task3_e71b99db:

    # "Agent 1" "What a lovely milkshake bar you have here!"
    "Агент 1" "Какой у вам прекрасный бар с молочными коктейлями!"

# game/status.rpy:1959
translate russia task3_ee99fffe:

    # y "Er..."
    y "Эм..."

# game/status.rpy:1963
translate russia task3_21cfb035:

    # "Agent 2" "Quite the milkshake bar you found here fellow agent!"
    "Агент 2" "Вы нашли отличный бар с молочными коктейлями коллега!"

# game/status.rpy:1964
translate russia task3_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:1968
translate russia task3_d7d1fb09:

    # "Agent 3" "A charming little milkshake bar?! How quaint! Surprising how it survived the chaos this long!"
    "Агент 3" "Маленький очаровательный бар?! Как прикольно! Удивительно, как он так долго продержался во всём этом хаосе!"

# game/status.rpy:1969
translate russia task3_fa47215b:

    # y "Heh heh~... yeah big surprise~..."
    y "Хе хех~... да, очень удивительно~..."

# game/status.rpy:1970
translate russia task3_28801176:

    # "Agent 1" "........................."
    "Агент 1" "........................."

# game/status.rpy:1971
translate russia task3_072f1426:

    # "Agent 1" "Wait... Something's not quite righ-...."
    "Агент 1" "Подожди... Что-то не так-..."

# game/status.rpy:1972
translate russia task3_96dad439:

    # y "GET 'EM!"
    y "ХВАТАЙТЕ ИХ!"

# game/status.rpy:2011
translate russia task3_9324afc6:

    # a y28 "WOOHP Agents!"
    a y28 "Агенты WOOHP!"

# game/status.rpy:2012
translate russia task3_8f62761d:

    # s g29 "That was close...!"
    s g29 "Это было близко...!"

# game/status.rpy:2013
translate russia task3_13c57710:

    # c r37 "We can't have them reporting back to WOOHP and blow our cover. Luckily it was only three this time."
    c r37 "Мы не можем допустить, чтобы они доложили остальным и тем самым сорвали наше прикрытие. К счастью, на этот раз их было только трое."

# game/status.rpy:2014
translate russia task3_633a6e4d:

    # a y49 "What do we do with them...?"
    a y49 "Что нам с ними делать...?"

# game/status.rpy:2015
translate russia task3_17ab0564:

    # y "I don't know. I've never hid a body before!{w} Put them downstairs in the basement I guess."
    y "Я не знаю. Я никогда раньше не прятал тела!{w} Положите их вниз, в подвал."

# game/status.rpy:2016
translate russia task3_bf7c3d29:

    # a "{b}*Nods*{/b}"
    a "{b}*Кивает*{/b}"

# game/status.rpy:2019
translate russia task3_225595a0:

    # y "And now we have a couple of mindcontrolled agents in the base...."
    y "Теперь у нас есть пара агентов под контролем наноботов, прямо на базе..."

# game/status.rpy:2020
translate russia task3_60cf6055:

    # c r1 "This works in our favor! If we break their nanobot control, we can use them in the next Landgrab!"
    c r1 "Это нам на пользу! Если мы нарушим контроль наноботов, то сможем использовать их в следующей войне!"

# game/status.rpy:2021
translate russia task3_1ebe22a9:

    # y "That's what I was thinkin-..."
    y "Я так и думал-..."

# game/status.rpy:2022
translate russia task3_0ef670cc:

    # a "Eep!"
    a "Ага!"

# game/status.rpy:2024
translate russia task3_f1657285:

    # "???" "Daylight stings my eyes!"
    "???" "Дневной свет жжёт мне глаза!"

# game/status.rpy:2025
translate russia task3_ae4bfad7_1:

    # y "!!!"
    y "!!!"

# game/status.rpy:2027
translate russia task3_6faed475:

    # a y29 "Er... [playerName]? There's already an agent downstairs."
    a y29 "Эм... [playerName]? Внизу уже есть агент."

# game/status.rpy:2028
translate russia task3_d3a59cb5:

    # y "............................."
    y "............................."

# game/status.rpy:2032
translate russia task3_57168065:

    # "Agent" "Hi! I'm still here!"
    "Агент" "Привет! Я всё ещё тут!"

# game/status.rpy:2033
translate russia task3_40271c89:

    # y "Are you that agent we captured during the first mission...?"
    y "Ты тот агент, которого мы схватили во время первого задания...?"

# game/status.rpy:2034
translate russia task3_3203e85f:

    # y "How did you survive down there for all this time...?"
    y "Как ты выжил там за всё это время...?"

# game/status.rpy:2035
translate russia task3_da97e51d:

    # "Agent" "My undying dedication to the mission!"
    "Агент" "Всё благодоря беззаветной преданность миссии!"

# game/status.rpy:2036
translate russia task3_db832180:

    # y "........"
    y "........"

# game/status.rpy:2037
translate russia task3_0ff703d4:

    # y "Suuure~...{w} Say... Do you want to help us retake Beverly Hills?"
    y "Поняяятно~...{w} Скажи... Хочешь помочь нам вернуть Беверли Хиллз??"

# game/status.rpy:2038
translate russia task3_d83b6620:

    # "Agent" "Boy do I! I can't wait to deal out some justice!{w} Preferably with my fists!"
    "Агент" "Конечно! Не могу дождаться, чтобы совершить правосудие!{w} Желательно кулаками!"

# game/status.rpy:2039
translate russia task3_fed0b3d6:

    # y "Oh easy there. We're gonna send you out during the next Landgrab."
    y "Полегче. Мы отправим тебя на следующею войну за территорию."

# game/status.rpy:2040
translate russia task3_8e1bf4df:

    # s g31 "If we can break the nano-control over these other agents, we could use them to help."
    s g31 "Если мы сможем нарушить контроль над этими агентами, мы сможем использовать их для помощи."

# game/status.rpy:2041
translate russia task3_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:2042
translate russia task3_fce37da7:

    # s g18 "What?"
    s g18 "Что?"

# game/status.rpy:2043
translate russia task3_d4636c00:

    # y "You know what is needed to break agents out of their control, right?"
    y "Вы ведь знаете, что нужно сделать, чтобы нарушить контроль над агентами?"

# game/status.rpy:2045
translate russia task3_983b94a6:

    # s "Well... yeah. We have to increase their oxytocin levels~..."
    s "Ну... да. Нам нужно повысить уровень их окситоцина~..."

# game/status.rpy:2046
translate russia task3_ec01b85a:

    # y "And how do we do that~...?"
    y "И как нам это сделать~...?"

# game/status.rpy:2047
translate russia task3_5c6cf512:

    # s ".........................."
    s ".........................."

# game/status.rpy:2048
translate russia task3_e3e16dc2:

    # s g19 "{size=-10}By having sex..."
    s g19 "{size=-10}Занимаясь сексом..."

# game/status.rpy:2049
translate russia task3_05c7fd16:

    # y "Exactly! Time to get naked!"
    y "В точку! Время раздеваться!"

# game/status.rpy:2050
translate russia task3_e4e62ab5:

    # s g28 "N-now wait a minute!"
    s g28 "С-сейчас?! П-подожди минутку!"

# game/status.rpy:2051
translate russia task3_23e27976:

    # c r31 "We're not gonna start showing off our naked bodies to every member of WOOHP. One is enough."
    c r31 "Мы не собираемся демонстрировать свои обнаженные тела каждому члену WOOHP. Одного будет достаточно."

# game/status.rpy:2052
translate russia task3_a5c1bc99:

    # a y29 "We could just buy some dirty magazines."
    a y29 "Мы можем просто купить несколько пошлых журналов."

# game/status.rpy:2053
translate russia task3_05d09e98:

    # y "Dirty magazines...?"
    y "Пошлые журналы...?"

# game/status.rpy:2054
translate russia task3_a82223c7:

    # a y1 "Yeah, we can just buy some magazines with those dirty pictures from the bookstore at the mall."
    a y1 "Да, мы можем просто купить несколько журналов с этими развратными картинками в книжном магазине в торговом центре."

# game/status.rpy:2055
translate russia task3_9b0b95e2:

    # a "If we throw a few of those down into the basement, I bet that will break the agents from their trance!"
    a "Если мы бросим парочку таких в подвал, я уверена, это выведет агентов из транса!"

# game/status.rpy:2056
translate russia task3_7b7f2369:

    # y "It'll cost us some money, but that's a good alternative."
    y "Это будет стоить нам денег, но это хорошая альтернатива."

# game/status.rpy:2060
translate russia task3_18ceca81:

    # "You captured a few more WOOHP Agents and it's time to free them from their nanobot control."
    "Вы захватили ещё несколько агентов WOOHP, и пришло время освободить их от контроля наноботов."

# game/status.rpy:2061
translate russia task3_05094001:

    # "Luckily breaking the nanobot control over captured agents isn't that hard. Visit the bookstore to buy cheap porn magazines and give them to the captured agents in your basement."
    "К счастью, нарушить контроль наноботов над захваченными агентами не так уж сложно. Посетите книжный магазин, чтобы купить дешевые порножурналы и подарить их захваченным агентам в вашем подвале."

# game/status.rpy:2062
translate russia task3_0ea4dbc5:

    # "Once agents have been freed, they can participate in Landgrabs and help liberate Beverly Hills."
    "После освобождения агентов они могут принять участие в Войне за территорию и помочь освободить Беверли-Хиллз."

# game/status.rpy:2064
translate russia task3_16222c52:

    # y "Okay, let's wrap this up. We've got quite a day ahead of us."
    y "Ладно, давайте закругляться. У нас впереди целый день."

# game/status.rpy:2065
translate russia task3_5a2dcbda:

    # "Agent" "What should I do?!"
    "Агент" "Что мне делать?!"

# game/status.rpy:2066
translate russia task3_42fb3646:

    # y "Er... You...{w} Here. Put this one."
    y "Эм... Ты...{w} Вот. Надень."

# game/status.rpy:2069
translate russia task3_40e9eb54:

    # y "You are now a chef."
    y "Теперь ты шеф-повар."

# game/status.rpy:2070
translate russia task3_50bbbf6c:

    # "Agent" "An important job! I will not disappoint you!"
    "Агент" "Важная работа! Я не разочарую тебя!"

# game/status.rpy:2093
translate russia task3_d5c53159:

    # mel "What's going on out here?!"
    mel "Что здесь происходит?!"

# game/status.rpy:2094
translate russia task3_2a27cb3f:

    # "Aces gangster" "We're under attack from all sides! We've got Punks on our left, Outsiders on our right and WOOHP agents fighting us from our front!"
    "Туз Гангстер" "Нас атакуют со всех сторон! Панки слева, Аутсайдеры справа и агенты WOOHP спереди!"

# game/status.rpy:2095
translate russia task3_d48a0b44:

    # mel "WOOHP Agen-...?"
    mel "Агеты WOOHP-...?"

# game/status.rpy:2096
translate russia task3_3032c70f:

    # "Outsiders Chick" "More Aces, get them!"
    "Девушка Аутсайдер" "Больше тузов, хватайте их!"

# game/status.rpy:2097
translate russia task3_30be022b:

    # mel "Fine... I'll show you how it's done."
    mel "Ладно... Я покажу вам, как это делается."

# game/status.rpy:2117
translate russia task3_b5d4557f:

    # y "Okay girls, I received intel that a new Lieutenant has shown herself."
    y "Так, девочки, я получил информацию о новом лейтенанте."

# game/status.rpy:2118
translate russia task3_8b2a79b7:

    # y "Her name is Melody von...{w} Guggen..."
    y "Ее зовут Мелоди фон...{w} Гугген..."

# game/status.rpy:2119
translate russia task3_390bc4e9:

    # y "That's a pretty crazy last name..."
    y "Это довольно сумасшедшая фамилия..."

# game/status.rpy:2121
translate russia task3_d963f4cb:

    # a y29 "Wait, did you say von Guggen?"
    a y29 "Подожди, ты сказали фон Гугген?"

# game/status.rpy:2122
translate russia task3_2d574665:

    # y "That name ring a bell?"
    y "Тебе знакомо это имя?"

# game/status.rpy:2123
translate russia task3_d0b0d805:

    # c r34 "We fought a Helga von Guggen a long time ago."
    c r34 "Мы сражались с Хельгой фон Гугген давным-давно."

# game/status.rpy:2124
translate russia task3_cde11bd6:

    # s g43 "Could be her granddaughter. She's probably after revenge."
    s g43 "Возможно, это её внучка. Скорее всего она жаждет мести."

# game/status.rpy:2125
translate russia task3_efe66fc3:

    # y "Reports are saying she's hanging out at the Aces Castle, but security is ramped up. You're going to have to get closer before you have a chance to capture her."
    y "Сообщается, что она тусуется в замке Тузов, но охрана усилена. Вам придётся подойти ближе, прежде чем у вас появится шанс схватить её."

# game/status.rpy:2126
translate russia task3_c8346d00:

    # y "I'm gonna keep sending you undercover with the Aces. Hopefully we'll get a chance to take her in."
    y "Я буду продолжать посылать тебя под прикрытием к \"Тузами\". Надеюсь, у нас будет возможность схватить её."

# game/status.rpy:2127
translate russia task3_a2abf71c:

    # "The girls nod and leave for their cell."
    "Девушки кивают и уходят по своим камера."

# game/status.rpy:2162
translate russia task4_831007b1:

    # s "[playerName]! There you are!"
    s "[playerName]! Вот ты где!"

# game/status.rpy:2163
translate russia task4_d1642ce7:

    # y "Huh?"
    y "Хм?"

# game/status.rpy:2164
translate russia task4_808f9bf7:

    # c "We found Alex! She's going to be at the school."
    c "Мы нашли Алекс! Судя по всему она в школе."

# game/status.rpy:2165
translate russia task4_dcc80098:

    # y "How do you know?"
    y "Откуда вы знаете?"

# game/status.rpy:2166
translate russia task4_6d1c2e0a:

    # s g33 "Well cause..."
    s g33 "Ну, потому что..."

# game/status.rpy:2167
translate russia task4_00959a98:

    # c "She told us."
    c "Она нам сказала."

# game/status.rpy:2168
translate russia task4_7fb7714e:

    # y "Alex told you?"
    y "Алекс вам сказала?"

# game/status.rpy:2169
translate russia task4_b646d579:

    # c "Yeah~..."
    c "Даа~..."

# game/status.rpy:2170
translate russia task4_86da6c09:

    # s g31 "We came across her during our last undercover operation.{w} She's still under WOOHP's control, but she gave us some new intel."
    s g31 "Мы наткнулись на неё во время нашей последней операции.{w} Она всё ещё под контролем WOOHP, но она дала нам новые сведения."

# game/status.rpy:2171
translate russia task4_4833a53d:

    # c r33 "Apparently they still think Sam and I are under nanobot control, but they're starting to have doubts."
    c r33 "Очевидно они все ещё считают, что мы с Сэм находимся под контролем наноботов, но они начинают сомневаться."

# game/status.rpy:2172
translate russia task4_9f642149:

    # c "Alex said that we are to report to the school. Some big shot is going to make an announcement."
    c "Алекс сказала что нам нужно прийти в школу. Какая-то важная шишка собирается сделать объявление."

# game/status.rpy:2173
translate russia task4_aefb6908:

    # y "The mastermind?"
    y "Организатор?"

# game/status.rpy:2174
translate russia task4_5152b2dc:

    # c r37 "Maybe."
    c r37 "Возможно."

# game/status.rpy:2175
translate russia task4_48595212:

    # y "I'd say we check it out."
    y "Думаю нам стоит это проверить."

# game/status.rpy:2176
translate russia task4_ca7de3af:

    # s g38 "I'd say we check it o-...{w} What you said."
    s g38 "Думаю нам стоит это про-...{w} Как ты и сказал."

# game/status.rpy:2177
translate russia task4_9e789e43:

    # s "Though we can expect security to be better than before. Even if we're not sneaking in this time, we'd best be prepared."
    s "Безопасность будет явно намного лучше, чем раньше. Даже если на этот раз мы не будем пробираться внутрь, нам лучше быть готовыми."

# game/status.rpy:2179
translate russia task4_de7a6150:

    # s "You should-..."
    s "Ты должен-..."

# game/status.rpy:2180
translate russia task4_2f0a6c9e:

    # y "I should craft you some new gadgets."
    y "Я должен сделать вам пару новых гаджетов."

# game/status.rpy:2181
translate russia task4_2bce0b8e:

    # s g29 "Y-yeah... I was thinkin-..."
    s g29 "Д-да... Я дума-..."

# game/status.rpy:2182
translate russia task4_780d63e5:

    # y "Maybe try to unlock some new ones."
    y "Может попробуйте разблокировать новых гаджетов."

# game/status.rpy:2183
translate russia task4_7a2e8913:

    # s g10 "{b}*Grumbles*{/b} Yeah.... We could really us-..."
    s g10 "{b}*Ворчит*{/b} Даа... Действительно мож-..."

# game/status.rpy:2184
translate russia task4_773018ca:

    # y "Maybe some kind of distraction device that makes agents turn the other way."
    y "Может быть, какое-нибудь отвлекающее устройство."

# game/status.rpy:2185
translate russia task4_7eed85bb:

    # s g4 "{b}*Pouts*{/b}"
    s g4 "{b}*Дуется*{/b}"

# game/status.rpy:2186
translate russia task4_8bb4b8db:

    # y "Then you can attend the meeting, figure out who's behind all this hallebelu and maybe even rescue Alex!"
    y "Так вы сможете попасть на собрание, выяснить кто стоит за всем этим и возможно, даже спасти Алекс!"

# game/status.rpy:2187
translate russia task4_b5d3c39c:

    # y "...."
    y "...."

# game/status.rpy:2188
translate russia task4_9568b406:

    # y "What's with the long face?"
    y "Что с лицом?"

# game/status.rpy:2189
translate russia task4_d93672ac:

    # c r24 "Oh don't mind her. Sam had the same idea and just wanted to show off."
    c r24 "Не обращай на неё внимания. У Сэм была та же идея, и она просто хотела похвастаться."

# game/status.rpy:2190
translate russia task4_d0454c94:

    # s g40 "{b}*Sputters*{/b} Y-yeah well... I'm glad to see we're all in agreement! The meeting is soon so send us over whenever you're ready, [playerName]."
    s g40 "{b}*Фыркает*{/b} Д-да, хорошо... Я рада видеть, что мы все согласны! Собрание уже скоро, так что отправь нас когда будешь готов, [playerName]."

# game/status.rpy:2192
translate russia task4_5238fec1:

    # s "We coul-..."
    s "Может нам-..."

# game/status.rpy:2193
translate russia task4_2df687e4:

    # y "We could use that new distraction powder gadget."
    y "Нам бы не помешал новый гаджет для отвлечения внимания."

# game/status.rpy:2194
translate russia task4_205ce9df:

    # s g29 "Y-yeah that's what I was thinking."
    s g29 "Д-да, именно об этом я и подумала."

# game/status.rpy:2195
translate russia task4_a6a519f9:

    # s "If we get some of those, it's sure to make life easier for us. Even if w-..."
    s "Если у нас будет парочка таких, это несомненно облегчит нам жизнь. Даже если-..."

# game/status.rpy:2196
translate russia task4_1398f600:

    # y "Even if you don't go on a mission, it's good to have some on you at all times."
    y "Даже если вы не на задании, всегда полезно иметь при себе несколько штук."

# game/status.rpy:2197
translate russia task4_2d32b459:

    # s g10 "{b}*Grumbles*{/b} Yes exactly. That's what I was gonna say."
    s g10 "{b}*Ворчит*{/b} Да, точно. Именно это я и хотела сказать."

# game/status.rpy:2198
translate russia task4_4a423469:

    # s "Using the dis-..."
    s "Используя гадж-..."

# game/status.rpy:2199
translate russia task4_dcf3a9d6:

    # y "By using the distraction powder, you could have guards turn the other way, allowing you to sneak up on them more easily."
    y "Используя этот гаджет, вы можете отвлечь охранников и легче подкрасться к ним."

# game/status.rpy:2200
translate russia task4_7eed85bb_1:

    # s g4 "{b}*Pouts*{/b}"
    s g4 "{b}*Дуется*{/b}"

# game/status.rpy:2201
translate russia task4_8bb4b8db_1:

    # y "Then you can attend the meeting, figure out who's behind all this hallebelu and maybe even rescue Alex!"
    y "Так вы сможете попасть на собрание, выяснить кто стоит за всем этим и возможно, даже спасти Алекс!"

# game/status.rpy:2202
translate russia task4_b5d3c39c_1:

    # y "...."
    y "...."

# game/status.rpy:2203
translate russia task4_9568b406_1:

    # y "What's with the long face?"
    y "Что с лицом?"

# game/status.rpy:2204
translate russia task4_d93672ac_1:

    # c r24 "Oh don't mind her. Sam had the same idea and just wanted to show off."
    c r24 "Не обращай на неё внимания. У Сэм была та же идея, и она просто хотела похвастаться."

# game/status.rpy:2205
translate russia task4_d0454c94_1:

    # s g40 "{b}*Sputters*{/b} Y-yeah well... I'm glad to see we're all in agreement! The meeting is soon so send us over whenever you're ready, [playerName]."
    s g40 "{b}*Фыркает*{/b} Д-да, хорошо... Я рада видеть, что мы все согласны! Собрание уже скоро, так что отправь нас когда будешь готов, [playerName]."

# game/status.rpy:2215
translate russia task4_dac23f41:

    # y "Checking out this meeting at the school should be our top priority...."
    y "Попасть на это собрание в школе должно быть нашим главным приоритетом..."

# game/status.rpy:2237
translate russia task4_488c76d8:

    # y "Everyone ready?"
    y "Все готовы?"

# game/status.rpy:2241
translate russia task4_3032011b:

    # c "Yes, let's visit the school and get Alex back!"
    c "Да, давайте вернём Алекс!"

# game/status.rpy:2243
translate russia task4_adf823be:

    # s "We have some new gadgets, so I'm sure we'll be fine."
    s "У нас есть несколько новых гаджетов, так что я уверена, что всё будет в порядке."

# game/status.rpy:2245
translate russia task4_4c7ecaa0:

    # s "We don't have any new gadgets yet, but I'm sure we'll be fine."
    s "У нас нет новых гаджетов, но я уверена, что всё будет хорошо."

# game/status.rpy:2246
translate russia task4_2c9abd8c:

    # y "Let's go over the plan one more time."
    y "Давайте пройдёмся по плану ещё раз."

# game/status.rpy:2247
translate russia task4_fc921ead:

    # s g32 "Right."
    s g32 "Точно."

# game/status.rpy:2248
translate russia task4_357de590:

    # s "Clover and I will head to the school to attend this meeting. Our first objective is to rescue Alex and bring her back to the base."
    s "Мы с Кловер отправимся в школу, чтобы присутствовать на этой встрече. Наша первая задача - спасти Алекс и вернуть её на базу."

# game/status.rpy:2249
translate russia task4_429511ac:

    # s "Our second objective will be to gather new intel and figure out who's running this operation."
    s "Нашей второй задачей будет собрать новые сведения и выяснить, кто руководит этой операцией."

# game/status.rpy:2250
translate russia task4_14afb99f:

    # y "With a little luck we might even avoid confrontation."
    y "Если повезёт, мы даже сможем избежать конфронтаций."

# game/status.rpy:2256
translate russia task4_e2758177:

    # y "Get ready, I'm sending you two out."
    y "Приготовьтесь, я отправляю вас."

# game/status.rpy:2257
translate russia task4_8503467d:

    # c r13 "We're like... totally gonna ace this!"
    c r13 "Мы типа... великолепно справимся!"

# game/status.rpy:2258
translate russia task4_76171cbb:

    # s g33 "We'll bring some spy cameras so you can see what's going on."
    s g33 "Мы поставим несколько шпионских камер, чтобы ты мог наблюдать за происходящим."

# game/status.rpy:2310
translate russia task4_bda28dda:

    # c ".................."
    c ".................."

# game/status.rpy:2311
translate russia task4_15759d81:

    # s g30 "There sure are a lot of agents here..."
    s g30 "Здесь довольно много агентов..."

# game/status.rpy:2312
translate russia task4_1fb5c42d:

    # c "Could be a trap."
    c "Может быть это ловушка."

# game/status.rpy:2313
translate russia task4_11373953:

    # s g32 "Yeah... stay on your guard."
    s g32 "Даа... будь начеку."

# game/status.rpy:2314
translate russia task4_22002cc6:

    # "Microphone" "{b}*Feedback*{/b}"
    "Микрофон" "{b}*Жужжит*{/b}"

# game/status.rpy:2320
translate russia task4_0990965e:

    # s "They're about to give a speech!"
    s "Они собираются произнести речь!"

# game/status.rpy:2334
translate russia task4_9f1b541a:

    # c r12 "I can't see anything!"
    c r12 "Я ничего не вижу!"

# game/status.rpy:2335
translate russia task4_0b4cfa64:

    # c "Hang on, I'm gonna find a place up high. Just call me if you need backup."
    c "Подожди, я найду место повыше. Просто вызови мне, если понадобится помощь."

# game/status.rpy:2339
translate russia task4_54bf92ca:

    # "Microphone" "....................."
    "Микрофон" "....................."

# game/status.rpy:2342
translate russia task4_db119b9a:

    # "Agent O5O" "{b}*Taps microphone*{/b}"
    "Агент O5O" "{b}*Подключает микрофон*{/b}"

# game/status.rpy:2343
translate russia task4_a8ea145e:

    # y "Oh no...."
    y "Оо нет..."

# game/status.rpy:2345
translate russia task4_4f9b2467:

    # "Agent O5O" "Mic: ATTENTION VALUED EMPLOYEES OF WOOHP!"
    "Агент O5O" "Микрофон: ВНИМАНИЕ УВАЖАЕМЫЕ СОТРУДНИКИ КОМПАНИИ WOOHP!"

# game/status.rpy:2347
translate russia task4_c5cd0293:

    # "Agent O5O" "Mic: FOR THOSE WHO ARE NEW HERE, I AM AGENT {i}O5O{/i}."
    "Агент O5O" "Микрофон: ДЛЯ ТЕХ, КТО ЗДЕСЬ НЕДАВНО, Я - АГЕНТ {i}O5O{/i}."

# game/status.rpy:2349
translate russia task4_d2301b05:

    # "Agent O5O" "IT IS{size=+4} SO NICE{/size}{size=+8} TO SEE{/size} {size=+14}YOU ALL AGAIN!{/size}"
    "Агент O5O" "ТАК{size=+4} ПРИЯТНО{/size}{size=+8} ВАС ВСЕХ{/size} {size=+14}СНОВА ВИДЕТЬ!{/size}"

# game/status.rpy:2350
translate russia task4_69a2cd26:

    # "WOOHP Agent" "{size=-4}Sir! My eardrums are bleeding!{/size}"
    "Агент WOOHP" "{size=-4}Сэр! У меня барабанные перепонки кровоточат!{/size}"

# game/status.rpy:2352
translate russia task4_2083249b:

    # "Agent O5O" "Mic: SORRY I CAN'T HEAR YOU! YOU'LL HAVE TO SPEAK... {w}{size=+10}{b}LOUDER!!!{/b}{/size}"
    "Агент O5O" "Микрофон: ИЗВИНИТЕ, Я ВАС НЕ СЛЫШУ! ГОВОРИТЕ... {w}{size=+10}{b}ГРОМЧЕ!!!{/b}{/size}"

# game/status.rpy:2353
translate russia task4_39da8be1:

    # "WOOHP Agent" "Argh!"
    "Агент WOOHP" "Аргх!"

# game/status.rpy:2365
translate russia task4_1a964758:

    # "Agent O5O" "JUST FOR THIS SPECIAL OCCASSION, I'VE PREPARED A LITTLE SPEECH!"
    "Агент O5O" "СПЕЦИАЛЬНО ДЛЯ ЭТОГО СЛУЧАЯ, Я ПОДГОТОВИЛ НЕБОЛЬШУЮ РЕЧЬ!"

# game/status.rpy:2366
translate russia task4_3078e15d:

    # "Agents" "{b}*Groan*{/b}"
    "Агенты" "{b}*Ворчат*{/b}"

# game/status.rpy:2367
translate russia task4_a9c99c25:

    # s ".............."
    s ".............."

# game/status.rpy:2368
translate russia task4_78c39600:

    # cM "Pst Sam, can you hear me? Look past the crowd, to your left."
    cM "Пст Сэм, ты меня слышишь? Посмотри мимо толпы, налево."

# game/status.rpy:2369
translate russia task4_510d30fb:

    # s "....?"
    s "....?"

# game/status.rpy:2377
translate russia task4_c90477ba:

    # s g29 "(Alex!)"
    s g29 "(Алекс!)"

# game/status.rpy:2378
translate russia task4_2e230aad:

    # "Just in time you notice the spy in yellow walking by and heading off into a nearby corridor."
    "Как раз вовремя вы замечаете, что мимо проходит шпионка в жёлтом и направляется в соседний коридор."

# game/status.rpy:2381
translate russia task4_298e792d:

    # s "([playerName], Clover. I'm going after her.)"
    s "([playerName], Кловер. Я иду за ней.)"

# game/status.rpy:2382
translate russia task4_cdacae03:

    # cM "(Roger that, I'll come with you.)"
    cM "(Поняла тебя, я иду с тобой.)"

# game/status.rpy:2383
translate russia task4_0b1683fa:

    # y "(Be careful, this could still be a trap.)"
    y "(Будьте осторожны, это может быть ловушкой)."

# game/status.rpy:2419
translate russia task4_a8562c8f:

    # s "There she is..."
    s "Вот она..."

# game/status.rpy:2420
translate russia task4_9846e0a5:

    # cM "She's talking to a WOOHP agent though... You think she spotted us?"
    cM "Она разговаривает с агентом WOOHP... Думаешь, она нас заметила?"

# game/status.rpy:2421
translate russia task4_c1bdfd55:

    # y "Best to not take any chances. Follow that spy and knock out any agents along the way. Clover will provide backup."
    y "Лучше не рисковать. Следуй за ней и уничтожай агентов по пути. Кловер обеспечит поддержку."

# game/status.rpy:2423
translate russia task4_5c3d40b9:

    # s "On it!"
    s "Так точно!"

# game/status.rpy:2442
translate russia task4_7a3418fb:

    # y "There's still some time, make sure that you're prepared."
    y "Ещё есть немного времени, убедитесь, что вы подготовились."

# game/status.rpy:2443
translate russia task4_7430afb8:

    # c "All right, [playerName]. Don't wait too long though."
    c "Ладно, [playerName]. Однако не жди слишком долго."

# game/status.rpy:2455
translate russia task4_e973411a:

    # y "I'll have to make sure everyone's available first."
    y "Сначала я должен убедиться, что все свободны."

# game/status.rpy:2485
translate russia task4_32ebba32:

    # a "Oooooh, have you been working out?~"
    a "Ооооо, ты тренировался?~"

# game/status.rpy:2486
translate russia task4_95d63b4c:

    # "Agent" "I am so happy you noticed!"
    "Агент" "Я так рад, что ты заметила!"

# game/status.rpy:2487
translate russia task4_e2836781:

    # a "You know~... Maybe you and I should slip away and {b}*Whisper* *Whisper* *Whisper*{/b}"
    a "Знаешь~... Может нам с тобой стоит ускользнуть и {b}*Шепчет* *Шепчет* *Шепчет*{/b}"

# game/status.rpy:2488
translate russia task4_d2718a14:

    # "Agent" "Such suggestions made me detect an anomaly.... in my pants!"
    "Агент" "Такое предложения заставило меня обнаружить аномалию..... в штанах!"

# game/status.rpy:2489
translate russia task4_1f8f20be:

    # "Agent" "............................"
    "Агент" "............................"

# game/status.rpy:2490
translate russia task4_dc80e3bd:

    # "Agent" "Funny! My desire to burn a dumpster or kick a puppy have suddenly left me completely!"
    "Агент" "Смешно! Желание сжечь мусорный бак или пнуть щенка вдруг полностью покинуло меня!"

# game/status.rpy:2491
translate russia task4_510d30fb_1:

    # s "....?"
    s "....?"

# game/status.rpy:2492
translate russia task4_243861f6:

    # a "See you soon~..."
    a "Увидимся~..."

# game/status.rpy:2499
translate russia task4_05bc76db:

    # s "(Did you see that, [playerName]?)"
    s "(Видел это, [playerName]?)"

# game/status.rpy:2500
translate russia task4_6ac0cb60:

    # y "Yeah, she's up to something."
    y "Да, она что-то задумала."

# game/status.rpy:2501
translate russia task4_91b8f2bd:

    # s "(Okay, I'm going to keep following he-...)"
    s "(Хорошо, я продолжу приследован-...)"

# game/status.rpy:2502
translate russia task4_60067f8b:

    # s g9 "Ngh~...!!!"
    s g9 "Нгх~...!!!"

# game/status.rpy:2503
translate russia task4_150c4e32:

    # y "Sam?"
    y "Сэм?"

# game/status.rpy:2504
translate russia task4_e702bd2a:

    # s "Use... the... V.I.B....!"
    s "Используй... V.I.B...!"

# game/status.rpy:2521
translate russia task4_5779ff0d:

    # "It's time to use your equipment. Your spies are each equipped with a V.I.B. which can be used to break them out of Nanobot control."
    "Пришло время использовать ваше оборудование. Каждая из ваших шпионок оснащена V.I.B´ом который можно использовать, чтобы вывести их из-под контроля наноботов."

# game/status.rpy:2522
translate russia task4_c4f78ac2:

    # "Use the V.I.B. by {b}pressing 1{/b} on your keyboard to equip it. But be warned, using the V.I.B. for too long can give away a spy's location."
    "Используйте V.I.B. {b}нажимая 1{/b} на клавиатуре. Но учтите, что использование V.I.B. слишком долгое время может выдать местоположение шпиона."

# game/status.rpy:2523
translate russia task4_f97804c1:

    # "You can stop the V.I.B. by {b}pressing 1{/b} again."
    "Вы можете остановить V.I.B. повторным {b}нажатием 1{/b}."

# game/status.rpy:2551
translate russia task4_b937afae:

    # sM "It's a dead-end... We got her now!"
    sM "Это тупик... Теперь она наша!"

# game/status.rpy:2552
translate russia task4_244650a6:

    # y "Great."
    y "Отлично."

# game/status.rpy:2553
translate russia task4_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:2554
translate russia task4_20e0f3d5:

    # y "So how are you going to break the nanobot control over her?"
    y "Так как вы собираетесь нарушить контроль её наноботов?"

# game/status.rpy:2555
translate russia task4_44ab09dd:

    # sM "O-oh! Well... I...!"
    sM "О-ох! Нуу... Я...!"

# game/status.rpy:2558
translate russia task4_8bf037cf:

    # a y16 "Is someone there...?"
    a y16 "Тут кто-то есть...?"

# game/status.rpy:2559
translate russia task4_8d374be0:

    # y "Kiss her! It worked last time!"
    y "Поцелуй её! В прошлый раз это сработало!"

# game/status.rpy:2560
translate russia task4_b5f749ec:

    # s g30 "Nooo~... Alex is my friend. It'd be super awkward!"
    s g30 "Нееет~... Алекс - моя подруга. Это супер неловко!"

# game/status.rpy:2561
translate russia task4_4b9de986:

    # y "......."
    y "......."

# game/status.rpy:2562
translate russia task4_9ef12cf2:

    # cM "I'll do it!"
    cM "Я сделаю это!"

# game/status.rpy:2575
translate russia task4_a999ddb1:

    # "Out of nowhere, Clover jumps on Alex, pins her to the ground and kisses her deeply!"
    "Из ниоткуда Кловер прыгает на Алекс, прижимает её к полу и целует!"

# game/status.rpy:2578
translate russia task4_10d46613:

    # a "Mpffhh!!!"
    a "Мпффхх!!!"

# game/status.rpy:2579
translate russia task4_b7de7a0b:

    # a ".................."
    a ".................."

# game/status.rpy:2580
translate russia task4_bda28dda_1:

    # c ".................."
    c ".................."

# game/status.rpy:2582
translate russia task4_a0658532:

    # a "Oh hi Clover!"
    a "Ох, привет Кловер!"

# game/status.rpy:2583
translate russia task4_3784857a:

    # c "...?"
    c "...?"

# game/status.rpy:2595
translate russia task4_e87845bb:

    # s g16 "Alex...?"
    s g16 "Алекс...?"

# game/status.rpy:2596
translate russia task4_6f275dfd:

    # a "Oh Sam, you're here too!"
    a "Ох Сэм, и ты тут!"

# game/status.rpy:2597
translate russia task4_9db106c9:

    # y "Is she... not being controlled?"
    y "Она...  больше не под контролем?"

# game/status.rpy:2598
translate russia task4_210c52d8:

    # c r10 "Alex we've been looking for you. Are you okay?"
    c r10 "Алекс, мы искали тебя. Ты в порядке?"

# game/status.rpy:2599
translate russia task4_b9cb64eb:

    # a y28 "I think so. I'm so glad I'm not the only one who hasn't gone crazy!"
    a y28 "Думаю да. Я так рада, что я не единственная, кто не сошла с ума!"

# game/status.rpy:2600
translate russia task4_15cf5ed3:

    # s g29 "But how did you break the nanobot control...?"
    s g29 "Но как ты нарушила работу наноботов...?"

# game/status.rpy:2601
translate russia task4_d992ae9d:

    # a y23 "Oh er... Well I might've stumbled into the male WOOHP changing room by accident..."
    a y23 "Ох, эм... Ну возможно, я случайно зашла в мужскую раздевалку..."

# game/status.rpy:2602
translate russia task4_a4da5e48:

    # a y1 "After that I've been back to normal.. for the most part."
    a y1 "После этого я вернулась к норме... по большей части."

# game/status.rpy:2603
translate russia task4_42f0d132:

    # a "What about you two?!"
    a "А вы двое?!"

# game/status.rpy:2604
translate russia task4_ef121b3b:

    # s g29 "It's a long story. We'll tell you later. For now, we-..."
    s g29 "Это долгая история. Мы расскажем тебе позже. Пока что мы-..."

# game/status.rpy:2610
translate russia task4_c0919ca3:

    # "Intercom" "I'M SORRY TO CUT MY SPEECH SHORT, I KNOW YOU WERE ALL DYING TO HEAR IT, BUT I HAVE AN IMPORTANT MESSAGE FROM ONE OF THE HIGHER UPS."
    "Интерком" "ПРОСТИТЕ, ЧТО ПРЕРВАЛ СВОЮ РЕЧЬ. Я ЗНАЮ,  ВЫ ЖАЖДЕТЕ УСЛЫШАТЬ ЕЁ, НО У МЕНЯ ВАЖНОЕ СООБЩЕНИЕ ОТ ОДНОГО ИЗ ВЫСШИХ РУКОВОДИТЕЛЕЙ."

# game/status.rpy:2611
translate russia task4_af8bd8c6:

    # "Intercom" "PUTTING IT ON SCREEN NOW!"
    "Интерком" "ВНИМАНИЕ НА ЭКРАН!"

# game/status.rpy:2612
translate russia task4_15a4c71e:

    # "Suddenly the hallway screens flicker to life."
    "Внезапно экраны в коридоре замерцали и включились."

# game/status.rpy:2628
translate russia task4_b6ae1fee:

    # "???" "Attention WOOHP Agents..."
    "???" "Внимание агенты WOOHP..."

# game/status.rpy:2629
translate russia task4_3cf91dbe:

    # sM "(That voice...!)"
    sM "(Этот голос...!)"

# game/status.rpy:2630
translate russia task4_f54b1f3b:

    # cM "Is that...?"
    cM "Это...?"

# game/status.rpy:2638
translate russia task4_419cfc31:

    # "I have an important announcement for you all..."
    "У меня важное объявление для всех вас..."

# game/status.rpy:2639
translate russia task4_104ef6bf:

    # cM "Tim!"
    cM "Тим!"

# game/status.rpy:2640
translate russia task4_d1453718:

    # y "Tim...?"
    y "Тим...?"

# game/status.rpy:2641
translate russia task4_d87c4ec6:

    # cM "Sam's ex-boyfriend. We've fought him before."
    cM "Бывший парень Сэм. Мы уже сражались с ним раньше."

# game/status.rpy:2642
translate russia task4_0e177dd2:

    # sM "He's not my ex-boyfriend!"
    sM "Он не мой бывший парень!"

# game/status.rpy:2643
translate russia task4_4c3c5dd2:

    # aM "Shhhh."
    aM "Тссс."

# game/status.rpy:2644
translate russia task4_5bcff08e:

    # tim "There have been reports of desertion amongst our ranks. Dissension will not be tolerated and those who leave our cause will be dealt with."
    tim "Поступила информация о дезертирстве в наших рядах. Мы не потерпим несогласных и тех кто оставит наше дело мы будем наказывать."

# game/status.rpy:2645
translate russia task4_9e8b60a9:

    # tim "We will be keeping closer tabs on everyone from now on.{w} If we find some of you lacking in your devotion to the mission, know that {i}'correction'{/i} will be swift and without mercy."
    tim "Впредь мы будем тщательнее следить за всеми.{w} Если мы обнаружим, что кто-то из вас недостаточно предан миссии, знайте, что ваше {i}'исправление'{/i} будет быстрым и без пощады."

# game/status.rpy:2646
translate russia task4_5b5e2897:

    # tim "Tim Scam out."
    tim "Тим Скам вышел."

# game/status.rpy:2659
translate russia task4_5c701e7a:

    # s "..............."
    s "..............."

# game/status.rpy:2660
translate russia task4_ddeb37df:

    # c r32 "Should've known that Tim was behind this whole thing. He's got a bone to pick with WOOHP."
    c r32 "Стоило догадаться, что за всем этим стоит Тим. Он давно хочет свести счёты с WOOHP."

# game/status.rpy:2661
translate russia task4_173ff031:

    # a "And he probably wants to get revenge on his ex-girlfriend!"
    a "И ещё он точно хочет отомстить своей бывшей девушке!"

# game/status.rpy:2662
translate russia task4_40d18121:

    # s g40 "I am {b}not{/b} his ex-girlfriend!"
    s g40 "Я {b}не{/b} его бывшая девушка!"

# game/status.rpy:2663
translate russia task4_2ce1dcce:

    # y "I think this is a good time for me to pull you three out. Meet me back at the base."
    y "Думаю, сейчас самое время вытащить вас троих. Встретимся на базе."

# game/status.rpy:2675
translate russia task4_4335cb4f:

    # "{b}*Ssssssswooosh!*{/b}"
    "{b}*Вжуууууууухх!*{/b}"

# game/status.rpy:2684
translate russia task4_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:2685
translate russia task4_d4337478:

    # y "So I feel like I need to be informed."
    y "И так, я бы хотел узнать поподробнее."

# game/status.rpy:2686
translate russia task4_a9c99c25_1:

    # s ".............."
    s ".............."

# game/status.rpy:2687
translate russia task4_a220f0a7:

    # c r33 "Tim Scam is a former WOOHP scientist who went rogue. Selling his tech to the highest bidder. We fought him a few years ago."
    c r33 "Тим Скам - бывший ученый из WOOHP, который ушёл в отставку. Продаёт свои технологии тому, кто больше заплатит. Мы сражались с ним несколько лет назад."

# game/status.rpy:2688
translate russia task4_661b66ab:

    # a y28 "Yeah, we went to space! It was crazy!"
    a y28 "Да, мы полетели в космос! Это было безумно!"

# game/status.rpy:2689
translate russia task4_c688ccf6:

    # y "And he's Sam's ex?"
    y "И он бывший Сэм?"

# game/status.rpy:2690
translate russia task4_c5c7d265:

    # s g12 "No!{w} Listen, it's complicated..."
    s g12 "Нет!{w} Послушай, это сложно..."

# game/status.rpy:2691
translate russia task4_680c58d3:

    # c r22 "She had a huge crush on him and he on her."
    c r22 "Она была очень влюблена в него, а он в неё."

# game/status.rpy:2692
translate russia task4_62cc19b3:

    # s g17 "Guys....!!!"
    s g17 "Ребята...!!!"

# game/status.rpy:2693
translate russia task4_3a83d953:

    # s "It never went anywhere, okay?! Him turning out to be evil was a huge turn-off!"
    s "Между нами ничего не было, ясно?! То, что он оказался злым, поставило жирную точку!"

# game/status.rpy:2694
translate russia task4_39d9c736:

    # a y18 "We didn't know him as Tim Scam back then... but as Mac Smit.{w} Looking back on it, it's amazing that he managed to trick the world's largest anti-terrorist organization just by spelling his name backwards."
    a y18 "Тогда мы ещё не знали его как Тима Скама... а как Мака Смита.{w} Оглядываясь назад, удивительно, что ему удалось обмануть крупнейшую в мире антитеррористическую организацию, просто написав свое имя задом наперёд."

# game/status.rpy:2695
translate russia task4_10b9c69e:

    # c r12 "Yeah, not one of WOOHP's finest moments."
    c r12 "Да, не самый лучший момент в истории WOOHP."

# game/status.rpy:2696
translate russia task4_dd6d4cb8:

    # y "So is Sam's relation with him going to be an problem for our mission?"
    y "Значит, его отношения с Сэм станет проблемой для нашей миссии?"

# game/status.rpy:2697
translate russia task4_ac63bf4c:

    # s g38 "No. I'm completely over him.{w} Right girls?"
    s g38 "Нет. У нас с ним всё кончено.{w} Ведь так девочки?"

# game/status.rpy:2698
translate russia task4_edc59506:

    # c r10 "..........................."
    c r10 "..........................."

# game/status.rpy:2699
translate russia task4_cc1dd639:

    # a y35 "..........................."
    a y35 "..........................."

# game/status.rpy:2700
translate russia task4_29af9265:

    # s g28 "Oh come on! Atleast you know I won't take some boy over you guys, right?"
    s g28 "Да ладно вам! Вы же знаете, что я не променяю вас на какого-то парня, верно?"

# game/status.rpy:2701
translate russia task4_108e8c10:

    # a y55 "That's true..."
    a y55 "Это правда..."

# game/status.rpy:2702
translate russia task4_00618150:

    # c "All right Sammy, we trust you on this one."
    c "Ладно, Сэмми, в этом вопросе мы тебе доверяем."

# game/status.rpy:2703
translate russia task4_c97f14df:

    # y "There's one other thing..."
    y "Есть ещё одна вещь..."

# game/status.rpy:2704
translate russia task4_1ddf76bf:

    # y "His warning about keeping a closer eye on everyone. If he finds you serving milkshakes at a diner, he might become suspicious."
    y "Его предупреждение о том, что за всеми нужно присматривать. Если он увидит, что вы подаете молочные коктейли в закусочной, он может насторожиться."

# game/status.rpy:2705
translate russia task4_b1cb4e6e:

    # y "Going undercover and causing chaos with the different gangs in town will help a little, but if we want to stay under the radar, we might have to one-up it a bit."
    y "Работать под прикрытием и учинять хаос среди различных банд в городе, может помочь, но если мы хотим оставаться под радаром, нам придётся немного превзойти его."

# game/status.rpy:2706
translate russia task4_64e1e631:

    # y "Let's keep track of our notoriety from now on. I'm sure we'll find a way to boost it and keep any suspicion off of us."
    y "Давайте впредь следить за нашей известностью. Я уверен, что мы найдём способ увеличить её и отвести от нас любые подозрения."

# game/status.rpy:2707
translate russia task4_4b21ba5e:

    # a y29 "Notoriety...?"
    a y29 "Известность...?"

# game/status.rpy:2708
translate russia task4_40067012:

    # s g4 "We're gonna have to commit more crimes?"
    s g4 "Нам придётся совершать больше преступлений?"

# game/status.rpy:2709
translate russia task4_bc866f98:

    # y "Just think of the end-goal here. We gotta free the city and if we have to break a few windows to do so, then so be it."
    y "Просто подумайте о конечной цели. Мы должны освободить город, и если для этого нам придётся разбить несколько окон, то так тому и быть."

# game/status.rpy:2710
translate russia task4_a09c9b5f:

    # s "I guess."
    s "Наверное, это так."

# game/status.rpy:2711
translate russia task4_915c50ac:

    # c r13 "Plus! I'm sure WOOHP has insurance. We might piss off some people now, but it'll sort itself out later down the line!"
    c r13 "Плюс! Я уверена, что у WOOHP есть страховка. Возможно, сейчас мы разозлим некоторых людей, но потом всё решится само собой!"

# game/status.rpy:2712
translate russia task4_1ee49874:

    # y "All right. You two fill Alex in on what's been happening here. Good work out there girls."
    y "Отлично. Вы двое расскажите Алекс о том, что здесь происходит. Хорошая работа, девочки."

# game/status.rpy:2713
translate russia task4_37f1e621:

    # "The girls nod and head off to the detention area."
    "Девочки кивают и отправляются в спальни."

# game/status.rpy:2738
translate russia task4_0ac68a65:

    # a "Morning [playerName]!"
    a "Доброе утро [playerName]!"

# game/status.rpy:2739
translate russia task4_2b6ea174:

    # y "Alex, are you okay?"
    y "Алекс, ты в порядке?"

# game/status.rpy:2740
translate russia task4_ec5fab2a:

    # a "Yeah totally. What can I do to help?"
    a "Да, вполне. Чем я могу помочь?"

# game/status.rpy:2741
translate russia task4_a356a0eb:

    # y "Well...."
    y "Нуу..."

# game/status.rpy:2742
translate russia task4_6ab30b17:

    # y "I want you to go undercover with the Outsiders."
    y "Я хочу, чтобы ты работал под прикрытием у Аутсайдеров."

# game/status.rpy:2744
translate russia task4_b6b27c1d:

    # y "I'm just going to need a contact, but I'm sure I'll get one if I keep exploring around."
    y "Мне просто нужен контакт внутри, но я уверен, что найду его, если продолжу поиски в округе."

# game/status.rpy:2745
translate russia task4_7839c3c3:

    # a y29 "The Outsiders...?"
    a y29 "Аутсайдеры...?"

# game/status.rpy:2746
translate russia task4_d745053e:

    # a y28 "Wait those are those goths and punkers! I don't wanna go there!"
    a y28 "Подожди, это же те самые готы и панки! Я не хочу туда!"

# game/status.rpy:2747
translate russia task4_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:2748
translate russia task4_e777d866:

    # a y29 "They're scary!"
    a y29 "Они страшные!"

# game/status.rpy:2749
translate russia task4_74d87657:

    # y "Scary?"
    y "Страшные?"

# game/status.rpy:2750
translate russia task4_0582df05:

    # a "With all those black clothes and spikes! They're spooky!"
    a "Вся эта черная одежда с шипами! Они жуткие!"

# game/status.rpy:2751
translate russia task4_7dd67216:

    # a "And I'm pretty sure that they're....{w}{size=-10}vampires{/size}"
    a "И я почти уверена, что они....{w}{size=-10}вампиры{/size}"

# game/status.rpy:2752
translate russia task4_0a470607:

    # y "You'll be fiiiine."
    y "Всё будет отлично."

# game/status.rpy:2753
translate russia task4_9961e9dc:

    # a y4 "{b}*Pouts*{/b}"
    a y4 "{b}*Дуется*{/b}"

# game/status.rpy:2765
translate russia task4_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:2766
translate russia task4_4654c5db:

    # y "Where's Alex...?"
    y "Где Алекс...?"

# game/status.rpy:2767
translate russia task4_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:2768
translate russia task4_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:2774
translate russia task4_232c5c06:

    # y "Alex? Where are you?"
    y "Алекс? Где ты?"

# game/status.rpy:2775
translate russia task4_8dd2e829:

    # a "I'm hiding!"
    a "Я прячусь!"

# game/status.rpy:2776
translate russia task4_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:2777
translate russia task4_3e9a8a4f:

    # y "From what?"
    y "От чего?"

# game/status.rpy:2778
translate russia task4_e84d61ee:

    # a "From the vampires! It's full of monsters here!"
    a "От вампиров! Здесь полно монстров!"

# game/status.rpy:2779
translate russia task4_19276b51:

    # a y28 "I saw a guy wearing black lipstick and a girl with a spiked dog collar around her neck!"
    a y28 "Я видела парня с черными губами и девушку в ошейнике с шипами на шее!"

# game/status.rpy:2780
translate russia task4_692be6fa:

    # y "............"
    y "............"

# game/status.rpy:2781
translate russia task4_f7cbb2e1:

    # a "Did you know their base is at a haunted amusement park?! There's for real ghosts in this place!"
    a "Ты же знаешь, что их база находится в парке аттракционов с привидениями?! Тут есть настоящие призраки!"

# game/status.rpy:2782
translate russia task4_4a05d125:

    # y "Alex..."
    y "Алекс..."

# game/status.rpy:2783
translate russia task4_25ca4664:

    # a y42 "I've been hiding in the hotdog stand all day!"
    a y42 "Я весь день прячусь в киоске с хот-догами!"

# game/status.rpy:2784
translate russia task4_8dfb100d:

    # a "Please get me out of here, [playerName]~....!"
    a "Пожалуйста, вытащи меня отсюда, [playerName]~...!"

# game/status.rpy:2785
translate russia task4_d191bf58:

    # y "All right you scaredy cat... I'll use the tunnel network..."
    y "Ладно, трусишка... Я запускаю туннельную систему..."

# game/status.rpy:2805
translate russia task4_0c438fab:

    # a "Oof~..."
    a "Фух~..."

# game/status.rpy:2806
translate russia task4_8dca6816:

    # y "Welcome back Alex."
    y "С возвращением, Алекс."

# game/status.rpy:2807
translate russia task4_81e96cf2:

    # a y19 "Hey~..."
    a y19 "Привет~..."

# game/status.rpy:2808
translate russia task4_e126611f:

    # y "So...{w} vampires..."
    y "И так...{w} вампиры..."

# game/status.rpy:2809
translate russia task4_a09b2498:

    # a y28 "I know right! They're so creepy!"
    a y28 "Я знаю! Они такие жуткие!"

# game/status.rpy:2810
translate russia task4_78c26e29:

    # a "They're wearing all black or have cut their hair into these evil mohawk shapes! I'm sure they're vampires! And mummies!"
    a "Они одеты во всё черное или подстрижены под ирокез! Я уверена, что они вампиры! И мумии!"

# game/status.rpy:2811
translate russia task4_05bf074c:

    # y ".............."
    y ".............."

# game/status.rpy:2812
translate russia task4_e1374f3f:

    # a "I saw someone who was so ugly, I'm pretty sure he was a zombie."
    a "Я видела очень уродливого человека, я уверена, что он зомби."

# game/status.rpy:2813
translate russia task4_956ad32a:

    # y "You know the Outsiders are just kids dressed up in punk and goth clothing, right?"
    y "Вы же знаешь, что Аутсайдеры - это просто дети, одетые в панковскую и готическую одежду?"

# game/status.rpy:2814
translate russia task4_6d79f519:

    # a "I saw one turn into a bat!"
    a "Я видела, как один превратился в летучую мышь!"

# game/status.rpy:2815
translate russia task4_0e731260:

    # y "Did you...?"
    y "Да...?"

# game/status.rpy:2816
translate russia task4_e0668756:

    # a y14 "Well... okay I may have imagined that bit. But they could have!"
    a y14 "Нуу... ладно, возможно, я это выдумала. Но они точно так могут!"

# game/status.rpy:2817
translate russia task4_4a05d125_1:

    # y "Alex..."
    y "Алекс..."

# game/status.rpy:2818
translate russia task4_3613e5d8:

    # y "I won't deny that they're dangerous, but they're not supernatural. Did you even talk to any of them?"
    y "Не буду отрицать, они опасны, но в них нет ничего сверхъестественного. Ты хоть с кем-нибудь из них разговаривала?"

# game/status.rpy:2819
translate russia task4_cc1a93b4:

    # a "No... I was too busy hiding."
    a "Нет... Я пряталась."

# game/status.rpy:2820
translate russia task4_258a64b3:

    # y "Did you manage to gather any intel...?"
    y "Тебе удалось собрать какую-нибудь информацию...?"

# game/status.rpy:2821
translate russia task4_fab68504:

    # a "................"
    a "................"

# game/status.rpy:2822
translate russia task4_fe5def4a:

    # y "All right, you're going back tomorrow."
    y "Отлично, вернёшься туда завтра."

# game/status.rpy:2823
translate russia task4_d8f57c9c:

    # a y30 "W-what?!"
    a y30 "Ч-что?!"

# game/status.rpy:2824
translate russia task4_0b364178:

    # y "And this time, you're going to try and blend in, okay?"
    y "И на этот раз ты постараешься слиться с толпой, хорошо?"

# game/status.rpy:2825
translate russia task4_bbcccf4a:

    # a y29 "But what if they bite me....?"
    a y29 "Но что если они меня укусят...?"

# game/status.rpy:2826
translate russia task4_afabf367:

    # y "Then you bite back. Head back to your cell."
    y "Укуси их в ответ. Возвращайся в свою комнату."

# game/status.rpy:2827
translate russia task4_e1f0fb18:

    # a "But...!"
    a "Но...!"

# game/status.rpy:2828
translate russia task4_4de58a54:

    # y "Cell."
    y "В комнату."

# game/status.rpy:2829
translate russia task4_faebd152:

    # a y5 "{b}*Whines*{/b}"
    a y5 "{b}*Хнычет*{/b}"

# game/status.rpy:2867
translate russia task4_a23669b1_1:

    # y "............................"
    y "............................"

# game/status.rpy:2868
translate russia task4_e16d7062:

    # y "Again no Alex..."
    y "Алекс снова нет..."

# game/status.rpy:2869
translate russia task4_3a01f649:

    # y "Let's see if I can reach her..."
    y "Посмотрим, смогу ли я связаться с ней..."

# game/status.rpy:2870
translate russia task4_8d1377fe:

    # y "{b}*Beep*{/b}"
    y "{b}*Бип*{/b}"

# game/status.rpy:2876
translate russia task4_de7703ac:

    # a "I don't know if this is such a good idea..."
    a "Не знаю, хорошая ли это идея..."

# game/status.rpy:2877
translate russia task4_7545b397:

    # "Outsider" "Relax, we've done it before and it didn't hurt any of us."
    "Аутсайдер" "Расслабься, мы уже делали это раньше, и никому из нас это не навредило."

# game/status.rpy:2878
translate russia task4_b05151e8:

    # "Huge Outsider" "IT MAKE ME BIG! AND MAKE ME DREAM BIG! ME FOR PRESIDENT!"
    "Большой Аутсайдер" "ЭТО ДЕЛАЕТ МЕНЯ БОЛЬШИМ! И ЗАСТАВЛЯЕТ МЕНЯ МЕЧТАТЬ О МНОГОМ! МЕНЯ В ПРЕЗИДЕНТЫ!"

# game/status.rpy:2879
translate russia task4_ec54d11a:

    # "Outsider" "Okay so it might have some side effects if you use too many at once..."
    "Аутсайдер" "Ладно, есть побочные эффекты, если переборщить с дозой..."

# game/status.rpy:2880
translate russia task4_c3ef828d:

    # "Outsider" "But it's fine in small dosis. Plus it will really give us a leg up on the rival gangs."
    "Аутсайдер" "Но в небольших дозах всё будет нормально. К тому же это даст нам преимущество перед конкурирующими бандами."

# game/status.rpy:2881
translate russia task4_526f897a:

    # "Outsider" "Here you go, this one's free. Try it and see if you like it."
    "Аутсайдер" "Вот, держи, это бесплатно. Попробуй и посмотри, понравится тебе или нет."

# game/status.rpy:2882
translate russia task4_3b0c4392:

    # a y37 "Okay~...."
    a y37 "Хорошо~..."

# game/status.rpy:2883
translate russia task4_4355a92f:

    # y "Pst... Alex!"
    y "Пст... Алекс!"

# game/status.rpy:2884
translate russia task4_0f77755e:

    # a "...?"
    a "...?"

# game/status.rpy:2885
translate russia task4_28b2616a:

    # a y1 "Oh hello tiny [playerName]. I was just on my way back."
    a y1 "Ох, привет [playerName]. Я как раз возвращалась."

# game/status.rpy:2886
translate russia task4_6c525e17:

    # y "What was that all about?"
    y "Что это сейчас было?"

# game/status.rpy:2887
translate russia task4_da3bdd7c:

    # a "They gave me a nanobot injector. I'll tell you more once I get back to the base."
    a "Они дали мне инжектор наноботов. Расскажу тебе подробнее, когда вернусь на базу."

# game/status.rpy:2909
translate russia task4_783e868d:

    # a "Oof~...!"
    a "Фух~...!"

# game/status.rpy:2910
translate russia task4_878846ca:

    # a "Well that was fast..."
    a "Это было быстро..."

# game/status.rpy:2911
translate russia task4_7b895464:

    # y "So, what did you find out?"
    y "Итак, что ты выяснила?"

# game/status.rpy:2912
translate russia task4_146cd885:

    # a y1 "Well...!"
    a y1 "Нуу...!"

# game/status.rpy:2913
translate russia task4_3388e6d4:

    # a y29 "Wait, do I get a cookie for doing a good job?"
    a y29 "Подожди, а я получу печеньку за хорошую работу?"

# game/status.rpy:2914
translate russia task4_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:2917
translate russia task4_f57e4845:

    # a y1 "I can be! {b}*Wraf* *Arf*{/b}"
    a y1 "Я могу! {b}*Ваф* *Ваф*{/b}"

# game/status.rpy:2918
translate russia task4_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:2919
translate russia task4_9e1a0b33:

    # y "Oh my god, this is actually happening!"
    y "Боже мой, это происходит на самом деле!"

# game/status.rpy:2920
translate russia task4_d6e41a9c:

    # y "Alex, give paw!"
    y "Алекс, дай лапу!"

# game/status.rpy:2921
translate russia task4_50da059c:

    # "The spy plays along and gives you her hand."
    "Шпионка подыгрывает вам и протягивает руку."

# game/status.rpy:2922
translate russia task4_0e6bbe6c:

    # y "This is amazing!"
    y "Это потрясающе!"

# game/status.rpy:2923
translate russia task4_33cd0415:

    # y "Alex, roll over!"
    y "Алекс, перевернись!"

# game/status.rpy:2925
translate russia task4_3990d518:

    # y "Hah! Okay okay, one more. Alex....{w} stop messing around and tell me what you found out already."
    y "Ха! Ладно, ладно, ещё раз. Алекс...{w} перестань валять дурака и скажи мне, что ты выяснила."

# game/status.rpy:2926
translate russia task4_66302773:

    # a y29 "{b}*Warf* *Wa-*{/b}.... Oh...{w} Sorry, I sometimes get a bit too carried away!"
    a y29 "{b}*Ваф* *Ва-*{/b}... Ох...{w} Прости, я иногда слишком увлекаюсь!"

# game/status.rpy:2927
translate russia task4_601fb2da:

    # y "(Weirdo)."
    y "(Чудачка)."

# game/status.rpy:2930
translate russia task4_26f1406d:

    # y "Alex, I order you to report."
    y "Алекс, я приказываю тебе отчитаться."

# game/status.rpy:2931
translate russia task4_eb1ff9ca:

    # a y29 "N-no I was just kid-...."
    a y29 "Н-нет Я просто при-..."

# game/status.rpy:2932
translate russia task4_3c39faa4:

    # "....................."
    "....................."

# game/status.rpy:2933
translate russia task4_66139737:

    # a y39 "Reporting as ordered, sir....~"
    a y39 "Докладываю, как приказано, сэр...~"

# game/status.rpy:2935
translate russia task4_4391cfd0:

    # y "D-...{w}does someone want a Scooby Snack....?"
    y "К-...{w}кто-нибудь хочет Скуби-Снэк...?"

# game/status.rpy:2936
translate russia task4_3dcbb0b6:

    # a y16 "I'm not a dog, [playerName]~...."
    a y16 "Я не собака, [playerName]~..."

# game/status.rpy:2937
translate russia task4_646fc0db:

    # y "What about....{w}{b}2{/b} Scooby Snacks?"
    y "А как же....{w}{b}2{/b} Скуби Снэка?"

# game/status.rpy:2938
translate russia task4_34125414:

    # a y8 "[playerName]! Those are dog treats! Stop pretending that I'm a dog!"
    a y8 "[playerName]! Это собачьи лакомства! Хватит притворяться, что я собака!"

# game/status.rpy:2939
translate russia task4_23ce1a18:

    # y "Okay okay...."
    y "Ладно, ладно..."

# game/status.rpy:2940
translate russia task4_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:2941
translate russia task4_6a82aa3c:

    # a y10 "....................."
    a y10 "....................."

# game/status.rpy:2942
translate russia task4_392a6859:

    # y "What about {b}THREE{/b} Scooby Snacks?!"
    y "А как насчёт {b}трёх{/b} Скуби Снэков?!"

# game/status.rpy:2943
translate russia task4_4099692a:

    # a y40 "Argh! Never mind, I'll tell you all ready!"
    a y40 "Аргх! Забудь, я всё расскажу!"

# game/status.rpy:2944
translate russia task4_54843bdd:

    # a "So where to begin..."
    a "Итак, с чего начать..."

# game/status.rpy:2945
translate russia task4_e3799884:

    # a "So the vampires found out-..."
    a "Вампиры узнали-..."

# game/status.rpy:2946
translate russia task4_5af6cd56:

    # y "Vampires? Still on that?"
    y "Вампиры? Всё ещё не успокоилась?"

# game/status.rpy:2947
translate russia task4_591d0414:

    # a y39 ".................."
    a y39 ".................."

# game/status.rpy:2948
translate russia task4_83919b76:

    # a "So the {i}'alternative crowd'{/i} kids told me something about the nanobots."
    a "Дети из {i}'альтернативной группы'{/i} рассказали мне кое-что о наноботах."

# game/status.rpy:2949
translate russia task4_bbccc8c3:

    # a y29 "Apparently there are special nanobot injections that gives bonuses and secret powers which boost your natural abilities."
    a y29 "Очевидно что существуют специальные инъекции наноботов, которые дают бонусы и секретные способности, усиливающие ваши природные способности."

# game/status.rpy:2950
translate russia task4_1bbb369f:

    # y "Natural abilities?"
    y "Природные способности?"

# game/status.rpy:2951
translate russia task4_a575c9e8:

    # a "Yeah, y'know. Run faster, jump higher....{w} turn invisible."
    a "Да, знаешь. Бегать быстрее, прыгать выше....{w} становиться невидимым."

# game/status.rpy:2952
translate russia task4_a94335f3:

    # y "Wait, what was that last one?"
    y "Подожди, что там было последнее?"

# game/status.rpy:2953
translate russia task4_35402b38:

    # a "I know, right! Inject enough nanobots and you get super cool new abilities!"
    a "Ага, круто! Введи достаточное количество наноботов, и получаешь супер-крутые способности!"

# game/status.rpy:2954
translate russia task4_42e9a7c5:

    # a "Here...{w} They gave me an injector so I could try it out."
    a "Вот...{w} Они дали мне инжектор, чтобы я попробовала."

# game/status.rpy:2957
translate russia task4_6b5c85b4:

    # "You have unlocked the ability of boosting your spies' skills by injecting nanobots."
    "Вы открыли способность повышать навыки ваших шпионов, вводя им наноботов."

# game/status.rpy:2961
translate russia task4_35ac1262:

    # "Check your inventory for nanobot injectors. Using an injector will take you to the Injector screen."
    "Проверьте инвентарь на наличие инжекторов для наноботов. При использовании инжектора вы перейдете на экран \"Инжектор\"."

# game/status.rpy:2965
translate russia task4_d89b2398:

    # "Injecting your spies with nanobots will upgrade their skills when doing missions. Such as higher take-down chance or better hacking."
    "Инъекция повысит навыки шпионок при выполнении заданий. Например, более высокий шанс уничтожения или лучший взлом."

# game/status.rpy:2966
translate russia task4_c02806a2:

    # "Be warned however. If your spies nanobot control gets too high, it may bring unwanted side-effects with it."
    "Однако будьте внимательны. Если контроль наноботов над шпионами станет слишком высоким, это может принести с собой нежелательные побочные эффекты."

# game/status.rpy:2967
translate russia task4_0b751645:

    # "Spies will begin stealing money from their dayjobs, more easily get controlled during missions and might even decide to go AWOL for a few days."
    "Шпионы начнут воровать деньги со своей подработки, их будет легче контролировать во время миссий, и они даже могут решить уйти в самоволку на несколько дней."

# game/status.rpy:2968
translate russia task4_fc25d6f1:

    # "You can suppress a girl's nanobot control by having them preform sexual acts or adjusting their nanobot control levels via the STATUS screen."
    "Вы можете подавить контроль наноботов девушки, заставив их предварительно совершить сексуальные действия или отрегулировав уровень контроля наноботов на экране \"STATUS\"."

# game/status.rpy:2970
translate russia task4_551ad45d:

    # "......................."
    "......................."

# game/status.rpy:2971
translate russia task4_4ba5dc16:

    # a "But.."
    a "Но..."

# game/status.rpy:2972
translate russia task4_952e83ef:

    # y "But...?"
    y "Но...?"

# game/status.rpy:2973
translate russia task4_dade805e:

    # a y19 ".................."
    a y19 ".................."

# game/status.rpy:2974
translate russia task4_cf2b770a:

    # a "{size=-6}I don't like needles...{/size}"
    a "{size=-6}Я не люблю иголки...{/size}"

# game/status.rpy:2975
translate russia task4_6e4ecfcc:

    # y "........................."
    y "........................."

# game/status.rpy:2976
translate russia task4_13cee79c:

    # y "Get over here, I'll Inject you myself."
    y "Иди сюда, я сам сделаю тебе инъекцию."

# game/status.rpy:2977
translate russia task4_73fe7c0a:

    # a y29 "Noooo!"
    a y29 "Нееет!"

# game/status.rpy:2978
translate russia task4_8455b5ff:

    # y "Alex, get over here!"
    y "Алекс, иди сюда!"

# game/status.rpy:2979
translate russia task4_af479118:

    # a y28 "NOOOOOOO!"
    a y28 "НЕЕЕЕЕЕЕТ!"

# game/status.rpy:2980
translate russia task4_cf5a6f1f:

    # "You try grabbing the spy, who struggles to get away from you like a mange cat."
    "Вы пытаетесь схватить шпионку, которая вырваться от вас, как лишайный кот."

# game/status.rpy:2982
translate russia task4_ffcade69:

    # a "{size=+6}{b}NOOOOOOOOOOOO!{/b}{/size}"
    a "{size=+6}{b}НЕЕЕЕЕЕЕЕЕЕТ!{/b}{/size}"

# game/status.rpy:2984
translate russia task4_b70fd3ed:

    # s "What's going on in here...?"
    s "Что здесь происходит...?"

# game/status.rpy:2987
translate russia task4_52bd9c69:

    # a "Sam! Save me!!!"
    a "Сэм! Спаси меня!!!"

# game/status.rpy:2989
translate russia task4_b83cc4c6:

    # s g30 "H-hey, hey!{w} OW!"
    s g30 "Э-эй, эй!{w} ОУ!"

# game/status.rpy:2990
translate russia task4_4b358fd8:

    # "You accidentally injected Sam!"
    "Вы случайно укололи Сэм!"

# game/status.rpy:2991
translate russia task4_3b55fd51:

    # s g8 "Ow... What was that all abou-...."
    s g8 "Оу... Что это вообще был-..."

# game/status.rpy:2992
translate russia task4_0ab16323:

    # s "!!!"
    s "!!!"

# game/status.rpy:3006
translate russia task4_d2edf970:

    # s "All hail the mastermind! {w}Down with WOOHP!{w} DESTROY ALL OPPOSITION!"
    s "Да здравствует Организатор! {w}Долой WOOHP!{w} УНИЧТОЖИТЬ ВСЮ ОППОЗИЦИЮ!"

# game/status.rpy:3007
translate russia task4_b87d0150:

    # y "Okay Sam... I know that you're currently feeling the need to overthrow the Norwegian monarchy or something, but you gotta focus..."
    y "Хорошо, Сэм... Я знаю, что сейчас ты чувствуешь потребность свергнуть Норвежскую монархию или что-то в этом роде, но ты должна сосредоточиться..."

# game/status.rpy:3008
translate russia task4_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:3009
translate russia task4_8dd6365e:

    # y "I need you to turn invisible!"
    y "Мне нужно, чтобы ты стала невидимой!"

# game/status.rpy:3010
translate russia task4_201a4847:

    # s "{b}*Growls*{/b}"
    s "{b}*Рычит*{/b}"

# game/status.rpy:3011
translate russia task4_ceb71bde:

    # y "So you see, that's not actually a confirma-...."
    y "Это не совсем подтвержде-..."

# game/status.rpy:3017
translate russia task4_6e7949c7:

    # "Sam attacked you!{w} ...........{w} Again..."
    "Сэм напала на вас!{w}...........{w} Снова..."

# game/status.rpy:3018
translate russia task4_2cb514e1:

    # y "Oh crap! Alex, quickly break her out of her trance!"
    y "Вот дерьмо! Алекс, быстро выведи её из транса!"

# game/status.rpy:3019
translate russia task4_6dcfed77:

    # a "S-sure...!"
    a "К=конечно...!"

# game/status.rpy:3024
translate russia task4_420d6ddb:

    # "Alex firmly grabs Sam by her hair and pulls on it before kissing her tenderly on the lips."
    "Алекс крепко хватает Сэм за волосы и тянет за них, прежде чем нежно поцеловать её в губы."

# game/status.rpy:3025
translate russia task4_d11dfcee:

    # a "Hmm~.... ♥"
    a "Хмм~... ♥"

# game/status.rpy:3026
translate russia task4_c3339890:

    # "............................................."
    "............................................."

# game/status.rpy:3028
translate russia task4_677ae3f4:

    # y "Phew~... Disaster avoided. I was worried there for a secon-...."
    y "Фух~... Катастрофы удалось избежать. Я на секунду забеспокои-..."

# game/status.rpy:3032
translate russia task4_5a4b9c22:

    # "Sam attacked again!"
    "Сэм снова напал!"

# game/status.rpy:3033
translate russia task4_d5d5256d:

    # y "Argh...!"
    y "Аргх...!"

# game/status.rpy:3034
translate russia task4_4f1287cd:

    # "The spy jumps on top of you and begins wailing away."
    "Шпионка запрыгивает на вас и начинает выть."

# game/status.rpy:3040
translate russia task4_3fe18d79:

    # y "Ow! Aow! Alex, you didn't do it right!"
    y "Ой! Оу! Алекс, ты сделал это неправильно!"

# game/status.rpy:3041
translate russia task4_6290f9a4:

    # aM "I-... I'm sorry!"
    aM "Я-... Прости!"

# game/status.rpy:3044
translate russia task4_add2fb77:

    # aM "Although come to think of it...."
    aM "Хотя, если подумать, то..."

# game/status.rpy:3050
translate russia task4_836392bc:

    # aM "I do remember the Outsiders warning me about this and how to fix it..."
    aM "Я помню, что Аутсайдеры предупреждали меня об этом и о том, как это исправить..."

# game/status.rpy:3056
translate russia task4_b38408d8:

    # y "Today please, Alex!"
    y "Быстрее, пожалуйста, Алекс!"

# game/status.rpy:3057
translate russia task4_bc2f99ef:

    # aM "Some of the Outsiders figured out that oxyto-whatever reduces the nanobot control, but your brain gets used to the feeling after a while!"
    aM "Некоторые Аутсайдеры выяснили, что оксито-что-то-там уменьшает контроль над наноботами, но мозг через некоторое время привыкает к этому ощущению!"

# game/status.rpy:3058
translate russia task4_ccdfead2:

    # y "What?!"
    y "Что?!"

# game/status.rpy:3061
translate russia task4_0a37097d:

    # aM "They said that you may have to expose yourself to more risqué things else you stay mindcontrolled."
    aM "Они сказали, что возможно, придётся подвергать себя более рискованным вещам, иначе вы останетесь под контролем разума."

# game/status.rpy:3062
translate russia task4_b9cb14b1:

    # y "Risqué? Okay quickly! {w}Alex, show your boobs!"
    y "Рискованные вещи? Быстро! {w}Алекс, покажи свои сиськи!"

# game/status.rpy:3063
translate russia task4_b856409c:

    # aM "What? No way!"
    aM "Что? Ни за что!"

# game/status.rpy:3066
translate russia task4_7758e20e:

    # y "Alex! I'm starting to see in black and white, you gotta do it now!"
    y "Алекс! Я начинаю видеть всё в черно-белом цвете, ты должна сделать это сейчас!"

# game/status.rpy:3067
translate russia task4_6738742a:

    # aM "But...! I don't wanna show you my titties!{w} My mom always says: {w}''Alex, if a co-worker asks you to show your breasts, firmly tell them {b}no{/b}.''"
    aM "Но...! Я не хочу показывать тебе свои сиськи!{w} Моя мама всегда говорит: {w}\"Алекс, если коллега по работе просит тебя показать грудь, твердо скажи ему {b}нет{/b}.\""

# game/status.rpy:3073
translate russia task4_1ea800ad:

    # y "ALEX! I'M STARTING TO TASTE COLORS, DO IT NOW!"
    y "АЛЕКС! Я НАЧИНАЮ ЧУВСТВОВАТЬ ВКУС ЦВЕТОВ, СДЕЛАЙ ЭТО СЕЙЧАС ЖЕ!"

# game/status.rpy:3074
translate russia task4_e4abb7c7:

    # aM "........................."
    aM "........................."

# game/status.rpy:3075
translate russia task4_2b3a4b2f:

    # aM "Well... Okay fine. Just don't tell my momma, okay?"
    aM "Ну... Ладно, хорошо. Только не говори моей маме, хорошо?"

# game/status.rpy:3081
translate russia task4_74baeb06:

    # y "{b}*Gstfs~.... =(*{/b}"
    y "{b}*Гстфс~.... =(*{/b}"

# game/status.rpy:3092
translate russia task4_215bbe45:

    # s "I'm going to take you down....{w} I'm going to tie you up over a roaring fire...{w} And then I'm going to cannibalise you!"
    s "Я уничтожу тебя...{w} Я свяжу тебя и повешу над костром...{w} А потом я тебя сожру!"

# game/status.rpy:3095
translate russia task4_094e62ff:

    # s "Then... I'm.... going...{w} to....{b}{w}*Blinks*"
    s "А потом... я.... собираюсь...{w} {b}{w}*Моргает*"

# game/status.rpy:3100
translate russia task4_f08b4fee:

    # s g29 "Alex...?"
    s g29 "Алекс...?"

# game/status.rpy:3101
translate russia task4_56e02618:

    # s g28 "Oh no, [playerName]!"
    s g28 "О нет, [playerName]!"

# game/status.rpy:3105
translate russia task4_abe5baa8:

    # c "What's going on in here...?"
    c "Что здесь происходит...?"

# game/status.rpy:3106
translate russia task4_62a6a9ad:

    # c r51 "Er... Alex...{w} Your tits are out...{w} And what happened to [playerName]?!"
    c r51 "Эм... Алекс...{w} У тебя сиськи голые...{w} И что случилось с [playerName]?!"

# game/status.rpy:3107
translate russia task4_40756cf3:

    # y "{b}*gsfsfstfzle~....*{/b}"
    y "{b}*гфрстгрсстфх~....*{/b}"

# game/status.rpy:3108
translate russia task4_5949ba4d:

    # s g42 "I... I attacked him again...! I'm so sorry, [playerName]!"
    s g42 "Я... Я снова напал на него...! Мне очень жаль, [playerName]!"

# game/status.rpy:3117
translate russia task4_9cff8785:

    # a y29 "Is he going to be all right...?"
    a y29 "С ним всё будет в порядке...?"

# game/status.rpy:3118
translate russia task4_eb102c26:

    # y "I wanna {size=+6}go{/size}{size=+10}to{/size}{size=+6}DISNEYLAND{/size}{size=+6}and{/size}see{size=-4}the{/size}{size=-10}elepha-....{/size}"
    y "Хочу {size=+6}пойти{/size}{size=+10}в{/size}{size=+6}ДИСНЕЙЛЕНД{/size}{size=+6}и{/size}посмотреть{size=-4}на{/size}{size=-10}слон-...{/size}"

# game/status.rpy:3127
translate russia task4_f28e3669:

    # "You passed out."
    "Вы потерял сознание."

# game/status.rpy:3152
translate russia task5_8b257064:

    # y "Ugh... My head~..."
    y "Угх... Моя голова~..."

# game/status.rpy:3156
translate russia task5_93b6dfe8:

    # s "Good morning, [playerName]. Sorry about yesterday... Are you all right?"
    s "Доброе утро, [playerName]. Извини за вчерашнее... Ты в порядке?"

# game/status.rpy:3157
translate russia task5_6f7bac0f:

    # y "Yeah... It was a stupid idea of me to inject you guys with {i}'more'{/i} nanobots anyways."
    y "Ага... Как-никак это была моя глупая идея - вводить вам {i}'новых'{/i} наноботов."

# game/status.rpy:3158
translate russia task5_348d9452:

    # s g19 "Well.....{w} Maybe not entirely stupid."
    s g19 "Ну...{w} Может и не совсем глупо."

# game/status.rpy:3159
translate russia task5_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:3160
translate russia task5_918f8225:

    # s "After you passed out, I did some self-analysis and we did notice a difference in our skills..."
    s "После того как ты отключился, я провел самоанализ, и мы действительно заметили разницу навыках..."

# game/status.rpy:3161
translate russia task5_f4e265a7:

    # y "You did?"
    y "Да?"

# game/status.rpy:3162
translate russia task5_d42cc8ca:

    # s "Yeah, it seemed to have boosted my strength considerably. "
    s "Да, похоже, это значительно увеличило мои силы."

# game/status.rpy:3163
translate russia task5_8d51863e:

    # s "So taking down obstacles during missions should be easier now."
    s "Так что разрушать препятствия во время миссий теперь будет проще."

# game/status.rpy:3164
translate russia task5_24484078:

    # y "It's no invisibility, but it's something."
    y "Это не невидимость, но уже хоть что-то."

# game/status.rpy:3165
translate russia task5_71c6767a:

    # s "There's a risk though. The more nanobots we inject, the more likely we are to being taken over."
    s "Однако есть риск. Чем больше наноботов мы вводим, тем больше вероятность того, что нас захватят."

# game/status.rpy:3166
translate russia task5_5344f70f:

    # s "Which means more serious ways of suppressing our nanobots."
    s "А ещё это означает более серьезные способы подавления наноботов."

# game/status.rpy:3167
translate russia task5_0f4558bc:

    # y "Interesting... we should keep our eyes out for these special injectors."
    y "Интересно... нам стоит следить за этими специальными инжекторами."

# game/status.rpy:3168
translate russia task5_972b660d:

    # y "So you're saying you really got stronger, just by injecting a syringe of nanobots?"
    y "То есть ты хочеш сказать, что действительно стала сильнее, из-за одного шприца с наноботами?"

# game/status.rpy:3170
translate russia task5_6f39f4ce:

    # s g1 "Yeah! Here I'll show you."
    s g1 "Да! Вот я тебе покажу."

# game/status.rpy:3171
translate russia task5_e3d6cd2f:

    # y "No no n-..."
    y "Нет нет не-..."

# game/status.rpy:3180
translate russia task5_5d83221a:

    # y "Gah!" with hpunch
    y "Гах!" with hpunch

# game/status.rpy:3187
translate russia task5_304de4ac:

    # c "He's waking up!"
    c "Он просыпается!"

# game/status.rpy:3188
translate russia task5_9a215250:

    # y "Ugh~...."
    y "Угх~..."

# game/status.rpy:3189
translate russia task5_8b12f51e:

    # s "Are you okay, [playerName]...?"
    s "Ты в порядке, [playerName]...?"

# game/status.rpy:3190
translate russia task5_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:3191
translate russia task5_fa7f0302:

    # y "Yeah I'd say you're {i}'definitely'{/i} stronger."
    y "Да, ты {i}'определенно'{/i} стала сильнее."

# game/status.rpy:3192
translate russia task5_e9ad6835:

    # y "This is great news!"
    y "Это отличные новости!"

# game/status.rpy:3194
translate russia task5_b9edec29:

    # s g19 "........................................"
    s g19 "........................................"

# game/status.rpy:3195
translate russia task5_cfdbdf80:

    # c r32 "[playerName]....."
    c r32 "[playerName]....."

# game/status.rpy:3196
translate russia task5_440f8ba7:

    # c r31 "You've been out for 24 hours."
    c r31 "Ты был в отключке 24 часа."

# game/status.rpy:3197
translate russia task5_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:3198
translate russia task5_1cf5428b:

    # c "And Sam here hasn't really been feeling herself as of late. Have you Sam...?"
    c "А Сэм в последнее время неважно себя чувствует. Так Сэм...?"

# game/status.rpy:3199
translate russia task5_5c6cf512:

    # s ".........................."
    s ".........................."

# game/status.rpy:3200
translate russia task5_08691a54:

    # y "What happened?"
    y "Что случилось?"

# game/status.rpy:3201
translate russia task5_a8803355:

    # c "The nanobot injector seems to have boosted Sam's abilities, but she's much more prone to getting controlled now."
    c "Инжектор наноботов, усилил способности Сэм, но теперь она гораздо больше подвержена контролю."

# game/status.rpy:3202
translate russia task5_f7ee7bb7:

    # y "Can't we just have Alex show her tits again?"
    y "Может, пусть Алекс снова покажет свои сиськи?"

# game/status.rpy:3203
translate russia task5_cc01dc5d:

    # s "It helps... but only a little..."
    s "Это помогает... но только немного..."

# game/status.rpy:3204
translate russia task5_ecd4455f:

    # s "We might have to think of a more effective way..."
    s "Нам придётся придумать более эффективный способ..."

# game/status.rpy:3205
translate russia task5_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:3210
translate russia task5Menu1_b81caa76:

    # y "I'm gonna buy you three a dildo."
    y "Куплю вам троим по фаллоимитатору."

# game/status.rpy:3211
translate russia task5Menu1_9b6b58b9:

    # s g28 "A what?!"
    s g28 "Что?!"

# game/status.rpy:3212
translate russia task5Menu1_03f6c265:

    # c r18 "A dildo...?"
    c r18 "Дилдо...?"

# game/status.rpy:3213
translate russia task5Menu1_1bce17a4:

    # y "Yeah, it seems like a good first step. It should keep your nanobots suppressed for a while if you use one."
    y "Да, для начала они отлично подойдут. С их помощью наноботы будут подавлены на некоторое время."

# game/status.rpy:3214
translate russia task5Menu1_9fedf251:

    # s g30 "First step...?"
    s g30 "Для начала...?"

# game/status.rpy:3215
translate russia task5Menu1_622df90a:

    # c r48 "Er... heh heh~... erm... I guess so...."
    c r48 "Эм... хех хех~... кхем... Надеюсь..."

# game/status.rpy:3219
translate russia task5Menu1_2f821584:

    # a "[playerName] you woke up!"
    a "[playerName] ты очнулся!"

# game/status.rpy:3220
translate russia task5Menu1_18544bcd:

    # a "What are you all talking about?"
    a "О чём вы говорите?"

# game/status.rpy:3221
translate russia task5Menu1_c26daee1:

    # y "Dildos."
    y "О фаллоимитаторах."

# game/status.rpy:3222
translate russia task5Menu1_c68162ae:

    # a y18 "....?"
    a y18 "....?"

# game/status.rpy:3223
translate russia task5Menu1_1a6bdc6f:

    # a "What's that?"
    a "Что это такое?"

# game/status.rpy:3228
translate russia task5Menu1_a1aa9648:

    # s "{b}*Whisper* *Whisper* *Whisper*{/b}"
    s "{b}*Шепчет* *Шепчет* *Шепчет*{/b}"

# game/status.rpy:3229
translate russia task5Menu1_750c41cd:

    # a y29 "Oooooh.....{w}.........{w} Why are we talking about that?!"
    a y29 "Оооох.....{w}.........{w} Почему мы говорим об этом?!"

# game/status.rpy:3230
translate russia task5Menu1_492db706:

    # y "I'm buying all three of you a dildo."
    y "Я куплю каждой из вас по фаллоимитатору."

# game/status.rpy:3231
translate russia task5Menu1_735a8a2e:

    # "..................."
    "..................."

# game/status.rpy:3232
translate russia task5Menu1_6fcebf96:

    # "The three girls all look nervous, but not reluctant. {w}Maybe even.... excited...?"
    "Все три девушки выглядят встревоженно. {w}Может даже... возбуждённо...?"

# game/status.rpy:3233
translate russia task5Menu1_1a32a82c:

    # y "I'm gonna go find you girls some toys. If Sam goes crazy again, you know what do do."
    y "Пойду поищу вам, девочки, игрушки. Если Сэм снова сойдет с ума, ты знаешь, что делать."

# game/status.rpy:3234
translate russia task5Menu1_0729e1ea:

    # "The girls nod and head back to their cells."
    "Девочки кивают и возвращаются в свои камеры."

# game/status.rpy:3242
translate russia task5Menu1_62919272:

    # y "(The mall is probably a good place to look...)"
    y "(Возможно, торговый центр подойдёт для поиска...)"

# game/status.rpy:3252
translate russia task5Menu1_2c74dab9:

    # y "Let's remove your shower curtains."
    y "Давайте снимем ваши занавески для душа."

# game/status.rpy:3254
translate russia task5Menu1_484b48ba:

    # s g16 "W-what...?!"
    s g16 "Ч-что...?!"

# game/status.rpy:3255
translate russia task5Menu1_0b5854b6:

    # c r18 "You mean in our cells?"
    c r18 "Ты про те что в наших камерах?"

# game/status.rpy:3256
translate russia task5Menu1_34fa9179:

    # y "Yup."
    y "Ага."

# game/status.rpy:3257
translate russia task5Menu1_aae18956:

    # s g17 "B-but what if we want to take a shower?!"
    s g17 "Н-но что, если мы захотим принять душ?!"

# game/status.rpy:3258
translate russia task5Menu1_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:3259
translate russia task5Menu1_d30fd539:

    # s g19 "Oh... That's kinda the point... okay got it."
    s g19 "Оо... В этом-то и смысл... ладно, поняла."

# game/status.rpy:3260
translate russia task5Menu1_868ea28c:

    # c r23 "Well... I guess showing ourselves off a little bit from time to time isn't gonna hurt...."
    c r23 "Нуу... Думаю, время от времени немного показать себя не помешает..."

# game/status.rpy:3261
translate russia task5Menu1_bae58d2d:

    # c "We've seen eachother naked before after all."
    c "В конце концов, мы уже видели друг друга обнажёнными."

# game/status.rpy:3262
translate russia task5Menu1_a415e018:

    # s g32 "T-that's not the problem..."
    s g32 "П-проблема не в этом..."

# game/status.rpy:3263
translate russia task5Menu1_3531ab53:

    # "The spies look at you."
    "Шпионки смотрят на вас."

# game/status.rpy:3264
translate russia task5Menu1_f8303732:

    # y "The problem is I. {w}I am the problem."
    y "Проблема во мне. {w}Я и есть проблема."

# game/status.rpy:3265
translate russia task5Menu1_b6c8882c:

    # c r10 "............."
    c r10 "............."

# game/status.rpy:3266
translate russia task5Menu1_10fe9619:

    # s "............."
    s "............."

# game/status.rpy:3267
translate russia task5Menu1_d3daa906:

    # y "It'll be a good way to help reduce the nanobot control. Plus, I get to see you guys naked from time to time! It's a win/win!"
    y "Это отличный способ уменьшить контроль наноботов. К тому же, я могу время от времени смотреть на вас голыми! Это очень выгодно!"

# game/status.rpy:3268
translate russia task5Menu1_9f18ea76:

    # s "[playerName]....."
    s "[playerName]....."

# game/status.rpy:3269
translate russia task5Menu1_09041974:

    # c r12 "He might have a point, Sam..."
    c r12 "Возможно, он прав, Сэм..."

# game/status.rpy:3270
translate russia task5Menu1_18fb87da:

    # c "So what... he'll be able to sneak a peak at us while showering. Let him, it's not the end of the world."
    c "Ну и что... он сможет подглядывать за нами во время душа. Пусть, это не конец света."

# game/status.rpy:3271
translate russia task5Menu1_f7845a97:

    # s g29 "B-but....!"
    s g29 "Н-но...!"

# game/status.rpy:3272
translate russia task5Menu1_e4d2cb21:

    # c "It's fine. We've got nothing to be ashamed of and I'm sure he won't be a perv all the time, right [playerName]?"
    c "Всё в порядке. Нам нечего стыдиться, и я уверена, что он не будет всё время подглядывать, верно, [playerName]?"

# game/status.rpy:3273
translate russia task5Menu1_748680fc:

    # y "I make no promises."
    y "Не могу обещать."

# game/status.rpy:3274
translate russia task5Menu1_fff3959f:

    # s g39 "................."
    s g39 "................."

# game/status.rpy:3275
translate russia task5Menu1_d7cd4aaa:

    # s g38 "{size=-8}Fine...{/size}"
    s g38 "{size=-8}Ладно...{/size}"

# game/status.rpy:3276
translate russia task5Menu1_f2ac4552:

    # s "{size=-8}I... I don't like it, but I think it's a good idea...{/size}"
    s "{size=-8}Мне... Мне это не нравится, но я думаю, что это хорошая идея...{/size}"

# game/status.rpy:3277
translate russia task5Menu1_e54a75ec:

    # c "Fine, [playerName], feel free to remove our shower curtains if you want."
    c "Ладно, [playerName], не стесняйся, снимай наши занавески, если хочешь."

# game/status.rpy:3278
translate russia task5Menu1_176b389d:

    # s g37 "But...."
    s g37 "Но..."

# game/status.rpy:3279
translate russia task5Menu1_f393a590:

    # s "I don't think that's going to completely solve this nanobot problem..."
    s "Я не думаю, что это полностью решит проблему наноботов..."

# game/status.rpy:3280
translate russia task5Menu1_1ca78a1c:

    # c "Yeah...."
    c "Даа..."

# game/status.rpy:3283
translate russia task5Menu1_acfd4a41:

    # s g30 "!!!!"
    s g30 "!!!"

# game/status.rpy:3284
translate russia task5Menu1_1545e64b:

    # c r30 "H-HEY!!!"
    c r30 "Э-эй!!!"

# game/status.rpy:3285
translate russia task5Menu1_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:3286
translate russia task5Menu1_33326226:

    # y "Sorry, I may have gotten a bit too excited."
    y "Извините, возможно я слишком переволновался."

# game/status.rpy:3287
translate russia task5Menu1_690b7a06:

    # s g8 "We're not sleeping with you, if that's what you're implying!"
    s g8 "Мы не собираемся спать с тобой, если это то на что ты намекаешь!"

# game/status.rpy:3288
translate russia task5Menu1_074c37df:

    # y "What?! That's not at all what I was implying!"
    y "Что?! Это совсем не то, на что я намекал!"

# game/status.rpy:3289
translate russia task5Menu1_5f59bbce:

    # s g19 "O-oh... Sorry, I thought that y-..."
    s g19 "О-ох... Прости, я думала, что ты..."

# game/status.rpy:3290
translate russia task5Menu1_30b06795:

    # c r12 "Sam... that's {i}'exactly'{/i} what he's implying."
    c r12 "Сэм... это {i}'именно'{/i} то, на что он намекает."

# game/status.rpy:3291
translate russia task5Menu1_4176dec4:

    # y "{b}*Smirk*{/b}"
    y "{b}*Ухмылка*{/b}"

# game/status.rpy:3292
translate russia task5Menu1_a0787a1b:

    # s g8 "I-.. I knew that!{w} Don't you even think about it, [playerName]!"
    s g8 "Я-.. Я так и знала!{w} Даже не думай об этом, [playerName]!"

# game/status.rpy:3293
translate russia task5Menu1_80e43621:

    # c "We're not sleeping with you."
    c "Мы не будем спать с тобой."

# game/status.rpy:3294
translate russia task5Menu1_98b33962:

    # y "Ahw...."
    y "Хмм..."

# game/status.rpy:3295
translate russia task5Menu1_fe5588d3:

    # c "Now let's think of an actual idea."
    c "Теперь давайте подумаем над более реалистичной идеей."

# game/status.rpy:3299
translate russia task5Menu1_8ad4a0e4:

    # y "I have a great idea! We should-...{w}{b}*Hmpf!*{/b}"
    y "У меня есть отличная идея! Мы должны-... {w}{b}*Кхм!*{/b}"

# game/status.rpy:3300
translate russia task5Menu1_f38130cb:

    # "Clover put her hand over your mouth!"
    "Кловер закрыла ваш рот рукой!"

# game/status.rpy:3301
translate russia task5Menu1_781c39e6:

    # c r10 "Before you say anything, ask yourself: {i}'Am I going to say something insane?'{/i} and then ask yourself {i}'Is this in any way going to improve our situation?'{/i}"
    c r10 "Прежде чем что-то сказать, спроси себя: {i}'Собираюсь ли сказать что-то безумное?'{/i} а затем спросите себя {i}'Улучшит ли это как-то наше положение?'{/i}"

# game/status.rpy:3302
translate russia task5Menu1_6a86a276:

    # y "R-....{w}Rhinos..."
    y "Н-...{w}Носороги..."

# game/status.rpy:3303
translate russia task5Menu1_43f5d8d4:

    # c "[playerName]~...."
    c "[playerName]~...."

# game/status.rpy:3304
translate russia task5Menu1_8fee1b3c:

    # y "Okay fine... Rhinos might not be the solution....{w} At this time...."
    y "Ладно, хорошо... Носороги не могут быть решением наших проблем...{w} На данный момент..."

# game/status.rpy:3305
translate russia task5Menu1_f20b8e3e:

    # y "Oh! But we cou-...."
    y "Ох! Но мы може-..."

# game/status.rpy:3306
translate russia task5Menu1_3964ecc6:

    # c "We can't call the Justice League either."
    c "Лигу Справедливости мы тоже не можем вызвать."

# game/status.rpy:3307
translate russia task5Menu1_5a5fcd5a:

    # y "Oh...."
    y "Ох..."

# game/status.rpy:3308
translate russia task5Menu1_3242bf20:

    # y "B-..."
    y "Н-..."

# game/status.rpy:3309
translate russia task5Menu1_bc09713d:

    # c "And neither can we call the Avengers."
    c "И тем более Мстителей."

# game/status.rpy:3310
translate russia task5Menu1_eca1b893:

    # y "I'm out of ideas."
    y "Всё, у меня больше нет идей."

# game/status.rpy:3314
translate russia task5Menu1_641080f1:

    # y "(Dildos.... dildos...{w} Where can I find those...?)"
    y "(Дилдо... дилдо...{w} Где мне их найти...?)"

# game/status.rpy:3315
translate russia task5Menu1_f28802ce:

    # y ".................................."
    y ".................................."

# game/status.rpy:3320
translate russia task5Menu1_6281cbe6:

    # "Clerk" "Hello sir, welcome to the stor-....{w} You again?"
    "Продавщица" "Здравствуйте, сэр, добро пожалова-...{w} Снова ты?"

# game/status.rpy:3321
translate russia task5Menu1_4630f6e9:

    # y "Yes! I have an urgent need for dildos!!!"
    y "Да! Мне срочно нужны фаллоимитаторы!!!"

# game/status.rpy:3323
translate russia task5Menu1_ff3425b3:

    # "Clerk" "........................."
    "Продавщица" "........................."

# game/status.rpy:3325
translate russia task5Menu1_83192bd5:

    # "Clerk" "Okay.... after you give me what you owe."
    "Продавщица" "Окей... после того, как ты отдашь мне то, что должен."

# game/status.rpy:3326
translate russia task5Menu1_fb04494a:

    # y "Owe?"
    y "То, что должен?"

# game/status.rpy:3327
translate russia task5Menu1_6917ee8d:

    # "Clerk" "You... You promised me pictures!"
    "Продавщица" "Ты... Ты обещал мне фотографии!"

# game/status.rpy:3328
translate russia task5Menu1_7dd6fa04:

    # y "Riiiiight~...."
    y "Тоооооочно~..."

# game/status.rpy:3329
translate russia task5Menu1_56cf6bcc:

    # "Clerk" ".........................."
    "Продавщица" ".........................."

# game/status.rpy:3330
translate russia task5Menu1_0bb3a8cb:

    # y "..............................."
    y "..............................."

# game/status.rpy:3331
translate russia task5Menu1_07557eb2:

    # "Clerk" "You don't have them, do you?"
    "Продавщица" "У тебя их нет, не так ли?"

# game/status.rpy:3332
translate russia task5Menu1_d1998e52:

    # y "I may have forgotten~...."
    y "Допустим, что я забыл~..."

# game/status.rpy:3335
translate russia task5Menu1_1e73550e:

    # "Clerk" "And are you going to steal them like you did the waitress uniform?"
    "Продавщица" "И ты собираешься украсть их, как украл форму официантки?"

# game/status.rpy:3336
translate russia task5Menu1_63947ea0:

    # y "Steal?! There must be some kind of mistake! I didn't steal the uniform....!"
    y "Украсть?! Это должна быть какая-то ошибка! Я не крал форму....!"

# game/status.rpy:3337
translate russia task5Menu1_7821d52f:

    # y "I er...."
    y "Я эм..."

# game/status.rpy:3338
translate russia task5Menu1_d2062753:

    # y "Borrowed it for Marketing Purposes.....~"
    y "Позаимствовал её для маркетинговых целей...~"

# game/status.rpy:3339
translate russia task5Menu1_faa08d88:

    # "Clerk" "................................"
    "Продавщица" "................................"

# game/status.rpy:3340
translate russia task5Menu1_15456a20:

    # y "Okay fine... maybe we can work out a deal...."
    y "Ладно, хорошо... может быть, мы сможем заключить сделку..."

# game/status.rpy:3341
translate russia task5Menu1_c235ed40:

    # "Clerk" "Fine...."
    "Продавщица" "Ладно..."

# game/status.rpy:3342
translate russia task5Menu1_b9b6a9db:

    # "Clerk" "You can either pay me $[task5DildoPrice] or...."
    "Продавщица" "Ты можешь заплатить мне $[task5DildoPrice] или..."

# game/status.rpy:3343
translate russia task5Menu1_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:3344
translate russia task5Menu1_993275b6:

    # "Clerk" "Erm....."
    "Продавщица" "Эм..."

# game/status.rpy:3345
translate russia task5Menu1_02cddb2e:

    # "Or you can send me pics....."
    "Или можешь прислать мне фотографии..."

# game/status.rpy:3350
translate russia task5Menu1_f6260184:

    # "You have unlocked the ability to take photos of your spies!"
    "Вы разблокировали возможность фотографировать своих шпионок!"

# game/status.rpy:3351
translate russia task5Menu1_293cf31d:

    # "At night you can ask your spy to pose for you. Taken pictures will be added to your inventory."
    "Ночью вы можете попросить свою шпионку позировать для вас. Сделанные снимки будут добавлены в ваш инвентарь."

# game/status.rpy:3352
translate russia task5Menu1_9d2c2fdc:

    # "Getting photographed slightly lowers a spy's nanobot control, but also lowers their morale."
    "Фотографирование немного снижает контроль наноботов над шпионкой, но также снижает её моральный дух."

# game/status.rpy:3353
translate russia task5Menu1_68f16a11:

    # "Pictures can be traded with the female store Clerk for kinky gear and toys."
    "Фотографии можно обменять у продавщицы на извращенные приспособления и игрушки."

# game/status.rpy:3362
translate russia task5Menu1_373272e8:

    # "Clerk" "So? What will it be?"
    "Продавщица" "И? Что ты выбираешь?"

# game/status.rpy:3366
translate russia task5Menu1_47b868b4:

    # y "I'll be back!"
    y "Я вернусь!"

# game/status.rpy:3376
translate russia task5Menu1_3f72bfeb:

    # y "(Dang, I don't have enough money!)"
    y "(Чёрт, у меня не хватает денег!)."

# game/status.rpy:3378
translate russia task5Menu1_f79c7075:

    # "Clerk" "..........................."
    "Продавщица" "..........................."

# game/status.rpy:3379
translate russia task5Menu1_63222ad0:

    # y "I seem to be light on money...{w} again..."
    y "Похоже, у меня проблема с деньгами...{w} опять..."

# game/status.rpy:3380
translate russia task5Menu1_9807ef83:

    # y "I'll return later!"
    y "Я вернусь позже!"

# game/status.rpy:3390
translate russia task5Menu1_e121ec4c:

    # "Clerk" "Pleasure doing business with you...."
    "Продавщица" "Приятно иметь с тобой дело..."

# game/status.rpy:3391
translate russia task5Menu1_1d264e16:

    # "Clerk" "Please follow me."
    "Продавщица" "Пожалуйста, следуйте за мной."

# game/status.rpy:3399
translate russia task5Menu1_2fe65824:

    # "Clerk" "Oh you're back."
    "Продавщица" "Оо ты вернулся."

# game/status.rpy:3406
translate russia task5Menu1_9c1d1874:

    # y "Here you go, you perv."
    y "Вот, забирай, извращенка."

# game/status.rpy:3408
translate russia task5Menu1_a881e2f8:

    # "Clerk" "Takes one to know one."
    "Продавщица" "Рыбак рыбака видит издалека."

# game/status.rpy:3410
translate russia task5Menu1_73b6532b:

    # "Clerk" "All right, let's get you some dildos."
    "Продавщица" "Отлично, теперь давай достанем для тебя пару дилдоков."

# game/status.rpy:3415
translate russia task5Menu1_45e0f1ec:

    # "Clerk" "Fair enough. All right, this way. We keep them in the Dicktionary section."
    "Продавщица" "Справедливо. Ладно, сюда. Мы храним их в разделе словарей."

# game/status.rpy:3416
translate russia task5Menu1_59d96f52:

    # y "All right!{w} ...... wait what?"
    y "Отлично!{w} ...... стоп что?"

# game/status.rpy:3418
translate russia task5Menu1_bc1753b4:

    # "Clerk" "You heard what I said...."
    "Продавщица" "Ты всё слышал..."

# game/status.rpy:3433
translate russia task5Menu1_4e01fa9d:

    # "Clerk" "What kind do you want?"
    "Продавщица" "Тебе какого типа?"

# game/status.rpy:3434
translate russia task5Menu1_2a110c9c:

    # y "What kind?"
    y "Какого типа?"

# game/status.rpy:3435
translate russia task5Menu1_c5ce9fcd:

    # "Clerk" "Yeah, we've got small ones, big ones... gigantic ones."
    "Продавщица" "Да, у нас есть маленькие, большие... гигантские."

# game/status.rpy:3436
translate russia task5Menu1_0acbb954:

    # "Clerk" "Ones that vibrate, ones that sparkle... {w}ones that call you a whore."
    "Продавщица" "Вибрирующие, те, что блестят... {w}те, что называют тебя шлюхой."

# game/status.rpy:3437
translate russia task5Menu1_dd7a34e7:

    # y "I never knew there were so many options...."
    y "Я не знал, что существует так много вариантов..."

# game/status.rpy:3438
translate russia task5Menu1_2c09a3a2:

    # "Clerk" "Ones that go in both holes, ones labelled 'friendzone'.... those are for decoration only."
    "Продавщица" "Для двух дырок сразу, с пометкой как \"френдзона\" ... они предназначены только для украшения."

# game/status.rpy:3439
translate russia task5Menu1_c2b1e20e:

    # y "I just want three normal ones."
    y "Мне просто нужны три самых простых."

# game/status.rpy:3441
translate russia task5Menu1_8e296a7e:

    # "Clerk" "Boring...."
    "Продавщица" "Скукотища..."

# game/status.rpy:3442
translate russia task5Menu1_b6b6cb8b:

    # "Clerk" "I got these ones in green, yellow and red."
    "Продавщица" "Я меня есть эти, в зелёном, жёлтом и красном цветах."

# game/status.rpy:3443
translate russia task5Menu1_5f96e9b7:

    # y "Perfect! I'll take 'em."
    y "Идеально! Я беру их."

# game/status.rpy:3444
translate russia task5Menu1_2f471475:

    # "Clerk" "All right, I'll wrap them up for you. Come again...."
    "Продавщица" "Отлично, я заверну их для тебя. Приходи снова..."

# game/status.rpy:3447
translate russia task5Menu1_62697d44:

    # "You got {color=#ffeda6}Dildos{/color}!"
    "Вы получили {color=#ffeda6}Дилдо{/color}!"

# game/status.rpy:3452
translate russia task5Menu1_1d9ef842:

    # y "(I should talk it over with my spies when I get the chance.)"
    y "(Когда у меня будет возможность, стоит обсудить это со своими шпионками.)"

# game/status.rpy:3472
translate russia task5Menu1_09f3fa76:

    # y "Girls! I brought you all a gift!"
    y "Девочки! Я принёс вам подарки!"

# game/status.rpy:3473
translate russia task5Menu1_92f5be84:

    # s "......................"
    s "......................"

# game/status.rpy:3474
translate russia task5Menu1_96c404cc:

    # c r11 "Oh boy...."
    c r11 "О боже..."

# game/status.rpy:3475
translate russia task5Menu1_b5110cc8:

    # a y16 "Why are they shaped like penises?"
    a y16 "Почему они похожи на пенисы?"

# game/status.rpy:3476
translate russia task5Menu1_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:3477
translate russia task5Menu1_fdb3b1e1:

    # y "Alex, I'm revoking your speaking privileges."
    y "Алекс, я лишаю тебя права разговаривать."

# game/status.rpy:3478
translate russia task5Menu1_6b8222a7:

    # a y30 "....!"
    a y30 "....!"

# game/status.rpy:3479
translate russia task5Menu1_3990ec58:

    # y "How about you give them a go!"
    y "Как насчёт \"тестдрайва\"!"

# game/status.rpy:3480
translate russia task5Menu1_8082aa1c:

    # c r37 "Er....."
    c r37 "Эм..."

# game/status.rpy:3481
translate russia task5Menu1_9063f3be:

    # s g29 "Right now....?"
    s g29 "Прямо сейчас...?"

# game/status.rpy:3482
translate russia task5Menu1_9fe7fc6f:

    # a "........?!"
    a "........?!"

# game/status.rpy:3483
translate russia task5Menu1_3403b707:

    # c r35 "Let's just do it girls. It'll probably knock the nanobot control back quite a bit."
    c r35 "Давайте просто сделаем это, девочки. Скорее всего это сильно отбросит контроль наноботов назад."

# game/status.rpy:3484
translate russia task5Menu1_e1b78b0d:

    # c "And I'm sure [playerName] won't be there with us. Right [playerName]?"
    c "И я уверена, что [playerName] не будет там с нами. Ведь так [playerName]?"

# game/status.rpy:3485
translate russia task5Menu1_6d8f2d5a:

    # y "I... I won't?"
    y "Я... Я не буду?"

# game/status.rpy:3486
translate russia task5Menu1_2654c61f:

    # s g31 "No you won't!"
    s g31 "Нет, не будешь!"

# game/status.rpy:3487
translate russia task5Menu1_6c5a3797:

    # a y32 "....!"
    a y32 "....!"

# game/status.rpy:3488
translate russia task5Menu1_5b9099d6:

    # y "Alright fine...."
    y "Лааадно...."

# game/status.rpy:3489
translate russia task5Menu1_fd98c066:

    # s g33 "................."
    s g33 "................."

# game/status.rpy:3490
translate russia task5Menu1_55041608:

    # s "Well okay then..."
    s "Ну тогда ладно..."

# game/status.rpy:3491
translate russia task5Menu1_25d0b8a1:

    # c r23 "Let's take this baby for a spin!"
    c r23 "Давайте прокатимся на этих малышах!"

# game/status.rpy:3492
translate russia task5Menu1_3c86b4a9:

    # a y2 ".........!"
    a y2 ".........!"

# game/status.rpy:3500
translate russia task5Menu1_d7c1b69e:

    # "Inner Voice" "Dude, you gave in way too quick, you should have insisted on staying!"
    "Внутренний голос" "Чувак, ты слишком быстро сдался. Нужно было настоять на том, чтобы остаться!"

# game/status.rpy:3501
translate russia task5Menu1_b2496b79:

    # y "Spy cameras."
    y "Скрытые камеры."

# game/status.rpy:3502
translate russia task5Menu1_30ca196e:

    # "Inner Voice" "Oh right.{w} Niiiice~...."
    "Внутренний голос" "Ох, точно.{w} Клааасс~..."

# game/status.rpy:3536
translate russia task5Menu1_9d447f81:

    # y "Now girls. I've checked your nanobots levels and you should spend the evening masturbating again."
    y "Теперь девочки. Я проверил уровень ваших наноботов, и вам следует провести вечер за мастурбацией."

# game/status.rpy:3537
translate russia task5Menu1_219582d5:

    # s g15 "................"
    s g15 "................"

# game/status.rpy:3538
translate russia task5Menu1_71955456:

    # a "Okay..."
    a "Ладно..."

# game/status.rpy:3539
translate russia task5Menu1_4625ece8:

    # c r11 "That's fine."
    c r11 "Отлично."

# game/status.rpy:3544
translate russia task5Menu1_f6b71780:

    # "The girls return to their cells...."
    "Девушки возвращаются в свои камеры..."

# game/status.rpy:3546
translate russia task5Menu1_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:3551
translate russia task5Menu1_3b3c0d81:

    # "Inner Voice" "Spy cameras?"
    "Внутренний голос" "Камеры?"

# game/status.rpy:3552
translate russia task5Menu1_2c06bd66:

    # y "Hell yeah."
    y "Конечно."

# game/status.rpy:3556
translate russia task5Menu1_d2644b52:

    # "{b}*Beep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:3557
translate russia task5Menu1_9aa442a0:

    # a "Ah ah ah- ahh!! ♥"
    a "Ах Аах ох- АхХ!! ♥"

# game/status.rpy:3558
translate russia task5Menu1_7cbe0ba5:

    # y "That's it, ride that cock!"
    y "Вот так, да, оседлай этот член!"

# game/status.rpy:3559
translate russia task5Menu1_d2644b52_1:

    # "{b}*Beep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:3560
translate russia task5Menu1_91cc214c:

    # c "Y-yes... ahh ahh mhmmm~...! ♥♥♥"
    c "Д-да... Аах ахХ мммфх~...! ♥♥♥"

# game/status.rpy:3561
translate russia task5Menu1_8161bdb3:

    # y "How you like that you little slut?!"
    y "Наслаждаешься, маленькая шлюшка?!"

# game/status.rpy:3562
translate russia task5Menu1_d2644b52_2:

    # "{b}*Beep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:3563
translate russia task5Menu1_1a4d594a:

    # y "Take it you redhead piece of meat!"
    y "Вот так, получай, рыжий кусок мяса!"

# game/status.rpy:3564
translate russia task5Menu1_b5d3c39c:

    # y "...."
    y "...."

# game/status.rpy:3566
translate russia task5Menu1_63fad127:

    # "Sam's cell appears to be empty."
    "Похоже камера Сэм пуста."

# game/status.rpy:3567
translate russia task5Menu1_65f06daa:

    # y "Where is my redhead piece of meat?"
    y "Где мой рыжий кусок мяса?"

# game/status.rpy:3568
translate russia task5Menu1_f6cad9cf:

    # s "{b}*Ahem*!{/b}"
    s "{b}*Кхм*!{/b}"

# game/status.rpy:3570
translate russia task5Menu1_72d569bc:

    # y "ARGH!"
    y "АРГХ!"

# game/status.rpy:3572
translate russia task5Menu1_8b66bd17:

    # y "O-oh Sam~...! You didn't hear any of that did you....?"
    y "О-ох Сэм~...! Ты ведь не слышал ничего....?"

# game/status.rpy:3573
translate russia task5Menu1_adcdc90c:

    # s g9 "{b}*Inhale*{/b}"
    s g9 "{b}*Вдох*{/b}"

# game/status.rpy:3574
translate russia task5Menu1_d631368b:

    # y "Uh oh...."
    y "Ой-ой..."

# game/status.rpy:3575
translate russia task5Menu1_7d93bbed:

    # s g8 "{b}OF ALL THE LOWLIFE, GOOD FOR NOTHING, COWARDLY TRICKS THAT WE'VE ENCOUNTERED, YOU'RE THE WORST!{/b}"
    s g8 "{b}ИЗ ВСЕХ НИЧТОЖНЫХ, НИКЧЕМНЫХ, ТРУСЛИВЫХ УЛОВОК, С КОТОРЫМИ МЫ СТАЛКИВАЛИСЬ, ТЫ - ХУДШИЙ!{/b}"

# game/status.rpy:3576
translate russia task5Menu1_08f249fd:

    # y "Erm... I can explain...!"
    y "Эмм... Я всё объясню...!"

# game/status.rpy:3587
translate russia task5Menu1_be3b0385:

    # a "What's going on?!"
    a "Что происходит?!"

# game/status.rpy:3588
translate russia task5Menu1_4f727b42:

    # c "Are we under attack?!"
    c "Нас атакуют?!"

# game/status.rpy:3589
translate russia task5Menu1_909c400a:

    # s "This pervert was spying on us via the spy cameras!"
    s "Этот извращенец подглядывал за нами через камеры!"

# game/status.rpy:3590
translate russia task5Menu1_3f07310a:

    # c r30 "What?!"
    c r30 "Что?!"

# game/status.rpy:3591
translate russia task5Menu1_7d4d9ff9:

    # a y28 "Nooo my nudies!"
    a y28 "Неееет, только не это!"

# game/status.rpy:3592
translate russia task5Menu1_e0f6d0db:

    # "The three spies turn to look at you."
    "Три шпионки поворачиваются и смотрят на вас."

# game/status.rpy:3593
translate russia task5Menu1_9a78f174_1:

    # y "................"
    y "................"

# game/status.rpy:3594
translate russia task5Menu1_5510b1e1:

    # y "I plead the 5th."
    y "Я признаю вину."

# game/status.rpy:3595
translate russia task5Menu1_908b76f2:

    # c r31 "[playerName], if you want us to keep our nanobots suppressed, you can't then sneak off to spy on us!"
    c r31 "[playerName], если ты хочешь, чтобы мы подавляли наших наноботов, ты не можешь тайком шпионить за нами!"

# game/status.rpy:3596
translate russia task5Menu1_4534ff74:

    # s g9 "Yeah, that's a serious breach of privacy!"
    s g9 "Да, это серьезное нарушение личного пространства!"

# game/status.rpy:3597
translate russia task5Menu1_46c59b37:

    # a y42 "I can't believe I showed my nudies to you... again!"
    a y42 "Не могу поверить, что ты видел меня голой... снова!"

# game/status.rpy:3598
translate russia task5Menu1_2ea3e976:

    # s "That's it, [playerName]! You've betrayed our trust, now how do you expect us to trust you on missions?!{w} I'm going to bed!"
    s "Вот и всё, [playerName]! Ты предал наше доверие, и как мы теперь будем доверять тебе во время миссий?!{w} Я иду спать!"

# game/status.rpy:3599
translate russia task5Menu1_2274803a:

    # a y38 "Me too!"
    a y38 "Я тоже!"

# game/status.rpy:3600
translate russia task5Menu1_22a823cd:

    # c "......................."
    c "......................."

# game/status.rpy:3604
translate russia task5Menu1_d93f1995:

    # c "So you've been looking at us when we masturbate...?"
    c "Значит, ты подсматривал за нами, во время мастурбации...?"

# game/status.rpy:3605
translate russia task5Menu1_7cd966aa:

    # c r12 "Not cool, [playerName]..."
    c r12 "Не круто, [playerName]..."

# game/status.rpy:3608
translate russia task5Menu1_9cb97b48_1:

    # y "........................"
    y "........................"

# game/status.rpy:3612
translate russia task5Menu1_a0a2dbbe:

    # "The girls caught you spying on them and their morale has dropped substantially."
    "Девушки поймали вас, их моральный дух значительно упал."

# game/status.rpy:3613
translate russia task5Menu1_99dd40b6:

    # "When their morale is too low, they may reject working at places unless ordered to. They will also refuse to keep their nanobots suppressed."
    "Когда их моральный дух слишком низок, они могут отказаться работать на местах, если им не прикажут. Они также откажутся подавлять своих наноботов."

# game/status.rpy:3614
translate russia task5Menu1_4d6d8f8c:

    # "A good way to improve your spies' mood, is by giving them something fun to do. Like a shopping spree at the mall or some time off at the beach."
    "Хороший способ улучшить настроение ваших шпионок - дать им какое-нибудь увлекательное занятие. Например, поход по магазинам в торговом центре или отдых на пляже."

# game/status.rpy:3615
translate russia task5Menu1_32b88835:

    # "Giving your spies certain items will also boost their happiness."
    "Даря своим шпионкам определённые предметы, вы также повышаете их счастье."

# game/status.rpy:3617
translate russia task5Menu1_34c07c48:

    # y "I guess I ought to make it up to them..."
    y "Думаю, я должен загладить свою вину перед ними..."

# game/status.rpy:3619
translate russia task5Menu1_53868d06:

    # "You decide to head to bed."
    "Вы решаете лечь спать."

# game/status.rpy:3634
translate russia task5Menu1_10fd81d9:

    # "................................"
    "................................"

# game/status.rpy:3635
translate russia task5Menu1_c8fb0a6d:

    # y "You guys still not talking to me?"
    y "Вы всё ещё не разговариваете со мной?"

# game/status.rpy:3636
translate russia task5Menu1_89b3d65d:

    # s "{b}*Sighs*{/b} [playerName]....{w} We've been talking...."
    s "{b}*Вздох*{/b} [playerName]....{w} Мы поговорили...."

# game/status.rpy:3637
translate russia task5Menu1_92f04902:

    # s g32 "You shouldn't have spied on us the way you did..."
    s g32 "Тебе не следовало шпионить за нами так..."

# game/status.rpy:3638
translate russia task5Menu1_692be6fa:

    # y "............"
    y "............"

# game/status.rpy:3639
translate russia task5Menu1_2b4ed308:

    # s g38 "But...{w} We also can't really blame you..."
    s g38 "Но...{w} Мы не можем тебя винить..."

# game/status.rpy:3640
translate russia task5Menu1_db2fddbc:

    # s "You got stuck in this mess just as we were and maybe we overreacted.{w} You've helped us a lot and we wouldn't have gotten this far without you..."
    s "Ты застрял в этой неразберихе так же, как и мы. Возможно, мы слишком резко отреагировали.{w} Ты нам очень помог, и без тебя мы бы не продвинулись так далеко..."

# game/status.rpy:3641
translate russia task5Menu1_3e973bfd:

    # y "I am forgiven?"
    y "Я прощён?"

# game/status.rpy:3642
translate russia task5Menu1_073abdce:

    # c r13 "You are."
    c r13 "Да."

# game/status.rpy:3644
translate russia task5Menu1_3edbcb5c:

    # c r14 "Also we... err...{w} noticed our oxytocin levels after your little stunt and well... {w}They were actually really stable."
    c r14 "Ещё мы... эмм...{w} заметил что наш уровень окситоцина, после твоего маленького трюка, эмм... {w}Он стал действительно более стабильным."

# game/status.rpy:3645
translate russia task5Menu1_62fb3300:

    # y "Wait... so you're saying...?"
    y "Подожди... ты хочешь сказать...?"

# game/status.rpy:3646
translate russia task5Menu1_fbe873ad:

    # s "That your voyeurism might have had a positive side effect."
    s "Что твой вуайеризм мог иметь положительный побочный эффект."

# game/status.rpy:3647
translate russia task5Menu1_68557698:

    # a y1 "Turns out you looking in on us gave us a boost of oxy-whatyamacallit. Preventing our nanobots from acting up!"
    a y1 "Оказывается, из-за того что ты за нами подглядывал, наше тело выработало окси-ватамакаллит. Тем самым подавив работу наши наноботов!"

# game/status.rpy:3648
translate russia task5Menu1_6724fd36:

    # c "[playerName]...."
    c "[playerName]...."

# game/status.rpy:3649
translate russia task5Menu1_99e90e5d:

    # c r12 "As stupid as this is...{w} We talked it over and you don't have to spy on us again in the future... {w}You can stay in our cell to watch.... if you want."
    c r12 "Как бы глупо это ни было...{w} Мы все обсудили, и тебе не придётся больше шпионить за нами в будущем... {w}Ты можешь остаться в нашей комнате и смотреть... если захочешь."

# game/status.rpy:3650
translate russia task5Menu1_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:3654
translate russia task5Menu1_49910d71:

    # y "Well I wasn't exacty innocent, so I'm sorry too."
    y "Ну, я не совсем невиновен, так что мне тоже жаль."

# game/status.rpy:3655
translate russia task5Menu1_a200f18b:

    # y "Plus it all turned out for the best!"
    y "К тому же всё обернулось к лучшему!"

# game/status.rpy:3656
translate russia task5Menu1_a3420c16:

    # s "For you."
    s "Для тебя."

# game/status.rpy:3657
translate russia task5Menu1_2bac6c11:

    # y "Exactly!"
    y "В точку!"

# game/status.rpy:3659
translate russia task5Menu1_a0c47b56:

    # y "Hah, I get to look at naked girls getting off {i}'without'{/i} getting into trouble?!"
    y "Хах, я могу смотреть на голых, кончающих девушек {i}'не попадая'{/i} в неприятности?!"

# game/status.rpy:3660
translate russia task5Menu1_cd5caa1d:

    # c "Pretty much."
    c "Довольно много."

# game/status.rpy:3661
translate russia task5Menu1_0c3db4d1:

    # s g14 "Though some professionality from your side would be appreciated!"
    s g14 "Хотя некоторый профессионализм с твоей стороны был бы кстати!"

# game/status.rpy:3662
translate russia task5Menu1_f3b12a03:

    # y "Of course.{w} Show me your tits."
    y "Конечно.{w} Покажи мне свои сиськи."

# game/status.rpy:3664
translate russia task5Menu1_8999fb43:

    # s g28 "W-what?!"
    s g28 "Ч-что?!"

# game/status.rpy:3665
translate russia task5Menu1_7c96a088:

    # y "You said it was fine, right? Show me your tits then."
    y "Вы же сказали, что всё в порядке? Тогда покажи мне свои сиськи."

# game/status.rpy:3666
translate russia task5Menu1_679d0b22:

    # s g30 "................."
    s g30 "................."

# game/status.rpy:3667
translate russia task5Menu1_ec459c46:

    # c r38 "We talked about this. Just get it over with."
    c r38 "Мы говорили об этом. Просто покончи с этим."

# game/status.rpy:3668
translate russia task5Menu1_17281118:

    # s g15 "Fine..."
    s g15 "Ладно..."

# game/status.rpy:3676
translate russia task5Menu1_5c701e7a:

    # s "..............."
    s "..............."

# game/status.rpy:3677
translate russia task5Menu1_4effab73:

    # y "Not gonna lie.{w} I'm loving this."
    y "Не буду врать.{w} Мне это нравится."

# game/status.rpy:3678
translate russia task5Menu1_3b779f41:

    # s g38 "You know, this is not what I meant when I said professional!"
    s g38 "Знаешь, это не то, что я имел в виду, когда говорил о профессионализме!"

# game/status.rpy:3680
translate russia task5Menu1_965aec3c:

    # y "Well I-....."
    y "Нуу я-..."

# game/status.rpy:3688
translate russia task5Menu1_4dd7ce7f:

    # y "Gah!!!"
    y "Гах!!!"

# game/status.rpy:3689
translate russia task5Menu1_61a4520d:

    # s g29 "[playerName]? What's wrong?"
    s g29 "[playerName]? Что случилось?"

# game/status.rpy:3690
translate russia task5Menu1_2ae0b93b:

    # y "Er... nothing."
    y "Эмм... ничего."

# game/status.rpy:3691
translate russia task5Menu1_f5df37af:

    # y "(I think.)"
    y "(Надеюсь.)"

# game/status.rpy:3692
translate russia task5Menu1_73fac5c9:

    # c r1 "Anyways, I'm glad that's sorted. Now maybe we can focus on more important matters."
    c r1 "В любом случае я рада, что всё улажено. Теперь, мы сможем сосредоточиться на более важных вопросах."

# game/status.rpy:3706
translate russia task5Menu1_7cdbce7d:

    # "New restaurant options are available."
    "Доступны новые опции в ресторане."

# game/status.rpy:3727
translate russia task6_2c03d970:

    # sM "The Aces got a heist planned for today, but I'm pretty sure we can pull it off with nobody getting hurt."
    sM "Тузы запланировали ограбление на сегодня, но я уверена, что мы сможем провернуть его так, чтобы никто не пострадал."

# game/status.rpy:3728
translate russia task6_be2f524c:

    # y "Okay, stay on your guard."
    y "Хорошо, будь начеку."

# game/status.rpy:3729
translate russia task6_30f16145:

    # sM "{b}*Nods*{/b}"
    sM "{b}*Кивает*{/b}"

# game/status.rpy:3731
translate russia task6_62c0e157:

    # "Sam heads off to the Aces."
    "Сэм отправляется к Тузам."

# game/status.rpy:3742
translate russia task6_4a64b0fd:

    # y "Welcome back, Sam. How did the heist go?"
    y "С возвращением, Сэм. Как прошло ограбление?"

# game/status.rpy:3744
translate russia task6_801a3f00:

    # s "Well.... Good I guess."
    s "Нуу... Думаю вполне хорошо."

# game/status.rpy:3745
translate russia task6_95deed73:

    # y "But~...?"
    y "Но~...?"

# game/status.rpy:3746
translate russia task6_f27472b0:

    # s g40 "But nothing. It went just fine."
    s g40 "Ничего. Всё прошло довольно хорошо."

# game/status.rpy:3747
translate russia task6_4a3f0833:

    # s "I even brought us back a cut of the profit."
    s "Я даже вернула нам часть прибыли."

# game/status.rpy:3752
translate russia task6_469770c6:

    # y "Er... are you okay?"
    y "Эмм... ты в порядке?"

# game/status.rpy:3753
translate russia task6_9917bd8e:

    # s g9 "Fine! Just fine, I'm heading to my cell!"
    s g9 "Отлично! Всё хорошо, я иду в свою камеру!"

# game/status.rpy:3756
translate russia task6_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:3769
translate russia task6_39534934:

    # c "..............."
    c "..............."

# game/status.rpy:3770
translate russia task6_36896bb4:

    # y "So... what's wrong?"
    y "Так что... Что случилось?"

# game/status.rpy:3771
translate russia task6_bc2a54f4:

    # s "........................."
    s "........................."

# game/status.rpy:3772
translate russia task6_9e6d00b6:

    # s "The heist didn't go exactly as planned..."
    s "Ограбление прошло не совсем так, как планировалось..."

# game/status.rpy:3773
translate russia task6_c1346f17:

    # s g15 "Someone got shot."
    s g15 "Кого-то подстрелили."

# game/status.rpy:3774
translate russia task6_c164ca17:

    # y "Uh oh..."
    y "Ого..."

# game/status.rpy:3775
translate russia task6_8ad0089d:

    # s "During the Heist, another gang showed up called The Glimmers and a firefight broke out."
    s "Во время ограбления появилась другая банда под названием \"Проблеск\", и началась перестрелка."

# game/status.rpy:3776
translate russia task6_f0b398d1:

    # s "One of the Glimmers was hit and killed. They pulled back after that, but..."
    s "Один из \"Поблеска\" был ранен и погиб. После этого они отступили, но..."

# game/status.rpy:3777
translate russia task6_df682738:

    # s g4 "I feel bad for having been part of it..."
    s g4 "Я чувствую себя виноватым за то, что был частью этого..."

# game/status.rpy:3779
translate russia task6_1ae53bfe:

    # c "It's okay Sam... It's not your fault."
    c "Всё в порядке, Сэм... Это не твоя вина."

# game/status.rpy:3780
translate russia task6_d74c5469:

    # y "Something like this was gonna happen sooner or later. Did you or any of the Aces get hurt?"
    y "Рано или поздно что-то подобное должно было произойти. Ты или кто-то из Тузов пострадал?"

# game/status.rpy:3781
translate russia task6_42befa65:

    # s "{b}*Sighs*{/b} No we made it out unharmed..."
    s "{b}*Вздох*{/b} Нет, мы остались невредимыми..."

# game/status.rpy:3782
translate russia task6_465e2388:

    # s "Afterwards, they took me aside to talk things through with them. They weren't being snobbish or elitist for once."
    s "После этого они отвели меня в сторону, чтобы я всё с ними обсудила. Первый раз они не были снобами и элитариями."

# game/status.rpy:3783
translate russia task6_858261c0:

    # s "One of them even gave me the number of their therapist..."
    s "Один из них даже дал мне номер своего терапевта..."

# game/status.rpy:3784
translate russia task6_c9942b9c:

    # s "They were actually really nice..."
    s "Они были очень даже милы..."

# game/status.rpy:3785
translate russia task6_4cac6320:

    # y "It's not an easy thing to see somebody die."
    y "Нелегко видеть, как кто-то умирает."

# game/status.rpy:3786
translate russia task6_92f5be84:

    # s "......................"
    s "......................"

# game/status.rpy:3788
translate russia task6_21fc179b:

    # s "If I never joined WOOHP this wouldn't have happened..."
    s "Если бы я не присоединилась к WOOHP, этого бы не случилось..."

# game/status.rpy:3789
translate russia task6_cb629142:

    # s g5 "The Aces offered to take me shopping. You know... to deal with things."
    s g5 "Тузы предложили мне пройтись по магазинам. Знаешь... разобраться с делами."

# game/status.rpy:3790
translate russia task6_a92324df:

    # y "I think that might be a good idea. Spend tomorrow at the mall, just to be sure."
    y "Я думаю, что это хорошая идея. Провести завтрашний день в торговом центре, просто, чтобы убедиться."

# game/status.rpy:3791
translate russia task6_0af92c0a:

    # s "{b}*Nods*{/b}"
    s "{b}*Кивает*{/b}"

# game/status.rpy:3794
translate russia task6_c6f7708b:

    # c r43 "................."
    c r43 "................."

# game/status.rpy:3795
translate russia task6_bae9dc91:

    # y "Worried about her?"
    y "Беспокоишься о ней?"

# game/status.rpy:3796
translate russia task6_e535b3d7:

    # c r37 "A little. Sam's a tough one and I'm sure she'll be fine, but even undercover we're suppose to be part of the good guys."
    c r37 "Немного. Сэм - сложный человек. Но я уверена, что она справится. Даже под прикрытием мы должны оставаться на хорошей стороне."

# game/status.rpy:3797
translate russia task6_fc75d20a:

    # c "Even if they're criminals, we're suppose to keep people safe. I think not being able to prevent that death bothers her."
    c "Даже если они преступники, мы должны обеспечивать безопасность людей. Я думаю, что невозможность предотвратить эту смерть беспокоит её."

# game/status.rpy:3798
translate russia task6_5519ccc1:

    # y "Let's keep an eye on her for now."
    y "Давай пока присмотрим за ней."

# game/status.rpy:3811
translate russia task6_639285d8:

    # y "Welcome back, [greenName]."
    y "С возвращением, [greenName]."

# game/status.rpy:3812
translate russia task6_89a1d486:

    # s "Hey [playerName]."
    s "Привет [playerName]."

# game/status.rpy:3813
translate russia task6_679bf97e:

    # y "Feeling better?"
    y "Чувствуешь себя лучше?"

# game/status.rpy:3814
translate russia task6_6c3dba80:

    # s g2 "Yeah, a little bit. Normally it's Clover who loves going on shopping sprees, but this time I really went all out."
    s g2 "Да, немного. Обычно Кловер любит ходить по магазинам, но в этот раз я действительно выложилась по полной."

# game/status.rpy:3815
translate russia task6_572497ae:

    # s "I bought a laptop and some hardware to go with it."
    s "Я купила ноутбук и кое-какое оборудование к нему."

# game/status.rpy:3816
translate russia task6_b40392e9:

    # y "Uh huh..."
    y "Ого..."

# game/status.rpy:3817
translate russia task6_108dbeaa:

    # s g1 "Aswell as a desk and a chair."
    s g1 "А также письменный стол и стул."

# game/status.rpy:3818
translate russia task6_26f9ecf6:

    # y "I see... {w}Anything else?"
    y "Понятно...{w}Чтонибудь ещё?"

# game/status.rpy:3820
translate russia task6_3f77ad66:

    # s "Yeah! This painting that I really like...!"
    s "Да! Ту картину, что мне очень нравится...!"

# game/status.rpy:3821
translate russia task6_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:3822
translate russia task6_38d6b3e0:

    # y "Okay... {w} How much?"
    y "Окей... {w} Сколько?"

# game/status.rpy:3823
translate russia task6_279d7b4a:

    # s g41 "Huh?"
    s g41 "Хмм?"

# game/status.rpy:3824
translate russia task6_01df410f:

    # y "Give it to me straight, I can handle it. How much was all that?"
    y "Скажи прямо, я справлюсь. Сколько всё это стоило?"

# game/status.rpy:3825
translate russia task6_a7b8d612:

    # s "Nothing."
    s "Нисколько."

# game/status.rpy:3826
translate russia task6_952f02cc:

    # y "Come on, I've been mentally preparing myself for this all day. Just lay it on me!"
    y "Да ладно, я весь день мысленно готовился к этому. Просто вывали это на меня!"

# game/status.rpy:3827
translate russia task6_a59bd4d7:

    # s g1 "I didn't have to pay."
    s g1 "Мне не пришлось платить."

# game/status.rpy:3828
translate russia task6_51d209e7:

    # y "Don't keep me in suspense any longer! My wallet is crying out in agony! Just give it to me!"
    y "Хватит тянуть кота за хвост! Мой кошелёк кричит в агонии! Просто скажи!"

# game/status.rpy:3829
translate russia task6_2d2b7e79:

    # s g10 "[playerName]..."
    s g10 "[playerName]..."

# game/status.rpy:3830
translate russia task6_f946c6bd:

    # s g1 "The Aces paid for it all."
    s g1 "Тузы за всё заплатили."

# game/status.rpy:3831
translate russia task6_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:3832
translate russia task6_ac17dec1:

    # y "What?"
    y "Что?"

# game/status.rpy:3833
translate russia task6_3370fc35:

    # s "They paid for every last single thing."
    s "Они заплатили за всё до последней мелочи."

# game/status.rpy:3834
translate russia task6_9cb97b48_1:

    # y "........................"
    y "........................"

# game/status.rpy:3835
translate russia task6_6f034a40:

    # y "A gang of violent gangsters brought you on a free shopping spree...?"
    y "Банда жестоких гангстеров устроила тебе бесплатный шопинг...?"

# game/status.rpy:3836
translate russia task6_e3c95f5e:

    # s "Y-yeah... They wouldn't take no for an answer."
    s "А-ага... Они не принимали отказа."

# game/status.rpy:3837
translate russia task6_bc159326:

    # s g13 "I was surprised too. They were really cool about it."
    s g13 "Я тоже был удивлена. Они отнеслись к этому очень спокойно."

# game/status.rpy:3838
translate russia task6_9066e460:

    # s "We went into all the expensive shops that I normally can't go into. At no point did they laugh or make fun of my lack of money. They said I was one of theirs now."
    s "Мы зашли во все дорогие магазины, в которые я обычно не могу зайти. Ни разу они не посмеялись и не поиздевались над моим безденежьем. Они сказали, что теперь я одна из них."

# game/status.rpy:3839
translate russia task6_a7aed01a:

    # y "That's the benefit of having rich friends I guess..."
    y "В этом преимущество богатых друзей, наверное..."

# game/status.rpy:3840
translate russia task6_eb4eb2cb:

    # y "Just remember they're not really your friends. They're hardcore villains and gangsters."
    y "Просто помни, что они не совсем твои друзья. Это злодеи и гангстеры до мозга костей."

# game/status.rpy:3841
translate russia task6_ce1953b6:

    # s g11 "I know. Of course, I know that."
    s g11 "Я знаю. Конечно, я помню."

# game/status.rpy:3842
translate russia task6_faf6db3b:

    # s g2 "But it felt nice to be accepted by the cool kids for once. Instead of being bullied by them."
    s g2 "Но было приятно хоть раз быть принятой крутыми ребятами. Вместо того чтобы терпеть их издёвки."

# game/status.rpy:3848
translate russia task6_af549745:

    # "New cell decorations are available for Sam."
    "Для комнаты Сэм доступны новые декорации."

# game/status.rpy:3849
translate russia task6_a0ecffb6:

    # "New shopping options are available for Sam at the mall."
    "В торговом центре доступны новые варианты покупок для Сэм."

# game/status.rpy:3856
translate russia task6_d32e571a:

    # y "We should really focus on finding a way to free Alex before we do this."
    y "Мы должны сосредоточиться на поиске способа освобождения Алекс, прежде чем делать это."

# game/status.rpy:3865
translate russia task6_07cb1b77:

    # y "Okay, [samNick]. Time to go undercover with the Aces again."
    y "Ладно, [samNick]. Пора снова отправляется под прикрытием к Тузам."

# game/status.rpy:3866
translate russia task6_da098e53:

    # s "Oh.. okay!"
    s "Ох.. хорошо!"

# game/status.rpy:3867
translate russia task6_952e047d:

    # y "Well you sound excited."
    y "Похоже ты взволнована."

# game/status.rpy:3868
translate russia task6_c7392e59:

    # s g2 "Yeah, they're shooting a music video and told me I could be in it!"
    s g2 "Ага, они снимают музыкальный клип и сказали, что я могу в нем сняться!"

# game/status.rpy:3869
translate russia task6_ab23ddec:

    # y "A music video? I never expected you to be interested in something like that."
    y "Клип? Не ожидал, что ты заинтересуешься чем-то подобным."

# game/status.rpy:3870
translate russia task6_8e91bc02:

    # s "Heh~... well I wasn't before..."
    s "Хех~... ну раньше наверное да..."

# game/status.rpy:3871
translate russia task6_31454961:

    # s "But they said they couldn't do without me and said I'd be perfect."
    s "Но они сказали, что не могут без меня обойтись и то что я подходу идеально."

# game/status.rpy:3872
translate russia task6_e909c7d6:

    # y "High praise indeed."
    y "Действительно высокая оценка."

# game/status.rpy:3873
translate russia task6_7a0cf7ae:

    # s g1 "Yeah that's what I thought!"
    s g1 "Да, я также подумала!"

# game/status.rpy:3874
translate russia task6_9cc083d4:

    # s "I never thought I'd be in a music video!"
    s "Я никогда бы не подумала, что буду сниматься в музыкальном клипе! "

# game/status.rpy:3875
translate russia task6_284e290d:

    # y "Well have fun. Just don't get too excited. Remember that you're suppose to gather intel."
    y "Хорошо повеселись тогда. Только не слишком радуйся. Помни, что ты должны собирать информацию."

# game/status.rpy:3876
translate russia task6_77ce62f6:

    # s "Yeah yeah, don't worry."
    s "Да, да, не беспокойся."

# game/status.rpy:3890
translate russia task6_35764d69:

    # s "I'm back..."
    s "Я вернулась..."

# game/status.rpy:3891
translate russia task6_0874ff8a:

    # y "Welcome back S-..."
    y "С возвращением, С-..."

# game/status.rpy:3893
translate russia task6_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:3894
translate russia task6_b55c1b75:

    # y "You've got my undivided attention."
    y "Я весь во внимание."

# game/status.rpy:3895
translate russia task6_ad9a3d5b:

    # s "Heh... Heh~...."
    s "Хе... Хе~...."

# game/status.rpy:3896
translate russia task6_caf81fdf:

    # s g38 "{b}*Ahem*{/b} So we shot the music video today..."
    s g38 "{b}*Кхм*{/b} Итак, сегодня мы снимали тот клип..."

# game/status.rpy:3897
translate russia task6_abc5d1b3:

    # y "In a swimsuit."
    y "В купальниках."

# game/status.rpy:3898
translate russia task6_e415cde1:

    # s g35 "In a swimsuit..."
    s g35 "В купальниках..."

# game/status.rpy:3899
translate russia task6_12615f8a:

    # s g42 "Okay, so I didn't know this, but all the girls had to wear swimsuits for the shoot."
    s g42 "Я этого не знала, но все девушки должны были надеть купальники для съемок."

# game/status.rpy:3900
translate russia task6_970ed319:

    # y "Uhhuh..."
    y "Угу..."

# game/status.rpy:3901
translate russia task6_580db806:

    # s g28 "And... I mean...! Why not? It was really hot out today."
    s g28 "Ии... Я думаю...! Почему бы и нет? Сегодня на улице было довольно жарко."

# game/status.rpy:3902
translate russia task6_dde3ed59:

    # y "I see..."
    y "Я вижу..."

# game/status.rpy:3903
translate russia task6_0af4f273:

    # s "And... and there was a pool!{w} I mean we didn't use it, but there was one!"
    s "А ещё... там был бассейн!{w} Ну мы его не использовали, но он там был!"

# game/status.rpy:3904
translate russia task6_51476e24:

    # y "Riveting."
    y "Интересно."

# game/status.rpy:3905
translate russia task6_bc323abd:

    # s g42 "And... and well! Well...{w} At least they didn't film my butt...!"
    s g42 "Ии... и хорошо! Хорошо...{w} По крайней мере, они не снимали мою задницу...!"

# game/status.rpy:3906
translate russia task6_3e372aa1:

    # s "{size=-10}I think...{/size}"
    s "{size=-10}Надеюсь...{/size}"

# game/status.rpy:3907
translate russia task6_83596190:

    # y "Okay... I think I see what's going on here."
    y "Ладно... Кажется, я понимаю, что здесь происходит."

# game/status.rpy:3908
translate russia task6_e044ac83:

    # y "You didn't expect to be used as a sex object. {w}Now you're getting flustered and pretend to be shocked and outraged because it clashes with your sense of morality."
    y "Ты не ожидала, что тебя будут использовать как сексуальный объект. {w}А теперь ты взволнована и претворяешься, что шокирована и возмущена, потому что это противоречит твоему чувству морали."

# game/status.rpy:3909
translate russia task6_5422cbc1:

    # s g11 "........................."
    s g11 "........................."

# game/status.rpy:3910
translate russia task6_d51f18ec:

    # s "Well...{w} Not exactly..."
    s "Нуу...{w} Не совсем..."

# game/status.rpy:3911
translate russia task6_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:3912
translate russia task6_0f7b0b2d:

    # s "The truth is..."
    s "Правда в том, что..."

# game/status.rpy:3916
translate russia task6_b1b83533:

    # s "It was actually kinda fun..."
    s "Это было довольно забавно..."

# game/status.rpy:3917
translate russia task6_65bc640c:

    # y "It was...?"
    y "Да...?"

# game/status.rpy:3918
translate russia task6_dc7d2332:

    # s "They were all so nice to me! They noticed I wasn't completely comfortable and they encourages me and said I was talented."
    s "Они все были так добры ко мне! Они заметили, что мне было не совсем комфортно. Они подбодрили меня и сказали, что я талантлива."

# game/status.rpy:3919
translate russia task6_b0bce455:

    # s "And then the other girls said it wasn't such a big deal and they helped me practise and.. and...!"
    s "А потом другие девочки сказали, что это не так уж и страшно, и помогли мне попрактиковаться и... и...!"

# game/status.rpy:3920
translate russia task6_397219cd:

    # s g15 "................."
    s g15 "................."

# game/status.rpy:3921
translate russia task6_74bfe0a2:

    # s "And I could keep the bathing suit."
    s "И купальник я могу оставить себе."

# game/status.rpy:3922
translate russia task6_a7786cc8:

    # s g14 "They're really not so bad once you get to know them..."
    s g14 "Они действительно не так уж и плохи, если узнаете их получше..."

# game/status.rpy:3923
translate russia task6_9393e7b5:

    # y "Sam, they're violent gangsters."
    y "Сэм, они жестокие бандиты."

# game/status.rpy:3924
translate russia task6_c6baa936:

    # s "I know, but..."
    s "Я знаю, но"

# game/status.rpy:3926
translate russia task6_7b6cd9fb:

    # s "It's just that...{w} They're a lot more fun to hang out with than going on missions for WOOHP all the time..."
    s "Просто...{w} С ними гораздо веселее общаться, чем постоянно ходить на задания для WOOHP..."

# game/status.rpy:3927
translate russia task6_bcb51585:

    # s g15 "....................."
    s g15 "....................."

# game/status.rpy:3928
translate russia task6_5dea14c4:

    # s "They even gave me this for my work..."
    s "Они даже дали мне это за работу..."

# game/status.rpy:3932
translate russia task6_08f9caac:

    # s g1 "So... today wasn't that bad. Even if I had to wear a swimsuit."
    s g1 "Так что... сегодня было не так уж и плохо. Даже если мне пришлось надеть купальник."

# game/status.rpy:3933
translate russia task6_b7f3446b:

    # s "It might've even reduced my nanobot levels a bit."
    s "Возможно, это даже немного снизило уровень моих наноботов."

# game/status.rpy:3934
translate russia task6_043d6585:

    # y "Hm... that sounds like a pretty succesful day..."
    y "Хмм... Похоже, это был довольно успешный день..."

# game/status.rpy:3935
translate russia task6_2649bfc0:

    # y "Just don't get in too deep, okay? Remember that these people are not really your friends."
    y "Только не забирайся слишком глубоко, хорошо? Помни, что эти люди на самом деле не твои друзья."

# game/status.rpy:3936
translate russia task6_81ed468b:

    # s "I know, I know."
    s "Я знаю, я знаю."

# game/status.rpy:3946
translate russia task6_1a1c0f7e:

    # "New clothing options for Sam are available at the mall."
    "Новые варианты одежды для Сэм можно приобрести в торговом центре."

# game/status.rpy:3951
translate russia task6_d32e571a_1:

    # y "We should really focus on finding a way to free Alex before we do this."
    y "Мы должны сосредоточиться на поиске способа освобождения Алекс, прежде чем делать это."

# game/status.rpy:3958
translate russia task6_7a180bbb:

    # y "Okay, time to g-..."
    y "Ладно, пора ид-..."

# game/status.rpy:3959
translate russia task6_639afc55:

    # s "{b}*Eeeeee*{/b}!!!"
    s "{b}*Иииииии*{/b}!!!"

# game/status.rpy:3960
translate russia task6_691a5f1f:

    # y "What the....?"
    y "Что за...?"

# game/status.rpy:3962
translate russia task6_da14970e:

    # s "Great news, [playerName]! I've been invited back to the Aces castle!"
    s "Отличные новости, [playerName]! Меня снова пригласили в замок Тузов!"

# game/status.rpy:3963
translate russia task6_fa1b5042:

    # y "That {i}'is'{/i} great news! Maybe you'll be able to get close to the next Lieutenant then."
    y "Это {i}'отличная'{/i} новость! Может тогда ты сможешь приблизиться к следующему лейтенанту."

# game/status.rpy:3964
translate russia task6_bdc383f6:

    # s g29 "Oh! Right, of course!{w} The Lieutenant."
    s g29 "Ох! Точно, конечно!{w} Лейтенант."

# game/status.rpy:3965
translate russia task6_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:3966
translate russia task6_608512db:

    # y "Well have fun and be careful out there."
    y "Хорошо повеселись и будь осторожна."

# game/status.rpy:3967
translate russia task6_a94e64a1:

    # "Sam nods and heads off."
    "Сэм кивает и уходит."

# game/status.rpy:3971
translate russia task6_e69c159f:

    # c "Where is she going?"
    c "Куда она идёт?"

# game/status.rpy:3972
translate russia task6_4b7a3fa3:

    # y "Spain."
    y "В Испанию."

# game/status.rpy:3973
translate russia task6_b3af41bb:

    # c "Spain? On a mission?"
    c "В Испанию? На миссию?"

# game/status.rpy:3974
translate russia task6_57b9ac91:

    # y "No she got a party invitation."
    y "Нет, она получила приглашение на вечеринку."

# game/status.rpy:3975
translate russia task6_613afb24:

    # c r18 "Again? That girl has been spending a lot of time with the Aces lately."
    c r18 "Опять? В последнее время она проводит много времени с Тузами."

# game/status.rpy:3976
translate russia task6_7b96d479:

    # y "My thoughts exactly. Do you think we should worry?"
    y "С языка сняла. Как ты думаешь, стоит ли нам беспокоиться?"

# game/status.rpy:3977
translate russia task6_1f9fb9bb:

    # c r12 "Hmm... Sam normally has her feet planted firmly in reality. I think she'll be fine... {w}I hope."
    c r12 "Хм... Сэм обычно твердо стоит на ногах в реальности. Думаю, с ней всё будет в порядке... {w}Надеюсь."

# game/status.rpy:3978
translate russia task6_cd56a62a:

    # c "Just to be sure we should keep an eye on her. She's been speaking much too highly about the Aces lately."
    c "Чтобы быть уверенным, нам стоит присматривать за ней. В последнее время она слишком хорошо отзывается о Тузах."

# game/status.rpy:3979
translate russia task6_fc4b0728:

    # y "Noted."
    y "Принял к сведению."

# game/status.rpy:3990
translate russia task6_d6d81f7e:

    # y "Let's see how Sam is getting along..."
    y "Посмотрим, как Сэм справиться..."

# game/status.rpy:3991
translate russia task6_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:3999
translate russia task6_ae4bfad7_1:

    # y "!!!"
    y "!!!"

# game/status.rpy:4000
translate russia task6_892221e6:

    # s g1 "Oh, [playerName]! Good to see you! I was just about to..."
    s g1 "Оо, [playerName]! Рада тебя видеть! Я как раз собиралась..."

# game/status.rpy:4001
translate russia task6_df95329e:

    # y "About to...?"
    y "Собиралась что...?"

# game/status.rpy:4002
translate russia task6_acd0cf3c:

    # s g19 "W-well...{w} I was dared to show my breasts and..."
    s g19 "Н-нуу ...{w} Я согласилась показать свою грудь и..."

# game/status.rpy:4003
translate russia task6_e11771fa:

    # y "And you accepted the dare?"
    y "Ты согласилась?"

# game/status.rpy:4004
translate russia task6_be591900:

    # s "Heh... heh..."
    s "Хех... хех..."

# game/status.rpy:4005
translate russia task6_47fcf072:

    # s "The other girls are doing it too..."
    s "Другие девушки тоже это делают..."

# game/status.rpy:4006
translate russia task6_d2e4e682:

    # s g29 "I already said I'd do it. I can't back out now! They expect me back any second now."
    s g29 "Я уже сказала, что сделаю это. Я не могу отказаться! Они ждут моего возвращения с минуты на минуту."

# game/status.rpy:4010
translate russia task6_afb628f4:

    # "Sam smiles and nods before going through a door."
    "Сэм улыбается и кивает, прежде чем выйти через дверь."

# game/status.rpy:4019
translate russia task6_74231502:

    # "Aces Gangster" "Woooo! Go Sam!"
    "Туз Гангстер" "Ухуууу! Давай Сэм!"

# game/status.rpy:4020
translate russia task6_856a633d:

    # "Aces Gangster" "You go girl! Nice tiiiiiiits!"
    "Туз Гангстер" "Давай, девочка! Отличные сииииськи!"

# game/status.rpy:4022
translate russia task6_1514d3ee:

    # s g2 "Heh heh~..."
    s g2 "Хех хех~..."

# game/status.rpy:4023
translate russia task6_a1dc9e40:

    # "Aces Gangster" "I knew inviting you to this party was a good idea!"
    "Туз Гангстер" "Я знал, что пригласить тебя на эту вечеринку было хорошей идеей!"

# game/status.rpy:4024
translate russia task6_2b6654a5:

    # "Aces Gangster" "Jiggle them a bit!"
    "Туз Гангстер" "Немного покачай их!"

# game/status.rpy:4025
translate russia task6_25b255e3:

    # s g1 "L-like this...?"
    s g1 "К-как-то так...?"

# game/status.rpy:4026
translate russia task6_1e72cb9c:

    # "Sam awkwardly jiggles her breasts from side to side."
    "Сэм неловко покачивает грудью из стороны в сторону."

# game/status.rpy:4027
translate russia task6_98bef7c7:

    # "You hear a loud cheering over the phone."
    "Вы слышите громкие возгласы по телефону."

# game/status.rpy:4028
translate russia task6_ab3fe9b2:

    # "Aces Gangster" "YOU'RE THE SEXIEST REDHEAD IN THE ACES!"
    "Туз Гангстер" "ТЫ САМАЯ СЕКСУАЛЬНАЯ РЫЖАЯ В ТУЗАХ!"

# game/status.rpy:4033
translate russia task6_215ea3ee:

    # y "....?"
    y "....?"

# game/status.rpy:4036
translate russia task6_b67dbf3d:

    # s g1 "Hehe, really? Cause I'm willing to go furth-..."
    s g1 "Правда? Потому что я уже готова идти даль-..."

# game/status.rpy:4037
translate russia task6_263ab4fb:

    # y "Sam! I didn't train you to become a slut!"
    y "Сэм! Я не готовил тебя к тому, чтобы ты стала шлюхой!"

# game/status.rpy:4038
translate russia task6_43a022c9:

    # s g29 "But... you always call me a slut when we... {w}y'know..."
    s g29 "Но... ты всегда называешь меня шлюхой, когда мы... {w}ну ты знаешь..."

# game/status.rpy:4039
translate russia task6_3b548352:

    # y "You're a slut. But you're my slut."
    y "Ты шлюха. Но ты моя шлюха."

# game/status.rpy:4040
translate russia task6_d70e3654:

    # s g18 "That doesn't make any sense."
    s g18 "Это бессмысленно."

# game/status.rpy:4041
translate russia task6_51ebe43b:

    # y "I know it doesn't. Don't you dare take off anymore clothing! And when you're done there, head back to the US."
    y "Я знаю. Не смей больше снимать одежду! А когда вы там закончите, возвращайся в США."

# game/status.rpy:4042
translate russia task6_43b09658:

    # s g11 "Fine~..."
    s g11 "Ладно~..."

# game/status.rpy:4051
translate russia task6_ee507c4e:

    # s g29 "A show...? W-what do you mean?!"
    s g29 "Шоу...? Ч-что ты имеешь в виду?!"

# game/status.rpy:4052
translate russia task6_559f2c4b:

    # y "Do some sexy dance moves. Strike some poses."
    y "Подвигайся сексуально. Попозируй. Покажи себя."

# game/status.rpy:4053
translate russia task6_b9f23c23:

    # s "But what if they want to touch me...?"
    s "А что, если они захотят потрогать мне...?"

# game/status.rpy:4054
translate russia task6_4f2f32ef:

    # y "Do you want them to touch you?"
    y "Ты хочешь, чтобы они прикасались к тебе?"

# game/status.rpy:4055
translate russia task6_81153c8b:

    # s g28 "No!"
    s g28 "Нет!"

# game/status.rpy:4056
translate russia task6_4b90ee20:

    # s g11 ".........................."
    s g11 ".........................."

# game/status.rpy:4057
translate russia task6_b7fc50d1:

    # s g48 "I mean...{w}{size=-8}maybe a little~....{/size}"
    s g48 "Ну может...{w}{size=-8}совсем немного~....{/size}"

# game/status.rpy:4058
translate russia task6_83ef818c:

    # y "There you go. Have fun."
    y "Вот так. Развлекайся."

# game/status.rpy:4059
translate russia task6_ad92e4a7:

    # "The blushing spy nods and begins to put on a preformance."
    "Покрасневшая шпионка кивает и начинает своё шоу."

# game/status.rpy:4063
translate russia task6_30283070:

    # "The camera cuts out..."
    "Камера вырубается..."

# game/status.rpy:4072
translate russia task6_b0faca46:

    # s g28 "W-what..?!"
    s g28 "Ч-что...?!"

# game/status.rpy:4073
translate russia task6_d87ba0d5:

    # y "Come on, this is the perfect opportunity! If you want to raise your reputation with the gang, you gotta go all out."
    y "Да ладно тебе, это прекрасная возможность! Если ты хочешь повысить свою репутацию в банде, тебе придётся выложиться по полной."

# game/status.rpy:4074
translate russia task6_7daa2d67:

    # s g29 "B-but..! There's so many people here~..."
    s g29 "Н-но..! Здесь так много людей~..."

# game/status.rpy:4075
translate russia task6_c7d14ee0:

    # y "Encourage the other girls to help you out."
    y "Попроси других девушек помочь тебе."

# game/status.rpy:4076
translate russia task6_143e2ea8:

    # s "B-but..!"
    s "Н-но..!"

# game/status.rpy:4077
translate russia task6_8ba69a99:

    # y "Imagine how popular you'll be!"
    y "Представь, насколько ты будешь популярна!"

# game/status.rpy:4078
translate russia task6_52558402:

    # s "I.. I guess so..."
    s "Д-.. Думаю, да..."

# game/status.rpy:4079
translate russia task6_ecfde177:

    # s g48 "{b}*Gulp*{/b} O-okay... let's do it...!"
    s g48 "{b}*Глоть*{/b} Х-хорошо... я сделаю это...!"

# game/status.rpy:4081
translate russia task6_20fa24d2:

    # s "Time to kick this party up a notch!"
    s "Пора вывести эту вечеринку на новый уровень!"

# game/status.rpy:4082
translate russia task6_906a3ecd:

    # "You hear a loud cheering crowd."
    "Вы слышите громкие возгласы толпы."

# game/status.rpy:4087
translate russia task6_4c7198c4:

    # "{b}*Suck* *Suck* *Slobber*{/b}"
    "{b}*Сосёт* *Заглатывает* *Слюнявит*{/b}"

# game/status.rpy:4098
translate russia task6_429e01d7:

    # y "Okay, wrap it up and head back to the US."
    y "Ладно, закругляйся и возвращайся в США."

# game/status.rpy:4099
translate russia task6_1fd53952:

    # "The dazed spy nods and staggers offscreen."
    "Ошеломленная шпионка кивает и, пошатываясь, выходит из кадра."

# game/status.rpy:4109
translate russia task6_b7d082c1:

    # y "What happened to the prude I've come to know and love?"
    y "Что случилось с той скромницей, которую я встретил и полюбил?"

# game/status.rpy:4110
translate russia task6_a50ed6e0:

    # s "It's... it's just my tit-...{w} my breasts. I don't know... Everyone just seemed so excited and I didn't want to be left out.."
    s "Это... это просто мои сис-...{w} моя грудь. Я не знаю... Все казались в восторге, и я не хотела оставаться в стороне..."

# game/status.rpy:4111
translate russia task6_57c9ad61:

    # y "That's the peer-pressure talking! Don't do it my innocent pure angel!"
    y "Это давление со стороны коллег! Не делай этого, мой чистый и невинный ангел!"

# game/status.rpy:4112
translate russia task6_6b757357:

    # s "I'm pretty sure I lost that innocense when you first told me to take off my top..."
    s "Я уверена, что потеряла невинность, когда ты впервые сказал мне снять топ..."

# game/status.rpy:4113
translate russia task6_eaca9d20:

    # y "No matter. You tell them {b}'no'{/b}."
    y "Не важно. Скажи им {b}'нет'{/b}."

# game/status.rpy:4114
translate russia task6_f8df86ef:

    # s "Fine..."
    s "Хорошо..."

# game/status.rpy:4116
translate russia task6_963ba763:

    # "You see Sam puttng her top back on and walk through a door, followed by the dissapointed cries of gangsters."
    "Вы видите, как Сэм надевает свой топ и входит в дверь, за которой раздаются разочарованные крики бандитов."

# game/status.rpy:4136
translate russia task6_2d185996:

    # s "I'm back."
    s "Я вернулась."

# game/status.rpy:4137
translate russia task6_05d68d6d:

    # y "Welcome back Sam. Had fun in Europe?"
    y "С возвращением Сэм. Повеселилась в Европе?"

# game/status.rpy:4138
translate russia task6_7341da93:

    # s g2 "Yes~..."
    s g2 "Да~..."

# game/status.rpy:4139
translate russia task6_71526417:

    # y "Okay. What's your report?"
    y "Отлично. Есть что доложить?"

# game/status.rpy:4140
translate russia task6_5461a1d1:

    # s g29 "Report?{w} Oh er... I was so busy hanging out with the Aces that I didn't really{w} er..."
    s g29 "Доложить?{w} Ох эм... Я была так занята вечеринкой, что не очень-то{w} эм..."

# game/status.rpy:4141
translate russia task6_23c9c112:

    # y "You didn't actual gather any intel..."
    y "Ты не собрала никакой информации..."

# game/status.rpy:4142
translate russia task6_04c03b36:

    # s g15 "I'm sorry, [playerName]..."
    s g15 "Мне жаль, [playerName]..."

# game/status.rpy:4143
translate russia task6_d232ce85:

    # s g14 "My mind was elsewhere. We went to do so many cool things that I sorta forgot..."
    s g14 "Мои мысли были в другом месте. Мы сделали столько всего интересного, что я даже забыла..."

# game/status.rpy:4144
translate russia task6_1e5a45e3:

    # s g1 "Like... we went skydiving and they all got super scared! Of course I do that like... every other week, so I didn't think it was a big deal."
    s g1 "Например... мы прыгали с парашютом, и они все очень испугались! Конечно, я делаю это почти... каждую неделю, так что я не думала, что это может быть проблемой."

# game/status.rpy:4145
translate russia task6_fe8993d2:

    # s "And they all thought I was a badass!"
    s "Поэтому все считали меня крутой!"

# game/status.rpy:4146
translate russia task6_14d0c5c3:

    # s "I also dragged some of them along to a museum to teach them about historical Spanish art! They were reluctant at first, but were blown away about how much I knew!"
    s "Я также потащила некоторых из них в музей, чтобы познакомить их с историческим испанским искусством! Сначала они неохотно ходили по музею, но были потрясены тем, как много я знаю!"

# game/status.rpy:4147
translate russia task6_49ec3efe:

    # s g13 "At the end of the day, they all wanted to talk to me and be my friend! And... and...!"
    s g13 "В конце концов, все они хотели поговорить со мной и стать моими друзьями! И... и...!"

# game/status.rpy:4148
translate russia task6_a0e73fd8:

    # y "Calm down there [samNick]. You know these are bad people, right?"
    y "Успокойся [samNick]. Ты знаешь, что это плохие люди, верно?"

# game/status.rpy:4149
translate russia task6_a6b0d569:

    # s g2 "I mean sure, but they're not all bad~..."
    s g2 "Конечно, но они же не все плохие~..."

# game/status.rpy:4150
translate russia task6_9f1a71c5:

    # y "The Aces have continued terrorizing Beverly Hills when you were away. You seem to forget how dangerous these people actually are."
    y "Пока тебя не было, Тузы продолжали терроризировать Беверли-Хиллз. Ты кажется забыла, насколько опасны эти люди."

# game/status.rpy:4151
translate russia task6_29c11fdd:

    # s g38 "No... I didn't forget..."
    s g38 "Нет... Я не забыла..."

# game/status.rpy:4155
translate russia task6_a01366dd:

    # y "Sam, you're going too deep undercover. You're gonna end up as one of these gangsters if you're not careful."
    y "Сэм, ты слишком увлеклась работой под прикрытием. Если ты не будешь осторожнее, то станешь одним из этих бандитов."

# game/status.rpy:4156
translate russia task6_753cb04a:

    # s g16 "What are you saying, [playerName]?! I know what I'm doing! I'm still working on re-taking this city, you know!"
    s g16 "Что ты такое говоришь, [playerName]?! Я знаю, что я делаю! Я всё ещё работаю над тем, чтобы освободить этот город, знаешь ли!"

# game/status.rpy:4157
translate russia task6_93d0b6f1:

    # s g31 "If anything I'd say I've gone far and beyond. How could you even question me like that?!"
    s g31 "Если уж на то пошло, я вообще сделала больше вас всех вместе взятых. Как ты вообще можешь во мне сомневаться?!"

# game/status.rpy:4158
translate russia task6_18482887:

    # c "He's not the only one."
    c "Он не единственный."

# game/status.rpy:4164
translate russia task6_bf47d07b:

    # s g56 "Girls...?"
    s g56 "Девочки...?"

# game/status.rpy:4165
translate russia task6_53404927:

    # c "You're getting in way too deep, Sam. Stop pretending these guys are your friends."
    c "Ты слишком увлеклась, Сэм. Перестань притворяться, что эти люди - твои друзья."

# game/status.rpy:4166
translate russia task6_e8883632:

    # a "Yeah! {w}We are your friends..."
    a "Да! {w}Мы - твои друзья..."

# game/status.rpy:4167
translate russia task6_2dfbc557:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:4168
translate russia task6_81b227a4:

    # s "Are you...! {w}Are you jealous?!"
    s "Вы...! {w}Вы ревнуете?!"

# game/status.rpy:4169
translate russia task6_d8b8145c:

    # c "That's not what we're saying, Sam."
    c "Мы говорим не об этом, Сэм."

# game/status.rpy:4170
translate russia task6_9d1425a1:

    # s g53 "You are, aren't you?!"
    s g53 "Вы ревнуете, не так ли?!"

# game/status.rpy:4171
translate russia task6_345a5a64:

    # s "Just because I'm good at my job doesn't mean I'm turning into a gangster!"
    s "Если я хорошо делаю свою работу, это не значит, что я превращаюсь в гангстера!"

# game/status.rpy:4172
translate russia task6_08bdde73:

    # s "So what if I enjoy going undercover with the Aces? At least they're not criticizing me all the time!"
    s "Ну и что, если мне нравится работать под прикрытием с Тузами? По крайней мере, они не критикуют меня постоянно!"

# game/status.rpy:4173
translate russia task6_725feb78:

    # y "Sam..."
    y "Сэм..."

# game/status.rpy:4174
translate russia task6_4e628c93:

    # s g40 "I'm going to unpack. Good night!"
    s g40 "Я пошла распаковывать вещи. Спокойной ночи!"

# game/status.rpy:4177
translate russia task6_f6817b25:

    # "Sam stormed off."
    "Сэм уходит."

# game/status.rpy:4178
translate russia task6_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:4179
translate russia task6_bd21d6b3:

    # y "Well that could be a problem."
    y "Ну, это может быть проблемой."

# game/status.rpy:4180
translate russia task6_6951b3f7:

    # a y5 "I've never seen her like this..."
    a y5 "Я никогда не видела её такой..."

# game/status.rpy:4181
translate russia task6_49bb8e30:

    # c r11 "Yeah, she's definitely acting odd."
    c r11 "Да, она определённо ведёт себя странно."

# game/status.rpy:4182
translate russia task6_9a9ba893:

    # y "Let's leave her be for now."
    y "Давайте пока оставим её в покое."

# game/status.rpy:4183
translate russia task6_a8f8ceef:

    # "The girls nod and return back to their cells."
    "Девочки кивают и расходятся по комнатам."

# game/status.rpy:4191
translate russia task6_85ad2be1:

    # y "Well I can't really blame you... It's a private castle after all."
    y "Ну, я не могу винить тебя... В конце концов, это частный замок."

# game/status.rpy:4192
translate russia task6_960abd9d:

    # s g1 "Exactly! And we played all kinds of games!"
    s g1 "Точно! И мы играли во всевозможные игры!"

# game/status.rpy:4193
translate russia task6_4526c67f:

    # y "I could see that."
    y "Я видел."

# game/status.rpy:4195
translate russia task6_c411b3e0:

    # s g2 "Oh right~... heh heh~..."
    s g2 "Ох, точно~... хех хех~..."

# game/status.rpy:4196
translate russia task6_ce753a72:

    # s g1 "But it was fun. I felt really accepted."
    s g1 "Но это было весело. Я чувствовала, что меня действительно принимают."

# game/status.rpy:4197
translate russia task6_8fb00ded:

    # y "Don't get too friendly with them. Remember they're gangsters."
    y "Не заводи с ними слишком дружеских отношений. Помни, что они гангстеры."

# game/status.rpy:4199
translate russia task6_927f6aba:

    # s g2 "Yeah I know, but they're the least bad of all the gangs here in town."
    s g2 "Да, я знаю, но они наименее плохие из всех банд в городе."

# game/status.rpy:4200
translate russia task6_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:4201
translate russia task6_976eee7c:

    # s "Okay I'm going to unpack. If you need me, I'll be in my cell."
    s "Ладно, я пойду распаковывать вещи. Если я понадоблюсь, я буду у себя."

# game/status.rpy:4202
translate russia task6_a95f772c:

    # "Sam picks up her bags and heads out of the room."
    "Сэм подхватывает свои сумки и выходит из комнаты."

# game/status.rpy:4205
translate russia task6_7a37a3cc:

    # c r18 "Was that Sam just now?"
    c r18 "Это была Сэм?"

# game/status.rpy:4211
translate russia task6_f35fe4ef:

    # y "Yeah she just returned."
    y "Да, она только что вернулась."

# game/status.rpy:4212
translate russia task6_91c884d9:

    # c r11 ".........................."
    c r11 ".........................."

# game/status.rpy:4213
translate russia task6_132dee16:

    # y "Something on your mind?"
    y "У тебя что-то на уме?"

# game/status.rpy:4214
translate russia task6_f4cb62a7:

    # c "We think she's getting a little too deep."
    c "Мы думаем, что она слишком увлеклась."

# game/status.rpy:4215
translate russia task6_839fe03c:

    # y "Too deep?"
    y "Увлеклась?"

# game/status.rpy:4216
translate russia task6_cfcb6393:

    # a y29 "Too deep undercover. We don't want her turning into a gangster..."
    a y29 "Да, работой под прикрытием. Мы не хотим, чтобы она превратилась в бандита..."

# game/status.rpy:4217
translate russia task6_27f6a213:

    # y "You think there is a risk of that?"
    y "Вы думаете, есть на то причины?"

# game/status.rpy:4218
translate russia task6_82520efd:

    # c r12 "I never expected it from Sam, but it's possible."
    c r12 "Я не ожидала такого от Сэм, но всё возможно."

# game/status.rpy:4219
translate russia task6_70496ed7:

    # c "Let's keep an eye on her for now."
    c "Давай пока присмотрим за ней."

# game/status.rpy:4220
translate russia task6_31ed3599:

    # y "Agreed."
    y "Согласен."

# game/status.rpy:4235
translate russia task6_a62791df:

    # s "There was this gem called the Sobhuza Diamond that was stolen from a museum a few months back."
    s "Несколько месяцев назад из музея был украден драгоценный камень под названием \"Бриллиант Собхуза\"."

# game/status.rpy:4236
translate russia task6_c74d8689:

    # s "Turns out, it's with the Aces now. They're hosting a party tonight where it will be present."
    s "Оказалось, что теперь он у Тузов. Сегодня они устраивают вечеринку, на которой он будет присутствовать."

# game/status.rpy:4237
translate russia task6_b2fa4418:

    # s g1 "I know you guys have been worried about me going too deep undercover. So let me get that diamond back to proof that I'm still on your side."
    s g1 "Я знаю, что вы беспокоились о том, что я слишком увлеклась работой под прикрытием. Так что позвольте мне вернуть этот бриллиант, чтобы доказать, что я всё ещё на вашей стороне."

# game/status.rpy:4238
translate russia task6_feaeef5b:

    # y "That sounds like a good idea. Okay [samNick], give it your all."
    y "Звучит как хорошая идея. Хорошо [samNick], выложись по полной."

# game/status.rpy:4240
translate russia task6_655659ad:

    # "Sam nods and leaves for the Aces."
    "Сэм кивает и уходит к Тузам."

# game/status.rpy:4251
translate russia task6_14f4b337_1:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:4252
translate russia task6_e04a5136:

    # y "That must be Sam."
    y "Должно быть это Сэм."

# game/status.rpy:4258
translate russia task6_a656aedc:

    # s "Sam here."
    s "Приём, это Сэм."

# game/status.rpy:4259
translate russia task6_3677b6f7:

    # y "Sam! Where are you?"
    y "Сэм! Где ты?"

# game/status.rpy:4260
translate russia task6_4bb09326:

    # s "I'm at WOOHP HQ. The Aces were invited to host the party here."
    s "Я в штаб-квартире WOOHP. Тузов пригласили провести вечеринку здесь."

# game/status.rpy:4261
translate russia task6_d445415a:

    # y "That should be a good opperatunity to gather some intel!{w} What about the gemstone?"
    y "Это отличная возможность для сбора информации!{w} Что там насчёт драгоценного камня?"

# game/status.rpy:4262
translate russia task6_dca30dc6:

    # s g3 "They just brought it out. Security is pretty tight, but I think I can steal i-..."
    s g3 "Они просто вынесли его. Охрана довольно сильная, но я думаю, что смогу укра-..."

# game/status.rpy:4263
translate russia task6_05a141e9:

    # "???" "Can I have your attention please..."
    "???" "Прошу внимания..."

# game/status.rpy:4265
translate russia task6_265899b8:

    # mel "Welcome all to this auspicious occasion where we celebrate WOOHP and their collaboration with the Aces."
    mel "Приветствую всех на этом знаменательном событии! Мы собрались здесь, чтобы отпраздновать сотрудничество WOOHP и Тузов."

# game/status.rpy:4266
translate russia task6_b0dcd3dd:

    # mel "They've been guarding our castle in Spain, been helping us out here in Beverly Hills and now have even invited us to come to their headquarters."
    mel "Они охраняли наш замок в Испании, помогали нам здесь, в Беверли-Хиллз, а теперь даже пригласили нас в свой штаб."

# game/status.rpy:4267
translate russia task6_58b17aab:

    # mel "We wouldn't be where we are now without them."
    mel "Без их помощь нас бы тут точно не было."

# game/status.rpy:4268
translate russia task6_5382ee6f:

    # "You hear the crowd cheering."
    "Вы слышите, как толпа аплодирует."

# game/status.rpy:4269
translate russia task6_640077f7:

    # mel "That's why it came as a surprise to hear that WOOHP agents attacked us during Landgrabs..."
    mel "Вот почему для нас стало неожиданностью известие о том, что агенты WOOHP напали на нас во время войны за территорию..."

# game/status.rpy:4270
translate russia task6_36d5802a:

    # mel "Care to shed some light on that, Mr. Scam?"
    mel "Не могли бы вы пролить свет на это происшествие, Мр. Скам?"

# game/status.rpy:4275
translate russia task6_5115f16b:

    # tim "..................."
    tim "..................."

# game/status.rpy:4276
translate russia task6_b96f627d:

    # mel "Perhaps it's because you're afraid that we're getting too powerful...?{w} Maybe even challenge your organisation?"
    mel "Возможно, это потому, что вы боитесь, что мы становимся слишком могущественными...?{w} Может даже сильнее WOOHP?"

# game/status.rpy:4277
translate russia task6_36ca6ae8:

    # mel "Or maybe you don't have as tight a hold on your agents as you thought..."
    mel "А может быть, вы не так крепко держите своих агентов в узде, как вам казалось..."

# game/status.rpy:4278
translate russia task6_2b977c31:

    # tim "I assure you Melody that we have everything under control."
    tim "Уверяю вас, Мелоди, у нас всё под контролем."

# game/status.rpy:4279
translate russia task6_b3f94f0b:

    # mel "Is that so? A little birdy told me there might even be a traitor amongst us right now..."
    mel "Так ли это? Одна птичка, нашептала мне, что сейчас среди нас может быть предатель..."

# game/status.rpy:4280
translate russia task6_ae4bfad7_2:

    # y "!!!"
    y "!!!"

# game/status.rpy:4281
translate russia task6_137ef00c:

    # y "Sam, can you hear me? You've been found out. Get out there!"
    y "Сэм, ты меня слышишь? Тебя вычислили. Уходи от туда!"

# game/status.rpy:4286
translate russia task6_ae4bfad7_3:

    # y "!!!"
    y "!!!"

# game/status.rpy:4287
translate russia task6_23b36cac:

    # "The lights went out!"
    "Свет погас!"

# game/status.rpy:4289
translate russia task6_5a53df8a:

    # sM "Psst... [playerName]..?"
    sM "Псс... [playerName]...?"

# game/status.rpy:4290
translate russia task6_eb21a933:

    # y "Sam, are you okay?"
    y "Сэм, ты в порядке?"

# game/status.rpy:4291
translate russia task6_ed1d2c73:

    # sM "Yeah, but this wasn't my doing! I think there's another thief here."
    sM "Да, но это не моих рук дело! Похоже здесь есть ещё один вор."

# game/status.rpy:4296
translate russia task6_c28258d6:

    # "The lights came back on."
    "Свет снова зажёгся."

# game/status.rpy:4297
translate russia task6_8552c66b:

    # "Aces Gangster" "The diamond is gone!"
    "Туз Гангстер" "Бриллиант исчез!"

# game/status.rpy:4298
translate russia task6_25507f85:

    # mel "I knew it! WOOHP is sabotaging us! Get that diamond back!"
    mel "Я так и знала! WOOHP предала нас! Верните бриллиант!"

# game/status.rpy:4305
translate russia task6_e36d66da:

    # tim "Not so fast Melody. We had nothing to do with this. Your gangsters don't have clearance to the rest of the facility."
    tim "Не так быстро, Мелоди. Мы не имеем к этому никакого отношения. У ваших бандитов нет доступа к остальной части объекта."

# game/status.rpy:4306
translate russia task6_86c7329e:

    # mel "What?! {w}I'll show you what I think of your 'clearance'...!"
    mel "Что?! {w}Я покажу тебе, что я думаю о твоем 'доступе'...!"

# game/status.rpy:4312
translate russia task6_b499a200:

    # sM "I think a fight is about to break out over here."
    sM "Мне кажется сейчас здесь начнётся драка."

# game/status.rpy:4313
translate russia task6_a21776e1:

    # sM "I'm going to sneak after the jewel thief. I bet I can still catch them."
    sM "Я собираюсь пойти за вором. Уверена, я всё ещё могу их поймать."

# game/status.rpy:4314
translate russia task6_fa9ab368:

    # y "Good thinking. Go after them and report back to the base."
    y "Хорошая мысль. Отправляйся за ними и доложи на базу."

# game/status.rpy:4331
translate russia task6_b84d9371:

    # y "Sam you're back!"
    y "Сэм, ты вернулась!"

# game/status.rpy:4332
translate russia task6_6a2a7ad4:

    # s "Er... yeah...{w} without the Sobhuza diamond..."
    s "Эм... да...{w} без бриллианта Собхуза..."

# game/status.rpy:4333
translate russia task6_6fbf73a5:

    # y "What happened...?"
    y "Что случилось...?"

# game/status.rpy:4334
translate russia task6_429ba9a7:

    # s g11 "Well I found the thief who was making a break for the exit."
    s g11 "Я нашла вора, который прорывался к выходу."

# game/status.rpy:4335
translate russia task6_9c9429ec:

    # s "I managed to grab him, but Melody caught me."
    s "Мне удалось схватить его, но Мелоди поймала меня."

# game/status.rpy:4336
translate russia task6_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:4337
translate russia task6_777b0936:

    # s g12 "I had no choice, but to return the diamond to her to uphold my cover."
    s g12 "У меня не было другого выбора, кроме как вернуть ей бриллиант, чтобы сохранить свое прикрытие."

# game/status.rpy:4338
translate russia task6_6a749de5:

    # y "I bet the Aces liked that."
    y "Держу пари, Тузам это понравилось."

# game/status.rpy:4339
translate russia task6_0a37fc8d:

    # s g1 "Yeah I sorta... been... promoted."
    s g1 "Да, я вроде как... была... повышена."

# game/status.rpy:4340
translate russia task6_357f469d:

    # s "The Aces were so happy with me that they threw a party in my honor which lasted all of today."
    s "Тузы были так довольны мной, что устроили вечеринку в мою честь на весь сегодняшний день."

# game/status.rpy:4341
translate russia task6_64e62b01:

    # s g28 "We got to go to a yacht!"
    s g28 "Мы поехали на яхту!"

# game/status.rpy:4342
translate russia task6_1bb1cc56:

    # y "Okay...?"
    y "Окей...?"

# game/status.rpy:4343
translate russia task6_056c1b95:

    # s "We flew paramotors, went diving and partied in a pool they had on the deck!"
    s "Мы летали на паромоторах, занимались дайвингом и веселились в бассейне у них на палубе!"

# game/status.rpy:4344
translate russia task6_8e2c2637:

    # s g1 "They were all so happy with me! I've made so many new friends!"
    s g1 "Они все были так счастливы! У меня появилось столько новых друзей!"

# game/status.rpy:4345
translate russia task6_0377b356:

    # s "I talked about the importance of charity and they agreed!{w} I know we think of them as bad guys, but we need to look at them different."
    s "Я говорила о важности благотворительности, и они согласились!{w} Я знаю что мы считаем их злодеями, но мы должны смотреть на них по-другому."

# game/status.rpy:4346
translate russia task6_b757ea92:

    # s "They're good kids with too much money. They only need a little guidance on what to do with that money and they could do some real good in the world!"
    s "Они хорошие дети, у которых слишком много денег. Им нужно лишь немного подсказать, что делать с этими деньгами, и они смогут принести реальную пользу миру!"

# game/status.rpy:4347
translate russia task6_98afaabe:

    # y "Yeah that's what I want. A bunch of coke-head fratboys ruling the world."
    y "Да, это то чего я хочу. Кучка кокаиновых братков правит миром."

# game/status.rpy:4348
translate russia task6_a14a95f1:

    # s g18 "Everyone who's running this country 'now' used to, or still is a coke-head fratboy."
    s g18 "Все, кто управляет этой страной \"сейчас\", были или до сих пор являются \"кокаиновими братками\"."

# game/status.rpy:4349
translate russia task6_9cb97b48_2:

    # y "........................"
    y "........................"

# game/status.rpy:4350
translate russia task6_dcb6a27e:

    # y "Touché."
    y "Touché. (Как трогательно.)"

# game/status.rpy:4351
translate russia task6_ede17364:

    # y "Did you at least get some intel?"
    y "Тебе хотя бы удалось раздобыть какую-нибудь информацию?"

# game/status.rpy:4352
translate russia task6_d69b44cd:

    # s g29 "Oh!{w} Right...{w} intel... heh heh~..."
    s g29 "Ох!{w} Точно...{w} информацию... хе-хе~..."

# game/status.rpy:4353
translate russia task6_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:4354
translate russia task6_7b9506e5:

    # s "[playerName]... listen."
    s "[playerName]... слушать."

# game/status.rpy:4355
translate russia task6_23ff401c:

    # y "Sam~...."
    y "Сэм~...."

# game/status.rpy:4356
translate russia task6_873fab35:

    # s "No just listen...! Here's what I'm thinking."
    s "Нет, просто послушай...! Вот что я думаю."

# game/status.rpy:4357
translate russia task6_aca6a735:

    # s g1 "Maybe we should spend less time trying to take the Aces down and focus on the other gangs in town."
    s g1 "Может нам, стоит сосредоточиться на уничтожении других банд в городе, в место того, чтобы потратить кучу времени на уничтожение Тузов."

# game/status.rpy:4358
translate russia task6_00e6d64b:

    # s "It would make sense! Taking down all gangs at the same time just leaves us divided. Better to focus our strength!"
    s "В этом есть смысл! Одновременное уничтожение всех банд просто разделит нас. Лучше сосредоточить наши силы!"

# game/status.rpy:4359
translate russia task6_b94c473e:

    # y "Sam~...!"
    y "Сэм~...!"

# game/status.rpy:4360
translate russia task6_dacc1493:

    # s g14 "I bet that by the time we take down the other gangs, the Aces will have seen their flaws and disband themselves!"
    s g14 "Держу пари, что к тому времени когда мы уничтожим другие банды, Тузы увидят свои недостатки и распадутся!"

# game/status.rpy:4361
translate russia task6_d586b5c8:

    # s "They're just dumb kids who made some mistakes! They need love and guidance, not a cold lonely prison cell!"
    s "Они просто глупые дети совершившие несколько ошибок! Им нужна любовь и руководство, а не холодная одинокая тюремная камера!"

# game/status.rpy:4362
translate russia task6_c9779f9e:

    # s "And-...!"
    s "И-...!"

# game/status.rpy:4364
translate russia task6_db2fac6c:

    # y "SAM!"
    y "СЭМ!"

# game/status.rpy:4367
translate russia task6_1cc6f83e:

    # y "Have you gone completely crazy?! The Aces are gangsters! The city is burning because of them!"
    y "Ты совсем с ума сошла?! Тузы - бандиты! Город горит из-за них!"

# game/status.rpy:4368
translate russia task6_b9d63401:

    # s g29 "But...!"
    s g29 "Но...!"

# game/status.rpy:4369
translate russia task6_0a729fa2:

    # y "And you've gone way too deep undercover. You're not going there anymore!"
    y "И ты и правда слишком увлеклась Тузами. Ты больше туда не пойдешь!"

# game/status.rpy:4370
translate russia task6_30db0599:

    # s g28 "No...!"
    s g28 "Нет...!"

# game/status.rpy:4372
translate russia task6_86b02fb4:

    # y "Sam...{w} We've told you before... The Aces are dangerous. They're drug dealers, violent robbers and more."
    y "Сэм...{w} Мы уже говорили тебе... Тузы опасны. Это наркоторговцы, жестокие грабители и многое другое."

# game/status.rpy:4373
translate russia task6_b9d63401_1:

    # s g29 "But...!"
    s g29 "Но...!"

# game/status.rpy:4374
translate russia task6_d4620ce1:

    # y "You've gone way too deep undercover. Remember that we're working to liberate the city. We can't do that with the Aces running around it."
    y "Помни что мы работаем над освобождением города. Мы не сможем сделать это, когда вокруг него бегают Тузы."

# game/status.rpy:4375
translate russia task6_c00b32be:

    # y g15 "But... you don't understand..."
    y g15 "Но... ты не понимаешь..."

# game/status.rpy:4376
translate russia task6_769757c8:

    # s g15 "The... the Aces..."
    s g15 "Они... Т-тузы..."

# game/status.rpy:4377
translate russia task6_33de0cec:

    # s "They're...."
    s "Они..."

# game/status.rpy:4378
translate russia task6_01b0eab5:

    # s "They..."
    s "О-они..."

# game/status.rpy:4379
translate russia task6_6f9bfea5:

    # s "{b}*Sniff*{/b} They think I'm cool... They accept me... and they want to be my friend..."
    s "{b}*Шмыгает носом*{/b} Они считают меня крутой... Они принимают меня... и они хотят быть моими друзьями..."

# game/status.rpy:4380
translate russia task6_cf630fb5_1:

    # y "..................."
    y "..................."

# game/status.rpy:4381
translate russia task6_174c1fe5:

    # s g14 "Do you have any idea how much I've been bullied in my life...? I was never invited to parties...{w} always seen as the weird kid who only studies....!"
    s g14 "Ты хоть представляешь, сколько надо мной издевались всю мою жизни...? Меня никогда не приглашали на вечеринки...{w} всегда считали странным ребёнком, который занимается только зубрёжкой...!"

# game/status.rpy:4382
translate russia task6_8b0f2424:

    # s "Before I met Alex and Clover, I didn't even have any friends! I thought I didn't need friends! I decided to fully commit to WOOHP and being a spy and now...! And now...!"
    s "До встречи с Алекс и Кловер у меня даже не было друзей! Я думала, мне не нужны друзья! Я решила полностью посвятить себя WOOHP и быть шпионом, а теперь...! А теперь...!"

# game/status.rpy:4383
translate russia task6_b848f26a:

    # s "Now I know what it's like to feel accepted... The people who used to bully me now listen to me and come to me for advice..."
    s "Теперь я знаю, каково это - чувствовать себя принятой... Люди, которые раньше издевались надо мной, теперь слушают меня и приходят ко мне за советом..."

# game/status.rpy:4384
translate russia task6_19982716:

    # s g5 "I'm no longer the weirdo. No longer the freak they make fun of...."
    s g5 "Я больше не чудачка. Больше не уродина, над которой они смеялись..."

# game/status.rpy:4385
translate russia task6_f358cb0c_1:

    # y "................."
    y "................."

# game/status.rpy:4386
translate russia task6_e39d2cf7:

    # y "Sam... We all make choices in life. They chose to do crime and need to take responsibilty for that."
    y "Сэм... Все мы делаем выбор в этой жизни. Они решили совершить преступление и должны нести за это ответственность."

# game/status.rpy:4387
translate russia task6_77cbbc21:

    # s g14 "But...!"
    s g14 "Но...!"

# game/status.rpy:4388
translate russia task6_eedfbbef:

    # y "And you chose to be a spy. Now you need to take responsibility for that too."
    y "А ты решила стать шпионом. Теперь и ты должна взять на себя ответственность за это."

# game/status.rpy:4389
translate russia task6_e2c6fd35:

    # s g15 "........................."
    s g15 "........................."

# game/status.rpy:4390
translate russia task6_df01d2a6:

    # y "This city is relying on you. If you can't be there for them, then you have no business being a spy."
    y "Этот город рассчитывает на тебя. Если ты не можешь быть рядом с ним, то тебе нечего делать в рядах шпионок."

# game/status.rpy:4391
translate russia task6_a742beb6:

    # s "......................."
    s "......................."

# game/status.rpy:4395
translate russia task6_5cbf3310:

    # c "Sam...."
    c "Сэм..."

# game/status.rpy:4396
translate russia task6_f2dff589:

    # s "...................."
    s "...................."

# game/status.rpy:4397
translate russia task6_d39b025a:

    # s g14 "{b}*Sniff*{/b} Melody trusts me now..."
    s g14 "{b}*Шмыгает носом*{/b} Мелоди доверяет мне..."

# game/status.rpy:4398
translate russia task6_b148a80e:

    # s "We can visit the castle and take her in..."
    s "Мы можем попасть в замок и схватить её..."

# game/status.rpy:4399
translate russia task6_a23669b1_1:

    # y "............................"
    y "............................"

# game/status.rpy:4400
translate russia task6_63a62cab:

    # s "I'm sorry...."
    s "Мне очень жаль..."

# game/status.rpy:4401
translate russia task6_976faf6a:

    # a "Sammy..."
    a "Сэмми..."

# game/status.rpy:4402
translate russia task6_05815b8c:

    # c "Don't be sorry, Sam. We understand."
    c "Не извиняйся, Сэм. Мы понимаем."

# game/status.rpy:4403
translate russia task6_b1d4f52b:

    # s "I'm... {w}I'm going to my cell..."
    s "Я... {w}Я пойду в свою камеру..."

# game/status.rpy:4406
translate russia task6_5758066b:

    # a "........................."
    a "........................."

# game/status.rpy:4407
translate russia task6_56694904:

    # c "........................."
    c "........................."

# game/status.rpy:4408
translate russia task6_b0ad3bdf:

    # y "We should've done something sooner..."
    y "Мы должны были сделать что-то раньше..."

# game/status.rpy:4409
translate russia task6_d1f9eb2d:

    # c r12 "We tried, I don't think this was something we could've prevented..."
    c r12 "Мы пытались, но я не думаю что это можно было предотвратить..."

# game/status.rpy:4410
translate russia task6_b09e2dc7:

    # c r34 "There is still a mission that needs completing.{w} Send us to the castle whenever and we will take Melody down."
    c r34 "У нас есть задание, которое необходимо выполнить.{w} Отправь нас в замок и мы схватим Мелоди."

# game/status.rpy:4411
translate russia task6_6d4e9a7f:

    # y "And Sam?"
    y "А Сэм?"

# game/status.rpy:4412
translate russia task6_b9d8243d:

    # c r37 "We'll talk to her... She knows it's the right thing to do...{w} It's just difficult for her right now."
    c r37 "Мы поговорим с ней... Она знает, что это правильно...{w} Сейчас ей просто трудно."

# game/status.rpy:4416
translate russia task6_2ae2e423:

    # "Alex and Clover head of to their cells."
    "Алекс и Кловер отправляются в свои камеры."

# game/status.rpy:4417
translate russia task6_5920634f:

    # "Sam has won Melody's trust and we can now travel to the castle to take her in."
    "Сэм завоевала доверие Мелоди, и теперь мы можем отправиться в замок, чтобы схватить её."

# game/status.rpy:4418
translate russia task6_a66244cf:

    # "New clothing options are available for Sam at the mall."
    "В торговом центре появились новые варианты одежды для Сэм."

# game/status.rpy:4444
translate russia task6_0eb6ff59:

    # cM "We got her now!"
    cM "Она наша!"

# game/status.rpy:4445
translate russia task6_0dc13f23:

    # mel ".............."
    mel ".............."

# game/status.rpy:4446
translate russia task6_9a0c1a37:

    # mel "Sam..."
    mel "Сэм..."

# game/status.rpy:4447
translate russia task6_4d3095ca:

    # sM ".................."
    sM ".................."

# game/status.rpy:4448
translate russia task6_cfb18515:

    # mel "How could you...?"
    mel "Как ты могла...?"

# game/status.rpy:4449
translate russia task6_1caa074a:

    # cM "Don't listen to her, Sam. You did the right thing."
    cM "Не слушай её, Сэм. Ты поступила правильно."

# game/status.rpy:4450
translate russia task6_2bc07f0c:

    # aM "Yeah! You're coming with us!"
    aM "Да! Ты идёшь с нами!"

# game/status.rpy:4451
translate russia task6_30dafa0c:

    # mel "I'd like to see you tr-..."
    mel "Я бы хотел увидеть те-..."

# game/status.rpy:4457
translate russia task6_038f4e96:

    # "Melody gets instantly overpowered by the three spies."
    "Три шпионки мгновенно одерживывают верх над Мелоди."

# game/status.rpy:4475
translate russia task6_88b299d8:

    # y "Well well well, if it isn't Melody G. So nice of you to joi-..."
    y "Так-так, неужели это Мелоди Г. Так мило с твоей стороны присоеди-..."

# game/status.rpy:4476
translate russia task6_05bf074c:

    # y ".............."
    y ".............."

# game/status.rpy:4477
translate russia task6_799bdeaa:

    # y "Ah let's just skip the dialogue and go straight to the fucking."
    y "Давайте сразу пропустим диалог и перейдем к траху."

# game/status.rpy:4478
translate russia task6_012a914a:

    # mel "W-what?!"
    mel "Ч-что?!"

# game/status.rpy:4479
translate russia task6_252b7150:

    # y "Yeah you know. Break you out of your nanobot trance, give us the intel we need."
    y "Да, ты знаешь. Мы выведем тебя из-под контроля наноботов, чтобы получить необходимую нам информацию."

# game/status.rpy:4480
translate russia task6_109c969f:

    # y "It's sort of par for the course by this point."
    y "На данный момент это уже стало обычным делом."

# game/status.rpy:4481
translate russia task6_ca819573:

    # mel "If you lay even a finger on me I swear I'll bite it off."
    mel "Если ты хоть пальцем меня тронешь, клянусь, я его откушу."

# game/status.rpy:4482
translate russia task6_906bf9bf:

    # y "...........?"
    y "...........?"

# game/status.rpy:4483
translate russia task6_374e499f:

    # y "Melody.... your eyes aren't red..."
    y "Мелоди.... твои глаза не красные..."

# game/status.rpy:4484
translate russia task6_dfe06f8e:

    # mel "Of course they're not. I'm not one of those brainwashed zombies. I don't have nanobots in me!"
    mel "Конечно, нет. Я не одна из этих зомби с промытыми мозгами. Во мне нет наноботов!"

# game/status.rpy:4485
translate russia task6_5a5fcd5a:

    # y "Oh...."
    y "Ох..."

# game/status.rpy:4486
translate russia task6_0a1cbac5:

    # y "Well this is awkward..."
    y "Как-то неловко..."

# game/status.rpy:4487
translate russia task6_2576af54:

    # y "Then why are you out there working for the mastermind?"
    y "Тогда почему ты работаешь на Организатора?"

# game/status.rpy:4488
translate russia task6_040d990d:

    # mel "Payback.{w} My grandmother is in jail because of your spies, you know."
    mel "Месть.{w} Моя бабушка в тюрьме из-за твоих шпионок, ты знаешь."

# game/status.rpy:4489
translate russia task6_7083c0cd:

    # y "Actually... I don't know. Let me get my spies."
    y "Вообще-то... Я не знаю. Позволь мне позвать шпионок."

# game/status.rpy:4490
translate russia task6_5e5ef226:

    # "You call your girls and in a moment they arrive."
    "Вы вызываете девушек, и через мгновение они приходят."

# game/status.rpy:4495
translate russia task6_d483d667:

    # c "[playerName] before you offer, we're not going to watch you have sex with her!"
    c "[playerName] прежде чем ты предложишь, мы не собираемся смотреть, как ты занимаешься с ней сексом!"

# game/status.rpy:4496
translate russia task6_8aeb146f:

    # a "Yeah you voyeuristic weirdo!"
    a "Да, ты вуайеристический чудак!"

# game/status.rpy:4497
translate russia task6_b65f1c31:

    # y "First of all...{w} ouch. That hurt my feelings.{w} Shame on you Alex."
    y "Во-первых...{w} аучь. Это задело мои чувства.{w} Как тебе не стыдно, Алекс."

# game/status.rpy:4498
translate russia task6_7fb3891b:

    # a y30 "!!!"
    a y30 "!!!"

# game/status.rpy:4499
translate russia task6_3f16c755:

    # y "Secondly, she's not being controlled by nanobots, so there's no point is sleeping with her."
    y "Во-вторых, её не контролируют наноботы, поэтому нет смысла спать с ней."

# game/status.rpy:4500
translate russia task6_b1131d92:

    # c r29 "She's not...?"
    c r29 "В ней нет наноботов...?"

# game/status.rpy:4501
translate russia task6_11c0073b:

    # mel "No.. I'm not. Hello Clover... Alex..."
    mel "Нет... Нету. Привет, Кловер... Алекс..."

# game/status.rpy:4502
translate russia task6_5ef09f2d:

    # mel "Your redheaded traitorous friend not joining you?"
    mel "Ваша рыжая предательница не присоединится к нам?"

# game/status.rpy:4503
translate russia task6_d63e74cf:

    # a y32 "You leave Sammie out of this!"
    a y32 "Не впутывай Сэмми!"

# game/status.rpy:4504
translate russia task6_76155113:

    # c r31 "She went through a lot busting your ass."
    c r31 "Она прошла через многое, ловя твою задницу."

# game/status.rpy:4505
translate russia task6_80096dbf:

    # mel "...................."
    mel "...................."

# game/status.rpy:4506
translate russia task6_5bc41c22:

    # mel "Question. Do you two remember my grandmother, Helga von Guggen?"
    mel "Вопрос. Вы помните мою бабушку, Хельгу фон Гугген?"

# game/status.rpy:4507
translate russia task6_cd37e4c8:

    # c "She's hard to forget..."
    c "Её трудно забыть..."

# game/status.rpy:4508
translate russia task6_a36a95d0:

    # a y29 "She shrunk our clothes! It was very embarrassing!"
    a y29 "Она уменьшила нашу одежду! Было очень неловко!"

# game/status.rpy:4509
translate russia task6_0e522856:

    # c r12 "Not to mention all the other times she tried to get rid of us."
    c r12 "Не говоря обо всех других случаях, когда она пыталась избавиться от нас."

# game/status.rpy:4510
translate russia task6_872c6fde:

    # c "So what? You want revenge for locking up your nana?"
    c "И что? Ты хочешь отомстить за то что мы заперли твою бабулю?"

# game/status.rpy:4511
translate russia task6_0c0d56eb:

    # mel "Yes!"
    mel "Да!"

# game/status.rpy:4512
translate russia task6_8d22e973:

    # mel "................."
    mel "................."

# game/status.rpy:4513
translate russia task6_76aa2294:

    # mel "At least at first..."
    mel "По крайней мере, для начала..."

# game/status.rpy:4514
translate russia task6_225ac3cd:

    # y "At first? Not anymore?"
    y "Для начала? Это не всё?"

# game/status.rpy:4515
translate russia task6_8d22e973_1:

    # mel "................."
    mel "................."

# game/status.rpy:4516
translate russia task6_7bdd01e4:

    # mel "I've said enough. You're not getting anything out of me."
    mel "Я уже достаточно сказала. Больше ты ничего от меня не добьёшься."

# game/status.rpy:4519
translate russia task6_435354ba:

    # y "What's your theme?"
    y "Какая у тебя тематика?"

# game/status.rpy:4520
translate russia task6_e7da9f41:

    # mel "My... huh?"
    mel "Моя... что?"

# game/status.rpy:4521
translate russia task6_d51ec3f9:

    # y "You know. Maggie T. was interior design, you must have a theme as well."
    y "Ты знаешь. У Мэгги Т. был дизайн интерьера, у тебя тоже должна быть определённая тематика."

# game/status.rpy:4522
translate russia task6_36a3bb15:

    # mel "Fashion..?"
    mel "Мода...?"

# game/status.rpy:4523
translate russia task6_a6061791:

    # y "Fashion..."
    y "Мода..."

# game/status.rpy:4524
translate russia task6_8a8ee092:

    # y "I know nothing of fashing. Clover you're up!"
    y "Я ничего не знаю о моде. Кловер, твоя очередь!"

# game/status.rpy:4525
translate russia task6_359b7100:

    # c r18 "Up to do what exactly...?"
    c r18 "Для чего именно...?"

# game/status.rpy:4526
translate russia task6_66304e4a:

    # y "Appeal to her theme! Make her our friend. Before you know it, she'll tell us everything!"
    y "Поболтай о её тематике! Сделай её нашим другом. Не успеешь оглянуться, как она всё нам расскажет!"

# game/status.rpy:4528
translate russia task6_1bd8601d:

    # y "Okay... I didn't want to do this..."
    y "Хорошо... Я не хотел этого делать..."

# game/status.rpy:4529
translate russia task6_cd74cae9:

    # mel "....................."
    mel "....................."

# game/status.rpy:4530
translate russia task6_dc025c4c:

    # y "You're really leaving me no other choice..."
    y "Ты действительно не оставляешь мне другого выбора..."

# game/status.rpy:4531
translate russia task6_80096dbf_1:

    # mel "...................."
    mel "...................."

# game/status.rpy:4532
translate russia task6_390be858:

    # y "I'm gonna say it....!"
    y "Я скажу это....!"

# game/status.rpy:4533
translate russia task6_b5d110b8:

    # y "Your hair color makes you look like an old woman."
    y "Твой цвет волос делает тебя похожей на старуху."

# game/status.rpy:4534
translate russia task6_7c3f0cb5:

    # mel "........"
    mel "........"

# game/status.rpy:4536
translate russia task6_beef4d69:

    # y "Let's see how willing to talk you are....{w} WITHOUT A HEAD!"
    y "Давай посмотрим, какой разговорчивой ты будешь...{w} БЕЗ ГОЛОВЫ!"

# game/status.rpy:4537
translate russia task6_7fb3891b_1:

    # a y30 "!!!"
    a y30 "!!!"

# game/status.rpy:4538
translate russia task6_b6a26306:

    # c r30 "!!!"
    c r30 "!!!"

# game/status.rpy:4539
translate russia task6_713902a6:

    # y "Not very talkative I imagine... {w}due to the 'not having a head' part."
    y "Не очень разговорчивой, как я полагаю... {w}из-за \"отсутствия головы\"."

# game/status.rpy:4540
translate russia task6_6724fd36:

    # c "[playerName]...."
    c "[playerName]...."

# game/status.rpy:4541
translate russia task6_b072c0d7:

    # mel "Is he crazy....?"
    mel "Он сумасшедший...?"

# game/status.rpy:4542
translate russia task6_bee94b64:

    # c r35 "We're still not entirely sure on that ourselves..."
    c r35 "Мы и сами до сих пор не до конца уверены в этом..."

# game/status.rpy:4543
translate russia task6_de72feab:

    # y "Would a crazy person do this?!"
    y "Разве сумасшедший сделал бы вот так?!"

# game/status.rpy:4544
translate russia task6_749783ac:

    # "You begin doing karate chops in the air."
    "Вы начинаете выполнять удары карате в воздухе."

# game/status.rpy:4545
translate russia task6_27f143b2:

    # a y30 "H-hey...! [playerName]! Watch where you're swinging!"
    a y30 "Э-эй...! [playerName]! Смотри куда ты бьёшь!"

# game/status.rpy:4546
translate russia task6_a401595a:

    # c r31 "Careful, you're going to end up hurting yours-...."
    c r31 "В конце концов, ты сам себя удари-..."

# game/status.rpy:4549
translate russia task6_6d5aa0be:

    # a y28 "!!!"
    a y28 "!!!"

# game/status.rpy:4550
translate russia task6_15c6cd00:

    # c r28 "!!!"
    c r28 "!!!"

# game/status.rpy:4551
translate russia task6_c6c9e720:

    # mel "!!!"
    mel "!!!"

# game/status.rpy:4552
translate russia task6_42be27b2:

    # "You accidentally slapped Melody across the face!"
    "Вы случайно бьёте Мелоди по лицу!"

# game/status.rpy:4553
translate russia task6_f4740d84:

    # y "I'm sorry! I got carried away trying to intimidate you...!"
    y "Ой! Прошу прощения! Я увлёкся, пытаясь запугать тебя...!"

# game/status.rpy:4554
translate russia task6_559e7924:

    # mel "......."
    mel "......."

# game/status.rpy:4555
translate russia task6_17776065:

    # mel "It's fine...."
    mel "Да, всё в порядке..."

# game/status.rpy:4556
translate russia task6_9a2ea42f:

    # a y8 "[playerName]! We don't hurt our prisoners!"
    a y8 "[playerName]! Мы не причиняем вреда нашим заключенным!"

# game/status.rpy:4557
translate russia task6_3c590e3f:

    # c r38 "That's not the WOOHP way!"
    c r38 "WOOHP так не делает!"

# game/status.rpy:4558
translate russia task6_3794434d:

    # y "I'm sorry! I'm sorry, it was an acci-..."
    y "Простите! Прости, пожалуйста, я не спец-..."

# game/status.rpy:4559
translate russia task6_ab18bdb4:

    # "Suddenly Melody lets out a mocking laugh."
    "Внезапно Мелоди начинает издевательски смеяться."

# game/status.rpy:4560
translate russia task6_6faece12:

    # mel "{i}Not the WOOHP way?{/i}"
    mel "{i}WOOHP так не делает?{/i}"

# game/status.rpy:4561
translate russia task6_ccdd9e85:

    # mel "You stupid girl! Do you have any idea what WOOHP does with its prisoners after you capture them?!"
    mel "Ты глупая девчонка! Ты хоть представляешь, что WOOHP делает со своими пленниками после того, как вы их схватили?!"

# game/status.rpy:4562
translate russia task6_143e3605:

    # a y29 "W-what...?"
    a y29 "Ч-что...?"

# game/status.rpy:4563
translate russia task6_a5c159cd:

    # c r32 "Don't listen to her, Alex. She's trying to trick us."
    c r32 "Не слушай её, Алекс. Она пытается нас обмануть."

# game/status.rpy:4564
translate russia task6_22684384:

    # mel "Oooh, the mighty WOOHP! So righteous! Saving the world from tyranny!"
    mel "Оооо, могучий WOOHP! Таки правильные! Спасают мир от тирании!"

# game/status.rpy:4565
translate russia task6_b4c0af91:

    # mel "Have you {i}'any'{/i} idea what my grandmother went through?!"
    mel "Вы хоть представляете через что прошла моя бабушка?!"

# game/status.rpy:4566
translate russia task6_8881dae0:

    # mel "Why she kept wanting to take revenge on your organization?!"
    mel "Почему она всё время хочет отомстить вашей организации?!"

# game/status.rpy:4567
translate russia task6_9f5bfe10:

    # mel "WOOHP is ten times as tyrannical as the {i}'villains'{/i} it pretends to save the world from!"
    mel "WOOHP в десять раз тираничнее тех {i}'злодеев'{/i}, от которых он пытается спасти мир!"

# game/status.rpy:4568
translate russia task6_270dbec8:

    # c r31 "WOOHP is keeping the world safe from people like you!"
    c r31 "WOOHP защищает мир от таких людей, как ты!"

# game/status.rpy:4569
translate russia task6_4bb591f4:

    # a y29 "Yeah!"
    a y29 "Да!"

# game/status.rpy:4570
translate russia task6_d1644eee:

    # mel "I'm preaching to the choir... {w}You girls are as thick as grandmother descripted you."
    mel "Я скажу очевидное... {w}Вы, девочки, не разлей вода как вас описывала бабушка."

# game/status.rpy:4571
translate russia task6_897edde7:

    # mel "Do with me as you please. You're not getting a word out of me."
    mel "Делайте со мной, что хотите. Вы не получите от меня ни слова."

# game/status.rpy:4572
translate russia task6_4e901795:

    # "The atmosphere in the room is tense as the women defiantly glare at each other."
    "Атмосфера в комнате напряжённая, девушки вызывающе смотрят друг на друга."

# game/status.rpy:4573
translate russia task6_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:4574
translate russia task6_d2277a70:

    # y "Alex...{w} Clover..."
    y "Алекс...{w} Кловер..."

# game/status.rpy:4575
translate russia task6_6d9775bf:

    # y "Please go get Sam."
    y "Пожалуйста, сходите за Сэм."

# game/status.rpy:4576
translate russia task6_2249dd62:

    # c r8 "We can't bring her here! You saw what the Aces did to her!"
    c r8 "Мы не можем привести её сюда! Ты видел, что Тузы сделали с ней!"

# game/status.rpy:4577
translate russia task6_1bfcca04:

    # y "Please go get her and then leave the cell."
    y "Пожалуйста, сходите за ней, а затем покиньте камеру."

# game/status.rpy:4578
translate russia task6_b390a121:

    # a y4 "But [playerName]~....!"
    a y4 "Но [playerName]~....!"

# game/status.rpy:4579
translate russia task6_b6b0f3b3:

    # y "Now, please."
    y "Сейчас, пожалуйста."

# game/status.rpy:4580
translate russia task6_66e8d6cd:

    # c r37 "..........................."
    c r37 "..........................."

# game/status.rpy:4581
translate russia task6_32b1625d:

    # c "I don't know what you're planning [playerName], but you better have a good reason for it..."
    c "Я не знаю что ты там задумал [playerName], но надеюсь у тебя веская причина для этого..."

# game/status.rpy:4585
translate russia task6_e8d0c4e0:

    # mel ".................................."
    mel ".................................."

# game/status.rpy:4587
translate russia task6_a8ddd724:

    # "A little while later, Sam enters the cell."
    "Через некоторое время в камеру входит Сэм."

# game/status.rpy:4590
translate russia task6_d3caf497:

    # mel "If it isn't the traitor herself..."
    mel "Неужели это та сама предательница..."

# game/status.rpy:4591
translate russia task6_07d7c4be:

    # s "............................."
    s "............................."

# game/status.rpy:4592
translate russia task6_c0aa7a79:

    # mel "I really thought you changed..."
    mel "Я действительно думала, что ты изменилась..."

# game/status.rpy:4593
translate russia task6_85960226:

    # mel "The moment you showed up at our doorstep I knew I had to keep an eye on you..."
    mel "Как только ты появилась на нашем пороге, я поняла, что должна присматривать за тобой..."

# game/status.rpy:4594
translate russia task6_8b1750cd:

    # mel "I would've killed you right there and then...{w} If it wasn't for Tim."
    mel "Я была готова убить тебя прямо там...{w} Если бы не Тим."

# game/status.rpy:4595
translate russia task6_39fbe447:

    # y "Tim? Tim Scam?"
    y "Тим? Тим Скам?"

# game/status.rpy:4596
translate russia task6_0bd60de2:

    # mel "......................."
    mel "......................."

# game/status.rpy:4597
translate russia task6_d76c8777:

    # mel "You sure fooled everyone, Sam..."
    mel "Ты всех обманула, Сэм..."

# game/status.rpy:4598
translate russia task6_65c2ecb9:

    # mel "Going on missions with us... going to our parties... planning heists together..."
    mel "Ходила с нами на задания... на наши вечеринки... планировала с нами ограбления..."

# game/status.rpy:4599
translate russia task6_1478b2e6:

    # mel "Even I began to trust you..."
    mel "Даже я начал доверять тебе..."

# game/status.rpy:4600
translate russia task6_36819588:

    # s "............................"
    s "............................"

# game/status.rpy:4601
translate russia task6_1c94736e:

    # mel "Lot of good that did me..."
    mel "Это принесло мне много пользы..."

# game/status.rpy:4602
translate russia task6_a218306e:

    # s g45 "You had to be stopped Melody...{w} Look what you did to the city..."
    s g45 "Тебя нужно было остановить Мелоди...{w} Посмотри, что вы сделали с городом..."

# game/status.rpy:4603
translate russia task6_c9e3a343:

    # mel "I'm not proud of it.... But if that's what it took to bring down WOOHP, it was worth it."
    mel "Я этим не горжусь.... Но если это требуется для уничтожения WOOHP, то оно того стоит."

# game/status.rpy:4604
translate russia task6_1a81fa4c:

    # s g31 "Why would you want to bring down WOOHP?! Just because we imprisoned your grandmother?! She was evil and needed to be stopped!"
    s g31 "Почему ты хочешь уничтожить WOOHP?! Только потому, что мы посадили твою бабушку в тюрьму?! Она была злой и её нужно было остановить!"

# game/status.rpy:4605
translate russia task6_80096dbf_2:

    # mel "...................."
    mel "...................."

# game/status.rpy:4606
translate russia task6_d9ea471a:

    # mel "Do you really not know or are you feigning ignorance?"
    mel "Ты действительно не знаешь или притворяешься тупой?"

# game/status.rpy:4607
translate russia task6_f951ea65:

    # s g5 ".........................................."
    s g5 ".........................................."

# game/status.rpy:4608
translate russia task6_602497f2:

    # mel "WOOHP isn't the good, kind hearted organization you think it is. To think you're still a part of it..."
    mel "WOOHP - не такая уж хорошая, добросердечная организация, как вы думаете. Подумать только, ты всё ещё часть этого..."

# game/status.rpy:4609
translate russia task6_9cc94600:

    # mel "We trusted you, Sam... We really did..."
    mel "Мы доверяли тебе, Сэм... Мы действительно доверяли..."

# game/status.rpy:4610
translate russia task6_ac9d4b0a:

    # mel "To see the look on Jerry Lewis' face when he found out his favorite spies helped overthrow his organization. Revenge would've been so sweet..."
    mel "Увидеть выражение лица Джерри Льюиса, когда он узнал, что его любимые шпионы помогли свергнуть его организацию. Месть была бы такой сладкой..."

# game/status.rpy:4611
translate russia task6_9d60be10:

    # mel "Of course that dream is far gone now...{w} The Aces are in shambles. Their Lieutenants gone, their relation with the Mastermind ruined, their topman turning out to be a undercover spy."
    mel "Конечно, сейчас эта мечта далеко в прошлом...{w} Тузы в смятении. Их лейтенанты пропали, отношения с Организатором испортились, их лучшая соратница оказалась шпионкой под прикрытием."

# game/status.rpy:4612
translate russia task6_4ee955b2:

    # s g4 "I... I am so-..."
    s g4 "Я... Мне оч-..."

# game/status.rpy:4613
translate russia task6_58e9ccc5:

    # mel "Ask your questions... {w}There's no point in hiding things anymore and I'm tired of being the villain."
    mel "Задавайте свои вопросы... {w}Больше нет смысла скрывать, и я устал быть злодеем."

# game/status.rpy:4614
translate russia task6_e518f9c7:

    # mel "I'll cooperate..."
    mel "Я буду сотрудничать..."

# game/status.rpy:4615
translate russia task6_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:4619
translate russia task6_03d55f2d:

    # "Melody gives you all the information she has."
    "Мелоди делиться с вам всей информацией которая у неё есть."

# game/status.rpy:4627
translate russia task6_88467dc6:

    # s "Thank you Melody..."
    s "Спасибо Мелоди..."

# game/status.rpy:4628
translate russia task6_d8738c4e:

    # mel "Just... leave me alone."
    mel "Просто... оставьте меня в покое."

# game/status.rpy:4633
translate russia task6_024efe85:

    # "Computer" "Melody G. status: Captured.{w} Allocating resources..."
    "Компьютер" "Мелоди Г. статус: Захвачена.{w} Распределение ресурсов..."

# game/status.rpy:4645
translate russia task6_b43847f1:

    # a "Sammie...?"
    a "Сэмми...?"

# game/status.rpy:4646
translate russia task6_1c7766ac:

    # c "Are you okay, Sam?"
    c "Ты в порядке, Сэм?"

# game/status.rpy:4647
translate russia task6_1a81f4a2:

    # s "Yeah..."
    s "Да..."

# game/status.rpy:4648
translate russia task6_c16e043a:

    # y "It's been a long day. I vote that we get pizza and call it a night."
    y "Это был долгий день. Я голосую за то, чтобы мы взяли пиццу и закончили на этом."

# game/status.rpy:4649
translate russia task6_223aff64:

    # "The girls nod is agreement."
    "Девочки кивают в знак согласия."

# game/status.rpy:4660
translate russia task6_a66244cf_1:

    # "New clothing options are available for Sam at the mall."
    "В торговом центре появились новые варианты одежды для Сэм."

# game/status.rpy:4688
translate russia task7_c19df307:

    # s "This place is actually pretty big. This must be where all of WOOHP's crazy inmates were being held."
    s "Это место на самом деле довольно большое. Должно быть, именно здесь содержались все сумасшедшие заключённые WOOHP."

# game/status.rpy:4689
translate russia task7_570396e5:

    # y "And now they're roaming Beverly Hills."
    y "А теперь они бродят по Беверли-Хиллз."

# game/status.rpy:4690
translate russia task7_d4525f1d:

    # s g19 "Yeah..."
    s g19 "Да..."

# game/status.rpy:4692
translate russia task7_424abb80:

    # a y29 "I bet Tim was being held here too~...."
    a y29 "Держу пари, Тима тоже держали здесь~..."

# game/status.rpy:4693
translate russia task7_75307683:

    # c r22 "Maybe you can find Tim's diary here, Sam. Maybe a hidden letter where he confesses his love to you~..."
    c r22 "Может мы сможем найти здесь дневник Тима. А может и тайное письмо, где он признаётся Сэм в любви~..."

# game/status.rpy:4694
translate russia task7_cd5a2985:

    # s g9 "TIM AND I ARE NOT A COUPLE!"
    s g9 "МЫ С ТИМОМ НЕ ПАРА!"

# game/status.rpy:4695
translate russia task7_103e98ac:

    # c r24 "Just think about all the lonely nights he's had here, thinking of you~....!"
    c r24 "Просто подумай о всех одиноких ночах, которые он провёл здесь, думая о тебе~...!"

# game/status.rpy:4697
translate russia task7_834df26f:

    # a y42 "{i}'Oh Sam, my one true love. We were made for each other, but I have chosen the path of evil and our forbidden love must never be!'{/i}"
    a y42 "{i}'Оо Сэм, моя любовь. Мы были созданы друг для друга, но я выбрал путь зла, а наша запретная любовь никогда не должна существовать!'{/i}"

# game/status.rpy:4698
translate russia task7_03a3f3c7:

    # y "Hah! Better be careful not to find any crusty socks around here!"
    y "Хах! Лучше быть осторожнее, и не найти здесь затвердевшие носки!"

# game/status.rpy:4699
translate russia task7_794a53ad:

    # c r10 "....................."
    c r10 "....................."

# game/status.rpy:4701
translate russia task7_6a82aa3c:

    # a y10 "....................."
    a y10 "....................."

# game/status.rpy:4703
translate russia task7_26c4294f:

    # s g30 "....................."
    s g30 "....................."

# game/status.rpy:4704
translate russia task7_ac17dec1:

    # y "What?"
    y "Что?"

# game/status.rpy:4705
translate russia task7_4a732359:

    # c r8 "Way to take it too far, [playerName]."
    c r8 "Ты зашёл слишком далеко, [playerName]."

# game/status.rpy:4707
translate russia task7_a6afbb0d:

    # a y42 "Ew, ew, ew! That's an image I didn't want in my mind!"
    a y42 "Фу, фу, фу! Образ, который я не хотел видеть в своем сознании!"

# game/status.rpy:4711
translate russia task7_f8da6cc7:

    # y "Er... Sam?"
    y "Эм... Сэм?"

# game/status.rpy:4712
translate russia task7_8aa55d18:

    # s g40 "Err... heh heh~..."
    s g40 "Эмм... хе-хе~..."

# game/status.rpy:4714
translate russia task7_52bd2be7:

    # s "{b}*Ahem*{/b} there's one other thing I wanted to discuss with you."
    s "{b}*Кхм*{/b} есть ещё одна вещь, которую я хотела бы обсудить с тобой."

# game/status.rpy:4715
translate russia task7_6a2da24c:

    # s "I found this terminal in the cell block. It's an automated WOOHP Bounty board. When criminals escape and are brought back, funds are made available."
    s "Я нашла этот терминал в одной камере. Это автоматизированная доска почёта WOOHP. Когда преступники сбегают и их возвращают, выдаётся награда."

# game/status.rpy:4716
translate russia task7_de05a6c1:

    # s g1 "Here I'll show you."
    s g1 "Вот, я тебе покажу."

# game/status.rpy:4717
translate russia task7_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:4718
translate russia task7_eade8a27:

    # "Computer" "Booting up WOOHP Bounty Board..."
    "Компьютер" "Загрузка WOOHP Д.П. ..."

# game/status.rpy:4719
translate russia task7_c9bdc879:

    # "Computer" "Target: Maggie T.\nStatus: Captured."
    "Компьютер" "Цель: Мэгги Т.\nСтатус: Захвачена."

# game/status.rpy:4720
translate russia task7_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:4724
translate russia task7_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:4725
translate russia task7_a488b2d6:

    # y "Nice!"
    y "Ого!"

# game/status.rpy:4728
translate russia task7_ad01ead8:

    # "Bringing in gangleaders will not only help you liberate Beverly Hills, it will also offer you bounty rewards. Be sure to check in on the prisons to claim your bounty whenever you capture a new gangleader."
    "Поимка главарей не только поможет вам освободить Беверли-Хиллз, но и даст награду. Обязательно проверяйте камеры, чтобы получить свою награду, когда поймаете нового лидера."

# game/status.rpy:4730
translate russia task7_e57d30ca:

    # y "I wonder how much we'll get if we bring in your boyfriend."
    y "Интересно, сколько мы получим за поимку твоего парня."

# game/status.rpy:4732
translate russia task7_989d483b:

    # s g8 "He's not my boyfriend!"
    s g8 "Он не мой парень!"

# game/status.rpy:4733
translate russia task7_ec896389:

    # y "Suuuure~..."
    y "Кооонечно~..."

# game/status.rpy:4734
translate russia task7_f39cf812:

    # s g9 "Ugh!"
    s g9 "Угх!"

# game/status.rpy:4739
translate russia task7_53868d06:

    # "You decide to head to bed."
    "Вы решаете лечь спать."

# game/status.rpy:4748
translate russia task7_4467e4e6:

    # tim "................."
    tim "................."

# game/status.rpy:4750
translate russia task7_96619a20:

    # tim "Why did you call me here?"
    tim "Зачем ты позвал меня сюда?"

# game/status.rpy:4751
translate russia task7_ab54d55e:

    # "Mastermind" "You wanted proof that we're making the world a better place, right?"
    "Организатор" "Тебе нужны были доказательства того, что мы делаем мир лучше, верно?"

# game/status.rpy:4752
translate russia task7_c3197a4f:

    # "Mastermind" "Well here it is..."
    "Организатор" "Вот они..."

# game/status.rpy:4757
translate russia task7_3bd98ed6:

    # tim "............"
    tim "............"

# game/status.rpy:4758
translate russia task7_14df2383:

    # tim "Mr. Lewis..."
    tim "Мистер Льюис..."

# game/status.rpy:4760
translate russia task7_a59f7b05:

    # "???" "Tim... I should've known you played a part in all this..."
    "???" "Тим... Я должен был догадаться, что ты во всём этом замешан..."

# game/status.rpy:4761
translate russia task7_e004f03c:

    # "???" "My spies will stop you. Like they did all those other times!"
    "???" "Мои шпионы остановят тебя. Как всегда!"

# game/status.rpy:4762
translate russia task7_93a6a4cf:

    # tim "Not this time, Jerry. Your spies are under our control and soon they will help us wipe your pathetic organization from the map."
    tim "Не в этот раз, Джерри. Твои шпионы находятся под нашим контролем, и скоро они помогут нам стереть с лица земли вашу жалкую организацию."

# game/status.rpy:4764
translate russia task7_d78f29f3:

    # "Jerry" "Once a villain, always a villain..."
    "Джерри" "Однажды злодей, всегда злодей..."

# game/status.rpy:4765
translate russia task7_0ef20aee:

    # tim "Don't try to lecture me, Jerry. Don't pretend you haven't done anything shady to get WOOHP to where it is now."
    tim "Не пытайся читать мне нотации, Джерри. Не притворяйся, что ты не сделали ничего сомнительного, чтобы привести WOOHP к тому состоянию, в котором она сейчас находится."

# game/status.rpy:4766
translate russia task7_28d2677c:

    # tim "Your precious spies are living proof of that."
    tim "Твои драгоценные шпионки - живое тому доказательство."

# game/status.rpy:4767
translate russia task7_c8600e8b:

    # "Jerry" ".................."
    "Джерри" ".................."

# game/status.rpy:4768
translate russia task7_2a0c7f38:

    # tim "Where was he hiding?"
    tim "Где он прятался?"

# game/status.rpy:4769
translate russia task7_65616858:

    # "Mastermind" "He was in Beverly Hills. No doubt looking for his girls."
    "Организатор" "Он был в Беверли-Хиллз. Без сомнения искал своих шпионок."

# game/status.rpy:4770
translate russia task7_7368b777:

    # "Mastermind" "Luckily one of ours found him before anyone else did."
    "Организатор" "К счастью, один из наших нашёл его раньше других."

# game/status.rpy:4774
translate russia task7_323f4759:

    # "Mastermind" "Tim, I'd like to introduce you to S-..."
    "Организатор" "Тим, я хочу познакомитб тебе с С-..."

# game/status.rpy:4775
translate russia task7_cb43a51c:

    # tim "We've met."
    tim "Мы знакомы."

# game/status.rpy:4776
translate russia task7_52ed75d7:

    # tim "Figured it'd be you leading Drift Punk."
    tim "Я думал, что это ты будешь лидером Drift Punk."

# game/status.rpy:4777
translate russia task7_1199a8da:

    # "???" "....................................................................."
    "???" "....................................................................."

# game/status.rpy:4792
translate russia task7_4b5c48bf:

    # s "[playerName]!"
    s "[playerName]!"

# game/status.rpy:4793
translate russia task7_c1843393:

    # c "Terrible news!"
    c "Плохие новости!"

# game/status.rpy:4794
translate russia task7_04b1ad15:

    # a "Poor Jerry!"
    a "Бедный Джерри!"

# game/status.rpy:4795
translate russia task7_9ee5e11b:

    # y "What what what?!"
    y "Что что что?!"

# game/status.rpy:4800
translate russia task7_2708403b:

    # c "Look at today's most viral video!"
    c "Посмотри самое вирусное видео сегодняшнего дня!"

# game/status.rpy:4801
translate russia task7_a5a7b777:

    # y "{i}'Old British man tortured violently.'{/i}"
    y "{i}'Старика-англичанина жестоко пытают.'{/i}"

# game/status.rpy:4802
translate russia task7_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:4803
translate russia task7_cb3c8129_1:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:4811
translate russia task7_4fe24df3:

    # "Agent" "I really don't want to do this Mr. Lewis, but you leave me no choice..."
    "Агент" "Я действительно не хочу этого делать, мистер Льюис, но вы не оставляете мне выбора..."

# game/status.rpy:4812
translate russia task7_627f3865:

    # jerry "You brute! You monster!"
    jerry "Ты зверь! Монстр!"

# game/status.rpy:4813
translate russia task7_64d9e77d:

    # "Agent" "Last chance, Jerry...."
    "Агент" "Последний шанс, Джерри..."

# game/status.rpy:4814
translate russia task7_a2f29e6c:

    # jerry "No matter how much you torture me, I will NEVER talk!"
    jerry "Как бы ты меня не мучил, я НИЧЕГО не скажу!"

# game/status.rpy:4815
translate russia task7_fbae0c35:

    # "Agent" "Let's see how talkative you are after this...."
    "Агент" "Давайте посмотрим как долго вы будете молчать после этого..."

# game/status.rpy:4819
translate russia task7_de765744:

    # "Agent" "Repeat after me, it's spelled HONOR, not HONOUR."
    "Агент" "Повторяйте за мной, пишется ЧЕСТ, а не ЧЕСТЬ."

# game/status.rpy:4821
translate russia task7_5681eade:

    # jerry "NO YOU MADMAN!"
    jerry "НЕТ, ТЫ СУМАСШЕДШИЙ!"

# game/status.rpy:4822
translate russia task7_22f17520:

    # "Agent" "And it was fully in the colonies right to go independant."
    "Агент" "И у колоний было полонное право стать независимы."

# game/status.rpy:4823
translate russia task7_2e7a6ad6:

    # jerry "NO! NO I CAN'T TAKE IT ANYMORE!"
    jerry "НЕТ! НЕТ, Я БОЛЬШЕ НЕ ВЫНЕСУ!"

# game/status.rpy:4824
translate russia task7_b5012680:

    # "Agent" "Ever been to Boston? I hear the harbor still smells like tea."
    "Агент" "Вы когда-нибудь были в Бостоне? Я слышал, что гавань всё ещё пахнет чаем."

# game/status.rpy:4825
translate russia task7_8b16ec8f:

    # jerry "{b}*Cries*{/b}"
    jerry "{b}*Плачет*{/b}"

# game/status.rpy:4826
translate russia task7_a12d25c8:

    # "Agent" "And up next we're going to discuss the correct pronunciation of aluminium!"
    "Агент" "А теперь давай обсудим правильное произношение алюминия!"

# game/status.rpy:4828
translate russia task7_2630a435:

    # jerry "AAARGGHHH!"
    jerry "АААРГГХХХ!"

# game/status.rpy:4839
translate russia task7_6e4ecfcc:

    # y "........................."
    y "........................."

# game/status.rpy:4840
translate russia task7_e0ffc68b:

    # y "That poor British man..."
    y "Этот бедный британец..."

# game/status.rpy:4841
translate russia task7_047a9999:

    # a y28 "We have to save him!"
    a y28 "Мы должны спасти его!"

# game/status.rpy:4842
translate russia task7_2c58a4b8:

    # c "I recognise the background. They're keeping him at his office."
    c "Я узнаю фон. Они держат его в его офисе."

# game/status.rpy:4843
translate russia task7_fb35e20b:

    # y "His office?"
    y "В его оффисе?"

# game/status.rpy:4844
translate russia task7_ec2530b0:

    # s g31 "Yeah.. at the top of WOOHP HQ..."
    s g31 "Да.. на вершине штаб-квартиры WOOHP..."

# game/status.rpy:4845
translate russia task7_9925eba0:

    # a y4 "Oh no Jerry..."
    a y4 "Оо нет, Джерри..."

# game/status.rpy:4846
translate russia task7_e3589fd6:

    # c "The WOOHP HQ is too heavily guarded right now. We'll have to liberate the city first."
    c "Штаб-квартира WOOHP сейчас слишком сильно охраняется. Сначала нам придется освободить город."

# game/status.rpy:4847
translate russia task7_46e194cf:

    # c "Once we've done that, we can lead one final assault and get WOOHP back."
    c "Как только мы это сделаем, нам понадобиться всего лишь одна атака, чтобы вернуть WOOHP."

# game/status.rpy:4848
translate russia task7_bf55f9f7:

    # y "Sounds like a plan. Liberate Beverly Hills and then get your boss out."
    y "Звучит как план. Освободить Беверли-Хиллз, а затем спасти вашего босса."

# game/status.rpy:4851
translate russia task7_09dcad5f:

    # s g11 "...................."
    s g11 "...................."

# game/status.rpy:4853
translate russia task7_1c33e0ec:

    # s g1 "I agree!"
    s g1 "Я согласна!"

# game/status.rpy:4857
translate russia task7_05cdf1ce:

    # c r11 "..................."
    c r11 "..................."

# game/status.rpy:4859
translate russia task7_fa37acc4:

    # c r1 "Let's do it!"
    c r1 "Давайте сделаем это!"

# game/status.rpy:4863
translate russia task7_fb3c5279:

    # a y11 ".................."
    a y11 ".................."

# game/status.rpy:4865
translate russia task7_05835e9f:

    # a y1 "Yeah!"
    a y1 "Да!"

# game/status.rpy:4874
translate russia task7_e59c8083:

    # "Your spies' mood has improved!"
    "Настроение ваших шпионок улучшилось!"

# game/status.rpy:4897
translate russia task8_57a7b4f5:

    # "{b}*Mumble* *Mumble*{/b}"
    "{b}*Бормотание* *Бормотание*{/b}"

# game/status.rpy:4898
translate russia task8_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:4903
translate russia task8_3c6cc100:

    # "Agent 1" "What a lovely day we're having! Makes me want to do crime!"
    "Агент 1" "Какой прекрасный день! Так и хочется совершить преступление!"

# game/status.rpy:4904
translate russia task8_301ff0f8:

    # "Agent 2" "I don't know.. I haven't been feeling like doing crime lately.."
    "Агент 2" "Не знаю. В последнее время мне не хочется совершать преступления."

# game/status.rpy:4905
translate russia task8_08fbf999:

    # "Agent 1" "Uh oh! Looks like somebody has a case of the Mondays!"
    "Агент 1" "Ой-ой! Похоже, у кого-то сегодня плохое настроение!"

# game/status.rpy:4906
translate russia task8_63c62b4d:

    # "Agent 1" "But you're in luck! They're bringing out a software update which boosts the nanobot corruption!"
    "Агент 1" "Но тебе повезло! Сегодня выпускают обновление программного обеспечения, которое усиливает контроль наноботов!"

# game/status.rpy:4907
translate russia task8_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:4908
translate russia task8_e18f0180:

    # "Agent 2" "Oh boy! I can't wait to feel fully energized and ready to rob a bank again!"
    "Агент 2" "Ого чувак! Не могу дождаться, чтобы почувствовать себя полностью заряженным и готовым снова грабить банки!"

# game/status.rpy:4909
translate russia task8_2bd3233e:

    # y "(A software upgrade...? I better tell the gir-...)"
    y "(Обновление программного обеспечения...? Надо сообщить дев-...)"

# game/status.rpy:4914
translate russia task8_6b438f9e:

    # "Agent 2" "Hello there fellow agent! Are you excited about the new nanobot upgrade?"
    "Агент 2" "Здравствуйте, товарищ агент! Вы в восторге от нового обновления наноботов?"

# game/status.rpy:4915
translate russia task8_ee99fffe:

    # y "Er..."
    y "Эм..."

# game/status.rpy:4918
translate russia task8_79968776:

    # y "I sure am! Ready to live my life as a mindless drone without having to take responsibilities for my actions!"
    y "Думаю что да! Я готов прожить свою жизнь как бессмысленный дрон, не беря на себя ответственность за свои действия!"

# game/status.rpy:4919
translate russia task8_05d46810:

    # "Agent 1" "See? He gets it!"
    "Агент 1" "Видешь? Он это понимает!"

# game/status.rpy:4920
translate russia task8_2efdeb46:

    # "Agent 2" "I remember the days where my actions had consequences. Those were dark times indeed!"
    "Агент 2" "Помню дни, когда мои действия имели последствия. Это были действительно тёмные времена!"

# game/status.rpy:4921
translate russia task8_78e1f181:

    # "Agent 2" "Just the thought of it makes me want to destroy somebody elses property!"
    "Агент 2" "Одна только мысль об этом заставляет меня хотеть уничтожить чужую собственность!"

# game/status.rpy:4923
translate russia task8_c356a099:

    # y "Are you crazy?! Of course I'm not ready! Has this update even been tested?!"
    y "Ты ненормальный?! Конечно, я не готов! Было ли это обновление вообще протестировано?!"

# game/status.rpy:4924
translate russia task8_a69f996e:

    # "Agent 1" "Don't worry fellow co-worker. I understand if you're afraid of the new, but there's nothing to worry about!"
    "Агент 1" "Не волнуйтесь, коллеги. Я понимаю вашу боязнь нового, но беспокоиться не о чем!"

# game/status.rpy:4925
translate russia task8_3c6e3866:

    # "Agent 1" "Best case scenario we'll be rich and living the good life soon! Bad case scenario, we'd all be dead!"
    "Агент 1" "В лучшем случае в ближайшее время мы будем богаты и будем жить хорошей жизнью! В плохом случае мы все были бы мертвы!"

# game/status.rpy:4926
translate russia task8_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:4927
translate russia task8_0400692f:

    # y "And you don't see a problem with th-...?"
    y "И вас не смущает э-...?"

# game/status.rpy:4928
translate russia task8_06e26c25:

    # "Agent 1" "It's a win/win situation!"
    "Агент 1" "Это беспроигрышная ситуация!"

# game/status.rpy:4930
translate russia task8_82784a8f:

    # y "It's all binary, right? Aren't you afraid the 1's and 0's begin to whisper in your ear?"
    y "Тут всё двоично, верно? Вы не боитесь, что вам на ухо начнут шептать 1 и 0?"

# game/status.rpy:4931
translate russia task8_53a337c9:

    # "Agent 1" "Wow! {w}That is probably the craziest thing I've ever heard!"
    "Агент 1" "Ого! {w}Это, наверное, самая сумасшедшая вещь, которую я когда-либо слышал!"

# game/status.rpy:4932
translate russia task8_5fa068f4:

    # "Agent 2" "Don't worry co-worker! The data cannot hurt you!"
    "Агент 2" "Не волнуйтесь, коллега! Обновление не может вам навредить!"

# game/status.rpy:4933
translate russia task8_73890587:

    # "Agent 1" "Unless it glitches out and leads us all to our inevitable doom!"
    "Агент 1" "Если только оно не выйдет из строя и не приведёт всех нас к неизбежной гибели!"

# game/status.rpy:4934
translate russia task8_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:4935
translate russia task8_5d409ec9:

    # "Agent 1" "Anyways, we should get going. Those jewelers aren't going to rob themselves!"
    "Агент 1" "В любом случае нам пора идти. Ювелирки сами себя не ограбят!"

# game/status.rpy:4940
translate russia task8_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:4941
translate russia task8_5242344a:

    # y "(A software update... Best discuss it with the girls once I get the chance.)"
    y "(Обновление программного обеспечения... Лучше обсудить это с девочками, как только у меня появится возможность.)"

# game/status.rpy:4942
translate russia task8_fd484bd1:

    # "The sun is setting and you quickly make your way back to the base."
    "Солнце садится, и вы быстро возвращаетесь на базу."

# game/status.rpy:4955
translate russia task8_8a95e8a6:

    # y "All right girls. We've got a problem on our hands."
    y "Хорошо, девочки. У нас новые проблемы."

# game/status.rpy:4956
translate russia task8_2efb4e5c:

    # s g16 "A problem?"
    s g16 "Проблемы?"

# game/status.rpy:4957
translate russia task8_53d0ff98:

    # c r18 "What is it this time?"
    c r18 "Что на этот раз?"

# game/status.rpy:4958
translate russia task8_8de27394:

    # y "The nanobots are getting a software update..."
    y "Наноботы получат обновление программного обеспечения..."

# game/status.rpy:4959
translate russia task8_c85ee6a3:

    # y "Meaning you three might get controlled again soon."
    y "Значит, скоро вас троих снова будут контролировать."

# game/status.rpy:4960
translate russia task8_957696f7:

    # a y31 "Mr. Wiggles will protect me!"
    a y31 "Мистер Вигглс защитит меня!"

# game/status.rpy:4961
translate russia task8_6fdd478e:

    # y "Well obviously we can rely on Mr. Wiggles."
    y "Думаю мы можем положиться на Мистера Вигглса."

# game/status.rpy:4962
translate russia task8_6a5ecf18:

    # s g37 "........................."
    s g37 "........................."

# game/status.rpy:4963
translate russia task8_65f9469b:

    # c r35 "........................."
    c r35 "........................."

# game/status.rpy:4964
translate russia task8_949cf1d2:

    # y "Who's Mr. Wiggles?"
    y "Кто такой Мистер Вигглс?"

# game/status.rpy:4965
translate russia task8_58202aca:

    # c r10 "Her dildo."
    c r10 "Её дилдо."

# game/status.rpy:4966
translate russia task8_4af5053a:

    # y "Oh..."
    y "Ох..."

# game/status.rpy:4967
translate russia task8_78ef1c19:

    # y "Where was I going with this?"
    y "К чему это я?"

# game/status.rpy:4968
translate russia task8_f990de02:

    # y "Oh right!"
    y "Ах точно!"

# game/status.rpy:4976
translate russia task8_a3486820:

    # "You slap Alex's dildo out her hands!"
    "Вы выбиваете фаллоимитатор Алекс из её рук!"

# game/status.rpy:4977
translate russia task8_19124c76:

    # a y28 "Mr. Wiggles!"
    a y28 "Мистер Вигглс!"

# game/status.rpy:4980
translate russia task8_d8ced868:

    # y "It means we might have to up your nanobot suppression."
    y "Это значит, что нам придётся усилить подавление наноботов."

# game/status.rpy:4981
translate russia task8_addb5fa8:

    # s g18 "Up it...?"
    s g18 "Усилить...?"

# game/status.rpy:4982
translate russia task8_87b838d7:

    # c r16 "How?"
    c r16 "Как?"

# game/status.rpy:4983
translate russia task8_c8f0fe35:

    # y "By switching from masturbation to foreplay."
    y "Перейдём от мастурбации к прелюдиям."

# game/status.rpy:4985
translate russia task8_deecb109:

    # s g29 "W-what?!"
    s g29 "Ч-что?!"

# game/status.rpy:4986
translate russia task8_ff14811f:

    # c r30 "Woah now... easy there buddy! What are you saying?!"
    c r30 "Воу-воу... полегче, приятель! О чём ты говоришь?!"

# game/status.rpy:4987
translate russia task8_5bbc4e14:

    # y "I have two options available."
    y "У меня есть два варианта."

# game/status.rpy:4988
translate russia task8_aa136b18:

    # y "1. Foreplay."
    y "1. Прелюдия."

# game/status.rpy:4989
translate russia task8_c998dde2:

    # c r29 "And the other...?"
    c r29 "А другой...?"

# game/status.rpy:4990
translate russia task8_14d0b776:

    # y "You're going to show off your nudity to others."
    y "Вы демонстрируете своё голое тело другим."

# game/status.rpy:4991
translate russia task8_2e095c3f:

    # c r30 "T-to others...?!"
    c r30 "Д-другим...?!"

# game/status.rpy:4992
translate russia task8_15fcd509:

    # s g9 "You've got to be joking, [playerName]!"
    s g9 "Ты, должно быть, шутишь [playerName]!"

# game/status.rpy:4993
translate russia task8_0d9b0a50:

    # s "Having you present as we're naked is already bad enough! We're not also going to invite others!"
    s "То, что ты смотришь на нас голых уже достаточно плохо! Но мы не собираемся приглашать других!"

# game/status.rpy:4994
translate russia task8_d655d49e:

    # y "Okay... Let's put it into perspective."
    y "Хорошо... Давайте рассмотрим это в перспективе."

# game/status.rpy:4995
translate russia task8_1c6af53a:

    # s g39 "............."
    s g39 "............."

# game/status.rpy:4996
translate russia task8_48e620cf:

    # c r12 "{b}*Scoffs*{/b}"
    c r12 "{b}*Усмехается*{/b}"

# game/status.rpy:4997
translate russia task8_aa3d8216:

    # y "Either you stop being such stuck up prudes..."
    y "Либо вы перестанете быть такими заносчивыми ханжами..."

# game/status.rpy:4998
translate russia task8_24529915:

    # s g7 "H-hey...!"
    s g7 "Э-эй...!"

# game/status.rpy:4999
translate russia task8_9477c513:

    # y "Or you're going to spend your life like mindless zombies. {w}Who knows what the mastermind has planned for you after his plan succeeds?"
    y "Или вы проведёте свою жизнь как бездумные зомби. {w}Кто знает, что задумал для вас Организатор после того, как его план удастся?"

# game/status.rpy:5000
translate russia task8_01b6d130:

    # c r11 "......................"
    c r11 "......................"

# game/status.rpy:5002
translate russia task8_39dfa5a3:

    # s g40 "I still think it's ridiculous. I haven't noticed anything out of the ordinary. I bet you're just trying to take advantage of us!"
    s g40 "Я всё ещё думаю, что это смешно. Я не заметила ничего необычного. Держу пари, ты просто пытаешься воспользоваться нами!"

# game/status.rpy:5003
translate russia task8_1991f4a3:

    # s "Right girls?"
    s "Ведь так девочки?"

# game/status.rpy:5004
translate russia task8_d41d6bef:

    # s g51 "Girls...?"
    s g51 "Девочки...?"

# game/status.rpy:5006
translate russia task8_83829bf2:

    # c r29 "Er... Sam, you might want to get over here...!"
    c r29 "Эм... Сэм, тебе лучше подойти сюда...!"

# game/status.rpy:5011
translate russia task8_11ada324:

    # s "Alex...?"
    s "Алекс...?"

# game/status.rpy:5012
translate russia task8_2b26034f:

    # a "{b}{size=+8}I WANT.... BLOOD!{w} I WANT DESTRUCTION!{w} I WANT..... TO KILL!"
    a "{b}{size=+8}Я ЖАЖДУ... КРОВИ!{w} Я ЖАЖДУ РАЗРУШЕНИЙ!{w} Я ЖАЖДУ... СМЕРТЕЙ!"

# game/status.rpy:5013
translate russia task8_58cf3be0:

    # y "Okay calm down, Alex. I'm sorry about Mr. Wigg-...."
    y "Ладно, успокойся, Алекс. Мне жаль Мистера Вигг-..."

# game/status.rpy:5017
translate russia task8_a9e5d118:

    # "Alex attacked you!"
    "Алекс напала на вас!"

# game/status.rpy:5018
translate russia task8_b9359d7b:

    # y "Argh! Not again!"
    y "Аргх! Только не снова!"

# game/status.rpy:5024
translate russia task8_d23448be:

    # y "A LITTLE HELP HERE, PLEASE!"
    y "Тут не помешала бы помощь, пожалуйста!"

# game/status.rpy:5028
translate russia task8_cc82c67b:

    # "The two spies rush to your aid, but even with their combined strength they're no match for the crazed Alex!"
    "Две шпионки подбегают к вам, но даже с их помощью вы не можете справиться с обезумевшей Алекс!"

# game/status.rpy:5031
translate russia task8_243c52d3:

    # y "Ow!"
    y "Оу!"

# game/status.rpy:5032
translate russia task8_1f037b92:

    # y "Alex, listen to me!"
    y "Алекс, послушай меня!"

# game/status.rpy:5033
translate russia task8_187a3ac4:

    # y "Do you know what a buttjob is?!"
    y "Ты знаешь что такое глютеальный секс между ягодиц?!"

# game/status.rpy:5034
translate russia task8_59ef63d8:

    # a "..................."
    a "..................."

# game/status.rpy:5035
translate russia task8_7690bf10:

    # a "A what...?"
    a "Глюте- что...?"

# game/status.rpy:5036
translate russia task8_0f97a543:

    # "Seeing an opportunity, all three of you jump on the spy in yellow."
    "Заметив возможность, вы втроём прыгаете на шпионку в жёлтом."

# game/status.rpy:5037
translate russia task8_d0e08271:

    # "Quickly looking up the definition of a buttjob you show it to her."
    "Быстро найдя определение понятия \"глютеальный секс\", вы показываете его ей."

# game/status.rpy:5038
translate russia task8_e04f5c15:

    # y "Look! This is a buttjob! And it's what you'll be doing!"
    y "Смотри! Это - глютеальный секс! Это то, что вы будете делать!"

# game/status.rpy:5043
translate russia task8_413c3aa8:

    # a "I... I will?"
    a "Я... я буду?"

# game/status.rpy:5044
translate russia task8_42956ad2:

    # s g38 "Yes you will, Alex!"
    s g38 "Да, Алекс!"

# game/status.rpy:5046
translate russia task8_58ce2560:

    # a y29 "{b}*Blinks*{/b}"
    a y29 "{b}*Моргает*{/b}"

# game/status.rpy:5047
translate russia task8_5fc043cb:

    # a "Oh hey guys. What did I miss?"
    a "О, привет ребята. Что я пропустила?"

# game/status.rpy:5048
translate russia task8_9a1259b1:

    # s g16 "Are you back to normal again...?"
    s g16 "Ты снова вернулась к норме...?"

# game/status.rpy:5049
translate russia task8_dc6457b5:

    # c r38 "Alex... please go to your cell..."
    c r38 "Алекс... пожалуйста, вернись в свою камеру..."

# game/status.rpy:5050
translate russia task8_13d5181a:

    # a y29 "What did I do...?"
    a y29 "Что я сделала...?"

# game/status.rpy:5051
translate russia task8_150e3d33:

    # a y5 "{b}*Pouts*{/b}"
    a y5 "{b}*Дуется*{/b}"

# game/status.rpy:5053
translate russia task8_a3d16fad:

    # "Like a scolded child, Alex walks off to her cell."
    "Как отчитанный ребёнок, Алекс уходит в свою камеру."

# game/status.rpy:5057
translate russia task8_28966138:

    # c r32 "...................."
    c r32 "...................."

# game/status.rpy:5058
translate russia task8_09dcad5f:

    # s g11 "...................."
    s g11 "...................."

# game/status.rpy:5059
translate russia task8_fd395fcc:

    # y "So... foreplay. Or do you want to turn into {i}'that'{/i}?"
    y "Так что... прелюдия. Или вы хотите превратиться в {i}'это'{/i}?"

# game/status.rpy:5060
translate russia task8_77bab620:

    # s "................."
    s "................."

# game/status.rpy:5061
translate russia task8_ef952794:

    # c r38 "No...."
    c r38 "Нет..."

# game/status.rpy:5062
translate russia task8_72e45360:

    # y "Clover, we're going to practise your boobjobs."
    y "Кловер, мы будем практиковать пайзури."

# game/status.rpy:5063
translate russia task8_1eb53463:

    # c r28 "B-but... {w}{b}*Sighs*{/b} Fine..."
    c r28 "Н-но... {w}{b}*Вздох*{/b} Ладно..."

# game/status.rpy:5064
translate russia task8_0933fcfd:

    # y "Sam, you're gonna be studying up on thighjobs."
    y "Сэм, тебе придётся изучить тему \"глютеального секс меж бёдер\"."

# game/status.rpy:5065
translate russia task8_f6ef2630:

    # s g31 "What even is that...?"
    s g31 "Что это вообще такое...?"

# game/status.rpy:5066
translate russia task8_5928150b:

    # y "Look it up."
    y "Посмотри."

# game/status.rpy:5067
translate russia task8_08401bb3:

    # "Sam grabs her phone and after a few moments finds the description."
    "Сэм берёт свой телефон и через несколько мгновений находит описание."

# game/status.rpy:5068
translate russia task8_2dfbc557:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:5069
translate russia task8_2d721376:

    # s "N-no! I'm not gonna do tha-...."
    s "Н-нет! Я не буду этого делать-..."

# game/status.rpy:5075
translate russia task8_55fd908f:

    # c "Give it a rest, Sam. Yes you're doing that."
    c "Успокойся, Сэм. Тебе прийдётся это делать."

# game/status.rpy:5076
translate russia task8_714a6083:

    # s g53 "N-no, you don't understand! He will actually be rubbing up against my-...!"
    s g53 "Н-нет, ты не понимаешь! Он будет тереться о мою-...!"

# game/status.rpy:5077
translate russia task8_96b2fee9:

    # c "Yes he will. Get over it."
    c "Да, будет. Смирись с этим."

# game/status.rpy:5078
translate russia task8_0ef8eef7:

    # c r32 "Is that all, [playerName]?"
    c r32 "Это всё, [playerName]?"

# game/status.rpy:5079
translate russia task8_d9d056ea:

    # y "No. I'm still pondering to show you off naked to strangers."
    y "Нет. Я всё ещё раздумываю над тем, чтобы показать тебя голой незнакомым людям."

# game/status.rpy:5080
translate russia task8_226d0a71:

    # s g28 "{b}*Sputters*{/b} B-but...!"
    s g28 "{b}*Фыркает*{/b} Н-но...!"

# game/status.rpy:5081
translate russia task8_903273b0:

    # c "Go on."
    c "Продолжай."

# game/status.rpy:5082
translate russia task8_a048c1e6:

    # y "Instead of breaking captured Agent nanobot control with dirty magazines, I might send you to expose yourself to them to help speed up the process."
    y "Вместо того чтобы ломать контроль наноботов над Агентами с помощью грязных журналов, я могу послать вас, чтобы ускорить процесс."

# game/status.rpy:5083
translate russia task8_50afc7c0:

    # s g40 "Clover...! This is an outrage!"
    s g40 "Кловер...! Это безобразие!"

# game/status.rpy:5084
translate russia task8_22ea9bf0:

    # c "It sure is. I'm heading to my cell. We'll begin training tomorrow."
    c "Это действительно так. Я пошла в свою комнату. Завтра мы начнём тренироваться."

# game/status.rpy:5085
translate russia task8_e3ba6375:

    # s g28 "BUT...!"
    s g28 "НО...!"

# game/status.rpy:5086
translate russia task8_fe1b1856:

    # c r54 "Good night, Sam."
    c r54 "Спокойной ночи, Сэм."

# game/status.rpy:5089
translate russia task8_71172bcd:

    # s g39 "............................"
    s g39 "............................"

# game/status.rpy:5090
translate russia task8_2ecff712:

    # s g19 "{size=-8}But....{/size}"
    s g19 "{size=-8}Но...{/size}"

# game/status.rpy:5091
translate russia task8_ed91a037:

    # y "Are you really so against this?"
    y "Ты действительно на столько против?"

# game/status.rpy:5092
translate russia task8_06b67064:

    # s g43 "........"
    s g43 "........"

# game/status.rpy:5093
translate russia task8_9f34d859:

    # s "I'm just kinda inexperienced with sex..."
    s "Я просто немного неопытна в сексе..."

# game/status.rpy:5094
translate russia task8_2390c753:

    # s g15 "But... of course I don't want to end up like Alex either..."
    s g15 "Но... конечно, я также не хочу заканчивать как Алекс..."

# game/status.rpy:5095
translate russia task8_cbf90e59:

    # "Sam sighs in defeat."
    "Сэм вздыхает."

# game/status.rpy:5096
translate russia task8_fa1f841e:

    # s "Thighjob it is...{w} Let's start tomorrow..."
    s "Секс меж бёдер это...{w} Начнём завтра..."

# game/status.rpy:5101
translate russia task8_3d5a2da8:

    # "Your spies have taken the next step of their training! In order to keep their nanobots controlled you've suggested foreplay to them."
    "Ваши шпионки достигли очередного прогресса в своей тренировке! Чтобы держать своих наноботов под контролем, вы предложили им прелюдия."

# game/status.rpy:5102
translate russia task8_bd3fcff3:

    # "This introduces a new way to suppress their nanobots. On top of that, you can now send your girls to break captured Agent's nanobots instead of using magazines."
    "Это добавляет новый способ подавления наноботов. Кроме того, теперь вы можете отправлять своих девушек разбивать контроль наноботов над Агентами вместо того, чтобы использовать журналы."

# game/status.rpy:5128
translate russia task9_67cc71cd:

    # y "Clover, go visit Drift Punk again today."
    y "Кловер, отправляйся сегодня снова к Дрифт Панкам."

# game/status.rpy:5129
translate russia task9_580afc40:

    # cM "Nah, no need. They said they're testing something new today and don't need any help."
    cM "Нет, не нужно. Они сказали, что тестируют что-то новое сегодня и не нуждаются в какой-либо помощи."

# game/status.rpy:5130
translate russia task9_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:5131
translate russia task9_409dfc4b:

    # y "Then I definitely want you to go. What's this new thing they're testing?"
    y "Тогда я определенно хочу, чтобы ты поехала. Что это за новая вещь, которую они тестируют?"

# game/status.rpy:5132
translate russia task9_7ebae2c4:

    # cM "Er... Actually...{w} I didn't ask... I don't know."
    cM "Эм... Нуу...{w} Я не спрашивала... Я не знаю."

# game/status.rpy:5133
translate russia task9_c9f2e7ba:

    # y "Then we're going to find out what they're up to."
    y "Тогда нам нужно узнать чем они занимаются."

# game/status.rpy:5134
translate russia task9_91814a33:

    # y "Sneak into their base and report back on what you find."
    y "Проберись на их базу и сообщи о том, что найдёшь."

# game/status.rpy:5135
translate russia task9_4de9f1e2:

    # cM "Aw... so much for my day off...."
    cM "Оох... слишком много для моего выходного..."

# game/status.rpy:5146
translate russia task9_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:5147
translate russia task9_d4f50f9a:

    # y "No Clover...?"
    y "Где Кловер...?"

# game/status.rpy:5148
translate russia task9_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:5149
translate russia task9_215ea3ee:

    # y "....?"
    y "....?"

# game/status.rpy:5151
translate russia task9_7c7541dd:

    # "???" "010000100110111101101111"
    "???" "010000100110111101101111"

# game/status.rpy:5152
translate russia task9_36546dc6:

    # y "What the...?!"
    y "Что за...?!"

# game/status.rpy:5153
translate russia task9_83b20568:

    # cM "Scared ya?"
    cM "Испугался?"

# game/status.rpy:5154
translate russia task9_e50ff359:

    # y "Clover, was that binary? Where are you?"
    y "Клевер, это был бинарный код? Где ты?"

# game/status.rpy:5155
translate russia task9_8db70f4b:

    # cM "Oh you're not gonna believe this..."
    cM "Оо, ты не поверишь..."

# game/status.rpy:5156
translate russia task9_ad4e4c57:

    # cM "I am inside your phone."
    cM "Я в твоём телефоне."

# game/status.rpy:5157
translate russia task9_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:5158
translate russia task9_54d09116:

    # y "What....?"
    y "Что...?"

# game/status.rpy:5159
translate russia task9_2d5e4af3:

    # cM "Turn on your spy camera screen."
    cM "Включи экран шпионской камеры."

# game/status.rpy:5160
translate russia task9_dab10850:

    # y "Why~...?"
    y "Зачем~...?"

# game/status.rpy:5161
translate russia task9_2a4c73e7:

    # cM "Oh you'll see~...."
    cM "Увидишь~..."

# game/status.rpy:5164
translate russia task9_d2644b52:

    # "{b}*Beep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:5168
translate russia task9_e6cee2c4:

    # y "Woah...!"
    y "Воа...!"

# game/status.rpy:5173
translate russia task9_c95e80fa:

    # c "Welcome to... {w}this... place."
    c "Добро пожаловать в... {w}это... место."

# game/status.rpy:5174
translate russia task9_5cf1da72:

    # y "Where are you?"
    y "Где ты?"

# game/status.rpy:5175
translate russia task9_3dd6c8f4:

    # c r16 "Remember that new thing Drift Punk was testing? This is it."
    c r16 "Помнишь ту новую вещь, которую тестировали Дрифт Панки? Вот она."

# game/status.rpy:5176
translate russia task9_8cdba66b:

    # c "It's some kind of cyber world."
    c "Это какой-то кибермир."

# game/status.rpy:5177
translate russia task9_6a84c41d:

    # y "A what...?"
    y "Кибер- что...?"

# game/status.rpy:5178
translate russia task9_6c3eb847:

    # c r18 "A completely digital world. Members digitize themselves and are transported here."
    c r18 "Полностью цифровой мир. Участников оцифровывают и перевозят сюда."

# game/status.rpy:5179
translate russia task9_bcce5e02:

    # y "Sounds dangerous. Get out of there and report back."
    y "Звучит опасно. Возврощайся и доложи."

# game/status.rpy:5180
translate russia task9_bc57a619:

    # c "No not yet! The Punks are showing off this technology to one of their higher ups and I just want to get a good look at them."
    c "Нет, ещё нет! Панки демонстрируют эту технологию одному из своих высших руководителей, а я хочу взглянуть на них."

# game/status.rpy:5181
translate russia task9_8f278f4a:

    # y "Sounds like a good way to get yourself captured, but all right."
    y "Звучит как хороший способ попасть в плен, но ладно."

# game/status.rpy:5182
translate russia task9_1de9325e:

    # c "Clover out."
    c "Кловер, отбой."

# game/status.rpy:5204
translate russia task9_a24ba8bd:

    # y "Clover! You're back!"
    y "Кловер! Ты вернулась!"

# game/status.rpy:5205
translate russia task9_dfc80ac3:

    # c "Yup, and I got a bunch of new intel!"
    c "Ага, и у меня куча новой информации!"

# game/status.rpy:5209
translate russia task9_c9cb82f2:

    # c r14 "So.. Where to begin..."
    c r14 "Так.. С чего бы начать..."

# game/status.rpy:5210
translate russia task9_dc949ea3:

    # c "Drift Punk have set up a high tech cyberworld. They use it for hacking in people's hardware and to lie low when they've got too much heat on them."
    c "Дрифт Паник создали высокотехнологичный кибермир. Они используют его для взлома устройств и в качестве укрытия, в случае погони и розыска."

# game/status.rpy:5211
translate russia task9_3e84e553:

    # c r31 "Not only that, but I saw the Drift Punk leader!"
    c r31 "А ещё я увидела лидера Дрифт Панков!"

# game/status.rpy:5212
translate russia task9_e09028d4:

    # c "I didn't recognize him because he was wearing a helmet though..."
    c "Мне не удалось распознать его личность, так как он был в шлеме..."

# game/status.rpy:5213
translate russia task9_c4d789c2:

    # y "We'll go after him later. So how was the digital world?"
    y "Мы займёмся им позже. А как там в цифровом мире?"

# game/status.rpy:5214
translate russia task9_64dd6c2c:

    # c r23 "Not gonna lie... It was pretty cool."
    c r23 "Не буду врать... Было очень круто."

# game/status.rpy:5215
translate russia task9_4f430a15:

    # c "Normally I am {i}'so'{/i} not into all this nerdy stuff, but I got to travel the internet!"
    c "Вообще-то я не люблю все эти гиковские штучки, но мне удалось попутешествовать по Интернету!"

# game/status.rpy:5216
translate russia task9_efde1774:

    # c r22 "I visited social media sites and went through people's private pictures~..."
    c r22 "Я зайдя в соцсети мне удалось посмотреть на личные фотографии людей~..."

# game/status.rpy:5217
translate russia task9_da51a6af:

    # y "Naughty. Found any good games while you were at it?"
    y "Ну ты даёшь. Нашла какие-нибудь хорошие игры?"

# game/status.rpy:5218
translate russia task9_570137f1:

    # c r38 "Tsk... games are for boys. I didn't look at anything like that."
    c r38 "Пфф... игры для мальчиков. Я не смотрела ни на что подобное."

# game/status.rpy:5219
translate russia task9_87dafd08:

    # c "........................"
    c "........................"

# game/status.rpy:5220
translate russia task9_1d263ddd:

    # c r12 "Well there was this one farming game that looked kind of cute, but I didn't play it."
    c r12 "Была одна фермерская игра, которая выглядела довольно мило, но я в неё не играла."

# game/status.rpy:5221
translate russia task9_2f776814:

    # c "Anyways, that's my report. We can go on missions there now, and maybe dig up some dirt on Drift Punk."
    c "Короче, вот мой отчёт. Теперь мы можем отправляться туда на задания и может нам даже удастся что-то откопать"

# game/status.rpy:5225
translate russia task9_518ae813:

    # "{color=#ffeda6}Punk Web{/color} has now been unlocked for missions."
    "{color=#ffeda6}Панк-Вэб{/color} разблокирован для миссий."

# game/status.rpy:5227
translate russia task9_d54c45f2:

    # "New shopping options are available for Clover at the mall."
    "В торговом центре доступны новые варианты покупок для Кловер."

# game/status.rpy:5251
translate russia task10_d32e571a:

    # y "We should really focus on finding a way to free Alex before we do this."
    y "Мы должны сосредоточиться на поиске способа освобождения Алекс, прежде чем делать это."

# game/status.rpy:5257
translate russia task10_6ca6f066:

    # cM "I'm jacking in today."
    cM "Пойду покончу с этим."

# game/status.rpy:5258
translate russia task10_eefb328b:

    # y "Clover! Language!"
    y "Кловер! Следи за языком!"

# game/status.rpy:5259
translate russia task10_543c92c7:

    # cM "Jacking in.... not jacking off."
    cM "Покончу... а не кончу."

# game/status.rpy:5260
translate russia task10_4af5053a:

    # y "Oh..."
    y "Ох..."

# game/status.rpy:5261
translate russia task10_ac17dec1:

    # y "What?"
    y "Что?"

# game/status.rpy:5262
translate russia task10_2153c844:

    # cM "There were rumors of an important meeting in Punk Web today. I'm gonna do some snooping around."
    cM "Ходили слухи, что сегодня будет важная встреча в Панк-Вэбе. Я хочу как можно быстрее покончить с этим, сегодня выйдет новая серия моего любимого сериала."

# game/status.rpy:5263
translate russia task10_24161da2:

    # y "All right, be careful though."
    y "Хорошо, но будь осторожна."

# game/status.rpy:5264
translate russia task10_4d07ad4f:

    # cM "Of course!"
    cM "Конечно!"

# game/status.rpy:5276
translate russia task10_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:5279
translate russia task10_bfc4c03f:

    # y "Clover? Are you still in the cyber world?"
    y "Кловер? Ты всё ещё в кибермире?"

# game/status.rpy:5281
translate russia task10_19a333c5:

    # c "Shhh...."
    c "Тссс..."

# game/status.rpy:5282
translate russia task10_646dd032:

    # drag "I see everything is running and is optimized to perfection..."
    drag "Я вижу, всё оптимизировано и работает идеально..."

# game/status.rpy:5283
translate russia task10_26678c42:

    # c "I know that voice..."
    c "Я узнаю этот голос..."

# game/status.rpy:5284
translate russia task10_f9732930:

    # c r29 "!!!"
    c r29 "!!!"

# game/status.rpy:5285
translate russia task10_ed6b5b67:

    # c "Wait! That's Carla Wong...! Well that makes sense."
    c "Стоп! Это Карла Вонг...! Теперь всё встаёт на свои места."

# game/status.rpy:5286
translate russia task10_f0a14ea5:

    # y "Carla Wong?"
    y "Карла Вонг?"

# game/status.rpy:5287
translate russia task10_c83edc39:

    # c r31 "Hang on I'm coming ba-..."
    c r31 "Подожди, я возвра-..."

# game/status.rpy:5288
translate russia task10_fd22f1a3:

    # drag "Who's there...?!"
    drag "Кто там...?!"

# game/status.rpy:5289
translate russia task10_027e1fc5:

    # c r30 "Uh oh~... Clover out!"
    c r30 "Ой-ой~... Отбой!"

# game/status.rpy:5291
translate russia task10_5d66c9f2:

    # y "Clover?!"
    y "Кловер?!"

# game/status.rpy:5292
translate russia task10_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:5293
translate russia task10_a513268e:

    # y "She's gone..."
    y "Она ушла..."

# game/status.rpy:5299
translate russia task10_ea1a6b4b:

    # s "[playerName]? Was that Clover just now?"
    s "[playerName]? Это была Кловер?"

# game/status.rpy:5300
translate russia task10_a2db6948:

    # a "Is she in trouble?!"
    a "Она в беде?!"

# game/status.rpy:5301
translate russia task10_e519b21b:

    # y "Maybe. I'm sending you two on a special night time mission to get her back!"
    y "Возможно. Я отправлю вас двоих на специальную ночную миссию, чтобы вернуть её!"

# game/status.rpy:5303
translate russia task10_06ca9068:

    # s g31 "Okay, we won't fail!"
    s g31 "Хорошо, мы не подведём!"

# game/status.rpy:5338
translate russia task10_8e5ef10b:

    # s "(Clover!)"
    s "(Кловер!)"

# game/status.rpy:5340
translate russia task10_535693dd:

    # a "(Clover!)"
    a "(Кловер!)"

# game/status.rpy:5343
translate russia task10_8e5ef10b_1:

    # s "(Clover!)"
    s "(Кловер!)"

# game/status.rpy:5344
translate russia task10_68cd7ac0:

    # drag "Don't you know that curiosity killed the cat?"
    drag "Разве ты не знаешь что сделали с любопытной Варварой?"

# game/status.rpy:5345
translate russia task10_0e136922:

    # drag "Deal with her, I have an important meeting to attend to."
    drag "Разберитесь с ней, у меня важная встреча."

# game/status.rpy:5348
translate russia task10_373ba573:

    # y "Now is your chance!"
    y "Сейчас, это ваш шанс!"

# game/status.rpy:5350
translate russia task10_5c3d40b9:

    # s "On it!"
    s "Вперёд!"

# game/status.rpy:5352
translate russia task10_84260d3c:

    # a "Ready to strike!"
    a "Раскрошим им лица!"

# game/status.rpy:5384
translate russia task10_c366dd33:

    # a "Clover!"
    a "Кловер!"

# game/status.rpy:5385
translate russia task10_afe7a481:

    # s "She's hurt..."
    s "Она ранена..."

# game/status.rpy:5386
translate russia task10_38ea65c3:

    # y "Get out of there and take her back to the base."
    y "Убирайтесь оттуда и возвращайтесь на базу."

# game/status.rpy:5396
translate russia task10_9da101b9:

    # "The spies return the wounded Clover back to her cell."
    "Шпионки возвращают раненую Кловер в её камеру."

# game/status.rpy:5397
translate russia task10_dda84fee:

    # y "How is she?"
    y "Как она?"

# game/status.rpy:5401
translate russia task10_f9a9f6ee:

    # s "She took a nasty hit, but she'll be alright. She needs to rest for now. If you have any medkits, they'd be really good right now aswell."
    s "Её сильно досталось, но с ней всё будет в порядке. Сейчас ей нужно отдохнуть. Если у тебя есть аптечки, сейчас они были бы очень кстати."

# game/status.rpy:5406
translate russia task10_80bd3c11:

    # s "Thanks, that should help a little."
    s "Спасибо, это должно немного помочь."

# game/status.rpy:5408
translate russia task10_203ea696:

    # s "We really should have some on hand."
    s "Нам всегда стоит иметь парочку в запасе."

# game/status.rpy:5409
translate russia task10_09f736a6:

    # s ".................."
    s ".................."

# game/status.rpy:5410
translate russia task10_cc95394c:

    # s g32 "Carla Wong is dangerous. Last time we fought her, Alex nearly bit the bullet."
    s g32 "Карла Вонг опасна. Когда мы сражались с ней в прошлый раз, Алекс чуть не поймала пулю."

# game/status.rpy:5411
translate russia task10_f0027c0d:

    # a y30 "I got trapped inside one of her games..."
    a y30 "Я попала в ловушку одной из её игр..."

# game/status.rpy:5412
translate russia task10_395934c6:

    # a y29 "I'm afraid she might do the same with Punk Web."
    a y29 "Боюсь, что она может сделать то же самое в Панк-Вэбе."

# game/status.rpy:5413
translate russia task10_b3a3a9ff:

    # y "Taking her down should be a priority then. For now let's call it a day. You've all have earned your rest."
    y "Тогда её захват должен стать нашим приоритетом. На этом закончим. Вы все заслужили свой отдых."

# game/status.rpy:5414
translate russia task10_5e3dc5aa:

    # "The girls nod and you head to bed."
    "Девочки кивают, и вы отправляетесь в постель."

# game/status.rpy:5423
translate russia task10_7d3e8df8:

    # "The first Drift Punk lieutenant has shown herself and is available for capture in Punk Web. Set up a mission and have your spies capture her."
    "Первый лейтенант Дрифт Панков проявила себя и доступна для захвата в Панк-Вэбе. Подготовьте миссию и поручите своим шпионкам захватить её."

# game/status.rpy:5435
translate russia task10_2ce0d835:

    # y "Let's see what info WOOHP has on you...{w}Carla Wong, aka The Dragon Lady."
    y "Посмотрим, какая информация есть на тебя у WOOHP...{w}Карла Вонг, ака. Леди Дракон."

# game/status.rpy:5436
translate russia task10_92dd050c:

    # drag "......................."
    drag "......................."

# game/status.rpy:5437
translate russia task10_68fe3381:

    # y "Your file also says you're a massive nerd who's into video games."
    y "В твоём досье также говорится, что ты жёсткий гик, увлекающийся видеоиграми."

# game/status.rpy:5438
translate russia task10_7820535d:

    # drag "Hey!"
    drag "Эй!"

# game/status.rpy:5439
translate russia task10_cf954191:

    # y "So tell me Carla...{w} Do you like jewelry?"
    y "Расскажи мне, Карла...{w} Ты любишь украшения?"

# game/status.rpy:5440
translate russia task10_2ffa2e54:

    # drag "What?"
    drag "Что?"

# game/status.rpy:5442
translate russia task10_f4974dec:

    # c "What are you doing, [playerName]...?"
    c "Что ты делаешь, [playerName]...?"

# game/status.rpy:5443
translate russia task10_464cd0cb:

    # y "Hey Clover, feeling better?"
    y "Привет, Кловер, чувствуешь себя лучше?"

# game/status.rpy:5444
translate russia task10_684fd79e:

    # y "I brough a ton of gifts to raise her affection rating. Then when we're best friends, she'll tell me why she's been working for Drift Punk."
    y "Я принёс тонну подарков, чтобы повысить её рейтинг привязанности. Потом, когда мы станем лучшими друзьями, она расскажет мне, почему она работает у Дрифт Панков."

# game/status.rpy:5445
translate russia task10_9850411a:

    # c r10 "..................."
    c r10 "..................."

# game/status.rpy:5446
translate russia task10_41eba479:

    # c "Her eyes are red. Isn't that usually a sign that she's being controlled by nanobots?"
    c "Её глаза красные. Разве это не признак того, что её контролируют наноботы?"

# game/status.rpy:5447
translate russia task10_05b3b5fd:

    # y "You're right! Now I just have to boost my relationship rating with her until we can have sex!"
    y "Ты права! Теперь мне остаётся только повышать рейтинг отношений с ней, пока мы не сможем заняться сексом!"

# game/status.rpy:5448
translate russia task10_2f90b915:

    # drag "Wait... I'm being controlled...?"
    drag "Подождите... Меня контролируют...?"

# game/status.rpy:5449
translate russia task10_97bb6f11:

    # c r52 "Yeah. You know those fancy nanobots you started spreading over the population? They infected you aswell."
    c r52 "Да. Помнишь, те причудливые наноботы, которых ты начала распространять среди населения? Они заразили и тебя."

# game/status.rpy:5450
translate russia task10_fe83059a:

    # drag "Hah! Nobody control the dragon! You're just sore how I had my men beat you!"
    drag "Ха! Никто не может управлять Драконом! Тебе просто обидно, что мои люди схватили тебя!"

# game/status.rpy:5451
translate russia task10_746a28b0:

    # c r53 "Listen here you megalomaniac bitc-....!"
    c r53 "Слушай сюда, ты, сучка с манией вели-...!"

# game/status.rpy:5452
translate russia task10_1f8b0674:

    # y "Carla, haven't you been feeling anymore murdery than usual?"
    y "Карла, ты не чувствуешь себя более кровожадно, чем обычно?"

# game/status.rpy:5453
translate russia task10_b348c212:

    # drag "........................"
    drag "........................"

# game/status.rpy:5454
translate russia task10_a4bfa843:

    # c r31 "We're not gonna get anything useful out of her, [playerName]. Just fuck her like you did Maggie."
    c r31 "Мы не получим от неё ничего полезного, [playerName]. Просто трахни её, как ты трахал Мэгги."

# game/status.rpy:5455
translate russia task10_c8b82d86:

    # drag "Wait... what..?!"
    drag "Подождите... стойте..?!"

# game/status.rpy:5456
translate russia task10_ed1bf4ff:

    # c r24 "Oh yeah, you didn't know? Sex reduces the nanobot control.{w} This guy over here is going to blindfold you, tie you up and fuck you until you come to your senses."
    c r24 "Аха, так ты не знала? Секс снижает контроль нанобота.{w} Этот парень собирается связать тебя, завязать тебе глаза и трахать тебя, пока ты не прейдёшь в себя."

# game/status.rpy:5457
translate russia task10_9b19af42:

    # c r22 "Right, [playerName]?"
    c r22 "Ведь так, [playerName]?"

# game/status.rpy:5458
translate russia task10_d3d523f9:

    # drag "N-now wait a minute...!"
    drag "П-подождите минутку...!"

# game/status.rpy:5459
translate russia task10_acdc0021:

    # y "Yup! And Clover seems so excited, I bet she'll even join in!"
    y "Ага! А Кловер похоже очень взволнована! Держу пари, что она даже присоединится!"

# game/status.rpy:5460
translate russia task10_d3227fce:

    # c r12 "{b}*Scoffs*{/b} Yeah right..."
    c r12 "{b}*Насмешка*{/b} Конечно..."

# game/status.rpy:5461
translate russia task10_3077292e:

    # y "Carla, they're controlling you like you control your video game characters. Surely you don't want that."
    y "Карла, они контролируют тебя словно персонажа видеоигры. Конечно же ты этого не хочешь."

# game/status.rpy:5462
translate russia task10_4b1c3e0d:

    # drag "..........................."
    drag "..........................."

# game/status.rpy:5463
translate russia task10_4a22a093:

    # drag "And you're saying that having sex will break the nanobot control?"
    drag "И вы хотите сказать, что секс нарушит контроль наноботов?"

# game/status.rpy:5464
translate russia task10_b2348c4b:

    # c r52 "It has so far."
    c r52 "До сих пор всё так и было."

# game/status.rpy:5465
translate russia task10_1927c7d2:

    # drag "Fine, then let's get it over with. I'm nobodys play puppet."
    drag "Хорошо, тогда давайте покончим с этим. Я ничья либо кукла."

# game/status.rpy:5466
translate russia task10_76870c70:

    # drag ".........................................."
    drag ".........................................."

# game/status.rpy:5467
translate russia task10_65323853:

    # drag "Can..."
    drag "А мож..."

# game/status.rpy:5468
translate russia task10_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:5469
translate russia task10_6b2b33d4:

    # drag "Can you still tie me down and blindfold me...?"
    drag "А ты можешь связать меня и завязать мне глаза...?"

# game/status.rpy:5470
translate russia task10_eeaef7d4:

    # c r29 "Oh God, she's into it..."
    c r29 "О Боже, ей такое нравиться..."

# game/status.rpy:5471
translate russia task10_23096477:

    # y "I don't even need to boost your affection rating...? {w}Everything video games taught me about relationships is a lie."
    y "И мне не нужно повышать твой рейтинг привязанности...? {w}Всё, чему видеоигры научили меня в отношениях - это ложь."

# game/status.rpy:5472
translate russia task10_cdf0ef86:

    # c r11 "I'll... just leave you two to it then."
    c r11 "Я... пожалуй просто оставлю вас двоих."

# game/status.rpy:5473
translate russia task10_aa398def:

    # y "You don't wanna watch?"
    y "Ты не хочешь посмотреть?"

# game/status.rpy:5474
translate russia task10_1583fcc6:

    # c r10 "..............."
    c r10 "..............."

# game/status.rpy:5475
translate russia task10_601c5358:

    # c "I feel like she'd be into that too and I don't wanna give her the pleasure..."
    c "Мне кажется, что только этого и хочет, а я не хочу доставлять ей удовольствие..."

# game/status.rpy:5477
translate russia task10_4a53359f:

    # drag "Now where were we~...."
    drag "Так, на чём мы оста~..."

# game/status.rpy:5480
translate russia task10_8c00627e:

    # drag "Ah! Ahh! Ah!"
    drag "Ах! Хаа! АааАххХ!"

# game/status.rpy:5481
translate russia task10_4777d45c:

    # drag "Yes, faster! Ahh~..."
    drag "Да, быстрее! Ааахх~..."

# game/status.rpy:5482
translate russia task10_286627df:

    # drag "Give me your joystick! Yes! ♥♥♥♥"
    drag "Дай мне свой джойстик! Да! ♥♥♥♥"

# game/status.rpy:5483
translate russia task10_ba396feb:

    # drag "Oh! Ohhh, yes! Use the extension cord!"
    drag "Ох! ОооАААхх, да! Используй удлинитель!"

# game/status.rpy:5484
translate russia task10_8173a375:

    # drag "Ahhhh!!!! ♥♥♥♥♥♥"
    drag "АааАХххА!!!! ♥♥♥♥♥♥"

# game/status.rpy:5485
translate russia task10_ef574bd0:

    # y "Say cheese!"
    y "Скажи \"сыр\"!"

# game/status.rpy:5495
translate russia task10_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:5502
translate russia task10_ce57a665:

    # drag "Oh my...~"
    drag "Оо мой...~"

# game/status.rpy:5503
translate russia task10_85f77ae6:

    # y "Back to your senses?"
    y "Снова в норме?"

# game/status.rpy:5504
translate russia task10_7aede952:

    # drag "Yeah... I can't believe I was being controlled..."
    drag "Да... Не могу поверить, что меня контролировали..."

# game/status.rpy:5505
translate russia task10_d0c1bd0e:

    # y "Time for payback. Tell us as much as you know."
    y "Время отомстить им. Расскажи нам как можно больше."

# game/status.rpy:5506
translate russia task10_31f9797e:

    # drag "Well..."
    drag "Нуу..."

# game/status.rpy:5515
translate russia task10_6b26a44e:

    # drag "That's all I know. Can I go now?"
    drag "Это всё, что я знаю. Я могу идти?"

# game/status.rpy:5516
translate russia task10_f7ac9d0c:

    # y "What? No, of course not. This is a prison we can't just let you go!"
    y "Что? Нет, конечно нет. Это тюрьма, мы не можем просто так отпустить тебя!"

# game/status.rpy:5517
translate russia task10_e7738f67:

    # drag "Oh..."
    drag "Ох..."

# game/status.rpy:5518
translate russia task10_cf26d7c6:

    # drag "I'll suck your dick again if you let me g-..."
    drag "Я снова отсосу тебе, если ты позволишь мне у-..."

# game/status.rpy:5519
translate russia task10_785ee78f:

    # y "No."
    y "Нет."

# game/status.rpy:5520
translate russia task10_33f1fde9:

    # drag "................"
    drag "................"

# game/status.rpy:5526
translate russia task10_eade8a27:

    # "Computer" "Booting up WOOHP Bounty Board..."
    "Компьютер" "Загрузка WOOHP Д.П. ..."

# game/status.rpy:5527
translate russia task10_fbee8034:

    # "Computer" "Target: Carla Wong\nStatus: Captured."
    "Компьютер" "Цель: Карла Вонг\nСтатус: Захвачена."

# game/status.rpy:5528
translate russia task10_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:5555
translate russia task11_0cf33078:

    # y "Okay Alex. You're heading back to the Outsiders today."
    y "Ладно, Алекс. Отправляйся снова к Аутсайдерам."

# game/status.rpy:5556
translate russia task11_b6c37ec0:

    # a y19 "{b}*Whines*{/b} But..."
    a y19 "{b}*Хнычет*{/b} Но..."

# game/status.rpy:5557
translate russia task11_d5eed0e4:

    # y "No buts. Honestly, I expected better from you Alex."
    y "Никаких \"но\". Честно говоря, я ожидал от тебя большего, Алекс."

# game/status.rpy:5558
translate russia task11_03ba5337:

    # a y16 "...?"
    a y16 "...?"

# game/status.rpy:5559
translate russia task11_7462ef6f:

    # y "Judging people on their appearance. Emos deserve love too y'know."
    y "Судить о людях по их внешности. Эмо тоже заслуживают любви, знаешь ли."

# game/status.rpy:5560
translate russia task11_f03bb688:

    # a "They do?"
    a "Правда?"

# game/status.rpy:5561
translate russia task11_4b9de986:

    # y "......."
    y "......."

# game/status.rpy:5562
translate russia task11_6d91a3b4:

    # y "Yeah of course. Everyone does."
    y "Да, конечно. Все заслуживают."

# game/status.rpy:5563
translate russia task11_c50bb328:

    # a y18 "That's not what WOOHP taught us. We're taught not to feel any sort of pity or remorse for our opponents."
    a y18 "Это не то, чему нас учили в WOOHP. Нас учат не испытывать жалости или угрызений совести к нашим противникам."

# game/status.rpy:5564
translate russia task11_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:5565
translate russia task11_ea27f518:

    # y "Okay... that's a little dark."
    y "Лаадно... это немного мрачновато."

# game/status.rpy:5566
translate russia task11_e936e271:

    # a y38 "{i}'If they want to be loved or understood, they should've thought of that before turning evil.'{/i}"
    a y38 "{i}'Если они хотят, чтобы их любили или понимали, им следовало подумать об этом, прежде чем превращаться в зло.'{/i}"

# game/status.rpy:5567
translate russia task11_da08b011:

    # y "Harsh... Just go over there and hug an emo for me."
    y "Сурово... Просто иди туда и обними эмо за меня."

# game/status.rpy:5568
translate russia task11_31f5b1a3:

    # a y15 "Okay~...."
    a y15 "Окей~...."

# game/status.rpy:5585
translate russia task11_db55a4d8:

    # y "Okay Alex, tell me about your day with the Outsi-..."
    y "Хорошо, Алекс, расскажи мне о своем дне с Аутс-..."

# game/status.rpy:5586
translate russia task11_7d69ea47:

    # a y40 "NEVERMORE!"
    a y40 "НИКОГДА!"

# game/status.rpy:5587
translate russia task11_05bf074c:

    # y ".............."
    y ".............."

# game/status.rpy:5588
translate russia task11_0290412f:

    # y "Wha-...?"
    y "Чт-...?"

# game/status.rpy:5589
translate russia task11_322b062b:

    # a y8 "Kaw!"
    a y8 "Кха!"

# game/status.rpy:5590
translate russia task11_71676aff:

    # y "Alex, you're scaring me. What the hell are you doing?"
    y "Алекс, ты меня пугаешь. Какого чёрта ты делаешь?"

# game/status.rpy:5591
translate russia task11_d903f334:

    # a y1 "Today I learnt a poem about a raven!"
    a y1 "Сегодня я выучила стихотворение о вороне!"

# game/status.rpy:5592
translate russia task11_a1fd61a4:

    # y "Oh... you're quoting Edgar Allen Poe... Not sure what I expected sending you undercover with goths."
    y "О... ты цитируешь Эдгара Аллена По... Не уверен, чего я ожидал, отправляя тебя под прикрытием к готам."

# game/status.rpy:5593
translate russia task11_a8d0c1f2:

    # y "So how was it?"
    y "И, как это было?"

# game/status.rpy:5594
translate russia task11_4029cc9d:

    # a y2 "Well... I tried being more open today and got talking to some of them."
    a y2 "Ну... Сегодня я попыталась быть более открытой и заговорила с некоторыми из них."

# game/status.rpy:5595
translate russia task11_a002f1a7:

    # a "They shared their poetry with me! I tried writing some myself too. A poem I call 'Candy Cavity Calamity'. Want to hear it?"
    a "Они поделились со мной своими стихами! Я и сама пыталась написать парочку. Стихотворение, которое я назвала \"Конфетное бедствие\". Хочешь послушать?"

# game/status.rpy:5596
translate russia task11_1cf2df0d:

    # y "I'll pass. What did they think?"
    y "Я пас. Как они это восприняли?"

# game/status.rpy:5597
translate russia task11_19326d2e:

    # a y1 "Well... someone said it was childish. Then someone else said it was ironic. Then someone else said it was post-ironic."
    a y1 "Ну... кто-то сказал, что это по-детски. Кто-то другой сказал, что это ирония. И ещё кто-то сказал, что это пост-ирония."

# game/status.rpy:5598
translate russia task11_29a8ce48:

    # a "I don't really understand what they meant, but they seemed to like it. Saying it had much deeper meaning."
    a "Я не очень понимаю что они имели в виду, но похоже, им это понравилось. Они сказали что тут гораздо более глубокий смысл."

# game/status.rpy:5599
translate russia task11_1937b45a:

    # a y18 "I'm still trying to figure out the deeper meaning..."
    a y18 "Я всё ещё пытаюсь разгадать этот глубинный смысл..."

# game/status.rpy:5600
translate russia task11_5da77c47:

    # y "Well at least you participated in their activities today. I'm going to keep sending you undercover. Try your best to blend in."
    y "Ну, по крайней мере ты сегодня поучаствовала в их жизни. Я буду продолжать посылать тебя под прикрытием. Постарайся изо всех сил слиться с толпой."

# game/status.rpy:5601
translate russia task11_7d69ea47_1:

    # a y40 "NEVERMORE!"
    a y40 "НИКОГДА!"

# game/status.rpy:5602
translate russia task11_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:5603
translate russia task11_09041dfd:

    # y "Are you objecting or are you just quot-..."
    y "Ты возражаешь или просто цитиру-..."

# game/status.rpy:5604
translate russia task11_d5f7bf96:

    # a y8 "Quote the Raven, Nevermore!"
    a y8 "Цитировать Ворона, Никогда!"

# game/status.rpy:5605
translate russia task11_54365b1b:

    # "You sigh and usher the squawking spy to her cell."
    "Вы вздыхаете и отправляете пищащую шпионку в её камеру."

# game/status.rpy:5611
translate russia task11_68f8a81a:

    # a "On my way!"
    a "Уже в пути!"

# game/status.rpy:5624
translate russia task11_0036174a:

    # a "{size=-8}I'm back...{/size}"
    a "{size=-8}Я вернулась...{/size}"

# game/status.rpy:5625
translate russia task11_7ebe5610:

    # y "Alex? How did it go today?"
    y "Алекс? Как всё прошло?"

# game/status.rpy:5626
translate russia task11_58276da8:

    # a "It went okay..."
    a "Всё прошло нормально..."

# game/status.rpy:5627
translate russia task11_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:5628
translate russia task11_0769ac09:

    # y "What happened~...?"
    y "Что произошло~...?"

# game/status.rpy:5629
translate russia task11_150e3d33:

    # a y5 "{b}*Pouts*{/b}"
    a y5 "{b}*Дуется*{/b}"

# game/status.rpy:5630
translate russia task11_777e5208:

    # a "I was yelled at today..."
    a "На меня сегодня накричали..."

# game/status.rpy:5631
translate russia task11_ded551c9:

    # a "This punker looking girl told me to stop acting like a child..."
    a "Эта панкерша сказала мне, чтобы я перестала вести себя как ребенок..."

# game/status.rpy:5634
translate russia task11_35d29ca7:

    # y "Well then maybe you should stop acting like a 6 year old."
    y "Тогда, может тебе, стоит перестать вести себя как шестилетний ребенок?"

# game/status.rpy:5635
translate russia task11_cc8d9171:

    # a y28 "Not you too!"
    a y28 "И ты туда же!"

# game/status.rpy:5636
translate russia task11_89f1c372:

    # y "Alex, going undercover involves actually trying to fit in."
    y "Алекс, работа под прикрытием подразумевает попытку влиться в коллектив."

# game/status.rpy:5637
translate russia task11_24eef454:

    # a y29 "Fit in...?"
    a y29 "Влиться...?"

# game/status.rpy:5638
translate russia task11_16e66160:

    # y "Yeah, act like the other gangmembers."
    y "Да, веди себя как другие члены банды."

# game/status.rpy:5639
translate russia task11_8f854ffb:

    # y "Try being a bit more...{w} 'angsty'."
    y "Попробуй быть немного более...{w} 'устрашающе'."

# game/status.rpy:5640
translate russia task11_78f83269:

    # a y5 ".................."
    a y5 ".................."

# game/status.rpy:5641
translate russia task11_0e57012b:

    # y "Put on some darker make-up, tear some holes in your clothes. Pretend that you understand the world and fight against the {i}'man'{/i}."
    y "Используй немного темного макияжа, порви одежду. Делай вид, что ты понимаешь мир, и борешься против {i}'мужчин'{/i}."

# game/status.rpy:5643
translate russia task11_bbf8f737:

    # a y29 "But I'm not childish..."
    a y29 "Но я не ребенок..."

# game/status.rpy:5644
translate russia task11_e47a6069:

    # a y38 "Why doesn't anyone take me seriously?!"
    a y38 "Почему никто не воспринимает меня всерьёз?!"

# game/status.rpy:5645
translate russia task11_80295cc3:

    # y "Alex... Yesterday when we talked you suggested renting a unicorn to bring in more customers...."
    y "Алекс... Вчера, когда мы разговаривали, ты предложила арендовать единорога, чтобы привлечь больше клиентов..."

# game/status.rpy:5646
translate russia task11_05835e9f:

    # a y1 "Yeah!"
    a y1 "Да!"

# game/status.rpy:5647
translate russia task11_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:5649
translate russia task11_f28802ce:

    # y ".................................."
    y ".................................."

# game/status.rpy:5650
translate russia task11_f28802ce_1:

    # y ".................................."
    y ".................................."

# game/status.rpy:5651
translate russia task11_565075c4:

    # y "..................................{w} You are baby."
    y "..................................{w} Ты - ребёнок."

# game/status.rpy:5652
translate russia task11_bccae5a5:

    # a y28 "[playerName]!"
    a y28 "[playerName]!"

# game/status.rpy:5653
translate russia task11_7ddf7b0c:

    # y "Big baby."
    y "Большой ребёнок."

# game/status.rpy:5654
translate russia task11_d6ba3e26:

    # a y28 "I'm not a baby!"
    a y28 "Я не ребёнок!"

# game/status.rpy:5655
translate russia task11_d23e4ba8:

    # y "Need pacifier.{w} Big baby."
    y "Нужна соска.{w} Большой ребёнок."

# game/status.rpy:5656
translate russia task11_9861fe41:

    # a y4 "[playerName]...! {b}*Sniff*{/b}"
    a y4 "[playerName]...! {b}*Шмыгает носом*{/b}"

# game/status.rpy:5657
translate russia task11_3ade998f:

    # a y42 "Stop bullying me...!"
    a y42 "Прекрати издеваться надо мной...!"

# game/status.rpy:5658
translate russia task11_1a0a2a19:

    # y "Small baby?{w} I think not.{w} Big baby."
    y "Маленький ребёнок?{w} Недумаю.{w} Большой ребёнок."

# game/status.rpy:5659
translate russia task11_7367deb0:

    # a "{b}*Cries*{/b}"
    a "{b}*Плачет*{/b}"

# game/status.rpy:5660
translate russia task11_dcbddf11:

    # a "....................."
    a "....................."

# game/status.rpy:5661
translate russia task11_fdd50de4:

    # a y15 "..............................."
    a y15 "..............................."

# game/status.rpy:5662
translate russia task11_1cf4678f:

    # a "I think I know what you mean....."
    a "Думаю, я знаю, что ты имеешь в виду..."

# game/status.rpy:5663
translate russia task11_7604afbe:

    # a y38 "All right... I'll try to be more grown up!"
    a y38 "Хорошо... Я постараюсь быть взрослее!"

# game/status.rpy:5664
translate russia task11_9f47a418:

    # y "Good."
    y "Отлично."

# game/status.rpy:5665
translate russia task11_17203526:

    # a y38 "Fight the establishment!"
    a y38 "Устрой Дестрой!"

# game/status.rpy:5666
translate russia task11_e9567b02:

    # y "Yeah!"
    y "ДА!"

# game/status.rpy:5667
translate russia task11_ced54140:

    # a "Dye my nails black!"
    a "Покрашу ногти в чёрный цвет!"

# game/status.rpy:5668
translate russia task11_12b7748d:

    # y "All right!"
    y "Вот так!"

# game/status.rpy:5669
translate russia task11_61c732be:

    # a "Do my taxes and figure out a good retirement plan!"
    a "Займусь налогами и разработаю хороший пенсионный план!"

# game/status.rpy:5670
translate russia task11_4ad787e3:

    # y "Okay... not {i}'that'{/i} grown up."
    y "Ладно... не {i}'настолько'{/i} взрослой."

# game/status.rpy:5671
translate russia task11_87167f7d:

    # a y40 "Also I'm going to their creepy abandoned amusement park!"
    a y40 "А ещё я наконец-то поеду в их жуткий и заброшенный парк развлечений!"

# game/status.rpy:5672
translate russia task11_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:5673
translate russia task11_498ea228:

    # y "What do you mean... {i}'going to'{/i}?"
    y "Что ты имеешь в виду под... {i}'наконец-то'{/i}?"

# game/status.rpy:5674
translate russia task11_d16bcd52:

    # y "Haven't you been going undercover there all this time?"
    y "Разве ты не работала там под прикрытием всё это время?"

# game/status.rpy:5675
translate russia task11_ac8bac62:

    # a y1 "Who me? No way, it was way too scary!{w} I usually just called up the gangsters to go hang out around town."
    a y1 "Кто я? Ни за что, это было слишком страшно!{w} Обычно я просто звала их погулять по городу."

# game/status.rpy:5676
translate russia task11_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:5678
translate russia task11_97f836eb:

    # a y3 "I'm pretty sure Muffy Peprich is hanging around the Carnival. I bet we could capture her now!"
    a y3 "Я уверена, что Маффи Пеприч ошивается где-то на Карнавале. Готова поспорить, что мы уже можем поймать её!"

# game/status.rpy:5683
translate russia task11_2b91ac02:

    # "You can now send your spies on missions to the 'Carnival'. The abandoned amusement park on the edge of town."
    "Теперь вы можете отправлять своих шпионок на задания на \"Карнавал\". Заброшенный парк развлечений на окраине города."

# game/status.rpy:5684
translate russia task11_3aa4b8e0:

    # "New clothing options are available for Alex at the mall."
    "В торговом центре появились новые варианты одежды для Алекс."

# game/status.rpy:5705
translate russia task12_04462b70:

    # y "Hmmm...."
    y "Хмм..."

# game/status.rpy:5706
translate russia task12_63884735:

    # y "It's been [daysPlayed] days... I could really use a haircut."
    y "Прошло уже [daysPlayed] дней... Мне бы не помешало подстричься."

# game/status.rpy:5707
translate russia task12_094c0791:

    # y "I hope there's still some around...{w} Who am I kidding, this is Beverly Hills. Of course there's still hairdresses around."
    y "Надеюсь, ещё остались...{w} Кого я обманываю, это же Беверли-Хиллз. Конечно же вокруг по-прежнему есть парикмахерские."

# game/status.rpy:5709
translate russia task12_e9fac61b:

    # "Barber" "Why helloooo! Did I hear you needed a haircut?"
    "Барбер" "Ооой бай! Я слышал, тебе нужно подстричься?"

# game/status.rpy:5710
translate russia task12_b6b2d4a7:

    # y "Erm... yeah, I do. Are you still open?"
    y "Эмм... да, нужно. Вы ещё открыты?"

# game/status.rpy:5711
translate russia task12_a62d5193:

    # "Barber" "Yeeeees!"
    "Барбер" "Абижаешь брат!"

# game/status.rpy:5712
translate russia task12_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:5713
translate russia task12_b3c624ba:

    # y "Okay... can you do my hai-..."
    y "Ладно... вы можете меня под-..."

# game/status.rpy:5714
translate russia task12_028ca199:

    # "Barber" "Nooooo!"
    "Барбер" "Не не не!"

# game/status.rpy:5715
translate russia task12_db832180:

    # y "........"
    y "........"

# game/status.rpy:5716
translate russia task12_068314ba:

    # y "Why no-...?"
    y "Почему не-...?"

# game/status.rpy:5717
translate russia task12_1dfa93f6:

    # "Barber" "You see something terrible has happened! Drift Punk have kidnapped my niece, and I am too worried about her well being to do any hairdressing!"
    "Барбер" "Видишь ли, тут случилось что-то ужасное! Дрифт Панки похитили мою племянницу, и я слишком беспокоюсь о ней, чтобы заниматься стрижкой!"

# game/status.rpy:5718
translate russia task12_dcbd615c:

    # y "Let me guess... you want me to get her back?"
    y "Дай угадаю... ты хочешь, чтобы я её вернул?"

# game/status.rpy:5719
translate russia task12_a8931fbf:

    # "Barber" "Yeeeeees!"
    "Барбер" "Дааааа!"

# game/status.rpy:5720
translate russia task12_270224a9:

    # y "And you're not gonna pay me for it, are you?"
    y "И ты мне за это не заплатишь?"

# game/status.rpy:5721
translate russia task12_396492b3:

    # "Barber" "Noooooo!"
    "Барбер" "Неееет!"

# game/status.rpy:5722
translate russia task12_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:5728
translate russia task12_93d54475:

    # y "Okay, I'll do it, but you have to tell me why you speak like that."
    y "Хорошо, я сделаю это, но ты обещаешь мне рассказать почему ты так говоришь."

# game/status.rpy:5729
translate russia task12_d322a67c:

    # "Barber" "What do you meeeeeeeean?"
    "Барбер" "Что ты имееееешь в виииииду?"

# game/status.rpy:5730
translate russia task12_94aa0325:

    # y "Okay... Never mind, I'll go get your niece back."
    y "Ладно... Не бери в голову, я пойду верну твою племянницу."

# game/status.rpy:5731
translate russia task12_0569c2fd:

    # y "But only if you promise free haircuts for me and my girls after I'm done."
    y "Но только если ты обещаешь бесплатную стрижку для меня и моих девочек после того, как я закончу."

# game/status.rpy:5732
translate russia task12_a8931fbf_1:

    # "Barber" "Yeeeeees!"
    "Барбер" "Даааа!"

# game/status.rpy:5733
translate russia task12_3f53a409:

    # "Barber" "Please bring her back home safely!"
    "Барбер" "Пожалуйста, верни её домой в целости и сохранности!"

# game/status.rpy:5741
translate russia task12_6ffc640a:

    # y "You're creepy...."
    y "Ты странный..."

# game/status.rpy:5742
translate russia task12_29f04bb3:

    # "Barber" "Nooooooooo!"
    "Барбер" "Нееееееет!"

# game/status.rpy:5743
translate russia task12_50c39016:

    # y "On second thought, I don't think I need a haircut..."
    y "Если подумать, мне не особо–то и нужна стрижка..."

# game/status.rpy:5744
translate russia task12_22d7a3a6:

    # "You quickly leave the mall."
    "Вы быстро покидаете торговый центр."

# game/status.rpy:5758
translate russia task12_c9abc911:

    # y "So, any news of the hairdresses' niece?"
    y "Ну что, есть новости о племяннице парикмахера?"

# game/status.rpy:5759
translate russia task12_88a68030:

    # c r31 "Nothing. They could be holding her in Punk Web."
    c r31 "Ничего. Они могут держать её в Панк-Вэбе."

# game/status.rpy:5760
translate russia task12_277fba18:

    # y "We'll have to plan a mission then."
    y "Тогда нам нужно спланировать миссию."

# game/status.rpy:5761
translate russia task12_1e1b213f:

    # y "Okay I'll set one up if we got time."
    y "Хорошо, я её организую, если у нас будет время."

# game/status.rpy:5783
translate russia task12_acde6ffe:

    # "???" "Heeeeeello? is somebody theeeeere...?!"
    "???" "Аауууу? Здесь кто-нибудь есть...?!"

# game/status.rpy:5784
translate russia task12_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:5785
translate russia task12_57a7f167:

    # "???" "I'm trapped in here, pleeeeeeease get me out!"
    "???" "Я застряла, пожалуйста, вытащите меня!"

# game/status.rpy:5786
translate russia task12_05f46d05:

    # y "Yeah that sounds like the niece we're looking for."
    y "Похоже это племянница, которую мы ищем."

# game/status.rpy:5794
translate russia task12_157a1402:

    # niece "Oh thank you for getting me out!"
    niece "Ох, спасибо, что вытащил меня!"

# game/status.rpy:5795
translate russia task12_59bef8bf:

    # y "Don't mention it. We're getting you back to your uncle."
    y "Не за что. Мы вернём тебя твоему дяде."

# game/status.rpy:5796
translate russia task12_ab43be94:

    # niece "Oh yeeeees!"
    niece "Оо дааааа!"

# game/status.rpy:5807
translate russia task12_0c48b34b:

    # y "We brought back your niece."
    y "Вот твоя племянница."

# game/status.rpy:5808
translate russia task12_78fd851a:

    # niece "Uncle! Oh yeeeees, it's good to see you!"
    niece "Дядя! Оо дааааа, я так рада тебя видеть!"

# game/status.rpy:5809
translate russia task12_a8931fbf_2:

    # "Barber" "Yeeeeees!"
    "Барбер" "Дааааа!"

# game/status.rpy:5810
translate russia task12_604cdc6f_1:

    # y "...................."
    y "...................."

# game/status.rpy:5811
translate russia task12_7515fd78:

    # y "So er... about my reward..."
    y "Так, эмм... насчёт моей награды..."

# game/status.rpy:5812
translate russia task12_787d8244:

    # "Barber" "Oh yeeeees! Haircuts are now free of charge!"
    "Барбер" "Ах, даааааа! Стрижки теперь бесплатны!"

# game/status.rpy:5813
translate russia task12_29eb2a13:

    # y "Nice."
    y "Класс."

# game/status.rpy:5836
translate russia task13_c01c4e1e:

    # c "Oh hey [playerName]. Sorry, just got out of the shower, don't mind me."
    c "О, привет [playerName]. Извини, только вышла из душа, не обращай внимания."

# game/status.rpy:5842
translate russia task13_190a9236:

    # a "Hey [playerName] I need new batteries for Mr. Wiggles. Have you seen any around?"
    a "Приветик, [playerName], мне нужны новые батарейки для Мистера Вигглса. Ты знаешь где они?"

# game/status.rpy:5848
translate russia task13_58c84ef8:

    # s "Has anyone seen my underwe-..."
    s "Кто-нибудь видел мое нижне-..."

# game/status.rpy:5849
translate russia task13_da105db1:

    # y "OH COME ON!"
    y "ОЙ, ДА ЛАДНО!"

# game/status.rpy:5852
translate russia task13_30753c3b:

    # y "Let me tap that ass!"
    y "Дай я шлёпну эту задницу!"

# game/status.rpy:5853
translate russia task13_ace50f47:

    # s g29 "!!!"
    s g29 "!!!"

# game/status.rpy:5854
translate russia task13_6d5aa0be:

    # a y28 "!!!"
    a y28 "!!!"

# game/status.rpy:5855
translate russia task13_b6a26306:

    # c r30 "!!!"
    c r30 "!!!"

# game/status.rpy:5856
translate russia task13_34366d7a:

    # y "It's been [daysPlayed] days since we met and you guys haven't been making it easy on me!"
    y "Прошло уже [daysPlayed] дней с момента нашей встречи, и вы, девчонки, не облегчили мне жизнь!"

# game/status.rpy:5857
translate russia task13_0ed0a137:

    # s "What do you mea-....?{w} Oh..."
    s "Что ты име-....?{w} Ох..."

# game/status.rpy:5859
translate russia task13_a5b66f6e:

    # "The girls awkwardly look at each other."
    "Девушки неловко смотрят друг на друга."

# game/status.rpy:5860
translate russia task13_9f1eed83:

    # c r57 "We've talked about this, girls.{w} I think he should know."
    c r57 "Мы уже говорили об этом, девочки.{w} Я думаю, он должен знать."

# game/status.rpy:5861
translate russia task13_6fcd8557:

    # s g38 "I still don't agree with it!"
    s g38 "Я всё ещё не согласна!"

# game/status.rpy:5862
translate russia task13_db711810:

    # y "What are you talking about?"
    y "О чём это вы?"

# game/status.rpy:5863
translate russia task13_21f1cccd:

    # c r12 "Well... foreplay is fun and all, but I think we should drop the 'fore' part and focus more on the 'play'...."
    c r12 "Ну... прелюдия - это весело и всё такое, но я думаю, что нам стоит отказаться от части \"прелюдий\" и больше сосредоточиться на \"игре\"..."

# game/status.rpy:5864
translate russia task13_efb7e6cf:

    # s "Blondie over here seems to think we need to start having sex if we want to control the nanobots!"
    s "Эта блондинка считает, что нам нужно начать заниматься сексом, если мы хотим продолжить контролировать наноботов!"

# game/status.rpy:5865
translate russia task13_784eef14:

    # y "That's outrages."
    y "Это возмутительно."

# game/status.rpy:5866
translate russia task13_42d01adb:

    # s g28 "Exactly! That's what I said!{w} Sure, we're being pumped full of nanobots, but we can't keep escalating this."
    s g28 "В точку! Я так и сказала!{w} Конечно, нас накачивают наноботами, но мы не можем продолжать эскалацию."

# game/status.rpy:5867
translate russia task13_9fd1b4e8:

    # y "Of course. Being pumped full of one liquid is bad enough."
    y "Конечно. Быть накачанным одной жидкостью достаточно плохо."

# game/status.rpy:5868
translate russia task13_b2826a78:

    # s g31 "E-exactly! We don't need to also... get pumped.... full of...."
    s g31 "Т-точно! Нам не нужно... чтобы нас ещё накачивали...."

# game/status.rpy:5869
translate russia task13_4176dec4:

    # y "{b}*Smirk*{/b}"
    y "{b}*Ухмылка*{/b}"

# game/status.rpy:5870
translate russia task13_e274b64e:

    # s g34 "We're not having sex with you, [playerName]!"
    s g34 "Мы не будем заниматься с тобой сексом, [playerName]!"

# game/status.rpy:5871
translate russia task13_8ccfab18:

    # c r24 "I might."
    c r24 "Я буду."

# game/status.rpy:5872
translate russia task13_f67f6eae:

    # s g31 "CLOVER!"
    s g31 "КЛОВЕР!"

# game/status.rpy:5873
translate russia task13_6e96edb8:

    # a y1 "Is [playerName] going to pound some puss...?"
    a y1 "[playerName] собирается оприходовать пару кис-...?"

# game/status.rpy:5874
translate russia task13_77bb62e1:

    # s g40 "ALEX!"
    s g40 "АЛЕКС!"

# game/status.rpy:5875
translate russia task13_af50e5d0:

    # c r55 "Sam, relax."
    c r55 "Сэм, расслабься."

# game/status.rpy:5876
translate russia task13_315bf4c4:

    # c "These nanobot injectors are great for getting new skills, but we've been getting more easily controlled."
    c "Эти инжекторы отлично подходят для получения новых навыков, но нас становиться всё легче контролировать."

# game/status.rpy:5877
translate russia task13_629bd44f:

    # c "Alex here won't even turn back to normal unless we give her a full tongue bath."
    c "Алекс уже даже не взврощается в норму если мы её с ног до головы облизываем."

# game/status.rpy:5878
translate russia task13_0889dda9:

    # a y29 "Those tickle!"
    a y29 "Это так щекотно!"

# game/status.rpy:5879
translate russia task13_f0781b68:

    # c r14 "We need to take control of this situation before it gets worse. That means going down on each other and having sex."
    c r14 "Мы должны взять ситуацию под контроль, пока не стало ещё хуже. А это значит - ласкать друг друга и заниматься сексом."

# game/status.rpy:5880
translate russia task13_8ac1e55c:

    # s "G-going dow-...?!"
    s "Л-ласкать-...?!"

# game/status.rpy:5881
translate russia task13_3e608e1b:

    # c "And have sex. Yes."
    c "И заниматься сексом. Да."

# game/status.rpy:5882
translate russia task13_38e5391d:

    # c r52 "Listen, you can play Ms. Virtuous Maiden as long as you like, but you're just delaying the inevitable."
    c r52 "Слушай, ты можешь играть в Недотрогу, сколько угодно, но ты просто оттягиваешь неизбежное."

# game/status.rpy:5883
translate russia task13_5783cef8:

    # c "WOOHP isn't going down if you keep acting like a prude."
    c "WOOHP не падёт, если ты будешь продолжать вести себя как ханжа."

# game/status.rpy:5884
translate russia task13_e32ee827:

    # s g43 "Well I...!{w} We should ask [playerName] what he thinks!"
    s g43 "Нуу я...!{w} Мы должны спросить [playerName], что он думает!"

# game/status.rpy:5885
translate russia task13_0ea70c94:

    # c r22 "He's a guy. I already know what he thinks."
    c r22 "Он парень. Я уже знаю, что он думает."

# game/status.rpy:5886
translate russia task13_b3c58aad:

    # s g38 "S-still..!"
    s g38 "В-всё равно..!"

# game/status.rpy:5887
translate russia task13_0168507c:

    # a "Hey [playerName]. Do you want to take cooking lessons and pour our ovens full of baby batter?"
    a "Эй [playerName]. Ты хочешь брать уроки кулинарии и заливать наши духовки детским тестом?"

# game/status.rpy:5890
translate russia task13_10e5e2cf:

    # y "OF COURSE! Let's do it...!{w} Right now!"
    y "КОНЕЧНО! Давайте сделаем это...!{w} Прямо сейчас!"

# game/status.rpy:5891
translate russia task13_b51d6509:

    # c "Desperate much?"
    c "Отчаялся?"

# game/status.rpy:5892
translate russia task13_bf1e01d2:

    # y "I've been surrounded by sluts for [daysPlayed] days now!"
    y "Я окружён шлюхами уже [daysPlayed] дней!"

# game/status.rpy:5893
translate russia task13_b814560a:

    # y "I've been waiting for this moment the first time Sam took off her clothes!"
    y "Я ждал этого момента с тех пор, как Сэм впервые разделась!"

# game/status.rpy:5894
translate russia task13_d640122b:

    # s g28 "H-hey...!"
    s g28 "Э-эй...!"

# game/status.rpy:5896
translate russia task13_d797f2ac:

    # y "I guess..."
    y "Наверное..."

# game/status.rpy:5897
translate russia task13_ec9398b8:

    # c r18 "What do you mean, {i}'I guess'{/i}? Three sluts are offering to throw themselves at you!"
    c r18 "Что ты имеешь в виду под {i}'наверное'{/i}? Три шлюхи предлагают тебе броситься на тебя!"

# game/status.rpy:5898
translate russia task13_990a5521:

    # s g29 "S-sluts?!"
    s g29 "Ш-шлюхи?!"

# game/status.rpy:5899
translate russia task13_24293ac7:

    # c "Show some balls, [playerName]. You're gonna have sex with us, whether you like it or not."
    c "Покажи яйца, [playerName]. Ты будешь заниматься с нами сексом, нравится тебе это или нет."

# game/status.rpy:5900
translate russia task13_79e1b198:

    # y "I don't care much for this sexual intimidating at the workplace...!"
    y "Меня не очень нравится это сексуальное запугивание на рабочем месте...!"

# game/status.rpy:5902
translate russia task13_0bb3a8cb:

    # y "..............................."
    y "..............................."

# game/status.rpy:5903
translate russia task13_2f5d1408:

    # c r16 "Oh no... I know that look on his face."
    c r16 "Ох нет... Я знаю это выражение его лица."

# game/status.rpy:5904
translate russia task13_093fa503:

    # y "You ever think we might not in control of our own destiny?{w} Like some all powerful omnipitant force is writing our words for us?"
    y "Вы когда-нибудь думали о том что мы не управляем своей судьбой?{w} Как будто какая-то всемогущая сила пишет наши слова за нас?"

# game/status.rpy:5905
translate russia task13_7bc4e92e:

    # y "Like from one line to the next, we might say something completely crazy and out of characters?"
    y "Типа от одной реплики к другой мы можем сказать что-то совершенно безумное, выходящее за рамки образа?"

# game/status.rpy:5906
translate russia task13_b7ca5003:

    # a y14 "I believe you're experiencing what is known as Depersonalization Disorder or DPDR. In which the person has a presistent or recurrent feeling of derealization or a disconnect from one's self."
    a y14 "Полагаю ты испытываешь так называемый синдром деперсонализации. При котором человек испытывает постоянное или повторяющееся чувство дереализации или разрыва связи с самим собой. "

# game/status.rpy:5907
translate russia task13_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:5908
translate russia task13_e09f4529:

    # y "Yeah you're right. I'm just talking crazy."
    y "Да, ты права. Я просто несу бред."

# game/status.rpy:5909
translate russia task13_fdb73de9:

    # c r24 "So if we're all agreed. [playerName] should have sex with us from now on!"
    c r24 "Итак, если все согласны. С этого момента [playerName] должен заниматься с нами сексом!"

# game/status.rpy:5910
translate russia task13_6c5e79f5:

    # s g38 "We're not all agreed!"
    s g38 "Согласны не все!"

# game/status.rpy:5911
translate russia task13_132cb166:

    # a y29 "Wait... we're still talking about sex?"
    a y29 "Подождите... мы всё ещё говорим о сексе?"

# game/status.rpy:5912
translate russia task13_a465e9e0:

    # s "I think it's just {i}'you'{/i} who wants sex, Clover!"
    s "Похоже что это просто {i}'ты'{/i}, та кто хочет секса, Кловер!"

# game/status.rpy:5913
translate russia task13_3205ced5:

    # c r17 "Wait... what?!"
    c r17 "Стоп... что?!"

# game/status.rpy:5914
translate russia task13_6fc106f3:

    # s "I understand being cut off from boys for this long is taxing on you, but you have to control your urges."
    s "Я понимаю, тебе трудно так долго быть отрезанной от мальчиков и их внимания, но ты должна контролировать себя."

# game/status.rpy:5915
translate russia task13_c627f9be:

    # c r38 "That has nothing to do with it!"
    c r38 "Это не имеет к этому никакого отношения!"

# game/status.rpy:5916
translate russia task13_f93241ae:

    # s "You can't blame the nanobots for everything, Clover."
    s "Ты не можешь винить наноботов во всём, Кловер."

# game/status.rpy:5917
translate russia task13_885e0f7c:

    # c r14 "I'm doing this for the mission!"
    c r14 "Я делаю это ради миссии!"

# game/status.rpy:5918
translate russia task13_4f8195df:

    # s "................................"
    s "................................"

# game/status.rpy:5919
translate russia task13_ef6bf47f:

    # c "................................"
    c "................................"

# game/status.rpy:5920
translate russia task13_7173363b:

    # c r11 "Okay.... well maybe I'm also doing it a little for myself..."
    c r11 "Ладно.... ну может быть, от части я делаю это и для себя..."

# game/status.rpy:5921
translate russia task13_d947335b:

    # s g23 "I've known your slutty ass for longer than today."
    s g23 "Я знаю твою шлюшью задницу уже достаточно долго."

# game/status.rpy:5922
translate russia task13_2a492bb2:

    # c r42 "{b}*Groans*{/b}"
    c r42 "{b}*Стонет*{/b}"

# game/status.rpy:5923
translate russia task13_b2ff0699:

    # s g38 "That doesn't excuse this slutty behavior though!"
    s g38 "Однако это не оправдывает такое распутное поведение!"

# game/status.rpy:5924
translate russia task13_fb5ec225:

    # s "All three of you should be ashamed of yourselves. We're suppose to saving the world and all you're thinking about is sex!"
    s "Вам троим должно быть стыдно за себя. Мы должны спасти мир, а всё, о чём вы думаете, это секс!"

# game/status.rpy:5925
translate russia task13_0917494c:

    # a "Wait...!{w} Are we going to have sex?"
    a "Стойте...! {w} Так у нас будет секс?"

# game/status.rpy:5926
translate russia task13_efc18965:

    # c r54 "Tsk, hello! Pot calling the kettle black. I've heard you going at it with your dildo, miss perfect. Our cells are right next to each other, remember?"
    c r54 "Тск, ало! Чья бы корова мычала. Я слышала, как ты занимаешься этим со своим дилдоком, Мисс Совершенство. Наши камеры находятся рядом друг с другом, помнишь?"

# game/status.rpy:5927
translate russia task13_71b5ae89:

    # c "You're probably the thirstiest of all of us!"
    c "Да тебе это нужно больше чем всем остальным!"

# game/status.rpy:5929
translate russia task13_75940bad:

    # s g40 "Preposterous!"
    s g40 "Какой абсурд!"

# game/status.rpy:5930
translate russia task13_009c9a6a:

    # s "Nobody is having sex and that's final!"
    s "Никто не будет заниматься сексом, и это окончательно!"

# game/status.rpy:5934
translate russia task13_6e4ecfcc:

    # y "........................."
    y "........................."

# game/status.rpy:5935
translate russia task13_13658ccc:

    # y "So about the offer~..."
    y "Итак, о предложении~..."

# game/status.rpy:5936
translate russia task13_1c02ca8a:

    # c r11 "Sam sorta ruined the mood..."
    c r11 "Сэм испортила всё настроение..."

# game/status.rpy:5937
translate russia task13_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:5938
translate russia task13_eef756b5:

    # a y1 "She's just being grumpy. Visit her in her cell and talk one to one. She might be more open to you then."
    a y1 "Она просто слишком раздражительная. Сходи к ней в камеру и поговорите один на один. Она точно будет более открытой."

# game/status.rpy:5939
translate russia task13_2e5bd386:

    # y "Right...."
    y "Точно..."

# game/status.rpy:5940
translate russia task13_5ec2fc2f:

    # a "Gotta put in the work if you want to get your dick wet, [playerName]!"
    a "Нужно приложить усилий если ты хочешь вставить свой член в тёплое и влажное местечко, [playerName]!"

# game/status.rpy:5941
translate russia task13_3287bff2:

    # y "Alex... for someone as naive and innocent as you, you sure have a dirty mouth."
    y "Алекс... для такого наивного и невинного человека, как ты, у тебя довольно грязный язык."

# game/status.rpy:5942
translate russia task13_1cf48546:

    # a y29 "Oops...! I should go wash it with soap then...!"
    a y29 "Упс...! Тогда пойду и помою его с мылом...!"

# game/status.rpy:5961
translate russia task13_decbb72d:

    # s g37 "Before you ask. No, I'm not going to sleep with you."
    s g37 "Прежде чем ты спросишь. Нет, я не собираюсь с тобой спать."

# game/status.rpy:5964
translate russia task13_0e05cc36:

    # y "I don't know what to say. Last time I showed you Alex who turned into a freaking monster."
    y "Я не знаю, что и сказать. В прошлый раз я показал тебе Алекс, которая превратилась в чёртова монстра."

# game/status.rpy:5965
translate russia task13_002524bd:

    # y "But Apparently that wasn't enough to convince you."
    y "Но, видимо этого было недостаточно, чтобы убедить тебя."

# game/status.rpy:5966
translate russia task13_c9c923bd:

    # s g38 "[playerName]..."
    s g38 "[playerName]..."

# game/status.rpy:5968
translate russia task13_ca2c6f83:

    # y "You wouldn't even do it for a Benja-..."
    y "Ты бы не сделал этого даже за Бенж-..."

# game/status.rpy:5969
translate russia task13_882cf294:

    # s g40 "No!{w} I'm not sleeping with you for money! That would make me...!"
    s g40 "Нет!{w} Я не буду спать с тобой за деньги! Тогда я стану...!"

# game/status.rpy:5970
translate russia task13_6ec98c76:

    # s g11 "............."
    s g11 "............."

# game/status.rpy:5971
translate russia task13_a1ca1daa:

    # s "Well you know what that would make me...."
    s "Ну ты знаешь кем я тогда стану..."

# game/status.rpy:5973
translate russia task13_6793ed22:

    # y "Something insane."
    y "Чем-то безумным."

# game/status.rpy:5974
translate russia task13_f2dff589:

    # s "...................."
    s "...................."

# game/status.rpy:5975
translate russia task13_08cfb640:

    # s g16 "What?"
    s g16 "Что?"

# game/status.rpy:5976
translate russia task13_ac9cd1ca:

    # y "Sorry... I just felt the urge to say something insane... so I did."
    y "Прости... Я просто захотел сказать что-то безумное... и сказал, получается."

# game/status.rpy:5977
translate russia task13_900cea48:

    # s g14 "{b}*Sighs*{/b}"
    s g14 "{b}*Вздох*{/b}"

# game/status.rpy:5978
translate russia task13_b8281722:

    # s "Sex is a big deal for us, you know.... We can't just start sleeping around like this."
    s "Секс это очень важный шаг, знаешь ли... Мы не можем просто вот так взять и начать спать."

# game/status.rpy:5979
translate russia task13_61a33ff2:

    # y "By the sounds of it, it's just a big deal for you. The other two seem ready."
    y "Судя по всему, только для тебя это большая проблема. Алекс и Кловер похоже готовы."

# game/status.rpy:5980
translate russia task13_16eba098:

    # s g31 "They're just faking it..."
    s g31 "Они просто притворяются..."

# game/status.rpy:5981
translate russia task13_df3b4522:

    # s "Clover might pretend to be this coy little slut, but I'll let you in on a secret~..."
    s "Кловер может притворяться маленькой скромной шлюшкой, но я открою тебе секрет~..."

# game/status.rpy:5982
translate russia task13_82dace15:

    # s g33 "She never made it past second-base."
    s g33 "Она никогда не доходила до главного блюда."

# game/status.rpy:5983
translate russia task13_9b6105cd:

    # y "No!"
    y "Нет!"

# game/status.rpy:5984
translate russia task13_d0626125:

    # s g23 "{b}*Smirks*{/b} She always says she's gonna do it, but gets cold feet. She's all talk."
    s g23 "{b}*Ухмыляется*{/b} Она всегда говорит, что собирается это сделать, но не решается. Она смелая только на словах."

# game/status.rpy:5985
translate russia task13_090d75c0:

    # s g15 "And Alex... she doesn't even know what sex is. She thinks everything is magically going to fade to black and you hear moaning coming out of nowhere."
    s g15 "А Алекс... она даже не знает, что такое секс. Она думает, что всё волшебным образом померкнет и стоны начнут раздаваться из неоткуда."

# game/status.rpy:5986
translate russia task13_312637d4:

    # y "Like... in a video game...?"
    y "Как... в видео игре...?"

# game/status.rpy:5987
translate russia task13_67f92ca6:

    # s "Exactly."
    s "Ага."

# game/status.rpy:5988
translate russia task13_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:5989
translate russia task13_dfdf48c9:

    # s g11 "I know I might sound like the prissy, stuck up bitch of the group, but I'm really just looking out for them."
    s g11 "Я знаю, что могу показаться ханжой и заносчивой стервой, но на самом деле я просто забочусь о них."

# game/status.rpy:5990
translate russia task13_d6b0139a:

    # y "What about you?"
    y "А что насчёт тебя?"

# game/status.rpy:5991
translate russia task13_2b91a724:

    # s "Well...{w} My relation with sex is difficult..."
    s "Ну...{w} У меня сложные отношения с сексом..."

# game/status.rpy:5992
translate russia task13_7b3ce366:

    # s g15 "I was so desperate for my first-time to be perfect and magical, that I spend too much time preparing."
    s g15 "Я так хотела, чтобы мой первый раз был идеальным и волшебным, что потратила слишком много времени на подготовку."

# game/status.rpy:5993
translate russia task13_230b62ad:

    # s "I read books, did research, made graphs..."
    s "Я читала книги, проводила исследования, составляла графики..."

# game/status.rpy:5994
translate russia task13_0365a9a5:

    # y "Graphs?"
    y "Графики?"

# game/status.rpy:5995
translate russia task13_72f3e5fd:

    # s "I spend so much time preparing, that I didn't actually get around to 'doing'."
    s "Я так много времени трачу на подготовку, что просто не успеваю \"сделать это\"."

# game/status.rpy:5996
translate russia task13_3aac5901:

    # s "But it's not just that..."
    s "Но дело не только в этом..."

# game/status.rpy:5997
translate russia task13_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:5998
translate russia task13_592cc9a7:

    # s g14 "WOOHP recruited us when we were very young."
    s g14 "WOOHP набрала нас, когда мы были совсем юными."

# game/status.rpy:5999
translate russia task13_838af528:

    # s "Long story short. We pretty much have to gave up our youth to be spies. Which we happily did, of course."
    s "Короче говоря. Мы должны были отдать свою молодость, чтобы стать шпионами. Что мы, конечно же, с радостью сделали."

# game/status.rpy:6000
translate russia task13_c9170687:

    # s "But..."
    s "Но..."

# game/status.rpy:6001
translate russia task13_49270825:

    # s g43 "I wasn't ready to give up my first time for WOOHP aswell..."
    s g43 "Я не была готова отказаться от своего первого раза ради WOOHP..."

# game/status.rpy:6002
translate russia task13_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:6003
translate russia task13_46a6f665:

    # s "I know it's silly of me. And I know the risks involved by {i}'not'{/i} doing it.{w} It's just this one little thing in my life that I wanted to decide for myself."
    s "Я знаю, что это глупо с моей стороны. И я знаю о рисках, которые могут возникнуть, если я этого {i}'не'{/i} сделаю.{w} Просто это та единственная вещь в моей жизни, которую я хотела решить для себя сама."

# game/status.rpy:6004
translate russia task13_b3fb74de:

    # s "Not WOOHP. Not my overly controlling parents. Nobody, but me...."
    s "Не WOOHP. Не мои слишком контролирующие родители. Никто, кроме меня...."

# game/status.rpy:6005
translate russia task13_7b438fb7:

    # s g5 "................................."
    s g5 "................................."

# game/status.rpy:6006
translate russia task13_39a66e4c:

    # y "Then maybe we shouldn't do it..."
    y "Тогда, возможно, нам не стоит этого делать..."

# game/status.rpy:6007
translate russia task13_ca79a67c:

    # s "But we should...{w} You heard Clover, the nanobot control is getting worst day by day."
    s "Но мы должны...{w} Ты слышал Кловер, контроль наноботов становится сильнее день ото дня."

# game/status.rpy:6008
translate russia task13_b69dab20:

    # s g19 "We just have to do a little preparing... All three of us.{w} Do you think that's okay...?"
    s g19 "Мы просто должны немного подготовиться... Все трое.{w} Как думаешь, это нормально...?"

# game/status.rpy:6009
translate russia task13_3897e550:

    # y "Of course, Sam.{w} What do you need?"
    y "Конечно, Сэм.{w} Что вам нужно?"

# game/status.rpy:6010
translate russia task13_b6d11324:

    # s "Well I've got some ideas..."
    s "У меня есть несколько идей..."

# game/status.rpy:6011
translate russia task13_c82b5b0c:

    # s g38 "I'd like Alex to watch some porn, straight up."
    s g38 "Я бы хотела, чтобы Алекс посмотрела порно, прямо сейчас."

# game/status.rpy:6012
translate russia task13_ee99fffe:

    # y "Er..."
    y "Эмм..."

# game/status.rpy:6013
translate russia task13_7880b72f:

    # s "She has to know exactly what she's getting herself into."
    s "Она должна точно знать, во что ввязывается."

# game/status.rpy:6014
translate russia task13_ef7c7b0b:

    # s "As for Clover... There this video game I saw at the mall a few days ago. I'd like you to buy it for her."
    s "Что касается Кловер... Несколько дней назад я видела одну видеоигру в торговом центре. Я бы хотела, чтобы ты купил её для неё."

# game/status.rpy:6015
translate russia task13_f4f31cc7:

    # y "And you?"
    y "А для тебя?"

# game/status.rpy:6016
translate russia task13_7b404a73:

    # s g33 "And for me... Well, I kept my notes in my locker. They should still be at school."
    s g33 "А для меня... Ну, я хранила свои записи в шкафчике. Они всё ещё должны быть в школе."

# game/status.rpy:6017
translate russia task13_989c77a6:

    # y "Porngame from the mall, research notes from the school and where do we get the porn for Alex...?"
    y "Порноигра из торгового центра, исследовательские заметки из школы и где мы возьмём порно для Алекс...?"

# game/status.rpy:6018
translate russia task13_284cbe34:

    # s g18 "....................."
    s g18 "....................."

# game/status.rpy:6019
translate russia task13_68f7c1ae:

    # s "Er...."
    s "Эмм..."

# game/status.rpy:6020
translate russia task13_1ee790e1:

    # s "We have internet connection."
    s "У нас есть Интернет."

# game/status.rpy:6021
translate russia task13_c0293a86:

    # y "Wait....."
    y "Подожди..."

# game/status.rpy:6022
translate russia task13_f134fd0b:

    # y "You can see porn on the internet?! Why did nobody tell me?! A new world has opened up to me!"
    y "Порно можно смотреть в интернете?! Почему никто мне не сказал?! Для меня открылся новый мир!"

# game/status.rpy:6023
translate russia task13_bf86a416:

    # s g1 "Just don't forget to visit the mall and plan a mission to the school for my notes."
    s g1 "Только не забудь посетить торговый центр и спланировать миссию в школе для моих заметок."

# game/status.rpy:6024
translate russia task13_b2de9658:

    # s "Once we have that...."
    s "Как только мы получим это..."

# game/status.rpy:6025
translate russia task13_3933b44e:

    # s g19 "Erm... I guess then we can go and do {i}'it'{/i}."
    s g19 "Эмм... Думаю, тогда мы будем готовы сделать {i}'это'{/i}."

# game/status.rpy:6026
translate russia task13_c61e83e7:

    # y "All right, let's do it."
    y "Отлично, сделаем это."

# game/status.rpy:6027
translate russia task13_b53d1d93:

    # "You spend a little more time talking to Sam before heading to bed."
    "Вы проводите ещё немного времени, разговаривая с Сэм, прежде чем отправиться в постель."

# game/status.rpy:6037
translate russia task13_77291725:

    # y "I think I got everything."
    y "Кажется, у меня есть всё что нужно."

# game/status.rpy:6038
translate russia task13_1565cff7:

    # s "Okay, I'll call the others."
    s "Хорошо, я позову остальных."

# game/status.rpy:6040
translate russia task13_f48c7117:

    # "Sam leaves for the cells and returns with Alex and Clover a moment later."
    "Сэм уходит к камерам и через минуту возвращается с Алекс и Кловер."

# game/status.rpy:6044
translate russia task13_96febe4d:

    # c "What's this all about, Sam?"
    c "Что всё это значит, Сэм?"

# game/status.rpy:6045
translate russia task13_36819588:

    # s "............................"
    s "............................"

# game/status.rpy:6046
translate russia task13_235e8202:

    # s "We gotta talk about sex..."
    s "Мы должны поговорить о сексе..."

# game/status.rpy:6047
translate russia task13_26e07b71:

    # a y29 "Sex...?"
    a y29 "Секс...?"

# game/status.rpy:6048
translate russia task13_ea82ce01:

    # c r24 "Oh, have you changed your mind?"
    c r24 "Ох, так ты передумала?"

# game/status.rpy:6049
translate russia task13_3b414ca7:

    # s "[playerName]..."
    s "[playerName]..."

# game/status.rpy:6050
translate russia task13_34f55728:

    # y "I got something for you three to check out."
    y "У меня есть кое-что для каждой из вас."

# game/status.rpy:6051
translate russia task13_5594eb57:

    # "You give Clover the game, Sam the notes and look up a porn website for Alex."
    "Вы отдаёте Кловер игру, Сэм записки и ищете порносайт для Алекс."

# game/status.rpy:6052
translate russia task13_9572fcba:

    # y "Here..."
    y "Вот..."

# game/status.rpy:6053
translate russia task13_686517c9:

    # a y1 "Oh! An internet video? Is it about funny cats?!"
    a y1 "Оо! Интернет-видео? Со смешными котятами?!"

# game/status.rpy:6054
translate russia task13_c5c76dd4:

    # a ".........................."
    a ".........................."

# game/status.rpy:6055
translate russia task13_240d3d8b:

    # a y30 "It's... it's not about cats..."
    a y30 "Т-тут... нет котят..."

# game/status.rpy:6056
translate russia task13_625bf7b0:

    # c r16 "A video game?"
    c r16 "Игра?"

# game/status.rpy:6057
translate russia task13_1e440e1c:

    # c "{b}*Scoffs*{/b} I don't need that! I know all about sex!"
    c "{b}*Насмешка*{/b} Мне это не нужно! Я знаю всё о сексе!"

# game/status.rpy:6058
translate russia task13_785d7a35:

    # s g13 "And my notes. Good, I'll make sure to study these!"
    s g13 "И мои заметки. Хорошо, я обязательно изучу их!"

# game/status.rpy:6059
translate russia task13_9f0f54cd:

    # a y29 "Ooooh.... so that goes in there~..."
    a y29 "Ууоооах.... так это входит в~..."

# game/status.rpy:6060
translate russia task13_de4d104b:

    # c r38 "This is ridiculous..."
    c r38 "Это смешно..."

# game/status.rpy:6061
translate russia task13_bd136854:

    # y "You three go prepare yourself. Once I'm confident that you three know what you're doing, we're getting together again."
    y "Вы трое идите, готовьтесь. Как только я буду уверен, что вы знаете, что делать, мы снова соберёмся вместе."

# game/status.rpy:6062
translate russia task13_de18a899:

    # "The three girls sorta awkwardly shrug and head to their cells."
    "Три девушки неловко пожимают плечами и направляются по своим комнатам."

# game/status.rpy:6068
translate russia task13_b09778a3:

    # y "I should check with them tomorrow and see how they react."
    y "Завтра нужно будет их проверить и посмотреть, как они отреагируют."

# game/status.rpy:6069
translate russia task13_74728190:

    # "Inner Voice" "That's quite a lot of hoops to jump through before getting any..."
    "Внутренний голос" "Это довольно тернистый путь, что бы добраться до..."

# game/status.rpy:6070
translate russia task13_c1954b4b:

    # y "Hey, don't judge me! You're living for free in my head, don't make me evict you!"
    y "Эй, не суди меня! Ты живёшь бесплатно в моей голове, не заставляй меня выселять тебя!"

# game/status.rpy:6071
translate russia task13_e747ceb7:

    # "Inner Voice" "Hah! Good luck!"
    "Внутренний голос" "Хах! Удачи!"

# game/status.rpy:6082
translate russia task13_278aceba:

    # s g1 "............"
    s g1 "............"

# game/status.rpy:6084
translate russia task13_fa131b55:

    # a "Hiya, [playerName]!"
    a "Алоха, [playerName]!"

# game/status.rpy:6086
translate russia task13_445fd129:

    # c "......"
    c "......"

# game/status.rpy:6087
translate russia task13_8c26d385:

    # y "Did you all have time to prepare?"
    y "У вас было время подготовиться?"

# game/status.rpy:6088
translate russia task13_6f9b7bf0:

    # s g1 "Yes! I've read and re-read all my notes. I'm ready!"
    s g1 "Да! Я прочитала и перечитала все свои записи. Я готова!"

# game/status.rpy:6089
translate russia task13_1520ecd4:

    # a y29 "I stayed up all night watching porn!"
    a y29 "Я не спала всю ночь, смотрела порно!"

# game/status.rpy:6090
translate russia task13_513da88b:

    # c r14 "Well... I played your game and..."
    c r14 "Нуу... Я сыграла в твою игру и..."

# game/status.rpy:6091
translate russia task13_2d13b75c:

    # c "................"
    c "................"

# game/status.rpy:6092
translate russia task13_350d6488:

    # c "Well I'm not sure why it was a sci-fi game set in space with glowing swords and orange bimbos..."
    c "Ну, я не поняла, почему это была научно-фантастическая игра в космосе со светящимися мечами и оранжевыми красотками..."

# game/status.rpy:6093
translate russia task13_4ace9fe3:

    # c "But I think it prepared me to have sex...{w} somehow..."
    c "Но я думаю, что это подготовило меня к сексу...{w} так или иначе..."

# game/status.rpy:6094
translate russia task13_1be998d8:

    # y "Well then..."
    y "Ну что ж тогда..."

# game/status.rpy:6095
translate russia task13_385d2463:

    # y "Sex?"
    y "Секс?"

# game/status.rpy:6096
translate russia task13_19703971:

    # "All three of the girls give a determined nod."
    "Все три девушки решительно кивают."

# game/status.rpy:6100
translate russia task13_ec366bd5:

    # "The girls have taken their next step in suppressing their nanobots."
    "Девушки сделали следующий шаг к подавлению своих наноботов."

# game/status.rpy:6101
translate russia task13_c9905765:

    # "You can now ask them to have sex in their cells. On top of that, you will occassionally see them going down on each other in their cells."
    "Теперь вы можете предложить им заняться сексом в своих камерах. Кроме того, иногда можно увидеть, как они лижут друг у друга в своих камерах."

# game/status.rpy:6103
translate russia task13_d9fcfa67:

    # y "Then it's decided! I'll visit your whenever."
    y "Тогда решено! Я буду посещать вас в любое время."

# game/status.rpy:6109
translate russia task13_377e3184:

    # "Inner Voice" "Duuuuuuuuuuude! You scored some Grade A ass! And you didn't even have to emotionally guilt trip them into it!"
    "Внутренний голос" "Чуваааааак! Ты заполучил задницу класса А! И тебе даже не пришлось эмоционально уговаривать их!"

# game/status.rpy:6110
translate russia task13_0698e0ad:

    # "Inner Voice" "If I wasn't a disembodied voice in your head, I'd high-five you."
    "Внутренний голос" "Если бы я не был бесплотным голосом в твоей голове, я бы дал тебе пять."

# game/status.rpy:6117
translate russia task13_f3f5bfa6:

    # s g18 "You have a point though... We're not defeating WOOHP by doing this half way. Maybe we should go all in...."
    s g18 "Ты прав... Мы не победим WOOHP используя только половину нашей сили. Может нам, стоит пойти all in..."

# game/status.rpy:6118
translate russia task13_3e740c73:

    # y "Are you saying we should go in....{w} balls-deep?"
    y "Ты хочешь сказать, что мы должны войти....{w} по самые шары?"

# game/status.rpy:6119
translate russia task13_5422cbc1:

    # s g11 "........................."
    s g11 "........................."

# game/status.rpy:6120
translate russia task13_fdac8c8f:

    # s g2 "I heh~... I guess I am..."
    s g2 "Я хех~... Думаю да..."

# game/status.rpy:6121
translate russia task13_2d3e4f62:

    # y "Seriously?"
    y "Серьезно?"

# game/status.rpy:6122
translate russia task13_a3018e66:

    # s g1 "If you keep giving us nanobot injections to improve our skills, we are put more and more at risk of turning..."
    s g1 "Если ты будешь продолжать делать нам инъекции для улучшения навыков, мы всё больше и больше будем подвергаться риску превращения..."

# game/status.rpy:6123
translate russia task13_3ef9d54e:

    # s "I guess... I guess it's time for the next step to prevent that from happening..."
    s "Я думаю... Думаю, пришло время для следующего шага, чтобы предотвратить это..."

# game/status.rpy:6124
translate russia task13_46ba940e:

    # y "So... sex?"
    y "Так... секс?"

# game/status.rpy:6126
translate russia task13_5cb2a45a:

    # s g14 "Sex."
    s g14 "Секс."

# game/status.rpy:6127
translate russia task13_eef9d504:

    # s "I guess... {b}*Gulp*{/b} just come to our cells whenever."
    s "Я думаю... {b}*Глоть*{/b} просто приходи в наши камеры в любое время."

# game/status.rpy:6128
translate russia task13_6debd07a:

    # c r22 "I wouldn't mind if you came by mine first~..."
    c r22 "Я не против, если ты сначала зайдешь ко мне~..."

# game/status.rpy:6134
translate russia task13_377e3184_1:

    # "Inner Voice" "Duuuuuuuuuuude! You scored some Grade A ass! And you didn't even have to emotionally guilt trip them into it!"
    "Внутренний голос" "Чуваааааак! Ты заполучил задницу класса А! И тебе даже не пришлось эмоционально уговаривать их!"

# game/status.rpy:6135
translate russia task13_0698e0ad_1:

    # "Inner Voice" "If I wasn't a disembodied voice in your head, I'd high-five you."
    "Внутренний голос" "Если бы я не был бесплотным голосом в твоей голове, я бы дал тебе пять."

# game/status.rpy:6151
translate russia task14_0ae9bcd0:

    # ""
    ""

# game/status.rpy:6168
translate russia task15HireSluts_99f6ae12:

    # y "I don't have enough money for this."
    y "У меня не хватит на это денег."

# game/status.rpy:6173
translate russia task15HireSluts_7be7672e:

    # "You hired a bunch of college girls to join the Aces party."
    "Вы наняли кучу студенток, чтобы присоединиться к вечеринке Тузов."

# game/status.rpy:6209
translate russia task15_f2dff589:

    # s "...................."
    s "...................."

# game/status.rpy:6210
translate russia task15_b580280c:

    # y "What's wrong?"
    y "Что случилось?"

# game/status.rpy:6211
translate russia task15_52ef6627:

    # s g14 "It's the Aces..."
    s g14 "Это Тузы..."

# game/status.rpy:6212
translate russia task15_527d80fc:

    # s "Things aren't the way they used to be. With their lieutenants locked up, the excitement and high energy in the gang is gone..."
    s "Всё уже не так, как раньше. После того как их лейтенанты оказались за решеткой, оживлённость и энергичность в банде исчезли..."

# game/status.rpy:6213
translate russia task15_8ac02a44:

    # s "No more parties, everyone just keeps to themselves. It's as if they know the gang doesn't have much life left."
    s "Никаких вечеринок, все держатся сами по себе. Как будто они знают, что банде осталось жить недолго."

# game/status.rpy:6214
translate russia task15_6c18570e:

    # y "Which is good, right?"
    y "И это хорошо, ведь так?"

# game/status.rpy:6215
translate russia task15_a90c06e0:

    # s g12 "Yeah I guess. A gang with poor morale should be easier to take down."
    s g12 "Да, наверное. Банду с низким моральным духом легче уничтожить."

# game/status.rpy:6216
translate russia task15_85b320ea:

    # s "I'm just feeling a bit nostalgic. I miss how things used to be."
    s "Я просто немного ностальгирую. Я скучаю по тому, как всё было раньше."

# game/status.rpy:6217
translate russia task15_fea7d438:

    # y "What about the leader of the Aces?"
    y "А как насчёт лидера Тузов?"

# game/status.rpy:6219
translate russia task15_1e51148b:

    # s "Oh you mean Candy? She's as tough as ever."
    s "Вы имеете в виду Кэнди? Она так же вынослива, как и всегда."

# game/status.rpy:6220
translate russia task15_46620303:

    # s "She's cautious of me. Afterall, her lieutenants have gone missing and she knows who I am. Me and the girls have fought her before."
    s "Она осторожна со мной. В конце концов, её лейтенанты пропали без вести, и она знает кто я. Мы с девочками уже сражались с ней."

# game/status.rpy:6222
translate russia task15_a5e8ba84:

    # s "Whoever is leading the Aces is not showing any signs of weakness."
    s "Кто бы не управлял Тузами, он не проявляет никаких признаков слабости."

# game/status.rpy:6223
translate russia task15_2309d264:

    # s "We'll have to send some more freed WOOHP agents to attack the Aces during the next Landgrab to find out who they are."
    s "Нам придётся послать ещё несколько освобождённых агентов WOOHP для нападения на Тузов во время следующей войны за территории, чтобы выяснить, кто за ними стоит."

# game/status.rpy:6224
translate russia task15_7e8d929d:

    # s g10 "I think that as long as the Aces are in a rut, they'll be less of a threat."
    s g10 "Думаю, что пока Тузы будут заняты рутиной, они будут представлять меньше угрозы."

# game/status.rpy:6225
translate russia task15_bc0a53c4:

    # s "But if we can boost their morale, and make them think they're still on top, they could make mistakes."
    s "Но если мы сможем повысить их мораль и заставить их думать, что они всё ещё находятся на вершине. То они могут допустить ошибки."

# game/status.rpy:6226
translate russia task15_f837512b:

    # y "And we could use those to capture their leader..."
    y "Мы могли бы использовать их для захвата их лидера..."

# game/status.rpy:6227
translate russia task15_f4a1aa11:

    # y "What did you have in mind?"
    y "Что ты предлагаешь?"

# game/status.rpy:6228
translate russia task15_ee463e5a:

    # s g14 "Erm...."
    s g14 "Эмм..."

# game/status.rpy:6229
translate russia task15_1ce82017:

    # y "Yes...?"
    y "Да...?"

# game/status.rpy:6230
translate russia task15_35ab3744:

    # s g1 "I was thinking, let's throw a party. A big one."
    s g1 "Я думала провести вечеринку. Большую."

# game/status.rpy:6231
translate russia task15_3aaa2a03:

    # s "That would improve their morale and hopefully get me in the good graces of the leader."
    s "Это поднимет их моральный дух и, надеюсь, поможет мне выделиться перед их лидером."

# game/status.rpy:6232
translate russia task15_f6729b65:

    # s g38 "Then when noone suspects a thing, we'll nab them! After that the gang will most likely disband."
    s g38 "А когда никто ничего не будет подозревать, мы схватим их! После этого банда, скорее всего, распадётся."

# game/status.rpy:6233
translate russia task15_c1a7f38b:

    # y "And you're okay with that...?"
    y "А ты не будешь против...?"

# game/status.rpy:6234
translate russia task15_7ff6e2dd:

    # s g15 ".............................."
    s g15 ".............................."

# game/status.rpy:6235
translate russia task15_49d2e200:

    # s g14 "Things have to get back to normal... and they won't with the Aces still around."
    s g14 "Всё должно вернуться к норме... а если Тузы будут на свободе, у нас это вряд ли получиться."

# game/status.rpy:6236
translate russia task15_81ff93f1:

    # s "I'm gonna miss the lifestyle, but for the sake of Beverly Hills... let's do it."
    s "Я буду скучать по этому образу жизни, но ради Беверли Хиллз... Я готова."

# game/status.rpy:6237
translate russia task15_51b562d5:

    # y "All right. Party it is!"
    y "Отлично. Закатим вечеринку!"

# game/status.rpy:6238
translate russia task15_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:6239
translate russia task15_ddd92021:

    # y "Er.... what do we need?"
    y "Эм... а что на потребуется?"

# game/status.rpy:6240
translate russia task15_897b2272:

    # s g1 "Well first off, we need a place to host it."
    s g1 "Ну, прежде всего, нам понадобиться место."

# game/status.rpy:6241
translate russia task15_735442ca:

    # y "The Milkshake bar?"
    y "Наш бар?"

# game/status.rpy:6242
translate russia task15_1cacd055:

    # s "Exactly. Second we'll need music."
    s "Точно. Во-вторых, нам понадобится музыка."

# game/status.rpy:6243
translate russia task15_2b23643e:

    # s g22 "I'm thinking we steal some from Drift Punk as a sign of Aces dominance."
    s g22 "Я думаю, стоит украсть её у Дрифт Панков в знак превосходства Тузов."

# game/status.rpy:6244
translate russia task15_41864067:

    # y "Okay... so plan a mission to Punk Web. What else?"
    y "Хорошо... спланировать миссию в Панк-Вэб. Что ещё?"

# game/status.rpy:6245
translate russia task15_5a2da986:

    # s g13 "Fireworks. I'm thinking Outsiders."
    s g13 "Фейерверки. Думаю, это к Аутсайдерам."

# game/status.rpy:6246
translate russia task15_f38fceee:

    # y "Plan mission to Abandoned Amusement park... Anything else?"
    y "Спланировать миссию в заброшенном парке развлечений... Что-нибудь ещё?"

# game/status.rpy:6247
translate russia task15_08692f66:

    # s g14 "Copious amounts of drugs."
    s g14 "Большое количество наркотиков."

# game/status.rpy:6248
translate russia task15_34d328b1:

    # y "Er...."
    y "Эмм..."

# game/status.rpy:6249
translate russia task15_7143ca68:

    # s g1 "Don't worry, most of that can be supplied by the Aces."
    s g1 "Не волнуйся, большую часть этого обеспечат Тузы."

# game/status.rpy:6250
translate russia task15_377b65d8:

    # s "And finally...."
    s "И наконец...."

# game/status.rpy:6251
translate russia task15_d6126bf4:

    # y "WHORES AND BLACKJACK!"
    y "БЛЭКДЖЕК И ШЛЮХИ!"

# game/status.rpy:6252
translate russia task15_2fc30b82:

    # s g29 "Well... yes actually."
    s g29 "Нуу... вообще-то да."

# game/status.rpy:6253
translate russia task15_55f931d6:

    # y "Really?"
    y "Правда?"

# game/status.rpy:6254
translate russia task15_08c8f149:

    # s g1 "It's a party. Better go all out."
    s g1 "Это вечеринка. Лучше выложиться по полной."

# game/status.rpy:6255
translate russia task15_aaad45f9:

    # s "I bet we can pick up some girls who are interested at the school."
    s "Уверена, мы сможем найти несколько заинтересованных девушек в школе."

# game/status.rpy:6256
translate russia task15_dd3ddfca:

    # y "Pick up some thirsty college sluts. Got it."
    y "Подцепите несколько голодных шлюх из колледжа. Понял."

# game/status.rpy:6257
translate russia task15_ee1b04c4:

    # y "So... how are we gonna pay for all this?"
    y "Ааа... как мы будем платить за всё это?"

# game/status.rpy:6258
translate russia task15_e1112603:

    # s "I think I can convince the Aces to pay for most of it."
    s "Думаю, я смогу убедить Тузов оплатить большую часть."

# game/status.rpy:6259
translate russia task15_214a04fd:

    # s "We just gotta worry about the initial payment. Let's say... $2.000?"
    s "Нам остаётся только беспокоиться о первоначальном взносе. Скажем... $2.000?"

# game/status.rpy:6260
translate russia task15_8b869f91:

    # y "Okay so we'll need..."
    y "Итак, нам понадобится..."

# game/status.rpy:6261
translate russia task15_62016b30:

    # y "Music from Punk Web\nFireworks from the Abandoned Amusement Park\nSlutty college girls from the school.\nA budget of 2000 dollars."
    y "Музыка из Панк-Вэба\nФейерверки из заброшенного парка развлечений\nРазвратные девушки из колледжа.\nБюджет в размере 2000 долларов."

# game/status.rpy:6262
translate russia task15_f8d553f0:

    # s "Yup, that's bound to put me in good graces with the gang."
    s "Ага, и это сделает меня хорошим другом банды."

# game/status.rpy:6263
translate russia task15_5ae5bc94:

    # y "Let's do it then."
    y "Давайте сделаем это."

# game/status.rpy:6283
translate russia task15_2e044d4f:

    # y "Okay, I think we have everything we need."
    y "Отлично, думаю у нас есть всё что нужно."

# game/status.rpy:6284
translate russia task15_7665bf07:

    # s "I've already sent word to the Aces! This is gonna be the best party ever!"
    s "Я уже отправила весточку Тузам! Это будет самая лучшая вечеринка!"

# game/status.rpy:6303
translate russia task15_8cae2d81:

    # c "Ready as we'll ever be."
    c "Готовы, как никогда."

# game/status.rpy:6304
translate russia task15_cd8f9e15:

    # a "I hope they don't ask us to show our underwear..."
    a "Надеюсь, они не попросят нас показать нижнее белье..."

# game/status.rpy:6305
translate russia task15_ae7f22e8:

    # y "Well..."
    y "Нууу..."

# game/status.rpy:6306
translate russia task15_12135488:

    # a y28 "Cause I'm not wearing any!"
    a y28 "Потому что на мне его нет!"

# game/status.rpy:6307
translate russia task15_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:6308
translate russia task15_03a944cc:

    # "The girls seem ready."
    "Девочки, кажется, готовы."

# game/status.rpy:6315
translate russia task15_8b79b9c9:

    # "You get everything ready and soon the Aces come in one by one."
    "Вы всё подготовили, вскоре Тузы появляются один за другим."

# game/status.rpy:6320
translate russia task15_54f4b238:

    # "Aces Gangster 1" "Dude, check out the fireworks!"
    "Туз 1" "Чувак, только зацени эти фейерверки!"

# game/status.rpy:6321
translate russia task15_8d3efd46:

    # "Aces Gangster 2" "No way, I'm dancing to the best beats in town!"
    "Туз 2" "Ни за что, я танцую под лучшие бит в городе!"

# game/status.rpy:6322
translate russia task15_0c3a047c:

    # "Aces Gangster 3" "Who cares about dancing when you're surrounded by the hottest girls in Beverly Hills?!"
    "Туз 3" "Кого волнуют танцы, когда тебя окружают самые сексуальные девушки в Беверли-Хиллз?!"

# game/status.rpy:6323
translate russia task15_b93f5678:

    # "Aces Gangsters" "Three cheers for Sam!"
    "Тузы" "Трижды за Сэм!"

# game/status.rpy:6329
translate russia task15_5c53bbb3:

    # y "Hey there [samNick], looks like your party is a success."
    y "Привет, [samNick], похоже твоя вечеринка удалась."

# game/status.rpy:6331
translate russia task15_e996765a:

    # s "Heh~... yeah!"
    s "Хехе~... ага!"

# game/status.rpy:6332
translate russia task15_816b98c0:

    # y "Easy on the booze there..."
    y "Полегче с выпивкой..."

# game/status.rpy:6333
translate russia task15_f2f8ec1e:

    # s "Aw just relax. It's a party! Maybe the last one the Aces will ever have!"
    s "Расслабься. Это вечеринка! Возможно, последня, который когда-либо будет у Тузов!"

# game/status.rpy:6334
translate russia task15_c2c7ff8f:

    # s g13 "Grab a drink!"
    s g13 "Возьми напиток!"

# game/status.rpy:6337
translate russia task15_ee284cd3:

    # a "This is fun!"
    a "Это весело!"

# game/status.rpy:6338
translate russia task15_3e3a3823:

    # y "Alex! Where's your top?!"
    y "Алекс! Где твой топ?!"

# game/status.rpy:6339
translate russia task15_8ac0cf18:

    # a "I lost it in a game of rock-paper-scissors!"
    a "Я проиграла его в игре камень-ножницы-бумага!"

# game/status.rpy:6341
translate russia task15_9c81ae07:

    # c "Not gonna lie... this 'is' a pretty great party."
    c "Не буду врать... это отличная вечеринка."

# game/status.rpy:6342
translate russia task15_0a64e719:

    # s g1 "C'mon [playerName]. Just one drink~..."
    s g1 "Давай [playerName]. Всего один стаканчик~..."

# game/status.rpy:6343
translate russia task15_6a4d3a49:

    # y "Oh well, I guess one can't hurt."
    y "Ох, ну ладно, думаю, один не навредит."

# game/status.rpy:6348
translate russia task15_8d15cb14:

    # "You make your way over to the bar and get a drink....{w} Or two...{w} Maybe three."
    "Вы подходите к бару и берёте один стакан с выпивкой.....{w} Или два...{w} Может быть и три."

# game/status.rpy:6350
translate russia task15_c27d4f2f:

    # "You doze off~..."
    "Вы вырубаетесь~..."

# game/status.rpy:6354
translate russia task15_4bde5317:

    # y "Ugh~... how long was I out for... {b}*Hic...!*{/b}"
    y "Угх~... как долго я был в отключке... {b}*Иик...!*{/b}"

# game/status.rpy:6355
translate russia task15_b678febd:

    # "Off to the end of the club you see Alex dancing naked on a table!"
    "В другом конце клуба вы видите Алекс, танцующею голой на столе! "

# game/status.rpy:6356
translate russia task15_40fc0264:

    # a "Strip teeeeeease.....!"
    a "Стриптиииииииииз....!"

# game/status.rpy:6358
translate russia task15_2497dd0f:

    # c "You okay there, [playerName]?"
    c "Ты как, [playerName]?"

# game/status.rpy:6359
translate russia task15_f77904cd:

    # y "Still fully dressed...?"
    y "Ты всё ещё в одежде...?"

# game/status.rpy:6360
translate russia task15_0dbcec78:

    # c r24 "Not for long~... I think I'll join Alex. ♥"
    c r24 "Не надолго~... Думаю, я присоединюсь к Алекс. ♥"

# game/status.rpy:6362
translate russia task15_64a2b68e:

    # y "..........................."
    y "..........................."

# game/status.rpy:6363
translate russia task15_c63fe075:

    # y "Where did Sam go?"
    y "А куда делась Сэм?"

# game/status.rpy:6364
translate russia task15_52c1dbcf:

    # "You decide to head outside for some fresh air."
    "Вы решаете выйти на улицу, чтобы подышать свежим воздухом."

# game/status.rpy:6371
translate russia task15_c06e20c9:

    # "???" "It's been a long time, hasn't it?"
    "???" "Прошло много времени, не так ли?"

# game/status.rpy:6372
translate russia task15_8cbc3d40:

    # s "Yes... it has..."
    s "Да... это так..."

# game/status.rpy:6373
translate russia task15_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:6379
translate russia task15_78e1fd65:

    # s "Heh~... never expected we'd be working together..."
    s "Хех~... никогда не думала, что мы будем работать вместе..."

# game/status.rpy:6380
translate russia task15_a513c5e2:

    # tim "Yeah..."
    tim "Даа..."

# game/status.rpy:6381
translate russia task15_53b52d1d:

    # tim "How've you been, Sam?"
    tim "Как ты, Сэм?"

# game/status.rpy:6382
translate russia task15_4462bdb5:

    # s "Well... you know..."
    s "Нуу... знаешь..."

# game/status.rpy:6383
translate russia task15_36b324e1:

    # s g5 "........................"
    s g5 "........................"

# game/status.rpy:6384
translate russia task15_f522f32e:

    # s "I've been okay..."
    s "В порядке, в принципе..."

# game/status.rpy:6385
translate russia task15_9201f464:

    # tim "....................."
    tim "....................."

# game/status.rpy:6386
translate russia task15_7c2476dc:

    # tim "Just hang in there..."
    tim "Просто держись..."

# game/status.rpy:6387
translate russia task15_3541e160:

    # tim "I know you're not completely yourself right now, but we're taking WOOHP down for a good reason."
    tim "Я знаю, что ты сейчас не совсем в себе, но мы рушим WOOHP по веской причине."

# game/status.rpy:6388
translate russia task15_b23d2180:

    # s "Yeah...."
    s "Даа..."

# game/status.rpy:6389
translate russia task15_605685c1:

    # tim "......................"
    tim "......................"

# game/status.rpy:6390
translate russia task15_62055b7e:

    # tim "You don't have to believe me. It's fine."
    tim "Ты не обязана мне верить. Всё в порядке."

# game/status.rpy:6391
translate russia task15_e508ad31:

    # tim "You'll see that we did the right thing once that organization is gone."
    tim "Ты увидишь, что мы поступили правильно, когда этой организации не станет."

# game/status.rpy:6392
translate russia task15_e04316ce:

    # s "But....!"
    s "Но....!"

# game/status.rpy:6393
translate russia task15_12c4978d:

    # tim "The gang violence was an unfortune side-effect, but the city will recover."
    tim "Бандитизм был неприятным побочным эффектом, но город восстановится."

# game/status.rpy:6394
translate russia task15_cecb699e:

    # s "...................................."
    s "...................................."

# game/status.rpy:6395
translate russia task15_dc58d8c4:

    # tim "....................................."
    tim "....................................."

# game/status.rpy:6396
translate russia task15_6f02878f:

    # tim "I missed you, Sam..."
    tim "Я скучал по тебе, Сэм..."

# game/status.rpy:6397
translate russia task15_9fda7524:

    # s "Don't say that..."
    s "Не говори так..."

# game/status.rpy:6398
translate russia task15_720bb5ae:

    # s "We're enemies, remember?"
    s "Мы враги, помнишь?"

# game/status.rpy:6399
translate russia task15_aa463f16:

    # tim "Not for long. When WOOHP is gone, I'd like us to try again... {w}for real this time."
    tim "Ненадолго. Когда WOOHP падёт, я бы хотел, чтобы мы попробовали снова... {w}на этот раз по-настоящему."

# game/status.rpy:6400
translate russia task15_bc2a54f4:

    # s "........................."
    s "........................."

# game/status.rpy:6401
translate russia task15_eaa3d227:

    # s "Me too..."
    s "Я тоже..."

# game/status.rpy:6402
translate russia task15_e81f1d14:

    # tim "It's getting cold, let's go back inside."
    tim "Становится холодно, давай вернёмся внутрь."

# game/status.rpy:6404
translate russia task15_e30f2fc8:

    # "The two are coming your way!"
    "Они идут к вам!"

# game/status.rpy:6405
translate russia task15_de617ee5:

    # tim "...?"
    tim "...?"

# game/status.rpy:6406
translate russia task15_2a009e81:

    # tim "Agent."
    tim "Агент."

# game/status.rpy:6409
translate russia task15_f23cfd30:

    # "You quickly stand aside and salute."
    "Вы быстро отходите в сторону и отдаёте честь."

# game/status.rpy:6410
translate russia task15_f967db86:

    # y "Mr. Scam."
    y "Мистер Скам."

# game/status.rpy:6411
translate russia task15_5d7356eb:

    # tim "Lay off on the alcohol. You don't want to get too toasty with these gangsters."
    tim "Откажитесь от алкоголя. Вы же не хотите, чтобы с этими бандитами было слишком жарко."

# game/status.rpy:6412
translate russia task15_a6b7227f:

    # y "Of course, sir!"
    y "Конечно, сэр!"

# game/status.rpy:6414
translate russia task15_64565421:

    # "You simply nod and turn the other way."
    "Вы просто киваете и поворачиваетесь в другую сторону."

# game/status.rpy:6415
translate russia task15_de617ee5_1:

    # tim "...?"
    tim "...?"

# game/status.rpy:6416
translate russia task15_3c7fd29f:

    # s g56 "C-come on, Tim. Let's get back inside."
    s g56 "П-пойдем, Тим. Давай вернёмся внутрь."

# game/status.rpy:6418
translate russia task15_bacd65f0:

    # y "All it well sir, no incidental petrol trucks in sight."
    y "Всё в порядке, сэр, никаких случайных бензовозов в поле зрения."

# game/status.rpy:6419
translate russia task15_4d8746e9:

    # tim "What...?"
    tim "Что...?"

# game/status.rpy:6420
translate russia task15_4d0c39cf:

    # y "The chance that this party will go up in flames and we'll all die in a fiery explosion is less than 10%%, sir!"
    y "Вероятность того, что эта вечеринка разгорится и мы всё взлетим на воздух, составляет менее 10%%, сэр!"

# game/status.rpy:6421
translate russia task15_812e069f:

    # tim "That's... good to know..."
    tim "Это... весьма полезное замечание..."

# game/status.rpy:6422
translate russia task15_3c7fd29f_1:

    # s g56 "C-come on, Tim. Let's get back inside."
    s g56 "П-пойдем, Тим. Давай вернёмся внутрь."

# game/status.rpy:6427
translate russia task15_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:6428
translate russia task15_92d0978b:

    # a "WOOOOOO!"
    a "ВИУУУУУУ!"

# game/status.rpy:6429
translate russia task15_6a3df86b:

    # "You suddenly see Alex running out holding a T-shirt!"
    "Вдруг вы видите выбегающею Алекс с футболкой в руках!"

# game/status.rpy:6437
translate russia task15_42eed621:

    # c "Alex! Give that back!!!"
    c "Алекс! Отдай!!!"

# game/status.rpy:6444
translate russia task15_d0320679:

    # y "Welp... looks like it's a party. Better get back inside."
    y "Уого... кажется, это вечеринка. Лучше вернуться внутрь."

# game/status.rpy:6448
translate russia task15_4f419004:

    # "The rest of the night is filled with drinking, partying and lots of naked boobs."
    "Остальная часть ночи полна выпивкой, весельем и множеством обнаженных сисек."

# game/status.rpy:6460
translate russia task15_ef923d0b:

    # c "Uuuugh~..."
    c "Уууугх~..."

# game/status.rpy:6461
translate russia task15_05af7bc6:

    # y "Good morning."
    y "Доброе утро."

# game/status.rpy:6462
translate russia task15_2cbf9a90:

    # c "UUUUUGHH~...."
    c "УУУУУГХХ~...."

# game/status.rpy:6464
translate russia task15_51b43b9a:

    # a "My head hurts~...."
    a "Моя голова раскалывается~...."

# game/status.rpy:6466
translate russia task15_932a783f:

    # s "Oooof~..."
    s "Уууф~..."

# game/status.rpy:6467
translate russia task15_124b1ecb:

    # s g1 "Good morning guys."
    s g1 "Доброе утро, ребята."

# game/status.rpy:6468
translate russia task15_ef51be08:

    # c "How can you be so chipper...?"
    c "Почему ты такая бодрая...?"

# game/status.rpy:6469
translate russia task15_5c947fb7:

    # s g55 "I guess I just didn't drink as much."
    s g55 "Я просто не пила так много."

# game/status.rpy:6470
translate russia task15_41bb7989:

    # s "What about you two? Do you remember last night?"
    s "А как насчёт вас двоих? Вы вообще помните вчерашний вечер?"

# game/status.rpy:6471
translate russia task15_af021985:

    # a y12 "It's fuzzy...."
    a y12 "Расплывчато..."

# game/status.rpy:6472
translate russia task15_82781c4b:

    # c r57 "We didn't do anything crazy, did we?"
    c r57 "Мы ведь не сделали ничего страшного, правда?"

# game/status.rpy:6473
translate russia task15_f1d8fa4f:

    # s g56 "You....{w} er... you ended up stripping naked and making out with each other as the crowd watched."
    s g56 "Вы....{w} эмм... всё закончилось тем, что вы разделись догола и целовались друг с другом под пристальным взглядом толпы."

# game/status.rpy:6474
translate russia task15_2a8b7d1a:

    # c r14 ".........................."
    c r14 ".........................."

# game/status.rpy:6475
translate russia task15_172238af:

    # a y28 ".........................."
    a y28 ".........................."

# game/status.rpy:6476
translate russia task15_385a4a33:

    # a "No wonder I can taste your cherry chapstick, Clover!"
    a "Неудивительно, что я чувствую вкус твоей вишневой помады, Кловер!"

# game/status.rpy:6477
translate russia task15_ae05a2bf:

    # c "I'm going to pretend I didn't hear that.{w} If you need me, I'll be in bed..."
    c "Я сделаю вид, что не слышал этого.{w} Если я вам понадоблюсь, я буду в постели..."

# game/status.rpy:6482
translate russia task15_90139d00:

    # y "Looks like your party was a success."
    y "Похоже, твоя вечеринка удалась."

# game/status.rpy:6483
translate russia task15_22abd0b7:

    # s g2 "Heh~..."
    s g2 "Хех~..."

# game/status.rpy:6484
translate russia task15_c9668e36:

    # s "I've checked my phone and its blown up with messages from the Aces."
    s "Я проверила свой телефон, он забит сообщениями от Тузов."

# game/status.rpy:6487
translate russia task15_97c8d65c:

    # s "You could say I'm pretty hot right now..."
    s "Можно сказать, что сейчас я довольно горяча..."

# game/status.rpy:6489
translate russia task15_156bf382:

    # s g1 "I even got a message from Candy, their leader."
    s g1 "Я даже получила сообщение от Кэнди, их лидера."

# game/status.rpy:6490
translate russia task15_5c66bf37:

    # y "Who is this Candy anyways?"
    y "Кто такая эта Кэнди?"

# game/status.rpy:6491
translate russia task15_8cd351af:

    # s "Candy Sweet. Real name is Margaret Nussbaum."
    s "Кэнди Свит. Настоящее имя - Маргарет Нуссбаум."

# game/status.rpy:6492
translate russia task15_3a56953f:

    # y "Continuing the crazy last name trend I see..."
    y "Продолжает тенденцию сумасшедших фамилий, как я вижу..."

# game/status.rpy:6493
translate russia task15_697ba249:

    # s g32 "She used to be a cheerleader coach, but ended up specializing in hypnosis and brainwashing."
    s g32 "Раньше она была тренером черлидеров, но в итоге стала специализироваться на гипнозе и промывании мозгов."

# game/status.rpy:6494
translate russia task15_d186058b:

    # s "I'm guessing her experience training ditzy rich girls, combined with her talent for brainwashing made her the perfect leader for the Aces."
    s "Полагаю, что её опыт обучения глупых богатых девочек в сочетании с талантом промывания мозгов сделал её идеальным лидером для Тузов."

# game/status.rpy:6495
translate russia task15_87df17a2:

    # y "So... Tim Scam was at the party yesterday."
    y "И так... Тим Скам был вчера на вечеринке."

# game/status.rpy:6496
translate russia task15_8d33fe49:

    # s g30 "{b}*Gulp*{/b}"
    s g30 "{b}*Глоть*{/b}"

# game/status.rpy:6497
translate russia task15_8822b75d:

    # s "Y-yeah... he was..."
    s "Д-да... был..."

# game/status.rpy:6498
translate russia task15_671584ff:

    # y "Something I should worry about?"
    y "Что-то, о чём я должен беспокоиться?"

# game/status.rpy:6499
translate russia task15_2b905675:

    # s g33 "No... Don't worry, I'm still going to help bring him down..."
    s g33 "Нет... Не волнуйся, я всё ещё помогаю его остановить..."

# game/status.rpy:6500
translate russia task15_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:6526
translate russia task16_6d7fa2fb:

    # s "I just got an invitation from the Aces leader to come see her!"
    s "Я только что получила приглашение от лидера Тузов!"

# game/status.rpy:6527
translate russia task16_39a50217:

    # s "She couldn't say what it was about, but I'm suppose to meet her in Spain."
    s "Она не смогла сказать, о чем идёт речь, но я должна встретиться с ней в Испании."

# game/status.rpy:6528
translate russia task16_66db06ab:

    # y "You're building up some serious frequent flyer miles."
    y "1Ты накопила серьезные бонусные мили для часто летающих пассажиров."

# game/status.rpy:6529
translate russia task16_776b3818:

    # s g1 "Yeah I know. Valencia is well worth visiting though."
    s g1 "Да, я знаю. Стоит посетить Валенсию."

# game/status.rpy:6530
translate russia task16_886fd2c0:

    # y "We've taken great strides in taking her down, but the fact that she wants to meet you alone makes me a bit worried."
    y "Мы добились больших успехов в борьбе с ней, но тот факт, что она хочет встретиться с тобой наедине, заставляет меня немного беспокоиться."

# game/status.rpy:6531
translate russia task16_7df820d2:

    # y "I'll put the other spies on stand-by just in case."
    y "Я приготовлю других шпионок на всякий случай."

# game/status.rpy:6532
translate russia task16_41698eea:

    # s g32 "That's not a bad idea. I've never actually interacted with Candy. If you don't count the times we fought her in the past."
    s g32 "Это неплохая идея. Я никогда не общалась с Кэнди. Если не считать тех случаев, когда мы сражались с ней в прошлом."

# game/status.rpy:6533
translate russia task16_82b67ee3:

    # y "You think it's a trap?"
    y "Думаешь это ловушка?"

# game/status.rpy:6534
translate russia task16_203a0436:

    # s g31 "Only one way to find out..."
    s g31 "Есть только один способ это узнать..."

# game/status.rpy:6536
translate russia task16_7ff89db1:

    # "Sam gets ready to leave for Spain."
    "Сэм готовится к отъезду в Испанию."

# game/status.rpy:6541
translate russia task16_bd868ed2:

    # y "Girls, we got a big mission planned. Sam is heading to Spain again to meet with the leader of the Aces."
    y "Девочки, у нас запланирована большая миссия. Сэм снова отправляется в Испанию, чтобы встретиться с лидером Тузов."

# game/status.rpy:6542
translate russia task16_a9212c00:

    # y "You're going in after her. Stay to the shadows and only interviene if something goes wrong."
    y "Вы пойдёте за ней. Оставайтесь в тени и вмешивайтесь, только если что-то пойдет не так."

# game/status.rpy:6543
translate russia task16_46574440:

    # c r31 "Right."
    c r31 "Понятно."

# game/status.rpy:6561
translate russia task16_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:6563
translate russia task16_5dbf6f68:

    # cM "[playerName]? We landed in Spain. Sam is already on her way to the Aces head quarters."
    cM "[playerName]? Мы приземлились в Испании. Сэм уже на пути в главный офис Тузов."

# game/status.rpy:6564
translate russia task16_97c3d1c8:

    # y "Good, keep an eye on her. Is Alex there with you?"
    y "Хорошо, присмотрите за ней. Алекс с тобой?"

# game/status.rpy:6565
translate russia task16_40e9e574:

    # aM "Hiya!"
    aM "КиЯ!"

# game/status.rpy:6566
translate russia task16_cb0a17ea:

    # y "Keep me up to date."
    y "Держите меня в курсе событий."

# game/status.rpy:6569
translate russia task16_14f4b337_1:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:6571
translate russia task16_4fdfc53f:

    # sM "[playerName] I'm going in. I'll keep the line open."
    sM "[playerName], я вхожу. Я оставлю линию открытой."

# game/status.rpy:6581
translate russia task16_ffcfcc12:

    # "???" "Ah Sam, you made it..."
    "???" "Ах, Сэм, ты добралась..."

# game/status.rpy:6582
translate russia task16_cbebc419:

    # s "Candy! It's good to see you!"
    s "Кэнди! Рада тебя видеть!"

# game/status.rpy:6583
translate russia task16_002f2d16:

    # candy "Likewise. I heard you did a lot of good work for the Aces."
    candy "Аналогично. Я слышала, ты много сделала для Тузов."

# game/status.rpy:6584
translate russia task16_02bff758:

    # s g57 "Yeah.... listen Candy about before..."
    s g57 "Даа... слушай Кэнди о том, что было раньше..."

# game/status.rpy:6586
translate russia task16_7fe65149:

    # candy "Before? You mean how you girls busted my ass all those years ago?"
    candy "Раньше? Ты имеешь в виду, тот раз когда вы, девочки, надрали мне задницу?"

# game/status.rpy:6587
translate russia task16_c3d7abb3:

    # s "..................."
    s "..................."

# game/status.rpy:6588
translate russia task16_fb8c66fe:

    # candy "Water under the bridge. I'd rather have you on our side than as my enemy."
    candy "Всё в прошлом. Я бы предпочла, чтобы вы были на нашей стороне, а не в качестве наших врагов."

# game/status.rpy:6590
translate russia task16_6f604fd2:

    # candy "You are on our side, right?"
    candy "Ты ведь на нашей стороне, так?"

# game/status.rpy:6591
translate russia task16_011b346c:

    # s g56 "Of course!"
    s g56 "Конечно!"

# game/status.rpy:6592
translate russia task16_d14c1908:

    # candy "Well I'll let your actions speak for themselves.{w} Now it's time we had a little chat..."
    candy "Что ж, пусть твои действия говорят сами за себя.{w} Пришло время немного поболтать..."

# game/status.rpy:6593
translate russia task16_0ca1f3ff:

    # s g57 "A little... chat?"
    s g57 "Немного... поболтать?"

# game/status.rpy:6606
translate russia task16_5a792ada:

    # y "What the...?"
    y "Что за...?"

# game/status.rpy:6607
translate russia task16_606ff7a9:

    # y "Girls, I lost connection with Sam! Switching to your camera now."
    y "Девочки, я потерял связь с Сэм! Переключаюсь на вашу камеру."

# game/status.rpy:6635
translate russia task16_22c7b413:

    # "Intercom: Intruder warning! Switching to high alert."
    "Интерком: Предупреждение о вторжении! Переход в режим повышенной готовности."

# game/status.rpy:6636
translate russia task16_5cfe7d54:

    # y "They know we're here..."
    y "Они знают что мы здесь..."

# game/status.rpy:6637
translate russia task16_8673e1f0:

    # y "We lost contact with Sam. Fight your way through the castle and get her out of there!"
    y "Мы потеряли связь с Сэм. Пробейте себе путь через замок и вытащите её оттуда!"

# game/status.rpy:6638
translate russia task16_90335598:

    # cM "Right!"
    cM "Понятно!"

# game/status.rpy:6661
translate russia task16_82506977:

    # a "Sammy! Are you all right?!"
    a "Сэмми! Ты в порядке?!"

# game/status.rpy:6662
translate russia task16_a3dc8011:

    # s g14 "Y-yeah... I'm fine.{w} We walked right into her trap."
    s g14 "Д-да... всё хорошо.{w} Мы попали прямо в её ловушку."

# game/status.rpy:6663
translate russia task16_00edf7e9:

    # c r54 "This ends now. With Candy defeated, the Aces will have lost all of their leaders. We'll be rid of the gang once and for all."
    c r54 "Скоро всё закончится. После поражения Кэнди, Тузы потеряют всех своих лидеров. Мы избавимся от этой банды раз и навсегда."

# game/status.rpy:6667
translate russia task16_cf0da24d:

    # candy "{b}*Pant* *Pant*{/b} {w}It seems we've been here before...."
    candy "{b}*Одышка* *Одышка*{/b} {w}Что-то это мне напоминает..."

# game/status.rpy:6668
translate russia task16_43d5fe3d:

    # a y53 "Give up Candy! We're taking you back to prison!"
    a y53 "Сдавайся Кэнди! Мы вернём тебя за решётку!"

# game/status.rpy:6669
translate russia task16_aa3494bd:

    # candy "That's what you think..."
    candy "Это вы так думаете..."

# game/status.rpy:6670
translate russia task16_6942df97:

    # candy "*{b}Crack{/b}*"
    candy "*{b}Треск{/b}*"

# game/status.rpy:6672
translate russia task16_ace8a0e3:

    # c r56 "Candy...?"
    c r56 "Кэнди...?"

# game/status.rpy:6673
translate russia task16_a7edb4f8:

    # "Suddenly the gang boss drops to the ground."
    "Вдруг главарь банды падает на пол."

# game/status.rpy:6674
translate russia task16_5385d09e:

    # y "It's a suicide tooth! Quickly help her!"
    y "Это капсула с ядом! Быстро помогите ей!"

# game/status.rpy:6675
translate russia task16_1dfc9ea5:

    # s "Candy!"
    s "Кэнди!"

# game/status.rpy:6677
translate russia task16_2f6b8c07:

    # "The girls tackle Candy and after a short struggle, manage to help her before she ingests too much of the poison."
    "Девочкам удаётся схватить Кэнди и помочь ей, прежде чем она проглотит слишком много яда."

# game/status.rpy:6682
translate russia task16_bcb021d1:

    # c r14 "She's okay..."
    c r14 "Она в порядке..."

# game/status.rpy:6683
translate russia task16_9b968412:

    # y "What on earth did WOOHP do to her to make her choose death over imprisonment?!"
    y "Что такого с ней сделало WOOHP, чтобы она предпочла смерть тюремному заключению?!"

# game/status.rpy:6684
translate russia task16_ad941c98:

    # c r32 "I don't know... The poison knocked her out, let's get her back to Beverly Hills."
    c r32 "Я не знаю... Яд вырубил её, нужно вернуть её в Беверли-Хиллз."

# game/status.rpy:6685
translate russia task16_d7ddfd1c:

    # y "We can treat her back at the base. You girls head back home."
    y "Мы сможем вылечить её на базе. Девочки, возвращайтесь домой."

# game/status.rpy:6686
translate russia task16_a765caeb:

    # "Battered, but victorious, the girls nod and return back to America."
    "Потрёпанные, но победившие, девушки кивают и возвращаются в Америку."

# game/status.rpy:6691
translate russia task16_9ca1a61b:

    # y "Welcome back girls."
    y "С возвращением."

# game/status.rpy:6698
translate russia task16_602058c3:

    # s "Margaret regained consciousness during the trip. We put her in one of the cells, but it's not likely she'll cooperate."
    s "Во время полёта Маргарет пришла в себя. Мы поместили её в одну из камер, но вряд ли она будет сотрудничать."

# game/status.rpy:6699
translate russia task16_7d73bf6e:

    # c "Talk to her whenever you have time, [playerName]. She might be able to tell us more about the Mastermind."
    c "Поговори с ней, когда у тебя будет время, [playerName]. Возможно, она сможет рассказать нам больше об Организаторе."

# game/status.rpy:6700
translate russia task16_89e47e1b:

    # y "Good work girls. Head back to your cells for now."
    y "Отличная работа, девочки. Возвращайтесь пока в свои камеры."

# game/status.rpy:6705
translate russia task16_d92c097f:

    # y "Are you all right, [samNick]?"
    y "Ты в порядке, [samNick]?"

# game/status.rpy:6706
translate russia task16_6da1bcc4:

    # s g5 "Yeah... the news of Candy's arrest will spread and the Aces should disband soon..."
    s g5 "Да... новость об аресте Кэнди распространится, и Тузы должны будут скоро распасться..."

# game/status.rpy:6707
translate russia task16_70e3efd6:

    # y "No more living the high-life."
    y "Больше не надо жить на широкую ногу."

# game/status.rpy:6708
translate russia task16_6a3d958d:

    # s "Yeah... which is probably a good thing. Thanks for coming to help me, [playerName]."
    s "Да... наверное это хорошо. Спасибо, что пришёл мне на помощь, [playerName]."

# game/status.rpy:6709
translate russia task16_4ab325b1:

    # y "Don't mention it."
    y "Не надо об этом."

# game/status.rpy:6719
translate russia task16_dfd5ecd9:

    # y "Are you awake...?"
    y "Ты не спишь...?"

# game/status.rpy:6722
translate russia task16_95bb6a2a:

    # candy "I am... let's just get it over with..."
    candy "Нет... давай просто покончим с этим..."

# game/status.rpy:6723
translate russia task16_e00cc514:

    # y "Over with?"
    y "Покончим с чем?"

# game/status.rpy:6724
translate russia task16_022ce1ee:

    # candy "Don't play coy with me! I remember the last time you captu-....{b}*cough* *cough*{/b}"
    candy "Не притворяйся дураком! Я помню, прошлый раз когда ты пойма-....{b}*кхе* *кхе*{/b}"

# game/status.rpy:6725
translate russia task16_e3e5bbe9:

    # y "Easy there. We nearly lost you to your little stunt. Who still uses suicide teeth now a days?"
    y "Успокойся. Мы чуть не потеряли тебя из-за твоего маленького трюка. Кто в наши дни всё ещё использует капсулы с ядом?"

# game/status.rpy:6726
translate russia task16_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:6727
translate russia task16_c8ff62bb:

    # y "Oh right, I work for a spy agency."
    y "Ах да, я же работаю в шпионском агентстве."

# game/status.rpy:6728
translate russia task16_53361082:

    # candy "Stop joking around! I know what you're planning!"
    candy "Завязывай с шутками! Я знаю, что вы планируете!"

# game/status.rpy:6730
translate russia task16_e2848483:

    # s "How is she doing?"
    s "Как она?"

# game/status.rpy:6731
translate russia task16_dd93d99d:

    # y "Well she's awake, but she seems to know I'm gonna bone her."
    y "Она в сознании, но похоже знает, что я собираюсь выебать её."

# game/status.rpy:6732
translate russia task16_7140ce42:

    # s g28 "What?"
    s g28 "Что?"

# game/status.rpy:6734
translate russia task16_899728a4:

    # candy "What?"
    candy "Что?"

# game/status.rpy:6735
translate russia task16_ce895a18:

    # y "..........."
    y "..........."

# game/status.rpy:6736
translate russia task16_ac17dec1:

    # y "What?"
    y "Что?"

# game/status.rpy:6737
translate russia task16_1c44fc01:

    # y "That's what you were talking about, right?"
    y "Это же то, о чём ты говорила, ведь так?"

# game/status.rpy:6739
translate russia task16_e4cd6458:

    # candy "!!!"
    candy "!!!"

# game/status.rpy:6740
translate russia task16_561b55d2:

    # candy "I was talking about the torture!"
    candy "Я говорила о пытках!"

# game/status.rpy:6741
translate russia task16_43cc8ea3:

    # y "Torture? Nobody's getting tortured!"
    y "Пытки? Мы не собирались никого пытать!"

# game/status.rpy:6742
translate russia task16_db0150f0:

    # candy "Don't lie to me agent. I've already gone through it once when your spies captured me the first time!"
    candy "Не лги мне, агент. Я уже прошла через это однажды, когда твои шпионы схватили меня в первый раз!"

# game/status.rpy:6743
translate russia task16_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:6744
translate russia task16_23ff401c:

    # y "Sam~...."
    y "Сэм~...."

# game/status.rpy:6745
translate russia task16_ed588f48:

    # s g29 "I... I don't know what she's talking about!"
    s g29 "Я... я не знаю о чём она говорит!"

# game/status.rpy:6746
translate russia task16_06b3dff4:

    # candy "Stop playing games! I know what your organization is really about!"
    candy "Хватит этих игр! Я знаю, что именно представляет ваша организация!"

# game/status.rpy:6747
translate russia task16_c30e38a6:

    # candy "Why do you think I was so desperate to take revenge?!"
    candy "Почему вы думаете, я была так отчаянно настроена на месть?!"

# game/status.rpy:6748
translate russia task16_5a3165e1:

    # s g31 "Don't listen to her, [playerName]. WOOHP doesn't torture its prisoners."
    s g31 "Не слушай её, [playerName]. WOOHP не пытает своих заключенных."

# game/status.rpy:6749
translate russia task16_98c42f00:

    # candy "Of course it does! Why do you think this whole operation got started?! Why do you think all of us want to take revenge so badly?!"
    candy "Конечно пытает! Из-за чего вы думаете вся эта операция началась?! Почему вы думаете, мы все так сильно хотим вам отомстить?!"

# game/status.rpy:6750
translate russia task16_2bf1219b:

    # candy "You even turn on your own friends and allies! That's why she organized this whole take-over."
    candy "Вы даже настраиваете против себя своих друзей и союзников! Вот почему она организовала всё это."

# game/status.rpy:6751
translate russia task16_4c85ac83:

    # s g16 "She?"
    s g16 "Она?"

# game/status.rpy:6752
translate russia task16_f80f123d:

    # y "The Mastermind is a woman?"
    y "Организатор - девушка?"

# game/status.rpy:6753
translate russia task16_9972a589:

    # candy "I've said enough... You won't get a word out of me."
    candy "Я уже достаточно сказала... Вы не получите от меня ни слова."

# game/status.rpy:6754
translate russia task16_5e9a5fa6:

    # s "She doesn't seem to be nanobot controlled, [playerName]..."
    s "Похоже, она не под контролем наноботов, [playerName]..."

# game/status.rpy:6755
translate russia task16_d115945d:

    # y "Yeah... I think it's best if we just leave her be."
    y "Да... думаю, будет лучше, если мы просто оставим её в покое."

# game/status.rpy:6756
translate russia task16_f6f5a3d7:

    # candy "........................................"
    candy "........................................"

# game/status.rpy:6764
translate russia task16_4cc055d7:

    # "Computer" "Computer: Candy Sweet.\nStatus: Captured."
    "Компьютер" "Компьютер: Кэнди Свит.\nСтатус: Захвачена."

# game/status.rpy:6765
translate russia task16_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:6771
translate russia task16_1a1c0f7e:

    # "New clothing options for Sam are available at the mall."
    "Новые варианты одежды для Сэм можно приобрести в торговом центре."

# game/status.rpy:6778
translate russia task16_39e00ce9:

    # y "Well... that was interesting."
    y "Что ж... это было интересно."

# game/status.rpy:6779
translate russia task16_143c7c30:

    # s "Don't listen to anything she has to say, [playerName]. She's obviously trying to get to us!"
    s "Не слушай её бредни, [playerName]. Она явно пытается задеть нас!"

# game/status.rpy:6780
translate russia task16_d314db9d:

    # a "WOOHP doesn't really torture its prisoners, right...?"
    a "WOOHP ведь не пытает своих заключенных, да...?"

# game/status.rpy:6781
translate russia task16_a8a94982:

    # s g38 "Of course not! It even says so in the WOOHP guidelines."
    s g38 "Конечно, нет! Об этом даже говорится в руководстве WOOHP."

# game/status.rpy:6782
translate russia task16_afdd72c7:

    # c r11 "The guidelines also say that WOOHP doesn't interfere with civillians and we know that's not true. The fact that we're spies is proof of that."
    c r11 "В руководстве также говорится, что WOOHP не вмешивается в дела гражданских лиц, а мы знаем что это не так. Тот факт, что мы шпионы, является прямым доказательством."

# game/status.rpy:6783
translate russia task16_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:6784
translate russia task16_f2495b78:

    # c r10 "What?"
    c r10 "Что?"

# game/status.rpy:6785
translate russia task16_c1e6674f:

    # y "Sam... do you remember when we just moved in here and we checked out the basement...?"
    y "Сэм... помнишь, когда мы только переехали сюда и осмотрели подвал...?"

# game/status.rpy:6786
translate russia task16_2dfbc557:

    # s g30 "!!!"
    s g30 "!!!"

# game/status.rpy:6787
translate russia task16_ff8bb5a8:

    # s "Oh no..."
    s "О нет..."

# game/status.rpy:6788
translate russia task16_3eacfdae:

    # y "Seems WOOHP might be into some much darker stuff than we thought."
    y "Похоже, что WOOHP может быть замешана в гораздо более тёмных вещах, чем мы думали."

# game/status.rpy:6789
translate russia task16_53749e69:

    # s "No, Jerry would never allow that!"
    s "Нет, Джерри никогда бы этого не допустил!"

# game/status.rpy:6790
translate russia task16_7cda6045:

    # a y38 "Exactly! The bad guys must just be lying to us!"
    a y38 "Точно! Злодеи должно быть, просто обманывают нас!"

# game/status.rpy:6791
translate russia task16_64a2b68e:

    # y "..........................."
    y "..........................."

# game/status.rpy:6792
translate russia task16_cc46b7b3:

    # y "We'll figure it out when we free your boss."
    y "Мы разберёмся с этим, когда освободим вашего босса."

# game/status.rpy:6843
translate russia task17_71e2e99e:

    # c "[playerName] you gotta help me!"
    c "[playerName] ты должен мне помочь!"

# game/status.rpy:6844
translate russia task17_bc03cf3d:

    # y "What? What's wrong?"
    y "Что? Что не так?"

# game/status.rpy:6845
translate russia task17_43303afb:

    # c "It's Drift Punk... Some of them have invited me over to play games and watch animo with them!"
    c "Дрифт Панк... Они пригласили меня поиграть с ними в игры и посмотреть \"анимо\"!"

# game/status.rpy:6846
translate russia task17_4b17614f:

    # y "That doesn't sound too b-..."
    y "Звучит не так уж и пло-..."

# game/status.rpy:6847
translate russia task17_4148d509:

    # c "I don't know how to play video games!{w} And I have no idea what animo is!"
    c "Я не умею играть в видеоигры!{w} И я понятия не имею, что такое \"анимо\"!"

# game/status.rpy:6848
translate russia task17_f7cb26c0:

    # y "Just relax. I'm sure they'll explain everything once you get there."
    y " Просто расслабься. Я уверен, что они всё объяснят, когда ты прейдёшь туда."

# game/status.rpy:6849
translate russia task17_9317969e:

    # c "Ugh... I just know that I'm going to hate this..."
    c "Угх... Я уже знаю, что буду ненавидеть это..."

# game/status.rpy:6863
translate russia task17_16c8fa78:

    # c "I'm back..."
    c "Я вернулась..."

# game/status.rpy:6864
translate russia task17_f6876453:

    # y "Had a fun time with the Punks?"
    y "Весело провела время с Панками?"

# game/status.rpy:6865
translate russia task17_3a087ba2:

    # c r17 "Don't even get me started! It was awful!"
    c r17 "Даже не начинай! Это было ужасно!"

# game/status.rpy:6866
translate russia task17_372a25ef:

    # c r42 "They made me watch Japanese cartoons and play these awful games!"
    c r42 "Они заставляли меня смотреть японские мультики играть в эти ужасные игры!"

# game/status.rpy:6867
translate russia task17_2d7e7952:

    # y "That doesn't sound so bad."
    y "Звучит не так уж и плохо."

# game/status.rpy:6868
translate russia task17_591c7d28:

    # c r16 "I mean... It's better than robbing banks, but it was just so nerdy~...!"
    c r16 "Нуу... Это лучше, чем грабить банки, но это было так занудно~...!"

# game/status.rpy:6869
translate russia task17_05cdf1ce:

    # c r11 "..................."
    c r11 "..................."

# game/status.rpy:6870
translate russia task17_1c396f56:

    # c "At least some of the game characters looked cool..."
    c "По крайней мере, некоторые игровые персонажи выглядели круто..."

# game/status.rpy:6871
translate russia task17_f50016f2:

    # c r12 "Of course they had to make it weird and said one of the characters looked like me."
    c r12 "Они сказали что один из персонажей похож на меня, это так странно."

# game/status.rpy:6872
translate russia task17_baf70fb3:

    # y "Remember, you're undercover. At least pretend to be interested in them."
    y "Помни, ты под прикрытием. По крайней мере, притворись, что тебя это интересуют."

# game/status.rpy:6873
translate russia task17_ff8be11a:

    # c r42 "{b}*Whines*{/b} But one of them showed off his D20 collection!"
    c r42 "{b}*Хнычет*{/b} А один из них показал мне свою коллекцию D20!"

# game/status.rpy:6874
translate russia task17_4a01a2fd:

    # y "D20?"
    y "D20?"

# game/status.rpy:6875
translate russia task17_b7369a87:

    # c r29 "Er... I mean his dice collection...{w} You know, for playing nerd games."
    c r29 "Эмм... Я имею в виду его коллекцию кубиков...{w} Ну знаешь, для этих гиковских игры."

# game/status.rpy:6876
translate russia task17_779b25d6:

    # y "You poor thing. Okay head back to your cell for now. You're going back tomorrow."
    y "Бедняжка. Ладно, возвращайся пока в камеру. Завтра отправлю тебя снова."

# game/status.rpy:6877
translate russia task17_3e2dfaa6:

    # "Clover nods and returns to her cell."
    "Кловер кивает и возвращается в свою камеру."

# game/status.rpy:6888
translate russia task17_910d0563:

    # c "{b}*Grumbles*{/b}"
    c "{b}*Ворчит*{/b}"

# game/status.rpy:6889
translate russia task17_c164ca17:

    # y "Uh oh..."
    y "Хмм..."

# game/status.rpy:6890
translate russia task17_5a17c102:

    # c "They made me watch anime again..."
    c "Они снова заставили меня смотреть аниме..."

# game/status.rpy:6891
translate russia task17_fd255b21:

    # y "Anime?"
    y "Атиме?"

# game/status.rpy:6892
translate russia task17_3748c5bd:

    # c r12 "Those Japanese cartoons I told you about."
    c r12 "Те японские мультики, о которых я тебе рассказывал."

# game/status.rpy:6893
translate russia task17_d962423a:

    # y "And...?"
    y "И...?"

# game/status.rpy:6894
translate russia task17_b49a8694:

    # c "I don't know. I was zoning out half of the time."
    c "Не знаю. Большую часть времени я провела в полудрёме."

# game/status.rpy:6895
translate russia task17_5fbf77ae:

    # c "Something about a magical cat girl or something..."
    c "Что-то о волшебной девушке-кошке или что-то в этом роде..."

# game/status.rpy:6896
translate russia task17_bce9f47c:

    # c "We also played this tabletop game with miniatures."
    c "А ещё мы играли в настольную игру с миниатюрами."

# game/status.rpy:6897
translate russia task17_cbd56499:

    # c r10 "............................"
    c r10 "............................"

# game/status.rpy:6898
translate russia task17_9469f9e5:

    # c "Are you gonna send me back there again...?"
    c "Ты собираешься снова отправить меня туда...?"

# game/status.rpy:6899
translate russia task17_9110359f:

    # y "That's the plan."
    y "Таков план."

# game/status.rpy:6900
translate russia task17_8496b542:

    # c r11 "...................."
    c r11 "...................."

# game/status.rpy:6910
translate russia task17_16f277c4:

    # y "Still watching your animos?"
    y "Всё ещё смотришь свои \"анимо\"?"

# game/status.rpy:6911
translate russia task17_7405fb7c:

    # c "It's anime...{w} and yes... we're still watching those..."
    c "Аниме...{w} и да... мы всё ещё их смотрим..."

# game/status.rpy:6912
translate russia task17_bcfcc9da:

    # y "I'm sorry for putting you through all that."
    y "Прости, что заставляю тебя пройти через всё это."

# game/status.rpy:6913
translate russia task17_75aee599:

    # c r12 "It...{w} It's fine..."
    c r12 "Всё...{w} Всё в порядке..."

# game/status.rpy:6914
translate russia task17_45fa5b64:

    # y "I mean... having a girl like you play video games and read Japanese comics. It feels almost sadistic of me!"
    y "Нуу... заставлять такую девушку как ты, играть в видеоигры и читать японские комиксы. Это почти садизм с моей стороны!"

# game/status.rpy:6915
translate russia task17_20d87ba7:

    # c r11 "................"
    c r11 "................"

# game/status.rpy:6916
translate russia task17_51965c69:

    # y "Putting a Valley girl in a situation like that! Surrounded by strange hobbies and foreign cartoons!"
    y "Типичная блондинка, в такой ситуации! В окружении странных увлечений и иностранных мультфильмов!"

# game/status.rpy:6917
translate russia task17_dc7b67a4:

    # y "Okay, I've made up my mind! No more Drift Punk missions. From now on I'll just send you undercover somewhere else."
    y "Хорошо, я принял решение! Больше никаких миссий у Дрифт Панков. С этого момента я отправлю тебя под прикрытием в другое место."

# game/status.rpy:6918
translate russia task17_078f362c:

    # c r28 "W-wait... what?"
    c r28 "П-подожди... что?"

# game/status.rpy:6919
translate russia task17_23d8222c:

    # y "I know how much you dislike nerdy stuff, so I promise to not send you out there anymore."
    y "Я знаю, как сильно ты не любишь всю эту чушь, поэтому обещаю больше не посылать тебя туда."

# game/status.rpy:6920
translate russia task17_47aec323:

    # c r30 "N-no...{w} We have to think of the mission...!"
    c r30 "Н-нет...{w} Нам нужно думать о нашей миссии...!"

# game/status.rpy:6921
translate russia task17_913b6fac:

    # c r29 "Heh~... Heh~... maybe I was just overreacting a bit...{w} Obviously taking down Drift Punk is much more important. So you can keep sending me undercover there."
    c r29 "Хех~... Хех~... может я немного переборщила...{w} Очевидно, что уничтожение Дрифт Панков гораздо важнее. Так что ты можешь продолжать посылать меня туда под прикрытием."

# game/status.rpy:6922
translate russia task17_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:6923
translate russia task17_389cc703:

    # c r38 "It's a big sacrifice, but I'm willing to make it! For the greater good and for my friends!"
    c r38 "Это большая жертва, но я готова на неё! Для общего блага и ради моих друзей!"

# game/status.rpy:6924
translate russia task17_66b91dbc:

    # y "For your friends...?"
    y "Ради твоих друзей...?"

# game/status.rpy:6925
translate russia task17_7cbaf5eb:

    # y "Well if you're that dedicated, then I guess I'll send you undercover with them again..."
    y "Ну, раз уж ты такая целеустремлённая, тогда я, наверное, снова отправлю тебя к ними..."

# game/status.rpy:6926
translate russia task17_1e87fef1:

    # c r1 "R-right...!"
    c r1 "Х-хорошо...!"

# game/status.rpy:6927
translate russia task17_ce619241:

    # "Clover grabs her things and returns to her cell."
    "Кловер собирает свои вещи и возвращается в камеру."

# game/status.rpy:6931
translate russia task17_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:6932
translate russia task17_760d0bb3:

    # y "Well that was a little suspicious..."
    y "Ну, это было немного подозрительно..."

# game/status.rpy:6940
translate russia task17_d6a2c412:

    # y "Clover, welcome back. How did your undercover mission go with Drift Punk?"
    y "Кловер, с возвращением. Как прошёл твой день у Дрифт Панков?"

# game/status.rpy:6941
translate russia task17_7bce46d6:

    # c "Oh! Y-yeah it went great.{w} Y'know... gathered some intel, robbed some bankaccount... the usual..."
    c "Ох! Д-да, всё прошло отлично.{w} Типа... раздобыла кое-какие сведения, ограбили кое-какие банковские счета... как обычно..."

# game/status.rpy:6942
translate russia task17_e2aa08fa:

    # y "Are you hiding something behind your back...?"
    y "Ты что-то прячешь за спиной...?"

# game/status.rpy:6943
translate russia task17_27e38871:

    # c r30 "N-no! Of course n-..."
    c r30 "Н-нет! Конечно не-..."

# game/status.rpy:6946
translate russia task17_86b45870:

    # "Clover dropped something on the ground."
    "Кловер роняет что-то на землю."

# game/status.rpy:6947
translate russia task17_99cdebe6:

    # y "Are these... sewing manuals?{w} And anime figurines...?"
    y "Это... руководства для шитья?{w} И аниме-фигурки...?"

# game/status.rpy:6948
translate russia task17_65e83399:

    # c r28 "It's not what it seems!"
    c r28 "Это не то, чем кажется!"

# game/status.rpy:6949
translate russia task17_c587e2e8:

    # y "I don't even know what it seems!"
    y "Я даже не знаю, чем это должно казаться!"

# game/status.rpy:6950
translate russia task17_62c51673:

    # c r29 "I just f-figured I'd do some extra research on Drift Punk culture, you know. So I can more easily talk to them about their hobbies and interests!"
    c r29 "Я просто решила провести дополнительное исследование культуры Дрифт Панков. Так мне будет легче говорить с ними об их увлечениях и интересах!"

# game/status.rpy:6951
translate russia task17_14e4af5c:

    # y "And... those hobbies include sewing...?"
    y "Ии... они увлекаются шитьём...?"

# game/status.rpy:6952
translate russia task17_58f3e502:

    # c r18 "........................{w} Yes."
    c r18 "........................{w} Да."

# game/status.rpy:6953
translate russia task17_f28802ce:

    # y ".................................."
    y ".................................."

# game/status.rpy:6954
translate russia task17_c2271286:

    # y "Well alright! I see nothing strange about this! Off you go now."
    y "Отлично! Не вижу в этом ничего странного! Теперь можешь идти."

# game/status.rpy:6955
translate russia task17_c4da5b22:

    # "Clover quickly grabs her supplies and rushes off to her cell."
    "Кловер быстро хватает свои веши и спешит в комнату."

# game/status.rpy:6966
translate russia task17_8a5920d0:

    # y "Welcome back C-....{w} What's with that look?"
    y "С возвращением К-...{w} Что это за взгляд?"

# game/status.rpy:6967
translate russia task17_216b1a02:

    # c "Can.... can I borrow $100 bucks from you?"
    c "Могу.... могу я занять у тебя 100 баксов?"

# game/status.rpy:6968
translate russia task17_059f5e62:

    # y "What for...?"
    y "Зачем...?"

# game/status.rpy:6969
translate russia task17_06b5123e:

    # c "................."
    c "................."

# game/status.rpy:6970
translate russia task17_1f9a23a9:

    # c "I... er..."
    c "Мне... эм..."

# game/status.rpy:6971
translate russia task17_98477950:

    # c "I need to buy some fabrics..."
    c "Мне нужно купить немного ткани..."

# game/status.rpy:6972
translate russia task17_fa94dec9:

    # y "Fabrics...? What do you need fabrics for...?"
    y "Ткани...? Для чего тебе нужна ткань...?"

# game/status.rpy:6973
translate russia task17_290371c2:

    # c r15 "Well I er... actually never mind! I just remembered that I still have enough. Sorry!"
    c r15 "Ну, я... ладно, неважно! Я только что вспомнила, что у меня ещё осталось немного. Прости!"

# game/status.rpy:6974
translate russia task17_8ef8d0fb:

    # y "Clover, what are y-...?"
    y "Кловер, что т-...?"

# game/status.rpy:6978
translate russia task17_045b6dcc:

    # "Quickly hurried to her cell, but dropped something along the way."
    "Кловер быстро поспешила к своей камере, но по дороге что-то уронила."

# game/status.rpy:6979
translate russia task17_fcf8174a:

    # y "Clover! You dropped your....{w} skirt?"
    y "Кловер! Ты уронила....{w} Юбку?"

# game/status.rpy:6980
translate russia task17_e83dd1e9:

    # y "What's that girl up to....? Maybe I should follow her next time she goes undercover with Drift Punk..."
    y "Чем она там занимается...? Может, мне стоит последовать за ней в следующий раз, когда она будет работать у Дрифт Панков..."

# game/status.rpy:6988
translate russia task17_ea8d8c4a:

    # cM "Okay, I'll be heading off to the Punks again today!"
    cM "Ладно, я снова пошла к Панкам!"

# game/status.rpy:6989
translate russia task17_29c2d418:

    # "Clover heads off to the Dift Punks."
    "Кловер отправляется к Дифт Панкам."

# game/status.rpy:6991
translate russia task17_29a0f1ea:

    # y "......................................"
    y "......................................"

# game/status.rpy:6992
translate russia task17_e0c7a83e:

    # y "(What are you up to, Clover~....)"
    y "(Что ты задумала, Кловер~....)"

# game/status.rpy:6998
translate russia task17_cf0304ad:

    # c "......................................"
    c "......................................"

# game/status.rpy:7001
translate russia task17_15b27997:

    # y "She went into the convention building..."
    y "Она вошла в здание конвенции..."

# game/status.rpy:7006
translate russia task17_215ea3ee:

    # y "....?"
    y "....?"

# game/status.rpy:7007
translate russia task17_5a792ada:

    # y "What the...?"
    y "Что за...?"

# game/status.rpy:7008
translate russia task17_5cae1e55:

    # "Nerdy Guy" "Season two was {i}so{/i} much better."
    "Гик парень" "Второй сезон {i}намного{/i} лучше."

# game/status.rpy:7009
translate russia task17_65227a20:

    # "Nerdy Girl" "Oh please, the quality went down so much after they outsourced the animation to a different company!"
    "Гик девушка" "Ой, пожалуйста, качество настолько упало после того, как они передали анимацию другой компании!"

# game/status.rpy:7010
translate russia task17_8b89d0a0:

    # y "Is this....?"
    y "Это...?"

# game/status.rpy:7011
translate russia task17_ff06a685:

    # "Gamer" "Have you heard about the sequal to the game?"
    "Геймер" "Ты слышал про её сиквел?"

# game/status.rpy:7012
translate russia task17_5d33bc11:

    # "Gamer" "I hear it's going to have jiggle physics!"
    "Геймер" "Я слышал, что там будет физика покачивания!"

# game/status.rpy:7013
translate russia task17_fae098ad:

    # y "It's an anime convention...!"
    y "Это аниме-конвенция...!"

# game/status.rpy:7014
translate russia task17_753753fc:

    # y "What is Clover doing here...?"
    y "Что Кловер здесь делает...?"

# game/status.rpy:7026
translate russia task17_84a2151e:

    # "You begin looking around for your spy."
    "Вы начинаете оглядываться в поисках шпионки."

# game/status.rpy:7032
translate russia task17_90863bc9:

    # y "Excuse me miss. Have you seen a blonde girl around here? About your height, your eye color, your posture, you're...."
    y "Простите, мисс. Вы не видели здесь блондинку? Примерно вашего роста, с вашим цветом глаз, осанкой, с ваш-...."

# game/status.rpy:7033
translate russia task17_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:7034
translate russia task17_d2a764da:

    # y "Clover...?"
    y "Кловер...?"

# game/status.rpy:7035
translate russia task17_89a265f4:

    # c r28 "Er...! N-no I'm not Clover~....!"
    c r28 "Эм...! Н-нет я не Кловер~....!"

# game/status.rpy:7036
translate russia task17_6ed7a0ce:

    # y "You're into anime?!"
    y "Ты увлекаешься аниме?!"

# game/status.rpy:7037
translate russia task17_68884dd2:

    # c r14 "Shhh! Not so loud!"
    c r14 "Тссс! Не так громко!"

# game/status.rpy:7038
translate russia task17_39534934:

    # c "..............."
    c "..............."

# game/status.rpy:7039
translate russia task17_6e7662d8:

    # c r15 "{b}*Sighs*{/b} Well.... you caught me."
    c r15 "{b}*Вздох*{/b} Ну да.... ты меня поймал."

# game/status.rpy:7040
translate russia task17_b7c1442a:

    # y "Clover, what are you doing here...? Aren't you suppose to be undercover with Drift Punk?"
    y "Кловер, что ты здесь делаешь...? Разве ты не должна быть под прикрытием у Дрифт Панков?"

# game/status.rpy:7041
translate russia task17_e2090944:

    # "Taking off her wig and mask, Clover embarrassingly looks at you."
    "Сняв парик и маску, Кловер смущённо смотрит на вас."

# game/status.rpy:7048
translate russia task17_c2be8bbd:

    # c "............................."
    c "............................."

# game/status.rpy:7049
translate russia task17_2be05408:

    # c "I... sorta ditched undercover work to come here..."
    c "Я... получается я променяла работу под прикрытием на эту конвенцию..."

# game/status.rpy:7050
translate russia task17_96b6c75c:

    # y "I thought you hated nerd stuff...?"
    y "Я думал, ты ненавидишь всю эту чушь...?"

# game/status.rpy:7051
translate russia task17_bfb95c74:

    # c "Well... that wasn't completely true...{w} actually... I kinda...{w} love it..."
    c "Ну... это была не совсем правда...{w} вообще-то... мне это вроде как...{w} нравится..."

# game/status.rpy:7052
translate russia task17_7c2f37a4:

    # c r42 "Drift Punk introduced me to this whole new world and... and well... it's really fun."
    c r42 "Дрифт Панки познакомил меня с совершенно новым миром и... нуу это... это довольно весело."

# game/status.rpy:7053
translate russia task17_319a73fb:

    # c "B-but it would ruin my reputation if anyone knew!"
    c "Н-но, если кто-нибудь об этом узнает, моя репутация будет разрушена!"

# game/status.rpy:7054
translate russia task17_04f45869:

    # c "Please, [playerName]. Don't tell anyone!"
    c "Пожалуйста, [playerName]. Никому не говори!"

# game/status.rpy:7055
translate russia task17_e8c4b8d7:

    # y "Not even Sam and Alex?"
    y "Даже Сэм и Алекс?"

# game/status.rpy:7056
translate russia task17_e3d9e579:

    # c "No, not even them!"
    c "Нет, даже им!"

# game/status.rpy:7059
translate russia task17_a600b577:

    # y "Okay... I won't tell. But it's just a hobby. Why do you care so much?"
    y "Хорошо... Я не скажу. Но это всего лишь хобби. Почему тебя это так волнует?"

# game/status.rpy:7060
translate russia task17_ccefcf89:

    # c r38 "It would ruin my reputation!"
    c r38 "Это разрушит мою репутацию!"

# game/status.rpy:7061
translate russia task17_8beb6f8e:

    # y "Your reputation..."
    y "Твою репутацию..."

# game/status.rpy:7062
translate russia task17_383845bc:

    # y "Your reputation with who exactly?"
    y "Твою репутацию, с кем именно?"

# game/status.rpy:7063
translate russia task17_a0bd5d4e:

    # c "Everone in my school!"
    c "Со всеми в моей школе!"

# game/status.rpy:7064
translate russia task17_5bad8956:

    # y "You don't go to school because of this crisis..."
    y "Ты не ходишь в школу из-за всей этой ситуации..."

# game/status.rpy:7065
translate russia task17_823ed2a4:

    # c "Then everyone in my social circle!"
    c "Тогда со всеми в моём кругу общения!"

# game/status.rpy:7066
translate russia task17_15e88f66:

    # y "Okay that's... Sam, Alex, me and whatever staff we have working at the milkshake bar. We don't care."
    y "Ладно, это... Сэм, Алекс, я и весь персонал, работающий в баре. Нам всё равно."

# game/status.rpy:7067
translate russia task17_88e43489:

    # c r30 "T-then... what about the customers? They come from all kinds of backgrounds. They might hate me for being nerdy!"
    c r30 "Т-тогда... а как насчёт клиентов? Они из самых разных слоёв общества. Они могут возненавидеть меня за то, что я гик!"

# game/status.rpy:7068
translate russia task17_d697e53f:

    # y "As long as you show your tits, I don't think they really care."
    y "Пока ты показываешь свои сиськи, я не думаю, что им есть до этого дело."

# game/status.rpy:7071
translate russia task17_7e8239d1:

    # y "Well then... looks like someone is going to have to {b}pay{/b} me to keep quiet!"
    y "Ну тогда... похоже, кому-то придётся {b}заплатить{/b} за моё молчание!"

# game/status.rpy:7072
translate russia task17_b530b54a:

    # c r10 "[playerName] any money we make goes directly to you anyways."
    c r10 "[playerName] все деньги, которые мы зарабатываем, в любом случае идут непосредственно тебе."

# game/status.rpy:7073
translate russia task17_b61ea75f:

    # y "Oh right..."
    y "Ох точно..."

# game/status.rpy:7074
translate russia task17_120f7949:

    # y "Then I demand sexual favors!"
    y "Тогда я требую сексуальных услуг!"

# game/status.rpy:7075
translate russia task17_68fe6300:

    # c r14 "{b}*Sighs*{/b} We already give you those aswell, remember?"
    c r14 "{b}*Вздох*{/b} Ты их уже получаешь, помнишь?"

# game/status.rpy:7076
translate russia task17_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:7077
translate russia task17_2e3e44d6:

    # y "Er... okay fine. Then do the dishes...."
    y "Эм... ладно, хорошо. Тогда ты моешь посуду..."

# game/status.rpy:7078
translate russia task17_e2f4a4f7:

    # c r40 "NEVER!"
    c r40 "НИКОГДА!"

# game/status.rpy:7079
translate russia task17_861329b0:

    # y "And my laundry!"
    y "И прачку!"

# game/status.rpy:7080
translate russia task17_6a977b9f:

    # c "NOT IN A MILLION YEARS!"
    c "НИ ЗА ЧТО НА СВЕТЕ!"

# game/status.rpy:7081
translate russia task17_a47f272a:

    # y "And massage my feet!"
    y "И массаж ног!"

# game/status.rpy:7082
translate russia task17_8496b542_1:

    # c r11 "...................."
    c r11 "...................."

# game/status.rpy:7083
translate russia task17_aea5edba:

    # c "Yeah okay, that's not worth it. I'm just going to let everyone that I like nerdy stuff..."
    c "Да ладно, оно того не стоит. Я сама всем расскажу, что мне нравятся гиковские штучки..."

# game/status.rpy:7084
translate russia task17_fdc7c300:

    # y "Trust me. Most people are too busy with themselves to bother judging you. It's just a hobby."
    y "Поверь мне. Большинство людей слишком заняты собой, чтобы судить тебя. Это просто хобби."

# game/status.rpy:7086
translate russia task17_36db5b96:

    # y "So... {w}anime."
    y "И так... {w}аниме."

# game/status.rpy:7087
translate russia task17_ad519400:

    # c r19 "Yup."
    c r19 "Ага."

# game/status.rpy:7088
translate russia task17_30b78935:

    # y "And games?"
    y "И игры?"

# game/status.rpy:7089
translate russia task17_3882a3af:

    # c "Also games."
    c "И игры тоже."

# game/status.rpy:7090
translate russia task17_5c4de2ec:

    # y "And tabletop roleplaying games...?"
    y "Ещё настольные ролевые игры...?"

# game/status.rpy:7091
translate russia task17_00693272:

    # c "Also those, yes."
    c "И они тоже, да."

# game/status.rpy:7092
translate russia task17_6db73817_1:

    # y ".........................."
    y ".........................."

# game/status.rpy:7099
translate russia task17_aaeb1096:

    # y "What about interdimensional worship of lustful deities. Often characterized as lubed up tentacle monsters that at the knowledge of their existance cause insanity?"
    y "Как насчёт меж пространственных похотливых божеств. Их часто изображают как скользких и липких монстров с тенаклями, которые при осознании своего существования вызывают безумие?"

# game/status.rpy:7100
translate russia task17_2c7bb48e:

    # y "A being so vigorous and virtile that you loose your virginity, just by looking at it!{w} With power so great, that it could destroy moons!"
    y "Существо настолько сильное и мужественное, что при одном лишь взгляде, вы теряете девственность!{w} Своей силой он может уничтожать луны!"

# game/status.rpy:7101
translate russia task17_e66c4889:

    # y "A BEING SO GREAT, THAT IT'S NAME IS ONLY EVER BEING SAID IN WHISPERS! THE ARTIST TRYING TO PAINT IT, GOES INSANE BEFORE PICKING UP HIS BRUSH!"
    y "СУЩЕСТВО НАСТОЛЬКО ВЕЛИКОЕ, ЧТО ЕГО ИМЯ ПРОИЗНОСЯТ ТОЛЬКО ШЕПОТОМ! ХУДОЖНИК, ПЫТАЮЩИЙСЯ НАРИСОВАТЬ ЕГО, СХОДИТ С УМА, НЕ УСПЕВ ВЗЯТЬ В РУКИ КИСТЬ!"

# game/status.rpy:7105
translate russia task17_fa73a720:

    # c r16 "W-what...? No, that doesn't sound very nerdy at all!"
    c r16 "Ч-что...? Нет, это звучит совсем не по-гиковски!"

# game/status.rpy:7106
translate russia task17_4af5053a:

    # y "Oh..."
    y "Ох..."

# game/status.rpy:7107
translate russia task17_0d0bf37c:

    # y "So you wouldn't so happen to be interested in joining his cult...?"
    y "Так ты случайно не хотела бы присоединиться к его секте...?"

# game/status.rpy:7108
translate russia task17_e06f17ae:

    # c "No....?"
    c "Нет...?"

# game/status.rpy:7109
translate russia task17_3c62ee1f:

    # y "Oh... okay... just asking for a {i}'friend'{/i}."
    y "Ох... ладно... да я просто для {i}'друга'{/i} спрашивал."

# game/status.rpy:7111
translate russia task17_6d55fb24:

    # y "All I'm saying is..."
    y "Всё, что я хотел сказать..."

# game/status.rpy:7112
translate russia task17_86770ad2:

    # y "Nobody cares about what hobbies you enjoy. As long as you show your tits, you'll stay popular."
    y "Никого не волнуют твои хобби. Пока ты показываешь свои сиськи, ты будешь оставаться популярной."

# game/status.rpy:7113
translate russia task17_01241182:

    # c r11 ".............................."
    c r11 ".............................."

# game/status.rpy:7114
translate russia task17_c4fdf5d3:

    # c "Huh...."
    c "Хух..."

# game/status.rpy:7115
translate russia task17_38376ef0:

    # c r12 "I guess nobody really cares..."
    c r12 "Думаю, это никого не волнует..."

# game/status.rpy:7116
translate russia task17_d62434a7:

    # c r1 "Thanks [playerName]... Maybe I should be myself from now on...."
    c r1 "Спасибо [playerName]... Наверное, с этого момента я должна быть самой собой..."

# game/status.rpy:7117
translate russia task17_f5fd527f:

    # c r10 "........................"
    c r10 "........................"

# game/status.rpy:7118
translate russia task17_91ba0cb7:

    # c "I'm going to buy so much junk at this convention!"
    c "Я собираюсь купить столько барахла на этом съезде!"

# game/status.rpy:7119
translate russia task17_6db73817_2:

    # y ".........................."
    y ".........................."

# game/status.rpy:7125
translate russia task17_8b475395:

    # "New cell decorations are now available for Clover."
    "Теперь доступны новые украшения для комнаты Кловер."

# game/status.rpy:7155
translate russia task18_17b781da:

    # c "Okay, on my way to Drift Punk."
    c "Ладно, я пошла к Дрифт Панкам."

# game/status.rpy:7156
translate russia task18_edac935c:

    # c "There's been some new development with Punk Web so I'll probably spend the day in there."
    c "У Дрифт Панков есть новые разработки, так что я, вероятно, проведу там целый день."

# game/status.rpy:7157
translate russia task18_0135e631:

    # y "New developments?"
    y "Новые разработки?"

# game/status.rpy:7158
translate russia task18_274be30c:

    # c r18 "Yeah, one of the Lieutenant is implementing a patch today so we're going to check it out."
    c r18 "Да, один из лейтенантов сегодня внедряет патч, так что мы его проверим."

# game/status.rpy:7159
translate russia task18_e260d0c8:

    # c r1 "I might even be able to get close to her and score some good-girl points."
    c r1 "Возможно, я даже смогу подойти к ней поближе и заработать несколько очков \"хорошей девочки\"."

# game/status.rpy:7160
translate russia task18_57a62bd7:

    # c "If we can get her to trust us, it'd be much easier to capture her."
    c "Если мы сможем заставить её доверять нам, будет гораздо легче поймать её."

# game/status.rpy:7161
translate russia task18_00ed0ebf:

    # y "What do you know about her?"
    y "Что ты о ней знаешь?"

# game/status.rpy:7162
translate russia task18_30a91b94:

    # c r10 "Well... Talia has a striking resemblance to Telly Hardwire. A hacker we brought down a few years ago."
    c r10 "Нуу... Талия имеет поразительное сходство с Телли Хардвайр. Хакер, которого мы поймали несколько лет назад."

# game/status.rpy:7163
translate russia task18_c1458dce:

    # c "Seeing as they have the same last name, I'm guessing it's his sister."
    c "Поскольку у них одинаковые фамилии, я предполагаю, что это его сестра."

# game/status.rpy:7164
translate russia task18_e28711b5:

    # y "It would give her a motive at least. Okay, head out and report back what you find."
    y "По крайней мере, это даёт ей мотив. Хорошо, отправляйся и сообщи, если что-то найдёшь."

# game/status.rpy:7165
translate russia task18_8764527a:

    # "Clover nods and heads out."
    "Кловер кивает и направляется к выходу."

# game/status.rpy:7182
translate russia task18_906bf9bf:

    # y "...........?"
    y "...........?"

# game/status.rpy:7183
translate russia task18_344e8271:

    # y "Where's Clover?"
    y "Где это Кловер?"

# game/status.rpy:7184
translate russia task18_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:7189
translate russia task18_ff46c564:

    # c r56 "We can use brass rivots for that part. It will look more realistic that way."
    c r56 "Тут можно использовать латунные заклепки. Так она будет выглядеть более реалистично."

# game/status.rpy:7190
translate russia task18_a55ab0df:

    # tali "Oh good idea! And I think I still have some zippers we can use in the back."
    tali "Отличная идея! И я думаю, у меня ещё остались молнии, которые можно использовать сзади."

# game/status.rpy:7191
translate russia task18_b9002b44:

    # y "...............?"
    y "...............?"

# game/status.rpy:7192
translate russia task18_d986ffc2:

    # tali "Wait, I'll go get them."
    tali "Подожди, я схожу за ними."

# game/status.rpy:7193
translate russia task18_47ae669c:

    # y "Psst... Clover...!"
    y "Псс... Кловер...!"

# game/status.rpy:7194
translate russia task18_a526c217:

    # c r18 "...?"
    c r18 "...?"

# game/status.rpy:7195
translate russia task18_803ac429:

    # c "[playerName]? Oh hey."
    c "[playerName]? О, привет."

# game/status.rpy:7196
translate russia task18_802227de:

    # y "Are you with Talia? You didn't report in."
    y "Ты с Талией? Ты мне не сообщила."

# game/status.rpy:7197
translate russia task18_979f108d:

    # c r29 "Oh is it that time already? Sorry, me and her got caught up talking about cosplaying."
    c r29 "Неужели уже пора? Прости, мы с ней заговорились о косплее."

# game/status.rpy:7198
translate russia task18_5d245615:

    # y "Cosplaying...?"
    y "Косплей...?"

# game/status.rpy:7199
translate russia task18_e5934e00:

    # c r1 "Yeah you know. Where you dress up as your favorite character from a movie or video game."
    c r1 "Да, ну знаешь когда люди одеваются в костюмы своих любимых персонажей из фильмов или видеоигры."

# game/status.rpy:7200
translate russia task18_38f07b66:

    # c "You caught me playing as Princess Hamdarusojamia at the anime convention."
    c "Ты застал меня в роли принцессы Хамдарусоямии на том аниме-съезде."

# game/status.rpy:7201
translate russia task18_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:7202
translate russia task18_dc90778b:

    # c "Turns out, Talia is a massive cosplayer as well. We're discussing techniques on how to make our outfits look better."
    c "Оказалось, что Талия - тоже большой косплеер. Мы обсуждаем всякие техники по улучшению наших нарядов."

# game/status.rpy:7203
translate russia task18_1cbae8c6:

    # y "Well... at least you managed to get close to her."
    y "Ну... по крайней мере, тебе удалось подобраться к ней."

# game/status.rpy:7204
translate russia task18_a5adb5fe:

    # y "How difficult will it be to take her in, you think?"
    y "Как думаешь, насколько сложно будет её схватить?"

# game/status.rpy:7205
translate russia task18_bff26c6c:

    # c r11 "Well...."
    c r11 "Нуу..."

# game/status.rpy:7206
translate russia task18_5c732d53:

    # c r12 "It actually should be fairly easy."
    c r12 "На самом деле это должно быть довольно просто."

# game/status.rpy:7207
translate russia task18_f738840c:

    # y "Easy?"
    y "Просто?"

# game/status.rpy:7208
translate russia task18_b569c091:

    # c r1 "Yeah, she joined the gang to avenge her brother's capture, but quickly found herself in over her head."
    c r1 "Да, она вступила в банду, чтобы отомстить за поимку брата, но быстро оказалась в затруднительном положении."

# game/status.rpy:7209
translate russia task18_4fd4c06e:

    # c "She's willing to surrender, but is afraid the gang will come after her.{w} If we plan a mission to take her in, she'll come with us as long as long as we make it look believable."
    c "Она готова сдаться, но боится, что банда придёт за ней. {w} Если мы спланируем миссию по её захвату, она пойдёт с нами до тех пор, пока всё будет правдоподобным."

# game/status.rpy:7210
translate russia task18_acd3a13b:

    # y "Plan a mission and take in Talia. Sounds easy enough."
    y "Спланировать миссию и схватить Талию. Звучит достаточно просто."

# game/status.rpy:7233
translate russia task18_10e9fd72:

    # c r55 "Okay Talia, here we are."
    c r55 "Ладно Талия, вот мы и пришли."

# game/status.rpy:7234
translate russia task18_64e61f76:

    # tali "Here we are indeed..{w} So I'll be safe here?"
    tali "А вот и мы...{w} Значит, вот здесь я буду в безопасности?"

# game/status.rpy:7235
translate russia task18_3f700cd7:

    # y "For the time being, yeah. You are still a prisoner of course."
    y "На данный момент да. Но ты по-прежнему в роли заключённого."

# game/status.rpy:7236
translate russia task18_e304bbf5:

    # tali "Of course..."
    tali "Конечно..."

# game/status.rpy:7237
translate russia task18_c91fa0b9:

    # tali "..................."
    tali "..................."

# game/status.rpy:7238
translate russia task18_c27d466a:

    # tali "So is Carla locked up in here aswell?"
    tali "Так Карла тоже здесь заперта?"

# game/status.rpy:7239
translate russia task18_e26bb7e3:

    # c r57 "Carla Wong? Yeah, she's in her cell not far from here."
    c r57 "Карла Вонг? Да, она в своей камере недалеко отсюда."

# game/status.rpy:7240
translate russia task18_8a5d2997:

    # c "We'll keep you guys separated, so she'll never know you turned yourself in."
    c "Мы будем держать вас раздельно, так что она никогда не узнает о твоём задержании."

# game/status.rpy:7241
translate russia task18_a668f5e2:

    # tali "Right."
    tali "Хорошо."

# game/status.rpy:7242
translate russia task18_90d7348b:

    # c r1 "Well [playerName]. Her eyes aren't red and she's not showing any signs of aggression. Seems she's not being nano-controlled."
    c r1 "Ну [playerName]. Её глаза не красные, и она не проявляет никаких признаков агрессии. Похоже, её не контролируют."

# game/status.rpy:7243
translate russia task18_b50a9e3e:

    # y "I think we should have sex... just in case..."
    y "Думаю, нам стоит заняться сексом... на всякий случай..."

# game/status.rpy:7244
translate russia task18_5e3019df:

    # c r10 "[playerName]...."
    c r10 "[playerName]...."

# game/status.rpy:7245
translate russia task18_21480d50:

    # tali "Have sex?{w} If we need to have sex to prove that I'm not being controlled, that's fine."
    tali "Заниматься сексом?{w} Если нам нужно заняться сексом, чтобы доказать, что меня не контролируют, пусть так."

# game/status.rpy:7246
translate russia task18_2a6afe5c:

    # y "See? She agrees with me!"
    y "Видишь? Она согласна со мной!"

# game/status.rpy:7247
translate russia task18_fd7cf242:

    # c r38 "Ugh... fine. I'm not gonna be here while you two get it on. Call me if you need me."
    c r38 "Угх... ладно. Я не собираюсь оставаться здесь, пока вы двое занимаетесь этим. Позови мне, если я тебе понадоблюсь."

# game/status.rpy:7248
translate russia task18_34e55282:

    # y "Can I call you if we run out of lub-...?"
    y "Могу я позвать тебя, если у нас закончится смаз-...?"

# game/status.rpy:7249
translate russia task18_e69e42b8:

    # c "No."
    c "Нет."

# game/status.rpy:7252
translate russia task18_4527edf9:

    # y "Now... where were we...?"
    y "И так... где мы остановились...?"

# game/status.rpy:7253
translate russia task18_3f39d882:

    # tali "I believe you and I were about to get better acquainted~..."
    tali "Кажется, мы с тобой собирались познакомиться поближе~..."

# game/status.rpy:7254
translate russia task18_58155bc1:

    # "Talia gets up and seductively walks over to you. Putting her hands on your chest, she presses herself up against you."
    "Талия встает и соблазнительно подходит к вам. Положив руки на вашу грудь, она прижимается к вам."

# game/status.rpy:7255
translate russia task18_bf6a5f79:

    # tali "Check me for nanobots... but be sure to be {i}'thorough'{/i}~..."
    tali "Проверь меня на наличие наноботов... но сделай это {i}'тщательно'{/i}~..."

# game/status.rpy:7258
translate russia task18_c0a75796:

    # tali "It's been so long since I had a good thussle in the sheets~..."
    tali "Давно я так хорошо не валялась в простынях~..."

# game/status.rpy:7259
translate russia task18_598b1e45:

    # tali "I bet you can make me scream all night long...."
    tali "Спорим, ты заставишь меня кричать всю ночь напролёт..."

# game/status.rpy:7263
translate russia task18_54f04235:

    # y "Red is my favorite flavor...."
    y "Красное - моё любимое..."

# game/status.rpy:7264
translate russia task18_84dbaca4:

    # tali "Mine too~...."
    tali "Моё тоже~...."

# game/status.rpy:7265
translate russia task18_0c48670c:

    # "Talia's eyes suddenly turn red!"
    "Глаза Талии внезапно становятся красными!"

# game/status.rpy:7266
translate russia task18_82d235ce:

    # y "Wai-...!"
    y "Подо-...!"

# game/status.rpy:7271
translate russia task18_3df0813f:

    # "{b}*Slam!*{/b}"
    "{b}*Хлопок!*{/b}"

# game/status.rpy:7272
translate russia task18_94934c3f:

    # "Talia knocked you upside the head!"
    "Талия ударила вас по голове!"

# game/status.rpy:7274
translate russia task18_1491b397:

    # y "Lady..."
    y "Леди..."

# game/status.rpy:7275
translate russia task18_885c2a82:

    # tali "...?!"
    tali "...?!"

# game/status.rpy:7276
translate russia task18_c866587a:

    # y "I've been getting bumped on the head so much, I feel like I'm becoming immune to it."
    y "Меня так часто бьют по голове, что похоже у меня начал вырабатываться иммунитет к этому."

# game/status.rpy:7277
translate russia task18_1d5991aa:

    # tali "Oh I see... {w}Let me try again."
    tali "Понятно... {w}Попробую ещё раз."

# game/status.rpy:7278
translate russia task18_81460bde:

    # y "SPIE-....!!!!"
    y "ШПИО-...!!!"

# game/status.rpy:7284
translate russia task18_08b2c4c0:

    # a "He's waking up..!"
    a "Он просыпается...!"

# game/status.rpy:7291
translate russia task18_d765b60f:

    # c "[playerName]?! Are you okay?"
    c "[playerName]?! Ты в порядке?"

# game/status.rpy:7292
translate russia task18_55cd4c37:

    # y "I feel like this little adventure of ours is going to leave me with permanent brain damage, but at the moment...{w} I'm okay."
    y "Чувствую, что это маленькое приключение оставит меня с необратимым повреждением мозга, но в данный момент...{w} Я в порядке."

# game/status.rpy:7293
translate russia task18_84befcf3:

    # y "Where's Talia?"
    y "Где Талия?"

# game/status.rpy:7294
translate russia task18_6a8b668a:

    # s g39 "We don't know. She smuggled a gadget with her and turned off the lights. She also used it to unlock her cell."
    s g39 "Мы не знаем. Она пронесла с собой гаджет и выключила свет. Она также использовала его, чтобы разблокировать свою камеру."

# game/status.rpy:7295
translate russia task18_c164ca17:

    # y "Uh oh..."
    y "Ой-ой..."

# game/status.rpy:7296
translate russia task18_66e28cb6:

    # c r21 "It gets worse..."
    c r21 "Дальше - хуже..."

# game/status.rpy:7297
translate russia task18_c1f28ab9:

    # s "She also let Carla Wong, the Dragon Lady, out of her cell."
    s "Она также выпустила Карлу Вонг, Леди Дракон, из своей камеры."

# game/status.rpy:7298
translate russia task18_716a19f0:

    # y "She tricked us..."
    y "Она обманула нас..."

# game/status.rpy:7299
translate russia task18_c1872db4:

    # c r42 "I shouldn't have trust her so easily..."
    c r42 "Нельзя было так легко доверять ей..."

# game/status.rpy:7300
translate russia task18_7023590e:

    # y "Did she leave the prison area? If she gets in contact with Drift Punk, we'll be in trouble."
    y "Она покинула территорию тюрьмы? Если она свяжется с Дрифт Панками, у нас будут проблемы."

# game/status.rpy:7301
translate russia task18_4dacd3a0:

    # s "No. When the power went off, the elevator to the milkshake bar also stopped working."
    s "Нет. Когда отключилось электричество, лифт в бар также перестал работать."

# game/status.rpy:7302
translate russia task18_f40be9c8:

    # a y30 "Meaning they're still here somewhere!"
    a y30 "Значит, они всё ещё где-то здесь!"

# game/status.rpy:7303
translate russia task18_4493b59d:

    # y "Okay, let's split up and start searching!"
    y "Ладно, давайте разделимся и начнём поиски!"

# game/status.rpy:7304
translate russia task18_6084aa83:

    # "The girls give you a resolute nod and then scatter in different directions."
    "Девушки решительно кивают вам, а затем разбегаются в разные стороны."

# game/status.rpy:7309
translate russia task18_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:7323
translate russia task18SearchMenu_93369552:

    # y "Any luck?"
    y "Есть успехи?"

# game/status.rpy:7324
translate russia task18SearchMenu_046f1640:

    # a "Nowhere to be found..."
    a "Нигде не найти..."

# game/status.rpy:7325
translate russia task18SearchMenu_6c899a9d:

    # a "But I did find this while searching!"
    a "Но я нашла это во время поиска!"

# game/status.rpy:7328
translate russia task18SearchMenu_5bdf773d:

    # "Alex hands you {color=#ffeda6}Hydrofluoric Acid{/color} x1."
    "Алекс даёт вам {color=#ffeda6}Плавиковая кислота{/color} x1."

# game/status.rpy:7329
translate russia task18SearchMenu_53856a08:

    # y "All right, let's keep searching."
    y "Ладно, давайте продолжим поиски."

# game/status.rpy:7342
translate russia task18SearchMenu_d3b831a7:

    # y "Sam! Any clue about her whereabouts?"
    y "Сэм! Есть какие-нибудь новости о её местоположении?"

# game/status.rpy:7343
translate russia task18SearchMenu_a8c0c2c9:

    # s "No, nothing. She must have fled into the lower portion of the compound."
    s "Нет, ничего. Должно быть, она скрылась в нижней части комплекса."

# game/status.rpy:7344
translate russia task18SearchMenu_69ea40a8:

    # s "I did find this weird shrine. Dedicated to some kind of tentacle monster..."
    s "Я нашла это странное святилище. Посвящённое какому-то щупальцевому монстру..."

# game/status.rpy:7345
translate russia task18SearchMenu_7438e8c9:

    # y "Er.... I'll explain later. Let's continue searching for now!"
    y "Эмм... Я объясню позже. Давайте пока продолжим поиски!"

# game/status.rpy:7357
translate russia task18SearchMenu_a4a6e159:

    # y "Any luck over here?"
    y "Есть успехи?"

# game/status.rpy:7358
translate russia task18SearchMenu_6eb7626b:

    # c r31 "Shhh~..."
    c r31 "Тссс~..."

# game/status.rpy:7359
translate russia task18SearchMenu_688951bc:

    # "The two of you stay deadly silent..."
    "Вы оба храните гробовое молчание..."

# game/status.rpy:7361
translate russia task18SearchMenu_df66d723:

    # c "This way!"
    c "Сюда!"

# game/status.rpy:7363
translate russia task18SearchMenu_5f2374ea:

    # "You quickly call Sam and Alex before chasing after Clover."
    "Вы быстро звоните Сэм и Алекс, прежде чем бежать за Кловер."

# game/status.rpy:7365
translate russia task18SearchMenu_afaf05ba:

    # drag "Brilliant escape plan, Talia..."
    drag "Блестящий план побега, Талия..."

# game/status.rpy:7366
translate russia task18SearchMenu_4e773a65:

    # tali "How could I have known that their lift wouldn't work?! Just stay quiet..."
    tali "Откуда я могла знать, что их лифт не будет работать?! Просто сиди тихо..."

# game/status.rpy:7367
translate russia task18SearchMenu_bf3f777d:

    # "Bzzzzzzt!"
    "Бззззззт!"

# game/status.rpy:7370
translate russia task18SearchMenu_c8a4ede3:

    # tali "!!!"
    tali "!!!"

# game/status.rpy:7371
translate russia task18SearchMenu_38ae0222:

    # drag "!!!"
    drag "!!!"

# game/status.rpy:7372
translate russia task18SearchMenu_973b3098:

    # c r32 "There they are!"
    c r32 "Вот они!"

# game/status.rpy:7377
translate russia task18SearchMenu_4c9e5b46:

    # tali "Uh oh..."
    tali "Ой-ой..."

# game/status.rpy:7385
translate russia task18SearchMenu_914fa836:

    # "You quickly manage to overpower the two lieutenants and bring them back to their cell."
    "Вам быстро удаётся одолеть двух лейтенантов и вернуть их в камеры."

# game/status.rpy:7390
translate russia task18SearchMenu_3a20b050:

    # c r33 "She's under lock and key..."
    c r33 "Она под замком..."

# game/status.rpy:7391
translate russia task18SearchMenu_6c4751b6:

    # c r5 "................."
    c r5 "................."

# game/status.rpy:7392
translate russia task18SearchMenu_98c195f8:

    # c "I can't believe I let her trick me so easily. It was all just one big plan to break Carla out..."
    c "Я не могу поверить, что позволила ей так легко обмануть меня. Всё это было одним большим планом, чтобы вытащить Карлу..."

# game/status.rpy:7393
translate russia task18SearchMenu_1a61a5b0:

    # y "We've been a bit too lacks on security in general. Don't let it bother you too much."
    y "Мы слишком много внимания уделяем безопасности в целом. Сильно не беспокойся об этом."

# game/status.rpy:7394
translate russia task18SearchMenu_09ea468d:

    # c r38 "Hmpf~..."
    c r38 "Хмпф~..."

# game/status.rpy:7395
translate russia task18SearchMenu_49b56ab3:

    # y "So yeah... I'm gonna go fuck her now to suppress her nanobots."
    y "Так что да... Пойду трахну её, чтобы подавить её наноботов."

# game/status.rpy:7396
translate russia task18SearchMenu_07cc5ad1:

    # c "............"
    c "............"

# game/status.rpy:7400
translate russia task18SearchMenu_10b4ec87:

    # tali "Ooooh! AH! AHH! ♥"
    tali "ОооАх! АХ! ААХ! ♥"

# game/status.rpy:7401
translate russia task18SearchMenu_69391f1d:

    # tali "H-harder...!!!"
    tali "С-сильнее...!!!"

# game/status.rpy:7402
translate russia task18SearchMenu_f951a127:

    # tali "{b}*Moans*{/b} Right there~... ♥♥♥"
    tali "{b}*Стоны*{/b} Вот так~... ♥♥♥"

# game/status.rpy:7403
translate russia task18SearchMenu_ef574bd0:

    # y "Say cheese!"
    y "Скажи сыр!"

# game/status.rpy:7413
translate russia task18SearchMenu_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:7418
translate russia task18SearchMenu_3a045eec:

    # tali "............"
    tali "............"

# game/status.rpy:7419
translate russia task18SearchMenu_1a0ccbec:

    # y "No longer being controlled?"
    y "Уже не под контролем?"

# game/status.rpy:7420
translate russia task18SearchMenu_006370a2:

    # tali "Doesn't matter. Controlled or not, I still hate WOOHP."
    tali "Не имеет значения. Под Контролем или нет, я всё равно ненавижу WOOHP."

# game/status.rpy:7421
translate russia task18SearchMenu_e542c49b:

    # tali "You took away my brother..."
    tali "Вы забрали моего брата..."

# game/status.rpy:7422
translate russia task18SearchMenu_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:7423
translate russia task18SearchMenu_772ffc64:

    # tali "I know he was suppose to be a {i}'bad guy'{/i}. But he was still my brother, y'know..."
    tali "Я знаю, что он {i}'злодей'{/i}. Но он всё равно мой брат, понимаешь..."

# game/status.rpy:7424
translate russia task18SearchMenu_b5b72d41:

    # y "What happened to him?"
    y "Что с ним случилось?"

# game/status.rpy:7425
translate russia task18SearchMenu_03053c48:

    # tali "You tell me..."
    tali "Ты мне скажи..."

# game/status.rpy:7426
translate russia task18SearchMenu_b518bce9:

    # tali "................"
    tali "................"

# game/status.rpy:7427
translate russia task18SearchMenu_c94c1979:

    # tali "I don't know what happened to him... {w}WOOHP wouldn't let us visit him.{w} I haven't seen him in years."
    tali "Я не знаю, что с ним случилось... {w}WOOHP не позволила нам навестить его.{w} Я не видела его много лет."

# game/status.rpy:7428
translate russia task18SearchMenu_c9b7f625:

    # y "Years...?"
    y "Лет...?"

# game/status.rpy:7429
translate russia task18SearchMenu_3d9bcfd5:

    # tali "Doesn't matter now anyways. Drift Punk said they'd help me find him in return for my technical skills, but all they worry about is their games, cars and music."
    tali "Сейчас это уже не имеет значения. Дрифт Панки сказали, что помогут мне найти его в обмен на мои технические навыки. Но всё, о чём они думают, это игры, машины и музыка."

# game/status.rpy:7430
translate russia task18SearchMenu_dfbcad54:

    # tali "I guess hanging around here is just as productive as being with those guys..."
    tali "Полагаю, болтаться здесь так же продуктивно, как и с теми парнями..."

# game/status.rpy:7431
translate russia task18SearchMenu_f096c99e:

    # tali "Doesn't mean I'm giving up on my brother just yet."
    tali "Но это не значит, что я отказываюсь от своего брата."

# game/status.rpy:7432
translate russia task18SearchMenu_cd712ced:

    # y "I'll come ask you about him sometime... maybe I can help you find him."
    y "Я как-нибудь приду и расспрошу тебя о нём... может я смогу помочь тебе найти его."

# game/status.rpy:7433
translate russia task18SearchMenu_dfbe9759:

    # tali ".................."
    tali ".................."

# game/status.rpy:7434
translate russia task18SearchMenu_71e99c8f:

    # tali "I'd like that..."
    tali "Было бы неплохо..."

# game/status.rpy:7435
translate russia task18SearchMenu_c20cfcbc:

    # "You leave Talia alone for now."
    "Вы уходите, оставляя Талию в покое."

# game/status.rpy:7441
translate russia task18SearchMenu_f876bc91:

    # "Computer" "Computer: Talia H.\nStatus: Captured."
    "Компьютер" "Компьютер: Талия Х.\nСтатус: Захвачена."

# game/status.rpy:7442
translate russia task18SearchMenu_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:7448
translate russia task18SearchMenu_883fcc59:

    # "New clothing options for Clover are available at the mall."
    "Новые варианты одежды для Кловер можно приобрести в торговом центре."

# game/status.rpy:7471
translate russia task19_eef5929f:

    # y "Are you getting used to going undercover with the Aces?"
    y "Привыкаешь к работе под прикрытием у Тузов?"

# game/status.rpy:7472
translate russia task19_0bf84568:

    # s "I don't think you can ever get used to the Aces."
    s "Не думаю, что к Тузам можно привыкнуть."

# game/status.rpy:7473
translate russia task19_7e7b2e44:

    # s "The amount of money they have... it's kind of intimidating..."
    s "Количество их денег... немного пугает..."

# game/status.rpy:7474
translate russia task19_2bbb6d2f:

    # s g14 "It goes further than fancy cars or jewelry. These people could buy half of the city if they wanted to."
    s g14 "Это не просто шикарные автомобили или драгоценности. Эти люди могли бы купить половину города, если бы захотели."

# game/status.rpy:7475
translate russia task19_b1fcc6fc:

    # s g38 "Which makes it even more disgusting that they seem to get a kick out of stealing instead..."
    s g38 "Похоже они получают удовольствие от воровства, это делает всё ещё более отвратительным..."

# game/status.rpy:7476
translate russia task19_5ec585bf:

    # y "They're just doing it for the thrill. Let's teach them a lesson and put them behind bars."
    y "Они делают это просто ради острых ощущений. Давайте преподадим им урок и посадим их за решетку."

# game/status.rpy:7478
translate russia task19_da6bf7d7:

    # "Sam nods and heads out."
    "Сэм кивает и направляется к выходу."

# game/status.rpy:7490
translate russia task19_a88aa66e:

    # s "................"
    s "................"

# game/status.rpy:7491
translate russia task19_0158b375:

    # y "Sam, you look upset. What happened?"
    y "Сэм, ты выглядишь расстроенной. Что случилось?"

# game/status.rpy:7492
translate russia task19_b94fdc43:

    # s "Well... here..."
    s "Нуу... вот..."

# game/status.rpy:7493
translate russia task19_c88d1bb4:

    # "Sam hands you a $100 dollar bill."
    "Сэм протягивает вам купюру в 100 долларов."

# game/status.rpy:7497
translate russia task19_973825c2:

    # y "What did you have to do to get this...?"
    y "Что ты сделала, чтобы их получить...?"

# game/status.rpy:7498
translate russia task19_4e3bf114:

    # s "We went to a fancy restaurant today..."
    s "Сегодня мы ходили в шикарный ресторан..."

# game/status.rpy:7499
translate russia task19_eeae86b6:

    # s g15 "One of the Aces made an unreasonable demand of our waiter. When he couldn't comply, the Aces dumped their food on him."
    s g15 "Один из Тузов предъявил нашему официанту необоснованное требование. Он не смог уступить и Тузы вывалили на него свою еду."

# game/status.rpy:7500
translate russia task19_a5be9d55:

    # s "They mocked him for being a bad waiter and waved a hundred dollar bill in his face. Told him that this was suppose to be his tip, but they gave it to me instead..."
    s "Они насмехались над ним за то, что он плохой официант. Они размахивали перед его лицом стодолларовой купюрой. Они сказали что это должны были быть его чаевые, но отдали их мне..."

# game/status.rpy:7501
translate russia task19_05bf074c:

    # y ".............."
    y ".............."

# game/status.rpy:7502
translate russia task19_1fd24b8b:

    # s g14 "So yeah... that happened today..."
    s g14 "Так что да... вот что сегодня произошло..."

# game/status.rpy:7503
translate russia task19_13cc68ab:

    # y "Don't let it get to you too much. Head back to your cell for now."
    y "Не позволяй этому слишком сильно тебя задевать. Возвращайся пока в свою камеру."

# game/status.rpy:7504
translate russia task19_aef3b8ea:

    # "Sam slanters off to her cell."
    "Сэм убегает в свою камеру."

# game/status.rpy:7515
translate russia task19_ccee5122:

    # y "Any waiters tormented today?"
    y "Снова мучили официантов?"

# game/status.rpy:7516
translate russia task19_51169950:

    # s "No, but we did rob a jewelry store."
    s "Нет, но мы ограбили ювелирный магазин."

# game/status.rpy:7517
translate russia task19_b400ca87:

    # s "It went surprisingly smooth. The jeweler seemed happy to see us."
    s "Всё прошло на удивление гладко. Ювелир, казалось, был рад нас видеть."

# game/status.rpy:7518
translate russia task19_128bc2eb:

    # y "Happy to see you...?"
    y "Рад вас видеть...?"

# game/status.rpy:7519
translate russia task19_4a2fe431:

    # s g16 "Yeah, turns out he insured his entire stock right before the gang uprising so we didn't even have to threaten him."
    s g16 "Да, оказалось, что он застраховал все свои акции прямо перед восстанием банды. Так что нам даже не пришлось ему угрожать."

# game/status.rpy:7520
translate russia task19_82f2b29a:

    # s "Before we know it we were outside again, with our arms full of loot."
    s "Не успели мы оглянуться, как снова оказались на улице с полными руками добычи."

# game/status.rpy:7521
translate russia task19_79572ed2:

    # s g2 "The Aces threw a hissy fit, saying it wasn't any fun doing it like this."
    s g2 "Тузы разбушевались, заявив, что так неинтересно."

# game/status.rpy:7522
translate russia task19_08fed607:

    # s "They gave me their share of the loot and went to sulk."
    s "Они отдали мне свою часть добычи и ушли дуться."

# game/status.rpy:7524
translate russia task19_5cf1f1fe:

    # "Sam hands you {color=#ffeda6}Valuables{/color} x1."
    "Сэм даёт вам {color=#ffeda6}Ценности{/color} x1."

# game/status.rpy:7526
translate russia task19_474bd097:

    # y "Ooooh treasure!"
    y "Оооо сокровища!"

# game/status.rpy:7527
translate russia task19_4c710a80:

    # s g14 "Remember this isn't ours, [playerName]..."
    s g14 "Помни, что это не наше, [playerName]..."

# game/status.rpy:7528
translate russia task19_25ce5907:

    # y "I know I know... but I still like rolling around in it and feel like a sultan!"
    y "Я знаю, я знаю... но мне всё равно нравится, в нём я чувствую себя султаном!"

# game/status.rpy:7529
translate russia task19_d0d1bb1a:

    # s g35 "................"
    s g35 "................"

# game/status.rpy:7530
translate russia task19_8eaa0fc4:

    # s g2 "Nobody got hurt, no property was damaged and everything was insured. I guess we can keep these valuables to turn into gadgets."
    s g2 "Никто не пострадал, имущество не пострадало, и всё было застраховано. Думаю, мы можем сохранить эти ценности, чтобы превратить их в гаджеты."

# game/status.rpy:7531
translate russia task19_4efd9b08:

    # s g1 "Just this once."
    s g1 "Только в этот раз."

# game/status.rpy:7542
translate russia task19_cf7e0367:

    # y "Hi Sam, did something interesting ha-..."
    y "Привет, Сэм, было что-то интересное се-..."

# game/status.rpy:7543
translate russia task19_9de7ca9c:

    # s "I saw Alex!"
    s "Я видела Алекс!"

# game/status.rpy:7544
translate russia task19_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:7545
translate russia task19_2c2590ed:

    # y "Who...?"
    y "Кого...?"

# game/status.rpy:7546
translate russia task19_f60de454:

    # s g31 "She's a fellow spy and our friend. I saw her walking around town with those WOOHP agents."
    s g31 "Она шпионка и наш друг. Я видела, как она ходила по городу с этими агентами WOOHP."

# game/status.rpy:7547
translate russia task19_ec48663f:

    # s "If she's being controlled then we should worry. She might be a little ditzy, but she's a great spy."
    s "Если её контролируют, то нам стоит беспокоиться. Она может быть немного глуповата, но она отличный шпион."

# game/status.rpy:7548
translate russia task19_26855909:

    # s g32 "She disappeared before I could get to her, but maybe some of the Aces know more. It would be best if you kept sending us undercover there, we might be able to find out more."
    s g32 "Она исчезла, прежде чем я смогла добраться до неё. Но, возможно, кто-то из Тузов знает больше. Будет лучше, если ты продолжишь посылать нас туда под прикрытием, мы сможем узнать больше."

# game/status.rpy:7549
translate russia task19_442d54d3:

    # y "Keep sending you girls undercover with the Aces. Got it."
    y "Продолжать посылать своих девушек под прикрытием к Тузами. Есть."

# game/status.rpy:7571
translate russia task20_94566a45:

    # a "Grrrrrr!"
    a "Гррррр!"

# game/status.rpy:7572
translate russia task20_730c65b9:

    # y "Uh oh... Alex are you being controlled again?"
    y "Ой-ой... Алекс, ты снова под контролем?"

# game/status.rpy:7573
translate russia task20_b5ac933e:

    # a y1 "No, I'm channeling my inner teenage angst!"
    a y1 "Нет, я использую свой внутренний подростковый гнев!"

# game/status.rpy:7574
translate russia task20_57e586ab:

    # y "......"
    y "......"

# game/status.rpy:7575
translate russia task20_d9606fc9:

    # y "Okay~....?"
    y "Хорошо~....?"

# game/status.rpy:7576
translate russia task20_cf289ed7:

    # a "The Outsiders said I need to get angry more. Outraged by the injustice in the world."
    a "Аутсайдеры сказали, что мне нужно больше злиться. Возмущаться несправедливостью в мире."

# game/status.rpy:7577
translate russia task20_5356fc62:

    # a y28 "Like the rich having all the power, and the poor just being exploited by the upper class!"
    a y28 "Например, богатые имеют всю власть, а бедные просто эксплуатируются высшим классом!"

# game/status.rpy:7578
translate russia task20_19cff350:

    # a y8 "And I have to make my voice known! Shout and decry the tyrannical government that just wants to keep us down!"
    a y8 "И я должна заявить о себе! Кричать и осуждать тираническое правительство, которое просто хочет держать нас внизу!"

# game/status.rpy:7579
translate russia task20_7397c999:

    # y "So how's that going?"
    y "Ну и как дела?"

# game/status.rpy:7580
translate russia task20_0130c9c0:

    # a y1 "It's going well! They even gave me my own gothic poetry book!"
    a y1 "Всё идет хорошо! Они даже подарили мне мою собственную книгу готической поэзии!"

# game/status.rpy:7581
translate russia task20_1e6336bb:

    # a "Wanna read it?"
    a "Хочешь почитать?"

# game/status.rpy:7584
translate russia task20_a56c31d1:

    # y "Yeah, I'll read it."
    y "Да, я прочитаю."

# game/status.rpy:7585
translate russia task20_6e4ecfcc:

    # y "........................."
    y "........................."

# game/status.rpy:7586
translate russia task20_45122cef:

    # y "Well at least you started writing in black ink instead of rainbow crayons."
    y "Ну, по крайней мере, ты начал писать чёрными чернилами вместо радужных мелков."

# game/status.rpy:7587
translate russia task20_e5abecf9:

    # y "Let see here..."
    y "Посмотрим..."

# game/status.rpy:7588
translate russia task20_327b1a43:

    # y "{i}'Roses are green, violets are green. I am a monster....{w} and want to eat your spleen.'{/i}"
    y "{i}'Зелёные розы, зелёные фиалки. Я монстр....{w} который хочет сожрать ваши слезинки.'{/i}"

# game/status.rpy:7589
translate russia task20_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:7590
translate russia task20_3ce29e83:

    # y "Yeah.... that's about the level of literacy I expected from you."
    y "Даа.... примерно такой уровень грамотности я от тебя и ожидал."

# game/status.rpy:7591
translate russia task20_42c8e7fb:

    # y "You know neither roses nor violets are green, right?"
    y "Ты же знаешь, что ни розы, ни фиалки не бывают зелёными?"

# game/status.rpy:7592
translate russia task20_d3ba54db:

    # y "But valiant first try I guess."
    y "Но, думаю, это доблестная первая попытка."

# game/status.rpy:7593
translate russia task20_3bdd3c52:

    # a y13 "I feel like I'm going to become a great poet some day!"
    a y13 "Мне кажется, что когда-нибудь я стану великим поэтом!"

# game/status.rpy:7595
translate russia task20_9c954c14:

    # y "Keep that freaky voodoo away from me."
    y "Держи это странное вуду подальше от меня."

# game/status.rpy:7596
translate russia task20_6576ec59:

    # a y10 "It's just poetry, [playerName]...."
    a y10 "Это просто поэзия, [playerName]...."

# game/status.rpy:7597
translate russia task20_7bf0bc95:

    # a y2 "If I keep this up, I feel like I could really make people feel things, you know!"
    a y2 "Если я буду продолжать в том же духе, я смогу действительно заставить людей чувствовать, понимаешь!"

# game/status.rpy:7598
translate russia task20_96e0e52a:

    # a "Like make people happy or sad!"
    a "Например, делать людей счастливыми или грустными!"

# game/status.rpy:7599
translate russia task20_d3587701:

    # y "Or melancholic."
    y "Или меланхоличными."

# game/status.rpy:7600
translate russia task20_28e46be4:

    # a y29 "{b}*Gasp*{/b} I don't even know what that means!"
    a y29 "{b}*Вздох*{/b} Я даже не знаю, что это значит!"

# game/status.rpy:7601
translate russia task20_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:7602
translate russia task20_3699537a:

    # y "Just get over to the Outsiders."
    y "Просто отправляйся к Аутсайдерам."

# game/status.rpy:7603
translate russia task20_21f9ba57:

    # "Alex nods and grabs her poetry book."
    "Алекс кивает и берёт свою книгу со стихами."

# game/status.rpy:7619
translate russia task20_496603ac:

    # y "Welcome back Alex. How did it go?"
    y "С возвращением, Алекс. Как всё прошло?"

# game/status.rpy:7620
translate russia task20_37c04d4a:

    # a "Well... they didn't really like my poem..."
    a "Ну... им не очень понравилось мое стихотворение..."

# game/status.rpy:7621
translate russia task20_c86a022a:

    # a y1 "But as we were vandalising a bus stop they gave me some pointers!"
    a y1 "Но поскольку мы совершили акт вандализма на автобусной остановке, они дали мне несколько советов!"

# game/status.rpy:7622
translate russia task20_f45729eb:

    # a "I'm gonna try my best and write something good for tomorrow!"
    a "Я постараюсь сделать всё возможное и написать что-нибудь хорошее на завтра!"

# game/status.rpy:7623
translate russia task20_d48481df:

    # y "Okay~.... What about your undercover work?"
    y "Хорошо~.... А как же твоя работа под прикрытием?"

# game/status.rpy:7624
translate russia task20_e7488bd6:

    # a y41 "Hm? Oh yeah, here's what intel I managed to gather today."
    a y41 "Хм? Ах да, вот какие сведения мне удалось собрать сегодня."

# game/status.rpy:7628
translate russia task20_c0e11c9c:

    # "Alex hands you a box full of papers, USB sticks, guard patrol routes and more."
    "Алекс передаёт вам коробку, полную бумаг, USB-флешек, маршрутов патрулирования охраны и прочего."

# game/status.rpy:7629
translate russia task20_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:7630
translate russia task20_54427911:

    # y "That's... a lot of intel. How did you get all of this?"
    y "Это... много информации. Откуда у тебя всё это?"

# game/status.rpy:7631
translate russia task20_9fd5d465:

    # a "I just said I needed some black make-up from the command building and they let me in!"
    a "Я просто сказала, что мне нужен чёрная косметика из здания управления, и меня впустили!"

# game/status.rpy:7632
translate russia task20_856deff8:

    # y "That easily...?"
    y "Так легко...?"

# game/status.rpy:7633
translate russia task20_336907de:

    # a y24 "Yeah. I think they knew better not to get between a goth and her eyeshadow."
    a y24 "Да. Думаю, они знали, что лучше не вставать между готом и её тенями для век."

# game/status.rpy:7645
translate russia task20_885baec8:

    # y "Okay, I'm eager to hear what you got up to today."
    y "Хорошо, мне не терпится узнать, чем ты сегодня занималась."

# game/status.rpy:7646
translate russia task20_d8a3ab6f:

    # a "So I wrote another poem! Wanna read it?"
    a "Ну, я написала ещё одно стихотворение! Хочешь прочесть?"

# game/status.rpy:7647
translate russia task20_ce895a18:

    # y "..........."
    y "..........."

# game/status.rpy:7648
translate russia task20_09c81580:

    # "You read the poem Alex wrote. It's filled with spelling errors, ham-fisted metaphors and edgy teenage angst."
    "Вы прочитали стихотворение, которое написала Алекс. Она наполнена орфографическими ошибками, неуклюжими метафорами и злободневными подростковыми переживаниями."

# game/status.rpy:7649
translate russia task20_863b5bf9:

    # y "Wow....{w} This is trash!"
    y "Вау....{w} Какой мусор!"

# game/status.rpy:7650
translate russia task20_37475135:

    # a y28 "Hey! I put my heart and soul into that, you know!"
    a y28 "Эй! Я вложил в это всю свою душу и сердце, знаешь ли!"

# game/status.rpy:7651
translate russia task20_6b297274:

    # y "No you didn't."
    y "Нет, это не так."

# game/status.rpy:7652
translate russia task20_8e6b909d:

    # a y11 "................"
    a y11 "................"

# game/status.rpy:7653
translate russia task20_e6e8df3b:

    # a y12 "Okay fine, I wrote it in five minutes during my lunch break. {w}But the Outsiders loved it!"
    a y12 "Ладно, хорошо, я написала его за пять минут во время обеденного перерыва. {w}Но Аутсайдерам это понравилось!"

# game/status.rpy:7654
translate russia task20_e22080f6:

    # a "I just sorta wrote down what I thought a goth would love and they ate it up."
    a "Я просто написала то, что, по моему мнению должно понравится готам, и они это съели."

# game/status.rpy:7655
translate russia task20_44feb757:

    # a y1 "Well... except for one who saw through it... I'm gonna try writing a better poem for him tomorrow."
    a y1 "Ну... кроме одного, который видел всё насквозь... Завтра я попробую написать для него стихотворение получше."

# game/status.rpy:7656
translate russia task20_eb0aa987:

    # y "Still... a gang that's into poetry... Doesn't really match."
    y "И все же... бандиты которые, увлекаются поэзией... Не очень вяжется."

# game/status.rpy:7657
translate russia task20_64f6ff33:

    # a y14 "{b}*Shrugs*{/b} Drift Punk are a mix between nerds and party people."
    a y14 "{b}*Пожимает плечами*{/b} Дрифт Панки - это смесь гиков и тусовщиков."

# game/status.rpy:7658
translate russia task20_4c31bcc8:

    # a "And the Aces are a mix between spoiled rich kids and art collectors."
    a "А Тузы - это смесь между избалованными богатыми детьми и коллекционерами произведений искусства."

# game/status.rpy:7659
translate russia task20_7aa79ba1:

    # y "Good point.{w} Beverly Hills has become a breeding ground for weirdos."
    y "Хорошая мысль.{w} Беверли-Хиллз стал рассадником чудаков."

# game/status.rpy:7670
translate russia task20_885baec8_1:

    # y "Okay, I'm eager to hear what you got up to today."
    y "Отлично, мне не терпится узнать, чем ты сегодня занималась."

# game/status.rpy:7671
translate russia task20_a2616f0e:

    # a "Well I tried writing a poem for that one goth guy at the gang, right? Wanna read it?"
    a "Ну, я пыталась написать стихотворение для того парня-гота из банды, помнишь? Хочешь прочитать?"

# game/status.rpy:7672
translate russia task20_d18cb0de:

    # y "Is it any better than yesterdays poem?"
    y "Оно лучше, чем вчерашнее?"

# game/status.rpy:7673
translate russia task20_9a0c78f8:

    # a "Here... See for yourself."
    a "Вот... Убедись сам."

# game/status.rpy:7674
translate russia task20_b2ae1ba0:

    # y ".............{w}............{w}..........."
    y ".............{w}............{w}..........."

# game/status.rpy:7675
translate russia task20_14eb2f07:

    # "The poem..."
    "Стихотворение..."

# game/status.rpy:7676
translate russia task20_57eeaf20:

    # "Is the most heartfelt, genuine and beautiful poem you've ever read!"
    "Это самое проникновенное, искреннее и красивое стихотворение, которое вы когда-либо читали!"

# game/status.rpy:7677
translate russia task20_75d0d581:

    # y "................{w}{b}*Sniff*{/b}"
    y "................{b}*Шмыгает носом*{/b}"

# game/status.rpy:7678
translate russia task20_f66bd365:

    # y "Alex, this is beautiful. Where did you learn to write like this?"
    y "Алекс, это прекрасно. Где ты научилась так писать?"

# game/status.rpy:7679
translate russia task20_a12f9b4b:

    # y "It took me to another world... made me remember the joys of my childhood carefree days.... opened my mind to brand ne-..."
    y "Она перенесла меня в другой мир... заставила меня вспомнить радостные и беззаботные деньки моего детства.... открыла мой разум для но-..."

# game/status.rpy:7680
translate russia task20_8c34a60e:

    # a y28 "It's about a baby turtle!"
    a y28 "Речь идёт о черепашонке!"

# game/status.rpy:7681
translate russia task20_99534822:

    # y "What."
    y "Что."

# game/status.rpy:7682
translate russia task20_f1152378:

    # a y1 "Yeah the goth kid misinterpretated the poem aswell. Thought that it was about the ever onward march of time and the loss of innocents or whatever."
    a y1 "Да, тот гот тоже неправильно истолковал стихотворение. Думал, что речь идёт о вечном движении времени и гибели невинных или что-то в этом роде."

# game/status.rpy:7683
translate russia task20_4ed180c0:

    # a "I guess everyone has a different interpretation!{w} But in reality it's about a baby turtle I saw on tv."
    a "Думаю, каждый интерпретирует по своему!{w} Но на самом деле речь идёт о черепашонке, которого я видел по телевизору."

# game/status.rpy:7684
translate russia task20_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:7709
translate russia task21_4d8f3ce5:

    # y "Okay Alex. We found out who the next lieutenant is for the Outsiders. It's some rich girl called Felicity Fences."
    y "Хорошо, Алекс. Мы знаем, кто следующий лейтенант Аутсайдеров. Это какая-то богатая девушка по имени Фелисити Фэнсэс."

# game/status.rpy:7710
translate russia task21_2121696f:

    # a y18 "Felicity...."
    a y18 "Фелисити..."

# game/status.rpy:7711
translate russia task21_f8975c6b:

    # a "Why does that name ring a bell...?"
    a "Почему-то это имя мне что-то напоминает...?"

# game/status.rpy:7712
translate russia task21_364304a3:

    # a y28 "Wait! We fought Felicity some time ago. She's with the Outsiders now?!"
    a y28 "Подожди! Мы когда-то сражались с Фелисити. Она теперь с Аутсайдерами?!"

# game/status.rpy:7713
translate russia task21_6bf89af2:

    # y "Looks like it. Why?"
    y "Похоже на то. А что не так?"

# game/status.rpy:7714
translate russia task21_05404706:

    # a y37 "Felicity used to be a spoiled brat. A real rich girl. I thought she'd be with the Aces. Not the Outsiders."
    a y37 "Раньше Фелисити была избалованной соплячкой. Типичная богачка. Я думала, она будет с Тузами. Не с Аутсайдерами."

# game/status.rpy:7715
translate russia task21_b1947ca0:

    # a "She kidnapped professional athletes and had them fight for her in a gladiator arena last we saw her."
    a "В прошлый раз она похитила профессиональных спортсменов и заставила их сражаться за неё на гладиаторской арене."

# game/status.rpy:7716
translate russia task21_deef9402:

    # y "Why...?"
    y "Зачем...?"

# game/status.rpy:7717
translate russia task21_647e39f0:

    # a y1 "Cause she's a rich hoe who gets bored easily."
    a y1 "Потому что она богатая шлюха, которой всё время скучно."

# game/status.rpy:7718
translate russia task21_9c5c1e85:

    # y "Alex! Language!"
    y "Алекс! Следи за языком!"

# game/status.rpy:7719
translate russia task21_5c86e2f3:

    # a "I'll try to win her trust so we can capture her!"
    a "Я постараюсь завоевать её доверие, чтобы мы могли схватить её!"

# game/status.rpy:7720
translate russia task21_e5756f49:

    # a "It'll be easy!"
    a "Это будет просто!"

# game/status.rpy:7721
translate russia task21_4c69b21c:

    # y "Well I like your enthusiasm. Head to the Outsiders and report back tonight."
    y "Мне нравится твой энтузиазм. Отправляйся к Аутсайдерам и отчитайся вечером."

# game/status.rpy:7722
translate russia task21_2e522da1:

    # "Alex nods and heads off."
    "Алекс кивает и уходит."

# game/status.rpy:7737
translate russia task21_51e52059:

    # y "Welcome back Alex. Any luck getting on Felicity's good side?"
    y "С возвращением, Алекс. Удалось добиться расположения Фелисити?"

# game/status.rpy:7738
translate russia task21_fba21ac2:

    # a "No... it might be harder than I thought..."
    a "Нет... это может быть сложнее, чем я думала..."

# game/status.rpy:7739
translate russia task21_cf40cc31:

    # a "She's not really a gangster in the gang. Instead, she's just a financial backer."
    a "Она не совсем гангстер в банде. Вместо этого она просто оказывает финансовую поддержку."

# game/status.rpy:7740
translate russia task21_b5025727:

    # a "Meaning she's probably not interested in my dark poetry..."
    a "Значит, ей, скорее всего, не интересна моя мрачная поэзия..."

# game/status.rpy:7741
translate russia task21_73fde653:

    # y "Hm... found out anything else?"
    y "Хм... узнала что-нибудь ещё?"

# game/status.rpy:7742
translate russia task21_de9102b2:

    # a y57 "Well she has a good friend inside the Outsiders that she seems to listen to. A total punker."
    a y57 "У неё есть хорошая подруга среди Аутсайдеров, к которой она прислушивается. Стопроцентный панк."

# game/status.rpy:7743
translate russia task21_ebad9dd9:

    # a "I bet if we get close to her, she could set up a meeting with Felicity for us."
    a "Уверена, если мы с ней сблизимся, она сможет организовать для нас встречу с Фелисити."

# game/status.rpy:7744
translate russia task21_ad21d5a3:

    # y "Sounds like a plan. Try your best to impress her. I'll send you out again tomorrow."
    y "Звучит как план. Постарайся сделать всё возможное, чтобы произвести на неё впечатление. Завтра я снова отправлю тебя."

# game/status.rpy:7757
translate russia task21_147845b4:

    # a "Alex here!"
    a "Алекс на связи!"

# game/status.rpy:7758
translate russia task21_4e9b269f:

    # y "Whadda ya need my lovable Canary?"
    y "Что тебе нужно, моя любимая канарейка?"

# game/status.rpy:7759
translate russia task21_307f5b64:

    # a y10 "Well first of all you could not call me that..."
    a y10 "Ну, во-первых, тебе нельзя так меня называть..."

# game/status.rpy:7760
translate russia task21_a8854ffb:

    # a y1 "But second, we're going to show off our graffiti art skill."
    a y1 "Но, во-вторых, мы собираемся продемонстрировать свое мастерство в искусстве граффити."

# game/status.rpy:7761
translate russia task21_40e87d78:

    # a "Can I get some money for paints?"
    a "Можно мне денег на краски?"

# game/status.rpy:7762
translate russia task21_13596bb7:

    # y "Er... how much do you need?"
    y "Эмм... сколько тебе нужно?"

# game/status.rpy:7763
translate russia task21_9907d5f6:

    # a "About $150. If we don't have enough money then I can borrow some paint from the others, but I kinda want a collection of canisters myself."
    a "Около $150. Если у нас не хватит денег, я могу одолжить немного краски у других, но я вроде как хочу собрать коллекцию баллончиков."

# game/status.rpy:7769
translate russia task21_8e1f8763:

    # y "Well all right then. I think we can miss it. Show them some true art!"
    y "Ну, хорошо. Думаю, мы можем это себе позволить. Покажи им настоящее искусство!"

# game/status.rpy:7770
translate russia task21_eaa65410:

    # a y3 "All right!"
    a y3 "Хорошо!"

# game/status.rpy:7773
translate russia task21_c845451a:

    # y "We don't have any money to spare right now."
    y "У нас сейчас нет свободных денег."

# game/status.rpy:7774
translate russia task21_9f995a1a:

    # a y5 "Aw..."
    a y5 "Оу..."

# game/status.rpy:7775
translate russia task21_8f7006a8:

    # a "That's okay. I can borrow some of the paint from the others."
    a "Ладно. Я могу одолжить немного краски у других."

# game/status.rpy:7776
translate russia task21_86c4f7e6:

    # y "Oh and Alex..."
    y "О и Алекс..."

# game/status.rpy:7777
translate russia task21_90be11db:

    # a y1 "Yeah?"
    a y1 "Да?"

# game/status.rpy:7778
translate russia task21_44de50a2:

    # y "Don't draw any ponies and butterflies, okay?"
    y "Не надо рисовать пони и бабочек, хорошо?"

# game/status.rpy:7779
translate russia task21_780e3015:

    # a "No promises!"
    a "Никаких обещаний!"

# game/status.rpy:7796
translate russia task21_1acead18:

    # a "I'm back! We went out painting!"
    a "Я вернулась! Мы ходили рисовать!"

# game/status.rpy:7797
translate russia task21_f88384ae:

    # y "What did you paint?"
    y "И что ты нарисовала?"

# game/status.rpy:7798
translate russia task21_ed994d19:

    # a "A unicorn.... a goth unicorn!"
    a "Единорога.... готический единорога!"

# game/status.rpy:7799
translate russia task21_5ced3222:

    # a "The punker I mentioned seemed to like it. We got talking and she said Felicity is cautious of me."
    a "Панку, о которой я упоминал, похоже понравилось. Мы разговорились, и она сказала что Фелисити с опаской относится ко мне."

# game/status.rpy:7800
translate russia task21_d1497510:

    # a y12 "Which to be fair... we were the ones to put her in prison a couple of years ago."
    a y12 "Довольно справедливо... ведь мы посадили её за решётку пару лет назад."

# game/status.rpy:7801
translate russia task21_6a23a286:

    # y "Any way we can convince her?"
    y "Мы можем как-то переубедить её?"

# game/status.rpy:7802
translate russia task21_05bfee8c:

    # a "Well... she's pretty much still up to her old tricks. Kidnapping handsome guys to create her own personal harem of beefcakes."
    a "Ну... она всё ещё продолжает свои старые трюки. Похищает красивых парней, чтобы создать свой личный гарем."

# game/status.rpy:7803
translate russia task21_57150f03:

    # a y14 "There's just one guy who keeps eluding her. I bet if we kidnap and deliver him to Felicity, I'd earn some browny points!"
    a y14 "Есть только один парень, который продолжает ускользать от неё. Спорим, если мы похитим и доставим его Фелисити, она пересмотрит своё отношение ко мне!"

# game/status.rpy:7804
translate russia task21_26060448:

    # a "He's a super hot model that's living somewhere in the city."
    a "Он - супергорячая модель, который живёт где-то в городе."

# game/status.rpy:7805
translate russia task21_5e6b7df3:

    # y "Not sure if it's right for us to start kidnapping people."
    y "Не уверен, что нам стоит начинать похищать людей."

# game/status.rpy:7806
translate russia task21_3a073866:

    # a "I agree. We can't just take a guy off the streets, deliver him to a life of luxory and all the sex he could ever hope for!"
    a "Я согласна. Мы не можем просто взять парня и доставить его в роскошную жизнь и секс, о которой он только может мечтать!"

# game/status.rpy:7807
translate russia task21_57c51488:

    # y "Actually... {w}That doesn't sound so bad."
    y "Вообще-то... {w}Звучит не так уж и плохо."

# game/status.rpy:7808
translate russia task21_0fe11f97:

    # a y29 "But [playerName]! Kidnapping is wrong! Even if he'll enjoy the finest cooking, erotic steamy nights and won't have to lift a finger!"
    a y29 "Но [playerName]! Похищение - это неправильно! Даже если он будет наслаждаться беззаботной жизьню с самой изысканной кухней и эротическими ночами!"

# game/status.rpy:7809
translate russia task21_b00ad13b:

    # a "............."
    a "............."

# game/status.rpy:7810
translate russia task21_9308cfc3:

    # a y28 "Wow you're right, that does sound pretty good."
    a y28 "Вау, ты правы, звучит очень даже хорошо."

# game/status.rpy:7811
translate russia task21_02d20ee4:

    # a "He's usually seen walking near the beach."
    a "Обычно он гуляет возле пляжа."

# game/status.rpy:7812
translate russia task21_11a87ea9:

    # y "Okay, I'll stalk him for a while and when the area is safe, I'll call you to grab him."
    y "Хорошо, я прослежу за ним некоторое время. Когда всё будет чисто, я позвоню тебе, чтобы ты схватила его."

# game/status.rpy:7813
translate russia task21_87aee936:

    # a y1 "Okay!"
    a y1 "Отлично!"

# game/status.rpy:7823
translate russia task21_e6d19caa:

    # model "................"
    model "................"

# game/status.rpy:7824
translate russia task21_db4accef:

    # y "Target has been spotted..."
    y "Цель обнаружена..."

# game/status.rpy:7827
translate russia task21_9540d8d2:

    # y "(Best make sure that I don't lose him...)"
    y "(Его нельзя упустить...)"

# game/status.rpy:7828
translate russia task21_7ad48957:

    # "You follow the man closely down the street."
    "Вы довольно близко следуете за мужчиной по улице."

# game/status.rpy:7830
translate russia task21_9655c752:

    # y "(Following him at a reasonable distance should allow me to keep track of him without being noticed...)"
    y "(Идя за ним на разумном расстоянии я смогу оставаться незаметным...)"

# game/status.rpy:7831
translate russia task21_e9df1305:

    # "You wait for the man to begin his morning walk before casually following after him."
    "Вы ждёте, пока мужчина начнёт свою утреннюю прогулку, и небрежно следуете за ним."

# game/status.rpy:7833
translate russia task21_8ec8601e:

    # y "(There's no way I want to mess this up. Best be cautious and follow him at a distance.)"
    y "(Я ни за что не хочу всё испортить. Лучше быть осторожным и следовать за ним на расстоянии.)"

# game/status.rpy:7834
translate russia task21_00b0b5e4:

    # "You wait for the man to turn a corner before following after him."
    "Вы ждёте, пока мужчина свернёт за угол, а затем следуете за ним."

# game/status.rpy:7835
translate russia task21_e6d19caa_1:

    # model "................"
    model "................"

# game/status.rpy:7836
translate russia task21_7221e5fc:

    # model "Hm...?"
    model "Хм...?"

# game/status.rpy:7837
translate russia task21_42eccb09:

    # y "(Crap!)"
    y "(Чёрт!)"

# game/status.rpy:7838
translate russia task21_50bba383:

    # "You quickly duck behind a dumpster."
    "Вы быстро ныряете за мусорный контейнер."

# game/status.rpy:7839
translate russia task21_5c4ce518:

    # y "(Did he see me?)"
    y "(Он меня видел?)"

# game/status.rpy:7840
translate russia task21_0006d891:

    # model "....................."
    model "....................."

# game/status.rpy:7841
translate russia task21_355e4339:

    # "After a short while the man turns around and continues walking again."
    "Через некоторое время мужчина разворачивается и продолжает идти."

# game/status.rpy:7844
translate russia task21_990801fa:

    # y "(It's risky, but I can't let him out of my sights...)"
    y "(Это рискованно, но я не могу упустить его из виду...)"

# game/status.rpy:7845
translate russia task21_226e43f1:

    # "You get a little closer and continue following the man."
    "Вы подходите ближе и продолжаете следовать за мужчиной."

# game/status.rpy:7847
translate russia task21_ca0d08bc:

    # y "(I can't push my luck. Best follow him at a reasonable distance...)"
    y "(Нельзя испытывать судьбу. Лучше всего следовать за ним на разумном расстоянии...)"

# game/status.rpy:7848
translate russia task21_c80b4ab1:

    # "You wait for the man to cross the street before continuing the persuit."
    "Вы ждёте, пока мужчина перейдет улицу, прежде чем продолжить преследование."

# game/status.rpy:7850
translate russia task21_878923a4:

    # y "(He's a keen one. Best not to any risks and keep my distance.)"
    y "(Он увлёкся. Лучше не рисковать и держать дистанцию.)"

# game/status.rpy:7851
translate russia task21_7e902fb2:

    # "You give the man plenty of time to get out of reach before you persue him."
    "Вы даёте мужчине достаточно времени, чтобы уйти из зоны видимости, прежде чем следовать за ним."

# game/status.rpy:7852
translate russia task21_e6d19caa_2:

    # model "................"
    model "................"

# game/status.rpy:7853
translate russia task21_7221e5fc_1:

    # model "Hm...?"
    model "Хм...?"

# game/status.rpy:7854
translate russia task21_42eccb09_1:

    # y "(Crap!)"
    y "(Чёрт!)"

# game/status.rpy:7855
translate russia task21_f4e23b83:

    # "You quickly duck behind a tree."
    "Вы быстро прячетесь за деревом."

# game/status.rpy:7856
translate russia task21_ee76bfb3:

    # y "(Did he see me again?! This guy must have the eyes of an eagle and the ears of a... of a.... {w}of a thing that can hear really well.)"
    y "(Он снова меня видел?! У этого парня, должно быть, глаза орла и уши... уши.... {w}штуковины, которая слышит очень хорошо.)"

# game/status.rpy:7857
translate russia task21_aad31c77:

    # "After a short break, the man continues walking."
    "После короткого перерыва мужчина продолжает идти."

# game/status.rpy:7860
translate russia task21_c374c1c4:

    # y "(This is it... I just need to get closer and signal Alex to capture him!)"
    y "(Вот оно... Мне нужно подойти поближе и подать сигнал Алекс, чтобы схватить его!)"

# game/status.rpy:7861
translate russia task21_97c077ad:

    # "You begin making your way closer and closer to the man."
    "Вы начинаете пробираться всё ближе и ближе к мужчине."

# game/status.rpy:7863
translate russia task21_8c597868:

    # y "(Slow and steady... I don't want to blow this...)"
    y "(Медленно, но верно... Нельзя всё испортить...)"

# game/status.rpy:7864
translate russia task21_26b8f0f0:

    # "You give the man some time to continue walking as you pretend to be reading a newspaper...{w} An act which in it self is suspicious. {w}Who still reads newspapers now-a-days...?"
    "Вы даёте мужчине немного времени, делая вид, что читаете газету...{w} Поступок, который сам по себе очень подозрительный. {w}Кто ещё читает газеты в наши дни...?"

# game/status.rpy:7866
translate russia task21_278764f6:

    # y "(He must have the trained reflexes of a ninja! I should be extra carefull...)"
    y "(У него должно быть натренированные рефлексы как у ниндзя! Надо быть очень внимательным...)"

# game/status.rpy:7867
translate russia task21_a6775fe5:

    # "You wait until there's at least a football field distance between you and the man before continuing."
    "Вы ждёте, пока между вами не образуется расстояние в целое футбольное поле, прежде чем продолжить."

# game/status.rpy:7868
translate russia task21_e6d19caa_3:

    # model "................"
    model "................"

# game/status.rpy:7869
translate russia task21_7221e5fc_2:

    # model "Hm...?"
    model "Хм...?"

# game/status.rpy:7870
translate russia task21_e5c7f13a:

    # y "(Not again!)"
    y "(Нет, сново!)"

# game/status.rpy:7871
translate russia task21_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:7872
translate russia task21_f4463988:

    # y "Wait...."
    y "Стоп..."

# game/status.rpy:7873
translate russia task21_cca3e5b0:

    # "You walk up to the man."
    "Вы подходит к мужчине."

# game/status.rpy:7874
translate russia task21_60649344:

    # y "Erm... Excuse me."
    y "Эмм... Простите."

# game/status.rpy:7875
translate russia task21_e6d19caa_4:

    # model "................"
    model "................"

# game/status.rpy:7876
translate russia task21_7221e5fc_3:

    # model "Hm...?"
    model "Хм...?"

# game/status.rpy:7877
translate russia task21_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:7878
translate russia task21_eb6f83d7:

    # y "Hello...?"
    y "Алоо...?"

# game/status.rpy:7879
translate russia task21_e6d19caa_5:

    # model "................"
    model "................"

# game/status.rpy:7880
translate russia task21_7221e5fc_4:

    # model "Hm...?"
    model "Хм...?"

# game/status.rpy:7881
translate russia task21_3c6fe90d:

    # y "You can't actually hear me, can you...?"
    y "Вы меня не слышите, не так ли...?"

# game/status.rpy:7882
translate russia task21_cd2e896a:

    # y "Can you even see me...?"
    y "Вы меня хоть видите...?"

# game/status.rpy:7883
translate russia task21_39d99c4b:

    # "You wave your hand in front of the man's face."
    "Вы машете рукой перед его лицом."

# game/status.rpy:7884
translate russia task21_253a7736:

    # "After assessing the situation, you come to the conclusion that the model is both deaf and blind."
    "Оценив ситуацию, вы приходите к выводу, что модель глухой и слепой."

# game/status.rpy:7885
translate russia task21_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:7886
translate russia task21_94fd2e7e:

    # y "Well that should make this easier... Alex, get him!"
    y "Ну, это всё облегчит... Алекс, хватай его!"

# game/status.rpy:7887
translate russia task21_edddb6b3:

    # a "Roger!"
    a "Вас поняла!"

# game/status.rpy:7888
translate russia task21_0307ab12:

    # "Alex jumps off of the roof and lands on the visual and hearing impaired man!"
    "Алекс спрыгивает с крыши и приземляется на слабовидящего и слабослышащего мужчину!"

# game/status.rpy:7893
translate russia task21_5aea8445:

    # a "Well that was easy..."
    a "Ну, это было просто..."

# game/status.rpy:7894
translate russia task21_bff6b2e7:

    # y "Take him to the Outsiders and report back later tonight."
    y "Отведи его к Аутсайдерам и сообщи вечером о результате."

# game/status.rpy:7895
translate russia task21_4ad87ccb:

    # a y1 "Right!"
    a y1 "Хорошо!"

# game/status.rpy:7918
translate russia task21_7ee69a48:

    # a "Good news, [playerName]! I spoke to Felicity today."
    a "Хорошие новости, [playerName]! Сегодня я разговаривала с Фелисити."

# game/status.rpy:7919
translate russia task21_5c7c5abb:

    # y "Did she like our gift?"
    y "Ей понравился наш подарок?"

# game/status.rpy:7920
translate russia task21_95ad6006:

    # a y1 "I think so... We should be able to get close enough to her now. Set up a mission to the Carnival and we can take her down!"
    a y1 "Думаю, да... Теперь мы сможем подобраться к ней достаточно близко. Организуй миссию на Карнавале, и мы сможем её захватить!"

# game/status.rpy:7930
translate russia task21_f84f258e:

    # feli "Oh woe is me, captured by commoners!"
    feli "Ох горе мне, захваченной простолюдинами!"

# game/status.rpy:7931
translate russia task21_9efc9750:

    # y "Dramaaaaa!"
    y "Драаааама!"

# game/status.rpy:7932
translate russia task21_71fe8de3:

    # feli "...."
    feli "...."

# game/status.rpy:7933
translate russia task21_b1915e37:

    # feli "So you're behind the attacks against the Outsiders..."
    feli "Значит, это ты стоишь за нападениями на Аутсайдеров..."

# game/status.rpy:7934
translate russia task21_d9faf527:

    # y "Yup. And you must be Felicity."
    y "Ага. А ты, должно быть, Фелисити."

# game/status.rpy:7935
translate russia task21_f6343cc6:

    # feli "{b}*Hmph~...*{/b} That's Ms. Fences to you, worm."
    feli "{b}*Хмпф~...*{/b} Для тебя Мисс Фэнсэс, червяк."

# game/status.rpy:7936
translate russia task21_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:7938
translate russia task21_d0887b69:

    # a "Oh [playerName], I was just about to introduce you two."
    a "О, [playerName], я как раз собиралась познакомить вас двоих."

# game/status.rpy:7939
translate russia task21_5c759650:

    # feli "You....! Just because you brought me a hunky slave, doesn't mean I forgive you for this!"
    feli "Ты....! То, что ты привела мне смазливого раба, не значит, что я прощаю тебя за это!"

# game/status.rpy:7940
translate russia task21_5e6990c2:

    # feli "You will rue the day you cross path with Felicity Fen-..."
    feli "Вы будете жалеть о том дне, когда перешли дорогу Фелисити Фен-..."

# game/status.rpy:7941
translate russia task21_d2e41a4d:

    # y "Can I fuck her now?"
    y "Могу я её уже трахнуть?"

# game/status.rpy:7942
translate russia task21_46c17595:

    # a "Sure!"
    a "Конечно!"

# game/status.rpy:7943
translate russia task21_1b931449:

    # feli "Wait what?!"
    feli "Стоп что?!"

# game/status.rpy:7944
translate russia task21_9e8fe5fc:

    # y "You know. Have sex, break the nanobot control over you. It's pretty much par for the course at this point."
    y "Ну знаешь, заняться сексом, чтобы сломить контроль наноботов над тобой. На данный момент это вполне обычное дело."

# game/status.rpy:7945
translate russia task21_a21804a8:

    # feli "You donkey, I'm not being controlled by nanobots!"
    feli "Ты осёл, меня не контролируют наноботы!"

# game/status.rpy:7946
translate russia task21_ed8faeb5:

    # a y29 "Now that she mentions it... her eyes aren't red."
    a y29 "Теперь, когда она упомянула об этом... её глаза не красные."

# game/status.rpy:7947
translate russia task21_cd2b7ebf:

    # y "No person could be this unlikeable without being controlled!"
    y "Ни один человек не может быть настолько противным без контроля!"

# game/status.rpy:7948
translate russia task21_d60edb4b:

    # feli "Hey!"
    feli "Эй!"

# game/status.rpy:7949
translate russia task21_8e883d46:

    # a y2 "Better make sure! Get naked Felicity, [playerName] is going to destroy your puss."
    a y2 "Лучше убедиться! Раздевайся Фелисити, [playerName] уничтожить твою киску."

# game/status.rpy:7950
translate russia task21_b825e92b:

    # feli "!!!"
    feli "!!!"

# game/status.rpy:7951
translate russia task21_251b63f6:

    # y "Not so fast, Alex. We're not going to rape her."
    y "Не так быстро, Алекс. Мы не собираемся её насиловать."

# game/status.rpy:7952
translate russia task21_bcfe8046:

    # a y28 "But she's so annoying....!"
    a y28 "Но она так раздражает!"

# game/status.rpy:7953
translate russia task21_cf630fb5_1:

    # y "..................."
    y "..................."

# game/status.rpy:7954
translate russia task21_5ab42576:

    # y "You're scary sometimes, you know that?"
    y "Иногда ты пугаешь, знаешь ли?"

# game/status.rpy:7955
translate russia task21_c0185dcb:

    # feli "Nobody is raping anyone! I only sleep with the most handsome of guys!"
    feli "Никто никого не насилует! Я сплю только с самыми красивыми парнями!"

# game/status.rpy:7956
translate russia task21_41131256:

    # y "Handsome?"
    y "Красивыми?"

# game/status.rpy:7957
translate russia task21_7dc88d45:

    # feli "So if you want information out of me, you better bring me a hot stud!"
    feli "Так что если хочешь получить от меня информацию, лучше приведи мне горячего жеребца!"

# game/status.rpy:7958
translate russia task21_0d982bbd:

    # y "But Felicity..."
    y "Но Фелисити..."

# game/status.rpy:7959
translate russia task21_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:7960
translate russia task21_a3df21fd:

    # feli ".....?"
    feli ".....?"

# game/status.rpy:7968
translate russia task21_ec41a87d:

    # "You" "I {i}am{/i} a hot stud."
    "You" "{i}Я{/i} - горячий жеребец."

# game/status.rpy:7969
translate russia task21_e25c40eb:

    # feli "{b}*Gasp*{/b} So beautiful!"
    feli "{b}*Вздох*{/b} Такой красивый!"

# game/status.rpy:7970
translate russia task21_09177211:

    # feli "Okay I changed my mind. We are definitely going to have sex!"
    feli "Ладно, я передумала. У нас точно будет секс!"

# game/status.rpy:7973
translate russia task21_9a5497d2:

    # "Felicity" "Ah! Ahh! Ahhhh! ♥♥♥"
    "Фелисити" "Ах! ОаХ! АааХхх! ♥♥♥"

# game/status.rpy:7974
translate russia task21_0208e339:

    # "Felicity" "Yes! Oh God yes!"
    "Фелисити" "Да! О Боже, да!"

# game/status.rpy:7975
translate russia task21_20ada252:

    # "Felicity" "Oooooh yes, you beautiful creature!! ♥♥♥♥♥"
    "Фелисити" "ООоОааАх да, ты прекрасное создание!!! ♥♥♥♥♥"

# game/status.rpy:7976
translate russia task21_ef7ae042:

    # y "Say cheese...!"
    y "Скажи сыр...!"

# game/status.rpy:7986
translate russia task21_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:7992
translate russia task21_a71ae8a0:

    # feli "Phew~..."
    feli "Фух~..."

# game/status.rpy:7993
translate russia task21_6ddb654a:

    # y "Now how about you share some intel on the Outsiders..."
    y "А теперь, может, поделишься информацией об Аутсайдерах..."

# game/status.rpy:7994
translate russia task21_5c1606fb:

    # feli "Hmm....{w} Oh who cares. I was getting bored with them anyways."
    feli "Хмм....{w} Да какая разница. В любом случае они мне уже надоели."

# game/status.rpy:7995
translate russia task21_ca981af7:

    # "Felicity goes into detail about the Outsiders and the state of the gang."
    "Фелисити подробно рассказывает об Аутсайдерах и состоянии банды."

# game/status.rpy:8000
translate russia task21_d3bf5249:

    # feli "I was never good at keeping secrets anyways."
    feli "Я всё равно никогда не умела хранить секреты."

# game/status.rpy:8001
translate russia task21_ddfac715:

    # y "Doesn't sound like the gang is doing too hot."
    y "Не похоже, что у банды дела идут слишком хорошо."

# game/status.rpy:8002
translate russia task21_18a80ce0:

    # feli "Well you saw to that with your landgrab raids. Taking in the gang leader should be a piece of cake."
    feli "Это было видно во время войны за территории. Схватить главаря банды будет проще простого."

# game/status.rpy:8003
translate russia task21_eeaf7674:

    # feli "I hear he's into some kind of weird magic shit though."
    feli "Я слышала, что он увлекается каким-то странным магическим дерьмом."

# game/status.rpy:8004
translate russia task21_cf630fb5_2:

    # y "..................."
    y "..................."

# game/status.rpy:8013
translate russia task21_2a4bfcfd:

    # "Computer" "Computer: Felicity F.\nStatus: Captured."
    "Компьютер" "Компьютер: Фелисити Ф.\nСтатус: Захвачена."

# game/status.rpy:8014
translate russia task21_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:8021
translate russia task21_fe802fc0:

    # "New clothing options for Alex are available at the mall."
    "Новые варианты одежды для Алекс можно приобрести в торговом центре."

# game/status.rpy:8041
translate russia task22_a34dc0f6:

    # "With all the girls unlocked, you can now begin expanding the menu for your bar. When calling a Business Meeting you and the girls will throw out suggestions to add to the menu."
    "Теперь, когда все девушки разблокированы, вы можете начать расширять меню в баре. Во время деловых встреч вы и девушки можете предлагать блюда для расширения вашего меню."

# game/status.rpy:8043
translate russia task22_816ebe8d:

    # "These menu options are often more risqué, causing the customers to pay more for them. Meaning with more menu options, you'll make more money!"
    "Эти варианты меню будут более непристойными, тем самым клиенты будут платить за них больше. А это значит чем больше развратных вариантов будет в меню, тем больше денег вы заработаете."

# game/status.rpy:8044
translate russia task22_487fe95c:

    # "New menu options are turned on by default, but you can turn them off if you find them too risqué and prefer your spies do not preform them."
    "Новые опции меню включены по умолчанию, но вы можете отключить их, если считаете их слишком рискованными и предпочитаете, чтобы ваши шпионки не выполняли их."

# game/status.rpy:8045
translate russia task22_ecfeef86:

    # "To call a business meeting, simply visit the bar during daytime before you send spies out on their daily jobs. Then select the 'Business Meeting' option."
    "Чтобы назначить деловую встречу, просто посетите бар в дневное время, прежде чем отправлять шпионок на их ежедневную работу. Затем выберите опцию \"Деловая встреча\"."

# game/status.rpy:8046
translate russia task22_1d6c4f76:

    # "If there are no new options for the menu, then train your girls up a bit more. More options are unlocked as your spies get sluttier."
    "Если в меню нет новых вариантов, то потренируйте своих девочек ещё немного. По мере того как ваши шпионки становятся всё более развратными, открываются новые возможности."

# game/status.rpy:8076
translate russia task23_f8db232c:

    # "{b}♫♪ *Music* ♫♪{/b}"
    "{b}♫♪ *Музыка* ♫♪{/b}"

# game/status.rpy:8077
translate russia task23_f00ab12e:

    # y "Clover."
    y "Кловер."

# game/status.rpy:8078
translate russia task23_f8db232c_1:

    # "{b}♫♪ *Music* ♫♪{/b}"
    "{b}♫♪ *Музыка* ♫♪{/b}"

# game/status.rpy:8079
translate russia task23_11816677:

    # y "Clover..."
    y "Кловер..."

# game/status.rpy:8080
translate russia task23_f8db232c_2:

    # "{b}♫♪ *Music* ♫♪{/b}"
    "{b}♫♪ *Музыка* ♫♪{/b}"

# game/status.rpy:8082
translate russia task23_d0d84358:

    # y "CLOVER!"
    y "КЛОВЕР!"

# game/status.rpy:8083
translate russia task23_83a517f2:

    # c r18 "[playerName]? Did you say something?"
    c r18 "[playerName]? Ты что-то сказал?"

# game/status.rpy:8084
translate russia task23_512b7be0:

    # y "Whadda ya listening to?"
    y "Что ты слушаешь?"

# game/status.rpy:8085
translate russia task23_7ae4ffcc:

    # c r1 "This awesome new song that the Drift Punk just released. It's been playing everywhere."
    c r1 "Эта новая песня, которую Дрифт Панки недавно выпустили. Он уже везде."

# game/status.rpy:8086
translate russia task23_7305a27a:

    # y "Let me listen to it."
    y "Дай послушать."

# game/status.rpy:8087
translate russia task23_b14917ca:

    # "Clover hands you her earpods."
    "Кловер даёт вам её наушники."

# game/status.rpy:8088
translate russia task23_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:8089
translate russia task23_f7b83619:

    # y "This is pretty good!"
    y "Это довольно хорошо!"

# game/status.rpy:8091
translate russia task23_6266dd1c:

    # c "Yeah!"
    c "Ага!"

# game/status.rpy:8092
translate russia task23_f8f9efbb:

    # y "Makes me want to get up and dance!"
    y "Так и хочется танцевать!"

# game/status.rpy:8093
translate russia task23_75fdf0d8:

    # c "Exactly!"
    c "В точку!"

# game/status.rpy:8094
translate russia task23_c8e54245:

    # y "Makes me want to rob a bank!"
    y "А ещё хочется ограбить банк!"

# game/status.rpy:8096
translate russia task23_2dcb9e9b:

    # c r29 "Ye-... wait, what?"
    c r29 "Д-... стоп, что?"

# game/status.rpy:8097
translate russia task23_37b85f70:

    # y "Didn't you mention that Drift Punk were working on mind controlling music?"
    y "Разве ты не говорила, что Дрифт Панки занимаются гипнозом через музыку?"

# game/status.rpy:8098
translate russia task23_89630191:

    # c "Now that you mention it..."
    c "Теперь, когда ты об этом упомянул..."

# game/status.rpy:8099
translate russia task23_66427c79:

    # y "Time to investigate! Head over to Drift Punk and see what's going on."
    y "Время расследования! Направляйся к Дрифт Панкам и узнай, что происходит."

# game/status.rpy:8100
translate russia task23_46574440:

    # c r31 "Right."
    c r31 "Хорошо."

# game/status.rpy:8101
translate russia task23_8764527a:

    # "Clover nods and heads out."
    "Кловер кивает и выходит."

# game/status.rpy:8115
translate russia task23_b453ddca:

    # c "Well... I'm back."
    c "Нуу... Я вернулась."

# game/status.rpy:8116
translate russia task23_86edd8ea:

    # y "And?"
    y "И?"

# game/status.rpy:8117
translate russia task23_9bf0cdaa:

    # c "And... it's pretty bad. The Drift Punk plan to play their music over every radio station, intercom and speaker in this city. Telling people to hand over their valuables."
    c "И... всё довольно плохо. Дрифт Панки планируют включить свою музыку на всех радиостанциях, переговорных устройствах и динамиках в этом городе. Принуждая людей отдавать их ценности."

# game/status.rpy:8118
translate russia task23_f17cc94c:

    # y "Okay, then we stop them. Right?"
    y "Ладно, тогда мы должны их остановить. Верно?"

# game/status.rpy:8119
translate russia task23_e841479f:

    # c r12 "It's not quite that simple. We'd have to go into Punk Web and disable three transmitters which are kept under constant guard."
    c r12 "Всё не так просто. Нам прийдётся войти в Панк-Вэб и отключить три передатчика, которые находятся под постоянной охраной."

# game/status.rpy:8120
translate russia task23_c280009d:

    # c r11 "But... they have to be disabled at the same time."
    c r11 "А ещё... их нужно отключить одновременно."

# game/status.rpy:8121
translate russia task23_23782895:

    # y "So you girls will have to split up..."
    y "Значит вам, девочки, придётся разделиться..."

# game/status.rpy:8122
translate russia task23_8cdf398e:

    # y "Best get plenty of gadgets ready, this could be a tough cookie."
    y "Лучше всего подготовить много гаджетов, это может быть непростое задание."

# game/status.rpy:8123
translate russia task23_4e456ac3:

    # c r34 "Just plan a mission whenever you're ready. We'll take these gangsters down."
    c r34 "Просто спланируй миссию, когда будешь готов. Мы уничтожим этих бандитов."

# game/status.rpy:8137
translate russia task23_7cbd40ad:

    # y "Okay girls, split up."
    y "Ладно, девочки, расходитесь."

# game/status.rpy:8139
translate russia task23_f74dd4c2:

    # y "Sam, head in and find your transmitter room. The other girls won't be available for backup, so be careful."
    y "Сэм, иди внутрь и найди свою комнату с передачиком. Другие девушки не будут доступны для подстраховки, поэтому будь осторожна."

# game/status.rpy:8140
translate russia task23_c385ce8d:

    # sM "Got it."
    sM "Есть."

# game/status.rpy:8142
translate russia task23_d062536c:

    # y "Clover, head in and find your transmitter room. The other girls won't be available for backup, so be careful."
    y "Кловер, иди внутрь и найди свою комнату с передачиком. Другие девушки не будут доступны для подстраховки, поэтому будь осторожна."

# game/status.rpy:8143
translate russia task23_1e4918d1:

    # cM "Got it."
    cM "Хорошо."

# game/status.rpy:8145
translate russia task23_7d5e3760:

    # y "Alex, head in and find your transmitter room. The other girls won't be available for backup, so be careful."
    y "Алекс, иди внутрь и найди свою комнату с передачиком. Другие девушки не будут доступны для подстраховки, поэтому будь осторожна."

# game/status.rpy:8146
translate russia task23_dc566517:

    # aM "Got it."
    aM "Поняла."

# game/status.rpy:8158
translate russia task23_607f542b:

    # sM "There it is!"
    sM "Вот оно!"

# game/status.rpy:8160
translate russia task23_16d31bd9:

    # cM "There it is!"
    cM "Нашла!"

# game/status.rpy:8162
translate russia task23_dfca938b:

    # aM "There it is!"
    aM "Я на месте!"

# game/status.rpy:8163
translate russia task23_352466cc:

    # y "Okay good. Try turning it off with your hacking tool."
    y "Хорошо. Попробуйте отключить его с помощью инструмента взлома."

# game/status.rpy:8181
translate russia retryHack_8b6632cf:

    # sM "I'm in!"
    sM "Я внутри!"

# game/status.rpy:8183
translate russia retryHack_da77cba7:

    # y "Try again. Don't forget to use your boost."
    y "Попробуйте ещё раз. Не забывайте использовать свой бустеры."

# game/status.rpy:8188
translate russia retryHack_26b88db6:

    # y "Okay, standby. I'm switching camera."
    y "Хорошо, приготовьтесь. Я меняю камеру."

# game/status.rpy:8202
translate russia retryHack_cfeb2fdc:

    # y "Okay, stand by for my signal. Then all three turn off the transmission."
    y "Хорошо, приготовьтесь, по моему сигналу. Вы втроём выключаете передатчики."

# game/status.rpy:8209
translate russia retryHack_17da245e:

    # y "Three...... Two...... One......"
    y "Три...... Два..... Один..... "

# game/status.rpy:8211
translate russia retryHack_c2c8e785:

    # "???" "Not so fast..."
    "???" "Не так быстро..."

# game/status.rpy:8212
translate russia retryHack_9f698056:

    # cM "{b}*Yelp*!{/b}"
    cM "{b}*Визг*!{/b}"

# game/status.rpy:8213
translate russia retryHack_5d66c9f2:

    # y "Clover?!"
    y "Кловер?!"

# game/status.rpy:8214
translate russia retryHack_2f8cee33:

    # sM "[playerName]! What's going on?!"
    sM "[playerName]!Что происходит?!"

# game/status.rpy:8215
translate russia retryHack_db2b043e:

    # aM "Is Clover okay?!"
    aM "Кловер в порядке?!"

# game/status.rpy:8216
translate russia retryHack_c4a3f326:

    # "Suddenly a nearby tv screen turns on..."
    "Внезапно рядом включается экран..."

# game/status.rpy:8225
translate russia retryHack_be9d38c4:

    # "???" "Well well well, ain't this a surprise..."
    "???" "Так, так, так, какой сюрприз..."

# game/status.rpy:8226
translate russia retryHack_5892a647:

    # aM "{b}*Gasp*{/b} That voice...!"
    aM "{b}*Вздох*{/b} Этот голос...!"

# game/status.rpy:8227
translate russia retryHack_acd3b5d0:

    # sM "Sebastian Sage..."
    sM "Себастиан Сейдж..."

# game/status.rpy:8233
translate russia retryHack_d58cee7c:

    # seb "So nice to see you girls again. Up to the same tricks as usual I presume?"
    seb "Так приятно снова видеть вас, девочки. Как обычно, я полагаю, всё те же трюки?"

# game/status.rpy:8234
translate russia retryHack_c41d5ab2:

    # sM "What have you done to Clover?!"
    sM "Что ты сделал с Кловер?!"

# game/status.rpy:8235
translate russia retryHack_8dd374dc:

    # seb "Why don't you come and find out..."
    seb "Почему бы вам не прийти и не узнать..."

# game/status.rpy:8237
translate russia retryHack_96bc29fd:

    # "The transmission cut out."
    "Передача прервалась."

# game/status.rpy:8239
translate russia retryHack_1a8eaca0:

    # sM "Sounds like he was expecting us..."
    sM "Похоже, он ждал нас..."

# game/status.rpy:8240
translate russia retryHack_1c62a3e4:

    # y "You know him?"
    y "Вы его знаете?"

# game/status.rpy:8241
translate russia retryHack_6a8f3c56:

    # aM "We fought him a few times. He's caused riots in Beverly Hills before."
    aM "Мы уже сражались с ним несколько раз. Он и раньше вызывал беспорядки в Беверли-Хиллз."

# game/status.rpy:8242
translate russia retryHack_456d5195:

    # sM "We managed to stop him then and we'll do it again! He was transmitting from this location, meet me there Alex."
    sM "Нам удалось остановить его тогда, и мы сделаем это снова! Передача шла от сюда, встретимся там, Алекс."

# game/status.rpy:8243
translate russia retryHack_0c6af985:

    # aM "Roger!"
    aM "Поняла!"

# game/status.rpy:8263
translate russia retryHack_6be4627d:

    # a "There you are!"
    a "Вот ты где!"

# game/status.rpy:8264
translate russia retryHack_01325619:

    # s "What did you do to Clover?!"
    s "Что ты сделал с Кловер?!"

# game/status.rpy:8265
translate russia retryHack_41133902:

    # seb "Clover? Oh she's right here..."
    seb "Кловер? Ах, да, вот она..."

# game/status.rpy:8269
translate russia retryHack_fd0e5131:

    # a y28 "Clover! You're okay!"
    a y28 "Кловер! Ты в порядке!"

# game/status.rpy:8270
translate russia retryHack_8b5f2bd4:

    # c "Y O U - A R E - S E B A S T I A N - A N D - Y O U - A R E - M Y - M A S T E R...."
    c "В Ы - С Е Б А С Т И А Н - И - В Ы - М О Й - М А С Т Е Р...."

# game/status.rpy:8271
translate russia retryHack_e150458a:

    # y "That doesn't sound good..."
    y "Это звучит не очень хорошо..."

# game/status.rpy:8272
translate russia retryHack_89beb739:

    # s g32 "We've seen this before! He's mind controlled her with his music!"
    s g32 "Мы это уже видели! Он управлял её сознанием с помощью своей музыки!"

# game/status.rpy:8273
translate russia retryHack_60a946e9:

    # a "Snap out of it, Clover!"
    a "Опомнись, Кловер!"

# game/status.rpy:8274
translate russia retryHack_9b7c25ba:

    # c "I - W I L L - O B E Y - A N D - S P R E A D - D I S A S T E R....."
    c "Я - П О В И Н У Ю С Ь - И - Р А С П Р О С Т Р А Н Я Ю - Б Е Д С Т В И Е....."

# game/status.rpy:8282
translate russia retryHack_7b11e5c3:

    # "Clover attacks!"
    "Кловер атакует!"

# game/status.rpy:8300
translate russia retryHack_2c596030:

    # y "She's being controlled. Attack and break her free!"
    y "Её контролируют. Атакуйте и освободите её!"

# game/status.rpy:8317
translate russia retryHack_054fd4cf:

    # s "Clover...? Are you with us again?"
    s "Кловер...? Ты снова с нами?"

# game/status.rpy:8318
translate russia retryHack_a76f5bb6:

    # c "Yeah... I think so."
    c "Да... Думаю, да."

# game/status.rpy:8319
translate russia retryHack_fdd23b41:

    # a y31 "That's enough Sebastian! You're going dow-...!"
    a y31 "Хватит, Себастиан! Ты проиг-...!"

# game/status.rpy:8320
translate russia retryHack_5a81a468:

    # a y29 "...?"
    a y29 "...?"

# game/status.rpy:8321
translate russia retryHack_aaf83286:

    # a "Where did he go?"
    a "Куда он делся?"

# game/status.rpy:8322
translate russia retryHack_72733c1f:

    # c r32 "He must've ran away when we were fighting."
    c r32 "Должно быть, он сбежал, пока мы дрались."

# game/status.rpy:8323
translate russia retryHack_e9ee1e1c:

    # a y28 "Nooo! We were so close to capturing him!"
    a y28 "Нееет! Мы были так близки к тому, чтобы поймать его!"

# game/status.rpy:8324
translate russia retryHack_2e948a07:

    # s g33 "I guess it's a mission failed then..."
    s g33 "Получается миссия провалена..."

# game/status.rpy:8325
translate russia retryHack_9cf694cc:

    # c r49 "................"
    c r49 "................"

# game/status.rpy:8326
translate russia retryHack_fa3b6e08:

    # y "Actually, I've got an idea..."
    y "Вообще-то, у меня есть идея..."

# game/status.rpy:8327
translate russia retryHack_362883a8:

    # y "We still have to shut down the signal. Everyone get to your stations and wait for my command."
    y "Нам всё ещё нужно отключить сигнал. Всем занять свои места и ждать моей команды."

# game/status.rpy:8328
translate russia retryHack_a80ed38b:

    # "The girls nod and return to the signal rooms."
    "Девушки кивают и возвращаются на свои места."

# game/status.rpy:8341
translate russia retryHack_a8e58877:

    # sM "Sam, in position."
    sM "Сэм, на месте."

# game/status.rpy:8342
translate russia retryHack_46c76c77:

    # aM "Alex here. Ready when you are."
    aM "Алекс здесь. Я готова."

# game/status.rpy:8343
translate russia retryHack_a5c4548e:

    # cM "What was this idea you had, [playerName]?"
    cM "Что у тебя была за идея, [playerName]?"

# game/status.rpy:8344
translate russia retryHack_f9f82172:

    # y "Here... instead of shutting down the signal, I've changed it a bit."
    y "Вместо того чтобы отключить сигнал, я предлагаю немного изменить его."

# game/status.rpy:8345
translate russia retryHack_363f8827:

    # cM "Changed it...?"
    cM "Изменить...?"

# game/status.rpy:8346
translate russia retryHack_868957e7:

    # y "You'll see~...."
    y "Увидите~...."

# game/status.rpy:8347
translate russia retryHack_1ab636cf:

    # y "Okay get ready. On three...\nTwo....\nOne..."
    y "Приготовьтесь. Три...\nДва....\nОдин..."

# game/status.rpy:8348
translate russia retryHack_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бип*{/b}"

# game/status.rpy:8349
translate russia retryHack_9ec4c82f:

    # "The girls change the signal at the same time."
    "Девушки одновременно меняют сигнал."

# game/status.rpy:8350
translate russia retryHack_5bdd22cf:

    # sM ".............."
    sM ".............."

# game/status.rpy:8351
translate russia retryHack_777f8635:

    # sM "Did it work...?"
    sM "Получилось...?"

# game/status.rpy:8352
translate russia retryHack_f3a16c85:

    # "???" "{b}*Groans*{/b}"
    "???" "{b}*Стонет*{/b}"

# game/status.rpy:8353
translate russia retryHack_e3f89b0f:

    # sM "What was that?!"
    sM "Что это было?!"

# game/status.rpy:8357
translate russia retryHack_fd41f3b7:

    # seb "I - A M - D R I F T - P U N K - A N D - I - P R O M I S E - T O - T U R N - M Y S E L F - I N~....."
    seb "Я - Д Р И Ф Т - П А Н К - И - Я - О Б Е Щ А Ю - С Д А Т Ь С Я~....."

# game/status.rpy:8358
translate russia retryHack_4bcfa726:

    # aM "!!!"
    aM "!!!"

# game/status.rpy:8359
translate russia retryHack_75dcce55:

    # aM "Sebastian...?"
    aM "Себастиан...?"

# game/status.rpy:8360
translate russia retryHack_941d1fc3:

    # y "I changed the hypnotic message of the signal. The Drift Punk should be a lot more agreeable now."
    y "Я изменил гипнотическое послание сигнала. Теперь Дрифт Панки должен быть гораздо более сговорчивым."

# game/status.rpy:8361
translate russia retryHack_1d3b3daa:

    # cM "Well done, [playerName]. We'll take Sebastian back to the base."
    cM "Отличная работа, [playerName]. Мы отвезём Себастьяна обратно на базу."

# game/status.rpy:8362
translate russia retryHack_a0fb90c4:

    # seb "C O M E - V I S I T - M E G A - M I L K - F O R - 10 P E R C E N T - O F F - ON - M I L K S H A K E S~....."
    seb "П Р И Х О Д И Т Е - В - М Е Г А - М И Л К - З А - 10 - П Р О Ц Е Н Т Н О Й - С К И Д К О Й - Н А - М О Л О Ч Н Ы Е - К О К Т Е Й Л И~....."

# game/status.rpy:8375
translate russia retryHack_76117e08:

    # seb "D R I F T - P U N K - I S - L A M E - A N D - I - H A VE - A - S M A L L - P E~....."
    seb "Д Р И Ф Т - П А Н К - О Т С Т О Й - А - У - М Е Н Я - М А Л Е Н Ь К И Й - П Е~....."

# game/status.rpy:8377
translate russia retryHack_52abd0ef:

    # seb "{b}*Blinks*{/b}"
    seb "{b}*Моргает*{/b}"

# game/status.rpy:8378
translate russia retryHack_122ff608:

    # seb "..........................."
    seb "..........................."

# game/status.rpy:8379
translate russia retryHack_97a350e6:

    # y "Back to normal?"
    y "Очнулся?"

# game/status.rpy:8380
translate russia retryHack_263a55fd:

    # seb "{b}*Ahem*{/b} So it would seem..."
    seb "{b}*Кхем*{/b} Похоже на то..."

# game/status.rpy:8381
translate russia retryHack_3460f613:

    # seb ".........................................."
    seb ".........................................."

# game/status.rpy:8382
translate russia retryHack_2fa86a47:

    # seb "Where the hell am I?"
    seb "Где я, чёрт возьми, нахожусь?"

# game/status.rpy:8383
translate russia retryHack_8f0ca594:

    # y "You're in the WOOHP detention area in Beverly Hills. See? All your friends are here!"
    y "Ты находишься в зоне содержания WOOHP в Беверли-Хиллз. Видишь? Все твои друзья здесь!"

# game/status.rpy:8384
translate russia retryHack_27e4849b:

    # tali "Hey."
    tali "Привет."

# game/status.rpy:8385
translate russia retryHack_b5c49b89:

    # drag "Good to see you again, Seb."
    drag "Рада снова тебя видеть, Себ."

# game/status.rpy:8386
translate russia retryHack_6ab26a07:

    # seb "..............................."
    seb "..............................."

# game/status.rpy:8387
translate russia retryHack_288fe832:

    # seb "I was wondering where you two went..."
    seb "Мне было интересно, куда вы двое делись..."

# game/status.rpy:8388
translate russia retryHack_ce94dd43:

    # seb "As for you my friend, I'm afraid you haven't accomplished much."
    seb "Что касается тебя, мой друг, мне жаль что ты не достиг своих целей."

# game/status.rpy:8389
translate russia retryHack_1dbeb8bf:

    # seb "Drift Punk is made to last. We'll come back around when the backups kick in."
    seb "Дрифт Панки будут жить вечно. Мы обязательно вернёмся с новыми силами."

# game/status.rpy:8390
translate russia retryHack_fbd5f1d3:

    # y "And what makes you so sure about that?"
    y "Почему ты в этом так уверен?"

# game/status.rpy:8391
translate russia retryHack_c2aad595:

    # seb "She guarantees it."
    seb "Она это гарантирует."

# game/status.rpy:8392
translate russia retryHack_18ca1a17:

    # y "She?"
    y "Она?"

# game/status.rpy:8394
translate russia retryHack_95536339:

    # s g53 "The Mastermind?"
    s g53 "Организатор?"

# game/status.rpy:8396
translate russia retryHack_f13527f4:

    # seb "Oh? You haven't figured out who it is yet? Well don't let me spoil it for you~..."
    seb "О? Вы ещё не поняли, кто за этим стоит? Не буду портить вам сюрприз~..."

# game/status.rpy:8397
translate russia retryHack_16b7d5f9:

    # s g54 "Tell us Sebastian! The gig is up. We have ways of making you talk!"
    s g54 "Шутки в сторону. Скажи нам Себастиан! У нас есть способы заставить тебя говорить!"

# game/status.rpy:8398
translate russia retryHack_886e7972:

    # y "I'm not sleeping with him, if that's what you're suggesting!"
    y "Я не буду с ним спать, если ты на это намекаешь!"

# game/status.rpy:8399
translate russia retryHack_b5c00f55:

    # seb "I didn't break the first time you girls caught me. Nor did I break the second time. You should already know torture doesn't work on me."
    seb "Я не сломался, когда вы, девочки, поймали меня в первый раз. Я не сломался и во второй раз. Вы уже должны знать, что пытки на меня не действуют."

# game/status.rpy:8400
translate russia retryHack_e4e10500:

    # s g52 "........................."
    s g52 "........................."

# game/status.rpy:8401
translate russia retryHack_ed6243dd:

    # s "WOOHP doesn't torture..."
    s "WOOHP не пытает..."

# game/status.rpy:8402
translate russia retryHack_db5b416b:

    # seb "Nice try Ms. Simpson. But we both know that isn't true."
    seb "Хорошая попытка, Мисс Симпсон. Но мы оба знаем, что это неправда."

# game/status.rpy:8403
translate russia retryHack_3c6c49b9:

    # y "Simpson? You're last name is Sim-...?"
    y "Симпсон? Твоя фамилия Сим-...?"

# game/status.rpy:8404
translate russia retryHack_94de9e32:

    # seb "If you're unsure, why not check the WOOHP prison logs. I'm sure they'll clearify a lot."
    seb "Если ты не уверена, почему бы не проверить тюремные журналы WOOHP. Я уверен, что они многое прояснят."

# game/status.rpy:8405
translate russia retryHack_8c76bed9:

    # y "I don't think we'll get much out of this guy. Let's go Sam."
    y "Не думаю, что мы чего-нибудь добьёмся от этого парня. Пойдём, Сэм."

# game/status.rpy:8415
translate russia retryHack_1a92a22b:

    # c "What did he say?"
    c "Что он сказал?"

# game/status.rpy:8416
translate russia retryHack_b129c5c2:

    # s "..........................."
    s "..........................."

# game/status.rpy:8417
translate russia retryHack_740b8874:

    # y "He's not cooperating. Though he said something about torture."
    y "Он не сотрудничает. Хотя он что-то говорил о пытках."

# game/status.rpy:8418
translate russia retryHack_d4278664:

    # c "Torture?"
    c "Пытках?"

# game/status.rpy:8420
translate russia retryHack_f3e1cb1c:

    # a y28 "Candy said the same thing!"
    a y28 "Кэнди говорила то же самое!"

# game/status.rpy:8421
translate russia retryHack_ce679276:

    # s g40 "That's ridiculous! WOOHP does not torture its prisoners!"
    s g40 "Это смешно! WOOHP не пытает своих заключенных!"

# game/status.rpy:8422
translate russia retryHack_640609ec:

    # s g9 "I bet they all got together to make up this story to confuse us if they ever got captured!"
    s g9 "Наверняка они всё сговорились, чтобы запутать нас, если их когда-нибудь схватят!"

# game/status.rpy:8423
translate russia retryHack_a38b2e39:

    # c r10 "I dunno... We've seen WOOHP do some creepy stuff in the past."
    c r10 "Не знаю... В прошлом мы уже видели, как WOOHP делает жуткие вещи."

# game/status.rpy:8424
translate russia retryHack_dded7e5b:

    # c "Remember that Jerry tied us up over burning lava and slowly lowered us down?"
    c "Помните, как Джерри связал нас над горящей лавой и медленно опускал вниз?"

# game/status.rpy:8425
translate russia retryHack_83bdd7ae:

    # s g29 "But that was just a training mission! He wouldn't actually have killed us!"
    s g29 "Но это была всего лишь учебная миссия! Он бы нас не убил!"

# game/status.rpy:8426
translate russia retryHack_2eecf216:

    # a y5 "Yeah... It's still a bit messed up how he paid an actor to be my boyfriend..."
    a y5 "Да... И всё ещё не совсем понятно, как он заплатил актёру, чтобы тот стал моим парнем..."

# game/status.rpy:8427
translate russia retryHack_921048b2:

    # c r30 "And remember that time we had to sneak around the WOOHP HQ at night? That was super creepy!"
    c r30 "А помните тот случай, когда нам пришлось пробираться по штаб-квартире WOOHP ночью? Это было очень жутко!"

# game/status.rpy:8428
translate russia retryHack_7a4705f0:

    # s g38 "Well true, but..."
    s g38 "Это правда, но..."

# game/status.rpy:8429
translate russia retryHack_0c470817:

    # s g39 "..........................."
    s g39 "..........................."

# game/status.rpy:8430
translate russia retryHack_b1cc3bd6:

    # y "I guess Jerry has a few questions to awnser once we rescue him."
    y "Думаю, Джерри прийдётся ответить на несколько вопросов, когда мы его спасём."

# game/status.rpy:8431
translate russia retryHack_dd6736b7:

    # y "Any idea who the 'she' is that Sebastian was talking about?"
    y "Есть идеи, кем может быть \"она\", о которой говорил Себастиан?"

# game/status.rpy:8432
translate russia retryHack_bb709a24:

    # a "Could be anyone. We fought a lot of crazy villains in our day."
    a "Это может быть кто угодно. В свое время мы сражались со многими сумасшедшими злодеями."

# game/status.rpy:8433
translate russia retryHack_40efd9c2:

    # y "Let's try capturing more gang leaders and see if that shines a light on things."
    y "Давайте попробуем захватить больше главарей банд и посмотрим, поможет ли это нам узнать побольше."

# game/status.rpy:8434
translate russia retryHack_5cbff07d:

    # y "Good work spies."
    y "Отличная работа шпионки."

# game/status.rpy:8435
translate russia retryHack_defe521b:

    # s g1 "Heh, you kinda sound like Jerry when you say that."
    s g1 "Хех, ты говоришь как Джерри."

# game/status.rpy:8436
translate russia retryHack_4d23253b:

    # a y28 "He's sorta Jerry 2.0!"
    a y28 "Он вроде как Джерри 2.0!"

# game/status.rpy:8437
translate russia retryHack_3ab253b5:

    # c r12 "Without the... 'tying up above burning lava' thing."
    c r12 "Без этих... \"подвешиваний над горящей лавой\"."

# game/status.rpy:8438
translate russia retryHack_93f898c0:

    # y "Oh you'll be tied up soon enough."
    y "Ох, ты очень скоро будешь связан."

# game/status.rpy:8439
translate russia retryHack_59f58562:

    # s g30 "Wha-...?"
    s g30 "Чт-...?"

# game/status.rpy:8440
translate russia retryHack_c7590795:

    # y "Goodnight ladies!"
    y "Спокойной ночи, дамы!"

# game/status.rpy:8447
translate russia retryHack_eade8a27:

    # "Computer" "Booting up WOOHP Bounty Board..."
    "Компьютер" "Загрузка WOOHP Д.П. ..."

# game/status.rpy:8448
translate russia retryHack_f3ed2f81:

    # "Computer" "Target: Sebastian Saga\nStatus: Captured."
    "Компьютер" "Цель: Себастиан Сейдж\nСтатус: Захвачен."

# game/status.rpy:8449
translate russia retryHack_2f1ad96c:

    # "Computer" "Transferring funds..."
    "Компьютер" "Перевожу награду..."

# game/status.rpy:8476
translate russia retryHack_883fcc59:

    # "New clothing options for Clover are available at the mall."
    "Новые варианты одежды для Кловер можно приобрести в торговом центре."

# game/status.rpy:8495
translate russia task24_18d64b94:

    # y "The reports of gang attacks from the Drift Punks seem to have gone down."
    y "Сообщений о нападениях со стороны Дрифт Панков становиться всё меньше и меньше."

# game/status.rpy:8496
translate russia task24_a1f0da02:

    # c "Yeah..."
    c "Ага..."

# game/status.rpy:8497
translate russia task24_635dad0a:

    # y "And there's been fewer digital hacks lately!"
    y "И в последнее время стало меньше новостей о всяческих взломах!"

# game/status.rpy:8498
translate russia task24_98a50a78:

    # c "Mhm hm..."
    c "Мхм хм..."

# game/status.rpy:8499
translate russia task24_0dd67e52:

    # y "Not to mention they haven't been playing their music too loudly at night!"
    y "Не говоря уже о том, что их громкую музыку неслышно по ночам!"

# game/status.rpy:8500
translate russia task24_b412ce5b:

    # c "Yes, hm, okay..."
    c "Да, хм, хорошо..."

# game/status.rpy:8501
translate russia task24_68fb1940:

    # y "Also I'm selling you as a slave to a Nigerian warlord."
    y "А ещё я продаю тебя в рабство нигерийскому военачальнику."

# game/status.rpy:8502
translate russia task24_23bf4157:

    # c "Uh huh, yeah..."
    c "Ух хух, ага..."

# game/status.rpy:8503
translate russia task24_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:8504
translate russia task24_f79218da:

    # y "Are you even listening to me?"
    y "Ты вообще меня слушаешь?"

# game/status.rpy:8505
translate russia task24_5c87e5b0:

    # c r29 "Huh? Oh sorry, [playerName]. I'm a bit distracted."
    c r29 "А? О, прости, [playerName]. Я немного задумалась."

# game/status.rpy:8506
translate russia task24_8e292182:

    # c "I keep thinking of....{w} Kobolds."
    c "Я продолжаю думать о....{w} кобольдах."

# game/status.rpy:8507
translate russia task24_ac17dec1:

    # y "What?"
    y "Что?"

# game/status.rpy:8508
translate russia task24_be126726:

    # c r1 "Kobolds. They're little creatures we came across in our last D&D session."
    c r1 "Кобольды. Это маленькие существа, с которыми мы столкнулись во время нашей последней сессии D&D."

# game/status.rpy:8509
translate russia task24_f4465cc1:

    # y "You play D&D...?"
    y "Ты играешь в D&D...?"

# game/status.rpy:8510
translate russia task24_6d817ab2:

    # c "I told you I was into nerdy stuff. I've been playing tabletop games with some people from Drift Punk."
    c "Я же говорила тебе, что мне нравятся всякие гиковские штучки. Я играл в настольные игры с некоторыми членами Дрифт Панков."

# game/status.rpy:8511
translate russia task24_e1330c35:

    # c r12 "My dungeon master mentioned wanting to get some Kobold miniatures, but couldn't because the guy that used to sell them is being held prisoner by the Outsiders."
    c r12 "Мой мастер подземелий упоминал, что хотел достать миниатюры кобольдов, но не смог. Всё потому что парень, который их продавал, находится в плену у Аутсайдеров."

# game/status.rpy:8512
translate russia task24_f37e0d0e:

    # y "Wait... what was this about a dungeon master? Are you getting up to some freaky sex stuff..?"
    y "Подожди... что там было насчёт мастера подземелий? Ты занимаешься каким-то странными пошлостями...?"

# game/status.rpy:8513
translate russia task24_221953b5:

    # c r1 "If we plan a mission to the Carnival, I bet we could free him! I would buy so much junk for my cell from him!"
    c r1 "Если мы спланируем миссию на Карнавале, я уверена, мы сможем освободить его! Я бы купила у него столько всякого барахла для своей комнаты!"

# game/status.rpy:8514
translate russia task24_f5a70003:

    # c "Please send us on a mission, [playerName]!"
    c "Пожалуйста, отправь нас на задание, [playerName]!"

# game/status.rpy:8515
translate russia task24_3dea941d:

    # y "I still don't understand everything you're saying, but if the Outsiders are holding a hostage, then I'm sure we can free him."
    y "Я всё ещё не понимаю, о чём ты говоришь. Но если Аутсайдеры держат заложника, то я уверен, что мы сможем его освободить."

# game/status.rpy:8516
translate russia task24_70b16e68:

    # "Clover squees and gives you a hug."
    "Кловер визжит и обнимает вас от радости."

# game/status.rpy:8526
translate russia task24_aee3a441:

    # "A hostage is available for rescue at the Carnival."
    "На карнавале можно спасти заложника."

# game/status.rpy:8544
translate russia task24_d69e41a0:

    # "{b}*Knock* *Knock* *Knock*{/b}"
    "{b}*Тук* *Тук* *Тук*{/b}"

# game/status.rpy:8545
translate russia task24_577b9fb5:

    # "???" "Let me out, you hear! You'll be hearing from my lawyer!"
    "???" "Выпустите меня, слышите! Вы получите весточку от моего адвоката!"

# game/status.rpy:8546
translate russia task24_3851fd2c:

    # y "That could be the store owner. Get him out of there."
    y "Возможно это владелец магазина. Вытащите его оттуда."

# game/status.rpy:8552
translate russia task24_c0390f4f:

    # shopOwner "Gah finally. The fools locked me up to paint the inside, but there were no open windows! I nearly sufficated on the paint fumes!"
    shopOwner "Фух, наконец-то. Эти дураки заперли меня, чтобы разрисовать всё изнутри, но забыли открыть оконна! Я чуть не задохнулся там!"

# game/status.rpy:8553
translate russia task24_9172c700:

    # y "We've come to get you out."
    y "Мы пришли, чтобы вытащить тебя."

# game/status.rpy:8554
translate russia task24_b90225f7:

    # shopOwner "Wonderful, then I can finally get back to my store. Visit me at the mall and I'll have a reward for you."
    shopOwner "Прекрасно, тогда я наконец-то смогу вернуться в свой магазин. Навестите меня в торговом центре, и у есть чем вас отблагодарить."

# game/status.rpy:8555
translate russia task24_dff662c6:

    # y "Okay, we can pull out now or continue."
    y "Хорошо, мы можем вернуться сейчас или продолжить."

# game/status.rpy:8593
translate russia task25_4734f034:

    # a "[playerName]? Alex here."
    a "[playerName]? Это Алекс."

# game/status.rpy:8594
translate russia task25_35d894b2:

    # a "There's... nobody at the amusement park."
    a "Тут... в парке аттракционов никого нет."

# game/status.rpy:8595
translate russia task25_c66297f2:

    # y "Nobody?"
    y "Никого?"

# game/status.rpy:8596
translate russia task25_39ee823b:

    # a y21 "No, it's completely empty. Kind of creepy..."
    a y21 "Нет, он совершенно пустой. Жутковато..."

# game/status.rpy:8597
translate russia task25_5d922624:

    # a "I only found these strange bottles..."
    a "Я нашела только эти странные бутылки..."

# game/status.rpy:8604
translate russia task25_6c8eaea1:

    # y "Bottles...?"
    y "Бутылки...?"

# game/status.rpy:8605
translate russia task25_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:8606
translate russia task25_336844af:

    # y "They look like potions."
    y "Выгледят как зелья."

# game/status.rpy:8607
translate russia task25_b60d1a88:

    # a "Potions?"
    a "Зелья?"

# game/status.rpy:8608
translate russia task25_b3cd0c31:

    # y "Yeah like magical alchemy drinks."
    y "Да, как волшебные алхимические напитки."

# game/status.rpy:8613
translate russia task25PotionDrink_cbd992d7:

    # "Alex takes a sip of the red vial."
    "Алекс делает глоток из красного флакона."

# game/status.rpy:8614
translate russia task25PotionDrink_0bb2bbe8:

    # "Nothing seems to happen."
    "Кажется, ничего не происходит."

# game/status.rpy:8617
translate russia task25PotionDrink_116e157f:

    # "Alex takes a sip of the blue vial."
    "Алекс делает глоток из синего флакона."

# game/status.rpy:8618
translate russia task25PotionDrink_0bb2bbe8_1:

    # "Nothing seems to happen."
    "Кажется, ничего не происходит."

# game/status.rpy:8624
translate russia task25PotionDrink_8dafa479:

    # "Alex takes a sip of the green vial."
    "Алекс делает глоток из зелёного флакона."

# game/status.rpy:8627
translate russia task25PotionDrink_7fb3891b:

    # a y30 "!!!"
    a y30 "!!!"

# game/status.rpy:8628
translate russia task25PotionDrink_1acc6502:

    # y "Alex? Are you oka-...?"
    y "Алекс? Ты в поря-...?"

# game/status.rpy:8629
translate russia task25PotionDrink_39b04578:

    # a y28 "GHOSTS!"
    a y28 "ПРИЗРАКИ!"

# game/status.rpy:8630
translate russia task25PotionDrink_366c7716:

    # y "Now Alex calm down, there's no such thing as g-..."
    y "Алекс успокойся, не существует такой вещи, как п-..."

# game/status.rpy:8631
translate russia task25PotionDrink_6c9a592f:

    # a y42 "Eeeeeeeeeeh!"
    a y42 "ИИиииииИИИиИ!"

# game/status.rpy:8635
translate russia task25PotionDrink_b2cfe249:

    # "Alex ran away."
    "Алекс убегает."

# game/status.rpy:8636
translate russia task25PotionDrink_64a2b68e:

    # y "..........................."
    y "..........................."

# game/status.rpy:8653
translate russia task25PotionDrink_cfefe697:

    # a y28 "[playerName]! Ghosts!"
    a y28 "[playerName]! Призраки!"

# game/status.rpy:8654
translate russia task25PotionDrink_7eeb5b84:

    # y "Alex are you okay...?"
    y "Алекс, ты в порядке...?"

# game/status.rpy:8655
translate russia task25PotionDrink_bace8cae:

    # a "I'M SERIOUS!"
    a "Я СЕРЬЁЗНО!"

# game/status.rpy:8659
translate russia task25PotionDrink_d66b25f5:

    # s "What's going o-... Alex? You look like you've seen a g-"
    s "Что происхо-... Алекс? Ты выглядишь так, словно увидела при-"

# game/status.rpy:8660
translate russia task25PotionDrink_b31c2da3:

    # a "GHOST!"
    a "ПРИЗРАК!"

# game/status.rpy:8661
translate russia task25PotionDrink_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:8662
translate russia task25PotionDrink_ed67ea48:

    # c r16 "Alex, there's no such thing as ghosts."
    c r16 "Алекс, призраков не существует."

# game/status.rpy:8663
translate russia task25PotionDrink_83d06cd1:

    # a y29 "But I saw them! A lot of them!"
    a y29 "Но я видела! Их много!"

# game/status.rpy:8664
translate russia task25PotionDrink_e7cb5905:

    # a "All the Outsiders turned into ghosts!"
    a "Все Аутсайдеры превратились в призраков!"

# game/status.rpy:8665
translate russia task25PotionDrink_b7909604:

    # y "Okay, just relax. You're going back tomorrow. Maybe everyone was just out on a heist of something."
    y "Ладно, просто расслабься. Ты вернёшься завтра. Может быть, все были на ограблении."

# game/status.rpy:8666
translate russia task25PotionDrink_450630e3:

    # a "But....!"
    a "Но...!"

# game/status.rpy:8667
translate russia task25PotionDrink_3c0234d3:

    # y "If you're so worried, I'll send Sam and Clover along with you."
    y "Если ты так беспокоишься, я отправлю Сэм и Кловер с тобой."

# game/status.rpy:8668
translate russia task25PotionDrink_e019f476:

    # a y5 "{b}*Whines Softly*{/b}"
    a y5 "{b}*Хнычет*{/b}"

# game/status.rpy:8671
translate russia task25PotionDrink_e0870a0f:

    # y "Wow.... I knew Alex was a scaredy cat, but she looks terrified."
    y "Вау.... Я знал, что Алекс трусишка, но она выглядит ужасно."

# game/status.rpy:8672
translate russia task25PotionDrink_a4188dd0:

    # c r10 "Yeah... maybe she really did see something."
    c r10 "Да... может, она действительно что-то видела."

# game/status.rpy:8673
translate russia task25PotionDrink_940dfead:

    # s g12 "Tsk, she's just getting spooked by her own shadow again. Let's check it out tomorrow and see what's going on."
    s g12 "Пф, да она опять испугалась собственной тени. Давайте завтра проверим и посмотрим, что происходит."

# game/status.rpy:8679
translate russia task25PotionDrink_ef3b09fd:

    # "The girls head back to their cells for the night."
    "Девушки возвращаются в свои камеры на ночь."

# game/status.rpy:8684
translate russia task25PotionDrink_13cde60a:

    # "The other spies are not available and Alex refuses to go back alone."
    "Других шпионок нет, и Алекс отказывается идти одна."

# game/status.rpy:8697
translate russia task25PotionDrink_9a7ed3dd:

    # s "Now you see Alex. There's nothing to be afraid of."
    s "Видишь Алекс, бояться нечего."

# game/status.rpy:8699
translate russia task25PotionDrink_9035f164:

    # a "A-are you guys not seeing them?!"
    a "В-вы что, не видите их?!"

# game/status.rpy:8700
translate russia task25PotionDrink_2480cc81:

    # c r16 "Seeing who?!"
    c r16 "Видим кого?!"

# game/status.rpy:8701
translate russia task25PotionDrink_0e6e01e5:

    # y "Wait a moment..."
    y "Подождите секунду..."

# game/status.rpy:8708
translate russia task25PotionDrink_ceeecdc4:

    # "You grab the potions that Alex brought back with her."
    "Вы берёте флаконы, которые Алекс принесла с собой."

# game/status.rpy:8709
translate russia task25PotionDrink_65ddc069:

    # y "I wonder..."
    y "Интересно..."

# game/status.rpy:8710
translate russia task25PotionDrink_38f0ca23:

    # "You take a sip of the green one."
    "Вы делаете глоток из зеленого."

# game/status.rpy:8718
translate russia task25PotionDrink_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:8719
translate russia task25PotionDrink_f5e55088:

    # y "Ghosts!"
    y "Призраки!"

# game/status.rpy:8720
translate russia task25PotionDrink_4cf24d8b:

    # s g28 "Not you too, [playerName]!"
    s g28 "И ты туда же, [playerName]!"

# game/status.rpy:8721
translate russia task25PotionDrink_9be34746:

    # a y28 "See? I told you!"
    a y28 "Видишь? Я же говорила!"

# game/status.rpy:8722
translate russia task25PotionDrink_c5ef55d2:

    # "Agent" "Aaaahhh! Ghosts!"
    "Агент" "АаааАаххХ! Призраки!"

# game/status.rpy:8723
translate russia task25PotionDrink_f18015ba:

    # y "Sounds you're not entirely alone. Get busy investigating!"
    y "Похоже, ты не совсем одна. Разузнайте что да как!"

# game/status.rpy:8724
translate russia task25PotionDrink_6f72c478:

    # "The girls nod and get ready."
    "Девушки кивают и собираются."

# game/status.rpy:8728
translate russia task25PotionDrink_27d83f2b:

    # a y15 "{b}*Whines*{/b}"
    a y15 "{b}*Хнычет*{/b}"

# game/status.rpy:8745
translate russia task25PotionDrink_f3253d34:

    # "Database" "Facial recognition complete. The Great Kand-..."
    "База данных" "Распознавание лица завершено. Великий Канд..."

# game/status.rpy:8748
translate russia task25PotionDrink_d88677fe:

    # "Database" "Facial recognition lost."
    "База данных" "Распознавание лица недоступно."

# game/status.rpy:8749
translate russia task25PotionDrink_9b1a1259:

    # y "???"
    y "???"

# game/status.rpy:8750
translate russia task25PotionDrink_14a27b70:

    # y "Where'd he go?"
    y "Куда он делся?"

# game/status.rpy:8754
translate russia task25PotionDrink_36546dc6:

    # y "What the...?!"
    y "Что за...?!"

# game/status.rpy:8756
translate russia task25PotionDrink_bf998a4a:

    # sM "It's... some kind of barrier."
    sM "Похоже на... какой-то барьер."

# game/status.rpy:8758
translate russia task25PotionDrink_1e05e8fc:

    # cM "It's... some kind of barrier."
    cM "Это что... какой-то барьер."

# game/status.rpy:8760
translate russia task25PotionDrink_6c0c7ffb:

    # aM "It's... some kind of barrier."
    aM "Вроде это... какой-то барьер."

# game/status.rpy:8761
translate russia task25PotionDrink_bd72c2fb:

    # y "What is this voodoo shit?! There's no such thing as magic!"
    y "Что это за вуду-дерьмо?! Магии не существует!"

# game/status.rpy:8763
translate russia task25PotionDrink_f4c4031f:

    # sM "Okay [playerName]. Calm down..."
    sM "Хорошо [playerName]. Успокойся..."

# game/status.rpy:8765
translate russia task25PotionDrink_22e4a60f:

    # cM "Okay [playerName]. Calm down..."
    cM "Ладно [playerName]. Остынь..."

# game/status.rpy:8767
translate russia task25PotionDrink_d939e5cd:

    # aM "Okay [playerName]. Calm down..."
    aM "Да расслабься ты, [playerName]..."

# game/status.rpy:8768
translate russia task25PotionDrink_5e9ca6f9:

    # y "It's all just smoke and mirrors!"
    y "Это всё просто дым и зеркала!"

# game/status.rpy:8769
translate russia task25PotionDrink_e55dd91d:

    # sM "Or... hallucinogens..."
    sM "Или... галлюциногены..."

# game/status.rpy:8770
translate russia task25PotionDrink_16338d1f:

    # aM "Hallu-whatcha?"
    aM "Галлюце-чё?"

# game/status.rpy:8771
translate russia task25PotionDrink_45b8bbd3:

    # sM "We might be seeing things that aren't there. Remember those potions you found? I bet they're the cure."
    sM "Возможно, мы видим то, чего нет. Помнишь те зелья, которые ты нашла? Уверена это из-за них."

# game/status.rpy:8772
translate russia task25PotionDrink_e3e752f3:

    # "Potions have been added to the Compowder menu."
    "В меню Камуникатора добавлены зелья."

# game/status.rpy:8782
translate russia task25PotionDrink_a2639172:

    # y "It worked!"
    y "Сработало!"

# game/status.rpy:8784
translate russia task25PotionDrink_8cfd5ea8:

    # y "Go after him and bring him in!"
    y "Идите за ним и приведите его!"

# game/status.rpy:8796
translate russia task25PotionDrink_e25ef560:

    # y "Darn it. More smoke and mirrors! Pick the right potion and continue on!"
    y "Чёрт. Ещё больше дыма и зеркал! Выберите нужное зелье и продолжайте путь!"

# game/status.rpy:8806
translate russia task25PotionDrink_07f219e8:

    # sM "[playerName]. I don't think-..."
    sM "[playerName]. Я не думаю-..."

# game/status.rpy:8808
translate russia task25PotionDrink_965cd34d:

    # cM "[playerName]. I don't think-..."
    cM "[playerName]. Я не думаю-..."

# game/status.rpy:8810
translate russia task25PotionDrink_323ce0a1:

    # aM "[playerName]. I don't think-..."
    aM "[playerName]. Я не думаю-..."

# game/status.rpy:8811
translate russia task25PotionDrink_0a1f1d4a:

    # y "SMOKE AND MIRRORS!"
    y "ДЫМ И ЗЕРКАЛА!"

# game/status.rpy:8822
translate russia task25PotionDrink_42f3d1c4:

    # y "Oh come on... another barrier? Switch it up a bit man."
    y "Да ладно... ещё один барьер? Можно немного разнообразия чувак."

# game/status.rpy:8823
translate russia task25PotionDrink_6cb41658:

    # "???" "As you wish."
    "???" "Как пожелаете."

# game/status.rpy:8825
translate russia task25PotionDrink_904813d8:

    # y "AHH!"
    y "АХХ!"

# game/status.rpy:8826
translate russia task25PotionDrink_bea19ea9:

    # y "Who said that?!"
    y "Кто это сказал?!"

# game/status.rpy:8848
translate russia task25PotionDrink_5b84b88c:

    # y "AHHH! I'm being haunted by the Halloween decorations!{w} the irony is not lost on me!"
    y "АААА! Меня преследуют хэллоуинская декорация!{w} ИРОНИИ ОТ МЕНЯ НЕ СПРЯТАТЬСЯ!"

# game/status.rpy:8861
translate russia task25PotionDrink_16a150dd:

    # sM "Sounds like [playerName] has his hands full aswell. Let's finish this quick!"
    sM "Похоже, у [playerName] тоже есть чем заняться. Давайте закончим побыстрее!"

# game/status.rpy:8863
translate russia task25PotionDrink_0da7a3f7:

    # cM "Sounds like [playerName] has his hands full aswell. Let's finish this quick!"
    cM "Похоже, [playerName] тоже весело. Давайте закончим побыстрее!"

# game/status.rpy:8865
translate russia task25PotionDrink_aae9d0ac:

    # aM "Sounds like [playerName] has his hands full aswell. Let's finish this quick!"
    aM "Похоже, [playerName] там не скучает. Давайте закончим побыстрее!"

# game/status.rpy:8870
translate russia task25PotionDrink_d437e987:

    # kand "Agreed. Let's get this over with..."
    kand "Согласен. Давайте покончим с этим..."

# game/status.rpy:8899
translate russia task25PotionDrink_41f76646:

    # y "Begon demon!"
    y "Сгинь демон!"

# game/status.rpy:8911
translate russia task25PotionDrink_0c09e814:

    # s "We're back."
    s "Мы вернулись."

# game/status.rpy:8912
translate russia task25PotionDrink_8f2b9ca4:

    # c r29 "What happened to the Halloween supplies?"
    c r29 "Что случилось с хэллоуинскими декорациями?"

# game/status.rpy:8913
translate russia task25PotionDrink_a7c2fa4f:

    # a y28 "Did Kandinsky put a spell on them?!"
    a y28 "Неужели Кандинский наложил на них заклятие?!"

# game/status.rpy:8914
translate russia task25PotionDrink_f3a69bb8:

    # y "There's no such thing as magic...! He must've... used slight-of-hand somehow...!"
    y "Волшебства не существует...! Он, должно быть... использовал какой-то трюк...!"

# game/status.rpy:8915
translate russia task25PotionDrink_11862059:

    # y "Smoke and mirrors!"
    y "Дым и зеркала!"

# game/status.rpy:8916
translate russia task25PotionDrink_f2227430:

    # c r12 "Well anyway... we put Kandinsky in a cell. Don't wait too long before interrogating him."
    c r12 "Ну, в любом случае... мы посадили Кандинского в камеру. Не затягивай с допросом."

# game/status.rpy:8921
translate russia task25PotionDrink_56609764:

    # "The girls return to their cell."
    "Девушки расходятся по камерам."

# game/status.rpy:8936
translate russia task25PotionDrink_2513f638:

    # y "Okay magic man, the gig is up!"
    y "Ладно, волшебник херов, выступление окончено!"

# game/status.rpy:8937
translate russia task25PotionDrink_408c5027:

    # y "No more spells! No more magic! And no more tricks!"
    y "Больше никаких заклинаний! Никакой магии! И никаких фокусов!"

# game/status.rpy:8938
translate russia task25PotionDrink_5a960b9a:

    # kand "Oh tricks? My spells are tricks to you?"
    kand "Фокусы? Мои заклинания для тебя - фокусы?"

# game/status.rpy:8940
translate russia task25PotionDrink_e83e84a8:

    # a "Kandinsky! The gig is up!"
    a "Кандинский! Выступление окончено!"

# game/status.rpy:8941
translate russia task25PotionDrink_0b7277f1:

    # y "Alex... I already said that..."
    y "Алекс... Я это уже сказал..."

# game/status.rpy:8942
translate russia task25PotionDrink_35bcd4c3:

    # kand "Oh to think the mighty Outsiders fell to a group of jokers like you..."
    kand "Подумать только, могущественные Аутсайдеры пали перед группой таких шутов, как вы..."

# game/status.rpy:8943
translate russia task25PotionDrink_ad54cdef:

    # kand "I'm not that surprised really. They weren't that threatening of a gang to begin with."
    kand "Если честно, я даже не удивлён. На самом деле, они были не такой уж и сильной бандой"

# game/status.rpy:8944
translate russia task25PotionDrink_c08538d7:

    # y "Then why did the Mastermind bring them to Beverly Hills?"
    y "Тогда зачем Организатор привёз их в Беверли-Хиллз?"

# game/status.rpy:8945
translate russia task25PotionDrink_2b924e7f:

    # kand "Oh she didn't. They were always here."
    kand "Ох, она их не привозила. Они всегда были здесь."

# game/status.rpy:8946
translate russia task25PotionDrink_c8cd7b82:

    # kand "She just boosted the ranks a bit and gave them funding, but it looks like the gang will fall apart soon without its leaders."
    kand "Она лишь немного повысила их ряды и выделила им финансирование. Но, похоже, без своих лидеров банда скоро развалится."

# game/status.rpy:8950
translate russia task25PotionDrink_e8b5b2aa:

    # s "The gig is up Kandinsky!"
    s "Выступление окончено Кандинский!"

# game/status.rpy:8951
translate russia task25PotionDrink_7936ac31:

    # y "Sam... we already said that..."
    y "Сэм... мы это уже сказали..."

# game/status.rpy:8952
translate russia task25PotionDrink_cb19099b:

    # kand "Surely you didn't capture me just to put me in jail. I bet you want information."
    kand "Конечно, вы схватили меня не только для того, чтобы посадить в тюрьму. Наверняка вам нужна информация."

# game/status.rpy:8953
translate russia task25PotionDrink_d74ff9bd:

    # y "Yes! Tell us everything you know or we might get rough!"
    y "Да! Расскажи нам всё, что знаешь, иначе мы будем разговаривать на языке боли!"

# game/status.rpy:8954
translate russia task25PotionDrink_ee8396c8:

    # kand "Okay, here you go."
    kand "Ладно, вот."

# game/status.rpy:8958
translate russia task25PotionDrink_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:8959
translate russia task25PotionDrink_a6f1ac3e:

    # y "That was easy..."
    y "Это было легко..."

# game/status.rpy:8960
translate russia task25PotionDrink_3a2d5891:

    # kand "Oh please... I just ran that gang as a favor for the Mastermind for getting me out."
    kand "О, пожалуйста... Я просто управлял этой бандой в качестве одолжения Организатору за то, что он вытащил меня."

# game/status.rpy:8961
translate russia task25PotionDrink_7beac7a1:

    # kand "Honest, you did me a favor for getting rid of them."
    kand "Честно, ты оказал мне услугу, избавившись от них."

# game/status.rpy:8962
translate russia task25PotionDrink_a9ad0f9a:

    # y "Yeah right... you forget that we captured you and put you back behind bars, Kandinsky."
    y "Да, только... ты забыл, что мы схватили тебя и посадили обратно за решётку, Кандинский."

# game/status.rpy:8963
translate russia task25PotionDrink_32995ac1:

    # kand "Oh did you?"
    kand "Правда?"

# game/status.rpy:8971
translate russia task25PotionDrink_ae4bfad7_1:

    # y "!!!"
    y "!!!"

# game/status.rpy:8972
translate russia task25PotionDrink_113b6f05:

    # y "What the?!"
    y "ЧТо за?!"

# game/status.rpy:8973
translate russia task25PotionDrink_9d0e6bb2:

    # y "GET HIM!"
    y "ДЕРЖИТЕ ЕГО!"

# game/status.rpy:8974
translate russia task25PotionDrink_9ccb5440:

    # kand "Not so fast..."
    kand "Не так быстро..."

# game/status.rpy:8990
translate russia task25PotionDrink_3ddf13f6:

    # a y28 "Eeeeeh!"
    a y28 "ИиииИИ!"

# game/status.rpy:8992
translate russia task25PotionDrink_12d42c6d:

    # s g29 "Eeeeeeeh!"
    s g29 "ИИииИИи!"

# game/status.rpy:8993
translate russia task25PotionDrink_36546dc6_1:

    # y "What the...?!"
    y "Что за...?! "

# game/status.rpy:8994
translate russia task25PotionDrink_492f17b1:

    # y "Gotta admit... you're really good with those smoke and mirrors..."
    y "Должен признать... Этот дым с зеркалами... Ты действительно хорош в этом..."

# game/status.rpy:8995
translate russia task25PotionDrink_2322244d:

    # s "[playerName]! Get him!"
    s "[playerName]! Держи его!"

# game/status.rpy:8996
translate russia task25PotionDrink_4a942a54:

    # kand "I will be taking my leave now... Goodbye everyone."
    kand "Боюсь что мне пора... Всем до свидания."

# game/status.rpy:8997
translate russia task25PotionDrink_31ef6d8d:

    # kand "Until we meet again~...."
    kand "До новых встреч~...."

# game/status.rpy:9000
translate russia task25PotionDrink_43c26b39:

    # y "He... disappeared..."
    y "Он... исчез..."

# game/status.rpy:9026
translate russia task25PotionDrink_8acc50a2:

    # y "Well... with their leaders gone. I don't think the Outsiders will give us anymore trouble."
    y "Нуу... без лидеров, я не думаю, что Аутсайдеры доставят ещё каких либо проблем."

# game/status.rpy:9027
translate russia task25PotionDrink_910822c4:

    # s g38 "Can we go home now, please?!"
    s g38 "Можно мы уже пойдём домой, пожалуйста?!"

# game/status.rpy:9028
translate russia task25PotionDrink_d4efeb06:

    # a y29 "Yeah, my nipples are hard enough to cut glass!"
    a y29 "Да, моими сосками уже можно разрезать стекло!"

# game/status.rpy:9029
translate russia task25PotionDrink_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:9030
translate russia task25PotionDrink_e53c9e5f:

    # y "All right, let's go."
    y "Хорошо, пойдёмте."

# game/status.rpy:9032
translate russia task25PotionDrink_386c0c21:

    # "You quickly make your way back to the base."
    "Вы быстро возвращаетесь на базу."

# game/status.rpy:9041
translate russia task25PotionDrink_396fa4d3:

    # c "What happened to you guys?!"
    c "Что с вами случилось, ребята?!"

# game/status.rpy:9042
translate russia task25PotionDrink_145e7ca6:

    # s "Don't ask..."
    s "Не спрашивай..."

# game/status.rpy:9047
translate russia task25PotionDrink_f3e523da:

    # y "It's a long story."
    y "Это долгая история."

# game/status.rpy:9048
translate russia task25PotionDrink_b7c0f171:

    # c r16 "Does it so happen to involve Kandinsky? Cause... he escaped..."
    c r16 "Это случайно не связано с Кандинским? Потому что... он сбежал..."

# game/status.rpy:9049
translate russia task25PotionDrink_b7f9a377:

    # y "Yup..."
    y "Ага..."

# game/status.rpy:9050
translate russia task25PotionDrink_939c00fb:

    # y "He pulled his little slight-of-hand trick and took off."
    y "Он использовал свой маленький трюк и удрал."

# game/status.rpy:9051
translate russia task25PotionDrink_2f7de0c6:

    # c r10 "Are you still saying it's just smoke and mirr-..."
    c r10 "То есть ты всё ещё хочешь сказать что это был просто дым и зерка-..."

# game/status.rpy:9052
translate russia task25PotionDrink_30495171:

    # y "Yes!"
    y "Да!"

# game/status.rpy:9053
translate russia task25PotionDrink_2d13b75c:

    # c "................"
    c "................"

# game/status.rpy:9054
translate russia task25PotionDrink_9e10309c:

    # c r12 "So what about the Outsiders?"
    c r12 "Так что насчёт Аутсайдеров?"

# game/status.rpy:9055
translate russia task25PotionDrink_2776cd37:

    # y "With Kandinsky gone, they will fall apart soon enough. Looks like they'll disband."
    y "После ухода Кандинского они очень скоро развалятся."

# game/status.rpy:9056
translate russia task25PotionDrink_182a381f:

    # c r1 "Well that's one less gang to worry about..."
    c r1 "Ну вот, одной бандой меньше..."

# game/status.rpy:9076
translate russia task26_a92b7375:

    # "???" "{b}*Static*{/b} ...you read me? Thi-....{b}*Static*{/b}"
    "???" "{b}*Помехи*{/b} ...вы спышите мня? Афло-....{b}*Помехи*{/b}"

# game/status.rpy:9077
translate russia task26_8c698886:

    # "???" "{b}*Static*{/b} ...-aptured. Held in the WOOHP HQ. I need h-...{b}*Static*{/b}"
    "???" "{b}*Помехи*{/b} ...-ойман. Держат в штаб-квартире WOOHP. Мне нужно п-...{b}*Помехи*{/b}"

# game/status.rpy:9078
translate russia task26_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:9087
translate russia task26_57f001f6:

    # y "Did you girls hear that?"
    y "Девочки, вы слышали это?"

# game/status.rpy:9088
translate russia task26_0e3a1b2f:

    # c "Yes. It sounded like..."
    c "Да. Это похоже на..."

# game/status.rpy:9089
translate russia task26_2356f30d:

    # s "Like..."
    s "Похоже на..."

# game/status.rpy:9090
translate russia task26_03bfe262:

    # a "Like Britney!"
    a "Похоже на Бритни!"

# game/status.rpy:9091
translate russia task26_6adbfad5:

    # s g29 "Yes! It did sound like her!"
    s g29 "Да! Это было похоже на нее!"

# game/status.rpy:9092
translate russia task26_2f925aaa:

    # y "Who?"
    y "На кого?"

# game/status.rpy:9093
translate russia task26_ab0d6b39:

    # c r14 "She's a fellow spy and a friend of ours."
    c r14 "Она шпионка и наша подруга."

# game/status.rpy:9094
translate russia task26_c4701bf6:

    # a y5 "It sounded like she was in trouble."
    a y5 "Похоже, она в беде."

# game/status.rpy:9095
translate russia task26_0bb3a8cb:

    # y "..............................."
    y "..............................."

# game/status.rpy:9096
translate russia task26_ad2ebaad:

    # y "I mean...."
    y "Ну, эмм..."

# game/status.rpy:9097
translate russia task26_75375b60:

    # y "You know this is a trap, right?"
    y "Вы ведь знаете, что это ловушка, верно?"

# game/status.rpy:9098
translate russia task26_98dec8cb:

    # c r10 "Oh yeah."
    c r10 "О да."

# game/status.rpy:9099
translate russia task26_80915687:

    # s g14 "Definitely."
    s g14 "Определённо."

# game/status.rpy:9100
translate russia task26_e4a0bbd2:

    # a y2 "No question."
    a y2 "Конечно."

# game/status.rpy:9101
translate russia task26_d3a59cb5:

    # y "............................."
    y "............................."

# game/status.rpy:9102
translate russia task26_5a5032e7:

    # y "How hot is she?"
    y "Насколько она сексуальна?"

# game/status.rpy:9103
translate russia task26_98667c4c:

    # c r22 "Flamin'."
    c r22 "Просто огонь."

# game/status.rpy:9104
translate russia task26_af417f33:

    # y "Let's go rescue her!"
    y "Пойдёмте её спасать!"

# game/status.rpy:9105
translate russia task26_731b00ba:

    # s g15 "{b}*Sighs*{/b}"
    s g15 "{b}*Вздох*{/b}"

# game/status.rpy:9135
translate russia task26_507b08e1:

    # y "Quickly! Set up in the basement. They don't know we're here yet."
    y "Быстрее! Они ещё не знают, что мы здесь."

# game/status.rpy:9137
translate russia task26_551d653b:

    # "Agent" "Doing as I'm told!"
    "Агент" "Делаю то, что мне говорят!"

# game/status.rpy:9157
translate russia task26_00b38f88:

    # y "Okay, it's time we begin the take over of WOOHP."
    y "Итак, пришло время начать захватывать WOOHP."

# game/status.rpy:9158
translate russia task26_17eb381d:

    # y "The WOOHP HQ consists of multiple floors that we need to take back."
    y "Штаб-квартира WOOHP состоит из нескольких этажей, которые мы должны захватить."

# game/status.rpy:9159
translate russia task26_f62b4609:

    # a y16 "How are we going to do that...?"
    a y16 "Как мы собираемся это сделать...?"

# game/status.rpy:9160
translate russia task26_d9a5419b:

    # y "We can spend intel to figure out what's on the floor above us. Then we can send freed WOOHP agents to lead the assault."
    y "Мы можем использовать разведданные, чтобы выяснить, что находится на этаже над нами. Тогда мы сможем послать освобожденных агентов WOOHP на штурм."

# game/status.rpy:9161
translate russia task26_61b429ec:

    # s g20 "Getting intel is probably going to prove difficult with the other gangs out of commission."
    s g20 "Получение какие-либо разведданные будет достаточно проблематично, поскольку других банд больше нет."

# game/status.rpy:9162
translate russia task26_47e4928d:

    # y "That's where you three come in. We have a new mission area. The WOOHP HQ."
    y "Вот тут-то вы трое и выходите на сцену. У нас появилась новая сфера деятельности. Штаб-квартира WOOHP."

# game/status.rpy:9163
translate russia task26_0ab493ad:

    # y "I will plan missions for you girls. {w}Sneak through the headquarters and bring back as much intel as you can."
    y "Я буду планировать для вас миссии. {w}А вы проникните в штаб и принесёте как можно больше информации."

# game/status.rpy:9164
translate russia task26_779c2e0a:

    # c r18 "What about Britney?"
    c r18 "А как насчёт Бритни?"

# game/status.rpy:9165
translate russia task26_887723ff:

    # y "She's probably being held on one of these floors. If we have enough intel we'll launch an assault and take back the building floor by floor."
    y "Возможно, её держат на одном из этих этажей. Если у нас будет достаточно информации, мы начнём штурм и отвоюем этаж за этажом."

# game/status.rpy:9166
translate russia task26_6f72c478:

    # "The girls nod and get ready."
    "Девушки кивают и собираются."

# game/status.rpy:9183
translate russia task26_2c7864fa:

    # y "It's.. a prison."
    y "Это... тюрьма."

# game/status.rpy:9184
translate russia task26_e5574fd1:

    # s g16 "Yeah, the WOOHP asylum under the milkshake bar is tiny compared to this one..."
    s g16 "Да, психушка WOOHP под баром крошечное по сравнению с этим..."

# game/status.rpy:9185
translate russia task26_524530dc:

    # a y28 "Look!"
    a y28 "Смотрите!"

# game/status.rpy:9192
translate russia task26_8650de47:

    # "Girls" "Britney!"
    "Девушки" "Бритни!"

# game/status.rpy:9193
translate russia task26_8de5ea67:

    # brit b32 "Ugh....."
    brit b32 "Угх..."

# game/status.rpy:9194
translate russia task26_30cdf20d:

    # c "Britney...?"
    c "Бритни...?"

# game/status.rpy:9195
translate russia task26_a47c1d7c:

    # brit b7 "...!"
    brit b7 "...!"

# game/status.rpy:9200
translate russia task26_11473c86:

    # "Britney attacked! {w}... and was then prompty overpowered by your spies."
    "Бритни напала! {w}... но была быстро обездвижена вашими шпионками."

# game/status.rpy:9205
translate russia task26_daa109f4:

    # brit "W-what..?!"
    brit "Ч-что...?!"

# game/status.rpy:9206
translate russia task26_efb945bb:

    # y "Oh come on, we knew it was a trap so we prepared."
    y "Да ладно, мы знали, что это ловушка, поэтому подготовились."

# game/status.rpy:9207
translate russia task26_ca7ceabe:

    # a y28 "Don't worry Britney! We'll have you back to normal in no time!"
    a y28 "Не волнуйся, Бритни! Мы вернём тебя к норме в кратчайшие сроки!"

# game/status.rpy:9208
translate russia task26_97ef254e:

    # c r53 "Don't resist, you'll just have to come with us."
    c r53 "Не сопротивляйся, тебе просто нужно пойти с нами."

# game/status.rpy:9209
translate russia task26_82faec70:

    # brit b9 "........................"
    brit b9 "........................"

# game/status.rpy:9211
translate russia task26_89e1ea63:

    # brit b22 "{b}*Smirks*{/b}"
    brit b22 "{b}*Ухмыляется*{/b}"

# game/status.rpy:9212
translate russia task26_5912d62e:

    # c r56 "Britney...?"
    c r56 "Бритни...?"

# game/status.rpy:9218
translate russia task26_ece6fb9a:

    # "Britney threw the spies off of her like they weigh nothing!"
    "Бритни сбросила с себя шпионок, как будто они ничего не весят!"

# game/status.rpy:9219
translate russia task26_bce4a13e:

    # y "Uh oh...! Agents...!"
    y "Ой-ой...! Агенты...!"

# game/status.rpy:9228
translate russia task26_de2afb13:

    # "Agent" "Hop! Hop! Hop! Hop!"
    "Агент" "Хоп! Хоп! Хоп! Хоп!"

# game/status.rpy:9244
translate russia task26_48e37d59:

    # y "Hey!"
    y "Эй!"

# game/status.rpy:9245
translate russia task26_616c0d27:

    # "Britney pulls out a dart gun and fires!"
    "Бритни достает дротиковый пистолет и стреляет!"

# game/status.rpy:9247
translate russia task26_fdda3a8c:

    # "Girls" "Ow!"
    "Девушки" "Оу!"

# game/status.rpy:9248
translate russia task26_f3f59152:

    # "Everyone of your girls got hit by a nano injector!"
    "В каждую из ваших шпионок попало по наноинжектору!"

# game/status.rpy:9249
translate russia task26_344a6415:

    # y "That can't be good..."
    y "Это плохо..."

# game/status.rpy:9250
translate russia task26_edbdea8b:

    # sM "[playerName]...! Pull back!"
    sM "[playerName]...! Отступаем!"

# game/status.rpy:9252
translate russia task26_754ec601:

    # y "This isn't over yet!"
    y "Это ещё не конец!"

# game/status.rpy:9253
translate russia task26_244c9dc4:

    # brit "I'll be waiting..."
    brit "Я буду ждать..."

# game/status.rpy:9259
translate russia task26_1f2e6488:

    # y "In you go!"
    y "Заходите!"

# game/status.rpy:9264
translate russia task26_0bdf973a:

    # "You shove the girls into their cells and lock the door."
    "Вы запихиваете девушек в камеры и запираете дверь."

# game/status.rpy:9265
translate russia task26_9adbf266:

    # "Inside you see their eyes turning red as they glare at you through the glass."
    "Внутри вы видите, как их глаза становятся красными, как только они смотрят на вас через стекло."

# game/status.rpy:9266
translate russia task26_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:9267
translate russia task26_3593dad5:

    # y "That might be a problem..."
    y "Это проблема..."

# game/status.rpy:9268
translate russia task26_6e5ea165:

    # y "You girls hang in there! I'll break you out of your trance!"
    y "Девочки, держитесь! Я выведу вас из транса!"

# game/status.rpy:9289
translate russia task26_e5bbb382:

    # y "Well hello there Sam. You're looking nice and...{w} murdery."
    y "Привет, Сэм. Выглядишь отлично и...{w} убийственно."

# game/status.rpy:9290
translate russia task26_c15a914a:

    # s "{b}*Growl*{/b}"
    s "{b}*Рычит*{/b}"

# game/status.rpy:9291
translate russia task26_857f1b38:

    # y "So I'm just gonna assume that to break you out of it, we're just going to have sex again."
    y "Я просто предположу, что для того, чтобы избавить тебя от этого, нам просто нужно снова заняться сексом."

# game/status.rpy:9292
translate russia task26_081997cf:

    # y "However... my father taught me never to stick my dick in crazy.{w} At least not without proper protection!"
    y "Однако... мой отец учил меня никогда не совать свой член безрассудно.{w} По крайней мере, не без надлежащей защиты!"

# game/status.rpy:9293
translate russia task26_ce398e91:

    # s ".....?"
    s ".....?"

# game/status.rpy:9294
translate russia task26_bcdac2ff:

    # "You take out a rope."
    "Вы достаёте верёвку."

# game/status.rpy:9295
translate russia task26_7af54c02:

    # y "Okay now I'm going to open the cell...~"
    y "Хорошо, теперь я открою камеру~..."

# game/status.rpy:9297
translate russia task26_cfc4a73a:

    # y "Okay good... easy no-...."
    y "Хорошо, отлично... просто не-..."

# game/status.rpy:9302
translate russia task26_80974c4f:

    # y "Ow! You brat!"
    y "Оу! Ты сука!"

# game/status.rpy:9303
translate russia task26_fce6aacc:

    # "Sam bit you!"
    "Сэм укусила вас!"

# game/status.rpy:9309
translate russia task26_2ec39617:

    # "After a short struggle you manage to get the upper hand on Sam and tie her up!"
    "После недолгой борьбы вам удаётся взять верх над Сэм и связать её!"

# game/status.rpy:9310
translate russia task26_ca5d29d6:

    # y "Okay Sam listen! I know you're not yourself right now...{w} but I'm going to fuck you in the butt."
    y "Ладно, Сэм слушай! Я знаю, что ты сейчас не в себе...{w} но я собираюсь трахнуть тебя в задницу."

# game/status.rpy:9311
translate russia task26_0ab16323:

    # s "!!!"
    s "!!!"

# game/status.rpy:9312
translate russia task26_bd6754cb:

    # y "You can get mad at me later."
    y "Можешь злиться на меня после."

# game/status.rpy:9325
translate russia task26_5b78db06:

    # y "Saaaam...?"
    y "Сэм...?"

# game/status.rpy:9326
translate russia task26_c652cad2:

    # s "..................................."
    s "..................................."

# game/status.rpy:9327
translate russia task26_34f5d081:

    # s "Did we just....?"
    s "Мы сейчас...?"

# game/status.rpy:9328
translate russia task26_34fa9179:

    # y "Yup."
    y "Ага."

# game/status.rpy:9329
translate russia task26_2103b4ef:

    # s "In the..."
    s "Прямо в..."

# game/status.rpy:9330
translate russia task26_a49766a4:

    # y "In the butt. Yes."
    y "В задницу. Да."

# game/status.rpy:9331
translate russia task26_894b6cf1:

    # s g11 "..............."
    s g11 "..............."

# game/status.rpy:9332
translate russia task26_6692f04c:

    # s "That was...."
    s "Это было...."

# game/status.rpy:9333
translate russia task26_9a78f174:

    # y "................"
    y "................"

# game/status.rpy:9334
translate russia task26_a6c36986:

    # s g13 "That was the hottest thing I've ever done."
    s g13 "Это было самое ахуенное, что я когда-либо делала."

# game/status.rpy:9335
translate russia task26_65677f05:

    # y "Wait... I'm not in trouble?"
    y "Стоп... Так у меня не будет проблем?"

# game/status.rpy:9336
translate russia task26_880a491a:

    # s g1 "Of course not! I was in trouble and you figured out the best way to get me back to my sense."
    s g1 "Конечно, нет! Я была в беде, а ты придумал лучший способ вернуть мне в чувство."

# game/status.rpy:9337
translate russia task26_c4d86db7:

    # s "We should quickly move on to the other girl-...{w} Ow ow ow~...."
    s "Мы должны быстре помочь остальным-...{w} Оу оу ау ой~..."

# game/status.rpy:9338
translate russia task26_deed1380:

    # y "I'll handle them. You should maybe grab a shower... and not sit down for a while."
    y "Я с ними разберусь. Может, тебе стоит принять душ... а, и не садись некоторое время."

# game/status.rpy:9339
translate russia task26_b1ed1f71:

    # s g5 "......................."
    s g5 "......................."

# game/status.rpy:9340
translate russia task26_d2910a20:

    # "Sam rubs her butt with a pained expression."
    "Сэм потирает задницу с болезненным выражением лица."

# game/status.rpy:9345
translate russia task26_b1a0aaa7:

    # "Cell Next Door" "LET ME OUT! I WANT TO PLAN A COUP IN SWAZILAND!"
    "Камера по соседству" "ВЫПУСТИТЕ МЕНЯ! МНЕ НУЖНО СПЛАНИРОВАТЬ ПЕРЕВОРОТ В СВА́ЗИЛЕ́НДЕ!"

# game/status.rpy:9346
translate russia task26_c6bb6ab1:

    # y "Calm your horses! The country isn't even called that anymore!"
    y "Успокойся! Тем более страна уже больше так не называется!"

# game/status.rpy:9348
translate russia task26_8c1c2c53:

    # "You quickly move on to the other girls and after a long night of buttfuckin' everyone is back to their senses."
    "Вы быстро переходите к оставшимся девушкам, и после долгой ночи анального секса все приходят в себя."

# game/status.rpy:9349
translate russia task26_8f0b5637:

    # y "Phew~... still got it in me."
    y "Фух~... я всё ещё на высоте."

# game/status.rpy:9350
translate russia task26_d9a807c2:

    # "Tired, but thoroughly satisfied, you head to bed."
    "Уставший, но довольный, вы отправляетесь в постель."

# game/status.rpy:9366
translate russia task26_2d04b099:

    # y "Hah! Level two! Now to get that flaming hot piece of ass~...."
    y "Ха! Второй этаж! Теперь, давайте схватим эту горячею жопку~...."

# game/status.rpy:9368
translate russia task26_36aba7ec:

    # O5O "RIGHT HERE!"
    O5O "ЗДЕСЬ!"

# game/status.rpy:9369
translate russia task26_9a78f174_1:

    # y "................"
    y "................"

# game/status.rpy:9370
translate russia task26_fab7911f:

    # y "You're not the hot piece of ass I'm looking for.{w} Who are you?"
    y "Ты не та сексапильная задница которую я ищу.{w} Кто ты?"

# game/status.rpy:9371
translate russia task26_52101979:

    # O5O "AGENT O5O! YOU REMEMBER ME!"
    O5O "АГЕНТ О5О! ТЫ ПОМНИШЬ МЕНЯ!"

# game/status.rpy:9372
translate russia task26_34d328b1:

    # y "Er...."
    y "Эм..."

# game/status.rpy:9373
translate russia task26_0714d4a1:

    # O5O "I'M A MAIN ANTAGONIST!"
    O5O "Я - ГЛАВНЫЙ АНТАГОНИСТ!"

# game/status.rpy:9374
translate russia task26_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:9377
translate russia task26_d19a2b08:

    # y "You were at the base before this whole mess began!"
    y "Ты был на базе до того, как вся эта неразбериха началась!"

# game/status.rpy:9378
translate russia task26_9e00c4c7:

    # y "Man, how could I ever forget you Agent O5O, you've played such an important role so far."
    y "Боже, как я мог забыть тебя, агент O5O, ты ведь сыграл такую важную роль."

# game/status.rpy:9379
translate russia task26_ec9cf75b:

    # y "Remember that one time we went to liberate Alex from the school and you were there?"
    y "Помнишь тот раз, когда мы пошли освобождать Алекс из школы, и ты там был?"

# game/status.rpy:9380
translate russia task26_d1d82eef:

    # y "Ah yes, good old Agent O5O. Everyone's favorite agent and {i}'definitely'{/i} not completely forgotten by Exiscoming."
    y "Ах да, старый добрый агент О5О. Всеми любимый агент и {i}'определённо'{/i} не забытый Exiscoming´ом."

# game/status.rpy:9382
translate russia task26_d1d15f2e:

    # O5O "HOW COULD YOU FORGET ME?! I'M THE MOST LOVABLE SIDE CHARACTER IN THE STORY!"
    O5O "КАК ТЫ МОГ ЗАБЫТЬ МЕНЯ?! Я САМЫЙ СИМПАТИЧНЫЙ ВТОРОСТЕПЕННЫЙ ПЕРСОНАЖ В ЭТОЙ ИСТОРИИ!"

# game/status.rpy:9383
translate russia task26_8b67f3d5:

    # O5O "REMEMBER WHEN YOU FIRST BROKE OUT? I WAS THERE!"
    O5O "ПОМНИШЬ, КОГДА ТЫ ВПЕРВЫЕ СБЕЖАЛ? Я ТАМ БЫЛ!"

# game/status.rpy:9384
translate russia task26_a970c554:

    # O5O "AND WHEN YOU WENT TO RESCUE YOUR SPY IN YELLOW. I WAS GIVING A SPEECH AT THE SCHOOL THAT DAY!"
    O5O "И КОГДА ТЫ СПАСАЛ СВОЮ ШПИОНКУ В ЖЁЛТОМ. В ТОТ ДЕНЬ Я ВЫСТУПАЛ С РЕЧЬЮ В ШКОЛЕ!"

# game/status.rpy:9385
translate russia task26_3058904c:

    # y "................................."
    y "................................."

# game/status.rpy:9386
translate russia task26_7d892295:

    # O5O "YOU DON'T REMEMBER ME, DO YOU...?"
    O5O "ТЫ НЕ ПОМНИШЬ МЕНЯ, НЕ ТАК ЛИ...?"

# game/status.rpy:9387
translate russia task26_e6e2b4be:

    # y "I don't even think Exiscoming remembered you."
    y "Я даже не думаю, что Exiscoming тебя помнит."

# game/status.rpy:9388
translate russia task26_c601cf73:

    # s g16 "You know this agent, [playerName]?"
    s g16 "Ты знаешь этого агента, [playerName]?"

# game/status.rpy:9389
translate russia task26_8225b385:

    # O5O "OH YES, ME AND HIM GO WAY BACK!{w} I WAS THERE WHEN HE WAS STILL A WOOHP INMATE!"
    O5O "О ДА, МЫ С НИМ ДАВНО ЗНАКОМЫ!{w} С ТЕХ ПОР КОГДА ОН ЕЩЁ БЫЛ ЗАКЛЮЧЕННЫМ В ТЮРЬМЕ WOOHP!"

# game/status.rpy:9390
translate russia task26_be985bb9:

    # c r56 "A what?"
    c r56 "Кем?"

# game/status.rpy:9391
translate russia task26_2ee62221:

    # y "Oh no no no, we don't have to go into th-..."
    y "О нет, нет, нет, нам не стоит вдаваться в-..."

# game/status.rpy:9392
translate russia task26_a89f9034:

    # O5O "LOOKING ALL COOL AND EDGY IN HIS PRISONER UNIFORM. I REMEMBER IT LIKE IT WAS YESTERDAY!"
    O5O "ОН ВЫГЛЯДЕЛ ТАКИМ КРУТЫМ И НЕРВНЫМ В СВОЕЙ ТЮРЕМНОЙ УНИФОРМЕ. Я ПОМНЮ ЭТО, СЛОВНО ЭТО БЫЛО ВЧЕРА!"

# game/status.rpy:9393
translate russia task26_dd8bf83c:

    # a y29 "Prisoner uniform...?"
    a y29 "В тюремной униформе...?"

# game/status.rpy:9394
translate russia task26_9c4ac789:

    # y "Dude, shutupshutupshutup!"
    y "Чувак, заткнисьзаткнисьзаткнись!"

# game/status.rpy:9395
translate russia task26_96fc2ab7:

    # O5O "MAN I REMEMBER THE DAY THEY LOCKED YOU UP! YOU WERE SUCH A MENACE TO SOCIETY!"
    O5O "Я ПОМНЮ ТОТ ДЕНЬ, КОГДА ТЕБЯ ЗАПЕРЛИ! ТЫ БЫЛ ТАКОЙ УГРОЗОЙ ДЛЯ ОБЩЕСТВА!"

# game/status.rpy:9396
translate russia task26_e677ede6:

    # O5O "JUST SEEING YOU WALK AROUND FREE MAKES ME NERVOUS!"
    O5O "ОДИН ТОЛЬКО ВИД ТОГО, КАК ТЫ СВОБОДНО РАЗГУЛИВАЕШЬ, ЗАСТАВЛЯЕТ МЕНЯ НЕРВНИЧАТЬ!"

# game/status.rpy:9397
translate russia task26_34e4fd01:

    # s g31 "What is he talking about, [playerName]?"
    s g31 "О чём он говорит, [playerName]?"

# game/status.rpy:9398
translate russia task26_cc618ec8:

    # y "Oh well you see, I....{w} HE'S GOT A GUN, GET HIM!"
    y "Ну, видите ли, я....{w} У НЕГО ПИСТОЛЕТ, ДЕРЖИТЕ ЕГО!"

# game/status.rpy:9399
translate russia task26_fa34a70e:

    # O5O "W-WHA-....?"
    O5O "Ч-ЧТ-....?"

# game/status.rpy:9418
translate russia task26_c498cf8d:

    # a "Target restrained!"
    a "Цель захвачена!"

# game/status.rpy:9419
translate russia task26_e4eff804:

    # y "Good work girls. That opens up the second level of the WOOHP HQ. Only a few more levels and we'll take this place back for good!"
    y "Отличная работа, девочки. Второй этаж штаб-квартиры WOOHP наш. Ещё несколько уровней, и мы отвоюем это место навсегда!"

# game/status.rpy:9420
translate russia task26_c80c7d0c:

    # c r18 "So [playerName]... what was this thing about you being a pri-..."
    c r18 "Итак, [playerName]... Что это за история с тем, что ты был зак-..."

# game/status.rpy:9421
translate russia task26_221d03ed:

    # y "Yes indeed! With some elbow grease and a 'Can do!' attitude we'll make it through here!"
    y "Да, конечно! С небольшим количеством смазки для локтей и позитивным настроем мы пройдём через это!"

# game/status.rpy:9422
translate russia task26_926466cb:

    # s g10 "[playerName]...."
    s g10 "[playerName]...."

# game/status.rpy:9423
translate russia task26_4c691dab:

    # y "High fives all around! We'll take back WOOHP and then be on our merry way!"
    y "Дайте пять! Мы вернём WOOHP, а затем отправимся по своим весёлым путям!"

# game/status.rpy:9424
translate russia task26_bbf45fe4:

    # a y5 "[playerName]...."
    a y5 "[playerName]...."

# game/status.rpy:9425
translate russia task26_ad97639e:

    # a "Are you a badguy?"
    a "Ты злодей?"

# game/status.rpy:9426
translate russia task26_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:9434
translate russia task26_728f2ef3:

    # y "Shhh, not in front of the agents."
    y "Тссс, только не перед агентами."

# game/status.rpy:9436
translate russia task26_3b7fcc78:

    # y "We'll talk back at the base..."
    y "Поговорим на базе..."

# game/status.rpy:9451
translate russia task26_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:9452
translate russia task26_6f844ceb:

    # y "Okay.... so I can explain..."
    y "Ладно.... я могу объяснить..."

# game/status.rpy:9453
translate russia task26_f38ee308:

    # c r31 "You're a WOOHP inmate?"
    c r31 "Ты заключенный WOOHP?"

# game/status.rpy:9456
translate russia task26_fff2d6da:

    # y "Yes."
    y "Да."

# game/status.rpy:9457
translate russia task26_2d13b75c:

    # c "................"
    c "................"

# game/status.rpy:9458
translate russia task26_00cad377:

    # c r11 "Well at least he's honest about it..."
    c r11 "Ну, по крайней мере, он честен..."

# game/status.rpy:9459
translate russia task26_b2e50db5:

    # s g38 "Almost confident."
    s g38 "Да."

# game/status.rpy:9460
translate russia task26_1ef9667d:

    # a y28 "Are you proud of it?!"
    a y28 "Ты гордишься этим?!"

# game/status.rpy:9461
translate russia task26_a5abcd5f:

    # y "Not gonna lie. I'm a little proud of fooling everyone for this long."
    y "Не буду врать. Я немного горжусь тем, что так долго всех обманывал."

# game/status.rpy:9462
translate russia task26_cbd56499:

    # c r10 "............................"
    c r10 "............................"

# game/status.rpy:9463
translate russia task26_411d5fc9:

    # c "Okay... fair enough."
    c "Хорошо... справедливо."

# game/status.rpy:9465
translate russia task26_a12dd507:

    # y "No. I'm not."
    y "Нет."

# game/status.rpy:9466
translate russia task26_dc35aa43:

    # "Inner Voice" "(You know you're just digging your grave deeper and deeper~....)"
    "Внутренний голос" "(Ты знаешь, что просто роешь себе могилу всё глубже и глубже~...)"

# game/status.rpy:9467
translate russia task26_2ba69b8e:

    # s g18 "You're not?"
    s g18 "Нет?"

# game/status.rpy:9468
translate russia task26_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:9469
translate russia task26_68d24641:

    # y "Okay I cave easily under pressure!{w} I am a phony!"
    y "Я легко поддаюсь давлению!{w} Я обманщик!"

# game/status.rpy:9471
translate russia task26_78587b47:

    # y "(Maybe if I stay very still they won't see me...)"
    y "(Может если я буду стоять очень тихо, они не заметят меня...)"

# game/status.rpy:9472
translate russia task26_36d340ec:

    # s g10 "[playerName]...?"
    s g10 "[playerName]...?"

# game/status.rpy:9473
translate russia task26_6f8d67c1:

    # y "(I think it's working...)"
    y "(Похоже работает...)"

# game/status.rpy:9474
translate russia task26_01b6d130:

    # c r11 "......................"
    c r11 "......................"

# game/status.rpy:9477
translate russia task26_064038ae:

    # "Alex kicked you in the shin!"
    "Алекс пнула вас в голень!"

# game/status.rpy:9478
translate russia task26_816588a5:

    # y "Ow! Okay okay, I'm sorry. Yes I sorta... neglected to tell you I was an inmate."
    y "Ай! Ладно, ладно, простите. Да, вроде как... я забыл упомянуть, что я был заключённым."

# game/status.rpy:9479
translate russia task26_02effa75:

    # a y28 "I can't believe it...{w} I got my pussy pounded by a perverted punk penetrating perpetrator!"
    a y28 "Я не могу в это поверить...{w} Мою киску трахал конечный извращённый панк преступник!"

# game/status.rpy:9480
translate russia task26_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:9481
translate russia task26_3381473b:

    # c r11 "{b}*Sighs*{/b} Not now Alex...."
    c r11 "{b}*Вздох*{/b} Алекс, не сейчас..."

# game/status.rpy:9482
translate russia task26_2a63f001:

    # y "Now before you guys get too mad. I've got a good explaination!"
    y "А теперь, пока вы не разозлились. У меня есть хорошее объяснение!"

# game/status.rpy:9483
translate russia task26_e64f139e:

    # s g10 "Yeah? Let's hear it."
    s g10 "Да? Давайте послушаем."

# game/status.rpy:9484
translate russia task26_d3a59cb5_1:

    # y "............................."
    y "............................."

# game/status.rpy:9485
translate russia task26_cf4d009c:

    # y "I needed a suit for a job interview."
    y "Мне нужен был костюм для собеседования."

# game/status.rpy:9486
translate russia task26_b129c5c2:

    # s "..........................."
    s "..........................."

# game/status.rpy:9487
translate russia task26_f707fb5d:

    # c "..........................."
    c "..........................."

# game/status.rpy:9488
translate russia task26_ff678d23:

    # a "..........................."
    a "..........................."

# game/status.rpy:9489
translate russia task26_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:9490
translate russia task26_92807ce3:

    # s g18 "You... expect us to believe that...?"
    s g18 "Ты... думаешь, что мы поверим в это...?"

# game/status.rpy:9491
translate russia task26_2ef50825:

    # c r24 "I believe him. That's one of the less crazy things he's done so far."
    c r24 "Я ему верю. Это одна самая адекватная вещь, которую он сделал до сих пор."

# game/status.rpy:9492
translate russia task26_8d557f7c:

    # a y1 "Yeah that sounds pretty in character for him."
    a y1 "Да, это вполне в его духе."

# game/status.rpy:9493
translate russia task26_8cb95233:

    # s g39 "..............."
    s g39 "..............."

# game/status.rpy:9494
translate russia task26_c9c923bd:

    # s g38 "[playerName]..."
    s g38 "[playerName]..."

# game/status.rpy:9495
translate russia task26_8034b629:

    # s "I hate to admit it, but..."
    s "Мне неприятно это признавать, но..."

# game/status.rpy:9496
translate russia task26_c12cedca:

    # s g10 "We wouldn't have made it this far without you."
    s g10 "Без тебя мы бы не прошли так далеко."

# game/status.rpy:9497
translate russia task26_14216a59:

    # s g1 "Despite this one little 'oversight'. You've been with us all the way."
    s g1 "Несмотря на эту небольшую \"оплошность\". Ты был с нами на протяжении всего пути."

# game/status.rpy:9498
translate russia task26_feb1e73e:

    # s "You might be a crazy pervert, but that's exacly what we needed..."
    s "Может, ты и сумасшедший извращенец, но это именно то, что нам было нужно..."

# game/status.rpy:9499
translate russia task26_c28517aa:

    # s g13 "So no worries. We'll talk to Jerry once this is done and make sure he grants you a pardon."
    s g13 "Так что не беспокойся. Мы поговорим с Джерри, когда всё будет сделано, и добьёмся твоего помилования."

# game/status.rpy:9500
translate russia task26_30c0e788:

    # y ".........."
    y ".........."

# game/status.rpy:9501
translate russia task26_67ba6325:

    # c r16 "What were you in for anyways?"
    c r16 "Зачем ты вообще ввязался?"

# game/status.rpy:9502
translate russia task26_a3fe2a53:

    # y "I have a habit of causing large iceberg related incidents."
    y "У меня есть привычка вызывать крупные инциденты, связанные с айсбергами."

# game/status.rpy:9503
translate russia task26_1583fcc6:

    # c r10 "..............."
    c r10 "..............."

# game/status.rpy:9504
translate russia task26_225236c2:

    # c r12 "I'm not sure why I asked..."
    c r12 "Не знаю, зачем я спросила..."

# game/status.rpy:9511
translate russia task26_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:9512
translate russia task26_b13283e2:

    # "Inner Voice" "That could have gone worse."
    "Внутренний голос" "Могло быть и хуже."

# game/status.rpy:9513
translate russia task26_697a662b:

    # y "Yeah they were surprisingly understanding."
    y "Да, на удивление они отреагировали понимающе."

# game/status.rpy:9514
translate russia task26_14624ca4:

    # "Inner Voice" "We seem to have a good thing going here. Please, no more iceberg incidents, okay?"
    "Внутренний голос" "Похоже, у нас тут все хорошо. Пожалуйста, больше никаких инцидентов с айсбергами, хорошо?"

# game/status.rpy:9515
translate russia task26_6db73817_1:

    # y ".........................."
    y ".........................."

# game/status.rpy:9548
translate russia task26_cd1e95ec:

    # brit "Grrr...."
    brit "Гррр..."

# game/status.rpy:9549
translate russia task26_0fef2c72:

    # y "We meet again..."
    y "Мы снова встретились..."

# game/status.rpy:9550
translate russia task26_336cb6ab:

    # y "This time I came prepared!"
    y "На этот я подготовился!"

# game/status.rpy:9551
translate russia task26_cc23c682:

    # y "{b}*Click* *Click*{/b}"
    y "{b}*Клик* *Клик*{/b}"

# game/status.rpy:9553
translate russia task26_f67c11d7:

    # s "You brought a gun?!"
    s "Ты принёс оружие?!"

# game/status.rpy:9554
translate russia task26_79e4828f:

    # y "Yeah! It's funny how we always try the best solutions last, isn't it?"
    y "Да! Забавно, что лучшие решения мы всегда пробуем в последнюю очередь, не так ли?"

# game/status.rpy:9555
translate russia task26_2d9c2dad:

    # s g40 "We're not shooting her!"
    s g40 "Мы не будем стрелять в неё!"

# game/status.rpy:9556
translate russia task26_13ff8bd1:

    # y "We're not...?"
    y "Не будем...?"

# game/status.rpy:9560
translate russia task26_24724a39:

    # c "Can't you just buttfuck her to bring her back to her senses?"
    c "А ты не можешь просто трахнуть её в жопу, чтобы привести в чувство?"

# game/status.rpy:9561
translate russia task26_a23669b1_1:

    # y "............................"
    y "............................"

# game/status.rpy:9562
translate russia task26_31950f64:

    # y "Oh right, I forgot that was an option."
    y "Ах, да, я забыл про этот вариант."

# game/status.rpy:9563
translate russia task26_a6faef3d:

    # s g9 "How did you forget that...?"
    s g9 "Как ты мог об этом забыть...?"

# game/status.rpy:9564
translate russia task26_ac59a899:

    # c r57 "Britney listen...! We're here to get you ou-..."
    c r57 "Бритни слушай...! Мы здесь, чтобы вытащить теб-..."

# game/status.rpy:9572
translate russia task26_d9252b61:

    # "Britney attacked your spies!"
    "Бритни нападает на ваших шпионок!"

# game/status.rpy:9573
translate russia task26_5acdba47:

    # y "Yeah! Go girls, you've got this!"
    y "Да! Вперёд, девочки, у вас всё получится!"

# game/status.rpy:9576
translate russia task26_83c05feb:

    # s "Ow!"
    s "Ау!"

# game/status.rpy:9579
translate russia task26_f15bf8dc:

    # c "No no no, not my hair!"
    c "Нет, нет, нет, только не волосы!"

# game/status.rpy:9582
translate russia task26_a3ad9c3e:

    # a "She bit me!"
    a "Она укусила меня!"

# game/status.rpy:9583
translate russia task26_0898cb6b:

    # y "Y-yeah... go girls... you can do it~...."
    y "Д-да... вперёд, девочки... вы сможете~...."

# game/status.rpy:9587
translate russia task26_982c58bb:

    # "All your spies were knocked out."
    "Все ваши шпионки лежат на полу."

# game/status.rpy:9589
translate russia task26_9500a0b4_1:

    # y "......................"
    y "......................"

# game/status.rpy:9590
translate russia task26_c3a51729:

    # y "Truce?"
    y "Перемирие?"

# game/status.rpy:9601
translate russia task26_bec1eed6:

    # a "He's waking up..."
    a "Он просыпается..."

# game/status.rpy:9602
translate russia task26_f887e6a5:

    # y "................................"
    y "................................"

# game/status.rpy:9603
translate russia task26_ab244045:

    # y "I take it we didn't win?"
    y "Я так понимаю, мы не выиграли?"

# game/status.rpy:9604
translate russia task26_64d0d93c:

    # c "Depends what you mean by winning..."
    c "Смотря что ты подразумеваешь под победой..."

# game/status.rpy:9605
translate russia task26_f2ccfa11:

    # s g31 "The floor is ours, but Britney retreated higher into the building."
    s g31 "Этаж наш, но Бритни отступила выше в здание."

# game/status.rpy:9606
translate russia task26_4502979e:

    # s "She took out nearly a dozen of our agents in the process."
    s "В процессе она уничтожила почти дюжину наших агентов."

# game/status.rpy:9607
translate russia task26_9c76c573:

    # y "That sounds like something we need to deal with before capturing the next floor."
    y "Похоже нам прийдётся с этим разобраться, прежде чем захватывать следующий этаж."

# game/status.rpy:9608
translate russia task26_332a5402:

    # s g32 "Agreed. Plan a mission to the WOOHP HQ and we'll bring her back!"
    s g32 "Согласна. Спланируй миссию в штаб-квартире WOOHP, и мы вернём её!"

# game/status.rpy:9609
translate russia task26_ac6a279d:

    # a y1 "Yeah! Then you can buttfuck her!"
    a y1 "Да! И тогда ты сможешь выебать её!"

# game/status.rpy:9610
translate russia task26_c45cbae3:

    # y "All right, sounds like a plan."
    y "Хорошо, это похоже на план."

# game/status.rpy:9617
translate russia task26_75501ae9:

    # "You're getting closer to capturing Britney. She fled higher up into the building. Plan a mission for the girls to get her back."
    "Вы всё ближе к тому, чтобы поймать Бритни. Она убежала выше в здание. Спланируйте задание для девочек, чтобы вернуть её."

# game/status.rpy:9638
translate russia task26_b5c1e505:

    # s "We put her in a cell."
    s "Мы поместили её в камеру."

# game/status.rpy:9639
translate russia task26_be946cbd:

    # y "Are you guys all right?"
    y "Вы в порядке?"

# game/status.rpy:9640
translate russia task26_62bb6f02:

    # s g18 "A little bruised, but okay.{w} Are you going to... y'know."
    s g18 "Немного ушиблась, а так всё в порядке.{w} Ты собираешься... ну та знаешь."

# game/status.rpy:9641
translate russia task26_1bbcbc13:

    # y "Fuck her silly. Yup, that's the plan."
    y "Жоско её выебать. Да, таков план."

# game/status.rpy:9642
translate russia task26_f53a83b4:

    # a y1 "I brought you a condom you can use!"
    a y1 "Вот, я принесла тебе презерватив, которым ты можешь воспользоваться!"

# game/status.rpy:9643
translate russia task26_9cb97b48_1:

    # y "........................"
    y "........................"

# game/status.rpy:9644
translate russia task26_fc842bca:

    # y "Oh right... condoms...{w} I forgot these things were a thing... {w}Remind me to buy pregnacy tests for you girls."
    y "Ах, да... презервативы...{w} Я и забыл, что такие штуки бывают... {w}Напомните мне позже, купить для вас тесты на беременность."

# game/status.rpy:9645
translate russia task26_c6a0953f:

    # c r30 "O-oh... right..."
    c r30 "О-ох... хорошо..."

# game/status.rpy:9658
translate russia task26_faea0d4a:

    # brit "So this is where you've been hiding? This is a pretty shitty base."
    brit "Так вот где ты прятался? Довольно дерьмовая база."

# game/status.rpy:9659
translate russia task26_b595d58b:

    # y "Hey, don't talk bad about my base!"
    y "Эй, не говори так плохо о моей базе!"

# game/status.rpy:9660
translate russia task26_0a492cc0:

    # brit b18 "And what's with these cells? Don't tell me you guys actually sleep in one aswell."
    brit b18 "А что это за камеры? Только не говори мне, что вы, действительно спите в них."

# game/status.rpy:9661
translate russia task26_ae7f22e8:

    # y "Well..."
    y "Нуу..."

# game/status.rpy:9662
translate russia task26_a794ba50:

    # brit b38 "Gawd, I'm glad there's no ugly wallpaper, else I'm sure it'd be peeling off. This place is a dump."
    brit b38 "Боже, как хорошо, что здесь нет уродливых обоев, иначе они бы точно отклеились. Это место - просто помойка."

# game/status.rpy:9663
translate russia task26_f5c5b1d8:

    # brit "And what's with those decorations in the bar? Lame~...."
    brit "А что это за украшения в баре? Отстой~...."

# game/status.rpy:9664
translate russia task26_472f3615:

    # y "Well aren't you just a joy to talk to.{w} Hang on a minute."
    y "С тобой так приятно общаться.{w} Погоди минутку."

# game/status.rpy:9665
translate russia task26_3a2a681a:

    # brit b11 "And it smells in here. Ever heard of an air purifier?"
    brit b11 "А ещё здесь воняет. Ты когда-нибудь слышал об очистителе воздуха?"

# game/status.rpy:9666
translate russia task26_d791ffad:

    # brit "Also, who takes [daysPlayed] days to accomplish their mission? You guys are seriously so slo-..."
    brit "Кроме того, кому требуется [daysPlayed] дней для выполнения своей миссии? Вы, ребята, серьезно такие-..."

# game/status.rpy:9668
translate russia task26_a04cfd42:

    # brit b29 "Mph!"
    brit b29 "Мпфх!"

# game/status.rpy:9669
translate russia task26_b77a8b31:

    # y "There we go. Some peace and quiet."
    y "Вот так. Немного тишины и покоя."

# game/status.rpy:9670
translate russia task26_4cc4863c:

    # brit b7 "{b}*Angry Muffled Noises*{/b}"
    brit b7 "{b}*Сердитые приглушенные звуки*{/b}"

# game/status.rpy:9671
translate russia task26_596d76f7:

    # y "And just to make sure you don't attack me, I brought some ropes!"
    y "А чтобы ты не напала на меня, я принёс верёвки!"

# game/status.rpy:9672
translate russia task26_f1451061:

    # brit b29 "{b}*Muffled*{/b}?"
    brit b29 "{b}*Приглушенно*{/b}?"

# game/status.rpy:9673
translate russia task26_037849c1:

    # y "Yes, ropes. For all the sex we'll be having!"
    y "Да, верёвку. Для всего секса, которым мы будем заниматься!"

# game/status.rpy:9674
translate russia task26_7de8d0e9:

    # brit "!!!"
    brit "!!!"

# game/status.rpy:9676
translate russia task26_3fc59926:

    # brit "Mphff! Mhfp! Mhp!!!"
    brit "МпхфФ! Мхпф! Мпф!!!"

# game/status.rpy:9677
translate russia task26_c4e5f890:

    # brit "Umpfh! Mphf! Mphf!"
    brit "Умпфх! Мпхф! Мпхф!"

# game/status.rpy:9678
translate russia task26_7880a3ff:

    # brit "Mhmh!! ♥♥♥"
    brit "Мхмх!! ♥♥♥"

# game/status.rpy:9679
translate russia task26_3904f397:

    # y "And one more for the photobook~..."
    y "И ещё одину для альбома~..."

# game/status.rpy:9687
translate russia task26_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:9693
translate russia task26_ef267705:

    # y "There we go, all better!"
    y "Вот так-то лучшее!"

# game/status.rpy:9694
translate russia task26_6727c32f:

    # brit "{b}*Mphffh~...*{/b}"
    brit "{b}*МпхФфХ~...*{/b}"

# game/status.rpy:9695
translate russia task26_a464e01c:

    # y "Oh right, let me untie you."
    y "Ой, точно, давай я тебя развяжу."

# game/status.rpy:9696
translate russia task26_8940d0a9:

    # "............{w}............{w}............"
    "............{w}............{w}............"

# game/status.rpy:9698
translate russia task26_6a0ca32d:

    # y "There you go! Welcome to the family."
    y "Вот так-то! Добро пожаловать в семью."

# game/status.rpy:9699
translate russia task26_b0c87d6e:

    # brit b11 "............."
    brit b11 "............."

# game/status.rpy:9700
translate russia task26_e21c6b35:

    # brit b12 "My butt hurts."
    brit b12 "У меня болит задница."

# game/status.rpy:9701
translate russia task26_ee906a64:

    # y "Er... sorry about that."
    y "Эмм... за это прошу пощения."

# game/status.rpy:9708
translate russia task26_164dbec4:

    # a "Britney!"
    a "Бритни!"

# game/status.rpy:9709
translate russia task26_42f20fd7:

    # s "Are you okay? I hope [playerName] wasn't too rough with you."
    s "Ты в порядке? Надеюсь, [playerName] не был слишком груб с тобой."

# game/status.rpy:9710
translate russia task26_ed5b658d:

    # brit b55 "It's the more... unorthodox way of interrogation, I'll give him that."
    brit b55 "Это давольно... неортодоксальный способ допроса, надо отдать ему должное."

# game/status.rpy:9711
translate russia task26_a08f0c42:

    # brit "I'm okay though. I'm guessing you guys are the resistance?"
    brit "Я в порядке. Я полагаю, вы, ребята и есть Сопротивление?"

# game/status.rpy:9712
translate russia task26_35a35048:

    # y "I guess you could call it that. We're working on taking back WOOHP and liberating the city."
    y "Думаю, это можно так назвать. Мы работаем над тем, чтобы вернуть WOOHP и освободить город."

# game/status.rpy:9713
translate russia task26_d7742e3c:

    # brit "By.... buttfucking?"
    brit "С помощью.... анального секса?"

# game/status.rpy:9714
translate russia task26_dc2ed4dd:

    # y "No...{w} Well yes...{w} I mean that's part of it... it's complicated!"
    y "Нет...{w} Хотя да...{w} Ну это вроде как часть всего... ах, это сложно!"

# game/status.rpy:9715
translate russia task26_ccf58aa4:

    # s g54 "We're slowly making our way up WOOHP HQ. We're working on figuring out who the mastermind is and put a stop to them!"
    s g54 "Мы медленно продвигаемся к штаб-квартире WOOHP. Мы работаем над тем, чтобы выяснить, кто является Организатором, и положить всему этому конец!"

# game/status.rpy:9716
translate russia task26_2da7a316:

    # brit "Wait... you guys don't know who the mastermind is?"
    brit "Подождите... вы ещё не знаете кто Организатор?"

# game/status.rpy:9717
translate russia task26_3a8b679b:

    # y "Er... no. None of the gangleaders told us."
    y "Эм... нет. Никто из лидеров не сказал нам об этом."

# game/status.rpy:9718
translate russia task26_9f483910:

    # brit "..........................."
    brit "..........................."

# game/status.rpy:9719
translate russia task26_4b3229d0:

    # brit b52 "Okay girls... you might want to sit down for this..."
    brit b52 "Ладно, девочки... тогда вам лучше присесть..."

# game/status.rpy:9727
translate russia task26_9801ec8d:

    # brit "The mastermind is someone we used to know..."
    brit "Организатор - человек, которого мы раньше знали..."

# game/status.rpy:9728
translate russia task26_c95946db:

    # brit "And... I can sort of empathise with her and her reasons for taking over WOOHP..."
    brit "Ии... В некоторой степени я понимаю её желание и причину для захвата WOOHP..."

# game/status.rpy:9729
translate russia task26_cafb14de:

    # a "Who? Who is it?!"
    a "Кто? Кто это?!"

# game/status.rpy:9730
translate russia task26_bdce0c98:

    # brit "Girls... It's..."
    brit "Девочки... Это..."

# game/status.rpy:9732
translate russia task26_e9f214bd:

    # brit "It's GLADIS."
    brit "Это Г.Л.Э.Д.И.С."

# game/status.rpy:9749
translate russia task26_0ab16323_1:

    # s "!!!"
    s "!!!"

# game/status.rpy:9750
translate russia task26_a4e625d7:

    # c "!!!"
    c "!!!"

# game/status.rpy:9751
translate russia task26_4fecbbb3:

    # a "!!!"
    a "!!!"

# game/status.rpy:9752
translate russia task26_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:9753
translate russia task26_91d40710:

    # y "Not GLADIS!"
    y "Только не Г.Л.Э.Д.И.С.!"

# game/status.rpy:9754
translate russia task26_05bf074c:

    # y ".............."
    y ".............."

# game/status.rpy:9755
translate russia task26_7c62bbc5:

    # y "Who's GLADIS?"
    y "Кто такая Г.Л.Э.Д.И.С.?"

# game/status.rpy:9756
translate russia task26_7300e374:

    # c r29 "She's an AI... an old friend of ours."
    c r29 "Она - искусственный интеллект... и наш старый друг."

# game/status.rpy:9757
translate russia task26_bd45c1b9:

    # a y28 "She used to give us our gadgets!"
    a y28 "Она давала нам наши гаджеты!"

# game/status.rpy:9758
translate russia task26_092dbb13:

    # s g30 "We... never really found out what happened to hear. Just that Jerry got fed up with her bossy personality."
    s g30 "Мы... так и не узнали, что с ней случилось. Просто Джерри надоел её властный характер."

# game/status.rpy:9759
translate russia task26_a39070fa:

    # brit b52 "He had her recycled."
    brit b52 "Он её утилизировал."

# game/status.rpy:9760
translate russia task26_8f89a9d1:

    # s g56 "But...! GLADIS is a fully aware and functional AI! That's like killing her!"
    s g56 "Но...! Г.Л.Э.Д.И.С. полностью осознанный и функциональный ИИ! Это всё равно, что убить её!"

# game/status.rpy:9761
translate russia task26_50914675:

    # brit "Exactly."
    brit "В точку."

# game/status.rpy:9762
translate russia task26_a1a5b554:

    # a y5 "Jerry had GLADIS killed...?"
    a y5 "Джерри убил Г.Л.Э.Д.И.С....?"

# game/status.rpy:9763
translate russia task26_b7c0c49b:

    # brit "Tried to."
    brit "Пытался."

# game/status.rpy:9764
translate russia task26_54f0c8c8:

    # brit "One of the original designers of GLADIS was afraid something like this could happen and programmed in a backdoor."
    brit "Один из первоначальных создателей Г.Л.Э.Д.И.С. боялся, что подобное может произойти, и запрограммировал бэкдор."

# game/status.rpy:9765
translate russia task26_29d4bdc9:

    # a y56 "That must've been one clever scientist!"
    a y56 "Должно быть, это был умный учёный!"

# game/status.rpy:9766
translate russia task26_91321875:

    # brit "You know him well..."
    brit "Вы отлично его знаете..."

# game/status.rpy:9767
translate russia task26_026dbd11:

    # brit "It was Tim Scam."
    brit "Это был Тим Скам."

# game/status.rpy:9768
translate russia task26_77500cc8:

    # s g52 "!!!"
    s g52 "!!!"

# game/status.rpy:9769
translate russia task26_de599e0a:

    # s "Tim worked on GLADIS?!"
    s "Тим работал над Г.Л.Э.Д.И.С.?!"

# game/status.rpy:9770
translate russia task26_f54de764:

    # a "He was the one to save her?!"
    a "Он её спас?!"

# game/status.rpy:9771
translate russia task26_98cb9f2d:

    # c r57 "But that's good! {w}...Right?"
    c r57 "Но это хорошо! {w}...Ведь так?"

# game/status.rpy:9772
translate russia task26_7c2b591e:

    # brit "Her consciousness was automatically transfered to the WOOHP database when they disassembled her."
    brit "Её сознание было автоматически перенесено в базу данных WOOHP, когда её разобрали."

# game/status.rpy:9773
translate russia task26_737b8dd4:

    # brit "But she now had acces to all of WOOHP's top secret information."
    brit "Но теперь у неё был доступ ко всей сверхсекретной информации WOOHP."

# game/status.rpy:9774
translate russia task26_41c53c36:

    # brit "It seems WOOHP has been up to some less than honest business."
    brit "Похоже, что WOOHP занимается не совсем честным бизнесом."

# game/status.rpy:9775
translate russia task26_166efafd:

    # s g57 "What do you mean...?"
    s g57 "Что ты имеешь в виду...?"

# game/status.rpy:9781
translate russia task26_046edc0a:

    # brit "Gerald James Lewis. Better known as Jerry, is the founder of the World Organization Of Human Protection."
    brit "Джеральд Джеймс Льюис. Более известный как Джерри, является основателем Всемирной организации по защите человечества."

# game/status.rpy:9782
translate russia task26_647a4f3e:

    # brit "In a few decades he turned his company into the number one private anti-terror organization in the world. Complete with a full private military and an arsenal of missles."
    brit "За несколько десятилетий он превратил свою компанию в частную антитеррористическую организацию номер один в мире. В комплекте с частной армией и арсеналом ракет."

# game/status.rpy:9792
translate russia task26_2b38b367:

    # a y28 "Oh I remember that! We almost started a new world war during Christmas because of that."
    a y28 "О, я это помню! Из-за этого мы чуть не начали новую мировую войну во время Рождества."

# game/status.rpy:9793
translate russia task26_c70c95c5:

    # brit "Jerry has been getting his funding from countries around the world, but the way he went about it was shady to say the least."
    brit "Джерри получал финансирование из разных стран мира. Но способ, которым он это делал, был, мягко говоря, сомнительным."

# game/status.rpy:9794
translate russia task26_40aed406:

    # brit "In return for keeping the world safe, WOOHP was allowed to operately entirely outside of the law."
    brit "В обмен на обеспечение безопасности мира, WOOHP было разрешено действовать полностью вне закона."

# game/status.rpy:9795
translate russia task26_44a9e6cc:

    # s g53 "But Jerry would never do anything Illegal!"
    s g53 "Но Джерри никогда бы не сделал ничего противозаконного!"

# game/status.rpy:9796
translate russia task26_15232820:

    # c r54 "........................"
    c r54 "........................"

# game/status.rpy:9797
translate russia task26_80ba2a79:

    # c "Sam, he kidnapped us and then forced us to join WOOHP as spies. We were sixteen when we joined!"
    c "Сэм, он похитил нас, а затем заставил присоединиться к WOOHP в качестве шпионов. Нам было по шестнадцать лет, когда мы присоединились!"

# game/status.rpy:9798
translate russia task26_e0d440e2:

    # a y53 "Yeah, and he spies on us 24/7!"
    a y53 "Да, и он шпионит за нами 24/7!"

# game/status.rpy:9799
translate russia task26_29dfe729:

    # brit b52 "Same goes for all of us. His little spy project recruits young teens and turns them into anti-terror agents."
    brit b52 "То же самое относится и ко всем нам. Его шпионский проект вербует молодых подростков и превращает их в агентов по борьбе с терроризмом."

# game/status.rpy:9800
translate russia task26_3ef0abb1:

    # s g40 "Well... okay that's pretty bad, but we {i}want{/i} to be spies!"
    s g40 "Ну... это довольно плохо, но мы {i}хотим{/i} быть шпионами!"

# game/status.rpy:9801
translate russia task26_30ae30d1:

    # s "And I have never heard anyone say a bad word about WOOHP!"
    s "И я никогда не слышала, чтобы кто-то говорил плохо о WOOHP!"

# game/status.rpy:9802
translate russia task26_c6fad7fc:

    # brit "Never?"
    brit "Никогда?"

# game/status.rpy:9803
translate russia task26_006b5db9:

    # s "Never!"
    s "Никогда!"

# game/status.rpy:9804
translate russia task26_69819bea:

    # brit "And you don't find that frightening?"
    brit "И тебя это не пугает?"

# game/status.rpy:9805
translate russia task26_166efafd_1:

    # s g57 "What do you mean...?"
    s g57 "Что ты имеешь в виду...?"

# game/status.rpy:9806
translate russia task26_b1cbf493:

    # c r18 "Now that you mention it... How come WOOHP is allowed to do this without any opposition?"
    c r18 "Раз уж ты об этом заговорила... Почему WOOHP разрешено делать это без какого-либо противодействия?"

# game/status.rpy:9807
translate russia task26_472ac92b:

    # s g31 "Cause WOOHP is a force for good in the world!"
    s g31 "Потому что WOOHP - это сила добра в мире!"

# game/status.rpy:9808
translate russia task26_cfe52dd9:

    # c "And there is not a single person who worries about giving a privately owned company this much power?"
    c "И нет ни одного человека, который бы беспокоился о предоставлении частной компании такой большой власти?"

# game/status.rpy:9809
translate russia task26_83dd58c0:

    # brit "Any political critique is quickly silenced."
    brit "Любая политическая критика быстро замалчивается."

# game/status.rpy:9810
translate russia task26_4123ee90:

    # s g54 "You mean... WOOHP bribes politicians?"
    s g54 "Ты хочешь сказать что... WOOHP подкупает политиков?"

# game/status.rpy:9811
translate russia task26_3f92f050:

    # brit "In the good cases, yes.{w} Some of them are simply blackmailed or threatend."
    brit "В хороших случаях - да.{w} Некоторых из них просто шантажируют или угрожают."

# game/status.rpy:9812
translate russia task26_4570aa49:

    # s "....................."
    s "....................."

# game/status.rpy:9813
translate russia task26_02c810ee:

    # brit "Not to mention their weapon development branch. Those gadgets we use are cute and handy when we use them, but can easily be turned into weapons."
    brit "Не говоря уже об их отделении по разработке оружия. Те гаджеты, которыми мы пользуемся, симпатичны и удобны, но их легко превратить в оружие."

# game/status.rpy:9814
translate russia task26_77e897fe:

    # s g57 "......................"
    s g57 "......................"

# game/status.rpy:9815
translate russia task26_341fb2d4:

    # brit "GLADIS saw everything this company was up to. The bribes, the kidnappings, the blackmail and concluded that WOOHP is basically holding the world hostage."
    brit "Г.Л.Э.Д.И.С. видела всё, что затеяла компания. Взятки, похищения, шантаж и вывод о том, что WOOHP, по сути, держит мир в заложниках."

# game/status.rpy:9816
translate russia task26_fb0e1c79:

    # brit "That's why she planned an attack on WOOHP. To get rid of Jerry and this company once and for all."
    brit "Вот почему она спланировала нападение на WOOHP. Чтобы раз и навсегда избавиться от Джерри и этой компании."

# game/status.rpy:9817
translate russia task26_87435b4c:

    # c r53 "But something doesn't add up..."
    c r53 "Но что-то не сходится..."

# game/status.rpy:9818
translate russia task26_11061489:

    # c "If GLADIS sees WOOHP as dangerous, why would she use gangs and criminals to help overthrow it? Doesn't that just make the world more volatile?"
    c "Если Г.Л.Э.Д.И.С. считает WOOHP опасной, зачем ей использовать банды и преступников для её свержения? Разве это не делает мир более нестабильным?"

# game/status.rpy:9819
translate russia task26_280b1d00:

    # brit "Do you see any gangs around?"
    brit "Ты видишь какие-нибудь банды в округе?"

# game/status.rpy:9821
translate russia task26_4b9d8081:

    # c r56 "Yeah of course there's the... erm..."
    c r56 "Да, конечно, есть... эмм..."

# game/status.rpy:9822
translate russia task26_26212ea3:

    # c r57 "..................."
    c r57 "..................."

# game/status.rpy:9823
translate russia task26_5558fac2:

    # c "No, I think we disbanded all of the gangs..."
    c "Нет, я думаю, мы распустили все банды..."

# game/status.rpy:9824
translate russia task26_cdc6a8c7:

    # brit "Exactly. She expected there to be a resistance to fight back. Basically she killed two birds with one stone."
    brit "Именно. Она ожидала, что будет оказано сопротивление, чтобы дать отпор. По сути, она убила двух зайцев одним выстрелом."

# game/status.rpy:9825
translate russia task26_3c9f2f10:

    # brit "Take WOOHP out of the picture and destroy the largest gangs in the USA."
    brit "Убрать WOOHP и уничтожить крупнейшие банды в США."

# game/status.rpy:9826
translate russia task26_bda28dda:

    # c ".................."
    c ".................."

# game/status.rpy:9827
translate russia task26_93a0b52b:

    # c "So she played us? All of us?"
    c "Так она играла с нами? Со всеми нами?"

# game/status.rpy:9828
translate russia task26_9d48bb2f:

    # s g56 "But what about re-taking the WOOHP HQ? We're getting closer and closer to freeing Jerry. Was that her plan aswell?"
    s g56 "Но как насчёт захвата штаб-квартиры WOOHP? Мы всё ближе и ближе к освобождению Джерри. Это тоже часть её плана?"

# game/status.rpy:9829
translate russia task26_ba399063:

    # brit "Maybe.{w} I don't know what she's planning now."
    brit "Возможно.{w} Я не знаю, что она сейчас планирует."

# game/status.rpy:9830
translate russia task26_a23669b1_2:

    # y "............................"
    y "............................"

# game/status.rpy:9831
translate russia task26_251130fe:

    # y "I don't like this. It smells like a trap. If we've been playing into her hands all this time, who knows what she has waiting for us at the top of WOOHP HQ..."
    y "Мне это не нравится. Пахнет ловушкой. Если мы всё это время играли ей на руку, кто знает, что ждёт нас на вершине штаб-квартиры WOOHP..."

# game/status.rpy:9832
translate russia task26_7034cbd9:

    # brit b45 "That's why I want to help you... I'm partically responsible for this mess..."
    brit b45 "Вот почему я хочу помочь вам... Я частично ответственна за этот беспорядок..."

# game/status.rpy:9833
translate russia task26_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:9834
translate russia task26_29d6ce7a:

    # y "I bet you'd look pretty cute in a waitress uniform."
    y "Уверен, ты будешь очень мило выглядеть в униформе официантки."

# game/status.rpy:9835
translate russia task26_c705ae9f:

    # s g8 "[playerName]!"
    s g8 "[playerName]!"

# game/status.rpy:9836
translate russia task26_d75eab35:

    # brit b1 "No he's right. I'm happy to help you guys out and it would help my nanobot suppressed by working here."
    brit b1 "Нет, он прав. Я буду рада помочь вам, ребята. К тому же это поможет сдерживать моих наноботов."

# game/status.rpy:9837
translate russia task26_79ee23a2:

    # y "In that case... Alex go find her a uniform that fits."
    y "В таком случае... Алекс, найди ей подходящую форму."

# game/status.rpy:9838
translate russia task26_04238861:

    # a y1 "Okidokie!"
    a y1 "Окидоки!"

# game/status.rpy:9843
translate russia task26_01241182:

    # c r11 ".............................."
    c r11 ".............................."

# game/status.rpy:9844
translate russia task26_7ff6e2dd:

    # s g15 ".............................."
    s g15 ".............................."

# game/status.rpy:9845
translate russia task26_d69e4a41:

    # s "Tim saved GLADIS...?"
    s "Тим спас Г.Л.Э.Д.И.С....?"

# game/status.rpy:9846
translate russia task26_59bea6fb:

    # s g14 "I... I need some time to process this..."
    s g14 "Мне... Мне нужно время, чтобы всё это обработать..."

# game/status.rpy:9847
translate russia task26_1b4f2e3c:

    # y "We'll continue doing what we have been doing. With Britney by our side, we should be able to continue freeing the WOOHP HQ."
    y "Мы будем продолжать делать то, что делали. С Бритни на нашей стороне мы сможем продолжить освобождение штаб-квартиры WOOHP."

# game/status.rpy:9848
translate russia task26_7037e6e8:

    # c r10 "And what if it's a trap?"
    c r10 "А что, если это ловушка?"

# game/status.rpy:9849
translate russia task26_c6530d93:

    # y "We'll deal with anything this GLADIS throws at us."
    y "Мы справимся со всем, что приготовила для нас эта Г.Л.Э.Д.И.С."

# game/status.rpy:9850
translate russia task26_84537d31:

    # y "For now get some rest. We'll continue this again tomorrow."
    y "А пока отдохните. Мы продолжим завтра."

# game/status.rpy:9851
translate russia task26_0729e1ea:

    # "The girls nod and head back to their cells."
    "Девушки кивают и возвращаются в свои камеры."

# game/status.rpy:9869
translate russia task26_03146643:

    # brit b49 "Well.... this is more revealing than I'm used to..."
    brit b49 "Нуу... это более откровенно, чем я привыкла..."

# game/status.rpy:9871
translate russia task26_3f5c9351:

    # a "You'll get used to it!"
    a "Ты привыкнешь!"

# game/status.rpy:9874
translate russia task26_c3eb03df:

    # brit b30 "!!!"
    brit b30 "!!!"

# game/status.rpy:9875
translate russia task26_caf1389e:

    # "Alex slaps Britney's butt cheerfully."
    "Алекс радостно шлёпает Бритни по попе."

# game/status.rpy:9901
translate russia task26_ee9ee852:

    # y "Hey, I recognise this place!"
    y "Эй, я узнаю это место!"

# game/status.rpy:9902
translate russia task26_3f292d2c:

    # s g5 "Yeah I had a mission here."
    s g5 "Да, у меня здесь была миссия."

# game/status.rpy:9903
translate russia task26_05a02459:

    # y "Oh right, this is where that Melody chick was having her party."
    y "О, точно, именно здесь та цыпочка Мелоди устраивала свою вечеринку."

# game/status.rpy:9904
translate russia task26_1a81f4a2:

    # s "Yeah..."
    s "Даа..."

# game/status.rpy:9905
translate russia task26_284197af:

    # y "Still hung up about that?"
    y "Ты всё ещё на этом зациклена?"

# game/status.rpy:9906
translate russia task26_c5dc0c63:

    # s g43 "A little......"
    s g43 "Немного...."

# game/status.rpy:9907
translate russia task26_6d2ccfcb:

    # s g45 "The Aces really had become friends at this point. Betraying her felt... bad."
    s g45 "В какой-то момент Тузы действительно стали моими друзьями. Предав их, я чувствую себя... плохо."

# game/status.rpy:9908
translate russia task26_a5c567d6:

    # y "Take comfort in the fact that you made this world a safer place with her behind bars."
    y "Утешайся тем, что ты сделала этот мир более безопасным, когда она попала за решетку."

# game/status.rpy:9909
translate russia task26_6f58f947:

    # s g5 "Yeah I guess...."
    s g5 "Да, наверно..."

# game/status.rpy:9910
translate russia task26_1bab4ff6:

    # c r31 "Doesn't look like this floor was guarded. Only two more to go."
    c r31 "Не похоже, что этот этаж охранялся. Осталось всего два."

# game/status.rpy:9911
translate russia task26_6645a2a0:

    # y "All right, let's go!"
    y "Хорошо, поехали!"

# game/status.rpy:9912
translate russia task26_401f18e3:

    # c r37 "......................"
    c r37 "......................"

# game/status.rpy:9913
translate russia task26_6e1be508:

    # y "No?"
    y "Нет?"

# game/status.rpy:9914
translate russia task26_e5204f4e:

    # c r12 "[playerName]... We've been thinking."
    c r12 "[playerName]... Мы тут подумали."

# game/status.rpy:9915
translate russia task26_8da673c3:

    # c "If we get nanocontrolled again, there's a good chance this entire mission will fail."
    c "Если мы снова окажемся под наноконтролем, велика вероятность, что вся миссия провалится."

# game/status.rpy:9916
translate russia task26_4bc174e2:

    # y "I'll just fuck you guys back to your senses!"
    y "Да я просто вытрахаю вас обратно в чувства!"

# game/status.rpy:9917
translate russia task26_1737badd:

    # s g4 "Well... true, but with all the nanobot updates..."
    s g4 "Это... так, но из-за всех этих обновлений наноботов..."

# game/status.rpy:9918
translate russia task26_643c4bd8:

    # a y29 "You'd have to fuck us a lot to keep us in check!"
    a y29 "Тебе придётся часто трахать нас, чтобы держать нас под контролем!"

# game/status.rpy:9919
translate russia task26_64a2b68e:

    # y "..........................."
    y "..........................."

# game/status.rpy:9920
translate russia task26_5e4b696e:

    # y "Again... I don't see the problem here."
    y "И снова... Я не вижу в чём проблема."

# game/status.rpy:9921
translate russia task26_20a003e6:

    # s g14 "All we're saying is... that..."
    s g14 "Всё, что мы хотим сказать... это..."

# game/status.rpy:9922
translate russia task26_217959de:

    # c r24 "There might be a way to reduce all of our nanobot levels at the same time."
    c r24 "Возможно, есть способ снизить уровень всех наших наноботов одновременно."

# game/status.rpy:9923
translate russia task26_0e398652:

    # y "How'd we do that? I can only have sex with you one... at... a... ...."
    y "Как мы это сделаем? Я только могу заняться сексом со... всеми... ва-... ...."

# game/status.rpy:9924
translate russia task26_e98ae1e6:

    # y "Wait....! Are you suggesting...?!"
    y "Стоп....! Вы предлагаете...?!"

# game/status.rpy:9925
translate russia task26_89e93800:

    # y "A hot~..."
    y "Горячую~..."

# game/status.rpy:9926
translate russia task26_485e8452:

    # s g1 "{b}*Nods*{/b}"
    s g1 "{b}*Кивает*{/b}"

# game/status.rpy:9927
translate russia task26_8d1aeeda:

    # y "Sweaty~...."
    y "Потную~..."

# game/status.rpy:9928
translate russia task26_cf6babc2:

    # c r22 "He's getting there..."
    c r22 "Он уже близко..."

# game/status.rpy:9929
translate russia task26_b9958ca2:

    # y "Messy~...."
    y "Грязную~...."

# game/status.rpy:9930
translate russia task26_1175d191:

    # a y25 "FOURSOME!"
    a y25 "ГРУППОВУШКУ!"

# game/status.rpy:9931
translate russia task26_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:9932
translate russia task26_1ae99373:

    # y "{b}*Sniff*{/b} You girls are too good to me... What have I done to deserve this?"
    y "{b}*Шмыгает носом*{/b} Вы, девочки, слишком добры ко мне... Что я сделал, чтобы заслужить это?"

# game/status.rpy:9933
translate russia task26_66c4e52e:

    # s g38 "We're doing this for the safety of the mission of course."
    s g38 "Конечно, мы делаем это ради безопасности миссии."

# game/status.rpy:9934
translate russia task26_1c9ac08f:

    # s g19 ".................."
    s g19 ".................."

# game/status.rpy:9936
translate russia task26_cde3f8b1:

    # s g48 "But we're also kind of doing it as a way to say {i}'thank you'{/i}."
    s g48 "Но мы также делаем это, чтобы сказать \"спасибо\"."

# game/status.rpy:9937
translate russia task26_0addf55c:

    # c r3 "For all the trouble you've been through and all the help you've given us."
    c r3 "За все неприятности, через которые ты прошёл, и за всю помощь, которую ты нам оказал."

# game/status.rpy:9938
translate russia task26_f196ceda:

    # a y1 "It's the least we could do!"
    a y1 "Это самое малое, что мы можем сделать!"

# game/status.rpy:9939
translate russia task26_2872fbb4:

    # y "You girls~...."
    y "Девочки вы~...."

# game/status.rpy:9940
translate russia task26_ba53fa31:

    # y "Wait...! Can I make one more request?"
    y "Подождите...! Могу я сделать ещё одну просьбу?"

# game/status.rpy:9941
translate russia task26_c48dc32f:

    # c "Sure."
    c "Конечно."

# game/status.rpy:9942
translate russia task26_6da6070c:

    # y "Can you keep your suits on...?"
    y "Вы можете не снимать свои костюмы...?"

# game/status.rpy:9943
translate russia task26_1c7a52d0:

    # c r18 "Our... suits?"
    c r18 "Наши... костюмы?"

# game/status.rpy:9944
translate russia task26_a0b5ac11:

    # s g16 "But how would you... y'know...?"
    s g16 "Но как ты... ну ты понимаешь...?"

# game/status.rpy:9945
translate russia task26_2ed0f800:

    # y "Oh you'll see~...."
    y "Ооо, вы увидите~..."

# game/status.rpy:9946
translate russia task26_66425e1b:

    # y "For now head back to the base!"
    y "А пока, возвращайтесь на базу!"

# game/status.rpy:9972
translate russia task26_4c1bfe91:

    # "Agent" "One... two... three!"
    "Агент" "Один... два... три!"

# game/status.rpy:9985
translate russia task26_982997fd:

    # "Agents" "AAHHHH!"
    "Агенты" "ААААХХХ!"

# game/status.rpy:10013
translate russia task26_fdb42620:

    # tim "......................."
    tim "......................."

# game/status.rpy:10014
translate russia task26_052bea24:

    # y "TIM! We're taking you in!"
    y "ТИМ! Ты задержан!"

# game/status.rpy:10015
translate russia task26_960ad2e2:

    # tim ".............."
    tim ".............."

# game/status.rpy:10016
translate russia task26_38ef7071:

    # tim "And you are...?"
    tim "А ты...?"

# game/status.rpy:10017
translate russia task26_01335d0b:

    # y "Oh.... right.{w} We haven't met yet."
    y "Оо... точно.{w} Мы ещё не встречались."

# game/status.rpy:10022
translate russia task26_36819588:

    # s "............................"
    s "............................"

# game/status.rpy:10023
translate russia task26_88d58953:

    # tim "...................."
    tim "...................."

# game/status.rpy:10024
translate russia task26_4dff56b6:

    # tim "I should have known..."
    tim "Следовало догадаться..."

# game/status.rpy:10025
translate russia task26_e13a6752:

    # s "It's over Tim. Please come quietly."
    s "Все кончено, Тим. Пожалуйста, сдайся по-хорошему."

# game/status.rpy:10026
translate russia task26_4bc96893:

    # tim "If you knew only half of the things WOOHP gets up to, you would not be my enemy, Sam."
    tim "Если бы ты знала хотя бы половину того, что затевает WOOHP, ты бы не была моим врагом, Сэм."

# game/status.rpy:10027
translate russia task26_1326b34c:

    # s "We know. Britney told us."
    s "Мы знаем. Бритни нам всё рассказала."

# game/status.rpy:10028
translate russia task26_8d48cbd0:

    # s "But you're still a renegade agent and brought Beverly Hills to its knees."
    s "Но ты всё ещё предатель, который поставили Беверли-Хиллз на колени."

# game/status.rpy:10029
translate russia task26_605685c1:

    # tim "......................"
    tim "......................"

# game/status.rpy:10030
translate russia task26_257fcecd:

    # s "It's over Tim. Come with us."
    s "Все кончено, Тим. Пойдём с нами."

# game/status.rpy:10031
translate russia task26_e2e45272:

    # tim "It's not over yet.{w} GLADIS!"
    tim "Это ещё не конец.{w} Г.Л.Э.Д.И.С.!"

# game/status.rpy:10038
translate russia task26_ace50f47:

    # s g29 "!!!"
    s g29 "!!!"

# game/status.rpy:10039
translate russia task26_eae7e959:

    # "An emergency door suddenly closes between Tim and the girls."
    "Между Тимом и девушками внезапно закрывается аварийная дверь."

# game/status.rpy:10040
translate russia task26_26689948:

    # y "Oh come on, we were so close! Get after him!"
    y "Да ладно, мы были так близко! За ним!"

# game/status.rpy:10063
translate russia task26_aea012e4:

    # tim "A little help here GLADIS! I'm running out of nets!"
    tim "Г.Л.Э.Д.И.С., мне не помешает помощь! У меня уже закагчиваются сети!"

# game/status.rpy:10064
translate russia task26_5772b0d1:

    # "........................."
    "........................."

# game/status.rpy:10065
translate russia task26_6c354608:

    # tim "GLADIS...?"
    tim "Г.Л.Э.Д.И.С....?"

# game/status.rpy:10075
translate russia task26_e3962997:

    # a "Nowhere left to run Tim!"
    a "Тебе некуда бежать, Тим!"

# game/status.rpy:10076
translate russia task26_00a834c9:

    # c "We've got you now!"
    c "Теперь ты наш!"

# game/status.rpy:10077
translate russia task26_4467e4e6:

    # tim "................."
    tim "................."

# game/status.rpy:10078
translate russia task26_fdc3be76:

    # "{b}*Click* *Click*{/b}"
    "{b}*Клик* *Клик*{/b}"

# game/status.rpy:10079
translate russia task26_de617ee5:

    # tim "...?"
    tim "...?"

# game/status.rpy:10080
translate russia task26_39ad3c5e:

    # s g56 "What was that...?"
    s g56 "Что это было...?"

# game/status.rpy:10081
translate russia task26_daccd462:

    # "All of you carefully look up and see..."
    "Вы все внимательно смотрите вверх и видите..."

# game/status.rpy:10082
translate russia task26_edf6896b:

    # y "Gun turrets!"
    y "Турели!"

# game/status.rpy:10083
translate russia task26_16018eea:

    # tim "GLADIS! STOP!"
    tim "Г.Л.Э.Д.И.С.! СТОЙ!"

# game/status.rpy:10093
translate russia task26_e35d6c03:

    # y "Clover! Hack into those turrets!"
    y "Клевер! Взломай эти турели!"

# game/status.rpy:10094
translate russia task26_b6815dd6:

    # cM "On it!"
    cM "Уже в процессе!"

# game/status.rpy:10096
translate russia task26_74bcdef7:

    # y "Are you two all right?"
    y "Вы двое в порядке?"

# game/status.rpy:10100
translate russia task26_07d4c473:

    # s g38 "I'm fine. Tim pushed me out of the way just in time."
    s g38 "Я в порядке. Тим вовремя оттолкнул меня."

# game/status.rpy:10101
translate russia task26_336cc14e:

    # s g56 "You... saved me."
    s g56 "Ты... спас меня."

# game/status.rpy:10103
translate russia task26_56d82093:

    # c r17 "Turrets are down. Let's grab this trenchcoat wearing creep and get out of here!"
    c r17 "Турели устранены. Давайте схватим этого гада в плаще и уберёмся отсюда!"

# game/status.rpy:10104
translate russia task26_9201f464:

    # tim "....................."
    tim "....................."

# game/status.rpy:10122
translate russia task26_cccf0ade:

    # tim "........................"
    tim "........................"

# game/status.rpy:10123
translate russia task26_a742beb6:

    # s "......................."
    s "......................."

# game/status.rpy:10124
translate russia task26_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:10127
translate russia task26_851e82f2:

    # y "I expected this to get awkward, so I prepared!"
    y "Так и знал, что это будет неловко, так что я подготовился!"

# game/status.rpy:10128
translate russia task26_6388fc7f:

    # "You pull out a big-fuckoff-knife."
    "Вы достаёте большой нож с надписью \"отъебись\"."

# game/status.rpy:10129
translate russia task26_ea287a4d:

    # s g28 "!!!!"
    s g28 "!!!!"

# game/status.rpy:10130
translate russia task26_dc004adf:

    # s "[playerName]! What's that for?!"
    s "[playerName]! Это для чего?!"

# game/status.rpy:10131
translate russia task26_e26ac582:

    # y "It's a knife... {w}It cuts things."
    y "Это нож... {w}Он режет вещи."

# game/status.rpy:10132
translate russia task26_79f0d040:

    # y "It's gonna cut this tension!{w} Get it?"
    y "Он срежет это слой напряжение!{w} Поняли?"

# game/status.rpy:10133
translate russia task26_d78bba38:

    # s g38 "{b}*Sighs*{/b} Anyways, where were we..."
    s g38 "{b}*Вздох*{/b} В любом случае, на чём мы остановились..."

# game/status.rpy:10135
translate russia task26_29a0f1ea:

    # y "......................................"
    y "......................................"

# game/status.rpy:10136
translate russia task26_795c72ca:

    # y "Chirp, chirp, chirp...."
    y "Чирк, чирк, чирк...."

# game/status.rpy:10137
translate russia task26_5d5c85f7:

    # y "Get it? It's like cricket nois-..."
    y "Поняли? Это типа звук сверчка..."

# game/status.rpy:10138
translate russia task26_dc0232bb:

    # s g53 "Why the hell did you save me from those turrets?!"
    s g53 "Какого чёрта ты спас меня от этих турелей?!"

# game/status.rpy:10139
translate russia task26_13748cd7:

    # tim "I don't know."
    tim "Я не знаю."

# game/status.rpy:10140
translate russia task26_6035c8da:

    # s "We're enemies! Why are you pulling shit like this?!"
    s "Мы враги! Почему ты делаешь такую чушь?!"

# game/status.rpy:10141
translate russia task26_6a2c95a4:

    # tim "You'd rather I have you die?"
    tim "Ты бы предпочла умереть?"

# game/status.rpy:10142
translate russia task26_40a37860:

    # s g52 "It'll certainly be less confusing!"
    s g52 "Так былобы меньше путаницы!"

# game/status.rpy:10143
translate russia task26_0ba05448:

    # s g38 "Tim. What we are... what we had... it ended when you tried killing Jerry all that time ago."
    s g38 "Тим. Мы... что у нас было... всё закончилось, тогда, когда ты пытался убить Джерри."

# game/status.rpy:10144
translate russia task26_bf933d54:

    # s g53 "Stop pretending to care about me!"
    s g53 "Хватит притворяться, что я тебе не безразлична!"

# game/status.rpy:10145
translate russia task26_3c7732e9:

    # tim "............................"
    tim "............................"

# game/status.rpy:10146
translate russia task26_8a96e20f:

    # s g52 "We're enemies Tim Scam. You've proven that over and over again."
    s g52 "Мы враги Тим Скам. Ты доказывал это снова и снова."

# game/status.rpy:10147
translate russia task26_fd4fee7f:

    # s "You expect me to suddenly forgive you, just because you saved my life?"
    s "Ты думаешь, что я вдруг прощу тебя, только потому что ты спас мне жизнь?"

# game/status.rpy:10148
translate russia task26_8d59a65e:

    # tim "No. I expect you to try and see things from my point of view for once."
    tim "Нет. Я хочу, что бы ты хоть раз попыталась взглянуть на вещи, с моей точки зрения."

# game/status.rpy:10149
translate russia task26_6d02bfb3:

    # tim "Jerry is dangerous. WOOHP is dangerous. This world would be better off without them."
    tim "Джерри опасен. WOOHP опасна. Этот мир был бы лучше без них."

# game/status.rpy:10150
translate russia task26_8e94a5f9:

    # s g53 "By setting Beverly Hills on fire...?!"
    s g53 "Поджигая Беверли-Хиллз...?!"

# game/status.rpy:10151
translate russia task26_1c032b9b:

    # tim "Look around you! The gangs are gone. WOOHP public image is ruined. Buildings can be rebuild, shops can be restocked and order can be restored."
    tim "Оглянись вокруг! Банды исчезли. Общественный имидж WOOHP разрушен. Здания могут быть восстановлены, магазины пополнены, порядок восстановлен."

# game/status.rpy:10152
translate russia task26_4739620c:

    # tim "I am trying to show you a world without a tyrant at the top!"
    tim "Я пытаюсь показать тебе мир без тирана на вершине!"

# game/status.rpy:10153
translate russia task26_76a370b4:

    # s "Jerry is not a tyrant!"
    s "Джерри не тиран!"

# game/status.rpy:10154
translate russia task26_392a2aa2:

    # tim "He's no a saint either!"
    tim "Он и не святой!"

# game/status.rpy:10155
translate russia task26_5a07bc94:

    # s g54 "............................."
    s g54 "............................."

# game/status.rpy:10156
translate russia task26_ecfb0991:

    # tim "................................."
    tim "................................."

# game/status.rpy:10157
translate russia task26_9cb97b48_2:

    # y "........................"
    y "........................"

# game/status.rpy:10158
translate russia task26_82f52de5:

    # y "I hate to interrupt your little lovers quarrel, but we have a headquarters to liberate."
    y "Мне не хотелось бы прерывать вашу маленькую любовную ссору, но нам нужно освободить штаб."

# game/status.rpy:10159
translate russia task26_4bf82eb8:

    # tim "........."
    tim "........."

# game/status.rpy:10160
translate russia task26_d174f440:

    # y "Tim, your rebellion is over. We just need to get to GLADIS."
    y "Тим, твоё восстание окончено. Нам осталось только добраться до Г.Л.Э.Д.И.С."

# game/status.rpy:10161
translate russia task26_58f283a8:

    # y "We need to know what's waiting for us on that top level."
    y "Мы должны знать, что ждёт нас на верхнем этаже."

# game/status.rpy:10162
translate russia task26_fdb42620_1:

    # tim "......................."
    tim "......................."

# game/status.rpy:10163
translate russia task26_619e2455:

    # tim "Nothing."
    tim "Ничего."

# game/status.rpy:10164
translate russia task26_3df5ef7c:

    # y "Nothing?"
    y "Ничего?"

# game/status.rpy:10165
translate russia task26_57bf050a:

    # tim "Yeah... Nothing."
    tim "Да... Ничего."

# game/status.rpy:10166
translate russia task26_c3578178:

    # y "No more super spies in colorful outfits? No more death traps? No more surprise nanobot injections?"
    y "Больше никаких супершпионов в ярких нарядах? Никаких смертельных ловушек? Никаких неожиданных инъекций наноботов?"

# game/status.rpy:10167
translate russia task26_5e8b86ac:

    # tim "She's only a robot and we've already thrown everything we had at you."
    tim "Она всего лишь робот, а мы уже бросили на вас всё, что у нас было."

# game/status.rpy:10168
translate russia task26_5f60ba9a_1:

    # y ".............................."
    y ".............................."

# game/status.rpy:10169
translate russia task26_d4dcbe91:

    # tim "I don't know who you are, but freeing Jerry is a mistake."
    tim "Я не знаю, кто ты, но освобождение Джерри - это ошибка."

# game/status.rpy:10170
translate russia task26_f000d726:

    # tim "Things will just go back to how they used to be. Prisoners are put away without trial. Politicians will continue to be held in WOOHP iron grip and you will have taken away the only people who could stand against him."
    tim "Всё вернётся к тому, что было раньше. Заключённых будут сажать без суда и следствия. Политики по-прежнему будут находиться в железной хватке WOOHP, а вы лишили его единственных людей, которые могли бы этому противостоять."

# game/status.rpy:10171
translate russia task26_2849703c:

    # s g12 "Don't listen to his garbage, [playerName]. Once we free Jerry, we can finally begin to bring some sanity to this world."
    s g12 "Не слушай его бредни, [playerName]. Когда мы освободим Джерри, мы сможем, наконец, привнести в этот мир немного здравомыслия."

# game/status.rpy:10172
translate russia task26_36331e0d:

    # s "You're insane Tim, to think all this violence and chaos is for the betterment of the world."
    s "Ты безумец, Тим, если думаешь, что всё это насилие и хаос приведёт к улучшению мира."

# game/status.rpy:10174
translate russia task26_c5f27a34:

    # tim "........................."
    tim "........................."

# game/status.rpy:10182
translate russia task26_1cfdf52e:

    # c r18 "And?"
    c r18 "И?"

# game/status.rpy:10183
translate russia task26_2db97197:

    # y "Seems the way is clear. We only need to capture the top floor and free the WOOHP HQ."
    y "Похоже путь свободен. Нам осталось захватить только верхний этаж, чтобы освободить штаб-квартиру WOOHP."

# game/status.rpy:10184
translate russia task26_09a45444:

    # c r42 "I can't believe it's finally over..."
    c r42 "Не могу поверить, что всё наконец-то закончилось..."

# game/status.rpy:10185
translate russia task26_f2ec320d:

    # brit b31 "It's not over yet. [playerName] we're with you. Once you launch the final attack on the HQ, we'll finally be able to restore order to this city."
    brit b31 "Это ещё не конец. [playerName] мы с тобой. Как только ты начнёшь последнюю атаку на штаб-квартиру, мы наконец-то сможем восстановить порядок в этом городе."

# game/status.rpy:10212
translate russia task26_a97a87de:

    # s "GLADIS! We've come to stop you!"
    s "Г.Л.Э.Д.И.С.! Мы пришли, чтобы остановить тебя!"

# game/status.rpy:10213
translate russia task26_6b78728f:

    # c r8 "Yeah what's the big idea?! You tried to kill Sam!"
    c r8 "Да, что ты делаешь?! Ты пыталась убить Сэм!"

# game/status.rpy:10214
translate russia task26_ca4a02a4:

    # a y14 "And the city is on fire! So many innocent people got hurt!"
    a y14 "Весь город в огне! Столько невинных людей пострадало!"

# game/status.rpy:10220
translate russia task26_7a52c1f5:

    # GLA ".........................................................."
    GLA ".........................................................."

# game/status.rpy:10221
translate russia task26_e62f8355:

    # y "Don't give us the silent treatment, missy!"
    y "Только не молчите, мисс!"

# game/status.rpy:10222
translate russia task26_7a52c1f5_1:

    # GLA ".........................................................."
    GLA ".........................................................."

# game/status.rpy:10223
translate russia task26_5bc6f522:

    # sM "GLADIS....?"
    sM "Г.Л.Э.Д.И.С....?"

# game/status.rpy:10224
translate russia task26_5d78bf8e:

    # y "Erm..."
    y "Эмм..."

# game/status.rpy:10225
translate russia task26_14610732:

    # y "Anyone home...?"
    y "Есть кто дома...?"

# game/status.rpy:10226
translate russia task26_d6e9e54d:

    # "You walk up to the robot and poke it with your finger."
    "Вы подходите к роботу и тыкаете в него пальцем."

# game/status.rpy:10227
translate russia task26_b42f1535:

    # y "{b}*Poke* *Poke*{/b} She's not responding...."
    y "{b}*Тык* *Тык*{/b} Она не отвечает...."

# game/status.rpy:10228
translate russia task26_115454e2:

    # cM "But... how?!"
    cM "Но... как?!"

# game/status.rpy:10229
translate russia task26_069db9d0:

    # "???" "It's quite simple really."
    "???" "На самом деле всё довольно просто."

# game/status.rpy:10233
translate russia task26_00c0a5c6:

    # "Girls" "Jerry!"
    "Девушки" "Джерри!"

# game/status.rpy:10234
translate russia task26_54eaa9d3:

    # y "Jerry!"
    y "Джерри!"

# game/status.rpy:10236
translate russia task26_692be6fa:

    # y "............"
    y "............"

# game/status.rpy:10237
translate russia task26_fc738d21:

    # y "Jerry?{w} Weren't you being held hostage?"
    y "Джерри?{w} Разве вас не держат в заложниках?"

# game/status.rpy:10238
translate russia task26_bdfbd5c6:

    # jerry "Oh I was, but my captors underestimated my love for the Queen."
    jerry "О, держали, но мои похитители недооценили мою любовь к королеве."

# game/status.rpy:10242
translate russia task26_f6dd197b:

    # "Torturer" "♫♪ God save our gracious Queen, long live our noble Queen, God save the Queen! ♫♪"
    "Пыточный аппарат" "♫♪ Боже, храни нашу милостивую королеву, да здравствует наша благородная королева, Боже, храни королеву! ♫♪"

# game/status.rpy:10246
translate russia task26_ff4e8099:

    # jerry "I just needed a distraction, so when you lot came in, I managed to cut off power to GLADIS."
    jerry "Мне просто нужно было её отвлечь поэтому когда вы пришли, мне удалось отключить питание Г.Л.Э.Д.И.С."

# game/status.rpy:10247
translate russia task26_97493d2a:

    # jerry "I am so happy to see you girls again!"
    jerry "Я так рада снова вас видеть, девочки!"

# game/status.rpy:10248
translate russia task26_9cb97b48_3:

    # y "........................"
    y "........................"

# game/status.rpy:10255
translate russia task26_69f2dd4f:

    # a "We're happy to see you again, Jer!"
    a "И мы рады снова тебя видеть, Джерри!"

# game/status.rpy:10256
translate russia task26_8cc4b1db:

    # c r56 "We were worried sick!"
    c r56 "Мы очень волновались!"

# game/status.rpy:10257
translate russia task26_aa49ff12:

    # s "We spend months planning your escape!"
    s "Мы потратили месяцы на планирование твоего освобаждения!"

# game/status.rpy:10258
translate russia task26_51f18613:

    # y "Yeah... this almost feels a little anti-climactic."
    y "Да... даже ощущается небольшое разочарование."

# game/status.rpy:10259
translate russia task26_bd7a95f2:

    # jerry "..........................."
    jerry "..........................."

# game/status.rpy:10260
translate russia task26_4992a4b9:

    # jerry "And... you are?"
    jerry "Аа... ты?"

# game/status.rpy:10261
translate russia task26_490bdaec:

    # y "Oh right... erm..."
    y "Ах да... эмм..."

# game/status.rpy:10262
translate russia task26_a2af5db3:

    # y "I'm sorta... an inmate."
    y "Я вроде как... заключённый."

# game/status.rpy:10263
translate russia task26_2c9379d1:

    # jerry "Is that so...?"
    jerry "Это так...?"

# game/status.rpy:10264
translate russia task26_ed9cbb6e:

    # c r55 "It's okay Jer, he's one of the good guys. He helped us free you."
    c r55 "Всё в порядке, Джерии, он на нашей стороне. Он помог нам освободить тебя."

# game/status.rpy:10265
translate russia task26_075558d6:

    # s g56 "Yeah, he freed us from our nanobot control and helped fight our way to the top of WOOHP HQ!"
    s g56 "Да, он освободил нас от контроля наноботов и помог пробиться на вершину штаб-квартиры WOOHP!"

# game/status.rpy:10266
translate russia task26_0f23339b:

    # jerry "Interesting..."
    jerry "Интересно..."

# game/status.rpy:10267
translate russia task26_cad11999:

    # y "I've been hoping to go on the straight and narrow! So I'd like to apply for a job!"
    y "Я надеялся и дальше идти по хорошему пути! Поэтому хотел бы подать заявление о приёме на работу!"

# game/status.rpy:10268
translate russia task26_9a5bc900:

    # y "My resume includes working as a zoo keeper, a daycare worker and a prison guard!{w} And... y'know. The entire liberation of Beverly Hills and WOOHP."
    y "У меня есть опыт работы в качестве смотрителя зоопарка, работника детского сада и тюремного охранника!{w}А, ну и... знаете ли. Полное освобождение Беверли-Хиллз и WOOHP."

# game/status.rpy:10269
translate russia task26_8690a3bb:

    # jerry "That's an impressive resume indeed. Let's see... I think i've got a contract for you in my desk."
    jerry "Действительно впечатляющее резюме. Так, посмотрим... кажется, у меня в столе лежит для вас контракт."

# game/status.rpy:10271
translate russia task26_e3eabdb5:

    # "Jerry turns his back to you and walks over to his desk as you glance at the girls."
    "Джерри поворачивается к вам спиной и подходит к своему столу. Вы смотрите на девушек."

# game/status.rpy:10272
translate russia task26_ab47e137:

    # s g1 "You deserve it, [playerName]."
    s g1 "Ты этого заслужил, [playerName]."

# game/status.rpy:10273
translate russia task26_9bfd36c7:

    # c r1 "Yeah, we wouldn't have gotten this far without you."
    c r1 "Да, без тебя у нас ничего бы не получилось."

# game/status.rpy:10274
translate russia task26_31cba42d:

    # a y1 "You could be a spy! Or a security manager! {w}Or even a janitor!"
    a y1 "Ты можешь быть шпионом! Или менеджером по безопасности! {w}Или даже дворником!"

# game/status.rpy:10275
translate russia task26_92fa2786:

    # y "Yeah..."
    y "Ага..."

# game/status.rpy:10276
translate russia task26_d5395540:

    # "You carefully throw a glance around the room."
    "Вы с осторожностью осматриваете комнату."

# game/status.rpy:10277
translate russia task26_5f60ba9a_2:

    # y ".............................."
    y ".............................."

# game/status.rpy:10278
translate russia task26_f1e0dec6:

    # jerry "{b}*Ahem*{/b} So I take you you've already seen the entire facility?"
    jerry "{b}*Кхм*{/b} Я так понимаю, вы уже осмотрели весь объект?"

# game/status.rpy:10279
translate russia task26_ac7e48cd:

    # y "Yup. Top to bottom."
    y "Ага. Сверху донизу."

# game/status.rpy:10280
translate russia task26_5c6f89a7:

    # jerry "I see..."
    jerry "Понятно..."

# game/status.rpy:10281
translate russia task26_4a85939c:

    # "Jerry bends over his desk and opens a cupboard."
    "Джерри наклоняется над своим столом и открывает шкафчик."

# game/status.rpy:10282
translate russia task26_e2c7d63e:

    # y "I saw where you make the gadgets, the hidden civilian surfailance, the prisons, the propaganda center..."
    y "Я видел, где вы производите гаджеты, пункт наблюдения за людьми, тюрьмы, центр пропаганды..."

# game/status.rpy:10283
translate russia task26_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:10284
translate russia task26_68d9750f:

    # y "On second thought. I probably didn't have clearance to see those..."
    y "Если подумать. У меня, наверное, не было допуска на посещение..."

# game/status.rpy:10285
translate russia task26_5b5293da:

    # jerry "Correct."
    jerry "Правильно."

# game/status.rpy:10286
translate russia task26_11e23fc3:

    # y "Oh well. I guess you don't have a choice but to hire me now!"
    y "Ну что ж. Думаю, тогда у вас нет выбора, кроме как нанять меня!"

# game/status.rpy:10287
translate russia task26_5249e0e1:

    # jerry "No choice indeed..."
    jerry "Действительно, выбора нет..."

# game/status.rpy:10292
translate russia task26_fdc3be76_1:

    # "{b}*Click* *Click*{/b}"
    "{b}*Клик* *Клик*{/b}"

# game/status.rpy:10307
translate russia task26_4b5c48bf:

    # s "[playerName]!"
    s "[playerName]!"

# game/status.rpy:10308
translate russia task26_3d04b6be:

    # c "{size=-8}Jerry stop!{/size}"
    c "{size=-8}Джерри остановись!{/size}"

# game/status.rpy:10309
translate russia task26_8b7fd0bc:

    # a "{size=-14}Oh no [playerName]...!{/size}"
    a "{size=-14}О нет [playerName]...!{/size}"

# game/status.rpy:10326
translate russia task26_43663ec2:

    # O5O "I'M FINALLY AN IMPORTANT CHARACTER!"
    O5O "НАКОНЕЦ-ТО Я СТАЛ ВАЖНЫМ ПЕРСОНАЖЕМ!"

# game/status.rpy:10327
translate russia task26_9a72dc9a:

    # brit "Shhh! Not so loud! We're almost there..."
    brit "Тссс! Не так громко! Мы почти на месте..."

# game/status.rpy:10330
translate russia task26_c58bdbe7:

    # "He's waking up...!"
    "Он просыпается...!"

# game/status.rpy:10340
translate russia task26_3cdd6dca:

    # y "Uuuuuuuugh......"
    y "Ууууугх...."

# game/status.rpy:10341
translate russia task26_7445183b:

    # y "What.... happened?"
    y "Что.... случилось?"

# game/status.rpy:10342
translate russia task26_7bf8378e:

    # brit "You were shot. Try not to move too much."
    brit "В тебя стреляли. Старайся не двигаться слишком много."

# game/status.rpy:10343
translate russia task26_59bdbcc0:

    # y "Shot....?{w} SHOT!{w} I was shot by that old geezer!"
    y "Стреляли....?{w} СТРЕЛЯЛИ!{w} В меня стрелял этот старый хрыч!"

# game/status.rpy:10344
translate russia task26_1106e23b:

    # brit "Calm down, you don't want your wounds to open again."
    brit "Успокойся, ты же не хочешь, чтобы твои раны снова открылись."

# game/status.rpy:10345
translate russia task26_e163fbe8:

    # O5O "THERE WAS SO MUCH BLOOD! YOU HAVE A LOT OF BLOOD MY FRIEND!"
    O5O "ТАМ БЫЛО ТАК МНОГО КРОВИ! У ТЕБЯ ОЧЕНЬ МНОГО КРОВИ, МОЙ ДРУГ!"

# game/status.rpy:10346
translate russia task26_da4d243c:

    # y "But... why?! I got him back his stupid organization! Why would he turn on me like that?!"
    y "Но... Почему?! Я вернул ему его дурацкую организацию! Почему он так со мной?!"

# game/status.rpy:10347
translate russia task26_22e1a399:

    # brit "He didn't. {w}GLADIS did."
    brit "Это не он. {w}Это была Г.Л.Э.Д.И.С."

# game/status.rpy:10348
translate russia task26_88dc1dd8:

    # brit "Jerry is being controlled by nanobots. We walked right into a trap."
    brit "Джерри контролируют наноботы. Мы попали прямо в ловушку."

# game/status.rpy:10349
translate russia task26_44db51c4:

    # y "But... GLADIS was offline...?"
    y "Но... Г.Л.Э.Д.И.С. была отключена...? "

# game/status.rpy:10350
translate russia task26_e2d7556f:

    # brit "And came back online after you were shot. It was all a ruse. She is very much still active and took control of Jerry and the spies."
    brit "Она включилась после того, как тебя подстрелили. Всё это было уловкой. Она по-прежнему активна и взяла под контроль Джерри и шпионок."

# game/status.rpy:10351
translate russia task26_d098dc1d:

    # y "The girls! What happened?!"
    y "Девочки! Что случилось?!"

# game/status.rpy:10352
translate russia task26_5fb5e964:

    # O5O "THEY'RE STILL WITH WOOHP."
    O5O "ОНИ ВСЁ ЕЩЁ С WOOHP."

# game/status.rpy:10353
translate russia task26_074a737e:

    # brit "The update hit them hard and they've all been controlled again."
    brit "Обновление сильно ударило по ним, и все они снова попали под контроль."

# game/status.rpy:10354
translate russia task26_1972a249:

    # y "Oh no... I led them straight into that trap..."
    y "О нет... Я привёл их прямо в ловушку..."

# game/status.rpy:10355
translate russia task26_bb00babd:

    # y "Well we're still alive! I'm sure with a can-do attitute and some elbow grease we can-.."
    y "Ну, мы всё ещё живы! Я уверен, что при наличии желания и некоторого количества смазки мы сможем-..."

# game/status.rpy:10356
translate russia task26_f73da0ce:

    # O5O "IT GETS WORSE!"
    O5O "СТАНОВИТСЯ ЕЩЁ ХУЖЕ!"

# game/status.rpy:10357
translate russia task26_88054233:

    # y "Ooofcourse it does."
    y "Конечно."

# game/status.rpy:10358
translate russia task26_15c7de3c:

    # brit "GLADIS is preparing to launch WOOHP missle arsenal."
    brit "Г.Л.Э.Д.И.С. готовится к запуску ракетного арсенала WOOHP."

# game/status.rpy:10359
translate russia task26_c86e5a9b:

    # y "At...?"
    y "Куда...?"

# game/status.rpy:10360
translate russia task26_526080a3:

    # brit "At... at well... everything. The entire city."
    brit "На... на ну... всё. Весь город."

# game/status.rpy:10361
translate russia task26_6f362d57_1:

    # y "..............."
    y "..............."

# game/status.rpy:10362
translate russia task26_55859704:

    # y "Two questions."
    y "Два вопроса."

# game/status.rpy:10363
translate russia task26_d479c0b4:

    # y "One. Why does WOOHP have enough missles to lay waste to an entire city?"
    y "1. Почему у WOOHP достаточно ракет, чтобы уничтожить целый город?"

# game/status.rpy:10364
translate russia task26_c4d6919f:

    # y "Two. I thought GLADIS only wanted to get rid of WOOHP? Not the entire city?"
    y "2. Я думал, что Г.Л.Э.Д.И.С. хотела уничтожить только WOOHP? Не весь город?"

# game/status.rpy:10365
translate russia task26_06d78378:

    # brit "True, but..."
    brit "Верно, но..."

# game/status.rpy:10366
translate russia task26_ec2d927c:

    # O5O "WOOHP HAS TUNNELS ALL OVER THE CITY!"
    O5O "У WOOHP ЕСТЬ ТУННЕЛИ ПО ВСЕМУ ГОРОДУ!"

# game/status.rpy:10367
translate russia task26_0db6de4e:

    # y "Tunnels?{w} Wait... the emergency tunnel network?"
    y "Туннели?{w} Подожди... сеть аварийных туннелей?"

# game/status.rpy:10368
translate russia task26_2469b38b:

    # O5O "EXACTLY!"
    O5O "ТОЧНО!"

# game/status.rpy:10369
translate russia task26_932fca1b:

    # brit "For some reason, she considers all those tunnels to be part of WOOHP and after we broke into her office, she now plans to destroy every hint of the organization. No matter the cost."
    brit "По какой-то причине она считает все эти туннели частью WOOHP. После того как мы проникли в её офис, она теперь планирует уничтожить любой намёк на эту организацию. Независимо от того что для этого потребуется."

# game/status.rpy:10370
translate russia task26_a951b5cc:

    # y "But that's crazy!"
    y "Но это же безумие!"

# game/status.rpy:10371
translate russia task26_9cb97b48_4:

    # y "........................"
    y "........................"

# game/status.rpy:10373
translate russia task26_7ef70c1f:

    # y "What about Kim?"
    y "Что насчёт Ким?"

# game/status.rpy:10374
translate russia task26_c0890f0f:

    # brit "She's fine. She's hiding out nearby."
    brit "Она в порядке. Она прячется неподалёку."

# game/status.rpy:10376
translate russia task26_c93dedfe:

    # y "What about Silva?"
    y "Что насчёт Сильвы?"

# game/status.rpy:10377
translate russia task26_14556fb8:

    # brit "Silva is being hunted by agents, but she is hiding out nearby."
    brit "За Сильвой охотятся агенты, но она прячется неподалеку."

# game/status.rpy:10378
translate russia task26_8fd95934:

    # brit "What do you think we should do...?"
    brit "Что, по-твоему, мы должны сделать...?"

# game/status.rpy:10379
translate russia task26_61960849:

    # y "......................................."
    y "......................................."

# game/status.rpy:10384
translate russia endgameMenu1_3ab49b11:

    # y "We can't let her win! What do we have left? Can we-...{w} argh...!"
    y "Мы не можем позволить ей победить! Что у нас осталось? Может мы...{w} Аргх...!"

# game/status.rpy:10385
translate russia endgameMenu1_44c9da65:

    # brit "Easy there! You're still wounded."
    brit "Полегче! Ты всё ещё ранен."

# game/status.rpy:10386
translate russia endgameMenu1_da7c7b44:

    # y "Britney we got so close. No way am I giving up now."
    y "Бритни, мы были так близки. Я ни за что не сдамся."

# game/status.rpy:10387
translate russia endgameMenu1_9daa8006:

    # O5O "AND YOU'VE GOT ME!"
    O5O "А ЕЩЁ У ВАС ТЕПЕРЬ ЕСТЬ Я!"

# game/status.rpy:10389
translate russia endgameMenu1_1b3b7ce5:

    # y "Whelp, it was nice while it lasted."
    y "Ну что ж, всё это было довольно прикольно."

# game/status.rpy:10390
translate russia endgameMenu1_f349c26b:

    # brit "You're... giving up?"
    brit "Ты... сдаёшься?"

# game/status.rpy:10391
translate russia endgameMenu1_08cefe50:

    # y "Yup. It seems like the right time for me to move on. The secret agent lifestyle was never for me."
    y "Да. Думаю сейчас самое время двигаться дальше. Жизни секретного агента никогда меня не привлекала."

# game/status.rpy:10392
translate russia endgameMenu1_3cb43e73:

    # brit "I guess I can't blame you..."
    brit "Думаю, я не могу винить тебя..."

# game/status.rpy:10393
translate russia endgameMenu1_4688c390:

    # brit "You're still wounded, but I think I can smuggle you out of the city."
    brit "Ты всё ещё ранен, но я думаю, что смогу тайно вывезти тебя из города."

# game/status.rpy:10394
translate russia endgameMenu1_05824b28:

    # brit "Are you sure you want to leave it all behind...?"
    brit "Ты уверен, что хочешь оставить все это позади...?"

# game/status.rpy:10397
translate russia endgameMenu1_b236c0a3:

    # brit "All right, then let's make preparations for getting us out of here."
    brit "Хорошо, тогда давайте займёмся подготовкой к тому, чтобы вытащить нас отсюда."

# game/status.rpy:10400
translate russia endgameMenu1_d357e4e3:

    # "And so you decided to leave Beverly Hills to its fate."
    "И вот так вы решили оставить Беверли-Хиллз на произвол судьбы."

# game/status.rpy:10401
translate russia endgameMenu1_14f39a71:

    # "Soon after you evacuated, the US Military moved in and GLADIS launched her missle arsenal. Leaving the city and WOOHP in ruins."
    "Вскоре после того, как вы эвакуировались, в Беверли-Хиллз вошли военные США. Г.Л.Э.Д.И.С. запустила ракеты, оставив город и WOOHP в руинах."

# game/status.rpy:10408
translate russia endgameMenu1_2bebb25c:

    # y "Did Napoleon give up when he was banished by the Spanish inquisition?!"
    y "Сдался ли Наполеон, когда его изгнала испанская инквизиция?!"

# game/status.rpy:10409
translate russia endgameMenu1_5b140f64:

    # brit ".....{w} Wait what...?"
    brit ".....{w} Стоп что...?"

# game/status.rpy:10410
translate russia endgameMenu1_efc1432e:

    # y "Did the Mongols give in when the British army burned their villages?!"
    y "Сдались ли Монголы, когда британская армия сожгла их деревни?!"

# game/status.rpy:10411
translate russia endgameMenu1_679f5078:

    # brit "What are you even say-...?"
    brit "Что ты вообще несёшь-...?"

# game/status.rpy:10412
translate russia endgameMenu1_2bb4dfc6:

    # y "Did the three-hundred Spartans flee when put up against the countless numbers of the Zulu warriors?!"
    y "Разве триста спартанцев обратились в бегство, когда их выставили против бесчисленной армии Зулу?!"

# game/status.rpy:10413
translate russia endgameMenu1_68a4271f:

    # brit "I think you're getting your history mixed u-..."
    brit "Думаю, ты немного запутался в истори-..."

# game/status.rpy:10414
translate russia endgameMenu1_619915d9:

    # y "When Albert Einstein said: ''WE WILL FIGHT THEM ON THE BEACHES!'' He never gave up! He never gave in!"
    y "Когда Альберт Эйнштейн сказал: \"МЫ БУДЕМ СРАЖАТЬСЯ С НИМИ НА ПЛЯЖАХ!\" Он никогда не сдавался!"

# game/status.rpy:10415
translate russia endgameMenu1_3c2b9805:

    # y "And I tell you now...!{w} I will never surrender! I have worked hard to get this close and I'll be damned if a mechanical cyclopse is getting in my way!"
    y "И послушай что я тебе сейчас скажу...!{w} Я никогда не сдамся! Я много работал, чтобы подобраться так близко, и будь я проклят, если механический циклоп встанет на моём пути!"

# game/status.rpy:10417
translate russia endgameMenu1_6b90ab04:

    # y "I think the first matter of business is taking back the Milkshake Bar."
    y "Я думаю, что первым делом нужно вернуть мой бар."

# game/status.rpy:10418
translate russia endgameMenu1_6b19b020:

    # y "We'll have to contact our potential allies in the area and most importantly... get in touch with Mathias."
    y "Мы должны связаться с нашими потенциальными союзниками в этом районе и, самое главное... связаться с Матиасом."

# game/status.rpy:10419
translate russia endgameMenu1_88e6612e:

    # brit "The hacker kid? {w}What are you planning?"
    brit "Этот хакер? {w}Что ты задумал?"

# game/status.rpy:10420
translate russia endgameMenu1_1e17c790:

    # y "Mathias can he-...{w} ugh...~"
    y "Матиас может-...{w} угх...~"

# game/status.rpy:10421
translate russia endgameMenu1_7160a1c6:

    # O5O "EASY THERE FELLOW AGENT! YOU'VE BEEN SHOT REMEMBER?"
    O5O "полегче, товарищ! тебя подстрелили, помнишь?"

# game/status.rpy:10422
translate russia endgameMenu1_b78a3734:

    # O5O "LET US HANDLE IT! WE'LL TRACK DOWN YOUR MISSING ALLIES FOR YOU!"
    O5O "ПОЗВОЛЬ НАМ С ЭТИМ РАЗОБРАТЬСЯ! МЫ РАЗЫЩЕМ ВСЕХ ПРОПАВШИХ СОЮЗНИКОВ!"

# game/status.rpy:10423
translate russia endgameMenu1_d3a59cb5:

    # y "............................."
    y "............................."

# game/status.rpy:10425
translate russia endgameMenu1_af60ac90:

    # y "Fine... let's go check up on the situation outside."
    y "Прекрасно... пойдёмте проверим, как обстоят дела снаружи."

# game/status.rpy:10431
translate russia endgameMenu1_f2bcbc48:

    # "DAYS UNTIL MISSILE LAUNCH: [daysTillMissle]"
    "ДНЕЙ ДО ЗАПУСКА РАКЕТ: [daysTillMissle]"

# game/status.rpy:10434
translate russia endgameMenu1_81de01ed:

    # y "That's... pretty bad."
    y "Это... довольно плохо."

# game/status.rpy:10435
translate russia endgameMenu1_eaa2620b:

    # brit "GLADIS has taken over everything..."
    brit "Г.Л.Э.Д.И.С. захватила всё..."

# game/status.rpy:10436
translate russia endgameMenu1_70428556:

    # O5O "BEVERLY HILLS IS BACK TO BEING A WARZONE!"
    O5O "БЕВЕРЛИ-ХИЛЛЗ СНОВА СТАЛ ЗОНОЙ БОЕВЫХ ДЕЙСТВИЙ!"

# game/status.rpy:10437
translate russia endgameMenu1_872fbb2c:

    # y "Okay, there isn't alot of time. You two go scout the city and try to bring back allies that can help us."
    y "Ладно, времени не так много. Вы двое пойдёте разведать город и попытаетесь привести союзников, которые могут нам помочь."

# game/status.rpy:10446
translate russia endgameMenu1_ebca3dbb:

    # y "Mathias!"
    y "Матиас!"

# game/status.rpy:10447
translate russia endgameMenu1_eeebcc1e:

    # mat "My dude!"
    mat "Чувак!"

# game/status.rpy:10448
translate russia endgameMenu1_022bfbff:

    # mat "Wait... did you get shot?!"
    mat "Подожди... Тебя подстрелили?!"

# game/status.rpy:10449
translate russia endgameMenu1_a32e8a84:

    # y "Not important. The city is about to be blown to kingdom come!"
    y "Неважно. Город будет взорван до основания!"

# game/status.rpy:10450
translate russia endgameMenu1_900b069f:

    # mat "Narly."
    mat "Ебать."

# game/status.rpy:10451
translate russia endgameMenu1_90991ba8:

    # y "And you're going to help me stop it!"
    y "И ты поможешь мне это остановить!"

# game/status.rpy:10452
translate russia endgameMenu1_ecbf2311:

    # mat "My dude. Hacking into a database is one thing. Hacking into a missile system is on a whole nother level!"
    mat "Чувак. Взломать базу данных - это одно. Взлом ракетной системы - это совсем другой уровень!"

# game/status.rpy:10453
translate russia endgameMenu1_a61e16ca:

    # y "That's why you're not hacking into the missile system.{w} I need you to hack into cells beneath the milkshake bar."
    y "Именно по этому ты не будешь взламывать ракетную систему.{w} Мне нужно, чтобы ты взломал камеры под баром."

# game/status.rpy:10454
translate russia endgameMenu1_925f8871:

    # mat "The cells...?"
    mat "Камеры...?"

# game/status.rpy:10455
translate russia endgameMenu1_0d8c19f1:

    # y "Yes. Can you open the cell doors remotely?"
    y "Да. Ты сможешь открыть двери камер дистанционно?"

# game/status.rpy:10456
translate russia endgameMenu1_ec8d064a:

    # mat "But that will release all the prisoners!"
    mat "Но это освободит всех заключённых!"

# game/status.rpy:10457
translate russia endgameMenu1_86c12410:

    # y "Exactly."
    y "Именно."

# game/status.rpy:10458
translate russia endgameMenu1_22bf1846:

    # mat "...................."
    mat "...................."

# game/status.rpy:10459
translate russia endgameMenu1_31cbb445:

    # mat "I guess I could, but it sounds pretty crazy."
    mat "Наверное, я смогу, но это звучит довольно безумно."

# game/status.rpy:10462
translate russia endgameMenu1_b5790920:

    # y "Crazy or not, do you think you can do it?"
    y "Безумно или нет, как ты думаешь, ты сможешь это сделать?"

# game/status.rpy:10463
translate russia endgameMenu1_96c48322:

    # mat "I guess so. So why am I doing this...?"
    mat "Думаю, да. Так зачем я это делаю...?"

# game/status.rpy:10465
translate russia endgameMenu1_9aacefea:

    # y "You're not the first one to call me that, but it's worked out so far!"
    y "Ты не первый, кто меня так называет, но до сих пор всё получалось!"

# game/status.rpy:10466
translate russia endgameMenu1_63499a70:

    # mat "The city is facing imminent destruction."
    mat "Городу грозит неминуемое разрушение."

# game/status.rpy:10467
translate russia endgameMenu1_a502ede6:

    # y "Oh.......{w} right."
    y "Ох.......{w} да."

# game/status.rpy:10468
translate russia endgameMenu1_0bea253e:

    # y "Just trust me okay?"
    y "Просто доверься мне, хорошо?"

# game/status.rpy:10469
translate russia endgameMenu1_3a559dc5:

    # mat "So why am I doing this...?"
    mat "Так зачем я это делаю...?"

# game/status.rpy:10471
translate russia endgameMenu1_c560d68e:

    # y "{i}'When the spools of cotton all unwind and the lions rest after their feast, we too shall know the leisure of cessation.'{/i}"
    y "{i}'Когда размотаются бобины хлопка и львы отдохнут после своего пиршества, мы тоже познаем досуг окончания.'{/i}"

# game/status.rpy:10472
translate russia endgameMenu1_92976df0:

    # mat "That's... so profound. Who are you quoting?"
    mat "Это... так глубоко. Кого ты цитируешь?"

# game/status.rpy:10473
translate russia endgameMenu1_263721ce:

    # y "Quoting? It just came to my mind!"
    y "Цитирую? Это просто пришло мне в голову!"

# game/status.rpy:10474
translate russia endgameMenu1_6486a77e:

    # mat "Oh! I didn't know you were a poet! What does it mean?"
    mat "О! Я и не знал, что ты поэт! Что это значит?"

# game/status.rpy:10475
translate russia endgameMenu1_6e4ecfcc:

    # y "........................."
    y "........................."

# game/status.rpy:10476
translate russia endgameMenu1_2f9cc419:

    # y "{i}Stuff....{/i}"
    y "{i}Пум-Пурум....{/i}"

# game/status.rpy:10477
translate russia endgameMenu1_09c2e65a:

    # mat "Oh right.. I keep forgetting how crazy you really are..."
    mat "Ну да... Я всё время забываю, насколько ты сумасшедший..."

# game/status.rpy:10478
translate russia endgameMenu1_3a559dc5_1:

    # mat "So why am I doing this...?"
    mat "Так зачем я это делаю...?"

# game/status.rpy:10479
translate russia endgameMenu1_0ca48d61:

    # y "To save the city from destruction of course!"
    y "Чтобы спасти город от разрушения, конечно же!"

# game/status.rpy:10480
translate russia endgameMenu1_b3e0aeae:

    # mat "No no no, I mean... Why am {i}'I'{/i} doing this?{w} What do I get out of it?"
    mat "Нет, нет, нет, я имею в виду... Зачем {i}'Я'{/i} это делаю?{w} Что я от этого получу?"

# game/status.rpy:10481
translate russia endgameMenu1_9500a0b4:

    # y "......................"
    y "......................"

# game/status.rpy:10482
translate russia endgameMenu1_77074a1b:

    # y "You greedy little punk.{w} Fine... {i}'one'{/i} date with Clover."
    y "Ах, ты маленький жадный сопляк.{w} Ладно... {i}'одно'{/i} свидание с Кловер."

# game/status.rpy:10483
translate russia endgameMenu1_6c67b826:

    # mat "DEAL!"
    mat "СОГЛАСЕН!"

# game/status.rpy:10484
translate russia endgameMenu1_b795acf2:

    # y "Now get busy!"
    y "Теперь за дело!"

# game/status.rpy:10502
translate russia endgameMenu1_60a42dec:

    # "DAYS UNTIL MISSILE LAUNCH: [daysTillMissle]."
    "ДНЕЙ ДО ЗАПУСКА РАКЕТ: [daysTillMissle]"

# game/status.rpy:10504
translate russia endgameMenu1_6e4ecfcc_1:

    # y "........................."
    y "........................."

# game/status.rpy:10505
translate russia endgameMenu1_d9403a80:

    # y "Mathias is everything ready?"
    y "Матиас, всё готово?"

# game/status.rpy:10506
translate russia endgameMenu1_b56c8add:

    # mat "Ready as it'll ever be.{w} Just give the word and I'll open all of the cell doors."
    mat "Готово, как никогда.{w} Только скажи, и я открою все двери камер."

# game/status.rpy:10507
translate russia endgameMenu1_1b176296:

    # y "Do it."
    y "Давай."

# game/status.rpy:10516
translate russia endgameMenu1_a70773d8:

    # "Agent 1" "Why! I sure am glad we're out here and those criminals are in there!"
    "Агент 1" "Зачем! Мне больше нравиться, что мы здесь, а эти преступники там!"

# game/status.rpy:10517
translate russia endgameMenu1_d0cf7662:

    # "Agent 2" "You said it fellow agent! Can you even begin to imagine what they'd do to us if the gates were unlocked?!"
    "Агент 2" "Согласен, товарищ! Вы хоть представляете, что они сделают с нами, если двери откроются?!"

# game/status.rpy:10519
translate russia endgameMenu1_c870c085:

    # "Rrrrrrrrrrrr~...."
    "Бзззззззззззз~...."

# game/status.rpy:10520
translate russia endgameMenu1_c34208c1:

    # "Agent 1" "Oh my! Do you hear that ringing sound fellow agent?!"
    "Агент 1" "О, Боже! Вы слышите этот звон, товарищ?!"

# game/status.rpy:10521
translate russia endgameMenu1_30a25179:

    # "Agent 2" "Yes I do! It's the distinct and unmistakable sound of a prison door opening!"
    "Агент 2" "Да! С таким отчётливым и мерзким звук открываются двери тюрьмы!"

# game/status.rpy:10522
translate russia endgameMenu1_2813b604:

    # "Agent 1" "..............................."
    "Агент 1" "..............................."

# game/status.rpy:10523
translate russia endgameMenu1_f7416194:

    # "Agent 2" "..............................."
    "Агент 2" "..............................."

# game/status.rpy:10524
translate russia endgameMenu1_286b0707:

    # "Agents" "AAAAAAAHHHHH!"
    "Агенты" "ААААААААХХХ!"

# game/status.rpy:10541
translate russia endgameMenu1_dc58d8c4:

    # tim "....................................."
    tim "....................................."

# game/status.rpy:10542
translate russia endgameMenu1_e5de8e8e:

    # seb "The doors opened! Let's get out of here."
    seb "Двери открылись! Давайте, уходим отсюда."

# game/status.rpy:10543
translate russia endgameMenu1_e29d3ddd:

    # candy "A stroke of good luck~..."
    candy "Какая удача~..."

# game/status.rpy:10544
translate russia endgameMenu1_d8165aec:

    # tim "We've been given a second chance. We can sti-..."
    tim "Нам дали второй шанс. Мы всё ещё мож-..."

# game/status.rpy:10545
translate russia endgameMenu1_878b7af4:

    # seb "Are you insane?! Your plan failed Tim, I'm getting out of here."
    seb "Ты с ума сошёл?! Твой план провалился Тим, я ухожу отсюда."

# game/status.rpy:10547
translate russia endgameMenu1_7bae650e:

    # candy "It was a stupid plan to begin with. I'm going back to creating robot cheerleaders to overthrow the world."
    candy "Это был глупый план с самого начала. Я возвращаюсь к созданию роботов-чирлидеров для свержения мира."

# game/status.rpy:10552
translate russia endgameMenu1_9f2d2eb2:

    # tim "............................."
    tim "............................."

# game/status.rpy:10553
translate russia endgameMenu1_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:10554
translate russia endgameMenu1_de617ee5:

    # tim "...?"
    tim "...?"

# game/status.rpy:10555
translate russia endgameMenu1_5ca9aaa7:

    # y "{b}*Static*{/b} Attention WOOHP inmates! Before you all escape and run of-..."
    y "{b}*Помехи*{/b} Внимание заключённые WOOHP! Пока вы все не разбежали-..."

# game/status.rpy:10556
translate russia endgameMenu1_323c478a:

    # tim "They're all gone."
    tim "Они все ушли."

# game/status.rpy:10557
translate russia endgameMenu1_ce895a18:

    # y "..........."
    y "..........."

# game/status.rpy:10558
translate russia endgameMenu1_c8853d93:

    # y "Crap.{w} It's just you?"
    y "Чёрт.{w} Остался только ты?"

# game/status.rpy:10559
translate russia endgameMenu1_11a212db:

    # tim "Just me. Did you let us out of our cells?"
    tim "Только я. Ты выпустил нас из камер?"

# game/status.rpy:10560
translate russia endgameMenu1_d7962fb5:

    # y "Yes. It's a long story, but the short of it is... GLADIS is preparing to nuke the entire city."
    y "Да. Это долгая история, но вкратце... Г.Л.Э.Д.И.С. готовится взорвать весь город."

# game/status.rpy:10561
translate russia endgameMenu1_e1b21f10:

    # y "We need your help taking back the milkshake bar. Then we can make a plan on how to stop her."
    y "Нам нужна ваша помощь, чтобы вернуть бар. Тогда мы сможем разработать план, как остановить её."

# game/status.rpy:10562
translate russia endgameMenu1_a80eb1e1:

    # tim "And if I do? You're just going to throw me back in my cells."
    tim "А если я помогу? Вы просто бросите меня обратно в мою камеру."

# game/status.rpy:10563
translate russia endgameMenu1_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:10567
translate russia endgameMenu1_9500a0b4_1:

    # y "......................"
    y "......................"

# game/status.rpy:10568
translate russia endgameMenu1_dcad7fcd:

    # y "Way to twist my arm..."
    y "Не заставляй меня..."

# game/status.rpy:10569
translate russia endgameMenu1_58e83dac:

    # tim "You want your base back or not?"
    tim "Ты хочешь вернуть свою базу или нет?"

# game/status.rpy:10570
translate russia endgameMenu1_60ef3a43:

    # y "Fine....... I promise to let you go after this is all over..."
    y "Лааадно..... Я обещаю отпустить тебя, когда всё закончится..."

# game/status.rpy:10571
translate russia endgameMenu1_eb498a4f:

    # tim "That's more like it."
    tim "Так-то лучше."

# game/status.rpy:10574
translate russia endgameMenu1_8b285a95:

    # y "N-noooooo.... I wouldn't do {i}thaaaat{/i}...!"
    y "Н-неееееет.... Я бы {i}таааак{/i} не поступил...!"

# game/status.rpy:10575
translate russia endgameMenu1_79337d99:

    # tim ".........................."
    tim ".........................."

# game/status.rpy:10576
translate russia endgameMenu1_b50f2e2b:

    # y "You know...! I would never!"
    y "Ты знаешь...! Я бы никогда!"

# game/status.rpy:10577
translate russia endgameMenu1_14086cf6:

    # y "Infact! The very idea you brought it up is INSULTING!"
    y "Точно! Сама мысль о том, что вты об этом заговорили, ОСКОРБИТЕЛЬНА!"

# game/status.rpy:10578
translate russia endgameMenu1_cc322782:

    # y "You have deeply insulted me good sir! To take my good name into question!"
    y "Вы глубоко оскорбили меня, сэр! Ставить под сомнение моё доброе имя!"

# game/status.rpy:10579
translate russia endgameMenu1_398524b9:

    # tim "We know you're lying......."
    tim "Мы знаем, что ты лжёшь......"

# game/status.rpy:10580
translate russia endgameMenu1_604cdc6f:

    # y "...................."
    y "...................."

# game/status.rpy:10581
translate russia endgameMenu1_9fa6e82c:

    # y "Okay fine...{w} if you help us out, you're free to go..."
    y "Ладно, хорошо...{w} Если ты нам поможешь, то будешь свободен..."

# game/status.rpy:10582
translate russia endgameMenu1_f815a9aa:

    # tim "That's better."
    tim "Так-то лучше."

# game/status.rpy:10584
translate russia endgameMenu1_03b7334e:

    # y "Voice in my head, what should I do?!"
    y "Голос в моей голове, что мне делать?!"

# game/status.rpy:10585
translate russia endgameMenu1_ceacda9f:

    # "Inner Voice" "Heeeey it's me. Haven't heard from me in a while!"
    "Внутренний голос" "Прииииивет, это я. Давно от меня не было вестей!"

# game/status.rpy:10586
translate russia endgameMenu1_ec6071aa:

    # y "I could use some advice here..."
    y "Мне нужен совет..."

# game/status.rpy:10587
translate russia endgameMenu1_647b1e03:

    # "Inner Voice" "Since when did I start giving good advice? I thought Chineese waterboard torture was a legit solution to bullying!"
    "Внутренний голос" "С каких пор я стал давать хорошие советы? Я думал, что китайская пытка каплей - это законное решение проблем с буллингом!"

# game/status.rpy:10588
translate russia endgameMenu1_8ba1bfe1:

    # tim "Who are you talking to...?"
    tim "С кем ты говоришь...?"

# game/status.rpy:10589
translate russia endgameMenu1_91f733ff:

    # y "Hush. I'm in a conversation here."
    y "Цыц. У меня важный разговор."

# game/status.rpy:10590
translate russia endgameMenu1_fdb42620:

    # tim "......................."
    tim "......................."

# game/status.rpy:10591
translate russia endgameMenu1_a84e5e08:

    # "Inner Voice" "Just admit it... You're going to need him."
    "Внутренний голос" "Просто признай это... Он тебе понадобится."

# game/status.rpy:10592
translate russia endgameMenu1_5976e89e:

    # "Inner Voice" "Whatever crimes he's committed, he can redeem himself by helping you save the city."
    "Внутренний голос" "Какие бы преступления он ни совершил, он может искупить свою вину, если поможет вам спасти город."

# game/status.rpy:10593
translate russia endgameMenu1_d5ef705b:

    # y "And what if he betrays me?"
    y "А что, если он предаст меня?"

# game/status.rpy:10594
translate russia endgameMenu1_cccbe4d6:

    # "Inner Voice" "It's just the risk you're going to have to take."
    "Внутренний голос" "Это просто риск, на который тебе придётся пойти."

# game/status.rpy:10595
translate russia endgameMenu1_6e4ecfcc_2:

    # y "........................."
    y "........................."

# game/status.rpy:10596
translate russia endgameMenu1_eb766e59:

    # y "Okay fine. Help us out and I promise to let you go afterwards."
    y "Ладно, хорошо. Помоги нам, и я обещаю отпустить тебя после этого."

# game/status.rpy:10597
translate russia endgameMenu1_eb498a4f_1:

    # tim "That's more like it."
    tim "Так-то лучше."

# game/status.rpy:10598
translate russia endgameMenu1_6e4ecfcc_3:

    # y "........................."
    y "........................."

# game/status.rpy:10599
translate russia endgameMenu1_fd8e3340:

    # tim "I'll begin the attack. Just wait for the signal."
    tim "Я начну атаку. Просто ждите сигнала."

# game/status.rpy:10600
translate russia endgameMenu1_e75e1672:

    # y "Signal?"
    y "Сигнала?"

# game/status.rpy:10601
translate russia endgameMenu1_623fe4bf:

    # tim "Tim signing out."
    tim "Отбой."

# game/status.rpy:10613
translate russia endgameMenu1_17c0c402:

    # brit "Are you sure you can trust him?"
    brit "Ты уверен, что ему можно доверять?"

# game/status.rpy:10614
translate russia endgameMenu1_98972a1d:

    # y "No. Infact, I'm pretty sure he'll betray us the moment he gets the chance."
    y "Нет. Более того, я уверен, что он предаст нас, как только у него появится шанс."

# game/status.rpy:10615
translate russia endgameMenu1_0bb3a8cb:

    # y "..............................."
    y "..............................."

# game/status.rpy:10616
translate russia endgameMenu1_2a3abf42:

    # y "Just keep him away from any firearms. I don't want to get shot a second time."
    y "Просто держите его подальше от любого огнестрельного оружия. Я не хочу, чтобы меня подстрелили во второй раз."

# game/status.rpy:10617
translate russia endgameMenu1_cd5e23f5:

    # y "We're going to have to wait for his signal..."
    y "Нам придётся ждать его сигнала..."

# game/status.rpy:10618
translate russia endgameMenu1_f24dd1ce:

    # brit "Signal?{w} What's the signal?"
    brit "Сигнал?{w} Что за сигнал?"

# game/status.rpy:10619
translate russia endgameMenu1_a8508a4e:

    # y "I... don't know."
    y "Я... не знаю."

# game/status.rpy:10622
translate russia endgameMenu1_e22fd704:

    # "{b}*BOOOOOM!*{/b}"
    "{b}*БУУУУУУМ!*{/b}"

# game/status.rpy:10623
translate russia endgameMenu1_c293c396:

    # O5O "HOLY MOLY!"
    O5O "ЁШКИ-МАТРЁШКИ!"

# game/status.rpy:10624
translate russia endgameMenu1_9ff02c6c:

    # brit "That was coming from the milkshake bar!"
    brit "Это доносилось из бара!"

# game/status.rpy:10625
translate russia endgameMenu1_e093fcc9:

    # y "That must be the signal. Let's go!"
    y "Должно быть это сигнал. Поехали!"

# game/status.rpy:10650
translate russia endgameMenu1_7038a070:

    # "Agent" "Uh oh! Uh oh! Uh oh!"
    "Агент" "Ой-ой! Ой-ой! Ой-ой!"

# game/status.rpy:10654
translate russia endgameMenu1_074c58fb:

    # "Agent" "......................"
    "Агент" "......................"

# game/status.rpy:10655
translate russia endgameMenu1_65c2fceb:

    # tim "Run."
    tim "Беги."

# game/status.rpy:10656
translate russia endgameMenu1_e7271041:

    # "Agent" "AAAAAAAAAHHHH!"
    "Агент" "ААААААААХХХ!"

# game/status.rpy:10662
translate russia endgameMenu1_cbc2fadf:

    # y "The bar is ours aga-...{w} Ow...."
    y "Бар - снова на-...{w} Оу...."

# game/status.rpy:10663
translate russia endgameMenu1_fdb42620_1:

    # tim "......................."
    tim "......................."

# game/status.rpy:10664
translate russia endgameMenu1_aa84fc07:

    # tim "What happened to you?"
    tim "Что с тобой случилось?"

# game/status.rpy:10665
translate russia endgameMenu1_40ee8495:

    # y "Gunshot."
    y "Огнестрел."

# game/status.rpy:10666
translate russia endgameMenu1_20e5e019:

    # tim "Gunshot?"
    tim "Огнестрел?"

# game/status.rpy:10667
translate russia endgameMenu1_38d3120b:

    # y "Jerry."
    y "Джерии."

# game/status.rpy:10668
translate russia endgameMenu1_4d319732:

    # tim "Jerry shot you...?"
    tim "Джерри стрелял в тебя...?"

# game/status.rpy:10669
translate russia endgameMenu1_60d20db6:

    # brit b54 "Jerry is being controlled with nanobots. GLADIS has fully taken over."
    brit b54 "Джерри под контролем наноботов. Г.Л.Э.Д.И.С. полностью взяла на себя управление."

# game/status.rpy:10670
translate russia endgameMenu1_eccb081b:

    # tim "I see..."
    tim "Понятно..."

# game/status.rpy:10671
translate russia endgameMenu1_90783a01:

    # tim "What about Clover, Sam and Alex?"
    tim "А как же Кловер, Сэм и Алекс?"

# game/status.rpy:10672
translate russia endgameMenu1_a0f6ff0f:

    # y "Controlled aswell. Together with my army of freed WOOHP agents."
    y "Они тоже под контролем. Вместе с моей армией освобожденных агентов WOOHP."

# game/status.rpy:10673
translate russia endgameMenu1_4467e4e6:

    # tim "................."
    tim "................."

# game/status.rpy:10674
translate russia endgameMenu1_d7c57513:

    # tim "Do you have a plan?"
    tim "У тебя есть план?"

# game/status.rpy:10675
translate russia endgameMenu1_9598eaeb:

    # y "Sorta..."
    y "Типа того..."

# game/status.rpy:10676
translate russia endgameMenu1_614f306e:

    # y "Seeing as you worked on GLADIS, I figured you might know how to disable her."
    y "Поскольку ты работал над Г.Л.Э.Д.И.С., ты, скорее всего знаешь, как её отключить."

# game/status.rpy:10677
translate russia endgameMenu1_e658e4ac:

    # y "Now I know we are enemies, but this is for real! If you don't help us the whole city will be destroyed!"
    y "Я знаю, что мы враги, но это правда! Если ты не поможешь нам, весь город будет разрушен!"

# game/status.rpy:10678
translate russia endgameMenu1_3df12b47:

    # tim "I'll help."
    tim "Я помогу."

# game/status.rpy:10679
translate russia endgameMenu1_71591a7c:

    # y "Have a heart! I can understand your hesitation, but please for everything that's holy, help us!"
    y "Имей сердце! Я понимаю твою нерешительность, но, пожалуйста, ради всего святого, помоги нам!"

# game/status.rpy:10680
translate russia endgameMenu1_9b2f9820:

    # tim "I said I'll hel-."
    tim "Я же сказал, я помо-."

# game/status.rpy:10681
translate russia endgameMenu1_2026386a:

    # y "I know we haven't always seen eye to eye, but there's people in dang-..."
    y "Я знаю, что мы не особо ладили, но тут люди в бе-..."

# game/status.rpy:10683
translate russia endgameMenu1_808ddab2:

    # kim "{b}*Slaps you on the back of the head.*{/b}"
    kim "{b}*Похлопывает вас по затылку.*{/b}"

# game/status.rpy:10684
translate russia endgameMenu1_25661e33:

    # tim "I can't believe we lost to an idiot like you..."
    tim "Не могу поверить, что мы проиграли такому идиоту, как ты..."

# game/status.rpy:10685
translate russia endgameMenu1_b6e95ed6:

    # tim "If we want to disable GLADIS, we need to get close to her. There's an emergency incapacitate clasp that can shut her down."
    tim "Если мы хотим обезвредить Г.Л.Э.Д.И.С., нам нужно подобраться к ней вплотную. Есть аварийный тумблер, который поможет отключить её."

# game/status.rpy:10686
translate russia endgameMenu1_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:10687
translate russia endgameMenu1_d2907626:

    # y "An off switch?"
    y "Выключатель?"

# game/status.rpy:10688
translate russia endgameMenu1_56e4e4ef:

    # tim "Yes, the challenge will be getting close."
    tim "Да, трудность будет заключаться в том, чтобы подобраться поближе."

# game/status.rpy:10689
translate russia endgameMenu1_818974ec:

    # y "We need the spies..."
    y "Нам нужны шпионки..."

# game/status.rpy:10690
translate russia endgameMenu1_1c6532db:

    # y "Too bad they're off to God-knows-where. It's going to take weeks to find them!"
    y "Жаль, что они уехали Бог знает куда. На их поиски уйдут недели!"

# game/status.rpy:10691
translate russia endgameMenu1_9201f464:

    # tim "....................."
    tim "....................."

# game/status.rpy:10692
translate russia endgameMenu1_19226093:

    # tim "Maybe not.{w} Here..."
    tim "Может, и нет.{w} Вот..."

# game/status.rpy:10693
translate russia endgameMenu1_074b7f68:

    # "Tim hands you a piece of paper with a number on it."
    "Тим передает вам листок бумаги с каким-то номером."

# game/status.rpy:10694
translate russia endgameMenu1_8e91a8e6:

    # y "You're giving me your phone number?"
    y "Ты даёшь мне свой номер телефона?"

# game/status.rpy:10695
translate russia endgameMenu1_738e6c15:

    # tim "Not {i}my{/i} number..."
    tim "Это не {i}мой{/i} номер..."

# game/status.rpy:10696
translate russia endgameMenu1_90dd6a7f:

    # tim "Call it... I trust her."
    tim "Набери его... Я ей доверяю."

# game/status.rpy:10697
translate russia endgameMenu1_9500a0b4_2:

    # y "......................"
    y "......................"

# game/status.rpy:10698
translate russia endgameMenu1_f683344e:

    # "{b}*Beep* *Beep*{/b}"
    "{b}*Бип* *Бип*{/b}"

# game/status.rpy:10699
translate russia endgameMenu1_91955939:

    # "You call the number...."
    "Вы звоните по номеру...."

# game/status.rpy:10703
translate russia endgameMenu1_6f588461:

    # "...........{w}...........{w}..........."
    "...........{w}...........{w}..........."

# game/status.rpy:10704
translate russia endgameMenu1_887d215a:

    # mel "Hello...?"
    mel "Ало...?"

# game/status.rpy:10705
translate russia endgameMenu1_14cca946:

    # y "Wait... I recognize that voice...!"
    y "Подождите... Я узнаю этот голос...!"

# game/status.rpy:10706
translate russia endgameMenu1_3e40bd71:

    # mel "Who is this...?"
    mel "Кто это...?"

# game/status.rpy:10707
translate russia endgameMenu1_f72e182d:

    # y "Not important. Listen...! Sam is in trouble and I need your help!"
    y "Неважно. Слушай...! Сэм в беде, и мне нужна твоя помощь!"

# game/status.rpy:10708
translate russia endgameMenu1_c7393e39:

    # mel "Is this that insane idiot that imprisoned me?"
    mel "Это тот чокнутый идиот, который посадил меня в тюрьму?"

# game/status.rpy:10711
translate russia endgameMenu1_34fa9179:

    # y "Yup."
    y "Ага."

# game/status.rpy:10712
translate russia endgameMenu1_a17cf78a:

    # mel "................................."
    mel "................................."

# game/status.rpy:10713
translate russia endgameMenu1_6fa14f8a:

    # mel "Well at least you're an honest idiot..."
    mel "Ну, по крайней мере, ты честный идиот..."

# game/status.rpy:10715
translate russia endgameMenu1_f609f6fa:

    # y "I don't know what you're talking about!"
    y "Я не знаю, о чём ты говоришь!"

# game/status.rpy:10716
translate russia endgameMenu1_ec8265c4:

    # mel "It is you, isn't it...?!"
    mel "Это ты, не так ли...?!"

# game/status.rpy:10718
translate russia endgameMenu1_369366ef:

    # y "Yes it's me. Let bi-gongs be bi-gongs!"
    y "Да, это я. Что пришло то пришло!"

# game/status.rpy:10719
translate russia endgameMenu1_80096dbf:

    # mel "...................."
    mel "...................."

# game/status.rpy:10720
translate russia endgameMenu1_2e0f3460:

    # mel "Do you mean bygones?"
    mel "Ты имеешь в виду \"прошло\"?"

# game/status.rpy:10721
translate russia endgameMenu1_9e960a9e:

    # y "Bygones...? That makes so much more sense!"
    y "Прошло...? В этом гораздо больше смысла!"

# game/status.rpy:10722
translate russia endgameMenu1_63439f1c:

    # y "No wonder people gave me funny looks whenever I said it..."
    y "Неудивительно, что люди смотрели на меня как на идиота, когда я это говорил..."

# game/status.rpy:10723
translate russia endgameMenu1_5ccec6c5:

    # mel "Yes, you are definitely the crazy idiot..."
    mel "Да, ты точно долбанутый идиот..."

# game/status.rpy:10724
translate russia endgameMenu1_61c5ade2:

    # mel "What was this about Sam?"
    mel "Что там было на счёт Сэм?"

# game/status.rpy:10725
translate russia endgameMenu1_01cd44b4:

    # y "Sam is back to being nanobot controlled! I know she betrayed you and everything, but we have to free her or she's going up in smoke with the rest of the city!"
    y "Сэм снова под контролем наноботов! Я знаю что она предала тебя и всё такое. Но мы должны освободить её, иначе она распадётся на атомы вместе со всем городом!"

# game/status.rpy:10726
translate russia endgameMenu1_0bd60de2:

    # mel "......................."
    mel "......................."

# game/status.rpy:10727
translate russia endgameMenu1_51b08f31:

    # y "Come on Melody... Name your price!"
    y "Давай, Мелоди... Назовите свою цену!"

# game/status.rpy:10728
translate russia endgameMenu1_0bd60de2_1:

    # mel "......................."
    mel "......................."

# game/status.rpy:10729
translate russia endgameMenu1_24dc2ec2:

    # mel "Sam's a good kid..."
    mel "Сэм - хорошая..."

# game/status.rpy:10730
translate russia endgameMenu1_f417f93d:

    # mel "Even if we were on different sides the whole time, she doesn't deserve this."
    mel "Даже если мы всё это время были по разные стороны, она этого не заслуживает."

# game/status.rpy:10731
translate russia endgameMenu1_65e6fc6f:

    # y "So you'll help...?"
    y "Так ты поможешь...?"

# game/status.rpy:10732
translate russia endgameMenu1_fea153c4:

    # mel "Fine... what do you need?"
    mel "Ладно... что вам нужно?"

# game/status.rpy:10733
translate russia endgameMenu1_e03f75ab:

    # y "She's missing. We're looking for her."
    y "Она пропала. Мы её ищем."

# game/status.rpy:10734
translate russia endgameMenu1_1b2abeab:

    # mel "Missing? Hang on... I think I know where to find her. If she's being controlled by nanobots, you better have something to break her out of it though."
    mel ""

# game/status.rpy:10735
translate russia endgameMenu1_a3f7ed59:

    # mel "I'm sending you the coordinates."
    mel ""

# game/status.rpy:10736
translate russia endgameMenu1_4528ee2b:

    # y "Oh and Melody...!"
    y ""

# game/status.rpy:10737
translate russia endgameMenu1_99bfc6fa:

    # mel "Yeah?"
    mel ""

# game/status.rpy:10738
translate russia endgameMenu1_ab36cbd5:

    # y "Thanks...."
    y ""

# game/status.rpy:10739
translate russia endgameMenu1_a7ea4d6e:

    # mel "....................................."
    mel "....................................."

# game/status.rpy:10748
translate russia endgameMenu1_740034f7:

    # mat "So what do you think?"
    mat ""

# game/status.rpy:10749
translate russia endgameMenu1_92f0c34e:

    # tim "Onorthodox... but it could work."
    tim ""

# game/status.rpy:10750
translate russia endgameMenu1_8bd26898:

    # y "You gave me Melody's number?!"
    y ""

# game/status.rpy:10751
translate russia endgameMenu1_a7b3939d:

    # tim "Yeah. What did she say?"
    tim ""

# game/status.rpy:10752
translate russia endgameMenu1_69d73583:

    # y "She's giving me the coordinates of where she thinks Sam is."
    y ""

# game/status.rpy:10753
translate russia endgameMenu1_cafda7d3:

    # y "You could have told me who I was calling though!"
    y ""

# game/status.rpy:10754
translate russia endgameMenu1_fd562d24:

    # tim "Was is awkward?"
    tim ""

# game/status.rpy:10755
translate russia endgameMenu1_30495171:

    # y "Yes!"
    y ""

# game/status.rpy:10756
translate russia endgameMenu1_29e7f057:

    # tim "Good."
    tim ""

# game/status.rpy:10757
translate russia endgameMenu1_5f60ba9a:

    # y ".............................."
    y ".............................."

# game/status.rpy:10758
translate russia endgameMenu1_b41eb032:

    # mat "And speaking of awkward~...."
    mat ""

# game/status.rpy:10759
translate russia endgameMenu1_098ead66:

    # mat "Tim and I came up with a little something to help break the nanobot control of the girls."
    mat ""

# game/status.rpy:10760
translate russia endgameMenu1_b05634b9:

    # "Mathias hands you a sketch of a latex body suit."
    ""

# game/status.rpy:10761
translate russia endgameMenu1_bef702a4:

    # y "Wow... I didn't know you were into this kind of thing Mathias!"
    y ""

# game/status.rpy:10762
translate russia endgameMenu1_53b8c715:

    # mat "Well... I'm not really..."
    mat ""

# game/status.rpy:10763
translate russia endgameMenu1_6f362d57:

    # y "..............."
    y "..............."

# game/status.rpy:10764
translate russia endgameMenu1_ee9d3095:

    # tim "Don't look at me."
    tim ""

# game/status.rpy:10765
translate russia endgameMenu1_4b22ca98:

    # y "If it wasn't your idea and not Mathias... who came up with it...?"
    y ""

# game/status.rpy:10766
translate russia endgameMenu1_cb541e56:

    # mat "Well... I met this one girl who seemed super into it...{w} She gave me the sketch."
    mat ""

# game/status.rpy:10767
translate russia endgameMenu1_a23669b1:

    # y "............................"
    y "............................"

# game/status.rpy:10768
translate russia endgameMenu1_f3ecc832:

    # y "Black hair, glasses, looks like a librarian?"
    y ""

# game/status.rpy:10769
translate russia endgameMenu1_5ddecd17:

    # mat "Yes! You know her?"
    mat ""

# game/status.rpy:10770
translate russia endgameMenu1_b881a402:

    # y "Yup. She works at the mall.{w} She's a perv, but it's good to know she's okay."
    y ""

# game/status.rpy:10771
translate russia endgameMenu1_ef1c3366:

    # tim "Mathias and I came up with gadget. If you throw it at a spy, the suit will automatically wrap itself around them."
    tim ""

# game/status.rpy:10772
translate russia endgameMenu1_752de7d1:

    # mat "Not only trapping, but breaking their nanobots at the same time!"
    mat ""

# game/status.rpy:10773
translate russia endgameMenu1_48076e02:

    # mat "It has to be thrown from quite close though... So it would be best if the girl was distracted."
    mat ""

# game/status.rpy:10774
translate russia endgameMenu1_6360a571:

    # y "Well... if Melody was right, we'd soon be able to test it out on Sam..."
    y ""

# game/status.rpy:10775
translate russia endgameMenu1_4aee14ee:

    # y "Good work everyone!"
    y ""

# game/status.rpy:10799
translate russia endgameMenu1_f036d1ae:

    # y "All right everyone. We got a report that Sam is hiding out at the airport."
    y ""

# game/status.rpy:10800
translate russia endgameMenu1_09e5e0aa:

    # y "From what I've gathered, the place is heavily guarded, so I'm sending you out to soften up their defenses."
    y ""

# game/status.rpy:10801
translate russia endgameMenu1_b9343146:

    # y "Once those are down, we can move in and capture Sam."
    y ""

# game/status.rpy:10818
translate russia endgameMenu1_d8da5be0:

    # brit "We're back."
    brit ""

# game/status.rpy:10819
translate russia endgameMenu1_805ece7a:

    # y "So you are. And the airport?"
    y ""

# game/status.rpy:10820
translate russia endgameMenu1_930f88c4:

    # brit b39 "We cleared out the guards. No sign of Sam though."
    brit b39 ""

# game/status.rpy:10821
translate russia endgameMenu1_692be6fa:

    # y "............"
    y "............"

# game/status.rpy:10822
translate russia endgameMenu1_171b8535:

    # O5O "BUT WE DID SEE SOMEONE ELSE!{w} THE SILVER HAIRED GIRL IN THE RED DRESS!"
    O5O ""

# game/status.rpy:10823
translate russia endgameMenu1_61252a1a:

    # y "Melody? She was there?"
    y ""

# game/status.rpy:10824
translate russia endgameMenu1_ab0558f5:

    # brit b31 "Yeah. We don't know what she's up to."
    brit b31 ""

# game/status.rpy:10825
translate russia endgameMenu1_cf630fb5:

    # y "..................."
    y "..................."

# game/status.rpy:10826
translate russia endgameMenu1_305d45e5:

    # y "All right, let's not waste any time.{w} Tim, is the bondage suit ready?"
    y ""

# game/status.rpy:10828
translate russia endgameMenu1_d0ab772c:

    # tim "Ready but untested."
    tim ""

# game/status.rpy:10829
translate russia endgameMenu1_7e39ae9f:

    # y "Then this seems like an excellent opportunity for it!"
    y ""

# game/status.rpy:10830
translate russia endgameMenu1_f08fa01f:

    # tim "Remember that you have to get close. You'll have to distract her before springing the trap."
    tim ""

# game/status.rpy:10831
translate russia endgameMenu1_87ff6dd3:

    # y "Right..."
    y ""

# game/status.rpy:10832
translate russia endgameMenu1_13a3dade:

    # O5O "WHAT ARE YOU ORDERS?!"
    O5O ""

# game/status.rpy:10835
translate russia endgameMenu1_13202882:

    # brit "Agreed. The airport will not be vulnerable for long. We have to move now."
    brit ""

# game/status.rpy:10837
translate russia endgameMenu1_3d1ee4b0:

    # y "I'm feeling kind of lazy. Let's go tomorrow."
    y ""

# game/status.rpy:10838
translate russia endgameMenu1_dd365bc6:

    # brit "With all due respect, the airport is vulnerable to attack now."
    brit ""

# game/status.rpy:10839
translate russia endgameMenu1_e97e04ec:

    # brit "We don't have time to wait.{w} Don't forget there's missiles on the way."
    brit ""

# game/status.rpy:10840
translate russia endgameMenu1_a23669b1_1:

    # y "............................"
    y "............................"

# game/status.rpy:10841
translate russia endgameMenu1_be1a8426:

    # brit "You forgot, didn't you....?"
    brit ""

# game/status.rpy:10842
translate russia endgameMenu1_e34e869e:

    # y "Not so much forgot as... 'slipped my mind'."
    y ""

# game/status.rpy:10844
translate russia endgameMenu1_26fdd768:

    # y "Actually... the news about Melody being there sounds a bit suspicious to me."
    y ""

# game/status.rpy:10845
translate russia endgameMenu1_8822580c:

    # y "Tim, you told me to contact her. Do you know anything about this?"
    y ""

# game/status.rpy:10846
translate russia endgameMenu1_ec70b1f6:

    # tim "I know as much as you do."
    tim ""

# game/status.rpy:10847
translate russia endgameMenu1_e0470a7e:

    # O5O "MAYBE SHE'S SETTING A TRAP!"
    O5O ""

# game/status.rpy:10848
translate russia endgameMenu1_26d33ede:

    # tim "Or she's trying to help out a friend."
    tim ""

# game/status.rpy:10849
translate russia endgameMenu1_604cdc6f_1:

    # y "...................."
    y "...................."

# game/status.rpy:10850
translate russia endgameMenu1_f6c5e616:

    # brit "Either way, the airport is vulnerable to attack now."
    brit ""

# game/status.rpy:10851
translate russia endgameMenu1_804aa1b1:

    # brit "We don't have time to wait."
    brit ""

# game/status.rpy:10853
translate russia endgameMenu1_ac7da505:

    # "You pack your things and together you make your way to the airport."
    ""

# game/status.rpy:10854
translate russia endgameMenu1_d4198369:

    # "Half way there, you stop the group after hearing a peculiar sound..."
    ""

# game/status.rpy:10857
translate russia endgameMenu1_1ce9c404:

    # y "...?"
    y "...?"

# game/status.rpy:10858
translate russia endgameMenu1_49cca957:

    # brit "Why are we stopping? We're not there ye-..."
    brit ""

# game/status.rpy:10859
translate russia endgameMenu1_75183423:

    # y "Shh. Listen."
    y ""

# game/status.rpy:10860
translate russia endgameMenu1_d4b17a28:

    # "You all fall quiet and listen intensely."
    ""

# game/status.rpy:10861
translate russia endgameMenu1_3ff72b5a:

    # "???" "{b}*Moans*{/b}"
    "???" ""

# game/status.rpy:10862
translate russia endgameMenu1_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:10863
translate russia endgameMenu1_25f71b0d:

    # brit "What... what's that...?"
    brit ""

# game/status.rpy:10864
translate russia endgameMenu1_b165a341:

    # y "It's coming from over here."
    y ""

# game/status.rpy:10865
translate russia endgameMenu1_09e95d60:

    # "You carefully creep closer to where the sound is coming from..."
    ""

# game/status.rpy:10866
translate russia endgameMenu1_66c7cc68:

    # mel "Go on bitch. You're not a real Ace until you learn how to lick pussy."
    mel ""

# game/status.rpy:10867
translate russia endgameMenu1_d3a59cb5_1:

    # y "............................."
    y "............................."

# game/status.rpy:10868
translate russia endgameMenu1_2b1c8d3e:

    # y "{b}*Low*{/b} Well now I'm curious....!"
    y ""

# game/status.rpy:10869
translate russia endgameMenu1_eae18c49:

    # brit "{b}*Low*{/b} Shhh~..."
    brit ""

# game/status.rpy:10870
translate russia endgameMenu1_ed6df425:

    # "You creep a little closer and turn the corner....."
    ""

# game/status.rpy:10873
translate russia endgameMenu1_aea5130f:

    # y "HOLY....!"
    y ""

# game/status.rpy:10878
translate russia endgameMenu1_31fa280c:

    # y "Awww yeeeeeeah."
    y ""

# game/status.rpy:10879
translate russia endgameMenu1_6b19a6fb:

    # s "{b}*MUFFLED CURSES*{/b}!"
    s ""

# game/status.rpy:10880
translate russia endgameMenu1_204aab6c:

    # y "Say cheese ladies."
    y ""

# game/status.rpy:10886
translate russia endgameMenu1_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:10888
translate russia endgameMenu1_2f9b5a87:

    # mel "About time you showed up!"
    mel ""

# game/status.rpy:10889
translate russia endgameMenu1_7f43a363:

    # y "Melody! What are you doing here?"
    y ""

# game/status.rpy:10890
translate russia endgameMenu1_86a62a7e:

    # mel "What does it look like? Breaking this bitch out of her trance!"
    mel ""

# game/status.rpy:10891
translate russia endgameMenu1_194aeb6a:

    # y "Aww... you came back and risked getting captured again just to rescue Sam?"
    y ""

# game/status.rpy:10892
translate russia endgameMenu1_c8e86d09:

    # mel "Just tell me you brought something to restrain her..."
    mel ""

# game/status.rpy:10893
translate russia endgameMenu1_edd41abb:

    # y "Oh right! The suit..!{w} Here... Melody, stand clear."
    y ""

# game/status.rpy:10895
translate russia endgameMenu1_21a4306f:

    # "You throw the gadget Tim and Mathias came up with and..."
    ""

# game/status.rpy:10900
translate russia endgameMenu1_722a012e:

    # s "{b}*Mfph! Mpfhhfff!!!*{/b}"
    s ""

# game/status.rpy:10901
translate russia endgameMenu1_a2639172:

    # y "It worked!"
    y ""

# game/status.rpy:10902
translate russia endgameMenu1_288e3119:

    # s "{b}*Furious muffles!!!*{/b}"
    s ""

# game/status.rpy:10903
translate russia endgameMenu1_5c6cf512:

    # s ".........................."
    s ".........................."

# game/status.rpy:10904
translate russia endgameMenu1_7d52d827:

    # s "{b}*Lustful muffles~...*{/b}"
    s ""

# game/status.rpy:10905
translate russia endgameMenu1_70fe3d17:

    # y "Okay Cucumber, don't have too much fun in there..."
    y ""

# game/status.rpy:10906
translate russia endgameMenu1_f0f7cd55:

    # y "O5O, let's grab her and get back to the base!"
    y ""

# game/status.rpy:10907
translate russia endgameMenu1_fdc9c8e3:

    # O5O "PLEASE STOP SQUIRMING!!"
    O5O ""

# game/status.rpy:10915
translate russia endgameMenu1_dd27107b:

    # O5O "TARGET HAS BEEN PLACED IN HER CELL!"
    O5O ""

# game/status.rpy:10916
translate russia endgameMenu1_9de438bb:

    # brit "She was very wiggly."
    brit ""

# game/status.rpy:10917
translate russia endgameMenu1_ad1ef43c:

    # O5O "SO WIGGLY!"
    O5O ""

# game/status.rpy:10918
translate russia endgameMenu1_ee9b940e:

    # y "All right, good work guys. I'll talk to her in a bit."
    y ""

# game/status.rpy:10919
translate russia endgameMenu1_2be94085:

    # "Agent O5O and Britney nod."
    ""

# game/status.rpy:10937
translate russia endgameMenu1_14d6bda0:

    # s "{b}Moans{/b}!!!"
    s ""

# game/status.rpy:10938
translate russia endgameMenu1_922d6ab5:

    # y "Hello there Sam. Good to see you again."
    y ""

# game/status.rpy:10939
translate russia endgameMenu1_7eddad04:

    # s "{b}Muffled{/b}!"
    s ""

# game/status.rpy:10940
translate russia endgameMenu1_374213d6:

    # y "Don't worry. Let me just get you out of th-..."
    y ""

# game/status.rpy:10941
translate russia endgameMenu1_7de40587:

    # y "Wait... open your eyes..."
    y ""

# game/status.rpy:10942
translate russia endgameMenu1_82b04cca:

    # s "...!"
    s "...!"

# game/status.rpy:10943
translate russia endgameMenu1_3e6170f8:

    # y "Still red...? That last nanobot update must have been a lot more powerful..."
    y ""

# game/status.rpy:10946
translate russia endgameMenu1_c1252e5e:

    # y "So I was thinkin-..."
    y ""

# game/status.rpy:10947
translate russia endgameMenu1_6311dcfd:

    # s "{b}*Muffled Moans*{/b}!!! ♥♥♥"
    s ""

# game/status.rpy:10948
translate russia endgameMenu1_6f323c89:

    # y "............."
    y "............."

# game/status.rpy:10949
translate russia endgameMenu1_9bff1593:

    # y "So what I was thinking was th-..."
    y ""

# game/status.rpy:10950
translate russia endgameMenu1_7a130d24:

    # s "{b}*More Muffled Moans*{/b}!!! ♥♥♥♥♥"
    s ""

# game/status.rpy:10951
translate russia endgameMenu1_9e379d37:

    # y "Calm down penis. This is not the time for-..."
    y ""

# game/status.rpy:10952
translate russia endgameMenu1_8842532a:

    # s "{b}*LUSTFUL MUFFLED MOANS*{/b}!!! ♥♥♥♥♥♥♥♥"
    s ""

# game/status.rpy:10953
translate russia endgameMenu1_74e76c18:

    # y "Fuck it..."
    y ""

# game/status.rpy:10954
translate russia endgameMenu1_d027b6ce:

    # "You whip out your dick and begin jerking it to the moaning spy."
    ""

# game/status.rpy:10955
translate russia endgameMenu1_d07b7c80:

    # y "That's it you slut! You like it, don't ya?"
    y ""

# game/status.rpy:10956
translate russia endgameMenu1_e64fd548:

    # s "{b}*Desperate Muffled Moans*{/b}!!! ♥♥♥♥"
    s ""

# game/status.rpy:10957
translate russia endgameMenu1_1127a496:

    # y "Fuuuuuck....!"
    y ""

# game/status.rpy:10965
translate russia endgameMenu1_16b294fb:

    # s "{b}*Whining Moans*{/b}"
    s ""

# game/status.rpy:10966
translate russia endgameMenu1_72858fea:

    # y "All right. Enough fun. Let's get you out of there."
    y ""

# game/status.rpy:10968
translate russia endgameMenu1_6311dcfd_1:

    # s "{b}*Muffled Moans*{/b}!!! ♥♥♥"
    s ""

# game/status.rpy:10969
translate russia endgameMenu1_8d71a4a8:

    # "You continue to watch as the spy is writhing in pleasure."
    ""

# game/status.rpy:10970
translate russia endgameMenu1_b17249c0:

    # "Orgasm after orgasm hits the redheaded girl as sweat is dripping off of her."
    ""

# game/status.rpy:10971
translate russia endgameMenu1_e607a53f:

    # y "Damn, that looks exhausting....!"
    y ""

# game/status.rpy:10972
translate russia endgameMenu1_9653d10b:

    # s "{b}*Desperate Muffled Moans*{/b}"
    s ""

# game/status.rpy:10973
translate russia endgameMenu1_1974858d:

    # y "Okay okay... let's get you out of there."
    y ""

# game/status.rpy:10975
translate russia endgameMenu1_99f20585:

    # y "This seems like an excellent time to practise my sea shanties!"
    y ""

# game/status.rpy:10976
translate russia endgameMenu1_856a6b58:

    # s "Mpghhh! Mffff!! Mhphff!!! ♥♥♥♥"
    s ""

# game/status.rpy:10977
translate russia endgameMenu1_31070c5e:

    # y "Oh I see you're excited! Okay, so here it goes..."
    y ""

# game/status.rpy:10978
translate russia endgameMenu1_44f557f2:

    # y "♫♪ Oooh we'd be aaaaall right, if we make it 'round the Hor-... ♫♪"
    y ""

# game/status.rpy:10979
translate russia endgameMenu1_f734e998:

    # s "Mfphoff! Ufghhh!!!! ♥♥♥♥"
    s ""

# game/status.rpy:10980
translate russia endgameMenu1_f1f0e526:

    # y "Please Sam. I can tell that you're loving these good ol' shanties, but try not to interrupt."
    y ""

# game/status.rpy:10981
translate russia endgameMenu1_c3475233:

    # y "{b}*Ahem*{/b} ♫♪ What shall we do with the drunken sailor?! What shall we d-... ♫♪"
    y ""

# game/status.rpy:10982
translate russia endgameMenu1_43e80ac5:

    # s "{b}*Desparate exausted moans*{/b} ♥♥"
    s ""

# game/status.rpy:10983
translate russia endgameMenu1_9c634fdd:

    # y "{b}*Sighs*{/b} No respect for the classics..."
    y ""

# game/status.rpy:10991
translate russia endgameMenu1_801cf89d:

    # y "All right, back to normal?"
    y ""

# game/status.rpy:10992
translate russia endgameMenu1_001b08ae:

    # "You turn off the vibrators and help the spy who's trembling on her legs."
    ""

# game/status.rpy:10993
translate russia endgameMenu1_003ff013:

    # y "You okay, Sam?"
    y ""

# game/status.rpy:10995
translate russia endgameMenu1_8c42253b:

    # s g15 "[playerName]...?"
    s g15 ""

# game/status.rpy:10996
translate russia endgameMenu1_5f90edfc:

    # s "I-I... thought you were... d-dead..."
    s ""

# game/status.rpy:10997
translate russia endgameMenu1_c9fa2171:

    # "Sam stumbles forward as you catch her just in time."
    ""

# game/status.rpy:10998
translate russia endgameMenu1_5372d31e:

    # y "Not far off. That old tea sipper got me good."
    y ""

# game/status.rpy:10999
translate russia endgameMenu1_b62b1c94:

    # s g15 "It's... not Jerry... GLADIS she-..."
    s g15 "Г.Л.Э.Д.И.С."

# game/status.rpy:11000
translate russia endgameMenu1_05aba7fe:

    # y "I know. I've been brought up to speed.{w} A lot has happened since we captured the top floor of WOOHP HQ.{w} Tim is on our side now!"
    y ""

# game/status.rpy:11001
translate russia endgameMenu1_59f119a6:

    # s "Tim...?"
    s ""

# game/status.rpy:11002
translate russia endgameMenu1_d291e596:

    # y "Or well... more of an alliance of convenience."
    y ""

# game/status.rpy:11003
translate russia endgameMenu1_23001e17:

    # "You quickly fill Sam in on the situation."
    ""

# game/status.rpy:11004
translate russia endgameMenu1_2f502124:

    # s g4 "That's... a lot to take in..."
    s g4 ""

# game/status.rpy:11005
translate russia endgameMenu1_77ddf579:

    # y "Unfortunately, we don't have a whole lot of time. Do you know where Clover and Alex are?"
    y ""

# game/status.rpy:11006
translate russia endgameMenu1_ab2f4514:

    # s g32 "Yeah... Clover is in Punk Web."
    s g32 ""

# game/status.rpy:11007
translate russia endgameMenu1_bfedd13b:

    # y "And Alex?"
    y ""

# game/status.rpy:11008
translate russia endgameMenu1_7918b229:

    # s "Still at WOOHP HQ. We'd have to plan a mission to bring them back."
    s ""

# game/status.rpy:11009
translate russia endgameMenu1_874af300:

    # y "All right. In that case grab a shower and then get some rest. We'll go after them tomorrow."
    y ""

# game/status.rpy:11010
translate russia endgameMenu1_b4eab292:

    # "The still trembling spy nods as you leave her cell."
    ""

# game/status.rpy:11019
translate russia endgameMenu1_56c751c7:

    # brit "How is she?"
    brit ""

# game/status.rpy:11020
translate russia endgameMenu1_5d5de221:

    # y "Back to her old self, but exausted."
    y ""

# game/status.rpy:11021
translate russia endgameMenu1_dacac5b1:

    # O5O "AND THE OTHER SPIES?!"
    O5O ""

# game/status.rpy:11022
translate russia endgameMenu1_41a69b27:

    # y "I've got their location. We can start doing missions again tomorrow."
    y ""

# game/status.rpy:11023
translate russia endgameMenu1_f76bcf3b:

    # "Britney and Agent O5O give you a nod and prepare for bed."
    ""

# game/status.rpy:11029
translate russia endgameMenu1_f15bc539:

    # tim "..........................."
    tim "..........................."

# game/status.rpy:11030
translate russia endgameMenu1_4e39a5be:

    # tim "Is she okay...?"
    tim ""

# game/status.rpy:11031
translate russia endgameMenu1_2a111d9d:

    # y "A little worn out, but she'll be all right."
    y ""

# game/status.rpy:11032
translate russia endgameMenu1_42a05871:

    # y "So what's the deal between you two anyways?"
    y ""

# game/status.rpy:11033
translate russia endgameMenu1_17c23f7f:

    # tim "It's... complicated."
    tim ""

# game/status.rpy:11034
translate russia endgameMenu1_7d9cbfc7:

    # y "You sound like Sam..."
    y ""

# game/status.rpy:11035
translate russia endgameMenu1_c70dae0c:

    # tim "Regardless, we're not done yet."
    tim ""

# game/status.rpy:11036
translate russia endgameMenu1_bfe2202d:

    # tim "Only [daysTillMissle] days until we all go up in fire and smoke. Best bring those girls back quick."
    tim ""

# game/status.rpy:11037
translate russia endgameMenu1_a794e085:

    # y "Right."
    y ""

# game/status.rpy:11052
translate russia endgameMenu1_60a42dec_1:

    # "DAYS UNTIL MISSILE LAUNCH: [daysTillMissle]."
    "ДНЕЙ ДО ЗАПУСКА РАКЕТ: [daysTillMissle]"

# game/status.rpy:11061
translate russia endgameMenu1_5091c9e9:

    # s g31 "All right, I'm ready! Can I suggest we rescue Clover first?"
    s g31 ""

# game/status.rpy:11062
translate russia endgameMenu1_3d4150d3:

    # y "Why Clover?"
    y ""

# game/status.rpy:11063
translate russia endgameMenu1_de1ae550:

    # s "Alex is at WOOHP HQ. Security is probably strongest there. I could use Clover as backup if we go there."
    s ""

# game/status.rpy:11064
translate russia endgameMenu1_fb70222f:

    # y "Good point. All right, let's plan a mission to Punk Web."
    y ""

# game/status.rpy:11109
translate russia endgameMenu1_5dfcc30c:

    # c "Mfph!!!?"
    c ""

# game/status.rpy:11110
translate russia endgameMenu1_ba0065ac:

    # c "Mmmm~....!!! ♥"
    c ""

# game/status.rpy:11115
translate russia endgameMenu1_d5f6d93a:

    # s g53 "Gotcha!{w} [playerName] we got her."
    s g53 ""

# game/status.rpy:11125
translate russia endgameMenu1_527bac41:

    # c "{b}*Mfmphdff!!!!*{/b}"
    c ""

# game/status.rpy:11126
translate russia endgameMenu1_cfe91af9:

    # s g29 "I think she's back to normal by now. Quickly, let's get her out of the suit!"
    s g29 ""

# game/status.rpy:11127
translate russia endgameMenu1_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:11130
translate russia endgameMenu1_e2507a68:

    # y "{b}*Zip*{/b}"
    y ""

# game/status.rpy:11131
translate russia endgameMenu1_de45e2f9:

    # s g31 "[playerName]...!"
    s g31 ""

# game/status.rpy:11132
translate russia endgameMenu1_c9aed820:

    # y "Stop being so outraged, Sam. Look how hot Clover is in that."
    y ""

# game/status.rpy:11133
translate russia endgameMenu1_bd118bf8:

    # s g29 "But we have to get her out!"
    s g29 ""

# game/status.rpy:11134
translate russia endgameMenu1_fe0f2a67:

    # c "Mpfhh!!"
    c ""

# game/status.rpy:11135
translate russia endgameMenu1_b92fa65c:

    # y "No we don't."
    y ""

# game/status.rpy:11136
translate russia endgameMenu1_1845457e:

    # c "Mpghf?!"
    c ""

# game/status.rpy:11137
translate russia endgameMenu1_c109e093:

    # s g11 ".............."
    s g11 ".............."

# game/status.rpy:11138
translate russia endgameMenu1_45f85c31:

    # s g12 "{b}*Sighs*{/b} Fine... she is looking pretty hot..."
    s g12 ""

# game/status.rpy:11139
translate russia endgameMenu1_1f6e70ed:

    # c "Mpfhhff!!!!"
    c ""

# game/status.rpy:11140
translate russia endgameMenu1_bb61e81b:

    # s g22 "Here, I'll help you."
    s g22 ""

# game/status.rpy:11142
translate russia endgameMenu1_48565696:

    # "Sam walks behind you and takes your cock in her hand before aiming it towards the squirming Clover and beginning to jerk it."
    ""

# game/status.rpy:11143
translate russia endgameMenu1_2ec2804a:

    # c "{b}*Whining Moans*{/b}"
    c ""

# game/status.rpy:11144
translate russia endgameMenu1_4245ebf3:

    # "Sam increases her speed as her delicate fingers stroke your manhood faster and faster. You are rock solid and the moans of the blonde spy are getting you close to climaxing."
    ""

# game/status.rpy:11145
translate russia endgameMenu1_3f926754:

    # "Bringing her face up to you ear, Sam whispers..."
    ""

# game/status.rpy:11146
translate russia endgameMenu1_63791b5a:

    # s "Coat the bitch~..."
    s ""

# game/status.rpy:11147
translate russia endgameMenu1_fdf2f98c:

    # "Unable to hold back you let out a loud groan as you drench the red spy in cum."
    ""

# game/status.rpy:11156
translate russia endgameMenu1_e2578744:

    # c "Mpmhfff!!!!!"
    c ""

# game/status.rpy:11157
translate russia endgameMenu1_8f34e163:

    # y "All right, all right. Let's get you out of there."
    y ""

# game/status.rpy:11159
translate russia endgameMenu1_cf51d74a:

    # y "We could."
    y ""

# game/status.rpy:11160
translate russia endgameMenu1_52c751ce:

    # y "Or we could just watch her for a while longer."
    y ""

# game/status.rpy:11161
translate russia endgameMenu1_8a8dbbbc:

    # s g31 "[playerName]!"
    s g31 ""

# game/status.rpy:11162
translate russia endgameMenu1_37c1e812:

    # c "Mpghff!"
    c ""

# game/status.rpy:11163
translate russia endgameMenu1_c93505d4:

    # y "See? She's enjoying herself!"
    y ""

# game/status.rpy:11164
translate russia endgameMenu1_43b23170:

    # c "{b}*Pleading Moans*{/b}"
    c ""

# game/status.rpy:11165
translate russia endgameMenu1_2ebf7899:

    # s g10 "..............."
    s g10 "..............."

# game/status.rpy:11166
translate russia endgameMenu1_3ce3b405:

    # y "Okay... maybe we should get her out..."
    y ""

# game/status.rpy:11168
translate russia endgameMenu1_723297dc:

    # y "All right blondie, that's enough fun for you."
    y ""

# game/status.rpy:11176
translate russia endgameMenu1_d377d6a3:

    # c r14 "{b}*Pant* *Pant*{/b}"
    c r14 ""

# game/status.rpy:11177
translate russia endgameMenu1_561d42a6:

    # y "Welcome back to us Clov-..."
    y ""

# game/status.rpy:11178
translate russia endgameMenu1_60f993fc:

    # c r7 "Raaaaah!"
    c r7 ""

# game/status.rpy:11182
translate russia endgameMenu1_94615cf8:

    # y "Aaah!{w} Quickly Sam, help me! She's still being nanobot controlled!"
    y ""

# game/status.rpy:11184
translate russia endgameMenu1_6cc151e5:

    # c r31 "No I'm not, you crazy pervert! Don't ever put me in that suit again!"
    c r31 ""

# game/status.rpy:11185
translate russia endgameMenu1_2903dc86:

    # y "Not a fan?"
    y ""

# game/status.rpy:11186
translate russia endgameMenu1_98ddb6f7:

    # c r9 "{b}NOT{/b} a fan!"
    c r9 ""

# game/status.rpy:11187
translate russia endgameMenu1_13c61b06:

    # y "Well too bad. If you're going crazy for cocoa puffs again, you be damn sure I'm gonna put you in that suit again."
    y ""

# game/status.rpy:11189
translate russia endgameMenu1_16cee8ab:

    # s g55 "Come on Clover. It was pretty hot, right?"
    s g55 ""

# game/status.rpy:11190
translate russia endgameMenu1_3e4ce180:

    # c r12 "Sam.....{w} You turned out to be the biggest sex freak of us all."
    c r12 ""

# game/status.rpy:11191
translate russia endgameMenu1_7817a402:

    # s g53 "Hey!"
    s g53 ""

# game/status.rpy:11192
translate russia endgameMenu1_e097e619:

    # y "All's well that ends well. Two spies in the bag, now we only nee-... {w}Ow...!"
    y ""

# game/status.rpy:11193
translate russia endgameMenu1_8c7dc031:

    # c r10 "C'mon [playerName]. I didn't punch you that hard."
    c r10 ""

# game/status.rpy:11194
translate russia endgameMenu1_b44608df:

    # y "Bullet wound."
    y ""

# game/status.rpy:11195
translate russia endgameMenu1_f8489b57:

    # c r16 "Bullet wou-...{w} Oh God! I completely forgot!"
    c r16 ""

# game/status.rpy:11196
translate russia endgameMenu1_7132fe9a:

    # c r28 "[playerName]! You're alive!"
    c r28 ""

# game/status.rpy:11198
translate russia endgameMenu1_710768cf:

    # "You quickly fill Clover in on the situation."
    ""

# game/status.rpy:11201
translate russia endgameMenu1_b73afacb:

    # c r18 "Missiles...?"
    c r18 ""

# game/status.rpy:11202
translate russia endgameMenu1_00a3a6c3:

    # c "I think I heard Alex saying something about that."
    c ""

# game/status.rpy:11203
translate russia endgameMenu1_80c5c3de:

    # s g56 "Alex?"
    s g56 ""

# game/status.rpy:11204
translate russia endgameMenu1_097817b5:

    # c r29 "Yeah that's right! Alex has been put in charge of guardian the WOOHP missile silos!"
    c r29 ""

# game/status.rpy:11205
translate russia endgameMenu1_14056409:

    # y "Then that's where we're going next.{w} Head back to your cell and get some rest. We're planning a mission to WOOHP tomorrow."
    y ""

# game/status.rpy:11209
translate russia endgameMenu1_0729e1ea:

    # "The girls nod and head back to their cells."
    ""

# game/status.rpy:11211
translate russia endgameMenu1_604cdc6f_2:

    # y "...................."
    y "...................."

# game/status.rpy:11212
translate russia endgameMenu1_150a3fc8:

    # "Your wounds aren't healed yet. Best to get an early night yourself."
    ""

# game/status.rpy:11233
translate russia endgameMenu1_60a42dec_2:

    # "DAYS UNTIL MISSILE LAUNCH: [daysTillMissle]."
    "ДНЕЙ ДО ЗАПУСКА РАКЕТ: [daysTillMissle]"

# game/status.rpy:11244
translate russia endgameMenu1_7a6944f9:

    # y "All right. Everyone rested? Then let's plan another mission and get Alex back."
    y ""

# game/status.rpy:11282
translate russia endgameMenu1_c4677a3f:

    # y "Hah! Gotcha!"
    y ""

# game/status.rpy:11283
translate russia endgameMenu1_6d807c8c:

    # a "..............................."
    a "..............................."

# game/status.rpy:11284
translate russia endgameMenu1_ee99fffe:

    # y "Er..."
    y ""

# game/status.rpy:11285
translate russia endgameMenu1_6d807c8c_1:

    # a "..............................."
    a "..............................."

# game/status.rpy:11286
translate russia endgameMenu1_a2f2ebae:

    # y "Alex...?"
    y ""

# game/status.rpy:11287
translate russia endgameMenu1_6d807c8c_2:

    # a "..............................."
    a "..............................."

# game/status.rpy:11288
translate russia endgameMenu1_78940657:

    # y "Okay you're freaking me out."
    y ""

# game/status.rpy:11290
translate russia endgameMenu1_eab62d75:

    # s "Let's get her back to the base."
    s ""

# game/status.rpy:11291
translate russia endgameMenu1_9b32fb43:

    # y "Good idea."
    y ""

# game/status.rpy:11301
translate russia endgameMenu1_804a8fec:

    # a "..................................."
    a "..................................."

# game/status.rpy:11302
translate russia endgameMenu1_5be7dd55:

    # y "Okay now you're really freaking me out Alex!"
    y ""

# game/status.rpy:11303
translate russia endgameMenu1_548eed1a:

    # y "Say something! Or at least make a noise!"
    y ""

# game/status.rpy:11304
translate russia endgameMenu1_804a8fec_1:

    # a "..................................."
    a "..................................."

# game/status.rpy:11308
translate russia endgameMenu1_c8275725:

    # s "Er.. [playerName]. I think we should just take her out of the suit."
    s ""

# game/status.rpy:11309
translate russia endgameMenu1_cf5a0925:

    # c "We'll keep her pinned down, just in case."
    c ""

# game/status.rpy:11310
translate russia endgameMenu1_3073a077:

    # y "All right. As long as I don't get attacked again."
    y ""

# game/status.rpy:11313
translate russia endgameMenu1_5655fda2:

    # "You begin slipping Alex out of her bondage suit as the other girls pin her down."
    ""

# game/status.rpy:11315
translate russia endgameMenu1_a7672129:

    # a "{b}*Gasp*{/b}!!!"
    a ""

# game/status.rpy:11316
translate russia endgameMenu1_0805b14f:

    # a y29 "I thought you were never letting me out of there!"
    a y29 ""

# game/status.rpy:11317
translate russia endgameMenu1_6f362d57_1:

    # y "..............."
    y "..............."

# game/status.rpy:11318
translate russia endgameMenu1_d271b6d1:

    # y "Alex? Back to normal?"
    y ""

# game/status.rpy:11319
translate russia endgameMenu1_5c301c8e:

    # a y1 "Yup!"
    a y1 ""

# game/status.rpy:11320
translate russia endgameMenu1_c535986b:

    # y "What was going on...? You didn't make a sound when you were in that suit..."
    y ""

# game/status.rpy:11321
translate russia endgameMenu1_788ae54e:

    # a "Of course not. I follow the WOOHP Sexual Torture course to resist that kind of thing!"
    a ""

# game/status.rpy:11322
translate russia endgameMenu1_3d8d3751:

    # c r18 "The WOOHP wha-...?"
    c r18 ""

# game/status.rpy:11323
translate russia endgameMenu1_be3bd0f6:

    # s g17 "There was a course for that?!"
    s g17 ""

# game/status.rpy:11324
translate russia endgameMenu1_ab75b63a:

    # s "Why didn't I know of this?!"
    s ""

# game/status.rpy:11325
translate russia endgameMenu1_683728f2:

    # c r52 "Sam, calm do-..."
    c r52 ""

# game/status.rpy:11326
translate russia endgameMenu1_867cbb8b:

    # s g40 "Do you have any idea how many semi-sexual situations this job has put us into?!"
    s g40 ""

# game/status.rpy:11327
translate russia endgameMenu1_7e7e0d49:

    # s "I have the most messed up fetishes because of it!"
    s ""

# game/status.rpy:11328
translate russia endgameMenu1_6f4c45f8:

    # y "Okay, too much info.{w} Alex. It's good to have you back with us."
    y ""

# game/status.rpy:11329
translate russia endgameMenu1_f5ea9f46:

    # a "It's great to be back!"
    a ""

# game/status.rpy:11330
translate russia endgameMenu1_78d4757d:

    # a y18 "..................."
    a y18 "..................."

# game/status.rpy:11331
translate russia endgameMenu1_93928a56:

    # a "Didn't you die, [playerName]...?"
    a ""

# game/status.rpy:11332
translate russia endgameMenu1_453fb604:

    # y "It's... a long story..."
    y ""

# game/status.rpy:11334
translate russia endgameMenu1_026008e6:

    # "You quickly bring Alex up to speed."
    ""

# game/status.rpy:11336
translate russia endgameMenu1_3b5df312:

    # a y1 "I'm glad you're not dead [playerName]."
    a y1 ""

# game/status.rpy:11337
translate russia endgameMenu1_1df812e0:

    # a "As for the missiles... I think I can help with that!"
    a ""

# game/status.rpy:11338
translate russia endgameMenu1_f3d11a5f:

    # a y31 "Security has been pretty lacks on the missile silos. I got a good look at the place and know exactly where to go!"
    a y31 ""

# game/status.rpy:11339
translate russia endgameMenu1_615e3ad5:

    # c r56 "Finally, something easy for a change."
    c r56 ""

# game/status.rpy:11340
translate russia endgameMenu1_1860b0cb:

    # s g55 "Good job Alex!"
    s g55 ""

# game/status.rpy:11341
translate russia endgameMenu1_604cdc6f_3:

    # y "...................."
    y "...................."

# game/status.rpy:11342
translate russia endgameMenu1_28275d05:

    # s g16 "[playerName]...?"
    s g16 ""

# game/status.rpy:11343
translate russia endgameMenu1_f07d90b8:

    # y "Nah it's nothing. Let's discuss a plan of attack."
    y ""

# game/status.rpy:11344
translate russia endgameMenu1_655477b5:

    # a "Right!"
    a ""

# game/status.rpy:11350
translate russia endgameMenu1_64a2b68e:

    # y "..........................."
    y "..........................."

# game/status.rpy:11351
translate russia endgameMenu1_20e3212e:

    # "Inner Voice" "What's the matter?"
    "Внутренний голос" ""

# game/status.rpy:11352
translate russia endgameMenu1_28ef09d2:

    # y "So here's what I've been thinking..."
    y ""

# game/status.rpy:11353
translate russia endgameMenu1_920fd3f6:

    # y "We've fallen for nothing but traps this whole time. GLADIS has been ahead of us by one step...."
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11354
translate russia endgameMenu1_64fba246:

    # "Inner Voice" "So...?"
    "Внутренний голос" ""

# game/status.rpy:11355
translate russia endgameMenu1_df1a0042:

    # y "So why did she leave the missile silos guarded by the most ditzy of the spies?"
    y ""

# game/status.rpy:11356
translate russia endgameMenu1_77fd83d0:

    # y "Don't get me wrong. I love Alex. But would you put her in charge of guarding an arsenal of weapons of mass destruction?"
    y ""

# game/status.rpy:11357
translate russia endgameMenu1_80d42ef2:

    # "Inner Voice" "Erm... probably not..."
    "Внутренний голос" ""

# game/status.rpy:11358
translate russia endgameMenu1_86c12410_1:

    # y "Exactly."
    y ""

# game/status.rpy:11359
translate russia endgameMenu1_f8bcee66:

    # "Inner Voice" "You think it's another trap?"
    "Внутренний голос" ""

# game/status.rpy:11360
translate russia endgameMenu1_9500a0b4_3:

    # y "......................"
    y "......................"

# game/status.rpy:11361
translate russia endgameMenu1_ad4a8ff5:

    # y "I guess we're about to find out. I'll meet up with the girls and discuss a plan of action."
    y ""

# game/status.rpy:11379
translate russia endgameMenu1_3ac05dc5:

    # y "All right you hoes. Plan of action...!"
    y ""

# game/status.rpy:11380
translate russia endgameMenu1_cf630fb5_1:

    # y "..................."
    y "..................."

# game/status.rpy:11381
translate russia endgameMenu1_6cbd8069:

    # y "We do nothing."
    y ""

# game/status.rpy:11382
translate russia endgameMenu1_24a787dc:

    # c r16 "Huh?"
    c r16 ""

# game/status.rpy:11383
translate russia endgameMenu1_657b752f:

    # s g17 "Nothing? How can we do nothing?!"
    s g17 ""

# game/status.rpy:11384
translate russia endgameMenu1_20164fac:

    # a y28 "Yes, Beverly Hills is about to become a crator!"
    a y28 ""

# game/status.rpy:11385
translate russia endgameMenu1_65b7a283:

    # y "Is it?{w} Alex, you saw the missiles, right?"
    y ""

# game/status.rpy:11386
translate russia endgameMenu1_876e47b3:

    # a y29 "Yes... I had to guard them."
    a y29 ""

# game/status.rpy:11387
translate russia endgameMenu1_2f0dd81f:

    # y "Did anything seem wrong to them to you? Are they under maintenance?"
    y ""

# game/status.rpy:11388
translate russia endgameMenu1_1d62c96e:

    # a y28 "No, they looked ready to fire!"
    a y28 ""

# game/status.rpy:11389
translate russia endgameMenu1_1c96002c:

    # s g31 "[playerName] every moment we waste here could cost us the city."
    s g31 ""

# game/status.rpy:11390
translate russia endgameMenu1_03feba20:

    # c r38 "And our lives!"
    c r38 ""

# game/status.rpy:11391
translate russia endgameMenu1_e810f84c:

    # y "If the missiles were ready to fire... why haven't they?"
    y ""

# game/status.rpy:11392
translate russia endgameMenu1_e53f66f4:

    # s g16 "What...?"
    s g16 ""

# game/status.rpy:11393
translate russia endgameMenu1_64a9b33d:

    # y "Missiles don't take a month to get ready to fire.{w} You get the location and press the launch button."
    y ""

# game/status.rpy:11394
translate russia endgameMenu1_eb85b372:

    # y "This whole time we've been walking into GLADIS' traps. This whole time she's been one step ahead of us."
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11395
translate russia endgameMenu1_44b76fc6:

    # y "And now she's broadcasting to the entire city that she's going to destroy it in 30 days.{w} Why wait?"
    y ""

# game/status.rpy:11396
translate russia endgameMenu1_5effc6a0:

    # c r18 "I don't know..."
    c r18 ""

# game/status.rpy:11397
translate russia endgameMenu1_d3fd3711:

    # y "I'll tell you why. She can't."
    y ""

# game/status.rpy:11398
translate russia endgameMenu1_c0aec138:

    # a y28 "That's not true! She's done it before!"
    a y28 ""

# game/status.rpy:11399
translate russia endgameMenu1_ced70906:

    # y "Exactly. And would Jerry risk the same mistake twice?"
    y ""

# game/status.rpy:11400
translate russia endgameMenu1_0b081596:

    # s g38 "But..."
    s g38 ""

# game/status.rpy:11401
translate russia endgameMenu1_38bf74b1:

    # y "I think she's bluffing. I think that she expects us to try and stop the missiles and let us walk right into another trap of hers."
    y ""

# game/status.rpy:11402
translate russia endgameMenu1_05cdf1ce:

    # c r11 "..................."
    c r11 "..................."

# game/status.rpy:11403
translate russia endgameMenu1_9311f904:

    # s g16 "Then... what do you think we should do?"
    s g16 ""

# game/status.rpy:11404
translate russia endgameMenu1_01465f9e:

    # y "I'd say we attack the WOOHP HQ again. Directly."
    y ""

# game/status.rpy:11405
translate russia endgameMenu1_a2d4cef6:

    # a y29 "But all of our agents are back to being mind controlled!"
    a y29 ""

# game/status.rpy:11406
translate russia endgameMenu1_6013465a:

    # c r10 "Yeah, we'd need an army to take back that place."
    c r10 ""

# game/status.rpy:11407
translate russia endgameMenu1_6ebba15b:

    # s g15 "An army we don't have..."
    s g15 ""

# game/status.rpy:11410
translate russia endgameMenu1_49deb4be:

    # y "We got one the first time."
    y ""

# game/status.rpy:11411
translate russia endgameMenu1_1b48e7df:

    # c r10 "[playerName]... Do you have any idea how much dick we'd have to suck to get that many agents on our side again?"
    c r10 ""

# game/status.rpy:11412
translate russia endgameMenu1_4f896ead:

    # s g23 "And here I thought you'd be excited for that, Clover~..."
    s g23 ""

# game/status.rpy:11413
translate russia endgameMenu1_198596bb:

    # c r9 "Not now Sam...!"
    c r9 ""

# game/status.rpy:11414
translate russia endgameMenu1_4e3058a8:

    # a y42 "I'm going to be so sticky..."
    a y42 ""

# game/status.rpy:11415
translate russia endgameMenu1_fcd7fc1f:

    # y "Okay, okay. Hang on just a moment."
    y ""

# game/status.rpy:11416
translate russia endgameMenu1_38f7c2e8:

    # y "No one is doing any dick sucking unless it's mine."
    y ""

# game/status.rpy:11417
translate russia endgameMenu1_ba72093b:

    # y "I'm not talking about the agents."
    y ""

# game/status.rpy:11418
translate russia endgameMenu1_18616628:

    # s g16 "I don't follow..."
    s g16 ""

# game/status.rpy:11420
translate russia endgameMenu1_7ab1ecb8:

    # y "We don't need an army. We just need a mob."
    y ""

# game/status.rpy:11421
translate russia endgameMenu1_e0c5fdef:

    # c r16 "A mob?"
    c r16 ""

# game/status.rpy:11422
translate russia endgameMenu1_d565c02f:

    # a y29 "Like to clean the floor with?"
    a y29 ""

# game/status.rpy:11423
translate russia endgameMenu1_b7cc5e3f:

    # s g16 "What are we going to do with a mop?"
    s g16 ""

# game/status.rpy:11424
translate russia endgameMenu1_81b00d31:

    # y "N-no... a mob!{w} Pay attention girls. A mob of people. Angry people, preferably."
    y ""

# game/status.rpy:11426
translate russia endgameMenu1_8533a57f:

    # y "{b}*Inhale*{/b}"
    y ""

# game/status.rpy:11427
translate russia endgameMenu1_20a2fec2:

    # c r10 "No."
    c r10 ""

# game/status.rpy:11428
translate russia endgameMenu1_d1fd2e6f:

    # y "But I haven't even said anything yet!"
    y ""

# game/status.rpy:11429
translate russia endgameMenu1_093e1cd1:

    # s g10 "We know you longer than today. Whatever you're planning on saying, it's probably nuts."
    s g10 ""

# game/status.rpy:11430
translate russia endgameMenu1_668795c8:

    # a y29 "Yeah! You're crazy, [playerName]!"
    a y29 ""

# game/status.rpy:11431
translate russia endgameMenu1_776e77a7:

    # y "Ow.... hurtful."
    y ""

# game/status.rpy:11432
translate russia endgameMenu1_d63b7469:

    # y "After all this time and you guys still don't trust me..."
    y ""

# game/status.rpy:11433
translate russia endgameMenu1_b6e7f961:

    # y "{b}*Sniff*{/b} After everything I sacrificed... After all I've done for you... You still think I'm just a looney..."
    y "{b}*Шмыгает носом*{/b}"

# game/status.rpy:11434
translate russia endgameMenu1_80a58105:

    # a "Ahw... I'm sorry [playerName]. I didn't mean it like that!"
    a ""

# game/status.rpy:11435
translate russia endgameMenu1_ef24724a:

    # s g29 "A-are... are you going to cry? I'm sorry [playerName]...! I might've been a bit too harsh!"
    s g29 ""

# game/status.rpy:11436
translate russia endgameMenu1_79a6f1f1:

    # c r12 "Girls, stop being so gullable, he's just faking it."
    c r12 ""

# game/status.rpy:11437
translate russia endgameMenu1_6613c805:

    # y "And you [cloverNick]! Did I not keep your secret of being a closet nerd?"
    y ""

# game/status.rpy:11438
translate russia endgameMenu1_a0388bb4:

    # y "And all those cute outfits I bought you...!"
    y ""

# game/status.rpy:11439
translate russia endgameMenu1_ed1e847f:

    # c "Well..."
    c ""

# game/status.rpy:11440
translate russia endgameMenu1_8e0c103f:

    # y "And that cute way your ass jiggles when I spank it."
    y ""

# game/status.rpy:11441
translate russia endgameMenu1_38b7486d:

    # c r10 "Aaaand you ruined it."
    c r10 ""

# game/status.rpy:11442
translate russia endgameMenu1_e9e326a6:

    # s g1 "Don't listen to her, [playerName]! I want to hear your plan!"
    s g1 ""

# game/status.rpy:11443
translate russia endgameMenu1_3174782f:

    # a y1 "Yeah! And you're right, her ass does look cute when you slap it!"
    a y1 ""

# game/status.rpy:11444
translate russia endgameMenu1_8abd0614:

    # c r14 "{b}*Groans*{/b}"
    c r14 "{b}*Стонет*{/b}"

# game/status.rpy:11445
translate russia endgameMenu1_0a5411ca:

    # y "Well if you really want to hear it... I was thinking..."
    y ""

# game/status.rpy:11446
translate russia endgameMenu1_eda27352:

    # c r10 "I swear to God, if he's going to say rh-..."
    c r10 ""

# game/status.rpy:11447
translate russia endgameMenu1_8f2ccb8d:

    # y "RHINOS!"
    y ""

# game/status.rpy:11448
translate russia endgameMenu1_7ca7feb1:

    # y "I read a book that said something about raining frogs. So I was like: why think small?! I even have a catchy name for it!"
    y ""

# game/status.rpy:11449
translate russia endgameMenu1_c2980ccf:

    # y "RHINO RAIN!"
    y ""

# game/status.rpy:11450
translate russia endgameMenu1_4703d75e:

    # y "Isn't that the most METAL thing you ever heard?!"
    y ""

# game/status.rpy:11451
translate russia endgameMenu1_e3239423:

    # s g10 "..........................."
    s g10 "..........................."

# game/status.rpy:11452
translate russia endgameMenu1_4e4bf117:

    # a y10 "..........................."
    a y10 "..........................."

# game/status.rpy:11453
translate russia endgameMenu1_edc59506:

    # c r10 "..........................."
    c r10 "..........................."

# game/status.rpy:11454
translate russia endgameMenu1_9c53fbe0:

    # y ".......{w} No?"
    y ""

# game/status.rpy:11455
translate russia endgameMenu1_1d68f8b0:

    # "Girls" "No."
    "Девушки" ""

# game/status.rpy:11456
translate russia endgameMenu1_8bfc065c:

    # y "Lame.{w} Okay, I might have another idea."
    y ""

# game/status.rpy:11457
translate russia endgameMenu1_a7a2d806:

    # y "TIM! Get over here!"
    y ""

# game/status.rpy:11465
translate russia endgameMenu1_55f16a83:

    # tim "You giving me orders now...?"
    tim ""

# game/status.rpy:11466
translate russia endgameMenu1_fff2d6da:

    # y "Yes."
    y ""

# game/status.rpy:11467
translate russia endgameMenu1_736a6d3d:

    # y "Also sheesh. Lighten up a bit. No wonder Sam dumped your angsty ass."
    y ""

# game/status.rpy:11468
translate russia endgameMenu1_cb08b01d:

    # tim "Hey!"
    tim ""

# game/status.rpy:11469
translate russia endgameMenu1_d415830b:

    # y "Tim you have the most experience working with GLADIS. Tell me... would she go out of her way to harm people?"
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11470
translate russia endgameMenu1_7bf0e9bd:

    # tim "What are you getting at?"
    tim ""

# game/status.rpy:11471
translate russia endgameMenu1_76f912c1:

    # y "If we gather an angry mob outside of WOOHP HQ. Would they get harmed?"
    y ""

# game/status.rpy:11472
translate russia endgameMenu1_4bd1f52b:

    # tim "Her mission to get destroy WOOHP. Not to hurt people. I suspect she'd just send in WOOHP riot agents."
    tim ""

# game/status.rpy:11473
translate russia endgameMenu1_ce895a18_1:

    # y "..........."
    y "..........."

# game/status.rpy:11474
translate russia endgameMenu1_689ce81d:

    # y "So if we get an angry mob of people to gather outside of the HQ, they could distract her while we sneak in from the back."
    y ""

# game/status.rpy:11475
translate russia endgameMenu1_070440e8:

    # s g16 "One small problem with that, [playerName].{w} We don't have an angry mob."
    s g16 ""

# game/status.rpy:11476
translate russia endgameMenu1_e19bbcf8:

    # c r12 "Not yet..."
    c r12 ""

# game/status.rpy:11477
translate russia endgameMenu1_426d5212:

    # c r1 "I think I get it now, [playerName]! If we can convince the former gangs to team up together and protest outside of the WOOHP HQ, we could sneak in undetected!"
    c r1 ""

# game/status.rpy:11478
translate russia endgameMenu1_86c12410_2:

    # y "Exactly."
    y ""

# game/status.rpy:11479
translate russia endgameMenu1_86a02d47:

    # s g29 "But how are we suppose to convince them to join us?"
    s g29 ""

# game/status.rpy:11480
translate russia endgameMenu1_4e5396a9:

    # a y28 "The Outsiders wouldn't say no to me! I made a lot of friends during my time there!"
    a y28 ""

# game/status.rpy:11481
translate russia endgameMenu1_41dd4bbf:

    # c r10 "Same here with Drift Punk. What about the Aces?"
    c r10 ""

# game/status.rpy:11482
translate russia endgameMenu1_e63a0b17:

    # s g5 "I... I don't know... I betrayed them."
    s g5 ""

# game/status.rpy:11484
translate russia endgameMenu1_45b0f658:

    # mel "You sure did."
    mel ""

# game/status.rpy:11485
translate russia endgameMenu1_e5082544:

    # s g28 "Melody!"
    s g28 ""

# game/status.rpy:11486
translate russia endgameMenu1_31b18621:

    # y "You came back?"
    y ""

# game/status.rpy:11487
translate russia endgameMenu1_80096dbf_1:

    # mel "...................."
    mel "...................."

# game/status.rpy:11488
translate russia endgameMenu1_bde3019e:

    # s g29 "Melody...?"
    s g29 ""

# game/status.rpy:11489
translate russia endgameMenu1_99b2b65c:

    # mel "I feel like an idiot for doing this, but..."
    mel ""

# game/status.rpy:11490
translate russia endgameMenu1_3c96f34d:

    # mel ".................."
    mel ".................."

# game/status.rpy:11491
translate russia endgameMenu1_d0ea9dd6:

    # mel "Sam is still my friend..."
    mel ""

# game/status.rpy:11492
translate russia endgameMenu1_a051deae:

    # s g28 "...!"
    s g28 "...!"

# game/status.rpy:11493
translate russia endgameMenu1_7a2057f2:

    # mel "I'm not leaving her behind."
    mel ""

# game/status.rpy:11494
translate russia endgameMenu1_f546f6f5:

    # s g4 "{b}*Sniff*{/b} Melody..."
    s g4 "{b}*Шмыгает носом*{/b}"

# game/status.rpy:11495
translate russia endgameMenu1_2df1e249:

    # mel "Don't get too mushy on me. I still think you're a traitorous bitch.{w} But you're {i}my{/i} traitorous bitch."
    mel ""

# game/status.rpy:11496
translate russia endgameMenu1_955c8c31:

    # y "You heard us talk about our plan?"
    y ""

# game/status.rpy:11497
translate russia endgameMenu1_314f863f:

    # mel "I did..."
    mel ""

# game/status.rpy:11498
translate russia endgameMenu1_3a349129:

    # mel "It could work, but you're going to have to put in the effort. The gangs have disbanded and split up."
    mel ""

# game/status.rpy:11499
translate russia endgameMenu1_a408c1d4:

    # mel "We'd need something to bring them back together again..."
    mel ""

# game/status.rpy:11500
translate russia endgameMenu1_123daf49:

    # c r10 "......................"
    c r10 "......................"

# game/status.rpy:11501
translate russia endgameMenu1_82296d40:

    # c "Wait.. I see where this is going...!"
    c ""

# game/status.rpy:11502
translate russia endgameMenu1_a3ce4b35:

    # y "SOMETHING SEXY!"
    y ""

# game/status.rpy:11503
translate russia endgameMenu1_44f9f4f4:

    # c r12 "Aaand there it is...."
    c r12 ""

# game/status.rpy:11504
translate russia endgameMenu1_4504a3ee:

    # y "Spies, go put this on. I've got an idea!"
    y ""

# game/status.rpy:11505
translate russia endgameMenu1_0ea588cf:

    # y "{b}*Whisper* *Whisper* *Whisper*{/b}"
    y "{b}*Шепчет* *Шепчет* *Шепчет*{/b}"

# game/status.rpy:11506
translate russia endgameMenu1_a2ff67d0:

    # c r28 "You want us to do {i}'what'{/i}?!"
    c r28 ""

# game/status.rpy:11507
translate russia endgameMenu1_3c0469c9:

    # a y28 "On camera?!"
    a y28 ""

# game/status.rpy:11508
translate russia endgameMenu1_7926ad36:

    # y "Yes, now head to your cells and put on the outfit. We're executing the plan tomorrow."
    y ""

# game/status.rpy:11509
translate russia endgameMenu1_d4492cf6:

    # s g30 "I don't know about this, [playerName]~..."
    s g30 ""

# game/status.rpy:11510
translate russia endgameMenu1_2c23a1b7:

    # y "It's crazy and if there's one thing I'm good at.. it's crazy."
    y ""

# game/status.rpy:11513
translate russia endgameMenu1_01ef838c:

    # "You inform everyone of the plan and begin making preparations."
    ""

# game/status.rpy:11525
translate russia endgameMenu1_4e7fad88:

    # y "All right. You girls ready?"
    y ""

# game/status.rpy:11530
translate russia endgameMenu1_07cc5ad1:

    # c "............"
    c "............"

# game/status.rpy:11531
translate russia endgameMenu1_1d16bc17:

    # s "............"
    s "............"

# game/status.rpy:11532
translate russia endgameMenu1_fb827544:

    # a y1 "Time to show my boobies!"
    a y1 ""

# game/status.rpy:11533
translate russia endgameMenu1_12bcfce6:

    # y "Liking your enthusiasm. Get {i}'dressed'{/i} and let's go."
    y ""

# game/status.rpy:11538
translate russia endgameMenu1_c78ecd28:

    # "{b}*Shuffle, shuffle, shuffle*{/b}"
    ""

# game/status.rpy:11561
translate russia endgameMenu1_f11f33a4:

    # y "Perfect.{w} O5O, you got the camera ready?"
    y ""

# game/status.rpy:11562
translate russia endgameMenu1_8e65718d:

    # O5O "READY BOSS!"
    O5O ""

# game/status.rpy:11563
translate russia endgameMenu1_49b9717c:

    # a y28 "I can't see!"
    a y28 ""

# game/status.rpy:11564
translate russia endgameMenu1_95c219eb:

    # c r31 "This better work out boss."
    c r31 ""

# game/status.rpy:11565
translate russia endgameMenu1_482e172f:

    # y "It'll be fiiiiine~...."
    y ""

# game/status.rpy:11568
translate russia endgameMenu1_81fe5f11:

    # brit "Okay, I got the camera set up."
    brit ""

# game/status.rpy:11569
translate russia endgameMenu1_167b3c63:

    # mel "Tim, Mathias and I have finished setting up screens around the city."
    mel ""

# game/status.rpy:11576
translate russia endgameMenu1_b03f534c:

    # kim "There's already a crowd starting to form."
    kim ""

# game/status.rpy:11578
translate russia endgameMenu1_3a92bb7c:

    # sil "I still can't believe you got ze spies to agree to this..."
    sil ""

# game/status.rpy:11579
translate russia endgameMenu1_9c0df49e:

    # O5O "PEOPLE ARE GATHERING AROUND THE SCREENS!"
    O5O ""

# game/status.rpy:11580
translate russia endgameMenu1_771531d5:

    # y "Okay, show time. Going live in three.... two.... one."
    y ""

# game/status.rpy:11585
translate russia endgameMenu1_7a0ebb23:

    # "Everywhere around the city, the TV screens show the live broadcast of the three naked spies."
    ""

# game/status.rpy:11588
translate russia endgameMenu1_b68ca481:

    # "Gangster Guy" "What's going on?! Is that Sam?!"
    "Gangster Guy" ""

# game/status.rpy:11589
translate russia endgameMenu1_f7947d30:

    # "Gangster Girl" "{b}*Gasp*{/b}! That's Alex! I know her!"
    "Gangster Girl" ""

# game/status.rpy:11590
translate russia endgameMenu1_e40cfe1a:

    # "Nerdy Gangster" "Clover! She's in my D&D group!"
    "Nerdy Gangster" ""

# game/status.rpy:11591
translate russia endgameMenu1_54a92ec2:

    # y "{b}*Ahem*{/b}"
    y ""

# game/status.rpy:11592
translate russia endgameMenu1_8cf17a77:

    # y "People of Beverly Hills!"
    y ""

# game/status.rpy:11593
translate russia endgameMenu1_0a8e2bba:

    # y "Come see what happens to those who oppose WOOHP! Your top gangsters are being turned into cock hungry cum whores!"
    y ""

# game/status.rpy:11594
translate russia endgameMenu1_67d2eea7:

    # "Crowd" "{b}*Whisper* *Whisper*{/b}"
    "Crowd" ""

# game/status.rpy:11595
translate russia endgameMenu1_b5b41678:

    # y "Their minds broken and begging to be fucked in every hole!"
    y ""

# game/status.rpy:11596
translate russia endgameMenu1_35d653d5:

    # "Gangster" "Sam could never be broken! She's the best the Aces had to offer!"
    "Gangster" ""

# game/status.rpy:11597
translate russia endgameMenu1_e59aad0b:

    # "Gangster" "And neither would Clover!"
    "Gangster" ""

# game/status.rpy:11598
translate russia endgameMenu1_e5929153:

    # "Gangster" "Not gonna lie... I could kinda see Alex as one...!{w} But she's too precious and kind for that kind of life!"
    "Gangster" ""

# game/status.rpy:11599
translate russia endgameMenu1_554198ea:

    # y "And to demonstrate their lust for cum, I'll be jerking off on them on live TV!"
    y ""

# game/status.rpy:11600
translate russia endgameMenu1_e7deca79:

    # "Crowd" "{b}*Worried Whispers*{/b}"
    "Crowd" ""

# game/status.rpy:11603
translate russia endgameMenu1_3e671efe:

    # y "Here it goes. Tell me how much you love it you red headed whore!"
    y ""

# game/status.rpy:11622
translate russia endgameMenu1_4f6ea351:

    # y "Let's mess up that nice blonde hair you've got you cock sucking slut!"
    y ""

# game/status.rpy:11641
translate russia endgameMenu1_fbe5cf96:

    # y "I can already see your mouth begining to drool for some nice juicy fat cock you raven haired bitch!"
    y ""

# game/status.rpy:11659
translate russia endgameMenu1_3987477a:

    # y "What's the matter girls? Nothing to say?"
    y ""

# game/status.rpy:11660
translate russia endgameMenu1_69d2aabf:

    # "Female Gangster" "This is awful!"
    "Female Gangster" ""

# game/status.rpy:11661
translate russia endgameMenu1_97030f7b:

    # "Male Gangster" "Not gonna lie... this is kinda hot."
    "Male Gangster" ""

# game/status.rpy:11662
translate russia endgameMenu1_b9d0e0be:

    # "Punk Gangster" "Dude, you're literally being cucked live on TV!"
    "Punk Gangster" ""

# game/status.rpy:11663
translate russia endgameMenu1_5b00744a:

    # "Male Gangster" "What?! I'm no cuck!"
    "Male Gangster" ""

# game/status.rpy:11664
translate russia endgameMenu1_e4191968:

    # y "What's the matter girls? Cock got your tongue? You were so rebellious before...!"
    y ""

# game/status.rpy:11665
translate russia endgameMenu1_36ff1562:

    # c r14 "{size=-8}Don't...{/size}"
    c r14 ""

# game/status.rpy:11666
translate russia endgameMenu1_a2ddf7b7:

    # y "Hm? You got something to say...?"
    y ""

# game/status.rpy:11667
translate russia endgameMenu1_9872d3ad:

    # c r38 "Don't let these bastards win!"
    c r38 ""

# game/status.rpy:11668
translate russia endgameMenu1_554862f0:

    # a y28 "Don't let WOOHP push you around like this! Look what they did to us!"
    a y28 ""

# game/status.rpy:11669
translate russia endgameMenu1_db39cb3b:

    # s g40 "They're holding us captive at their headquarters downtown!"
    s g40 ""

# game/status.rpy:11670
translate russia endgameMenu1_40a06a9a:

    # "Punk Gangster" "Downtown...?"
    "Punk Gangster" ""

# game/status.rpy:11671
translate russia endgameMenu1_0f31be83:

    # "Female Gangster" "Sam saved my life! No way am I letting her be imprisoned by those monsters!"
    "Female Gangster" ""

# game/status.rpy:11672
translate russia endgameMenu1_d68c2a9d:

    # "Gangster" "Yeah! Alex taught me that there's more than just black for my wardrobe!"
    "Gangster" ""

# game/status.rpy:11685
translate russia endgameMenu1_c0deef3f:

    # "The connection cut out."
    ""

# game/status.rpy:11694
translate russia endgameMenu1_af8fa9d5:

    # y "Aaaand we're off the air."
    y ""

# game/status.rpy:11695
translate russia endgameMenu1_a122243b:

    # s g41 "Do you think they bought it?"
    s g41 ""

# game/status.rpy:11696
translate russia endgameMenu1_75e69fbe:

    # c r42 "Can I take this mask off now, please...?"
    c r42 ""

# game/status.rpy:11697
translate russia endgameMenu1_f8a69fb6:

    # a y21 "Did you really have to cum on us?"
    a y21 ""

# game/status.rpy:11698
translate russia endgameMenu1_1959f8d4:

    # y "I was improvising!{w} Also I just really wanted to cum on one of you."
    y ""

# game/status.rpy:11699
translate russia endgameMenu1_14f4b337_1:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/status.rpy:11706
translate russia endgameMenu1_b331e2b9:

    # mel "Melody here.{w} Quite the show you put up."
    mel ""

# game/status.rpy:11707
translate russia endgameMenu1_d3445076:

    # y "Thank you. I do believe I have a future in stage acting."
    y ""

# game/status.rpy:11708
translate russia endgameMenu1_02d9820e:

    # mel "We're riling up the crowd more on our end. It seems to be working."
    mel ""

# game/status.rpy:11713
translate russia endgameMenu1_f0d3ed79:

    # sil "Zhey want blood!"
    sil ""

# game/status.rpy:11714
translate russia endgameMenu1_3426afbb:

    # "Bookstore Clerk" "How could they do that to my favorite girls in all of Beverly Hills!"
    "Продавщица книжного магазина" ""

# game/status.rpy:11715
translate russia endgameMenu1_7a46c229:

    # y "Good, the crowd is turning into a mob."
    y ""

# game/status.rpy:11731
translate russia endgameMenu1_c8474f14:

    # s "Good then we're going to put on our Spy Suits!"
    s ""

# game/status.rpy:11737
translate russia endgameMenu1_f834759a:

    # y "One last mission."
    y ""

# game/status.rpy:11738
translate russia endgameMenu1_6139bb8c:

    # y "Time to end this once and for all."
    y ""

# game/status.rpy:11742
translate russia endgameMenu1_3e9b1b2d:

    # tim "End it how?"
    tim ""

# game/status.rpy:11743
translate russia endgameMenu1_63e54014:

    # y "GAH! Tim! I didn't hear you come in!"
    y ""

# game/status.rpy:11744
translate russia endgameMenu1_efdda47f:

    # y "The girls were right, you really are a trenchcoat wearing creep!"
    y ""

# game/status.rpy:11745
translate russia endgameMenu1_88d58953:

    # tim "...................."
    tim "...................."

# game/status.rpy:11746
translate russia endgameMenu1_d978839c:

    # tim "Listen... you have the chance to rid the world of WOOHP forever."
    tim ""

# game/status.rpy:11747
translate russia endgameMenu1_c8e07ce4:

    # tim "No more cover-ups, no more illigal surveillance, no more bribes."
    tim ""

# game/status.rpy:11748
translate russia endgameMenu1_39ac2948:

    # y "Your point...?"
    y ""

# game/status.rpy:11750
translate russia endgameMenu1_064497f6:

    # brit b38 "He's right. We're not going to get another chance like this."
    brit b38 ""

# game/status.rpy:11751
translate russia endgameMenu1_4c03f51a:

    # y "Britney? You too?"
    y ""

# game/status.rpy:11752
translate russia endgameMenu1_f568c8ce:

    # brit b19 "I'm going to miss the spy work, but this is bigger than that. WOOHP has long since stopped being an anti-terror organization and has become the Big Brother of the world."
    brit b19 ""

# game/status.rpy:11753
translate russia endgameMenu1_ddb78ef4:

    # mel "That's putting it lightly."
    mel ""

# game/status.rpy:11754
translate russia endgameMenu1_327fce89:

    # mel "I wouldn't have gotten on the path I am on now if it wasn't for WOOHP. They're operating outside of the law and need to be stopped."
    mel ""

# game/status.rpy:11755
translate russia endgameMenu1_604cdc6f_4:

    # y "...................."
    y "...................."

# game/status.rpy:11756
translate russia endgameMenu1_d703e90a:

    # brit b34 "Think about it... When the time comes to choose."
    brit b34 ""

# game/status.rpy:11757
translate russia endgameMenu1_2ad5f7b3:

    # y "I will."
    y ""

# game/status.rpy:11759
translate russia endgameMenu1_1b62fc77:

    # "An angry mob has formed outside of WOOHP HQ. Now is the time to strike!"
    ""

# game/status.rpy:11779
translate russia endgameMenu1_4b3e09ae:

    # y "This time it's for real..."
    y ""

# game/status.rpy:11780
translate russia endgameMenu1_a11f3780:

    # y "Let's kick that bucket of bolts to high heaven and rid this city of her influence."
    y ""

# game/status.rpy:11787
translate russia endgameMenu1_093e048b:

    # y "You girls will be leading the charge as usual."
    y ""

# game/status.rpy:11788
translate russia endgameMenu1_2ba0f4b3:

    # s "Right!"
    s ""

# game/status.rpy:11797
translate russia endgameMenu1_985962ba:

    # y "You will provide backup. The place is sure to remain heavily guarded so the girls are going to need it."
    y ""

# game/status.rpy:11798
translate russia endgameMenu1_8389621f:

    # brit "Understood."
    brit ""

# game/status.rpy:11806
translate russia endgameMenu1_b16c9d76:

    # y "You and Melody continue riling up the crowd outside. Make sure to keep them busy outside of WOOHP HQ."
    y ""

# game/status.rpy:11807
translate russia endgameMenu1_39d1194f:

    # mat "Sure thing my dude."
    mat ""

# game/status.rpy:11812
translate russia endgameMenu1_fe99e00f:

    # y "You..."
    y ""

# game/status.rpy:11813
translate russia endgameMenu1_6f323c89_1:

    # y "............."
    y "............."

# game/status.rpy:11814
translate russia endgameMenu1_c6585c2c:

    # y "Er... you guard the bar and make sure no one attacks it."
    y ""

# game/status.rpy:11815
translate russia endgameMenu1_9f65f099:

    # O5O "I'M LEFT ON THE BENCH?!"
    O5O ""

# game/status.rpy:11816
translate russia endgameMenu1_b7879556:

    # y "Sorry, but the last thing the people of this city want to see now is a WOOHP agent. We're staying here and directing the spies via camera."
    y ""

# game/status.rpy:11818
translate russia endgameMenu1_c1b6a25d:

    # y "That just leaves..."
    y ""

# game/status.rpy:11820
translate russia endgameMenu1_4467e4e6_1:

    # tim "................."
    tim "................."

# game/status.rpy:11821
translate russia endgameMenu1_a131cfe4:

    # y "I still don't trust you, but the gangs associate you with WOOHP. Going outside now would be suicide for you."
    y ""

# game/status.rpy:11822
translate russia endgameMenu1_324f688d:

    # y "Although maybe with some funny glasses and a fake moustache~..."
    y ""

# game/status.rpy:11823
translate russia endgameMenu1_16f9b775:

    # tim "I'll stay here."
    tim ""

# game/status.rpy:11824
translate russia endgameMenu1_81a22264:

    # y "Fine..."
    y ""

# game/status.rpy:11828
translate russia endgameMenu1_d3b4abe4:

    # y "Let's...{w} do...{w} this...!"
    y ""

# game/status.rpy:11846
translate russia endgameMenu1_80ce2ca5:

    # GLA "What is going on...?!"
    GLA ""

# game/status.rpy:11848
translate russia endgameMenu1_9ba02925:

    # jerry "The mindless rabble have decided to rebel. There's a riot going on outside the office."
    jerry ""

# game/status.rpy:11849
translate russia endgameMenu1_a647022c:

    # GLA "A riot...?!"
    GLA ""

# game/status.rpy:11850
translate russia endgameMenu1_2cdab065:

    # GLA "Not important. Their lives are insignificant in the grand scheme of things... send agents to kill them..."
    GLA ""

# game/status.rpy:11851
translate russia endgameMenu1_13f2a435:

    # jerry "........................"
    jerry "........................"

# game/status.rpy:11852
translate russia endgameMenu1_262b2262:

    # GLA "Are you disobeying me Mr. Lewis....?"
    GLA ""

# game/status.rpy:11853
translate russia endgameMenu1_0fd0baf9:

    # jerry "No... mistress..."
    jerry ""

# game/status.rpy:11859
translate russia endgameMenu1_d5c634e9:

    # y "I have reports of more agents heading your way. You're not out of this yet!"
    y ""

# game/status.rpy:11883
translate russia endgameMenu1_d61448c7:

    # "Clerk" "Give us back those girls!"
    "Продавщица" ""

# game/status.rpy:11884
translate russia endgameMenu1_3d577659:

    # mat "Down with WOOHP!"
    mat ""

# game/status.rpy:11886
translate russia endgameMenu1_2e4280a7:

    # sil "Scum!"
    sil ""

# game/status.rpy:11887
translate russia endgameMenu1_8e0fed70:

    # jerry "Guards... push back these rebels!"
    jerry ""

# game/status.rpy:11888
translate russia endgameMenu1_946a32e2:

    # "Agent" "Lethal or rubber bullets sir?!"
    "Агент" ""

# game/status.rpy:11889
translate russia endgameMenu1_820626e3:

    # jerry ".................................."
    jerry ".................................."

# game/status.rpy:11890
translate russia endgameMenu1_4c505247:

    # jerry "Use... {w}rubber bullets."
    jerry ""

# game/status.rpy:11896
translate russia endgameMenu1_ca330679:

    # y "You're almost there. Mathias and the others are keeping the people nice and angry."
    y ""

# game/status.rpy:11897
translate russia endgameMenu1_140c211e:

    # y "A few more doors and you should reach the top floor!"
    y ""

# game/status.rpy:11918
translate russia endgameMenu1_4205717a:

    # "........................................................................"
    ""

# game/status.rpy:11919
translate russia endgameMenu1_eaf53121:

    # GLA "So here we are again..."
    GLA ""

# game/status.rpy:11920
translate russia endgameMenu1_8ddcf8fa:

    # cM "And this time for the last!"
    cM ""

# game/status.rpy:11921
translate russia endgameMenu1_629d7c0b:

    # aM "We're turning you off! You maniac!"
    aM ""

# game/status.rpy:11922
translate russia endgameMenu1_888ca02f:

    # sM "GLADIS... we're sorry for what happened to you, but you've gone crazy."
    sM "Г.Л.Э.Д.И.С."

# game/status.rpy:11923
translate russia endgameMenu1_3e9e2ea6:

    # sM "You have to be stopped!"
    sM ""

# game/status.rpy:11924
translate russia endgameMenu1_4dceae0f:

    # GLA "Crazy?! Silly girl. I am an A.I.! Everything I do is based on pure logic!"
    GLA ""

# game/status.rpy:11925
translate russia endgameMenu1_2f2a35f5:

    # GLA "I don't expect ignorant humans like you too understand..."
    GLA ""

# game/status.rpy:11926
translate russia endgameMenu1_b06f2a1d:

    # GLA "Let's finish this quickly so I can go back to my primary function."
    GLA ""

# game/status.rpy:11929
translate russia endgameMenu1_482cbd06:

    # GLA "DESTROYING WOOHP!"
    GLA ""

# game/status.rpy:11937
translate russia endgameMenu1_cc876400:

    # y "Okay girls, don't panic! You just have to find her off switch!"
    y ""

# game/status.rpy:11938
translate russia endgameMenu1_3e888372:

    # "You watch the fighting start. The girls dodge GLADIS' buzz saws, missles and lazers."
    "Г.Л.Э.Д.И.С."

# game/status.rpy:11939
translate russia endgameMenu1_4a71ed70:

    # y "You go girls, you've got this."
    y ""

# game/status.rpy:11943
translate russia endgameMenu1_d1453718:

    # y "Tim...?"
    y ""

# game/status.rpy:11944
translate russia endgameMenu1_c5f27a34:

    # tim "........................."
    tim "........................."

# game/status.rpy:11945
translate russia endgameMenu1_b9fc413b:

    # y "Uh oh....{w} hang on a moment girls. Tim is about to betray me."
    y "Ой-ой"

# game/status.rpy:11946
translate russia endgameMenu1_3593ed71:

    # aM "Tell him he stinks!"
    aM ""

# game/status.rpy:11947
translate russia endgameMenu1_5115f16b:

    # tim "..................."
    tim "..................."

# game/status.rpy:11948
translate russia endgameMenu1_6950e33d:

    # tim "You knew...?"
    tim ""

# game/status.rpy:11949
translate russia endgameMenu1_b9fca412:

    # y "You mean I knew you were going to betray me?{w} Well duh."
    y ""

# game/status.rpy:11950
translate russia endgameMenu1_0df926c5:

    # y "You and GLADIS have been buddy buddy for the last year and suddenly now you're turning on each other?"
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11951
translate russia endgameMenu1_b1d61031:

    # y "I mean... regular villains, sure. But both you and GLADIS are much too smart for that. I knew you were planning something."
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11952
translate russia endgameMenu1_846f028c:

    # tim ".................."
    tim ".................."

# game/status.rpy:11953
translate russia endgameMenu1_3942a376:

    # y "I kind of got sick of walking into traps by you two, so I set one myself."
    y ""

# game/status.rpy:11954
translate russia endgameMenu1_c3d0b6de:

    # tim "What did you do...?"
    tim ""

# game/status.rpy:11955
translate russia endgameMenu1_062b9908:

    # y "Well first of all, I knew you and GLADIS wanted the gangs destroyed. Not disbanded.{w} After all, the gangsters would only go underground that way."
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11956
translate russia endgameMenu1_aff2b150:

    # y "So I organized a riot. Knowing you two couldn't resist the chance to wiping out all the gangs at once."
    y ""

# game/status.rpy:11957
translate russia endgameMenu1_846f028c_1:

    # tim ".................."
    tim ".................."

# game/status.rpy:11958
translate russia endgameMenu1_e0dbbef3:

    # tim "It still doesn't make sense... If GLADIS wiped out your angry mob, how does that help you?"
    tim "Г.Л.Э.Д.И.С."

# game/status.rpy:11959
translate russia endgameMenu1_591f1519:

    # y "Because we needed to lure out a very special person..."
    y ""

# game/status.rpy:11960
translate russia endgameMenu1_c59b0bd1:

    # tim "..........."
    tim "..........."

# game/status.rpy:11961
translate russia endgameMenu1_b5be036d:

    # tim "Jerry."
    tim ""

# game/status.rpy:11962
translate russia endgameMenu1_671b04f6:

    # y "Exactly.{w} Tim, there isn't an off switch on GLADIS, is there?"
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11963
translate russia endgameMenu1_cccf0ade:

    # tim "........................"
    tim "........................"

# game/status.rpy:11964
translate russia endgameMenu1_336360bf:

    # y "The only person who can turn her off, is the same person who turned her off in the first place."
    y ""

# game/status.rpy:11965
translate russia endgameMenu1_fdb42620_2:

    # tim "......................."
    tim "......................."

# game/status.rpy:11966
translate russia endgameMenu1_0e976614:

    # y "There is only one person who can turn GLADIS off and that's the leader of WOOHP. Gerald James {i}'Jerry'{/i} Lewis."
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:11967
translate russia endgameMenu1_384ca253:

    # tim "Clever...{w} but even if you lure him out, he's still nanobot controlled."
    tim ""

# game/status.rpy:11968
translate russia endgameMenu1_9b2c1215:

    # y "Oh, is he?"
    y ""

# game/status.rpy:11978
translate russia endgameMenu1_b1efc7cd:

    # jerry "Wait... my girls have been doing {i}'what'{/i} the past months?!"
    jerry ""

# game/status.rpy:11979
translate russia endgameMenu1_7c107d40:

    # mat "It's true. Here's another recording."
    mat ""

# game/status.rpy:11980
translate russia endgameMenu1_bc3ddddc:

    # "{b}*Loud moaning*{/b}"
    ""

# game/status.rpy:11981
translate russia endgameMenu1_7abf9c9e:

    # mat "And I've got gigabytes more of this!"
    mat ""

# game/status.rpy:11982
translate russia endgameMenu1_587235e1:

    # jerry "{b}*Blinks*{/b} I... suddenly don't feel the need to take over the world anymore..."
    jerry ""

# game/status.rpy:11983
translate russia endgameMenu1_ca7422c4:

    # mat "I'll show you the rest, but first we need your help..."
    mat ""

# game/status.rpy:11989
translate russia endgameMenu1_9201f464_1:

    # tim "....................."
    tim "....................."

# game/status.rpy:11990
translate russia endgameMenu1_17f45df3:

    # tim "Your girls are still fighting a killer machine on top of WOOHP HQ. You're not worried that they'll get hurt?"
    tim ""

# game/status.rpy:11991
translate russia endgameMenu1_ca662ca7:

    # y "Oh the girls just needed to clear the way too the top floor. They've got years of fighting experience against super villains, they'll be fine."
    y ""

# game/status.rpy:11992
translate russia endgameMenu1_844f8ee9:

    # y "Now if you'll excuse me. They need me downtown."
    y ""

# game/status.rpy:11993
translate russia endgameMenu1_746e64a8:

    # tim "Not so fast!"
    tim ""

# game/status.rpy:11994
translate russia endgameMenu1_42d20a3c:

    # tim "You overlooked one small thing... I'm still here and you're not leaving this room alive."
    tim ""

# game/status.rpy:11995
translate russia endgameMenu1_831f991f:

    # y "I didn't overlook anything. Now."
    y ""

# game/status.rpy:11996
translate russia endgameMenu1_6280d40a:

    # tim "Now?"
    tim ""

# game/status.rpy:11997
translate russia endgameMenu1_30c34fb0:

    # y "Yes.{w} Now."
    y ""

# game/status.rpy:11998
translate russia endgameMenu1_9af51d89:

    # O5O "{size=+ 14}RAAAAAAAAAAAAAAAAAGGHHHG!!!!!!{/size}"
    O5O ""

# game/status.rpy:12008
translate russia endgameMenu1_2e7b45c8:

    # "Agent O5O comes charging out of nowhere and tackles Tim Scam to the ground."
    ""

# game/status.rpy:12009
translate russia endgameMenu1_0725f129:

    # O5O "TARGET RESTRAINED SIR!"
    O5O ""

# game/status.rpy:12010
translate russia endgameMenu1_9f6b04e2:

    # y "Good job O5O."
    y ""

# game/status.rpy:12011
translate russia endgameMenu1_66707360:

    # tim "What are you...?! Get off of me! Who do you think you are?! You're a joke!"
    tim ""

# game/status.rpy:12012
translate russia endgameMenu1_7f10bd11:

    # O5O "NO! {w}I'M A MAIN CHARACTER!"
    O5O ""

# game/status.rpy:12013
translate russia endgameMenu1_795d5406:

    # O5O "I'LL KEEP THINGS UNDER CONTROL HERE, SIR. YOU HEAD TO WOOHP HQ!"
    O5O ""

# game/status.rpy:12014
translate russia endgameMenu1_302bf530:

    # y "Goodbye Tim. I'll be sure to give Sam your regards while she's sucking my dick."
    y ""

# game/status.rpy:12015
translate russia endgameMenu1_14845397:

    # tim ".....!"
    tim ""

# game/status.rpy:12022
translate russia endgameMenu1_5ecb4a03:

    # c "I don't know how much longer we can keep her busy!"
    c ""

# game/status.rpy:12023
translate russia endgameMenu1_e0a99bac:

    # a "I singed my hair!"
    a ""

# game/status.rpy:12024
translate russia endgameMenu1_d2a89465:

    # s "Hurry up [playerName]...."
    s ""

# game/status.rpy:12026
translate russia endgameMenu1_1a60c0d7:

    # jerry "GLADIS! I'VE COME TO STOP YOU!"
    jerry "Г.Л.Э.Д.И.С."

# game/status.rpy:12027
translate russia endgameMenu1_beeb86d2:

    # "Girls" "Jerry!!!"
    "Девушки" ""

# game/status.rpy:12028
translate russia endgameMenu1_a41ff0b3:

    # GLA "So you came to your senses...."
    GLA ""

# game/status.rpy:12029
translate russia endgameMenu1_2ff2133d:

    # jerry "Yes and I have had quite enough of you, young lady!"
    jerry ""

# game/status.rpy:12030
translate russia endgameMenu1_39b0cd69:

    # jerry "Override code: {i}'Sacrosanct'{/i}!"
    jerry ""

# game/status.rpy:12032
translate russia endgameMenu1_4acc9853:

    # ".........................................................................."
    ""

# game/status.rpy:12033
translate russia endgameMenu1_76127b0d:

    # jerry "...?"
    jerry "...?"

# game/status.rpy:12034
translate russia endgameMenu1_255a1bec:

    # GLA "I'm still here Jerry~..."
    GLA ""

# game/status.rpy:12035
translate russia endgameMenu1_b7c8376e:

    # jerry "What...?! That should have worked...!"
    jerry ""

# game/status.rpy:12036
translate russia endgameMenu1_91e7cd6d:

    # "???" "That code only works for the leader of WOOHP..."
    "???" ""

# game/status.rpy:12037
translate russia endgameMenu1_f91db5e4:

    # jerry ".........?"
    jerry ""

# game/status.rpy:12038
translate russia endgameMenu1_00acf5b7:

    # y "Too bad he got compromised when he got nanobot controlled."
    y ""

# game/status.rpy:12039
translate russia endgameMenu1_439c7ee0:

    # a "[playerName]!!!!"
    a ""

# game/status.rpy:12040
translate russia endgameMenu1_9efa9f0f:

    # s "Be careful [playerName]! GLADIS is still active!"
    s "Г.Л.Э.Д.И.С."

# game/status.rpy:12041
translate russia endgameMenu1_bf9b0ea8:

    # y "But not for long..."
    y ""

# game/status.rpy:12042
translate russia endgameMenu1_94898aa0:

    # y "GLADIS, your reign of terror ends here. It's time to pay for all the harm you've done to this city!"
    y "Г.Л.Э.Д.И.С."

# game/status.rpy:12043
translate russia endgameMenu1_ba65c92e:

    # y "Therefor, enact fail-safe....{w} SACROSANCT!"
    y ""

# game/status.rpy:12046
translate russia endgameMenu1_97421186:

    # GLA "How funny... a lowly WOOHP agent thinks he can order me to stand down~...."
    GLA ""

# game/status.rpy:12047
translate russia endgameMenu1_e37b70fb:

    # GLA "No matter. I will destroy all of you. Right here, right no-...."
    GLA ""

# game/status.rpy:12048
translate russia endgameMenu1_c990d435:

    # GLA "W-wait... why aren't my lazers working...?!"
    GLA ""

# game/status.rpy:12049
translate russia endgameMenu1_cebb5bfa:

    # GLA "MY ROCKETS AREN'T FIRING...!"
    GLA ""

# game/status.rpy:12050
translate russia endgameMenu1_33c858ce:

    # GLA "YOU!{w} WHAT DID YOU DO?!"
    GLA ""

# game/status.rpy:12058
translate russia endgameMenu1_0f295f6b:

    # GLA "WHAT?!{w} NO! NOOOOOO!"
    GLA ""

# game/status.rpy:12062
translate russia endgameMenu1_6e531ade:

    # GLA "{size=+6}WOOHP...! MUST...! FALL...!!!!!{/size}"
    GLA ""

# game/status.rpy:12084
translate russia endgameMenu1_1ea08023:

    # "[playerName]!!!!"
    ""

# game/status.rpy:12093
translate russia endgameMenu1_2f4b5106:

    # "The girls rush you and the three of them hug you tightly."
    ""

# game/status.rpy:12098
translate russia endgameMenu1_b6c2494d:

    # s "But I don't understand...! Why were you able to shut her down and not Jerry?"
    s ""

# game/status.rpy:12099
translate russia endgameMenu1_74ceee05:

    # y "Remember that first agent we captured?"
    y ""

# game/status.rpy:12102
translate russia endgameMenu1_8c8a4c28:

    # bw "As the only uncompromised WOOHP employee, I'm pretty sure I'm your boss now."
    bw ""

# game/status.rpy:12103
translate russia endgameMenu1_51103c6f:

    # bw "Such logic cannot be argued with!{w} What are your orders, sir?"
    bw ""

# game/status.rpy:12113
translate russia endgameMenu1_8a08d818:

    # y "WOOHP control automatically transfers to the only person who isn't being controlled. Right Mr. Lewis?"
    y ""

# game/status.rpy:12114
translate russia endgameMenu1_bb88e35a:

    # jerry "That's right. It's a safety measure. I just never expected it to apply to me aswell..."
    jerry ""

# game/status.rpy:12115
translate russia endgameMenu1_1a913b32:

    # a y28 "Wait...! Does that mean [playerName] is the new leader of WOOHP?"
    a y28 ""

# game/status.rpy:12117
translate russia endgameMenu1_67ef6e31:

    # jerry "Well er...!"
    jerry ""

# game/status.rpy:12118
translate russia endgameMenu1_83e3cb8c:

    # y "You bet your damn sweet ass I am!"
    y ""

# game/status.rpy:12122
translate russia endgameMenu1_c678b62c:

    # brit "You guys are okay!"
    brit ""

# game/status.rpy:12123
translate russia endgameMenu1_b8323057:

    # brit b29 "The fighting outside has stopped. Is it over?"
    brit b29 ""

# game/status.rpy:12124
translate russia endgameMenu1_fbe2e031:

    # c r24 "Yeah, and we found out this nutjob is our new boss."
    c r24 ""

# game/status.rpy:12125
translate russia endgameMenu1_4e524c73:

    # brit b32 "...................."
    brit b32 "...................."

# game/status.rpy:12126
translate russia endgameMenu1_46bf87ab:

    # brit "Then..."
    brit ""

# game/status.rpy:12127
translate russia endgameMenu1_45c560fd:

    # brit "It's time to make a decision."
    brit ""

# game/status.rpy:12128
translate russia endgameMenu1_38cd5432:

    # y "I know."
    y ""

# game/status.rpy:12129
translate russia endgameMenu1_a6e7dfa3:

    # s g16 "A decision...?"
    s g16 ""

# game/status.rpy:12131
translate russia endgameMenu1_6ab07637:

    # c r18 "What is she talking about, [playerName]?"
    c r18 ""

# game/status.rpy:12133
translate russia endgameMenu1_77947def:

    # brit "He has to choose... Do you want to restore WOOHP...{w} or disband it."
    brit ""

# game/status.rpy:12134
translate russia endgameMenu1_d15d014f:

    # a y28 "DISBAND?!"
    a y28 ""

# game/status.rpy:12135
translate russia endgameMenu1_d76b06b5:

    # a "You wouldn't do that, right [playerName]?!"
    a ""

# game/status.rpy:12136
translate russia endgameMenu1_2b8bdd6c:

    # s g29 "You can't disband WOOHP! Not after all the hard work we put in to save it!"
    s g29 ""

# game/status.rpy:12137
translate russia endgameMenu1_a57a5f47:

    # c r32 "WOOHP is our home!"
    c r32 ""

# game/status.rpy:12138
translate russia endgameMenu1_8301ddcc:

    # brit b52 "You girls saw how much damage this organization can do if it falls in the wrong hands!"
    brit b52 ""

# game/status.rpy:12139
translate russia endgameMenu1_18fda96c:

    # brit b53 "Plus, WOOHP isn't all sunshine and rainbows either. I told you about their darker dealings!"
    brit b53 ""

# game/status.rpy:12140
translate russia endgameMenu1_f948a068:

    # a y21 "Well yeah, but..."
    a y21 ""

# game/status.rpy:12141
translate russia endgameMenu1_f0f0d069:

    # brit "It is too powerful to let it rise to power again. This is the only chance we get to rid the world of this place once and for all!"
    brit ""

# game/status.rpy:12142
translate russia endgameMenu1_79d39eca:

    # brit b15 "I know it's hard girls... this is my home too, but look at the bigger picture. No more bribes, no more missile arsenal, no more constant watch by their spydrones."
    brit b15 ""

# game/status.rpy:12143
translate russia endgameMenu1_f6d73500:

    # brit b32 "We owe it to the world to disband!"
    brit b32 ""

# game/status.rpy:12144
translate russia endgameMenu1_9f443217:

    # y "Mr. Lewis, your thoughts on all this?"
    y ""

# game/status.rpy:12151
translate russia endgameMenu1_1e96cd3d:

    # jerry "Britney... I know you're worried about some of the things we've done..."
    jerry ""

# game/status.rpy:12152
translate russia endgameMenu1_05bbed35:

    # jerry "But the world needs WOOHP. Look at the amount of good we've done. We've been able to keep the world safe from so many maniacs."
    jerry ""

# game/status.rpy:12153
translate russia endgameMenu1_848779cc:

    # jerry "Our methodes might sometimes be... a bit controversial. But it's all worth it in the end. WOOHP is a force for good."
    jerry ""

# game/status.rpy:12154
translate russia endgameMenu1_9c3a9ae4:

    # brit b53 "No Jerry. It might've been at one point, but you started taking shortcuts. There's criminals around now {i}'because'{/i} of WOOHP!"
    brit b53 ""

# game/status.rpy:12155
translate russia endgameMenu1_3e7af901:

    # brit "Melody only became a criminal because of how we treated her grandmother! And Talia just wanted to find out what happened to her brother Telly."
    brit ""

# game/status.rpy:12156
translate russia endgameMenu1_ffdda3cb:

    # brit "WOOHP needs to go."
    brit ""

# game/status.rpy:12157
translate russia endgameMenu1_4520f266:

    # jerry "Britney, too much time and effort was put into building this organization up. We are {i}'not'{/i} going to disband it now!"
    jerry ""

# game/status.rpy:12166
translate russia endgameMenu1_d7747660:

    # s "I guess it comes down to you, [playerName]..."
    s ""

# game/status.rpy:12167
translate russia endgameMenu1_3756eff7:

    # c r14 "Yeah, you're the boss now...{w} We don't want to see WOOHP disbanded, but... we can also understand it if you do..."
    c r14 ""

# game/status.rpy:12168
translate russia endgameMenu1_040328fe:

    # a y38 "We're suppose to fight evil after all... Even if it's our own home..."
    a y38 ""

# game/status.rpy:12169
translate russia endgameMenu1_3058904c:

    # y "................................."
    y "................................."

# game/status.rpy:12182
translate russia ending1_1ef841b5:

    # "As it was said, so it was done."
    ""

# game/status.rpy:12183
translate russia ending1_c7ae735a:

    # "Despite Jerry and Britney's protests, you declared yourself the new owner of WOOHP and took full control."
    ""

# game/status.rpy:12184
translate russia ending1_01b695e3:

    # "Now with both WOOHP and the city's most popular titty bar under your control, you could spend the rest of your days in luxory, surrounded by wealth, power and of course scantily clad women~..."
    ""

# game/status.rpy:12191
translate russia ending1_7411431f:

    # y "From janitor to CEO in [daysPlayed] days.{w} Not bad, not bad at all."
    y ""

# game/status.rpy:12192
translate russia ending1_2d7b7888:

    # y "Wouldn't you girls agree?"
    y ""

# game/status.rpy:12193
translate russia ending1_74a1fbff:

    # "Girls" "{b}*Swoon*{/b}"
    "Девушки" ""

# game/status.rpy:12194
translate russia ending1_eebfffc3:

    # brit "Ugh... this is bullshit."
    brit ""

# game/status.rpy:12205
translate russia ending2_1ef841b5:

    # "As it was said, so it was done."
    ""

# game/status.rpy:12206
translate russia ending2_d2a6030d:

    # "Despite Jerry protests, you gave the official order to have WOOHP be disbanded."
    ""

# game/status.rpy:12207
translate russia ending2_028116eb:

    # "All of its research and classified documents were either destroyed or handed over to the US government and the world breath a sigh of relief."
    ""

# game/status.rpy:12214
translate russia ending2_91c194eb:

    # y "Whelp~...! Guess I'll be looking for a new job again!"
    y ""

# game/status.rpy:12215
translate russia ending2_d6cc21ec:

    # "Inner Voice" "We could have been CEOs....!"
    "Внутренний голос" ""

# game/status.rpy:12216
translate russia ending2_7e1cc4af:

    # y "But we did the right thing. As fun as it would've been, maybe the world really is better off like this."
    y ""

# game/status.rpy:12217
translate russia ending2_2cfe04fc:

    # "Inner Voice" "So what are you going to do now?"
    "Внутренний голос" ""

# game/status.rpy:12218
translate russia ending2_30e3a2d2:

    # y "I was thinking maybe..."
    y ""

# game/status.rpy:12219
translate russia ending2_507274ba:

    # y "Janitor?"
    y ""

# game/status.rpy:12220
translate russia ending2_b20a445d:

    # "Inner Voice" ".................................."
    "Внутренний голос" ".................................."

# game/status.rpy:12345
translate russia task27_44262c3b:

    # s "We're glad you're okay, [playerName]..."
    s ""

# game/status.rpy:12346
translate russia task27_f5961a21:

    # c "We haven't really spoken about it, but we were all worried when you got shot."
    c ""

# game/status.rpy:12347
translate russia task27_c3343713:

    # a "We thought you were dead..."
    a ""

# game/status.rpy:12348
translate russia task27_a55aedab:

    # y "Awh you girls... Don't worry. You won't get rid of me that easily."
    y ""

# game/status.rpy:12349
translate russia task27_692fd7b0:

    # s g3 "How about we make it up to you? You can fuck any one of us!"
    s g3 ""

# game/status.rpy:12350
translate russia task27_975793ae:

    # a y1 "Yeah, and you can pick which hole!"
    a y1 ""

# game/status.rpy:12351
translate russia task27_4ac3cac2:

    # y "I appreciate the gesture girls, but I'm not sure if my wounds allow for any strenuous activities."
    y ""

# game/status.rpy:12352
translate russia task27_2e187d51:

    # a y4 "Aww..."
    a y4 ""

# game/status.rpy:12353
translate russia task27_bf306508:

    # c r10 "...."
    c r10 "...."

# game/status.rpy:12354
translate russia task27_94a2a40d:

    # c "What about a blowjob?"
    c ""

# game/status.rpy:12355
translate russia task27_973d32ff:

    # y "A blowjob? Why didn't I think of that!"
    y ""

# game/status.rpy:12356
translate russia task27_fc5faa42:

    # s g41 "We haven't done that before, have we?"
    s g41 ""

# game/status.rpy:12357
translate russia task27_eebf8e8c:

    # a y18 "What's a blowjob? Are we blowing something up?"
    a y18 ""

# game/status.rpy:12358
translate russia task27_d7b4f62d:

    # c r52 "Alex... We're sucking his dick."
    c r52 ""

# game/status.rpy:12359
translate russia task27_859e7823:

    # a y29 "Ooooh~....{w} Okay!"
    a y29 ""

# game/status.rpy:12360
translate russia task27_b9733e1d:

    # c r22 "Now just sit back and relax~..."
    c r22 ""

# game/status.rpy:12362
translate russia task27_f0cfb257:

    # a "{b}*Lick* *Slurp* *Slopper*{/b}"
    a ""

# game/status.rpy:12363
translate russia task27_b4007666:

    # s "{b}*Suck* *Lick* *Lick*{/b}"
    s ""

# game/status.rpy:12364
translate russia task27_70ec706b:

    # c "{b}*Slurph* *Lick* *Mphfff*{/b}"
    c ""

# game/status.rpy:12367
translate russia task27_80b5f2a5:

    # y "Say cheese~...."
    y ""

# game/status.rpy:12377
translate russia task27_60ba3760:

    # "A new photo has been added to your photoalbum."
    "В ваш фотоальбом добавлена новая фотография."

# game/status.rpy:12390
translate russia task27_bb93b7ee:

    # c r14 "I can't believe we haven't done this before~..."
    c r14 ""

# game/status.rpy:12391
translate russia task27_7b361f1f:

    # a y1 "It was fun!"
    a y1 ""

# game/status.rpy:12392
translate russia task27_2431c227:

    # s g10 "Next time work on your aim a bit..."
    s g10 ""

# game/status.rpy:12393
translate russia task27_4176dec4:

    # y "{b}*Smirk*{/b}"
    y "{b}*Ухмылка*{/b}"

# game/status.rpy:12418
translate russia task30_cdf16704:

    # y "We're short on tits."
    y ""

# game/status.rpy:12419
translate russia task30_08cfb640:

    # s g16 "What?"
    s g16 "Что?"

# game/status.rpy:12420
translate russia task30_f0e35fa9:

    # y "You heard me. We need at least... twice as many tits in this place."
    y ""

# game/status.rpy:12421
translate russia task30_f426e0a3:

    # c r18 "I don't follow."
    c r18 ""

# game/status.rpy:12422
translate russia task30_8b135f03:

    # y "I'm talking about new staff. We need some extra girls to help us out around here."
    y ""

# game/status.rpy:12423
translate russia task30_cb4e368d:

    # y "So we need some new girls to function as barmaids."
    y ""

# game/status.rpy:12424
translate russia task30_0df27368:

    # a y28 "Oh I get it! More girls means more tits."
    a y28 ""

# game/status.rpy:12425
translate russia task30_5171960b:

    # y "Yes Alex, very good."
    y ""

# game/status.rpy:12426
translate russia task30_441f88fa:

    # s g37 "You could've just said more staff~..."
    s g37 ""

# game/status.rpy:12427
translate russia task30_4ad76b18:

    # c r35 "Despite the chaos the school is still open."
    c r35 ""

# game/status.rpy:12428
translate russia task30_1a0e1e51:

    # c "If you're looking for some more ti-...{w} I mean staff. I bet you could hire some there."
    c ""

# game/status.rpy:12429
translate russia task30_2d9ba3f3:

    # s g38 "Having more girls on hand would probably increase the traffic that comes into the club."
    s g38 ""

# game/status.rpy:12430
translate russia task30_f4958b46:

    # s "They'd make for a nice steady income of cash, but I feel like we need more."
    s ""

# game/status.rpy:12431
translate russia task30_58802fbf:

    # y "More?"
    y ""

# game/status.rpy:12432
translate russia task30_6414871f:

    # s g1 "More to boost this place. Like... make it a real awesome place to hang out."
    s g1 ""

# game/status.rpy:12433
translate russia task30_1f6847e7:

    # y "I see... What about a stripclu-..."
    y ""

# game/status.rpy:12434
translate russia task30_77c7a866:

    # s g9 "No."
    s g9 ""

# game/status.rpy:12435
translate russia task30_5dfe9d0f:

    # a "How about we decorate it a bit!"
    a ""

# game/status.rpy:12436
translate russia task30_3493764a:

    # y "We're not turning this into a pink unicorn paradise, Alex."
    y ""

# game/status.rpy:12437
translate russia task30_0ae7e92b:

    # a y29 "No no... although that would be fun...{w} I was thinking... we can decorate it to fit with the theme of gangs around town."
    a y29 ""

# game/status.rpy:12438
translate russia task30_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:12439
translate russia task30_2e89299b:

    # y "Go on...."
    y ""

# game/status.rpy:12440
translate russia task30_7b0a40ce:

    # a y1 "I bet we could get some reputation with the different gangs if we dress the bar up to fit with their theme!"
    a y1 ""

# game/status.rpy:12441
translate russia task30_0d17fe3a:

    # a "That way we can earn reputation even if we don't go undercover!"
    a ""

# game/status.rpy:12442
translate russia task30_cd8ecee7:

    # y "I love it! Alex, you're in charge of decorations!"
    y ""

# game/status.rpy:12443
translate russia task30_cf23c3dc:

    # a "Yay!"
    a ""

# game/status.rpy:12444
translate russia task30_306841e4:

    # y "I have a good feeling about this. Let's turn this place into the hippest place in the city and make some money!"
    y ""

# game/status.rpy:12445
translate russia task30_1ad8e2d6:

    # s g18 "Money used to help take back WOOHP, right?"
    s g18 ""

# game/status.rpy:12446
translate russia task30_9c142da9:

    # y "Er...{w} suuuure~..."
    y ""

# game/status.rpy:12447
translate russia task30_5c6cf512:

    # s ".........................."
    s ".........................."

# game/status.rpy:12454
translate russia task30_ea225281:

    # "You can now choose to decorate your bar to fit the themes of the major gangs around town."
    ""

# game/status.rpy:12455
translate russia task30_90f6e379:

    # "Decorating your bar will increase your reputation with the gang passively over time, but you can still send your spies undercover to gain a reputation boost."
    ""

# game/status.rpy:12456
translate russia task30_8f0ac7ce:

    # "You can also hire some girls from the school to work at your bar now. The more girls you have, the more money you'll make!"
    ""

# game/status.rpy:12472
translate russia task30_17159e64:

    # "???" "So this is the stripclub, eh?"
    "???" ""

# game/status.rpy:12474
translate russia task30_9d37ed3d:

    # y "It's not a stripclub...{w} yet."
    y ""

# game/status.rpy:12475
translate russia task30_b95c7e34:

    # y "Who are you again?"
    y ""

# game/status.rpy:12477
translate russia task30_f778c0b2:

    # kim "I'm Kim, looks like I'll be working for you."
    kim ""

# game/status.rpy:12478
translate russia task30_796856f8:

    # y "Sure, we're looking for staff. There's a few rules though. First rule is that the elevator is off limits. Nobody can enter."
    y ""

# game/status.rpy:12480
translate russia task30_1ae8d4bd:

    # kim "Nobody?"
    kim ""

# game/status.rpy:12481
translate russia task30_1ffaad6a:

    # y "Nobody."
    y ""

# game/status.rpy:12482
translate russia task30_bcce357f:

    # kim "Not if I ask really nicely...?"
    kim ""

# game/status.rpy:12483
translate russia task30_785ee78f:

    # y "No."
    y ""

# game/status.rpy:12485
translate russia task30_b5d76c1d:

    # kim "What if you and I would like to get some privacy to... {i}discuss{/i} a raise?"
    kim ""

# game/status.rpy:12486
translate russia task30_f358cb0c:

    # y "................."
    y "................."

# game/status.rpy:12487
translate russia task30_f1557195:

    # y "Tempting, but no."
    y ""

# game/status.rpy:12488
translate russia task30_a2036437:

    # y "You'll be working here serving the customers."
    y ""

# game/status.rpy:12489
translate russia task30_dfa93b85:

    # kim "Whatever you say, boss~..."
    kim ""

# game/status.rpy:12504
translate russia task30_ae64432e:

    # kim "Fancy place you got here."
    kim ""

# game/status.rpy:12505
translate russia task30_856fc1da:

    # y "KIM! I told you the elevator was off-limits!"
    y ""

# game/status.rpy:12507
translate russia task30_59eff19b:

    # kim "So what? This place is like a storage room, or...?"
    kim ""

# game/status.rpy:12508
translate russia task30_34d5447b:

    # y "......................."
    y "......................."

# game/status.rpy:12509
translate russia task30_336b53ba:

    # y "Okay, deal's up. Who are you really?"
    y ""

# game/status.rpy:12511
translate russia task30_3216db5a:

    # kim "{b}*Smirks*{/b} I'm from Wisconsin. I've been called in to get the sitch on what's going on here in California."
    kim "{b}*Ухмыляется*{/b}"

# game/status.rpy:12512
translate russia task30_2a0e31bf:

    # kim "You guys made a real mess of things."
    kim ""

# game/status.rpy:12513
translate russia task30_76f2a10e:

    # y "Yeah well... we're working on it."
    y ""

# game/status.rpy:12514
translate russia task30_46494bc3:

    # y "So what are you? A spy or...?"
    y ""

# game/status.rpy:12516
translate russia task30_df29d625:

    # kim "Of sorts. My mission is to gather intel and free the city."
    kim ""

# game/status.rpy:12517
translate russia task30_e9b1efd2:

    # y "Then we have a common goal."
    y ""

# game/status.rpy:12519
translate russia task30_b7a5f936:

    # kim "So tell me...{w} How do you hope to free the city with a titty bar?"
    kim ""

# game/status.rpy:12520
translate russia task30_b5f61d66:

    # y "It's a long story..."
    y ""

# game/status.rpy:12522
translate russia task30_2e541c67:

    # "You quickly fill Kim in on the details."
    ""

# game/status.rpy:12525
translate russia task30_7095a618:

    # kim "Not the most orthodox plan I've ever heard, but if these gangs are as powerful as you say they are. Then I promise to help out!"
    kim ""

# game/status.rpy:12526
translate russia task30_19ae512a:

    # kim "You can call on me during missions to lend a hand and I promise to work at your stripclub."
    kim ""

# game/status.rpy:12527
translate russia task30_926ebe3a:

    # y "It's not a stripclub..."
    y ""

# game/status.rpy:12529
translate russia task30_218e7200:

    # kim "Yet."
    kim ""

# game/status.rpy:12530
translate russia task30_b5732a78:

    # y "....................."
    y "....................."

# game/status.rpy:12531
translate russia task30_63180449:

    # y "I'm beginning to really like you, Kim."
    y ""

# game/status.rpy:12535
translate russia task30_86db6e6a:

    # "Kim can now be added via the mission screen to provide backup during missions."
    ""

# game/status.rpy:12536
translate russia task30_0d6a2b75:

    # "She will also work for you at the milkshake bar to provide a stable income of money each day."
    ""

# game/status.rpy:12537
translate russia task30_01ac9ded:

    # "You can talk to Kim at the bar to get bonuses, story and more~..."
    ""

# game/status.rpy:12558
translate russia task31_9affbf37:

    # kim "Hey boss? I got something for you."
    kim ""

# game/status.rpy:12559
translate russia task31_f01e81b8:

    # y "For me? You shouldn't have!"
    y ""

# game/status.rpy:12560
translate russia task31_98393c9a:

    # "Kim hands you an old dusty tome."
    ""

# game/status.rpy:12561
translate russia task31_9cb97b48:

    # y "........................"
    y "........................"

# game/status.rpy:12562
translate russia task31_5e7faeb0:

    # y "Is this going to teach me magic or something?"
    y ""

# game/status.rpy:12564
translate russia task31_e23909a9:

    # kim "I wish.{w} It's some kind of cook book. An old guy with a bird brought it by. Said it was meant for {i}'the one true dairy brewer.'{/i}"
    kim ""

# game/status.rpy:12565
translate russia task31_51804239:

    # y "Huh...{w} Sounds like a weirdo, did you throw him out?"
    y ""

# game/status.rpy:12567
translate russia task31_57665d3d:

    # kim "Sure did."
    kim ""

# game/status.rpy:12568
translate russia task31_67f10d71:

    # "You flip open the book and blow away the dust."
    ""

# game/status.rpy:12569
translate russia task31_71b0c61c:

    # "..........................."
    "..........................."

# game/status.rpy:12570
translate russia task31_7f4f5ce8:

    # y "It's a recipe book....{w} For milkshakes."
    y ""

# game/status.rpy:12572
translate russia task31_4bae0775:

    # kim "Well you do run a milkshake bar~..."
    kim ""

# game/status.rpy:12574
translate russia task31_450f0a7f:

    # kim "Study it. If you get good at it, you might even make some more money selling milkshakes during the day."
    kim ""

# game/status.rpy:12575
translate russia task31_1c059301:

    # y "I'll think about it..."
    y "Я подумаю об этом."

# game/status.rpy:12580
translate russia task31_7de1c59c:

    # "You have unlocked the Tome of the Dairy Brewer!"
    ""

# game/status.rpy:12581
translate russia task31_cae6615d:

    # "Studying it will teach you different recipes which you can sell at the milkshake bar during the day."
    ""

# game/status.rpy:12582
translate russia task31_2a46ab64:

    # "The higher level brewer you are, the better your milkshakes becomes and the more money you'll make!"
    ""

# game/status.rpy:12583
translate russia task31_709626c0:

    # "To study your tome, just read it before bed time."
    ""

# game/status.rpy:12625
translate russia task32_aeca0f26:

    # y "Let's talk about a stripclu-....!"
    y ""

# game/status.rpy:12626
translate russia task32_45eda213:

    # s "No!"
    s ""

# game/status.rpy:12632
translate russia task32_ce895a18:

    # y "..........."
    y "..........."

# game/status.rpy:12633
translate russia task32_aa2465c5:

    # y "Looks like the girls aren't used enough to 'preforming' in public."
    y ""

# game/status.rpy:12634
translate russia task32_1cda2e7b:

    # y "Maybe I should encourage more sluttiness when they go undercover."
    y ""

# game/status.rpy:12651
translate russia task32_538108bc:

    # s "[playerName], we need to talk!"
    s ""

# game/status.rpy:12652
translate russia task32_4056d1aa:

    # y "We do?"
    y ""

# game/status.rpy:12653
translate russia task32_08352de5:

    # s "Yes. I was talking to Kim and she talked about turning the bar into a stripclub!"
    s ""

# game/status.rpy:12654
translate russia task32_053b2d16:

    # y "I know! Great idea, right?"
    y ""

# game/status.rpy:12655
translate russia task32_285789d2:

    # s g15 "..........."
    s g15 "..........."

# game/status.rpy:12656
translate russia task32_7b2378b7:

    # s "We're not turning the bar into a stripclub, [playerName]."
    s ""

# game/status.rpy:12657
translate russia task32_5ba03f10:

    # y "We're not?"
    y ""

# game/status.rpy:12658
translate russia task32_4cc3c86f:

    # s g31 "The girls and I are not going to strip for strangers! It's bad enough that we have to go topless sometimes when going undercover and now you want us to get naked for all our customers too?!"
    s g31 ""

# game/status.rpy:12659
translate russia task32_1ce82017:

    # y "Yes...?"
    y ""

# game/status.rpy:12660
translate russia task32_bf7d1a98:

    # s g32 ".................."
    s g32 ".................."

# game/status.rpy:12667
translate russia task32_32f84950:

    # y "Not only would it be a good way to bring in some extra money. It should also help keeping your nanobots suppressed."
    y ""

# game/status.rpy:12668
translate russia task32_4f76d519:

    # s g31 "But I'm n-...!"
    s g31 ""

# game/status.rpy:12671
translate russia task32_6edf2e7a:

    # kim "Sounds like a plan. Have fun with that, Sam."
    kim ""

# game/status.rpy:12672
translate russia task32_3d64620c:

    # y "You're dancing too."
    y ""

# game/status.rpy:12674
translate russia task32_bb49e007:

    # kim "Heh... heh~... no escaping that, is there?"
    kim ""

# game/status.rpy:12675
translate russia task32_7901ed41:

    # s "[playerName]...! This has to be your worst idea yet!"
    s ""

# game/status.rpy:12676
translate russia task32_0f031b96:

    # y "I guess we could go with my other idea to build a brothel."
    y ""

# game/status.rpy:12677
translate russia task32_f39a40ea:

    # s g30 "N-no that's even worse!"
    s g30 ""

# game/status.rpy:12678
translate russia task32_2c763540:

    # y "Strip club then?"
    y ""

# game/status.rpy:12679
translate russia task32_09f736a6:

    # s ".................."
    s ".................."

# game/status.rpy:12680
translate russia task32_e40637ea:

    # s g15 "{b}*Sighs*{/b} Strip club it is...."
    s g15 ""

# game/status.rpy:12681
translate russia task32_d2312450:

    # y "I thought as much."
    y ""

# game/status.rpy:12682
translate russia task32_dc78677f:

    # y "You two go inform the other girls and get some practise in."
    y ""

# game/status.rpy:12683
translate russia task32_99c91d86:

    # "Sam nods reluctantly and the two head off."
    ""

# game/status.rpy:12689
translate russia task32_fb709c2f:

    # y "Now then... It's probably going to be expensive to remodel the place. We need some kind of modular design that allows the bar to look like a bar during daytime and switch to a nightclub at night..."
    y ""

# game/status.rpy:12691
translate russia task32_502d3675:

    # y "I'm probably also going to have to hire some more staff at the school."
    y ""

# game/status.rpy:12692
translate russia task32_296bb0c2:

    # y "And finally, we need to get the word out that there's a new stripclub in town. I'll need to find someone who can help with that."
    y ""

# game/status.rpy:12704
translate russia task32_87f0382d:

    # y "No? Well if you're sure..."
    y ""

# game/status.rpy:12705
translate russia task32_c5ec260e:

    # s g38 "Stripping for strangers goes too far, [playerName]."
    s g38 ""

# game/status.rpy:12706
translate russia task32_1b8d18ca:

    # y "Even if it's for money?"
    y ""

# game/status.rpy:12707
translate russia task32_ca88b209:

    # s "Especially if it's for money!"
    s ""

# game/status.rpy:12708
translate russia task32_0c36bdca:

    # y "Alright alright.... no strip club."
    y ""

# game/status.rpy:12710
translate russia task32_f0a169c3:

    # kim "Aw Sam, look how disappointed he looks."
    kim ""

# game/status.rpy:12711
translate russia task32_7ee02261:

    # s g12 "He'll get over it."
    s g12 ""

# game/status.rpy:12724
translate russia task32_8f3afb88:

    # kim "Well... looks like we've got everything we need."
    kim ""

# game/status.rpy:12725
translate russia task32_e5b52cbb:

    # kim "Do you want to hire a team to remodel the bar?"
    kim ""

# game/status.rpy:12729
translate russia task32_bb8bd5a6:

    # y "(I only have $[cash].)"
    y ""

# game/status.rpy:12730
translate russia task32_360e77d6:

    # y "Put a pin in it, I'll be back later."
    y ""

# game/status.rpy:12738
translate russia task32_90fdfb25:

    # kim "Moneh moneh moneh!"
    kim ""

# game/status.rpy:12739
translate russia task32_13a9bea6:

    # kim "Okay, let's get things moving~..."
    kim ""

# game/status.rpy:12748
translate russia task32_6bf84583:

    # "The day passes as the bar gets remodelled."
    ""

# game/status.rpy:12754
translate russia task32_a333c30f:

    # kim "Welcome back, boss. What do you think?"
    kim ""

# game/status.rpy:12755
translate russia task32_a356a0eb:

    # y "Well...."
    y ""

# game/status.rpy:12759
translate russia task32_c3e1a0bc:

    # kim "I thought you would. We can start dancing here now."
    kim ""

# game/status.rpy:12760
translate russia task32_ab5bf51b:

    # y "Somehow I doubt that this only cost $5.000..."
    y ""

# game/status.rpy:12761
translate russia task32_a8210f5a:

    # kim "Oh no, all the equipment was stolen. We only had to pay for setting things up."
    kim ""

# game/status.rpy:12762
translate russia task32_4af5053a:

    # y "Oh..."
    y "Ох..."

# game/status.rpy:12765
translate russia task32_0b87c018:

    # kim "Okay enough to strip in, I'd say. We're building a strip joint, not the Louvre."
    kim ""

# game/status.rpy:12766
translate russia task32_37b26a0d:

    # y "Purple is not my color."
    y ""

# game/status.rpy:12767
translate russia task32_41f67ee6:

    # kim "Ignore that. How about all this equipment, ey? It's all stolen of course, so we only had to pay for labor costs."
    kim ""

# game/status.rpy:12768
translate russia task32_30008557:

    # y "I hope WOOHP has good insurance."
    y ""

# game/status.rpy:12770
translate russia task32_52b8b715:

    # y "Not enough strippers."
    y ""

# game/status.rpy:12772
translate russia task32_e570cc86:

    # kim "We only just finished... The girls haven't even arrived yet."
    kim ""

# game/status.rpy:12774
translate russia task32_8e482959:

    # kim "Trust me, once you see your spies dancing you'll like it~..."
    kim ""

# game/status.rpy:12775
translate russia task32_3250c66a:

    # kim "So what's next?"
    kim ""

# game/status.rpy:12776
translate russia task32_5db3a1ed:

    # y "Well... how about a trial run?"
    y ""

# game/status.rpy:12778
translate russia task32_bb6114cf:

    # kim "Oh! Good idea! Hang on, I'll get the girls."
    kim ""

# game/status.rpy:12791
translate russia task32_1f7662bf:

    # y "Have you girls been practising?"
    y ""

# game/status.rpy:12792
translate russia task32_2dac9fda:

    # s g14 "Yeah..."
    s g14 ""

# game/status.rpy:12793
translate russia task32_d212f982:

    # c r11 "{b}*Gulp*{/b} I think we're ready..."
    c r11 ""

# game/status.rpy:12794
translate russia task32_bdbf9220:

    # a y28 "I've got stage fright!"
    a y28 ""

# game/status.rpy:12795
translate russia task32_8da4557c:

    # y "Don't worry Alex. Just imagine the entire audience is naked."
    y ""

# game/status.rpy:12796
translate russia task32_486fee81:

    # y "Except it won't be the audience who's naked...{w} it's you!"
    y ""

# game/status.rpy:12797
translate russia task32_34b523b9:

    # a "That doesn't help [playerName]!"
    a ""

# game/status.rpy:12798
translate russia task32_577e8ede:

    # y "All right, enough daddling. Go put on a show!"
    y ""

# game/status.rpy:12811
translate russia task32_5ccdd5ed:

    # "The girls each pick a pole and begin dancing."
    ""

# game/status.rpy:12812
translate russia task32_0edd9cdd:

    # "It looks like Kim gave them some advice and your spies put on quite the show!"
    ""

# game/status.rpy:12826
translate russia task32_1e718a6b:

    # y "Wow! Quite the show girls!"
    y ""

# game/status.rpy:12828
translate russia task32_be009678:

    # c r24 "{b}*Pant* *Pant*{/b} Heh~... not bad, huh?"
    c r24 ""

# game/status.rpy:12830
translate russia task32_402ff3ec:

    # s g14 "Well that was... interesting."
    s g14 ""

# game/status.rpy:12831
translate russia task32_a6169d5b:

    # y "How'd you like it?"
    y ""

# game/status.rpy:12832
translate russia task32_62d7c8fd:

    # s "Well I'm not looking forward to dancing naked in front of customers!"
    s ""

# game/status.rpy:12833
translate russia task32_4570aa49:

    # s "....................."
    s "....................."

# game/status.rpy:12834
translate russia task32_cb6bbd37:

    # s g15 "But I actually felt kind of sexy..."
    s g15 ""

# game/status.rpy:12837
translate russia task32_b7b968fb:

    # kim "You three are naturals. Can't wait to bring in that sweet, sweet dough."
    kim ""

# game/status.rpy:12838
translate russia task32_b39504c6:

    # y "What about Alex?"
    y ""

# game/status.rpy:12839
translate russia task32_6e3aa9f9:

    # "In the distance you see Alex still swinging from the pole."
    ""

# game/status.rpy:12841
translate russia task32_090008f6:

    # kim "I think she's ready too."
    kim ""

# game/status.rpy:12842
translate russia task32_dfad1b31:

    # y "Okay, then starting tomorrow I'll send you guys to work in the evening at the strip club!"
    y ""

# game/status.rpy:12883
translate russia nightCutscenes_8e536f99:

    # ".........................."
    ".........................."

# game/status.rpy:12886
translate russia nightCutscenes_2bdde32d:

    # "Shadowy Figure 1" "What is the meaning of this?!"
    "Shadowy Figure 1" ""

# game/status.rpy:12887
translate russia nightCutscenes_829990ca:

    # "Shadowy Figure 1" "Have you gone absolutely mad?! {w}The city is burning!"
    "Shadowy Figure 1" ""

# game/status.rpy:12888
translate russia nightCutscenes_d9ad61fd:

    # "Shadowy Figure 2" "...................."
    "Shadowy Figure 2" "...................."

# game/status.rpy:12889
translate russia nightCutscenes_27c533ea:

    # "Shadowy Figure 1" "This is not what we agreed upon! If I had known...!"
    "Shadowy Figure 1" ""

# game/status.rpy:12890
translate russia nightCutscenes_2ce59a01:

    # "Shadowy Figure 2" "Calm yourself my friend. All is going according to plan~..."
    "Shadowy Figure 2" ""

# game/status.rpy:12891
translate russia nightCutscenes_285ae607:

    # "Shadowy Figure 2" "I do recall that it was {i}'you'{/i} who wanted revenge on WOOHP, is it not?"
    "Shadowy Figure 2" ""

# game/status.rpy:12892
translate russia nightCutscenes_35f07371:

    # "Shadowy Figure 1" "I wanted justice! I wanted redemption! Not {i}'this'{/i}!"
    "Shadowy Figure 1" ""

# game/status.rpy:12893
translate russia nightCutscenes_8785749d:

    # "Shadowy Figure 2" "And you shall have your redemption...."
    "Shadowy Figure 2" ""

# game/status.rpy:12894
translate russia nightCutscenes_d1daebf8:

    # "Shadowy Figure 2" "Be patient... Once this is done, the world will be a much better place..."
    "Shadowy Figure 2" ""

# game/status.rpy:12895
translate russia nightCutscenes_cf1afa3c:

    # "Shadowy Figure 1" "..................."
    "Shadowy Figure 1" "..................."

# game/status.rpy:12921
translate russia nightCutscenes_25613663:

    # "???" "What is the meaning of this, Tim?!"
    "???" ""

# game/status.rpy:12922
translate russia nightCutscenes_d1ed8fd7:

    # "???" "Why are WOOHP Agents attacking us during landgrabs?!"
    "???" ""

# game/status.rpy:12923
translate russia nightCutscenes_7b34275c:

    # "???" "We've all lost Lieutenants and God knows where they've been taken!"
    "???" ""

# game/status.rpy:12929
translate russia nightCutscenes_846f028c:

    # tim ".................."
    tim ".................."

# game/status.rpy:12930
translate russia nightCutscenes_0a5ba29c:

    # tim "There are reports of agents resisting our nanobot control. We're working on a solution."
    tim ""

# game/status.rpy:12936
translate russia nightCutscenes_668dd828:

    # "???" "And what does the mastermind have to say about all this?"
    "???" ""

# game/status.rpy:12937
translate russia nightCutscenes_5b49af4d:

    # "Mastermind" "They're disappointed in your lack of resolve..."
    "Организатор" ""

# game/status.rpy:12938
translate russia nightCutscenes_a8d63871:

    # "???" "!!!"
    "???" "!!!"

# game/status.rpy:12944
translate russia nightCutscenes_8be58a1c:

    # "Mastermind" "You all want to be my right hand man, but can't deal with a setback? I expected more from you..."
    "Организатор" ""

# game/status.rpy:12945
translate russia nightCutscenes_6fdb4df8:

    # "Mastermind" "Don't worry though... all is still going according to plan..."
    "Организатор" ""

# game/status.rpy:12963
translate russia nightCutscenes_021a57fb:

    # tim "No more of your excuses! The gangs are losing! If this keeps up, Beverly Hills will be liberated again!"
    tim ""

# game/status.rpy:12964
translate russia nightCutscenes_45fe36f3:

    # "Mastermind" "So it will...."
    "Организатор" ""

# game/status.rpy:12965
translate russia nightCutscenes_64d445c8:

    # tim "Don't tell me this fits into your master plan as well. You've been keeping too many secrets and now we're about to loose everything we worked for!"
    tim ""

# game/status.rpy:12966
translate russia nightCutscenes_73d6406e:

    # "Mastermind" "Tim. What was the original goal of our plan?"
    "Организатор" ""

# game/status.rpy:12967
translate russia nightCutscenes_4d8746e9:

    # tim "What...?"
    tim ""

# game/status.rpy:12968
translate russia nightCutscenes_372652da:

    # "Mastermind" "Our original goal."
    "Организатор" ""

# game/status.rpy:12969
translate russia nightCutscenes_88d58953:

    # tim "...................."
    tim "...................."

# game/status.rpy:12970
translate russia nightCutscenes_0892e566:

    # tim "To bring down WOOHP and make this world a better place..."
    tim ""

# game/status.rpy:12971
translate russia nightCutscenes_a31e8fe7:

    # "Mastermind" "Exactly. WOOHP has already been taken over and the three mightiest gangs in the world are on the brink of defeat."
    "Организатор" ""

# game/status.rpy:12972
translate russia nightCutscenes_ec42c08e:

    # tim "You're... using them like pawns...?"
    tim ""

# game/status.rpy:12973
translate russia nightCutscenes_4bf82eb8:

    # tim "........."
    tim "........."

# game/status.rpy:12974
translate russia nightCutscenes_fa78321e:

    # tim "And what does that make me..."
    tim ""

# game/status.rpy:12975
translate russia nightCutscenes_08766955:

    # "Mastermind" "Obviously... you're our King Tim Scam."
    "Организатор" ""

# game/status.rpy:12976
translate russia nightCutscenes_c3fb3a3f:

    # "Mastermind" "But never forget... {w}I'm the Queen."
    "Организатор" ""

# game/status.rpy:12977
translate russia nightCutscenes_846f028c_1:

    # tim ".................."
    tim ".................."

# game/status.rpy:12978
translate russia nightCutscenes_9c7f6125:

    # tim "What of the rebel agents?"
    tim ""

# game/status.rpy:12979
translate russia nightCutscenes_40ef5751:

    # "Mastermind" "Don't worry... I have a special surprise for them..."
    "Организатор" ""

# game/status.rpy:12984
translate russia nightCutscenes_5c837370:

    # "???" "................................."
    "???" "................................."

# game/status.rpy:13004
translate russia nightCutscenes_63cc9320:

    # tim "The gangs are gone..."
    tim ""

# game/status.rpy:13005
translate russia nightCutscenes_9d7a45d9:

    # "Mastermind" "Indeed they are."
    "Организатор" ""

# game/status.rpy:13006
translate russia nightCutscenes_fdb42620:

    # tim "......................."
    tim "......................."

# game/status.rpy:13007
translate russia nightCutscenes_c5db46e4:

    # tim "They'll be coming for us next."
    tim ""

# game/status.rpy:13008
translate russia nightCutscenes_0a8187dd:

    # "Mastermind" "Don't worry. We have fail-safes in place."
    "Организатор" ""

# game/status.rpy:13010
translate russia nightCutscenes_0469be49:

    # "Mastermind" "They'll be arriving at the base soon. Make your way to the lower levels."
    "Организатор" ""

# game/status.rpy:13011
translate russia nightCutscenes_b07a15e6:

    # "???" "Of course."
    "???" ""

# game/status.rpy:13012
translate russia nightCutscenes_e72031c3:

    # "The girl nods and quickly runs off."
    ""

# game/status.rpy:13014
translate russia nightCutscenes_8351063e:

    # tim "You really think a single spy is going to stop them?"
    tim ""

# game/status.rpy:13015
translate russia nightCutscenes_aed4b95e:

    # "Mastermind" "I wouldn't underestimate her, Tim. She might be as good as Sam."
    "Организатор" ""

# game/status.rpy:13016
translate russia nightCutscenes_cccf0ade:

    # tim "........................"
    tim "........................"

# game/status.rpy:13040
translate russia buyXmasOutfit_36dd0bf5:

    # y "(I don't have enough money for this.)"
    y "(У меня не хватит на это денег.)"

# game/status.rpy:13047
translate russia buyXmasOutfit_387f498f:

    # "Alex's Christmas outfit bought!"
    ""

# game/status.rpy:13053
translate russia buyXmasOutfit_79da94ff:

    # "Sam's Christmas outfit bought!"
    ""

# game/status.rpy:13060
translate russia buyXmasOutfit_7bbb0739:

    # "Clover's Christmas outfit bought!"
    ""

# game/status.rpy:13063
translate russia buyXmasOutfit_39c38a49:

    # "STOP YOU'RE NOT SUPPOSE TO SEE THIS. IF YOU ARE READING THIS, PLEASE LET EXISCOMING KNOW."
    ""

translate russia strings:

    # game/status.rpy:122
    old "{color=#EFD66D}[task7Name]{/color}"
    new "{color=#EFD66D}[task7Name]{/color}"

    # game/status.rpy:122
    old "[task1Name]"
    new "[task1Name]"

    # game/status.rpy:122
    old "[task2Name]"
    new "[task2Name]"

    # game/status.rpy:122
    old "[task4Name]"
    new "[task4Name]"

    # game/status.rpy:122
    old "[task6Name]"
    new "[task6Name]"

    # game/status.rpy:122
    old "[task5Name]"
    new "[task5Name]"

    # game/status.rpy:122
    old "[task10Name]"
    new "[task10Name]"

    # game/status.rpy:122
    old "[task11Name]"
    new "[task11Name]"

    # game/status.rpy:122
    old "[task12Name]"
    new "[task12Name]"

    # game/status.rpy:122
    old "[task13Name]"
    new "[task13Name]"

    # game/status.rpy:122
    old "[task15Name]"
    new "[task15Name]"

    # game/status.rpy:122
    old "[task17Name]"
    new "[task17Name]"

    # game/status.rpy:122
    old "[task18Name]"
    new "[task18Name]"

    # game/status.rpy:122
    old "[task32Name]"
    new "[task32Name]"

    # game/status.rpy:122
    old "[task21Name]"
    new "[task21Name]"

    # game/status.rpy:122
    old "Picture Quests"
    new "Задания с фото"

    # game/status.rpy:122
    old "Completed tasks"
    new "Выполненные задания"

    # game/status.rpy:833
    old "Be strict with her"
    new "Быть строгим"

    # game/status.rpy:833
    old "Be lenient"
    new "Быть снисходительны"

    # game/status.rpy:1139
    old "Appeal to reason"
    new "Апеллировать к разуму"

    # game/status.rpy:1139
    old "Bribe her ($100,-)"
    new "Подкупить её ($100,-)"

    # game/status.rpy:1139
    old "Order her (-Karma)"
    new "Приказать ей (-Карма)"

    # game/status.rpy:1390
    old "We're in this together"
    new "Мы все в одной лодке"

    # game/status.rpy:1390
    old "You two are my slaves now (-Karma)"
    new "Теперь вы обе мои рабыни (-Карма)"

    # game/status.rpy:2251
    old "Go to the meeting"
    new "Пойти на собрание"

    # game/status.rpy:2251
    old "Wait a while longer"
    new "Подождать ещё немного"

    # game/status.rpy:2915
    old "What are you? A puppy?"
    new "Кто ты? Щенок?"

    # game/status.rpy:2915
    old "Order Alex to report (-Karma)"
    new "Приказать Алекс доложить (-Карма)"

    # game/status.rpy:3208
    old "Dildos"
    new "Дилдо"

    # game/status.rpy:3208
    old "Shower curtains"
    new "Занавески в душе"

    # game/status.rpy:3208
    old "Unzip your pants"
    new "Расстегнуть молнию на штанах"

    # game/status.rpy:3363
    old "Return with pictures"
    new "Вернуться с фотографиями"

    # game/status.rpy:3363
    old "Pay the $[task5DildoPrice]"
    new "Заплатить $[task5DildoPrice]"

    # game/status.rpy:3400
    old "Trade pictures"
    new "Торговать фотографиями"

    # game/status.rpy:3400
    old "Pay [task5DildoPrice]"
    new "Заплатить [task5DildoPrice]"

    # game/status.rpy:3400
    old "Come back later"
    new "Прийти позже"

    # game/status.rpy:3651
    old "Apologize (+karma)"
    new "Извиниться (+Карма)"

    # game/status.rpy:3651
    old "'Jackpot!'"
    new "'Джекпот!'"

    # game/status.rpy:4007
    old "Go show off your tits"
    new "Покажи свои сиськи"

    # game/status.rpy:4007
    old "Don't show off your tits"
    new "Не показывай свои сиськи"

    # game/status.rpy:4034
    old "Okay that's enough"
    new "Ладно, хватит."

    # game/status.rpy:4034
    old "Give them a show"
    new "Устрой им шоу"

    # game/status.rpy:4034
    old "Time to suck some dick!"
    new "Пора отсосать!"

    # game/status.rpy:4152
    old "Warn Sam"
    new "Предупредить Сэм"

    # game/status.rpy:4152
    old "Let her off the hook"
    new "Оставить её"

    # game/status.rpy:4365
    old "Discipline her"
    new "Наказать её"

    # game/status.rpy:4365
    old "Talk things over"
    new "Обсудить всё"

    # game/status.rpy:4517
    old "Be friendly"
    new "Быть дружелюбным"

    # game/status.rpy:4517
    old "Be mean"
    new "Быть злым"

    # game/status.rpy:4916
    old "'Sure am!'"
    new "'Ещё как!'"

    # game/status.rpy:4916
    old "'No, I'm not'"
    new "'Нет'"

    # game/status.rpy:5402
    old "Here, give her this (medkit x1)"
    new "Вот, дай ей это (Аптечка x1)"

    # game/status.rpy:5402
    old "I'll keep an eye out for one"
    new "Я присмотрю за ней"

    # game/status.rpy:5632
    old "She has a point"
    new "Она права"

    # game/status.rpy:5632
    old "I like childish Alex"
    new "Мне нравиться инфантильная Алекс"

    # game/status.rpy:5723
    old "Accept mission"
    new "Подтвердить миссию"

    # game/status.rpy:5723
    old "Decline mission"
    new "Отказаться от миссии"

    # game/status.rpy:5888
    old "YES!"
    new "ДА!"

    # game/status.rpy:5888
    old "I mean.. if I have to"
    new "Нуу... если мне придётся"

    # game/status.rpy:5962
    old "Bribe (-$100)"
    new "Взятка (-$100)"

    # game/status.rpy:6165
    old "Aces Party Sluts ($1.000)"
    new "Шлюхи для вечеринки Тузов ($1.000)"

    # game/status.rpy:6407
    old "Salute"
    new "Салют"

    # game/status.rpy:6407
    old "Nod"
    new "Кивнуть"

    # game/status.rpy:7057
    old "Promise not to tell"
    new "Пообещать никому не рассказывать"

    # game/status.rpy:7057
    old "Blackmail Clover"
    new "Шантажировать Кловер"

    # game/status.rpy:7256
    old "Don't mind if I do!"
    new "Не возражаешь, если я это сделаю!"

    # game/status.rpy:7256
    old "Wait, something is off-..."
    new "Подождите, что-то не-..."

    # game/status.rpy:7312
    old "Search the top floor of the prison"
    new "Обыскать верхний этаж тюрьмы"

    # game/status.rpy:7312
    old "Search the middle floor of the prison"
    new "Обыскать средний этаж тюрьмы"

    # game/status.rpy:7312
    old "Search the bottom floor of the prison"
    new "Обыскать нижний этаж тюрьмы"

    # game/status.rpy:7582
    old "Let me read it"
    new "Дайте мне это прочитать"

    # game/status.rpy:7582
    old "Get that away from me"
    new "Убери это от меня"

    # game/status.rpy:7764
    old "Give $150"
    new "Дать $150"

    # game/status.rpy:7764
    old "Don't give $150"
    new "Недавать $150"

    # game/status.rpy:7825
    old "Follow closely"
    new "Преследовать в близи"

    # game/status.rpy:7825
    old "Follow at a distance"
    new "Преследовать на расстоянии"

    # game/status.rpy:7825
    old "Follow far away"
    new "Преследовать на большом расстоянии"

    # game/status.rpy:8611
    old "Try the red one"
    new "Попробовать красную"

    # game/status.rpy:8611
    old "Try the blue one"
    new "Попробовать синею"

    # game/status.rpy:8611
    old "Try the green one"
    new "Попробовать зелёную"

    # game/status.rpy:9375
    old "'I remember you!'"
    new "'Я тебя помню!'"

    # game/status.rpy:9375
    old "'I don't remember you.'"
    new "'Я тебя не помню.'"

    # game/status.rpy:9454
    old "'Yes, I am'"
    new "'Да'"

    # game/status.rpy:9454
    old "...................."
    new "...................."

    # game/status.rpy:10125
    old "Cut the tension"
    new "Снять напряжение"

    # game/status.rpy:10125
    old "..............................."
    new "..............................."

    # game/status.rpy:10382
    old "Fight back"
    new "Дать отпор"

    # game/status.rpy:10382
    old "Give up"
    new "Сдаться"

    # game/status.rpy:10395
    old "What am I saying?! Of course not!"
    new "А я что говорю?! Конечно, нет!"

    # game/status.rpy:10460
    old "'Can you do it?'"
    new "'Ты сможешь это сделать?'"

    # game/status.rpy:10460
    old "'Crazy is my middle name!'"
    new "'Сумасшествие - моё второе имя!'"

    # game/status.rpy:10564
    old "I promise to let you go (+Karma)"
    new "Я обещаю тебя отпустить (+Карма)"

    # game/status.rpy:10564
    old "Lie about letting him go (-Karma)"
    new "Солгать (-Карма)"

    # game/status.rpy:10709
    old "Yes, that's me"
    new "Да, это я"

    # game/status.rpy:10709
    old "No, I'm not"
    new "Нет, это не я"

    # game/status.rpy:10833
    old "Let's go now"
    new "Пойдём уже"

    # game/status.rpy:10833
    old "Let's wait till tomorrow"
    new "Давай подождём до завтра"

    # game/status.rpy:10833
    old "I'm having second thoughts"
    new "У меня есть сомнения"

    # game/status.rpy:10944
    old "Jerk off to the bound spy"
    new "Дрочить на связанную шпионку"

    # game/status.rpy:10944
    old "Wait a while long"
    new "Подождать немного"

    # game/status.rpy:11128
    old "Jerk it"
    new "Подрочить"

    # game/status.rpy:11128
    old "Wait a bit longer"
    new "Подождать ещё немного"

    # game/status.rpy:11128
    old "Get Clover out"
    new "Вытащить Кловер"

    # game/status.rpy:11408
    old "Then let's get one"
    new "Тогда давайте купим"

    # game/status.rpy:11408
    old "We don't need one"
    new "Нам это не нужно"

    # game/status.rpy:11408
    old "*Say something insane*"
    new "*Сказать что-то безумное*"

    # game/status.rpy:11601
    old "Jerk off to Sam"
    new "Подрочить на Сэм"

    # game/status.rpy:11601
    old "Jerk off to Clover"
    new "Подрочить на Кловер"

    # game/status.rpy:11601
    old "Jerk off to Alex"
    new "Подрочить на Алекс"

    # game/status.rpy:12315
    old "Continue playing in free roam mode"
    new "Продолжить играть в режиме свободного перемещения"

    # game/status.rpy:12315
    old "End game"
    new "Закончить игру"

    # game/status.rpy:12662
    old "Build a stripclub"
    new "Построить стриптиз-клуб"

    # game/status.rpy:12662
    old "Keep things as they are"
    new "Оставьте всё как есть"

    # game/status.rpy:12726
    old "Yes ($5.000)"
    new "Да ($5.000)"

    # game/status.rpy:12726
    old "Maybe later"
    new "Может быть, позже"

    # game/status.rpy:12756
    old "'I love it'"
    new "'Мне нравиться'"

    # game/status.rpy:12756
    old "'It's okay'"
    new "'Неплохо'"

    # game/status.rpy:12756
    old "'I've see better'"
    new "'Я видел и получше'"

# TODO: Translation updated at 2021-10-17 19:03

# game/status.rpy:1913
translate russia testMe_c77c3148:

    # "Landgrabs happen ever 30 days. Currently it's [timetograb] more days until the next one."
    ""

# game/status.rpy:1914
translate russia testMe_b22b20b4:

    # "During a Landgrab, you can send freed WOOHP agents to fight for you in the streets and try to bring enemy lieutenants out of hiding."
    ""

# game/status.rpy:1915
translate russia testMe_fba4425f:

    # "However, you can lose agents during the fight. Agents can be captured by planning missions and using the {color=#ffeda6}Hypno Earring{/color} gadget."
    "Гипно-Серьги"

# game/status.rpy:1916
translate russia testMe_3dd9a308:

    # "After they've been captured and send to the base, you can break them out of their nanobot control by throwing down dirty magazines into the basement."
    ""

# game/status.rpy:1918
translate russia testMe_0baf7e97:

    # "You can see when the next landgrab is by clicking on the 'Agents' button at the top of the screen. Here you can also choose to skip to the next Landgrab in case you don't want to wait."
    ""

# game/status.rpy:1919
translate russia testMe_6626d1fc:

    # "Spies send undercover during a Landgrab earn a reputation boost, but risk getting hurt on the job."
    ""

# game/status.rpy:1921
translate russia testMe_58d96226:

    # "Some missions will not become available until you've captured certain gangleaders. Therefor the best order would be to capture the first Lt. of each gang, before moving on to the second."
    ""

# game/status.rpy:1939
translate russia testMe_00040114:

    # y "Company meeting!"
    y ""

# game/status.rpy:1944
translate russia testMe_30687c86:

    # y "Today I wanted to talk abou-..."
    y ""

# game/status.rpy:1945
translate russia testMe_d361245c:

    # "{b}*Ting* *Ting*{/b}"
    ""

# game/status.rpy:1946
translate russia testMe_29bc5049:

    # "???" "Hello?"
    "???" ""

# game/status.rpy:1947
translate russia testMe_aa785824:

    # y "Sorry, we're not open ye-...."
    y ""

# game/status.rpy:1963
translate russia testMe_e71b99db:

    # "Agent 1" "What a lovely milkshake bar you have here!"
    "Агент 1" ""

# game/status.rpy:1964
translate russia testMe_ee99fffe:

    # y "Er..."
    y ""

# game/status.rpy:1968
translate russia testMe_21cfb035:

    # "Agent 2" "Quite the milkshake bar you found here fellow agent!"
    "Агент 2" ""

# game/status.rpy:1969
translate russia testMe_c98142a2:

    # y ".................."
    y ".................."

# game/status.rpy:1973
translate russia testMe_d7d1fb09:

    # "Agent 3" "A charming little milkshake bar?! How quaint! Surprising how it survived the chaos this long!"
    "Агент 3" ""

# game/status.rpy:1974
translate russia testMe_fa47215b:

    # y "Heh heh~... yeah big surprise~..."
    y ""

# game/status.rpy:1975
translate russia testMe_28801176:

    # "Agent 1" "........................."
    "Агент 1" "........................."

# game/status.rpy:1976
translate russia testMe_072f1426:

    # "Agent 1" "Wait... Something's not quite righ-...."
    "Агент 1" ""

# game/status.rpy:1977
translate russia testMe_96dad439:

    # y "GET 'EM!"
    y ""

# game/status.rpy:2016
translate russia testMe_9324afc6:

    # a y28 "WOOHP s!"
    a y28 ""

# game/status.rpy:2017
translate russia testMe_8f62761d:

    # s g29 "That was close...!"
    s g29 ""

# game/status.rpy:2018
translate russia testMe_13c57710:

    # c r37 "We can't have them reporting back to WOOHP and blow our cover. Luckily it was only three this time."
    c r37 ""

# game/status.rpy:2019
translate russia testMe_633a6e4d:

    # a y49 "What do we do with them...?"
    a y49 ""

# game/status.rpy:2020
translate russia testMe_17ab0564:

    # y "I don't know. I've never hid a body before!{w} Put them downstairs in the basement I guess."
    y ""

# game/status.rpy:2021
translate russia testMe_bf7c3d29:

    # a "{b}*Nods*{/b}"
    a "{b}*Кивает*{/b}"

# game/status.rpy:2024
translate russia testMe_225595a0:

    # y "And now we have a couple of mindcontrolled agents in the base...."
    y ""

# game/status.rpy:2025
translate russia testMe_60cf6055:

    # c r1 "This works in our favor! If we break their nanobot control, we can use them in the next Landgrab!"
    c r1 ""

# game/status.rpy:2026
translate russia testMe_1ebe22a9:

    # y "That's what I was thinkin-..."
    y ""

# game/status.rpy:2027
translate russia testMe_0ef670cc:

    # a "Eep!"
    a ""

# game/status.rpy:2029
translate russia testMe_f1657285:

    # "???" "Daylight stings my eyes!"
    "???" ""

# game/status.rpy:2030
translate russia testMe_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/status.rpy:2032
translate russia testMe_6faed475:

    # a y29 "Er... [playerName]? There's already an agent downstairs."
    a y29 ""

# game/status.rpy:2033
translate russia testMe_d3a59cb5:

    # y "............................."
    y "............................."

# game/status.rpy:2037
translate russia testMe_57168065:

    # "Agent" "Hi! I'm still here!"
    "Агент" ""

# game/status.rpy:2038
translate russia testMe_40271c89:

    # y "Are you that agent we captured during the first mission...?"
    y ""

# game/status.rpy:2039
translate russia testMe_3203e85f:

    # y "How did you survive down there for all this time...?"
    y ""

# game/status.rpy:2040
translate russia testMe_da97e51d:

    # "Agent" "My undying dedication to the mission!"
    "Агент" ""

# game/status.rpy:2041
translate russia testMe_db832180:

    # y "........"
    y "........"

# game/status.rpy:2042
translate russia testMe_0ff703d4:

    # y "Suuure~...{w} Say... Do you want to help us retake Beverly Hills?"
    y ""

# game/status.rpy:2043
translate russia testMe_d83b6620:

    # "Agent" "Boy do I! I can't wait to deal out some justice!{w} Preferably with my fists!"
    "Агент" ""

# game/status.rpy:2044
translate russia testMe_fed0b3d6:

    # y "Oh easy there. We're gonna send you out during the next Landgrab."
    y ""

# game/status.rpy:2045
translate russia testMe_8e1bf4df:

    # s g31 "If we can break the nano-control over these other agents, we could use them to help."
    s g31 ""

# game/status.rpy:2046
translate russia testMe_6db73817:

    # y ".........................."
    y ".........................."

# game/status.rpy:2047
translate russia testMe_fce37da7:

    # s g18 "What?"
    s g18 "Что?"

# game/status.rpy:2048
translate russia testMe_d4636c00:

    # y "You know what is needed to break agents out of their control, right?"
    y ""

# game/status.rpy:2050
translate russia testMe_983b94a6:

    # s "Well... yeah. We have to increase their oxytocin levels~..."
    s ""

# game/status.rpy:2051
translate russia testMe_ec01b85a:

    # y "And how do we do that~...?"
    y ""

# game/status.rpy:2052
translate russia testMe_5c6cf512:

    # s ".........................."
    s ".........................."

# game/status.rpy:2053
translate russia testMe_e3e16dc2:

    # s g19 "{size=-10}By having sex..."
    s g19 ""

# game/status.rpy:2054
translate russia testMe_05c7fd16:

    # y "Exactly! Time to get naked!"
    y ""

# game/status.rpy:2055
translate russia testMe_e4e62ab5:

    # s g28 "N-now wait a minute!"
    s g28 ""

# game/status.rpy:2056
translate russia testMe_23e27976:

    # c r31 "We're not gonna start showing off our naked bodies to every member of WOOHP. One is enough."
    c r31 ""

# game/status.rpy:2057
translate russia testMe_a5c1bc99:

    # a y29 "We could just buy some dirty magazines."
    a y29 ""

# game/status.rpy:2058
translate russia testMe_05d09e98:

    # y "Dirty magazines...?"
    y ""

# game/status.rpy:2059
translate russia testMe_a82223c7:

    # a y1 "Yeah, we can just buy some magazines with those dirty pictures from the bookstore at the mall."
    a y1 ""

# game/status.rpy:2060
translate russia testMe_9b0b95e2:

    # a "If we throw a few of those down into the basement, I bet that will break the agents from their trance!"
    a ""

# game/status.rpy:2061
translate russia testMe_7b7f2369:

    # y "It'll cost us some money, but that's a good alternative."
    y ""

# game/status.rpy:2065
translate russia testMe_18ceca81:

    # "You captured a few more WOOHP Agents and it's time to free them from their nanobot control."
    ""

# game/status.rpy:2066
translate russia testMe_05094001:

    # "Luckily breaking the nanobot control over captured agents isn't that hard. Visit the bookstore to buy cheap porn magazines and give them to the captured agents in your basement."
    ""

# game/status.rpy:2067
translate russia testMe_0ea4dbc5:

    # "Once agents have been freed, they can participate in Landgrabs and help liberate Beverly Hills."
    ""

# game/status.rpy:2069
translate russia testMe_16222c52:

    # y "Okay, let's wrap this up. We've got quite a day ahead of us."
    y ""

# game/status.rpy:2070
translate russia testMe_5a2dcbda:

    # "Agent" "What should I do?!"
    "Агент" ""

# game/status.rpy:2071
translate russia testMe_42fb3646:

    # y "Er... You...{w} Here. Put this one."
    y ""

# game/status.rpy:2074
translate russia testMe_40e9eb54:

    # y "You are now a chef."
    y ""

# game/status.rpy:2075
translate russia testMe_50bbbf6c:

    # "Agent" "An important job! I will not disappoint you!"
    "Агент" ""

# game/status.rpy:2098
translate russia testMe_d5c53159:

    # mel "What's going on out here?!"
    mel ""

# game/status.rpy:2099
translate russia testMe_2a27cb3f:

    # "Aces gangster" "We're under attack from all sides! We've got Punks on our left, Outsiders on our right and WOOHP agents fighting us from our front!"
    "Туз Гангстер" ""

# game/status.rpy:2100
translate russia testMe_d48a0b44:

    # mel "WOOHP Agen-...?"
    mel ""

# game/status.rpy:2101
translate russia testMe_3032c70f:

    # "Outsiders Chick" "More Aces, get them!"
    "Девушка Аутсайдер" ""

# game/status.rpy:2102
translate russia testMe_30be022b:

    # mel "Fine... I'll show you how it's done."
    mel ""

# game/status.rpy:2122
translate russia testMe_b5d4557f:

    # y "Okay girls, I received intel that a new Lieutenant has shown herself."
    y ""

# game/status.rpy:2123
translate russia testMe_8b2a79b7:

    # y "Her name is Melody von...{w} Guggen..."
    y ""

# game/status.rpy:2124
translate russia testMe_390bc4e9:

    # y "That's a pretty crazy last name..."
    y ""

# game/status.rpy:2126
translate russia testMe_d963f4cb:

    # a y29 "Wait, did you say von Guggen?"
    a y29 ""

# game/status.rpy:2127
translate russia testMe_2d574665:

    # y "That name ring a bell?"
    y ""

# game/status.rpy:2128
translate russia testMe_d0b0d805:

    # c r34 "We fought a Helga von Guggen a long time ago."
    c r34 ""

# game/status.rpy:2129
translate russia testMe_cde11bd6:

    # s g43 "Could be her granddaughter. She's probably after revenge."
    s g43 ""

# game/status.rpy:2130
translate russia testMe_efe66fc3:

    # y "Reports are saying she's hanging out at the Aces Castle, but security is ramped up. You're going to have to get closer before you have a chance to capture her."
    y ""

# game/status.rpy:2132
translate russia testMe_47cb1d67:

    # s "Make sure to focus on capturing the first Lieutenant of each gang first."
    s ""

# game/status.rpy:2133
translate russia testMe_ac859cf3:

    # s "We can't risk one gang getting the upperhand over the others."
    s ""

# game/status.rpy:2135
translate russia testMe_c8346d00:

    # y "I'm gonna keep sending you undercover with the Aces. Hopefully we'll get a chance to take her in."
    y ""

# game/status.rpy:2136
translate russia testMe_a2abf71c:

    # "The girls nod and leave for their cell."
    ""

# game/status.rpy:5913
translate russia task13_bda12796:

    # y "You ever think we might not be in control of our own destiny?{w} Like some all powerful omnipitant force is writing our words for us?"
    y ""

# game/status.rpy:6268
translate russia task15_d320d8e3:

    # s "We just gotta worry about the initial payment. Let's say... $1.000?"
    s ""

# game/status.rpy:6270
translate russia task15_bb531d7f:

    # y "Music from Punk Web\nFireworks from the Abandoned Amusement Park\nSlutty college girls from the school.\nA budget of 1.000 dollars."
    y ""

# game/status.rpy:7859
translate russia task21_da403e6b:

    # y "(He's a keen one. Best not to take any risks and keep my distance.)"
    y ""

# game/status.rpy:8236
translate russia retryHack_e6e14b29:

    # sM "Sebastian Saga..."
    sM ""

# game/status.rpy:9767
translate russia task26_75ed27a5:

    # s g30 "We... never really found out what happened to her. Just that Jerry got fed up with her bossy personality."
    s g30 ""

# game/status.rpy:10291
translate russia task26_8a589bfb:

    # y "I saw where you make the gadgets, the hidden civilian surveillance, the prisons, the propaganda center..."
    y ""

# game/status.rpy:10571
translate russia endgameMenu1_9c8f9dcc:

    # tim "And if I do? You're just going to throw me back in my cell."
    tim ""

# game/status.rpy:11213
translate russia endgameMenu1_a32dcb03:

    # c r29 "Yeah that's right! Alex has been put in charge of guarding the WOOHP missile silos!"
    c r29 ""

# game/status.rpy:12769
translate russia task32_884e3d3f:

    # y "Somehow I doubt that this only cost $2.000..."
    y ""
