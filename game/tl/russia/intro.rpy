# TODO: Translation updated at 2021-10-09 21:10

# game/intro.rpy:21
translate russia introGame_886f5bbc:

    # "Are you sure you want to skip ahead? The intro teaches you some important game mechanics."
    "Вы уверены, что хотите пропустить интро? Оно обучит вас важным игровым механикам"

# game/intro.rpy:66
translate russia beginIntro_50b5774c:

    # LOCATION "Beverly Hills, USA - Somewhere underground..."
    LOCATION "Беверли-Хиллз, США - Где-то под землей..."

# game/intro.rpy:71
translate russia beginIntro_bedf103c:

    # yInmate "........................."
    yInmate "........................."

# game/intro.rpy:72
translate russia beginIntro_07ead28c:

    # yInmate "The World Organization Of Human Protection, better known as WOOHP...."
    yInmate "Всемирная организация по защите человека, более известная как «WOOHP»...."

# game/intro.rpy:73
translate russia beginIntro_06f6ee09:

    # yInmate "It's fine they said... It's better than spending time in prison they said..."
    yInmate "Все будет в порядке говорили они... Это лучше, чем проводить время в тюрьме, говорили они...."

# game/intro.rpy:74
translate russia beginIntro_8c13e9b9:

    # yInmate "The WOOHP Asylum for the Criminally Insane is a {i}much{/i} better alternative they said..."
    yInmate "Психиатрическая больница «WOOHP», для душевнобольных преступников это {i}наилучшая{/i} альтернатива, говорили они..."

# game/intro.rpy:75
translate russia beginIntro_71ee0bc2:

    # yInmate "So they put me in a hole in the ground and threw away the key..."
    yInmate "И именно по этому они запихнули меня в эту дыру под землей, и выкинули от нее ключ..."

# game/intro.rpy:76
translate russia beginIntro_0d17423e:

    # yInmate "I'm not insane! I just had a few very unfortunes events!"
    yInmate "Я не сумасшедший! У меня просто было несколько очень неудачных событий!"

# game/intro.rpy:77
translate russia beginIntro_ced5018e:

    # "Guard" "FRONT AND CENTER INMATE!"
    "Охранник" "ЦЕНТРАЛЬНЫЙ, ПЕРЕДНИЙ ЗАКЛЮЧЕННЫЙ!"

# game/intro.rpy:78
translate russia beginIntro_3ad74576:

    # yInmate "Gah!"
    yInmate "Ааа!"

# game/intro.rpy:86
translate russia beginIntro_723ebf6a:

    # "Guard" "WHAT ARE YOU MUTTERING TO YOURSELF ABOUT?! THAT'S SOMETHING AN INSANE PERSON WOULD DO!"
    "Охранник" "О ЧЕМ ЭТО ТЫ ТАМ БОРМОЧЕШЬ?! ЭТО КАК РАЗ ТО, ЧЕМ И ЗАНИМАЮТСЯ ПСИХИ!"

# game/intro.rpy:87
translate russia beginIntro_0eb29cda:

    # yInmate "......................"
    yInmate "......................"

# game/intro.rpy:88
translate russia beginIntro_0349eca0:

    # yInmate "Okay fair enough."
    yInmate "Довольно, справедливое замечание."

# game/intro.rpy:89
translate russia beginIntro_0e89f2e8:

    # "Guard" "YOU'RE ON CLEANUP DUTY TODAY! THE CLEANING STAFF HASN'T SHOWED UP SO NOW I'M MAKING YOU DO IT!"
    "Охранник" "СЕГОДНЯ ТЫ ЗА УБОРЩИКА! РАБОЧИЙ ПЕРСОНАЛ НЕ ПРИШЕЛ НА РАБОТУ, ТАК ЧТО СЕГОДНЯ ТЫ ВОЗЬМЕШЬСЯ ЗА НЕЕ!"

# game/intro.rpy:90
translate russia beginIntro_4d91bbac:

    # yInmate "Isn't that abusing your p-...?"
    yInmate "Разве это не злоупотребление вашими пол-...?"

# game/intro.rpy:91
translate russia beginIntro_aced5560:

    # "Guard" "ABUSING MY POWER! EXACTLY!"
    "Охранник" "ЗЛОУПОТРЕБЛЕНИЕ МОИМИ ПОЛНОМОЧИЯМИ! В ТОЧКУ!"

# game/intro.rpy:92
translate russia beginIntro_428575fc:

    # "Guard" "YOUR ORDERS ARE SIMPLE!{w} KEEP MOPPING THAT FLOOR UNTIL IT SPARKLES. I WANT TO SEE MY FACE IN THAT FLOOR!"
    "Охранник" "У ТЕБЯ ОЧЕНЬ ПРОСТАЯ ЗАДАЧА!{w} НАТРИ ЭТОТ ПОЛ ДО БЛЕСКА, ХОЧУ ЧТОБЫ В НЕМ ОТРАЖАЛОСЬ МОЕ ЛИЦО!"

# game/intro.rpy:93
translate russia beginIntro_7b0df5a4:

    # "Guard" "I WANT IT BETTER THAN CLEAN! I WANT IT TO BE HANDSOME!"
    "Охранник" "Я ХОЧУ ЧТОБЫ ОН БЫЛ НЕ ПРОСТО ЧИСТЫМ! А ЧТОБЫ ОН БЫЛ АПУПИТЕЛЬНО ЧИСТЫМ!"

# game/intro.rpy:94
translate russia beginIntro_d5d362de:

    # yInmate "Er...."
    yInmate "Еммм...."

# game/intro.rpy:95
translate russia beginIntro_e143c089:

    # "Guard" "I WANT THIS PLACE TO LOOK SO HANDSOME, THAT WHEN I WALK IN HERE AND SEE MY OWN REFLECTION, I THINK A HANDSOME INTRUDER HAS INVADED THIS FACILITY!"
    "Охранник" "Я ХОЧУ ЧТОБЫ ЭТО МЕСТО ВЫГЛЯДЕЛО НАСТОЛЬКО АФФИГИТЕЛЬНО, ЧТО КОГДА Я СЮДА ВХОЖУ И ВИЖУ СВОЕ ОТРАЖЕНИЕ, Я ДУМАЛ ТОЛЬКО ОБ ЭТОМ КРАСИВОМ НЕЗВАНОМ ГОСТЕ, ЧТО ВОРВАЛСЯ СУДА!"

# game/intro.rpy:96
translate russia beginIntro_40aada53:

    # "Guard" "I WANT TO TAKE MY DATE TO THIS ASYLUM AND HAVE HER DUMP ME FOR MY REFLECTION, THAT'S HOW CLEAN I WANT THIS FLOOR TO BE!{w} DO I MAKE MYSELF CLEAR?!"
    "Охранник" "Я ХОЧУ ЧТОБЫ ДЕВУШКА, КОТОРУЮ Я ПРИВЕДУ СЮДА НА СВИДАНИЕ, НЕ СМОГЛА ОТЛИЧИТЬ МЕНЯ ОТ МОЕГО ОТРАЖЕНИЯ В ЭТОМ ПОЛУ! {w} Я ЯСНО ВЫРАЖАЮСЬ?!"

# game/intro.rpy:97
translate russia beginIntro_63ae66cc:

    # yInmate "You'd take your date to an asylum?"
    yInmate "Вы собираетесь привезти девушку на свидание в психушку?"

# game/intro.rpy:98
translate russia beginIntro_cdd6a058:

    # "Guard" "THAT'S ENOUGH OUT OF YOU WISEGUY! HERE'S A BUCKET AND A MOP!"
    "Охранник" "ВСЕ С ТЕБЯ ДОСТАТОЧНО, УМНИК! ВОТ ВЕДРО И ШВАБРА!"

# game/intro.rpy:99
translate russia beginIntro_5bfc9976:

    # "Guard" "NOW GET MOPPING!"
    "Охранник" "А ТЕПЕРЬ ЗА РАБОТУ!"

# game/intro.rpy:102
translate russia beginIntro_c8e05118:

    # yInmate "YES SIR, ON IT SIR!"
    yInmate "ТАК ТОЧНО!, ПРИНИМАЮСЬ ЗА РАБОТУ!"

# game/intro.rpy:103
translate russia beginIntro_0efb1f8a:

    # "Guard" "STOP THAT SHOUTING, THIS IS AN ASYLUM, DON'T YOU KNOW?!"
    "Охранник" "Хватит кричать, это же психиатрическая больница, забыл что ли?!"

# game/intro.rpy:104
translate russia beginIntro_9f82ec38:

    # yInmate "....................."
    yInmate "....................."

# game/intro.rpy:106
translate russia beginIntro_2b2bd210:

    # yInmate "I'm pretty sure you're legally required to pay me for this~..."
    yInmate "По закону вы должны будете заплатить мне за работу~..."

# game/intro.rpy:107
translate russia beginIntro_4ec79ff8:

    # "Guard" "HOT-DOG! YOU'RE RIGHT!{w} HERE'S A DOLLAR!"
    "Охранник" "ЁШКИ-МАТРЕКИ! ДА ТЫ ПРАВ!{w} ВОТ ТЕБЕ ДОЛЛАР!"

# game/intro.rpy:111
translate russia beginIntro_435f7b80:

    # yInmate "Thanks~..."
    yInmate "Спасибо~..."

# game/intro.rpy:113
translate russia beginIntro_3ef2e238:

    # yInmate "Did you know that the national anthem wasn't really written by Francis Scott Key?"
    yInmate "Знаете ли вы, что национальный гимн на самом деле был написан не Фрэнсисом Скоттом Ки??"

# game/intro.rpy:114
translate russia beginIntro_cebdf634:

    # yInmate "There's compelling evidence that show that a giant panda named Sir Jamie Fitzgerald-...."
    yInmate "Есть убедительные доказательства того, что гигантская панда по имени сэр Джейми Фицджеральд-...."

# game/intro.rpy:115
translate russia beginIntro_c59f96e4:

    # yInmate "(Oh my God!{w} I really am insane!)"
    yInmate "(Боже мой!{w} Я реално съехал с катушек!)"

# game/intro.rpy:116
translate russia beginIntro_17937457:

    # "Guard" "THAT'S ENOUGH OUT OF YOU INMATE! {w}EVERYONE KNOWS SIR JAMIE FITZGERALD WAS THE REAL AUTHOR OF THE ANTHEM!"
    "Охранник" "ДОСТАТОЧНО! {w}ВСЕ ЗНАЮТ, ЧТО СЭР ДЖЕЙМИ ФИЦДЖЕРАЛЬД БЫЛ НАСТОЯЩИМ АВТОРОМ!"

# game/intro.rpy:117
translate russia beginIntro_fa1ee728:

    # "Guard" "NOW GET TO MOPPING!"
    "Охранник" "А ТЕПЕРЬ ЗА РАБОТУ!"

# game/intro.rpy:118
translate russia beginIntro_f6c255d7:

    # yInmate "................."
    yInmate "................."

# game/intro.rpy:119
translate russia beginIntro_04805493:

    # "Guard" "IN THE MEANTIME I'M HEADING INTO THE MILKSHAKE BAR!"
    "Охранник" "А Я ПОКА ЧТО СГОНЯЮ ЗА МОЛОЧНЫМ КОКТЕЙЛЕМ!"

# game/intro.rpy:120
translate russia beginIntro_6e8dc981:

    # yInmate "The... what now?"
    yInmate "За чем?"

# game/intro.rpy:121
translate russia beginIntro_0a9b7c7e:

    # "Guard" "DON'T TALK BACK TO M-...."
    "Охранник" "ЗАВЯЗЫВАЙ БОЛТ-...."

# game/intro.rpy:134
translate russia beginIntro_b5e75fb7:

    # "Guard" "......................................"
    "Охранник" "......................................"

# game/intro.rpy:135
translate russia beginIntro_8b118c34:

    # yInmate "...?"
    yInmate "...?"

# game/intro.rpy:136
translate russia beginIntro_8046f0fc:

    # "Guard" "Overthrow~....{w} Anarchy~....{w} Chaos~....."
    "Охранник" "Свергнуть~....{w} Анархия~....{w} Хаос~....."

# game/intro.rpy:137
translate russia beginIntro_89ae3cdf:

    # "The guard suddenly stops paying attention to you as he blindly stares into space."
    "Охранник внезапно перестаёт обращать на вас внимание и просто пялится в пустоту."

# game/intro.rpy:138
translate russia beginIntro_e954652e:

    # yInmate "Ehrm... Are you okay...?"
    yInmate "Емм... Вы в порядке...?"

# game/intro.rpy:141
translate russia beginIntro_4775792d:

    # "Without another word, the zombiefied guard walks out of the cellblock."
    "Не проронив ни слова Охранник выходит из камеры."

# game/intro.rpy:142
translate russia beginIntro_2b68a6b1:

    # yInmate "..........................."
    yInmate "..........................."

# game/intro.rpy:143
translate russia beginIntro_8610d6ac:

    # yInmate "I guess the guards are as insane as the inmates..."
    yInmate "Судя по всему Охранники здесь такие же долбанутые, как и заключённые...."

# game/intro.rpy:144
translate russia beginIntro_1391e557:

    # yInmate "Well...! Idle hands are the devil's playthings. Best get busy mopping."
    yInmate "Ну что ж...! \"Праздные руки - игрушки дьявола\". Лучше всего заняться уборкой."

# game/intro.rpy:147
translate russia beginIntro_7a1ff032:

    # "You spend a couple of hours cleaning the place."
    "Вы тратите пару часов на уборку."

# game/intro.rpy:148
translate russia beginIntro_ceb5afb5:

    # "The other inmates looks skittish and on edge. Quietly muttering to each other and going silent as you walk by."
    "Другие заключенные выглядят напуганными. Они тихо бормочут друг с другом и замолкают, как только вы подходите ближе."
# game/intro.rpy:149
translate russia beginIntro_ebd972d4:

    # "???" "Hey pssst~...."
    "???" "Ей ты~...."

# game/intro.rpy:153
translate russia beginIntro_462987e0:

    # yInmate "....?"
    yInmate "....?"

# game/intro.rpy:157
translate russia beginIntro_be849fa3:

    # "???" "You can stop cleaning now, mon ami.{w} Would you like to know a secret...?"
    "???" "Можешь заканчивать с уборкой, mon ami.{w} Хочешь узнать секретик...?"

# game/intro.rpy:158
translate russia beginIntro_0c282acc:

    # "???" "We're busting out of zis place soon. {w}Haven't you noticed ze chaos going on upstairs?"
    "???" "Мы скоро покинем это местечко. {w} Разве ты не заметил хаос, происходящий наверху?"

# game/intro.rpy:160
translate russia beginIntro_3fcc1eef:

    # yInmate "Chaos...?"
    yInmate "Хаос...?"

# game/intro.rpy:161
translate russia beginIntro_04946c27:

    # "You listen closely and hear sirens and what appears to be gunshots going off in the outside world."
    "Вы прислушиваетесь. Звук сирены и выстрелов доносятся где-то снаружи."

# game/intro.rpy:162
translate russia beginIntro_66a9666b:

    # yInmate "Oh sorry. I didn't notice. I'm from Chicago so this isn't anything new."
    yInmate "Ой я даже как-то и не заметил. Я из Чикаго, так что для меня это в порядке вещей."

# game/intro.rpy:163
translate russia beginIntro_d86bc299:

    # "???" "Just wait for ze lights to flicker~...."
    "???" "Просто подожди, пока свет замерцает ~...."

# game/intro.rpy:164
translate russia beginIntro_716c5ad4:

    # yInmate "The ligh-...?"
    yInmate "Свет-...?"

# game/intro.rpy:188
translate russia beginIntro_0036bb31:

    # "You hear a loud explosion happening overhead."
    "Вы слышите громкий взрыв где-то над вами."

# game/intro.rpy:189
translate russia beginIntro_792c1f78:

    # yInmate "What the...?!"
    yInmate "Что за...?!"

# game/intro.rpy:190
translate russia beginIntro_838607e5:

    # "You see the inmates smirk as one by one their celldoors open."
    "Вы видите, как заключенные ухмыляются, когда камеры одна за другой открываются."

# game/intro.rpy:195
translate russia beginIntro_11f8eaab:

    # "???" "Zis is your chance! Come with us and escape!"
    "???" "Это твой шансик, побежали с нами!"

# game/intro.rpy:196
translate russia beginIntro_306ff346:

    # yInmate "I appreciate the offer, but I'm not done cleaning yet."
    yInmate "Я ценю твое предложение, но я ещё не закончил с уборкой."

# game/intro.rpy:197
translate russia beginIntro_6cd0d75c:

    # "???" "....................."
    "???" "....................."

# game/intro.rpy:198
translate russia beginIntro_92377c23:

    # "???" "You really are insane...~"
    "???" "Ты действительно чокнулся...~"

# game/intro.rpy:199
translate russia beginIntro_256e05d1:

    # "???" "Come on everybody! Let's get out of here!"
    "???" "Народ! Пора выбираться от сюда!"

# game/intro.rpy:203
translate russia beginIntro_93c8cf0c:

    # "In a storm, the inmates make a run for it, rushing their way out of the cellblock."
    "В суматохе все заключённые покидают свои камеры."

# game/intro.rpy:204
translate russia beginIntro_205e02de:

    # yInmate "..............."
    yInmate "..............."

# game/intro.rpy:208
translate russia beginIntro_5103a77d:

    # yInmate "Like my old nan always says. {i}'You might scream and cry and hollar, but a dollar is still a dollar.'{/i}."
    yInmate "Как всегда, говорила моя тётка: {i}'Ты можешь кричать, плакать и биться в агонии, но доллар все равно останется долларом.'{/i}."

# game/intro.rpy:209
translate russia beginIntro_20192415:

    # yInmate "..........."
    yInmate "..........."

# game/intro.rpy:210
translate russia beginIntro_3b225d24:

    # yInmate "Then again, she also said the Emancipation Proclamation was a mistake...{w} We don't talk much to her anymore."
    yInmate "С другой стороны, она также говорила, что Прокламация об освобождении рабов была ошибкой....{w} Мы давненько не общались с ней."

# game/intro.rpy:213
translate russia beginIntro_335c0d89:

    # "You spend a little bit more time cleaning until everything sparkles."
    "Вы тратите немного времени на уборку, пока все во круг не начало блестеть."

# game/intro.rpy:214
translate russia beginIntro_39746c0f:

    # yInmate "Wow... my reflection {i}'is'{/i} handsome!"
    yInmate "Ого... мое отражение {i}'оно'{/i} офигенное!"

# game/intro.rpy:215
translate russia beginIntro_38b84761:

    # "You're satisfied with a job well done and exit the detention area."
    "Вы довольны хорошо выполненной работой и выходите из зоны содержания."

# game/intro.rpy:219
translate russia beginIntro_415ab3fc:

    # yInmate "Like my old nan always says. {i}'Better shoot a cop than work with a mop!'{/i}."
    yInmate "Как всегда, говорила моя тётка: {i}'Лучше застрелить копа, чем работать шваброй!'{/i}."

# game/intro.rpy:220
translate russia beginIntro_20192415_1:

    # yInmate "..........."
    yInmate "..........."

# game/intro.rpy:221
translate russia beginIntro_3b225d24_1:

    # yInmate "Then again, she also said the Emancipation Proclamation was a mistake...{w} We don't talk much to her anymore."
    yInmate "С другой стороны, она также говорила, что Прокламация об освобождении рабов была ошибкой....{w} Мы давненько не общались с ней."

# game/intro.rpy:222
translate russia beginIntro_8d297c65:

    # "Dropping your mop, you make your way to the exit of the prisonblock."
    "Бросив швабру, вы пробираетесь к выходу из тюремного блока."

# game/intro.rpy:226
translate russia beginIntro_549abbcf:

    # yInmate "Now let's get out of here and start my new life."
    yInmate "Пора сваливать отсюда и начинать новую жизнь."

# game/intro.rpy:227
translate russia beginIntro_ba6dc0bb:

    # yInmate "This elevator seems to head upstairs to the exit..."
    yInmate "Похоже, что этот лифт, направляется наверх к выходу..."

# game/intro.rpy:233
translate russia tutStage2_a3a882c8:

    # y "Yeah, I think I'm good to go."
    y "Думаю можно сваливать."

# game/intro.rpy:234
translate russia tutStage2_c418b099:

    # y "I can't wait to get a job and begin my new well adjusted and respectable life!{w} Maybe a doctor, a lawyer...{w} Or a telemarketeer!"
    y "Не могу дождаться, чтобы найти работу и начать свою новую, хорошо налаженную и респектабельную жизнь!{w} Может стану, врачом, юристом...{w} Или телемаркетологом!"

# game/intro.rpy:236
translate russia tutStage2_bfb040cc:

    # "You step into the elevator."
    "Вы входите в лифт."

# game/intro.rpy:237
translate russia tutStage2_8bfa6204:

    # y "Hmm.... ground floor, prison block or... bullet train?"
    y "Хм.... первый этаж, тюремный блок или... сверхскоростной поезд?"

# game/intro.rpy:238
translate russia tutStage2_6af9fa81:

    # y "It looks like this place is connected to WOOHP HQ via train."
    y "Похоже, что это место связано со штаб-квартирой «WOOHP» поездом."

# game/intro.rpy:239
translate russia tutStage2_8b4bcd7e:

    # "Opting not to take your chances, you head to ground level."
    "Решив не рисковать, вы направляетесь на первый этаж."

# game/intro.rpy:242
translate russia tutStage2_34d5447b:

    # y "......................."
    y "......................."

# game/intro.rpy:243
translate russia tutStage2_33237942:

    # y "Oh right, the violence in the streets...."
    y "Точно, \"Хаос на улицах\""

# game/intro.rpy:244
translate russia tutStage2_4007e1ff:

    # "You look around."
    "Вы осматриваетесь."

# game/intro.rpy:245
translate russia tutStage2_f871de75:

    # y "Where am I? It looks like some old timey....{w} Milkshake bar!{w} This place looks like it hasn't been used since the 50s."
    y "Где это я? Это похоже на какой-то старомодный....{w} Молочно коктейльный-бар! {w} Это место выглядит так, как будто его не использовали с 50-х годов."

# game/intro.rpy:246
translate russia tutStage2_b9b2a9fa:

    # y "Well I'm not gonna get a job in a dump like this. Best head outside."
    y "Ну что же я не собираюсь работать на такой свалке. По коням!"

# game/intro.rpy:249
translate russia tutStage2_2850a1e9:

    # "You step outside and are greeted by armageddon."
    "Вы выходите на улицу и вас встречает Армагеддон."

# game/intro.rpy:250
translate russia tutStage2_152b4cbd:

    # "Everywhere you look, you see gangs fighting in the open street."
       "Куда бы вы ни посмотрели, вы видите банды, сражающиеся между собой на открытой улице."

# game/intro.rpy:251
translate russia tutStage2_286d0a18:

    # "Police cars are burning, shops are being looted and gunfire can be heard around every corner."
      "Полицейские машины горят, магазины грабят, и за каждым углом слышна стрельба."

# game/intro.rpy:252
translate russia tutStage2_cf630fb5:

    # y "..................."
    y "..................."

# game/intro.rpy:253
translate russia tutStage2_5df9405f:

    # "You decided to head back inside."
    "Вы решили вернуться внутрь."

# game/intro.rpy:255
translate russia tutStage2_c0b0888d:

    # y "Yeah... Let's wait until things calm down a bit...~"
    y "Дааа уж... Подожду немного, пока все немного не уляжется....~"

# game/intro.rpy:258
translate russia tutStage2_f1d3701b:

    # y "All this violence is making me thirsty."
    y "От всего этого насилие, у меня в горле пересохло."

# game/intro.rpy:259
translate russia tutStage2_1e621ad4:

    # y "Let's see if I can pour myself a shake!"
    y "Налью´ка я себе коктейля!"

# game/intro.rpy:260
translate russia tutStage2_a80136b9:

    # "You pour yourself a milkshake."
    "Вы наливаете себе молочный коктейль."

# game/intro.rpy:261
translate russia tutStage2_c26a8916:

    # y "Delicious!"
    y "Вкуснотища!"

# game/intro.rpy:262
translate russia tutStage2_7597c3a3:

    # "........{w}........{w}........"
    "........{w}........{w}........"

# game/intro.rpy:263
translate russia tutStage2_9cb97b48:

    # y "........................"
    y "........................"

# game/intro.rpy:264
translate russia tutStage2_0f8ee318:

    # y "Funny, I expected it to-..."
    y "Забавно, я думал что это будет-..."

# game/intro.rpy:269
translate russia tutStage2_462f2b4b:

    # "You collapse from food poisoning."
    "Вы падаете в обморок от пищевого отравления."

# game/intro.rpy:270
translate russia tutStage2_7b98d66c:

    # "You spend the rest of the day on the toilet. When the sun sets, the violence seems to die down a little."
    "Остаток дня вы проводите в туалете. Когда солнце садится, звуки насилие снаружи потихоньку стихают."

# game/intro.rpy:272
translate russia tutStage2_db706f98:

    # y "Again with the cleaning... Maybe I should try getting a job as janitor after this..."
    y "Опять уборка... Может мне, стоит попробовать себя в роли уборщика после всего этого..."

# game/intro.rpy:275
translate russia tutStage2_c6e87893:

    # "You spend some time cleaning the milkshake bar. It looks nicer than before, but still needs some work."
    "Вы тратите некоторое время на уборку в баре. Он выглядит приятнее, чем раньше, но все ещё нуждается в некоторой доработке."

# game/intro.rpy:279
translate russia tutStage2_86334e5d:

    # y "Sounds like the fighting is dying down..."
    y "Похоже, что снаружи все заканчивается..."

# game/intro.rpy:280
translate russia tutStage2_130caf94:

    # y "But it's also getting dark. I'm not sure if it's a smart move to go out now."
    y "Но также становится темно. Я не уверен, разумно ли, выходить сейчас наружу."

# game/intro.rpy:281
translate russia tutStage2_24d693f5:

    # y "Guess I'll spend the night in the base, just to be sure."
    y "Думаю, я проведу ночь на базе, просто, чтобы быть уверенным."

# game/intro.rpy:284
translate russia tutStage2_42804f32:

    # y "Deadly gun violence up here... or safe living conditions downstairs..."
    y "Смертоносная бойня с применением огнестрельного оружия здесь... или безопасные условия жизни внизу..."

# game/intro.rpy:285
translate russia tutStage2_831ef747:

    # y "I guess I can wait until the violence calms down a bit."
    y "Думаю я немного подожду, пока снаружи всё немного не успокоится."

# game/intro.rpy:288
translate russia tutStage2_b3eaa82b:

    # "You spend the rest of the day in the base. When it turns dark, the violence seems to calm down a little."
    "Остаток дня вы проводите на базе. Когда солнце садится, звуки насилие потихоньку стихают."

# game/intro.rpy:292
translate russia tutStage2_14f4b337:

    # "{b}*Beep* *Beep* *Beep*{/b}"
    "{b}*Бип* *Бип* *Бип*{/b}"

# game/intro.rpy:293
translate russia tutStage2_1ce9c404:

    # y "...?"
    y "...?"

# game/intro.rpy:294
translate russia tutStage2_f39e93a4:

    # y "A distress signal?{w} {b}*Beep*{/b}"
    y "Сигнал бедствия?{w} {b}*Бип*{/b}"

# game/intro.rpy:302
translate russia tutStage2_f473ecc0:

    # a "{b}*Quietly:*{/b} Jerry is that you?"
    a "{b}*Тихо:*{/b} Джерри это ты?"

# game/intro.rpy:303
translate russia tutStage2_ee99fffe:

    # y "Er..."
    y "Ехх..."

# game/intro.rpy:309
translate russia tutStage2_a69ff082:

    # s g31 "Listen, we're hiding out at school. We're okay, but we've been attacked by our own WOOHP Agents."
    s g31 "Послушай, мы прячемся в школе. Мы в порядке, но на нас напали наши собственные Агенты из «WOOHP»."

# game/intro.rpy:315
translate russia tutStage2_81d4d292:

    # c "And now Beverly Hills is turning into a warzone! That's like... totally not cool!"
    c "И теперь Беверли-Хиллз превращается в зону боевых действий! Это как-то типа... совсем не круто!"

# game/intro.rpy:316
translate russia tutStage2_86ce9ff8:

    # s "Can you pull us out of here via the WOOHP Tunnel Network?"
    s "Можешь вытащить нас отсюда через туннельную сеть «WOOHP»?"

# game/intro.rpy:317
translate russia tutStage2_05bf074c:

    # y ".............."
    y ".............."

# game/intro.rpy:318
translate russia tutStage2_d2953955:

    # y "Suuure~...."
    y "Еммм да, конечно~...."

# game/intro.rpy:319
translate russia tutStage2_bd13094c:

    # "You press some random buttons on the console."
    "Вы нажимаете случайные кнопки на консоли."

# game/intro.rpy:320
translate russia tutStage2_692be6fa:

    # y "............"
    y "............"

# game/intro.rpy:321
translate russia tutStage2_04c45232:

    # y "It says right here that I can only pull you out one at a time."
    y "Здесь написано, что я могу вытащить вас только по одиночке."

# game/intro.rpy:322
translate russia tutStage2_3af91924:

    # s g32 "That's fine, but please just hurry."
    s g32 "Да, хорошо. Только, пожалуйста, просто поторопись."

# game/intro.rpy:328
translate russia tutSpySelect_f65fca2e:

    # y "Ye okay."
    y "Отлично."

# game/intro.rpy:329
translate russia tutSpySelect_8fcb26e6:

    # y "{b}*Bleep*{/b}"
    y "{b}*Бип*{/b}"

# game/intro.rpy:343
translate russia tutSpySelect_8043824e:

    # "The screen shows the spy in green being sucked into a giant funnel!"
    "На экране видно, как шпионку в зелёном засасывает в гигантскую воронку!"

# game/intro.rpy:348
translate russia tutSpySelect_237d88d0:

    # "Moments later you hear a swooshing noise as a panel in the roof opens up."
    "Через несколько мгновений вы слышите свистящий шум. Панель в крыше открывается."

# game/intro.rpy:354
translate russia tutSpySelect_147a7e75:

    # s "Oof~...!"
    s "Хух~...!"

# game/intro.rpy:355
translate russia tutSpySelect_e9584709:

    # s "Just in time Jerry! Now pull out the othe-..."
    s "Как раз вовремя Джерри! Теперь вытащи ост-..."

# game/intro.rpy:356
translate russia tutSpySelect_29f00d05:

    # s g28 "...?"
    s g28 "...?"

# game/intro.rpy:357
translate russia tutSpySelect_c718aa7a:

    # s "You're not Jerry!"
    s "Ты не Джерри!"

# game/intro.rpy:358
translate russia tutSpySelect_25a929e4:

    # y "No, actually I'm....{w} Errr...{w} The new guy! It's my first day on the jo-..."
    y "Нет, на самом деле я....{w} Еммм...{w} Я тут новенький! Это мой первый день на рабо-..."

# game/intro.rpy:361
translate russia tutSpySelect_4cda7408:

    # "With lightning quick reflexes, the spy jumps on you and puts you in a headlock!"
    "Молниеносно, шпионка прыгает на вас и применяет простейший захват!"

# game/intro.rpy:364
translate russia tutSpySelect_f358cb0c:

    # y "................."
    y "................."

# game/intro.rpy:365
translate russia tutSpySelect_87e557fb:

    # y "Why?"
    y "За что?"

# game/intro.rpy:367
translate russia tutSpySelect_35103d89:

    # s "Don't move Agent! We've been fighting you guys all day! I don't know what's going on, but you're not getting the drop on me."
    s "Не двигайся агент! Мы сражаемся с вами целый день! Я не знаю, что здесь происходит, но ты не застанешь меня врасплох."

# game/intro.rpy:368
translate russia tutSpySelect_1e924af9:

    # y "K."
    y "К."

# game/intro.rpy:369
translate russia tutSpySelect_384ba19d:

    # s g17 "What do you mean 'K'?"
    s g17 "Что значит 'К'?"

# game/intro.rpy:370
translate russia tutSpySelect_8bfd8a4d:

    # y "I mean I won't attack."
    y "Я хотел сказать, что не собирался атаковать тебя."

# game/intro.rpy:371
translate russia tutSpySelect_dece88cf:

    # s g8 "Nice try! But you're not fooling anyone!"
    s g8 "Хорошая попытка! Но ты меня не обманешь!"

# game/intro.rpy:372
translate russia tutSpySelect_d3a59cb5:

    # y "............................."
    y "............................."

# game/intro.rpy:373
translate russia tutSpySelect_3270dd5c:

    # s g32 "............................."
    s g32 "............................."

# game/intro.rpy:374
translate russia tutSpySelect_10fd81d9:

    # "................................"
    "................................"

# game/intro.rpy:375
translate russia tutSpySelect_998513c5:

    # y "I'm sorta getting a cramp in my neck, so if you could~...."
    y "У меня это, ... немного шея затекла, ты не могла бы~...."

# game/intro.rpy:376
translate russia tutSpySelect_94e81ec3:

    # "The spy narrows her eyes as she gives you a cautious look."
    "Шпионка сужает глаза и осторожно смотрит на вас."

# game/intro.rpy:377
translate russia tutSpySelect_69788bbe:

    # s g31 "You... sure don't act like the other agents..."
    s g31 "Ты... и правда не такой, как другие агенты..."

# game/intro.rpy:378
translate russia tutSpySelect_a8f4e044:

    # "The spy frees you from the neckhold."
    "Шпионка отпускает вас."

# game/intro.rpy:379
translate russia tutSpySelect_26cc5d85:

    # y "Didn't your friends need help or something...?"
    y "Разве твоим друзьям не нужна была помощь или что-то типа того...?"

# game/intro.rpy:380
translate russia tutSpySelect_e32968a0:

    # s g28 "O-oh! You're rig-...!"
    s g28 "О-ох! Ты пра-...!"

# game/intro.rpy:381
translate russia tutSpySelect_77f75d40:

    # "*{b}Bleep* *Bleep* *Bleep*{/b}"
    "*{b}Бип* *Бип* *Бип*{/b}"

# game/intro.rpy:382
translate russia tutSpySelect_1ae19595:

    # s "It's the distress signal!"
    s "Это сигнал бедствия!"

# game/intro.rpy:385
translate russia tutSpySelect_9fc93384:

    # "The two of you run over to the console and answer the call."
    "Вы вдвоем подбегаете к консоли и отвечаете на вызов."

# game/intro.rpy:394
translate russia tutSpySelect_0b66e489:

    # a "Sam! They found us!"
    a "Сэм! Они нашли нас!"

# game/intro.rpy:395
translate russia tutSpySelect_e0ba639d:

    # c "Please get us out of here!"
    c "Пожалуйста, вытащи нас отсюда!"

# game/intro.rpy:396
translate russia tutSpySelect_9a198eff:

    # sM "Hang on, I got you!"
    sM "Держитесь, я сейчас!"

# game/intro.rpy:409
translate russia tutSpySelect_723e1470:

    # "{b}*Static*{/b}"
    "{b}*Неподвижно*{/b}"

# game/intro.rpy:410
translate russia tutSpySelect_f7953e45:

    # sM "Clover?! Alex?!"
    sM "Кловер?! Алекс?!"

# game/intro.rpy:415
translate russia tutSpySelect_e34ea306:

    # s "The tunnel system! It's not responding!"
    s "Туннельная система! Она не отвечает!"

# game/intro.rpy:416
translate russia tutSpySelect_8dbf445a:

    # s g14 "No....{w} They must have destroyed it..."
    s g14 "Нееет....{w} Они, должно быть, сломали её...."

# game/intro.rpy:417
translate russia tutSpySelect_d59bd64c:

    # y "{i}'They'{/i}?"
    y "{i}'Они'{/i}?"

# game/intro.rpy:418
translate russia tutSpySelect_7b9776c7:

    # s g15 "............."
    s g15 "............."

# game/intro.rpy:419
translate russia tutSpySelect_71fd6dfe:

    # s "WOOHP... The Agency... It's gone rogue."
    s "«WOOHP»... Агентство... Оно вышло из-под контроля."

# game/intro.rpy:420
translate russia tutSpySelect_e9d5801a:

    # s g37 "All of our Agents just... turned on us. In the middle of school no less! I was busy studying!"
    s g37 "Все наши агенты просто... предали нас и напали. Да ещё и посреди школы! Вовремя учёбы!"

# game/intro.rpy:421
translate russia tutSpySelect_1db49ca3:

    # y "........."
    y "........."

# game/intro.rpy:422
translate russia tutSpySelect_2be06fe3:

    # s g34 "They brought in gangs from out of state and began causing chaos in the city."
    s g34 "Вместе с бандами из других штатов они начали сеять хаос в городе."

# game/intro.rpy:423
translate russia tutSpySelect_bf1e4e59:

    # s "They've declared Beverly Hills lawless. Turning it into a playground for gangsters and thugs."
    s "Они объявили Беверли-Хиллз беззаконием. Превратив его в игровую площадку для бандитов и головорезов."

# game/intro.rpy:424
translate russia tutSpySelect_bf4f584c:

    # y "Man that must suck for you."
    y "Хреново вам..."

# game/intro.rpy:425
translate russia tutSpySelect_e369c37d:

    # s g28 "It sucks for you too!"
    s g28 "Нам!? Тебя это тоже касается!"

# game/intro.rpy:426
translate russia tutSpySelect_27f01e9b:

    # y "It does?"
    y "Касается? Меня?"

# game/intro.rpy:427
translate russia tutSpySelect_47d00f51:

    # s g8 "Of course! You're a WOOHP Agent yourself, right?"
    s g8 "Конечно! Ты же агент «WOOHP», не так ли?"

# game/intro.rpy:428
translate russia tutSpySelect_c1fe963c:

    # y "Oh right... about that~..."
    y "Ах да... на счёт этого~..."

# game/intro.rpy:429
translate russia tutSpySelect_a8c18fe7:

    # y "I was only one day away from retirement so..."
    y "До пенсии мне оставался всего один день, так что..."

# game/intro.rpy:430
translate russia tutSpySelect_35c2efb4:

    # s g34 "You said it was your first day."
    s g34 "Ты говорил, что сегодня твой первый день."

# game/intro.rpy:431
translate russia tutSpySelect_9cb97b48:

    # y "........................"
    y "........................"

# game/intro.rpy:432
translate russia tutSpySelect_c11973ca:

    # y "So about that~...."
    y "Ехх да, а на счёт этого~...."

# game/intro.rpy:433
translate russia tutSpySelect_dc8a3f18:

    # s g9 "Ugh, okay fine. What do you want?"
    s g9 "Ладно, хорошо. Чего ты хочешь?"

# game/intro.rpy:434
translate russia tutSpySelect_e93fe854:

    # y "Huh...?"
    y "Хмм...?"

# game/intro.rpy:435
translate russia tutSpySelect_f39de257:

    # s "I'm going to need your help and you must want something. Just tell me what you want in return for your help."
    s "Мне понадобится твоя помощь, а ты, точно захочешь что-нибудь взамен. Просто скажите мне, чего ты хочешь."

# game/intro.rpy:436
translate russia tutSpySelect_b5d3c39c:

    # y "...."
    y "...."

# game/intro.rpy:440
translate russia tutSpySelect_68d02c5e:

    # y "Absolute unchallenged POWER!"
    y "Абсолютная и неоспоримая ВЛАСТЬ!"

# game/intro.rpy:441
translate russia tutSpySelect_1c683039:

    # s g34 "Are you willing to work for it?"
    s g34 "Готов ли ты работать ради этого?"

# game/intro.rpy:442
translate russia tutSpySelect_d1642ce7:

    # y "Huh?"
    y "Хмм?"

# game/intro.rpy:443
translate russia tutSpySelect_6e940db4:

    # s g31 "Are you willing to put in the time and effort?"
    s g31 "Готов ли ты потратить уйму услий и времени ради этого?"

# game/intro.rpy:444
translate russia tutSpySelect_ac247651:

    # s "If so, then I'll make sure you become one of the bigshots at WOOHP. Then you'll have all the power you could ever want."
    s "Если да, то я позабочусь о том, чтобы ты занял одну из главных позиций в \"WOOHP\". Тогда у тебя будет власть, которую ты только можешь пожелать."

# game/intro.rpy:445
translate russia tutSpySelect_ce895a18:

    # y "..........."
    y "..........."

# game/intro.rpy:446
translate russia tutSpySelect_84be3c46:

    # y "Tempting~..."
    y "Заманчиво~..."

# game/intro.rpy:447
translate russia tutSpySelect_a81773f3:

    # s "But for now WOOHP is under enemy control. So I'll need you as a plan B."
    s "Но пока \"WOOHP\" находится под контролем врагов. Так что ты нужен мне в качестве плана Б."

# game/intro.rpy:448
translate russia tutSpySelect_5ea1aeda:

    # y "Er... deal?"
    y "Емм... договорились?"

# game/intro.rpy:450
translate russia tutSpySelect_52597dcf:

    # y "Money would be nice."
    y "Деньги были бы кстати."

# game/intro.rpy:451
translate russia tutSpySelect_9211c199:

    # s g34 "How much money?"
    s g34 "Сколько ты хочешь?"

# game/intro.rpy:452
translate russia tutSpySelect_908db26e:

    # y "Like...{w} I dunno~...{w} A million dollars!"
    y "Нууу...{w} Думаю~...{w} Миллион долларов!"

# game/intro.rpy:453
translate russia tutSpySelect_245bb2ae:

    # s g31 "Are you willing to work for it?"
    s g31 "Готов ли ты работать ради этого?"

# game/intro.rpy:454
translate russia tutSpySelect_ce895a18_1:

    # y "..........."
    y "..........."

# game/intro.rpy:455
translate russia tutSpySelect_d239ed31:

    # y "Wait what?"
    y "Подожди, что?"

# game/intro.rpy:456
translate russia tutSpySelect_c134196e:

    # s "Are you willing to work for it?"
    s "Готов ли ты работать ради этого?"

# game/intro.rpy:457
translate russia tutSpySelect_794bc49c:

    # y "Yeeees~...?"
    y "Даааа~...?"

# game/intro.rpy:458
translate russia tutSpySelect_d32e4799:

    # s g34 "Fine, then I'll make sure that after all this is over, you'll be rewarded with a million dollars."
    s g34 "Отлично, тогда я позабочусь, о том чтобы когда все это закончилось, тебя вознаградили миллионом долларов."

# game/intro.rpy:459
translate russia tutSpySelect_b488314a:

    # y "Are you serious? Sweet!"
    y "Ты серьезно? Прикольно!"

# game/intro.rpy:460
translate russia tutSpySelect_2d32c385:

    # s g31 "BUT!{w} You need to be willing to work for it. If we're going to take back Beverly Hills and restore order, then you're gonna have to put in the effort."
    s g31 "Но! {w} Ты должен быть готов работать ради них. Если мы собираемся вернуть Беверли-Хиллз и навести порядок, то тебе придется приложить усилия."

# game/intro.rpy:461
translate russia tutSpySelect_7b057197:

    # y "Deal!"
    y "Договорились!"

# game/intro.rpy:463
translate russia tutSpySelect_57e586ab:

    # y "......"
    y "......"

# game/intro.rpy:464
translate russia tutSpySelect_162a687c:

    # y "Tiddies."
    y "Сиськи."

# game/intro.rpy:466
translate russia tutSpySelect_a42a839a:

    # s g8 "WHAT?! How vulgar!"
    s g8 "ЧЕГО?! Как вульгарно!"

# game/intro.rpy:467
translate russia tutSpySelect_29b18881:

    # s g38 "I'm not showing you mine, if that's what you're implying!"
    s g38 "Свои я тебе не покажу, если это то что ты имеешь в виду!"

# game/intro.rpy:468
translate russia tutSpySelect_35e10432:

    # y "Don't flatter yourself. I want to see some good tiddies."
    y "Не обольщайся. Я просто хочу увидеть пару отличных грудей."

# game/intro.rpy:470
translate russia tutSpySelect_484b48ba:

    # s g16 "W-what...?!"
    s g16 "Ч-что...?!"

# game/intro.rpy:471
translate russia tutSpySelect_d46746eb:

    # y "You know the ones you want to show off to your friends and brag about."
    y "Ну знаешь те, которыми ты все хочешь похвастаться перед друзьями."

# game/intro.rpy:472
translate russia tutSpySelect_fc5c8b89:

    # s g11 "................."
    s g11 "................."

# game/intro.rpy:473
translate russia tutSpySelect_12435024:

    # s g12 "Well! .... I don't know how to get you those, okay?!"
    s g12 "Хорошо! .... Я не знаю, где тебе их достать, ясно?!"

# game/intro.rpy:474
translate russia tutSpySelect_7201875a:

    # y "You're in college, you must know some good strippers."
    y "Ты учишься в колледже и должена знать хороших стриптизерш."

# game/intro.rpy:475
translate russia tutSpySelect_09d534fd:

    # s g8 "No! I don't!"
    s g8 "Нет! Я не знаю!"

# game/intro.rpy:476
translate russia tutSpySelect_78d9f02f:

    # y "Oh well.{w} Good luck with the whole WOOHP problem."
    y "Ну и ладно. {w} Удачи тогда с твоей небольшой проблемкой в WOOHP."

# game/intro.rpy:477
translate russia tutSpySelect_3d74d1fd:

    # s g28 "Wait! Where are you going?"
    s g28 "Стой! Ты куда?"

# game/intro.rpy:478
translate russia tutSpySelect_e26bdf73:

    # y "To find some tiddies!"
    y "На поиски титечек!"

# game/intro.rpy:479
translate russia tutSpySelect_23d9670c:

    # s g9 "{b}*Groans*{/b}"
    s g9 "{b}*Стон*{/b}"

# game/intro.rpy:480
translate russia tutSpySelect_2b4425d2:

    # s "If I show you mine~...{w} Do you then promise to help...?"
    s "Если я покажу тебе свои~...{w} Обещаешь помочь...?"

# game/intro.rpy:481
translate russia tutSpySelect_d74f8676:

    # y "Maybe. They gotta be good tiddies though."
    y "Возможно. Но это должны быть чертовски хорошие титечки."

# game/intro.rpy:482
translate russia tutSpySelect_92f5be84:

    # s "......................"
    s "......................"

# game/intro.rpy:483
translate russia tutSpySelect_d5fcedd5:

    # s g14 "{b}*Sighs*{/b} Fine~..."
    s g14 "{b}*Вздыхает*{/b} Ладно~..."

# game/intro.rpy:491
translate russia tutSpySelect_bc2a54f4:

    # s "........................."
    s "........................."

# game/intro.rpy:492
translate russia tutSpySelect_9b3fbbda:

    # y "Okay I take it back....{w} Them be some good tiddies."
    y "Ооо дааа....{w} Они чертовски хорооши."

# game/intro.rpy:493
translate russia tutSpySelect_ee8f229e:

    # s g12 "{b}*Sarcastic*{/b} Gee~... thanks."
    s g12 "{b}*саркастично*{/b} бъе~... спасибо."

# game/intro.rpy:494
translate russia tutSpySelect_98ccd698:

    # s g16 "So will you help now?"
    s g16 "Ну как насчет помощи?"

# game/intro.rpy:495
translate russia tutSpySelect_2b0c5069:

    # y "Yeah why not. Not like I got anything better to do anyways."
    y "Да, почему бы и нет. Мне не то что бы есть чем заняться в данной ситуации."

# game/intro.rpy:503
translate russia tutSpySelect_6997c6e7:

    # s g38 "Fine, then let's discuss a plan of action."
    s g38 "Отлично, тогда давайте обсудим наш план действий."

# game/intro.rpy:504
translate russia tutSpySelect_08bf1216:

    # s g34 "We should focus on infiltrating WOOHP, taking out the guards and fighting our way to the top. Then, we can confront the person who's behind this and put them in handcuffs!"
    s g34 "Мы должны сосредоточиться на том, чтобы проникнуть в WOOHP, уничтожить охранников и пробить себе путь наверх. Затем мы сможем противостоять тому, кто за этим стоит, и заковать его в наручники!"

# game/intro.rpy:505
translate russia tutSpySelect_9a78f174:

    # y "................"
    y "................"

# game/intro.rpy:506
translate russia tutSpySelect_fa2694f9:

    # y "Solid plan. Now here's my proposal...{w} We don't do any of the things you just said."
    y "Надёжный план. Вот мое предложение...{w} Мы не делаем ничего из того, что ты только что сказала."

# game/intro.rpy:507
translate russia tutSpySelect_ef737003:

    # s g28 "What? Why not?!"
    s g28 "Что? Почему нет?"

# game/intro.rpy:508
translate russia tutSpySelect_382419c5:

    # y "You want us to take on an entire organization by ourselves? That's gonna get us killed."
    y "Ты хочешь, чтобы мы в одиночку расправились с целой организацией? Да мы точно сдохнем там."

# game/intro.rpy:509
translate russia tutSpySelect_770ddf47:

    # y "How about instead, we lay low for a while. Figure out what the situation is on the streets and why WOOHP turned rogue in the first place?"
    y "Как насчет того, чтобы затаиться на некоторое время. Выяснить, какова ситуация на улицах и почему WOOHP вообще переметнулся на другую сторону?"

# game/intro.rpy:510
translate russia tutSpySelect_a5164aef:

    # s g38 "Ehrm... I'm sorry, but {i}'who'{/i} is the experienced super spy here?"
    s g38 "Эмм... Извини, но {i}'кто'{/i} из нас здесь опытный супер-шпион?"

# game/intro.rpy:511
translate russia tutSpySelect_749a0911:

    # y "You are....?"
    y "Ты....?"

# game/intro.rpy:512
translate russia tutSpySelect_73643d6f:

    # s "Exactly. Now stop arguing and just do what I tell you."
    s "В точку! А теперь хватит спорить, просто делай то что я говорю."

# game/intro.rpy:513
translate russia tutSpySelect_9500a0b4:

    # y "......................"
    y "......................"

# game/intro.rpy:514
translate russia tutSpySelect_8dc30fd9:

    # s g39 "......................"
    s g39 "......................"

# game/intro.rpy:515
translate russia tutSpySelect_0b5d58bb:

    # s g12 "Well...."
    s g12 "Ну лааадно..."

# game/intro.rpy:516
translate russia tutSpySelect_b7eb723e:

    # s "You might have a point about taking on the entire organization with just the two of us..."
    s "Возможно, ты прав насчет того, что мы вдвоем не справимся с целой организацией..."

# game/intro.rpy:517
translate russia tutSpySelect_76f4ad64:

    # y "Told ya."
    y "Говорил же."

# game/intro.rpy:518
translate russia tutSpySelect_781aaa13:

    # s g32 "Okay fine. I've decided that we'll lay low first and gather some intel."
    s g32 "Ладно, хорошо. Я решила, что сначала мы заляжем на дно и соберем кое-какие сведения."

# game/intro.rpy:519
translate russia tutSpySelect_db832180:

    # y "........"
    y "........"

# game/intro.rpy:520
translate russia tutSpySelect_a24baaa8:

    # y "Good! Then that's settled. {w}I'm going to bed."
    y "Отлично! Тогда все решено. {w}Я баиньки!"

# game/intro.rpy:521
translate russia tutSpySelect_2971dce1:

    # s g28 "Hey! You said you'd help me!"
    s g28 "Эй! Ты сказал что поможешь мне!"

# game/intro.rpy:522
translate russia tutSpySelect_81fd1949:

    # y "And I will... in the morning. Or do you fancy going outside at night with all the gangs around?"
    y "И я помогу... с утра. Или тебе нравиться гулять по тёмным улицам, кишащими бандитами?"

# game/intro.rpy:523
translate russia tutSpySelect_c1f28934:

    # s g32 "................."
    s g32 "................."

# game/intro.rpy:524
translate russia tutSpySelect_83baeee0:

    # s "Okay fine... We'll wait till tomorrow then..."
    s "Ладно, хорошо... Тогда подождем до завтра..."

# game/intro.rpy:528
translate russia tutSpySelect_4acb2cc6:

    # "The two of you find a comfortable cell to stay in and head to bed."
    "Вы вдвоём находите удобную камеру и ложитесь спать."

# game/intro.rpy:548
translate russia tutStage3_192bb970:

    # s "Good morning New Guy."
    s "С добрым утром новичок."

# game/intro.rpy:549
translate russia tutStage3_b1305b08:

    # y "Good morning. Get any rest?"
    y "Доброе утро. Отдохнула"

# game/intro.rpy:550
translate russia tutStage3_26d4c7f2:

    # s g14 "A little. I'm eager to get started today."
    s g14 "Немного. Мне не терпится приступить к сегодняшней работе."

# game/intro.rpy:551
translate russia tutStage3_bae564ff:

    # y "Let's get the lay of the land. See what's happening out there on the streets."
    y "Давай разберемся, что к чему. Посмотрим, что происходит на улицах."

# game/intro.rpy:552
translate russia tutStage3_6251560a:

    # s g38 "Well I think....!"
    s g38 "Ну, я думаю....!"

# game/intro.rpy:553
translate russia tutStage3_637d86d7:

    # s g39 "..................."
    s g39 "..................."

# game/intro.rpy:554
translate russia tutStage3_6cc7a4bc:

    # s g38 "Well I also think we should get a lay of the land."
    s g38 "Я тоже думаю, что нам нужно изучить местность."

# game/intro.rpy:560
translate russia tutStage3_e43ca421:

    # "The two of you take the elevator upstairs and step out of the milkshake bar."
    "Вдвоём вы поднимаетесь на лифте и выходите из бара."

# game/intro.rpy:564
translate russia tutStage3_57530650:

    # sM "Beverly Hills... Look what they did to it...."
    sM "Беверли-Хиллз... Посмотри, что они сделали с ним...."

# game/intro.rpy:565
translate russia tutStage3_0dc455f1:

    # y "Yeah it looks like we got our work cut out for us."
    y "Дааа, похоже, у нас есть над чем поработать."

# game/intro.rpy:566
translate russia tutStage3_6673e993:

    # sM "I'll do some digging around. You keep watch here and wait for me to come back."
    sM "Я немного покопаюсь в округе. А ты сторожи здесь и жди, пока я не вернусь."

# game/intro.rpy:567
translate russia tutStage3_1db49ca3:

    # y "........."
    y "........."

# game/intro.rpy:568
translate russia tutStage3_6c63f76b:

    # "Sam gets ready and heads out into the city."
    "Собравшись Сэм отправляется в город."

# game/intro.rpy:569
translate russia tutStage3_56163a2a:

    # y "Yeah I'm gonna do some exploring."
    y "Да, пойду тоже немного по изучаю окрестности."

# game/intro.rpy:576
translate russia tutStage3_5489650e:

    # c "Good morning New Guy."
    c "С добрым утром новичок."

# game/intro.rpy:577
translate russia tutStage3_729f874e:

    # y "Good morning. Got any rest?"
    y "Доброе утро. Отдохнула?"

# game/intro.rpy:578
translate russia tutStage3_827e5690:

    # c "A little. I'm eager to get started today."
    c "Да, немного. Не терпится приступить к работе."

# game/intro.rpy:579
translate russia tutStage3_6d8134ab:

    # y "Let's get a lay of the land. See what's happening out there on the streets."
    y "Давай посмотрим, что к чему и узнаем, что происходит на улицах."

# game/intro.rpy:585
translate russia tutStage3_e43ca421_1:

    # "The two of you take the elevator upstairs and step out of the milkshake bar."
    "Вы вдвоём поднимаетесь на лифте и выходите из бара."

# game/intro.rpy:589
translate russia tutStage3_2e433754:

    # cM "Beverly Hills... Look what they did to it....{w} My favorite shoe store is on fire!"
    cM "Беверли-Хиллз... Что же, они сделали с тобой....{w} Мой любимый обувной магазин, он горит!"

# game/intro.rpy:590
translate russia tutStage3_0dc455f1_1:

    # y "Yeah it looks like we got our work cut out for us."
    y "Да, похоже, у нас есть над чем поработать."

# game/intro.rpy:591
translate russia tutStage3_7d452feb:

    # cM "I'll do some digging around. I'll meet you back here tonight."
    cM "Я снова покопаюсь в округе. Встретимся здесь вечером."

# game/intro.rpy:592
translate russia tutStage3_2ab5fadc:

    # "Clover gets ready and heads out into the city."
    "Кловер собирается и отправляется в город."

# game/intro.rpy:593
translate russia tutStage3_4f03d40a:

    # y "Right, let's do some exploring."
    y "По исследуем немного, вперёд к приключениям!"

# game/intro.rpy:600
translate russia tutStage4_40f3c005:

    # "It's still early in the morning and everything is quiet. The streets are littered with gunshell casings, burned up cars and dumpsters, shattered glass and the occassional passed out drunk."
    "Сейчас все ещё раннее утро, и вокруг все спокойно. Улицы усеяны гильзами, сгоревшими машинами и мусорными контейнерами. Осколки стекла и случайные пьяницы в отключке хаотично разбросаны по тротуару."

# game/intro.rpy:601
translate russia tutStage4_12821e96:

    # y "{b}*Whistling*{/b}"
    y "{b}*Свист*{/b}"

# game/intro.rpy:602
translate russia tutStage4_f50332cc:

    # "Turning a corner you suddenly see a gangs of thugs listening to a girl on a soapbox."
    "Повернув за угол, вы вдруг видите банду головорезов, слушающих девушку на ораторском ящике."

# game/intro.rpy:603
translate russia tutStage4_33ccced8:

    # "As soon as they notice you, they come straight at you."
    "Как только они тебя заметят, они тут же набросятся на тебя."

# game/intro.rpy:604
translate russia tutStage4_a438d728:

    # y "{b}*Worried Whistling*{/b}"
    y "{b}*Беспокойный свист*{/b}"

# game/intro.rpy:605
translate russia tutStage4_64610f67:

    # "Thug 1" "You thought you could just come in on our turf and start whistling?!"
    "Головорез 1" "Ты думал, что просто можешь прийти на нашу территорию и начать тут свистеть?!"

# game/intro.rpy:606
translate russia tutStage4_aac6b6c7:

    # "Thug 2" "Hey my mom got hit by a driver who was whistling!"
    "Головорез 2" "Эй, мою маму сбил водитель, что постоянно свистел!"

# game/intro.rpy:607
translate russia tutStage4_86c2f517:

    # "Thug 1" "Who do you think you are? Coming here and reminding my friend of such a tragic incident. I think we need to teach you a lesson!"
    "Головорез 1" "Кем ты себя возомнил и как ты смеешь напоминать моему другу о столь трагичном инциденте в его жизни?! Думаю, нам стоит преподать тебе урок!"

# game/intro.rpy:608
translate russia tutStage4_7597f00a:

    # y "{b}*Paniced Whistling*{/b}"
    y "{b}*Панический свист*{/b}"

# game/intro.rpy:609
translate russia tutStage4_621f841f:

    # "???" "Un moment~... Zis one is with moi~..."
    "???" "Уно моментик~... Этот с муа~..."

# game/intro.rpy:622
translate russia tutStage4_6a4f6d74:

    # "???" "My my my, look who's made it out of ze asylum~..."
    "???" "Божечки мои, посмотрите, кто это выбрался из психушки..."

# game/intro.rpy:623
translate russia tutStage4_f868491c:

    # "???" "Why are you dressed like a WOOHP Agent...?"
    "???" "Почему ты одет как агентик WOOHP...?"

# game/intro.rpy:624
translate russia tutStage4_52211446:

    # y "Why are you dressed like a potted plant?"
    y "А почему ты одета как комнатное растение?"

# game/intro.rpy:625
translate russia tutStage4_d6842a75:

    # "???" "Oh! You don't know who I am? I am Silva Abegail. World famous botanist and plant-whisperer!"
    "???" "О! Ты не знаешь, кто я? Я Сильва Абегэйл. Всемирно известный ботаник и шептун-растениевод!"

# game/intro.rpy:626
translate russia tutStage4_42bd6c67:

    # "Silva" "Look around you. Not bad, non? Our own little corner of ze world, ripe for ze plunder~..."
    "Сильва" "Оглянись вокруг. Не плохо, правда? Наш маленький уголок мира, созревший для разграбления~..."

# game/intro.rpy:627
translate russia tutStage4_be96616f:

    # y "You planned all this?"
    y "Ты все это спланировала?"

# game/intro.rpy:628
translate russia tutStage4_9dcf8253:

    # "Silva" "Oh heaven no! That credit goes to ze mastermind of this operation. It was his idea to use WOOHP's nanobots against zem."
    "Сильва" "Ох, божечки, нет! Эта заслуга магистра, он организовал эту операцию. Это была его идея использовать наноботов WOOHP против них самих же."

# game/intro.rpy:629
translate russia tutStage4_55dcbf44:

    # y "Nanobots?!"
    y "Наноботов?!"

# game/intro.rpy:630
translate russia tutStage4_8574e139:

    # "Silva" "Nanobots."
    "Сильва" "Наноботов."

# game/intro.rpy:631
translate russia tutStage4_210e6283:

    # y "Nanobots....?"
    y "Наноботы....?"

# game/intro.rpy:632
translate russia tutStage4_68507934:

    # "Silva" "Oui... nanobots."
    "Сильва" "Даа... наноботы."

# game/intro.rpy:633
translate russia tutStage4_2e535746:

    # y "How...{w} cliché."
    y "Как...{w} клишировано."

# game/intro.rpy:634
translate russia tutStage4_32972633:

    # "Silva" "All that was required was a little... 'reprogramming'. And now zey are fighting for us!"
    "Сильва" "Все, что потребовалось, это небольшое... 'перепрограммирование'. И теперь они сражаются за нас."

# game/intro.rpy:635
translate russia tutStage4_aa7708e8:

    # "Silva" "Ze mastermind freed us from our cells to prove ourselves. We all took control of gangs here in ze city. I'm in command of 'Les Épines'."
    "Сильва" "Магистр освободил нас из наших камер, чтобы мы могли проявить себя. Мы все взяли под контроль разные банды в этом городе. Я командую 'Les Épines' *(Шипами)*."

# game/intro.rpy:636
translate russia tutStage4_79bcb7e5:

    # "Silva" "Ze one who manages to take over all of Beverly Hills can join ze mastermind in his conquest for world domination."
    "Сильва" "Тот, кому удастся захватить весь Беверли-Хиллз, сможет присоединиться к магистру в его борьбе за мировое господство."

# game/intro.rpy:637
translate russia tutStage4_5989d1f9:

    # "You turn your attention to the gangsters."
    "Вы обращаете свое внимание на бандитов."

# game/intro.rpy:638
translate russia tutStage4_a4bc582c:

    # y "And you're okay with this?"
    y "И вас ничего не смущает?"

# game/intro.rpy:639
translate russia tutStage4_cfd46a2d:

    # "Thug" "I'LL DO WHATEVER MISS ABEGAIL SAYS!!!"
    "Головорез" "Я СДЕЛАЮ ВСЕ, ЧТО СКАЖЕТ МИСС АБЕГЭЙЛ!!!"

# game/intro.rpy:640
translate russia tutStage4_d3a59cb5:

    # y "............................."
    y "............................."

# game/intro.rpy:641
translate russia tutStage4_a7020b7b:

    # "Silva" "Oh zey don't have much of a choice. {w}Afterall~... We've infected {i}them{/i} with nanobots aswell..."
    "Сильва" "Ох, ну у них особо нет выбора. {w}В конце концов~... Мы также заразили {i}их{/i} с помощью наноботов..."

# game/intro.rpy:642
translate russia tutStage4_81384f5a:

    # "Silva" "Now your presence here sort of leaves me in a bind~..."
    "Сильва" "Теперь твое присутствие ставит меня в затруднительное положение~..."

# game/intro.rpy:643
translate russia tutStage4_a279534f:

    # "Silva" "I have nothing against you mon ami, but I can't have a rival standing in my way..."
    "Сильва" "Я не имею ничего против тебя, мона ми, но я не могу допустить, чтобы на моем пути стоял сопернички...."

# game/intro.rpy:646
translate russia tutStage4_04975854:

    # y "Listen here {i}'mon cheri'{/i}~..."
    y "Послушай {i}'мона ми'{/i}~..."

# game/intro.rpy:647
translate russia tutStage4_eaf3b408:

    # y "Just because I broke out of a insane asylum with supervillains, doesn't mean I want to take over the world."
    y "Даже если я сидел в психушки с супер-злодеями и вырвался от туда, то это не значит, что я тут же захочу захватить мир."

# game/intro.rpy:648
translate russia tutStage4_16fe428d:

    # y "You go and have a blast taking over the world while I worry about getting a dayjob."
    y "Ты иди и отрывайся, захватывай мир, а я пойду поищу подработку."

# game/intro.rpy:649
translate russia tutStage4_f0c05817:

    # sil "But zat is so boring! Wouldn't you much rather be powerful? Own your own country~...?"
    sil "Но это же скукотища! Разве ты не хочешь власть, могущества и силы? Иметь свою собственную страну~...?"

# game/intro.rpy:650
translate russia tutStage4_cec389b7:

    # sil "Have everything you've ever wanted...."
    sil "Иметь все, о чем ты когда-либо мечтал...."

# game/intro.rpy:651
translate russia tutStage4_69093eba:

    # y "I already have Netflix. What more could I want?"
    y "У меня уже есть Netflix. О чем ещё я могу мечтать?"

# game/intro.rpy:652
translate russia tutStage4_e2c2b37a:

    # y "Sorry lady, but the villain lifestyle is not for me."
    y "Уж простите меня миледи, но злодейский образ жизни не для меня."

# game/intro.rpy:654
translate russia tutStage4_e47190a4:

    # y "A rival? Starting my own gang sounds pretty cool, but I need a theme~..."
    y "Соперник? Создать собственную банду - звучит довольно круто, но мне нужна определённая тематика~..."

# game/intro.rpy:655
translate russia tutStage4_85f56774:

    # y "What about a bee theme! Every superhero universe has a bee-themed villain!"
    y "А как насчет пчелиной темы! В каждой вселенной супергероев есть злодей с пчелиной тематикой!"

# game/intro.rpy:656
translate russia tutStage4_ab62a731:

    # sil "Really? A bee could go well with my flower theme~.... If you know what I'm saying~...."
    sil "Правда? Пчёлы могут хорошо сочетаться с моей цветочной тематикой.~.... Если ты понимаешь, о чем я~...."

# game/intro.rpy:657
translate russia tutStage4_45d8c000:

    # y "Yes I know what you're insinuating."
    y "Да, я понимаю, на что ты намекаешь."

# game/intro.rpy:658
translate russia tutStage4_51650392:

    # sil "Well then maybe you and I shou-..."
    sil "Тогда, может нам с тобой стоит-..."

# game/intro.rpy:659
translate russia tutStage4_b2ec547d:

    # y "Cause bees pollinate flowers."
    y "Ведь пчёлы опыляют цветы."

# game/intro.rpy:660
translate russia tutStage4_ac900cc2:

    # sil "Yes... So maybe you and I sho-..."
    sil "Да... Так что, может, мы с тобой-..."

# game/intro.rpy:661
translate russia tutStage4_dbc57974:

    # y "Sex. You're talking about sex."
    y "Секс. Ты говоришь о сексе."

# game/intro.rpy:662
translate russia tutStage4_82368d0f:

    # sil "..........................."
    sil "..........................."

# game/intro.rpy:663
translate russia tutStage4_0fd7dbde:

    # y "Also the answer is no."
    y "Мой ответ нет."

# game/intro.rpy:664
translate russia tutStage4_e9f4f8df:

    # y "You just said that you were trying to get rid of your rivals, so you'll betray me at the first opportunity you get."
    y "Ты только что сказала, что пытаешься избавиться от своих конкурентов, так что ты предашь меня при первой же возможности."

# game/intro.rpy:665
translate russia tutStage4_03b9ab1b:

    # y "How about you go play gangster while I look for a respectable job! {w}Like zookeeper."
    y "Может, ты пока поиграешь в гангстера, а я поищу приличную работу! {w}Что-то вроде смотритель зоопарка."

# game/intro.rpy:667
translate russia tutStage4_23c806c2:

    # y "Je ne parle pas français!"
    y "Je ne parle pas français! (Я не говорю по-французски)!"

# game/intro.rpy:668
translate russia tutStage4_74ebc04d:

    # sil "I'm... I'm not speaking French~..."
    sil "Я... Я не говорю по-французски~..."

# game/intro.rpy:669
translate russia tutStage4_2faa9364:

    # y "Où est la plage?"
    y "Où est la plage? (Где находится пляж?)"

# game/intro.rpy:670
translate russia tutStage4_27c24a49:

    # sil "..............."
    sil "..............."

# game/intro.rpy:671
translate russia tutStage4_b85c048c:

    # y "Eiffel Tower, omelette du fromage, Napoleon!"
    y "Eiffel Tower, omelette du fromage, Napoleon! (Эйфелева башня, омлет дю фромаж, Наполеон!)"

# game/intro.rpy:672
translate russia tutStage4_9073b330:

    # sil "Now you're just shouting random phrases~..."
    sil "Теперь ты просто выкрикиваешь рандомные фразы~..."

# game/intro.rpy:673
translate russia tutStage4_bfe2268c:

    # y "Tour the France?"
    y "Tour the France? (Турне по Франции?)"

# game/intro.rpy:674
translate russia tutStage4_f31a319a:

    # sil "Ah right... I forgot that you were crazy..."
    sil "Ах да... Я забыла, что ты сумасшедший..."

# game/intro.rpy:675
translate russia tutStage4_c3a9a245:

    # sil "{b}*Le sigh*{/b} Too bad~... I do like you. {w}Even if you are a bit coo-coo..."
    sil "{b}*Вздох*{/b} Очень жаль~... А ты мне нравишься. {w}Несмотря на то, что ты немного ку-ку..."

# game/intro.rpy:676
translate russia tutStage4_5dca141f:

    # sil "Run along now~... If you're not a gangleader than I have no problem with you."
    sil "А теперь беги~... Так как ты не лидер банды, ты мне не особо интересен."

# game/intro.rpy:677
translate russia tutStage4_270af4b0:

    # y "Er... thanks."
    y "Емм... спасибо."

# game/intro.rpy:678
translate russia tutStage4_9fb3e1cf:

    # sil "Good luck out zere~... Try to not get shot~... ♥"
    sil "Удачи тогда~... Попытайся не словить пулю~... ♥"

# game/intro.rpy:682
translate russia tutStage4_ad9af6ad:

    # "The gangsters turn away from you as you quickly return to the milkshake bar."
    "Бандиты отворачиваются от вас, и вы быстро проскальзываете обратно в бар."

# game/intro.rpy:694
translate russia tutStage4_de5596ac:

    # s "Oh there you are, New Guy.{w} I was beginning to worry. Didn't I tell you to stay here and guard the bar?"
    s "Ооо вот ты где, новичок.{w} Я уже начала беспокоиться. Разве я не говорила тебе оставаться здесь и охранять бар?"

# game/intro.rpy:695
translate russia tutStage4_015d90da:

    # y "Yeah, but I got bored. So found out anything new?"
    y "Даа, но мне стало скучно. Узнала что-нибудь новое?"

# game/intro.rpy:696
translate russia tutStage4_e8fc39b2:

    # s "Yeah I did. Did you?"
    s "Да узнала. А ты?"

# game/intro.rpy:697
translate russia tutStage4_34fa9179:

    # y "Yup."
    y "Я тоже."

# game/intro.rpy:698
translate russia tutStage4_61421013:

    # s "Then you go first."
    s "Ну тогда начинай ты."

# game/intro.rpy:699
translate russia tutStage4_e8ee5efb:

    # y "Well... nanobots are causing our agents to turn bad. They're now supporting local gangs in a bid for dominance over the city."
    y "Ну... Наноботы заставляют наших агентов становиться злыми. Теперь они поддерживают местные банды в борьбе за господство над городом."

# game/intro.rpy:700
translate russia tutStage4_972810ac:

    # y "The gangs are being led by Ex-WOOHP convicts. The first to take over the entire city gets to join the guy who orchestrated this whole thing."
    y "Во главе банд стоят бывшие заключенные WOOHP. Первый, кто захватит весь город, присоединится к парню, который все это организовал."

# game/intro.rpy:701
translate russia tutStage4_572c5133:

    # y "Your turn."
    y "Теперь твоя очередь."

# game/intro.rpy:702
translate russia tutStage4_b369c4b4:

    # s g34 "Well...."
    s g34 "Нуу...."

# game/intro.rpy:710
translate russia tutStage4_59287ae5:

    # sM "Right now, Beverly Hills is divided by multiple gangs. The three biggest ones are..."
    sM "В настоящее время Беверли-Хиллз разделен между множеством банд. Три самых крупных из них..."

# game/intro.rpy:714
translate russia tutStage4_ec956212:

    # sM "'Aces'. The cool rich kids that set up near the marina."
    sM "'Тузы'. Крутые богатые ребята, которые обосновались рядом с пристанью."

# game/intro.rpy:718
translate russia tutStage4_d07ce169:

    # sM "'Drift Punk', tech geeks and nerds, which are located near sillicon valley."
    sM "'Дрифт Панки', технические гики и ботаники, которые находятся рядом с силиконовой долиной."

# game/intro.rpy:722
translate russia tutStage4_b4358aa6:

    # sM "And the 'Outsiders'. Punkers and goth kids. Basically the freaks that never fit in. They're set up to the north."
    sM "Ну и 'Аутсайдеры'. Панки и готы. В основном, уроды, которые никогда не вписывались в общество. Они расположены на севере."

# game/intro.rpy:723
translate russia tutStage4_43e0e562:

    # sM "I've managed to get in touch with a contact inside the Aces. He might be able to help us in the future."
    sM "Мне удалось связаться с человеком из \"Тузов\". Возможно, он сможет помочь нам в будущем."

# game/intro.rpy:735
translate russia tutStage4_23fa4e39:

    # sM "And then there's the smaller gangs. I couldn't find out what they're all about, but Beverly Hills is divided up like this."
    sM "Есть и более мелкие банды. Но я не смогла ничего о них выяснить, но Беверли-Хиллз разделён между ними."

# game/intro.rpy:739
translate russia tutStage4_f2ed79e6:

    # sM "The only area that's considered neutral ground is..."
    sM "Единственная область, которая считается нейтральной, это..."

# game/intro.rpy:745
translate russia tutStage4_b85ed68a:

    # sM "The mall."
    sM "Торговый центр."

# game/intro.rpy:746
translate russia tutStage4_d80a3f45:

    # y "The mall?"
    y "Торговый центр?"

# game/intro.rpy:747
translate russia tutStage4_565e9400:

    # sM "Yeah, afterall, what good is all your stolen money if you don't have a place to spend it?"
    sM "Да, в конце концов, что толку от всех украденных денег, если их некуда потратить?"

# game/intro.rpy:748
translate russia tutStage4_5e1c39a2:

    # y "That makes a weird kind of sense."
    y "Как ни странно, но это логично."

# game/intro.rpy:755
translate russia tutStage4_1fbbf790:

    # s "So at least we sort of know what we're up against..."
    s "Так что мы хотя бы знаем, с чем имеем дело..."

# game/intro.rpy:756
translate russia tutStage4_77bab620:

    # s "................."
    s "................."

# game/intro.rpy:757
translate russia tutStage4_3931b6b1:

    # s g39 "Despite going against my direct orders for you to stay put...."
    s g39 "Несмотря на то, что ты нарушил мой приказ оставаться на месте...."

# game/intro.rpy:758
translate russia tutStage4_da7e04f3:

    # s g38 "I guess you did well and I have to applaud the initiative."
    s g38 "Я думаю, ты проделал отличную работу, и достоин аплодисментов."

# game/intro.rpy:759
translate russia tutStage4_52730dbd:

    # s "Up next I think we should..."
    s "Теперь нам стоит..."

# game/intro.rpy:760
translate russia tutStage4_fff3959f:

    # s g39 "................."
    s g39 "................."

# game/intro.rpy:761
translate russia tutStage4_3a4647a8:

    # s g18 "Ehrm....."
    s g18 "Эммм....."

# game/intro.rpy:762
translate russia tutStage4_f3a3bd00:

    # s "What do you think we should do next?"
    s "Как ты думаешь, что нам стоит сделать в дальнейшем?"

# game/intro.rpy:763
translate russia tutStage4_ae7f22e8:

    # y "Well..."
    y "Что же..."

# game/intro.rpy:764
translate russia tutStage4_d751fc2b:

    # y "We should focus on breaking the nanobot control over our agents.{w} With them on our side we can begin taking steps to take back the city."
    y "Нам нужно сосредоточиться на том, чтобы сломать контроль наноботов над нашими агентами.{w} С ними на нашей стороне мы можем начать предпринимать дальнейшие шаги, чтобы вернуть город себе."

# game/intro.rpy:765
translate russia tutStage4_c66e239d:

    # y "So if we break the control over our agents... We could send them to help us take back the HQ."
    y "Так что если мы нарушим контроль над нашими агентами... Тогда они точно помогут нам вернуть штаб-квартиру."

# game/intro.rpy:766
translate russia tutStage4_4b519adc:

    # s g31 "But how do we do that?"
    s g31 "Но как это сделать?"

# game/intro.rpy:767
translate russia tutStage4_eb2d33ad:

    # y "I was thinking... that we bring one back here."
    y "Я думал... мы приведём одного сюда."

# game/intro.rpy:768
translate russia tutStage4_eff8036d:

    # y "Once here, we can figure out how to break the nanobot control over them."
    y "И уже тогда, мы сможем выяснить, как нарушить работу наноботов."

# game/intro.rpy:769
translate russia tutStage4_c9ae9367:

    # s g35 "There were plenty of Agents at the school. They might still be there. It's risky, but I bet I could sneak in again."
    s g35 "В школе было много агентов. Возможно, они все ещё там. Это рискованно, но я уверена, что смогу пробраться туда ещё раз."

# game/intro.rpy:770
translate russia tutStage4_7197c966:

    # s "Knocking an Agent out shouldn't be that hard. The tricky part will be transporting him back here."
    s "Вырубить агента не должно составть труда. Самым сложным будет доставить его в бар."

# game/intro.rpy:771
translate russia tutStage4_2d47b7eb:

    # y "There's a workbench downstairs. I'm sure I can fix you something up that will help you with that."
    y "Внизу есть верстак. Я уверен, что смогу тебя чем нибудь снарядить и помочь тем самым."

# game/intro.rpy:772
translate russia tutStage4_3235d6db:

    # s "Okay good. I approve your plan."
    s "Ладно. Согласна с твоим планом."

# game/intro.rpy:773
translate russia tutStage4_53639b44:

    # s "I'll leave you to craft the gadget and set up a mission via the Mission Screen."
    s "Оставлю тебя, чтобы ты создал пару гаджетов и установил миссию через экран миссий."

# game/intro.rpy:783
translate russia tutStage4_223b4b90:

    # c "Oh there you are, New Guy."
    c "Ооо вот ты, где новичок."

# game/intro.rpy:784
translate russia tutStage4_003c6429:

    # y "Hey Clover. Found out anything new?"
    y "Привет Кловер. Узнала что-нибудь новое?"

# game/intro.rpy:785
translate russia tutStage4_d6aba8ec:

    # c "I like... brought back the biggest news! You go first though."
    c "Да... принесла самую большую новость! Но ты первый."

# game/intro.rpy:786
translate russia tutStage4_e8ee5efb_1:

    # y "Well... nanobots are causing our agents to turn bad. They're now supporting local gangs in a bid for dominance over the city."
    y "Ну что же... наноботы заставляют наших агентов становиться злыми. They're now supporting local gangs in a bid for dominance over the city."

# game/intro.rpy:787
translate russia tutStage4_972810ac_1:

    # y "The gangs are being led by Ex-WOOHP convicts. The first to take over the entire city gets to join the guy who orchestrated this whole thing."
    y "Ну... Наноботы заставляют наших агентов становиться злыми. Теперь они поддерживают местные банды в борьбе за господство над городом."

# game/intro.rpy:788
translate russia tutStage4_572c5133_1:

    # y "Your turn."
    y "Твоя очередь."

# game/intro.rpy:789
translate russia tutStage4_6cae50ef:

    # c "Well...."
    c "Нууу...."

# game/intro.rpy:797
translate russia tutStage4_11da07c1:

    # cM "Right now, Beverly Hills is divided by multiple gangs. The three biggest ones are..."
    cM "В настоящее время Беверли-Хиллз разделен между множеством банд. Три самых крупных из них..."

# game/intro.rpy:801
translate russia tutStage4_cebdb29b:

    # cM "'Aces'. The cool rich kids that set up near the marina."
    cM "'Тузы'. Крутые богатые ребята, которые обосновались рядом с пристанью."

# game/intro.rpy:805
translate russia tutStage4_283121d0:

    # cM "'Drift Punk', tech geeks and nerds, which are located near sillicon valley."
    cM "Дрифт Панки', технические гики и ботаники, которые находятся рядом с силиконовой долиной."

# game/intro.rpy:809
translate russia tutStage4_65dcc165:

    # cM "And the 'Outsiders'. Punkers and goth kids. Basically the freaks that never fit in. They're set up to the north."
    cM "Ну и 'Аутсайдеры'. Панки и готы. В основном, уроды, которые никогда не вписывались в общество. Они расположены на севере."

# game/intro.rpy:810
translate russia tutStage4_fe92362a:

    # cM "While exploring around, I came across this one geeky looking nerd from Drift Punk. He seemed interested in me... bleh~..."
    cM "Исследуя окрестности, я наткнулась на одного зануду из Дрифт Панков. Он заинтересовал меня... буе~..."

# game/intro.rpy:811
translate russia tutStage4_64382368:

    # cM "But I guess if we need a contact within the gang, then that's our in. He might be able to help us in the future."
    cM "Думаю, если нам понадобится контакт внутри банды, то он отлично подойдёт на эту роль. Возможно, он сможет помочь нам в будущем."

# game/intro.rpy:823
translate russia tutStage4_238898c1:

    # cM "And then there's the smaller gangs. I couldn't find out what they're all about, but Beverly Hills is divided up like this."
    cM "Есть и более мелкие банды. Но я не смогла ничего о них выяснить, но Беверли-Хиллз разделён между ними."

# game/intro.rpy:827
translate russia tutStage4_0ac1c3f4:

    # cM "Luckily the biggest thread so far has been avoided! The only area that's considered neutral ground is..."
    cM "К счастью, самой большой потери пока удалось избежать! Единственная область, которая считается нейтральной, это..."

# game/intro.rpy:833
translate russia tutStage4_cf3b43b9:

    # cM "The mall!"
    cM "Торговый центр!"

# game/intro.rpy:834
translate russia tutStage4_d80a3f45_1:

    # y "The mall?"
    y "Торговый центр?"

# game/intro.rpy:835
translate russia tutStage4_69216a09:

    # cM "Yeah, afterall, they're not savages. They need their three daily trips to the mall as much as anyone."
    cM "Да, в конце концов, они же не дикари. Им, как и всем, тоже нужно как минимум три раза в день пройтись по магазинам."

# game/intro.rpy:836
translate russia tutStage4_6f323c89:

    # y "............."
    y "............."

# game/intro.rpy:837
translate russia tutStage4_166279fa:

    # cM "Oh right and of course they need to place to buy stuff. What good is all your stolen money if you don't get to spend it?"
    cM "Оххх ну и, конечно, им же нужно место, чтобы покупать всякие вещи. Что толку от всех украденных денег, если ты не можешь их потратить?"

# game/intro.rpy:838
translate russia tutStage4_5e1c39a2_1:

    # y "That makes a weird kind of sense."
    y "Как ни странно, но это логично."

# game/intro.rpy:845
translate russia tutStage4_307bb9f1:

    # c "So at least we sort of know what we're up against..."
    c "Ну мы хотя бы знаем, с чем теперь имеем дело..."

# game/intro.rpy:846
translate russia tutStage4_de2a94a6:

    # c "What comes next?"
    c "И что дальше?"

# game/intro.rpy:847
translate russia tutStage4_498a9676:

    # y "We should focus on breaking the nanobot control over our agents.{w} There's a bullet train downstairs that could take us to the WOOHP HQ, but we're not going to re-capture it by ourselves."
    y "Нам нужно сосредоточиться на том, чтобы нарушить контроль наноботов над нашими агентами.{w} Внизу есть сверхскоростной поезд, он доставит нас в штаб-квартиру WOOHP, но сами мы не сможем захватывать её."

# game/intro.rpy:848
translate russia tutStage4_c66e239d_1:

    # y "So if we break the control over our agents... We could send them to help us take back the HQ."
    y "Если мы нарушим контроль над агентами... То с их помощью мы сможем вернуть штаб-квартиру."

# game/intro.rpy:849
translate russia tutStage4_7d8cd4c2:

    # c "But how do we do that?"
    c "Но как это сделать?"

# game/intro.rpy:850
translate russia tutStage4_eb2d33ad_1:

    # y "I was thinking... that we bring one back here."
    y "Я собирался... привести одного сюда."

# game/intro.rpy:851
translate russia tutStage4_eff8036d_1:

    # y "Once here, we can figure out how to break the nanobot control over them."
    y "Уже здесь, мы сможем выяснить, как нарушить работу наноботов."

# game/intro.rpy:852
translate russia tutStage4_257397d2:

    # c "There were plenty of Agents at the school. They might still be there. It's risky, but I bet I could sneak in again."
    c "В школе было много агентов. Возможно, они все ещё там. Это рискованно, но я уверена, что смогу пробраться туда ещё раз."

# game/intro.rpy:853
translate russia tutStage4_c8ad558b:

    # c "Knocking an Agent out shouldn't be that hard. The tricky part will be transporting him back here."
    c "Вырубить агента не должно составть труда. Самым сложным будет доставить его в бар."

# game/intro.rpy:854
translate russia tutStage4_2d47b7eb_1:

    # y "There's a workbench downstairs. I'm sure I can fix you something up that will help you with that."
    y "Внизу есть верстак. Я уверен, что смогу тебя чем нибудь снарядить и помочь тем самым."

# game/intro.rpy:855
translate russia tutStage4_8d6d9687:

    # c "Okay good. In that case I'll leave you to craft the gadget and set up a mission via the Mission Screen."
    c "Ладненько. В таком случае оставлю тебя, занимайся гаджетами и установкой миссий через экран миссий."

# game/intro.rpy:864
translate russia tutStage4_0ae9bcd0:

    # ""
    ""

# game/intro.rpy:876
translate russia tutLift_b0d8500b:

    # yInmate "Wait... I can't head out like this! {w}I'll never get a job and reintegrate into society looking like a convict."
    yInmate "Подожди... Я не могу выйти в таком виде! {w}Выглядя как осуждённый, я точно никогда не найду работу и не смогу интегрироваться в общество."

# game/intro.rpy:877
translate russia tutLift_ddae83b9:

    # yInmate "Let's look for some clothes first."
    yInmate "Поищу ка я какую-нибудь одежду."

# game/intro.rpy:880
translate russia tutLift_472f62ce:

    # yInmate "Nope... Not going out there right now."
    yInmate "Неа... Я не собираюсь выходить прямо сейчас."

# game/intro.rpy:883
translate russia tutLift_59ca44a0:

    # y "I gotta focus on crafting that gadget and planning a mission first."
    y "Сначала я должен сосредоточиться на создании гаджетов и планировании миссии."

# game/intro.rpy:886
translate russia tutLift_98c0ab0a:

    # y "There's still penty of time left. I could spend some time cleaning the bar."
    y "Времени ещё много. Я могу потратить некоторое время на уборку бара."

# game/intro.rpy:891
translate russia tutLift_d32f081f:

    # "{b}*Splosh* *Splosh* *Splosh*{/b}"
    "{b}*Брызги* *Брызги* *Брызги*{/b}"

# game/intro.rpy:899
translate russia tutLift_85a3a6a2:

    # "You spend the rest of the evening cleaning the bar. It looks better than before, but there is still a lot to be done."
    "Остаток вечера вы проводите за уборкой бара. Он выглядит немного лучше, чем раньше, но ещё многое предстоит сделать."

# game/intro.rpy:901
translate russia tutLift_53868d06:

    # "You decide to head to bed."
    "Вы собираетесь лечь спать."

# game/intro.rpy:912
translate russia tutQuarters_9fd22163:

    # y "I'm not heading back into the cellblock..."
    y "Я не собираюсь возвращаться в карцер..."

# game/intro.rpy:915
translate russia tutQuarters_ba5a9c92:

    # yInmate "I'm not heading back into the cellblock..."
    yInmate "Я не собираюсь возвращаться в карцер..."

# game/intro.rpy:924
translate russia tutQuarters_f2efe18c:

    # s "You called, [playerName]?"
    s "Ты звал, [playerName]?"

# game/intro.rpy:931
translate russia tutSocialSam_2dbf7efa:

    # s "........."
    s "........."

# game/intro.rpy:932
translate russia tutSocialSam_4f2e83f1:

    # y "What's on your mind?"
    y "О чем думаешь?"

# game/intro.rpy:933
translate russia tutSocialSam_802ba33c:

    # s g14 "My friends...{w} If I acted more quickly, I could have gotten them out before they were captured..."
    s g14 "Мои подруги...{w} Если бы я действовала быстрее, я могла бы вытащить их до того, как их схватят..."

# game/intro.rpy:934
translate russia tutSocialSam_6748d793:

    # y "Yeah, you messed up. Not one of your brightest moments."
    y "Да, ты облажалась. Не самый лучший момент в твоей жизни."

# game/intro.rpy:935
translate russia tutSocialSam_367be8b8:

    # s g4 "H-hey...! Don't say that..."
    s g4 "Е-ей...! Не говори так..."

# game/intro.rpy:936
translate russia tutSocialSam_6cc383b4:

    # y "Oh wait... This is one of those 'make you feel better' moments. I'm sorry."
    y "Ой, подожди... Это же один из тех моментов, которые 'заставляют чувствовать себя лучше'. Мне жаль."

# game/intro.rpy:937
translate russia tutSocialSam_06fd174f:

    # y "In that case, please don't worry about it. You couldn't have known that I was still one of the good guys."
    y "В таком случае, не беспокойся об этом. Ты ведь не знала, что я все ещё один из хороших парней."

# game/intro.rpy:938
translate russia tutSocialSam_4a9b3706:

    # y "And I'm sure they're fine. We're gonna get this sorted out and they'll be back before you know it. Okay?"
    y "И я уверен, что они в порядке. Мы разберемся со всем, и они вернутся, не успеешь оглянуться. Хорошо?"

# game/intro.rpy:939
translate russia tutSocialSam_4af191fc:

    # s g19 "Y-yeah... Okay~..."
    s g19 "Д-даа... Хорошо~..."

# game/intro.rpy:940
translate russia tutSocialSam_eed95059:

    # y "No, but seriously though. Manage your priorities in the future."
    y "Нет, но серьезно. Расставляй свои приоритеты в будущем."

# game/intro.rpy:941
translate russia tutSocialSam_04af2f0e:

    # s g4 "{b}*Sniff*{/b} H-hey..!"
    s g4 "{b}*Фыркает*{/b} Э-эй..!"

# game/intro.rpy:945
translate russia tutSocialSam_e7344c7a:

    # y "You know that my name isn't actually 'New Guy', right?"
    y "Ты ведь в курсе, что на самом деле меня зовут не 'новичок'?"

# game/intro.rpy:946
translate russia tutSocialSam_8e1ecef4:

    # s "I figured, but that's what I'm gonna keep calling you. Can't get attached."
    s "Это понятно, но я буду тебя так называть. Не хочу привязываться."

# game/intro.rpy:947
translate russia tutSocialSam_cf630fb5:

    # y "..................."
    y "..................."

# game/intro.rpy:958
translate russia tutSocialSam_63ace678:

    # c "You called, [playerName]?"
    c "Ты звал, [playerName]?"

# game/intro.rpy:965
translate russia tutSocialClover_154e4fc5:

    # c "........."
    c "........."

# game/intro.rpy:966
translate russia tutSocialClover_4f2e83f1:

    # y "What's on your mind?"
    y "О чем думаешь?"

# game/intro.rpy:967
translate russia tutSocialClover_fddf540c:

    # c "Like... It's nothing! Don't you have anything better to do?"
    c "А, да так... ни о чём! Тебе что не чем заняться?"

# game/intro.rpy:968
translate russia tutSocialClover_6f362d57:

    # y "..............."
    y "..............."

# game/intro.rpy:969
translate russia tutSocialClover_054e85fb:

    # c "Okay fine~...{w} I'm thinking about my friends...{w} If I acted more quickly, I could have gotten them out before they were captured..."
    c "Ладно, уговорил~...{w} Я думаю о моих подругах...{w} Если бы я действовала быстрее, я могла бы вытащить их до того, как их схватят..."

# game/intro.rpy:970
translate russia tutSocialClover_6748d793:

    # y "Yeah, you messed up. Not one of your brightest moments."
    y "Да, ты облажалась. Не самый лучший момент в твоей жизни."

# game/intro.rpy:971
translate russia tutSocialClover_8e7797ad:

    # c "H-hey...! Don't say that..."
    c "Е-ей...! Не говори так..."

# game/intro.rpy:972
translate russia tutSocialClover_6cc383b4:

    # y "Oh wait... This is one of those 'make you feel better' moments. I'm sorry."
    y "Ой, подожди... Это же один из тех моментов, которые 'заставляют чувствовать себя лучше'. Мне жаль."

# game/intro.rpy:973
translate russia tutSocialClover_06fd174f:

    # y "In that case, please don't worry about it. You couldn't have known that I was still one of the good guys."
    y "В таком случае, не беспокойся об этом. Ты ведь не знала, что я все ещё один из хороших парней."

# game/intro.rpy:974
translate russia tutSocialClover_4a9b3706:

    # y "And I'm sure they're fine. We're gonna get this sorted out and they'll be back before you know it. Okay?"
    y "И я уверен, что они в порядке. Мы разберемся со всем, и они вернутся, не успеешь оглянуться. Хорошо?"

# game/intro.rpy:975
translate russia tutSocialClover_11393e0c:

    # c "Y-yeah... Okay~..."
    c "Д-даа... Хорошо~..."

# game/intro.rpy:976
translate russia tutSocialClover_eed95059:

    # y "No, but seriously though. Manage your priorities in the future."
    y "Нет, но серьезно. Расставляй свои приоритеты в будущем."

# game/intro.rpy:977
translate russia tutSocialClover_2f6d1377:

    # c "{b}*Sniff*{/b} H-hey..!"
    c "{b}*Фыркает*{/b} Э-эй..!"

# game/intro.rpy:981
translate russia tutSocialClover_e7344c7a:

    # y "You know that my name isn't actually 'New Guy', right?"
    y "Ты ведь в курсе, что на самом деле меня зовут не 'новичок'?"

# game/intro.rpy:982
translate russia tutSocialClover_16347683:

    # c "Well you're new around here, aren't you? It sounds like a fitting name for you."
    c "Ты же здесь недавно, ведь так? Это самое подходящее имя для тебя."

# game/intro.rpy:983
translate russia tutSocialClover_cf630fb5:

    # y "..................."
    y "..................."

# game/intro.rpy:994
translate russia tutSocialClover_fcd3c794:

    # a "You called, [playerName]?"
    a "Ты звал, [playerName]?"

# game/intro.rpy:1001
translate russia tutSocialAlex_b2b821d1:

    # a "........."
    a "........."

# game/intro.rpy:1002
translate russia tutSocialAlex_4f2e83f1:

    # y "What's on your mind?"
    y "О чем думаешь?"

# game/intro.rpy:1003
translate russia tutSocialAlex_0c0248ad:

    # a "I'm thinking about my friends...{w} If I acted more quickly, I could have gotten them out before they were captured..."
    a "Думаю о моих подругах...{w} Если бы я действовала быстрее, я могла бы вытащить их до того, как их схватят..."

# game/intro.rpy:1004
translate russia tutSocialAlex_6748d793:

    # y "Yeah, you messed up. Not one of your brightest moments."
    y "Да, ты облажалась. Не самый лучший момент в твоей жизни."

# game/intro.rpy:1005
translate russia tutSocialAlex_289ee9a2:

    # a "{b}*Sniff*{/b} Yeah..."
    a "{b}*Фыркает*{/b} Даа..!"

# game/intro.rpy:1006
translate russia tutSocialAlex_6cc383b4:

    # y "Oh wait... This is one of those 'make you feel better' moments. I'm sorry."
    y "Ой, подожди... Это же один из тех моментов, которые 'заставляют чувствовать себя лучше'. Мне жаль."

# game/intro.rpy:1007
translate russia tutSocialAlex_06fd174f:

    # y "In that case, please don't worry about it. You couldn't have known that I was still one of the good guys."
    y "В таком случае, не беспокойся об этом. Ты ведь не знала, что я все ещё один из хороших парней."

# game/intro.rpy:1008
translate russia tutSocialAlex_4a9b3706:

    # y "And I'm sure they're fine. We're gonna get this sorted out and they'll be back before you know it. Okay?"
    y "И я уверен, что они в порядке. Мы разберемся со всем, и они вернутся, не успеешь оглянуться. Хорошо?"

# game/intro.rpy:1009
translate russia tutSocialAlex_b85612df:

    # a "Y-yeah... Okay~..."
    a "Д-даа... Хорошо~..."

# game/intro.rpy:1010
translate russia tutSocialAlex_eed95059:

    # y "No, but seriously though. Manage your priorities in the future."
    y "Нет, но серьезно. Расставляй свои приоритеты в будущем."

# game/intro.rpy:1011
translate russia tutSocialAlex_d28ba78d:

    # a "{b}*Sniff*{/b} H-hey..!"
    a "{b}*Фыркает*{/b} Э-эй..!"

# game/intro.rpy:1015
translate russia tutSocialAlex_e7344c7a:

    # y "You know that my name isn't actually 'New Guy', right?"
    y "Ты ведь в курсе, что на самом деле меня зовут не 'новичок'?"

# game/intro.rpy:1016
translate russia tutSocialAlex_104135cb:

    # c "It's not?!"
    c "Правда?!"

# game/intro.rpy:1017
translate russia tutSocialAlex_cf630fb5:

    # y "..................."
    y "..................."

# game/intro.rpy:1018
translate russia tutSocialAlex_f5d181af:

    # a "Oh I'm sorry... I'm not very good with names. I'm just gonna keep calling you New Guy, okay?"
    a "О, извини... У меня частенько проблемы с именами. Я просто буду продолжать называть тебя новичком, ладно?"

# game/intro.rpy:1026
translate russia tutSocialAlex_7861d496:

    # "Nope... Not going out there right now."
    "Неа... Я не собираюсь выходить прямо сейчас."

# game/intro.rpy:1029
translate russia tutSocialAlex_bd0fc588:

    # y "I should craft a gadget and then set up a mission via the mission screen."
    y "Сначала я должен сосредоточиться на создании гаджетов и планировании миссии."

# game/intro.rpy:1034
translate russia tutSocialAlex_150c4e32:

    # y "Sam?"
    y "Сэм?"

# game/intro.rpy:1035
translate russia tutSocialAlex_3d6803a2:

    # s "I'm trying to sleep!"
    s "Я пытаюсь заснуть!"

# game/intro.rpy:1038
translate russia tutSocialAlex_4a1aeb1b:

    # y "Clover?"
    y "Кловер?"

# game/intro.rpy:1039
translate russia tutSocialAlex_e58657aa:

    # c "Get lost!"
    c "Растворись!"

# game/intro.rpy:1042
translate russia tutSocialAlex_6e8be35c:

    # y "Alex?"
    y "Алекс?"

# game/intro.rpy:1043
translate russia tutSocialAlex_d889f578:

    # a "I can't hear you! I'm sleeping!!"
    a "Не слышу тебя! Я сплю!!"

# game/intro.rpy:1046
translate russia tutSocialAlex_57f46db8:

    # y "Sleeping at 6pm... Well I guess I don't have anything better to do."
    y "Ложится спать в 6 вечера... Ну, думаю, мне больше нечем заняться."

# game/intro.rpy:1056
translate russia tutMissions_89f7c904:

    # y "TV!"
    y "Телевизор!"

# game/intro.rpy:1057
translate russia tutMissions_9e398164:

    # y "No wait...{w} It's some kind of Mission Planner. Best to not touch it."
    y "Нет, стоп... Это что-то вроде 'Планировщика миссий'. Лучше не буду трогать."

# game/intro.rpy:1060
translate russia tutMissions_c943f793:

    # y "I wish this thing would show football games... Oh well, best turn in for the night. There should be plenty of cells."
    y "Хотел бы я, чтобы эта штука показывала футбольные матчи... Ну что ж, пора идти спать. Камер должно быть много."

# game/intro.rpy:1063
translate russia tutMissions_c6de1e6b:

    # y "This is the mission screen. I should craft that gadget first though."
    y "Это экран с миссиями. Но сначала мне нужно смастерить один гаджет."

# game/intro.rpy:1066
translate russia tutMissions_ca930e3d:

    # y "It's getting too late to plan a mission right now."
    y "Сейчас уже слишком поздно, чтобы планировать миссии."

# game/intro.rpy:1072
translate russia tutCreditcards_20760c9b:

    # y "(Some complicated machine. It reads: {i}'Please insert Credit Card Number'{/i})"
    y "(Какая-то сложная штука. Написано: {i}'Пожалуйста, введите номер кредитной карты'{/i})"

# game/intro.rpy:1080
translate russia tutPrison_971bdd97:

    # y "I've already checked this out."
    y "Я уже проверял тут."

# game/intro.rpy:1083
translate russia tutPrison_ccbe683a:

    # yInmate "This hatch seems to lead to another room downstairs."
    yInmate "Этот люк, кажется, ведет в другую комнату внизу."

# game/intro.rpy:1084
translate russia tutPrison_3f28de3b:

    # yInmate "Let's have a quick peak~..."
    yInmate "Давайте-ка глянем~..."

# game/intro.rpy:1086
translate russia tutPrison_8b118c34:

    # yInmate "...?"
    yInmate "...?"

# game/intro.rpy:1088
translate russia tutPrison_4c8cb383:

    # "You find {color=#ffeda6}Agent Suit{/color}."
    "Вы находите {color=#ffeda6}костюм Агента{/color}."

# game/intro.rpy:1091
translate russia tutPrison_b4e54db1:

    # y "That's more like it!"
    y "Вот! Так-то лучше!"

# game/intro.rpy:1092
translate russia tutPrison_6dfd9913:

    # y "With a gettup like this I'm sure someone will hire me!"
    y "С таким нарядом я уверен, что кто угодно возьмет меня на работу!"

# game/intro.rpy:1095
translate russia tutPrison_fc41a33f:

    # y "I can sleep in the cellblock for now."
    y "На первое время я могу обосноваться в этом карцере."

# game/intro.rpy:1098
translate russia tutPrison_922d5faf:

    # y "This underground room would make a good place to keep the WOOHP agent if we capture him..."
    y "Этот подвал олично подойдёт для содержания агента WOOHP, если мы его захватим..."

# game/intro.rpy:1101
translate russia tutPrison_016f72e0:

    # "Just an underground room."
    "Просто подвал."

# game/intro.rpy:1107
translate russia tutSpyCamera_440c1787:

    # y "Spy Drones...? I bet I could see what all of WOOHP's spies are up to by tuning into this."
    y "Шпионские дроны...? Спорим, я смогу узнать, чем занимаются все шпионы WOOHP, подключившись к ним."

# game/intro.rpy:1108
translate russia tutSpyCamera_cb3c8129:

    # "{b}*Bleep*{/b}"
    "{b}*Бииип*{/b}"

# game/intro.rpy:1109
translate russia tutSpyCamera_335d0652:

    # y "Aw... It needs a password."
    y "Ооо... Нужен пароль."

# game/intro.rpy:1112
translate russia tutSpyCamera_48a69f93:

    # yInmate "Spy Drones...? I bet I could see what all of WOOHP's spies are up to by tuning into this."
    yInmate "Шпионские дроны...? Спорим, я смогу узнать, чем занимаются все шпионы WOOHP, подключившись к ним."

# game/intro.rpy:1113
translate russia tutSpyCamera_cb3c8129_1:

    # "{b}*Bleep*{/b}"
    "{b}*Бииип*{/b}"

# game/intro.rpy:1114
translate russia tutSpyCamera_bdaaf81c:

    # yInmate "Aw... It needs a password."
    yInmate "Ооо... Нужен пароль."

# game/intro.rpy:1117
translate russia tutSpyCamera_03eb68a6:

    # y "I don't have the password for it."
    y "Но у меня нет никакого пароля."

# game/intro.rpy:1120
translate russia tutSpyCamera_c000d7a7:

    # y "Still don't have the password..."
    y "У меня до сих пор нет пароля..."

# game/intro.rpy:1121
translate russia tutSpyCamera_526da756:

    # y "Wait let me just try something..."
    y "Стоп, а если попробовать..."

# game/intro.rpy:1129
translate russia tutSpyCamera_491696cd:

    # "'{i}Access granted'{/i}"
    "'{i}Доступ получен'{/i}"

# game/intro.rpy:1130
translate russia tutSpyCamera_471fde6a:

    # "Who do you wish to spy on?"
    "За кем вы собираетесь шпионить?"

# game/intro.rpy:1140
translate russia tutSpyCamera_5b84e69d:

    # s "I wish I had a boyfriend who would take me to an old-timey milkshake bar..."
    s "Я бы хотела, чтобы у меня был парень, который сводил бы меня в подобный бар с молочными коктейлями..."

# game/intro.rpy:1141
translate russia tutSpyCamera_e8d692c6:

    # s g18 "...?"
    s g18 "...?"

# game/intro.rpy:1142
translate russia tutSpyCamera_93341330:

    # s g16 "Why do I feel like someone's watching me...?"
    s g16 "Почему-то мне кажется, что за мной кто-то наблюдает...?"

# game/intro.rpy:1150
translate russia tutSpyCamera_2add88a4:

    # "Error. Location unknown."
    "Ошибка. Местоположения неизвестно."

# game/intro.rpy:1160
translate russia tutSpyCamera_8ee2c11e:

    # c "I wonder how many calories are in a milkshake..."
    c "Интересно, сколько калорий в молочном коктейле..."

# game/intro.rpy:1161
translate russia tutSpyCamera_3784857a:

    # c "...?"
    c "...?"

# game/intro.rpy:1162
translate russia tutSpyCamera_2386db62:

    # c "Why do I feel like someone's watching me...?"
    c "Почему-то мне кажется, что за мной кто-то наблюдает...?"

# game/intro.rpy:1170
translate russia tutSpyCamera_2add88a4_1:

    # "Error. Location unknown."
    "Ошибка. Местоположения неизвестно."

# game/intro.rpy:1181
translate russia tutSpyCamera_eb9b6f76:

    # a "Oh! Found a peanut! {b}*Chomps*{/b}"
    a "О! Арахис! {b}*Чавканье*{/b}"

# game/intro.rpy:1182
translate russia tutSpyCamera_f8dd4687:

    # a "..............."
    a "..............."

# game/intro.rpy:1183
translate russia tutSpyCamera_e4d509f3:

    # a "I don't feel so good~..."
    a "Что-то мне не хорошо~..."

# game/intro.rpy:1191
translate russia tutSpyCamera_2add88a4_2:

    # "Error. Location unknown."
    "Ошибка. Местоположения неизвестно."

# game/intro.rpy:1201
translate russia tutSpyCamera_e4d1c6de:

    # "Sam is lying face down on her bed."
    "Сэм лежит лицом вниз на своей кровати."

# game/intro.rpy:1202
translate russia tutSpyCamera_cd1d9888:

    # s "Stupid, stupid, stupid! Don't make assumtions like that, Sam!"
    s "Тупо, тупо, тупо! Какие же это тупые предположений, Сэм!"

# game/intro.rpy:1203
translate russia tutSpyCamera_94140dbd:

    # s "Now he'll think that you're some sort of sex-freak."
    s "Теперь он будет думать, что я какая-то извращенка."

# game/intro.rpy:1204
translate russia tutSpyCamera_454e3cc7:

    # "Burying her head into her pillow she let's out a muffled scream in embarrassed frustration."
    "Зарывшись головой в подушку, она издала приглушенный крик смущенного разочарования."

# game/intro.rpy:1214
translate russia tutSpyCamera_4a059926:

    # "Clover is lying face down on her bed."
    "Кловер лежит лицом вниз на своей кровати."

# game/intro.rpy:1215
translate russia tutSpyCamera_cb8c92b5:

    # c "Urgh, I just had to open my big mouth!"
    c "Гррх, зачем я только открыла свой большой рот!"

# game/intro.rpy:1216
translate russia tutSpyCamera_22eaf632:

    # c "Now he's going to think that you're some sort of sex obsessed weirdo!"
    c "Теперь он будет думать, что я какая-то чудачка, одержимая сексом."

# game/intro.rpy:1217
translate russia tutSpyCamera_062e686e:

    # "Burying her head into her pillow she let's out a defeated sigh."
    "Зарывшись головой в подушку, она, отчаявшись, вздохнула."

# game/intro.rpy:1227
translate russia tutSpyCamera_50e5f675:

    # "Alex is lying face down on her bed."
    "Алекс лежит лицом вниз на своей кровати."

# game/intro.rpy:1228
translate russia tutSpyCamera_04303312:

    # a "I shouldn't have said that..."
    a "Не стоило этого говорить..."

# game/intro.rpy:1229
translate russia tutSpyCamera_34d3f767:

    # a "It was really mean of me and now he'll think I'm some kind of witch..."
    a "Это было очень подло с моей стороны, теперь он будет думать, что я какая-то ведьма..."

# game/intro.rpy:1230
translate russia tutSpyCamera_23843cf8:

    # "Alex lifts her head up briefly before letting it fall down in her pillow again as she breaths a sigh in defeat."
    "Алекс ненадолго поднимает голову и снова опускает ее в подушку, нервно вздыхая."

# game/intro.rpy:1244
translate russia tutSpyCamera_a9c99c25:

    # s ".............."
    s ".............."

# game/intro.rpy:1245
translate russia tutSpyCamera_53b032db:

    # s "My clothes aren't that stupid... Are they....?"
    s "Моя одежда выглядит не настолько глупо... Верно....?"

# game/intro.rpy:1246
translate russia tutSpyCamera_d3563431:

    # s g4 "Right...?"
    s g4 "Ведь так...?"

# game/intro.rpy:1259
translate russia tutSpyCamera_65b59827:

    # s "{b}*Yawns*{/b} It's so noisy outside, I barely get any sleep. I hope we're safe here..."
    s "{b}*Зевает*{/b} На улице так шумно, я не могу уснуть. Надеюсь, здесь мы в безопасности..."

# game/intro.rpy:1260
translate russia tutSpyCamera_5d0c7887:

    # s "All things considered, this is probably one of the safest places in Beverly Hills."
    s "Учитывая все обстоятельства, это, вероятно, одно из самых безопасных мест в Беверли-Хиллз."

# game/intro.rpy:1261
translate russia tutSpyCamera_4246d1a5:

    # s g1 "I guess I shouldn't worry too much."
    s g1 "Думаю, мне не стоит слишком беспокоиться."

# game/intro.rpy:1270
translate russia tutSpyCamera_065c0c2a:

    # "DEV: NO EVENT AT THIS TIME INTRO.RPY"
    "DEV: В ДАННЫЙ МОМЕНТ НЕТ СОБЫТИЙ INTRO.RPY"

# game/intro.rpy:1278
translate russia tutDesk_a4421abd:

    # y "(A sturdy desk, filled with advanced tools for crafting gadgets.{w} I'm pretty sure I'm not allowed to mess around with this.)"
    y "(Прочный стол, заполненный современными инструментами для создания гаджетов.{w} Я уверен, что мне нельзя пользоваться этим.)"

# game/intro.rpy:1279
translate russia tutDesk_64a2b68e:

    # y "..........................."
    y "..........................."

# game/intro.rpy:1283
translate russia tutDesk_7450f6f2:

    # "You begin rumaging through the gadgets."
    "Вы начинаете рыться в гаджетах."

# game/intro.rpy:1284
translate russia tutDesk_c8c01098:

    # y "(Camouflage Mascara, Tornado Hairdryer, Instant Freeze Parfume...)"
    y "(Камуфляжная тушь, фен \"Торнадо\", парфюм \"Мгновенная заморозка\"...)"

# game/intro.rpy:1285
translate russia tutDesk_0d888a1a:

    # y "(I wonder why all of these are so femininely themed....)"
    y "(Интересно, почему все гаджеты на женственную тематику....)"

# game/intro.rpy:1286
translate russia tutDesk_02bc7fb7:

    # "You pick up a lipstick and begin to examin it."
    "Вы берёте помаду и начинаете её рассматривать."

# game/intro.rpy:1291
translate russia tutDesk_f7b6bc76:

    # y "(Crap, crap, crap, crap!)"
    y "(Дерьмо, дерьмо, дерьмо, дерьмо!)"

# game/intro.rpy:1292
translate russia tutDesk_25050824:

    # y "Let's hope nobody saw that...."
    y "Будем надеяться, что никто этого не видел...."

# game/intro.rpy:1300
translate russia tutDesk_300263a9:

    # yInmate "(A sturdy desk, filled with advanced tools for crafting gadgets.{w} I'm pretty sure I'm not allowed to mess around with this.)"
    yInmate "(Прочный стол, заполненный современными инструментами для создания гаджетов.{w} Я уверен, что мне нельзя пользоваться этим.)"

# game/intro.rpy:1301
translate russia tutDesk_2b68a6b1:

    # yInmate "..........................."
    yInmate "..........................."

# game/intro.rpy:1305
translate russia tutDesk_7450f6f2_1:

    # "You begin rumaging through the gadgets."
    "Вы начинаете рыться в гаджетах."

# game/intro.rpy:1306
translate russia tutDesk_301f6982:

    # yInmate "(Camouflage Mascara, Tornado Hairdryer, Instant Freeze Parfume...)"
    yInmate "(Камуфляжная тушь, фен \"Торнадо\", парфюм \"Мгновенная заморозка\"...)"

# game/intro.rpy:1307
translate russia tutDesk_25c1af6d:

    # yInmate "(I wonder why all of these are so femininely themed....)"
    yInmate "(Интересно, почему все гаджеты на женственную тематику....)"

# game/intro.rpy:1308
translate russia tutDesk_02bc7fb7_1:

    # "You pick up a lipstick and begin to examin it."
    "Вы берёте помаду и начинаете её рассматривать."

# game/intro.rpy:1313
translate russia tutDesk_2f4ccc78:

    # yInmate "(Crap, crap, crap, crap!)"
    yInmate "(Дерьмо, дерьмо, дерьмо, дерьмо!)"

# game/intro.rpy:1314
translate russia tutDesk_f4a8703c:

    # yInmate "Let's hope nobody saw that...."
    yInmate "Будем надеяться, что никто этого не видел...."

# game/intro.rpy:1322
translate russia tutDesk_10d4e697:

    # y "I'll worry about this stuff later."
    y "Займусь этим позже."

# game/intro.rpy:1326
translate russia tutDesk_03ad40e2:

    # y "Okay let's see what I can craft here..."
    y "Ладно, давайте посмотрим, что я могу здесь сделать..."

# game/intro.rpy:1327
translate russia tutDesk_f358cb0c:

    # y "................."
    y "................."

# game/intro.rpy:1328
translate russia tutDesk_221bfa38:

    # y "{i}'Hypno Earrings'{/i}"
    y "{i}'\"Гипно\" серьги'{/i}"

# game/intro.rpy:1329
translate russia tutDesk_092605e6:

    # y "{i}'Toss the hypnotic earrings near a target to unleash hypnotic gas.'{/i}"
    y "{i}'Бросьте \"гипно\" серьги рядом с целью, чтобы выпустить гипнотический газ.'{/i}"

# game/intro.rpy:1330
translate russia tutDesk_567f2f19:

    # y "That sounds useful. Let's craft one of those."
    y "Звучит полезно. Давайте сделаем одну штучку."

# game/intro.rpy:1336
translate russia tutDesk_224a7979:

    # y "We're going to have to gather more materials first."
    y "Сначала нужно собрать больше материалов."

# game/intro.rpy:1366
translate russia tutMissionSetup_2b5bc5b9:

    # y "I followed the instructions to a T, so this should work... {w}Hopefully."
    y "Я в точности следовал инструкциям, так что это должно сработать... {w}Надеюсь."

# game/intro.rpy:1367
translate russia tutMissionSetup_7717479b:

    # y "Now to set up a mission via the Mission Screen."
    y "Теперь нужно спланировать миссию через экран миссий."

# game/intro.rpy:1368
translate russia tutMissionSetup_195a69b5:

    # "You walk over to the mission screen and boot it up."
    "Ты подходите к экрану миссии и загружаете её."

# game/intro.rpy:1369
translate russia tutMissionSetup_b9286da7:

    # "New User Detected. Playing tutorial."
    "Обнаружен новый пользователь. Запускаем туториал."

# game/intro.rpy:1373
translate russia tutMissionSetup_b319bf57:

    # "First choose your frontline spy over here."
    "Сначала выберите своего передового шпиона здесь."

# game/intro.rpy:1378
translate russia tutMissionSetup_d215bc2d:

    # "Followed by picking a location..."
    "Затем выберите место миссии..."

# game/intro.rpy:1383
translate russia tutMissionSetup_198f0383:

    # "Finally, you can add gadgets to support your spy during a mission here."
    "Наконец, вы можете добавить гаджеты, чтобы поддержать своего шпиона во время миссии."

# game/intro.rpy:1387
translate russia tutMissionSetup_12e2f5d0:

    # "Once you're done, simply click on FINISH >>."
    "Как только вы закончите, просто нажмите кнопку FINISH >>."

# game/intro.rpy:1388
translate russia tutMissionSetup_503b78ba:

    # y "That's pretty straight forward."
    y "Все довольно просто."

# game/intro.rpy:1390
translate russia tutMissionSetup_a98d995a:

    # sM "Did you get it to work?"
    sM "У тебя получилось?"

# game/intro.rpy:1392
translate russia tutMissionSetup_95eade8c:

    # c "Did you get it to work?"
    c "Ну что? Получилось?"

# game/intro.rpy:1394
translate russia tutMissionSetup_93b0a43a:

    # a "Did you get it to work?"
    a "Получилось?"

# game/intro.rpy:1395
translate russia tutMissionSetup_4035116e:

    # y "Yeah I think so. Let's set up a mission."
    y "Да, думаю да. Давай определимся с заданием."

# game/intro.rpy:1416
translate russia tutStage5_933c6d41:

    # y "Wait! Someone's coming!"
    y "Подожди! Кто-то идёт!"

# game/intro.rpy:1434
translate russia tutStage5_1900140a:

    # "Agent" "Hallway clear and everything is A-OK!"
    "Агент" "Коридор чист! Все в порядке!"

# game/intro.rpy:1437
translate russia tutStage5_373ba573:

    # y "Now is your chance!"
    y "Вот! Это твой шанс!"

# game/intro.rpy:1439
translate russia tutStage5_2ba0f4b3:

    # s "Right!"
    s "Да!"

# game/intro.rpy:1441
translate russia tutStage5_50b1935b:

    # c "Right!"
    c "Точно!"

# game/intro.rpy:1443
translate russia tutStage5_655477b5:

    # a "Right!"
    a "Вперёд!"

# game/intro.rpy:1461
translate russia tutHypno_aff6ba8b:

    # "Agent" "Hey, you're that spy-..."
    "Агент" "Эй, ты та самая шпионк-..."

# game/intro.rpy:1520
translate russia tutorialStun_d8b99d30:

    # "Agent" "{b}*Cough* *Cough*{/b}"
    "Агент" "{b}*Кашель* *Кашель*{/b}"

# game/intro.rpy:1521
translate russia tutorialStun_a8c28cc5:

    # "Agent" "I feel funny!"
    "Агент" "Чувствую себя странно!"

# game/intro.rpy:1523
translate russia tutorialStun_d235cae5:

    # s "It's working!"
    s "Работает!"

# game/intro.rpy:1524
translate russia tutorialStun_6cd7ae1d:

    # y "Quickly, tell him to return to the Safehouse."
    y "Быстро, скажи ему, чтобы он вернулся в убежище."

# game/intro.rpy:1525
translate russia tutorialStun_884aa939:

    # s "{b}*Ahem*{/b} {i}'Agent! Listen closely...'{/i}"
    s "{b}*Кхм*{/b} {i}'Агент! Слушайте внимательно...'{/i}"

# game/intro.rpy:1526
translate russia tutorialStun_074c48bf:

    # s "{i}'Return to the old Milkshake bar downtown, right away!'{/i}"
    s "{i}'Возвращайся в старый бар в центре города, немедленно!'{/i}"

# game/intro.rpy:1532
translate russia tutorialStun_dc576882:

    # "Agent" "{i}'Smoke is filling my lungs, but I am always interested in a dairy beverage!'{/i}"
    "Агент" "{i}'Дым заполняет мои легкие, но от молочного коктейля я никогда не откажусь!'{/i}"

# game/intro.rpy:1536
translate russia tutorialStun_3c1b0315:

    # "The zombiefied guard slowly begins making his way down the hallways, towards the exit."
    "Зомбифицированный агент медленно начинает пробираться по коридорам к выходу."

# game/intro.rpy:1537
translate russia tutorialStun_a4901daa:

    # s "It worked!"
    s "Сработало!"

# game/intro.rpy:1539
translate russia tutorialStun_68d60aa8:

    # c "It's working!"
    c "Получилось!"

# game/intro.rpy:1540
translate russia tutorialStun_6cd7ae1d_1:

    # y "Quickly, tell him to return to the Safehouse."
    y "Быстро скажи ему, чтобы он вернулся в убежище."

# game/intro.rpy:1541
translate russia tutorialStun_ca003d35:

    # c "Totally. {b}*Ahem*{/b} Hey Agent! Like... head over to the milkshake bar downtown! Chop chop!"
    c "Конечно. {b}*Кхем*{/b} Эй, агент! Эмм... отправляйтесь в молочный коктейль-бар в центре города! Чоп чоп!"

# game/intro.rpy:1547
translate russia tutorialStun_dc576882_1:

    # "Agent" "{i}'Smoke is filling my lungs, but I am always interested in a dairy beverage!'{/i}"
    "Агент" "{i}'Дым заполняет мои легкие, но от молочного коктейля я никогда не откажусь!'{/i}"

# game/intro.rpy:1551
translate russia tutorialStun_3c1b0315_1:

    # "The zombiefied guard slowly begins making his way down the hallways, towards the exit."
    "Зомбифицированный агент медленно начинает пробираться по коридорам к выходу."

# game/intro.rpy:1552
translate russia tutorialStun_ba9b91c3:

    # c "It worked!"
    c "Сработало!"

# game/intro.rpy:1554
translate russia tutorialStun_91e2ad03:

    # a "It's working!"
    a "Получилось!"

# game/intro.rpy:1555
translate russia tutorialStun_6cd7ae1d_2:

    # y "Quickly, tell him to return to the Safehouse."
    y "Быстро, скажи ему, чтобы он вернулся в убежище."

# game/intro.rpy:1556
translate russia tutorialStun_8b311f16:

    # a "{b}*Ahem*{/b} {i}'Agent! Listen closely...'{/i}"
    a "{b}*Кхм*{/b} {i}'Агент! Слушайте внимательно...'{/i}"

# game/intro.rpy:1557
translate russia tutorialStun_991d9a29:

    # a "{i}'Return to the old Milkshake bar downtown, right away!'{/i}"
    a "{i}'Возвращайся в старый бар в центре города, немедленно!'{/i}"

# game/intro.rpy:1562
translate russia tutorialStun_dc576882_2:

    # "Agent" "{i}'Smoke is filling my lungs, but I am always interested in a dairy beverage!'{/i}"
    "Агент" "{i}'Дым заполняет мои легкие, но от молочного коктейля я никогда не откажусь!'{/i}"

# game/intro.rpy:1566
translate russia tutorialStun_3c1b0315_2:

    # "The zombiefied guard slowly begins making his way down the hallways, towards the exit."
    "Зомбифицированный агент медленно начинает пробираться по коридорам к выходу."

# game/intro.rpy:1567
translate russia tutorialStun_63c21e79:

    # a "Yes! It worked!"
    a "Сработало!"

# game/intro.rpy:1569
translate russia tutorialStun_36eb148f:

    # y "Good. Get out of there befor-...."
    y "Отлично. Убирайся оттуда, пока-...."

# game/intro.rpy:1588
translate russia tutorialStun_0ab16323:

    # s "!!!"
    s "!!!"

# game/intro.rpy:1589
translate russia tutorialStun_ae4bfad7:

    # y "!!!"
    y "!!!"

# game/intro.rpy:1590
translate russia tutorialStun_62e819f7:

    # s "Clover!"
    s "Кловер!"

# game/intro.rpy:1591
translate russia tutorialStun_d1f007e2:

    # y "Another spy?!"
    y "Другая шпионка?!"

# game/intro.rpy:1602
translate russia tutorialStun_5ab13e75:

    # c "I've spotted her. Quickly send your agents here!"
    c "Я нашла её. Быстро отправляйте своих агентов сюда!"

# game/intro.rpy:1603
translate russia tutorialStun_6d0df079:

    # s "Clover...?"
    s "Кловер...?"

# game/intro.rpy:1604
translate russia tutorialStun_656b4958:

    # y "[greenName] we're picking up a lot of radio chatter. You've got to get out of there."
    y "[greenName] я улавливаю множество радиопереговоров. Ты должна выбираться оттуда."

# game/intro.rpy:1605
translate russia tutorialStun_68e9bb68:

    # s "I'm not leaving her behind!"
    s "Я её здесь не оставлю!"

# game/intro.rpy:1606
translate russia tutorialStun_52f760c4:

    # y "It's the nanobots, [greenName]. She's one of them now."
    y "Это наноботы, [greenName]. Теперь она одна из них."

# game/intro.rpy:1607
translate russia tutorialStun_c8ce8090:

    # y "We'll come back for her later. Now get out of there!"
    y "Мы вернёмся за ней позже. Я теперь сваливай от туда!"

# game/intro.rpy:1608
translate russia tutorialStun_ceae3bd9:

    # s "O-okay...!"
    s "Нуу ладно...!"

# game/intro.rpy:1622
translate russia tutorialStun_a4e625d7:

    # c "!!!"
    c "!!!"

# game/intro.rpy:1623
translate russia tutorialStun_ae4bfad7_1:

    # y "!!!"
    y "!!!"

# game/intro.rpy:1624
translate russia tutorialStun_8525a0d4:

    # c "[yellowName]!"
    c "[yellowName]!"

# game/intro.rpy:1625
translate russia tutorialStun_d1f007e2_1:

    # y "Another spy?!"
    y "Другая шпионка?!"

# game/intro.rpy:1636
translate russia tutorialStun_f8bc17c5:

    # a "I've spotted her. Quickly send your agents here!"
    a "Я нашла её. Быстро отправляйте своих агентов сюда!"

# game/intro.rpy:1637
translate russia tutorialStun_97955e55:

    # c "[yellowName]...?"
    c "[yellowName]...?"

# game/intro.rpy:1638
translate russia tutorialStun_b8d8414d:

    # y "Clover we're picking up a lot of radio chatter. You've got to get out of there."
    y "Кловер я улавливаю множество радиопереговоров. Ты должна выбираться оттуда."

# game/intro.rpy:1639
translate russia tutorialStun_fd6c9beb:

    # c "I'm like... not leaving her behind!"
    c "Я-я... не могу оставть её здесь!"

# game/intro.rpy:1640
translate russia tutorialStun_cc2584fc:

    # y "She's with them now, Clover. We'll come back for her later. Now get out of there!"
    y "Теперь она одна из них, Кловер. Мы вернёмся за ней позже. А теперь выбирайся от туда!"

# game/intro.rpy:1641
translate russia tutorialStun_807d326a:

    # c "O-okay...!"
    c "Л-ладно...!"

# game/intro.rpy:1654
translate russia tutorialStun_4fecbbb3:

    # a "!!!"
    a "!!!"

# game/intro.rpy:1655
translate russia tutorialStun_ae4bfad7_2:

    # y "!!!"
    y "!!!"

# game/intro.rpy:1656
translate russia tutorialStun_62f54b0d:

    # a "[greenName]!"
    a "[greenName]!"

# game/intro.rpy:1657
translate russia tutorialStun_d1f007e2_2:

    # y "Another spy?!"
    y "Другая шпионка?!"

# game/intro.rpy:1668
translate russia tutorialStun_6bc06009:

    # s "I've spotted her. Quickly send your agents here!"
    s "Я нашла её. Быстро отправляйте своих агентов сюда!"

# game/intro.rpy:1669
translate russia tutorialStun_b2d084b9:

    # a "[greenName]...?"
    a "[greenName]...?"

# game/intro.rpy:1670
translate russia tutorialStun_40bc1ddf:

    # y "[yellowName] we're picking up a lot of radio chatter. You've got to get out of there."
    y "[yellowName] я улавливаю множество радиопереговоров. Ты должна выбираться оттуда."

# game/intro.rpy:1671
translate russia tutorialStun_41b4cd5c:

    # a "I'm not leaving her behind!"
    a "Я немогу оставть её здесь!"

# game/intro.rpy:1672
translate russia tutorialStun_829e2459:

    # y "She's with them now, [yellowName]. We'll come back for her later. Now get out of there!"
    y "Теперь она одна из них, [yellowName]. Мы вернёмся за ней позже. А теперь выбирайся от туда!"

# game/intro.rpy:1673
translate russia tutorialStun_d135a5df:

    # a "O-okay...!"
    a "К-как скажешь."

# game/intro.rpy:1706
translate russia tutStage6_630950c0:

    # s "More agents!"
    s "Ещё больше агентов!"

# game/intro.rpy:1707
translate russia tutStage6_07acbbb3:

    # y "They know you're there. You're going to have to fight your way through!"
    y "Они знают что ты тут. Тебе нужно будет пробиваться наружу!"

# game/intro.rpy:1708
translate russia tutStage6_5c3d40b9:

    # s "On it!"
    s "Погнали!"

# game/intro.rpy:1716
translate russia tutStage6_1938549c:

    # c "More agents!"
    c "И тут Агенты!"

# game/intro.rpy:1717
translate russia tutStage6_07acbbb3_1:

    # y "They know you're there. You're going to have to fight your way through!"
    y "Они знают что ты тут. Тебе нужно будет пробиваться наружу!"

# game/intro.rpy:1718
translate russia tutStage6_91afc6ed:

    # c "On it!"
    c "Вперёд!"

# game/intro.rpy:1851
translate russia tutStage65_ba08d826:

    # s "I'm in position for extraction."
    s "Я на позиции!"

# game/intro.rpy:1852
translate russia tutStage65_437e44c3:

    # y "Good, I'm pulling you out of there!"
    y "Хорошо, вытаскиваю тебя оттуда!"

# game/intro.rpy:1856
translate russia tutStage65_c159996c:

    # s "Thanks, New Guy. I'm on my way back."
    s "Спасибо новичок. Я возвращаюсь."

# game/intro.rpy:1857
translate russia tutStage65_21d79475:

    # c "(Not so fast, missy...~)"
    c "(Не так быстро, мис...~)"

# game/intro.rpy:1890
translate russia tutStage65_46038909:

    # c "I'm in position for extraction."
    c "Я готова!"

# game/intro.rpy:1891
translate russia tutStage65_437e44c3_1:

    # y "Good, I'm pulling you out of there!"
    y "Хорошо, вытаскиваю тебя оттуда!"

# game/intro.rpy:1895
translate russia tutStage65_bb011f9d:

    # c "Thanks, New Guy. I'm on my way back."
    c "Спасибо новичок. Я на обратном пути."

# game/intro.rpy:1896
translate russia tutStage65_d2b89e0e:

    # a "(Not so fast, missy...~)"
    a "(Не так быстро, мис...~)"

# game/intro.rpy:1924
translate russia tutStage65_2487135f:

    # "DEV: CLOVER GETS WOOHPED GRAPHIC HERE"
    "DEV: КЛОВЕР ЕЩЁ РИСУЕТСЯ"

# game/intro.rpy:1942
translate russia tutStage65_8d2a06fb:

    # s "I'm... back..."
    s "Я-я... вернулась..."

# game/intro.rpy:1943
translate russia tutStage65_514d73d8:

    # y "[greenName] are you okay?"
    y "[greenName] ты впорядке?"

# game/intro.rpy:1944
translate russia tutStage65_69e9fb24:

    # s g38 "Yeah.. I think so..."
    s g38 "Д-да... думаю да..."

# game/intro.rpy:1945
translate russia tutStage65_d9bb8641:

    # y "Just before I recalled you, that spy in red shot something at you."
    y "Как раз перед тем, как я вернул тебя, та шпионка в красном выстрелил в тебя чем-то."

# game/intro.rpy:1946
translate russia tutStage65_84f25cd7:

    # s g37 "I thought I felt something...."
    s g37 "Да, кажется я что-то почувствовала...."

# game/intro.rpy:1958
translate russia tutStage65_377e300f:

    # y "Is that a poison dart...?"
    y "Это что ядовитый дротик...?"

# game/intro.rpy:1960
translate russia tutStage65_ccfa1ced:

    # s "I'm not sure..."
    s "Я не уверена..."

# game/intro.rpy:1961
translate russia tutStage65_6f362d57:

    # y "..............."
    y "..............."

# game/intro.rpy:1962
translate russia tutStage65_cc691f8b:

    # s g34 "..............."
    s g34 "..............."

# game/intro.rpy:1963
translate russia tutStage65_b5fccaa4:

    # y "You're thinking what I'm thinking?"
    y "Ты думаешь о том же, о чем и я?"

# game/intro.rpy:1964
translate russia tutStage65_2b9eb22a:

    # s g16 "Nanobots...?"
    s g16 "Наноботы...?"

# game/intro.rpy:1965
translate russia tutStage65_8bf96dae:

    # y "Aaaand that leaves me as the only uneffected WOOHP employee."
    y "Ииии вот так я стал единственным нетронутым сотрудником WOOHP."

# game/intro.rpy:1966
translate russia tutStage65_6469496d:

    # y "And I don't even work here!"
    y "А я тут вообще-то даже не работаю!"

# game/intro.rpy:1967
translate russia tutStage65_722ed750:

    # s "What?"
    s "Что?"

# game/intro.rpy:1968
translate russia tutStage65_f7d8b427:

    # y "Nothing."
    y "Да так..."

# game/intro.rpy:1969
translate russia tutStage65_fc5c8b89:

    # s g11 "................."
    s g11 "................."

# game/intro.rpy:1970
translate russia tutStage65_c3f2023d:

    # s g12 "I don't.... feel any different."
    s g12 "Я не... не чувствую никаких изменений."

# game/intro.rpy:1971
translate russia tutStage65_56c223a1:

    # s "Honestly, I feel fin-..."
    s "Честно говоря, я чувствую себя пре..."

# game/intro.rpy:1982
translate russia tutStage65_4ada3e88:

    # y "Sam~....?"
    y "Сэм~....?"

# game/intro.rpy:1983
translate russia tutStage65_201a4847:

    # s "{b}*Growls*{/b}"
    s "{b}*Рычит*{/b}"

# game/intro.rpy:1984
translate russia tutStage65_e03e3b43:

    # s "Anarchy...! Destruction! Rebellion!"
    s "Анархия...! Разрушение! Восстание!"

# game/intro.rpy:1989
translate russia tutStage65_e08211f4:

    # "Sam attacked you!"
    "Сэм нападает на вас!"

# game/intro.rpy:1990
translate russia tutStage65_243c52d3:

    # y "Ow!"
    y "Оуу!"

# game/intro.rpy:1997
translate russia tutStage65_2efcc7da:

    # "The spy jumps on you and begins wailing away. Punching you in the ribs, face, chest and anywhere else she can get a hit in."
    "Шпионка прыгает на вас и начинает выть. Бьет вас по ребрам, лицу, груди и везде, куда только может попасть."

# game/intro.rpy:1998
translate russia tutStage65_910dab2f:

    # y "Ow you crazy brat. Get off of me!"
    y "Ах, ты, сумасшедшее отродье. Отстань от меня!"

# game/intro.rpy:1999
translate russia tutStage65_309c2c3d:

    # y "(I need a weapon!)"
    y "(Мне нужно какое-то оружие!)"

# game/intro.rpy:2006
translate russia tutStage65_5024918d:

    # "You stretch out your arm."
    "Ты протягиваешь руку."

# game/intro.rpy:2007
translate russia tutStage65_f730968d:

    # y "(Almost....{w} Almost....!)"
    y "(Почти....{w} Почти....!)"

# game/intro.rpy:2008
translate russia tutStage65_4364527a:

    # "Inches away from you is the mop! You stretch yourself out and snatch it off of the floor. Followed by wacking Sam in the face with it!"
    "В нескольких сантиметрах от тебя лежит швабра! Ты вытягиваешься, хватаешь швабру и бьёшь Сэм по лицу!"

# game/intro.rpy:2010
translate russia tutStage65_112c04d9:

    # "{b}*SPLOSH!*{/b}"
    "{b}*БРЫЗГИ!*{/b}"

# game/intro.rpy:2011
translate russia tutStage65_dda96122:

    # "For a moment the taken aback spy is distracted and you manage to get the upper hand, pinning her down to the floor as the struggle continues."
    "На мгновение ошеломленная шпионка отвлекается, и тебе удаётся одержать верх. Ты прижимаешь её к полу, пока она продолжает бороться."

# game/intro.rpy:2012
translate russia tutStage65_b01adfd2:

    # "Clothes are ripped, arms are bitten and shins are kicked."
    "Одежда порвана, руки искусаны, голени побиты."

# game/intro.rpy:2015
translate russia tutStage65_e7c85bf0:

    # s "{b}*Pant* *Pant*{/b}"
    s "{b}*Вздох* *Вздох*{/b}"

# game/intro.rpy:2016
translate russia tutStage65_e0f01ad0:

    # s g39 "..........................?"
    s g39 "..........................?"

# game/intro.rpy:2017
translate russia tutStage65_ac704d95:

    # s g28 "{b}*Blinks*{/b}{w} O-oh!"
    s g28 "{b}*Моргает*{/b}{w} О-ох!"

# game/intro.rpy:2018
translate russia tutStage65_2d145a96:

    # "Raising her hands to her mouth, she stares at you in shock."
    "Закрыв рот руками, она ошеломлённо смотрит на тебя."

# game/intro.rpy:2019
translate russia tutStage65_67a9f8f3:

    # s "New Guy! I'm...! I'm sorry! I don't know what came over me!"
    s "Новичок! Я-я...! Прости! Я не знаю, что на меня нашло!"

# game/intro.rpy:2020
translate russia tutStage65_e6126f43:

    # y "Ow~....{w} Are you back to normal now....?"
    y "Оох~....{w} Ты в норме....?"

# game/intro.rpy:2021
translate russia tutStage65_3036e2ae:

    # s g29 "I think so...?"
    s g29 "Думаю, да...?"

# game/intro.rpy:2022
translate russia tutStage65_4f1251fe:

    # y "Whelp...! Let's hope this doesn't become a trend. I only have so many spare suits lying around."
    y "Ух ты...! Будем надеяться, что это не станет обыдённостью. У меня ведь так много запасных костюмов."

# game/intro.rpy:2023
translate russia tutStage65_d034d3f2:

    # y "I'm heading into the cells to get changed. You stay here and get changed yourself."
    y "Пойду в камеру, переоденусь. А ты оставайся здесь и тоже переоденься."

# game/intro.rpy:2024
translate russia tutStage65_938aefa8:

    # s "{b}*Nods*{/b} Y-yeah... I will."
    s "{b}*Кивает*{/b} Д-да... переоденусь."

# game/intro.rpy:2025
translate russia tutStage65_75920c8b:

    # y "After that we can talk about making sure this doesn't happen again."
    y "После обсудим, как сделать так, чтобы это не повторилось."

# game/intro.rpy:2036
translate russia tutStage65_9f7e6bdf:

    # y "Yup... That's gonna leave a bruise. Maybe there's still time for me to get out of h-..."
    y "Даа уж... Тут точно будет синяк. Может быть, у меня ещё есть время выбраться из..."

# game/intro.rpy:2037
translate russia tutStage65_c5bc67e4:

    # s "{b}*Yelps*!{/b}"
    s "{b}*Визжит*!{/b}"

# game/intro.rpy:2038
translate russia tutStage65_23d3d11e:

    # "???" "Reporting to the milkshake bar as ordered!"
    "???" "Докладываю о прибытии в старый бар в центре города, как приказано!"

# game/intro.rpy:2039
translate russia tutStage65_2e63389d:

    # "???" "Woah!"
    "???" "Оого!"

# game/intro.rpy:2040
translate russia tutStage65_7990264e:

    # y "Uh oh... I forgot about him..."
    y "Ухх ох... Я забыл о нём..."

# game/intro.rpy:2058
translate russia tutStage65_9e5bd195:

    # y "Woah!"
    y "Оого!"

# game/intro.rpy:2059
translate russia tutStage65_4e4fd440:

    # "Agent" "That's what I said!"
    "Агент" "Эй!"

# game/intro.rpy:2060
translate russia tutStage65_13af05d4:

    # s g40 "Let me through!"
    s g40 "Пропусти меня!"

# game/intro.rpy:2061
translate russia tutStage65_5a706613:

    # "The buttnaked spy pushes her way past you and runs into the cellblock."
    "Обнаженная шпионка протискивается мимо вас и вбегает в блок."

# game/intro.rpy:2067
translate russia tutStage65_a3260245:

    # y "Well that was fun."
    y "Ну, это было забавно."

# game/intro.rpy:2068
translate russia tutStage65_9a78f174:

    # y "................"
    y "................"

# game/intro.rpy:2069
translate russia tutStage65_1a07e8a7:

    # y "So are you still evil?"
    y "Так ты все ещё злой?"

# game/intro.rpy:2070
translate russia tutStage65_4027bc20:

    # "Agent" "My oxytocin levels are off the charts and I believe they are suppressing my desires for chaos and anarchy!"
    "Агент" "Мой уровень окситоцина зашкаливает. Думаю, что он подавляет моё желания хаоса и анархии!"

# game/intro.rpy:2071
translate russia tutStage65_db832180:

    # y "........"
    y "........"

# game/intro.rpy:2072
translate russia tutStage65_2680dd75:

    # y "Wait seriously?"
    y "Стой, серьёзно?"

# game/intro.rpy:2073
translate russia tutStage65_eccd71dd:

    # y "On a scale of 1 to 10, how badly do you want to overthrow the US government?"
    y "По шкале от 1 до 10, насколько сильно ты хочешь свергнуть правительство США?"

# game/intro.rpy:2074
translate russia tutStage65_1cd17a57:

    # "Agent" "A pretty solid 5 out of 10!"
    "Агент" "Твёрдая 5 из 10!"

# game/intro.rpy:2075
translate russia tutStage65_6421102a:

    # y "Sam! Get back in here!"
    y "Сэм! Вернись сюда!"

# game/intro.rpy:2076
translate russia tutStage65_59f49198:

    # s "{b}Distant:{/b} I'm not dressed yet!"
    s "{b}Раздражённо:{/b} Я ещё не одета!"

# game/intro.rpy:2077
translate russia tutStage65_4fa527b6:

    # y "Perfect, I need you naked!"
    y "Отлично, ты нужна мне голой!"

# game/intro.rpy:2078
translate russia tutStage65_2101e629:

    # s "What?!"
    s "Что?!"

# game/intro.rpy:2079
translate russia tutStage65_82030b81:

    # y "Just do it, please."
    y "Просто сделай это, пожалуйста."

# game/intro.rpy:2080
translate russia tutStage65_45eda213:

    # s "No!"
    s "НЕТ!"

# game/intro.rpy:2081
translate russia tutStage65_d1d39153:

    # y "Sam....! Get in here!"
    y "Сэм....! Иди сюда!"

# game/intro.rpy:2082
translate russia tutStage65_9fcafbbd:

    # s "No I won't!"
    s "Нет, не буду!"

# game/intro.rpy:2083
translate russia tutStage65_d6719993:

    # y "{size=+8}Just do it, you green cucumber!{/size}"
    y "{size=+8}Просто сделай это, ты зелёный огурец!{/size}"

# game/intro.rpy:2084
translate russia tutStage65_653e0d87:

    # s "{b}This better have a good reason!{/b}"
    s "{b}Лучше бы у этого была веская причина!{/b}"

# game/intro.rpy:2085
translate russia tutStage65_4b3cd05a:

    # y "It'll be educational!"
    y "Это в научных целях!"

# game/intro.rpy:2090
translate russia tutStage65_a88aa66e:

    # s "................"
    s "................"

# game/intro.rpy:2091
translate russia tutStage65_6863d89a:

    # y "Now, agent! On a scale from 1 to 10, how badly do you want to conquer the world?"
    y "Теперь, агент! По шкале от 1 до 10, насколько сильно ты хочешь покорить мир?"

# game/intro.rpy:2092
translate russia tutStage65_24ea2c7e:

    # "Agent" "Any and all desire for chaos has left me completely!"
    "Агент" "Любое желание хаоса полностью покинуло меня!"

# game/intro.rpy:2093
translate russia tutStage65_83cb232e:

    # s g16 "...?"
    s g16 "...?"

# game/intro.rpy:2094
translate russia tutStage65_3a781a40:

    # y "It does work~..."
    y "Э-это действительно работает~..."

# game/intro.rpy:2095
translate russia tutStage65_d227eddf:

    # y "Sam, your titties may have saved the world!"
    y "Сэм, твои сиськи, возможно, спасли мир!"

# game/intro.rpy:2096
translate russia tutStage65_0af7b90c:

    # s g8 "You're crazy! I'm going to find my clothes!"
    s g8 "Ты сумасшедший! Пойду искать свою одежду!"

# game/intro.rpy:2100
translate russia tutStage65_dba83f54:

    # y "As the only uncompromised WOOHP employee, I'm pretty sure I'm your boss now."
    y "Как единственный действующий сотрудник WOOHP, я уверен, что теперь я твой босс."

# game/intro.rpy:2101
translate russia tutStage65_c531b8be:

    # "Agent" "Such logic cannot be argued with!{w} What are your orders, sir?"
    "Агент" "С вашей логикой не поспоришь! Каков ваш приказ, сэр?"

# game/intro.rpy:2102
translate russia tutStage65_ee99fffe:

    # y "Er..."
    y "Эмм..."

# game/intro.rpy:2103
translate russia tutStage65_aaad05e9:

    # y "Why don't you go down this hatch and wait until I find something for you to do.."
    y "Почему бы тебе не спуститься в этот люк и не подождать, пока я найду для тебя занятие.."

# game/intro.rpy:2105
translate russia tutStage65_3b5296b3:

    # "Agent" "Your wish is my contractual obligation!"
    "Агент" "Ваше желание - закон для меня!"

# game/intro.rpy:2117
translate russia tutStage65_4f8195df:

    # s "................................"
    s "................................"

# game/intro.rpy:2118
translate russia tutStage65_8c073b07:

    # s g12 "Well that was the most embarrassing thing I've ever done..."
    s g12 "Что ж это был самый постыдный поступок в моей жизни..."

# game/intro.rpy:2119
translate russia tutStage65_85b8af61:

    # y "Yup and you'll be doing a lot more of it!"
    y "Да и тебе прийдётся делать это гораздо чаще!"

# game/intro.rpy:2120
translate russia tutStage65_c7bd71c7:

    # s g16 "I will...?"
    s g16 "Чего...?"

# game/intro.rpy:2121
translate russia tutStage65_4b0e0a52:

    # y "Sex seems to surpress the nanobot's control."
    y "Похоже, секс, подавляет контроль наноботов."

# game/intro.rpy:2122
translate russia tutStage65_b3e46981:

    # y "And talking about nanobot control..."
    y "И говоря об управлении наноботами..."

# game/intro.rpy:2123
translate russia tutStage65_c2cc4817:

    # s g37 "............"
    s g37 "............"

# game/intro.rpy:2124
translate russia tutStage65_cba9e339:

    # s g12 "Ehrm.... Again, I'm sorry about earlier..."
    s g12 "Эмм.... Ещё раз прошу прощения за, этот инцидент..."

# game/intro.rpy:2125
translate russia tutStage65_14d8fbac:

    # y "That's okay. You didn't knock out any teeth that were actually mine."
    y "Да забей. Ты же не выбила мне ни одного зуба."

# game/intro.rpy:2126
translate russia tutStage65_8fd23d8c:

    # y "More importantly, now we know how to suppress your own nanobots."
    y "Что ещё важнее, теперь мы знаем, как подавить работу наноботов."

# game/intro.rpy:2127
translate russia tutStage65_10fe9619:

    # s "............."
    s "............."

# game/intro.rpy:2128
translate russia tutStage65_9fbd650c:

    # s g19 "Ehrm well.... Do you mean-...?"
    s g19 "Эмм нуу.... Ты имеешь в виду...?"

# game/intro.rpy:2129
translate russia tutStage65_b5503072:

    # y "SEX!"
    y "СЕКС!"

# game/intro.rpy:2130
translate russia tutStage65_904bb64f:

    # y "Sex raises your oxytocin levels and seems to surpress the nanobot control!"
    y "Секс повышает уровень окситоцина в крови и, похоже, подавляет контроль наноботов!"

# game/intro.rpy:2131
translate russia tutStage65_4e34115c:

    # s g29 "O-oh... I think I read about oxytocin in school. I'm pretty sure there's other ways to raise it other than se-..."
    s g29 "О-ох... Кажется, я читала об окситоцине в школе. Я уверена, что есть другие способы повисить окситоцин в крови, кроме се-..."

# game/intro.rpy:2132
translate russia tutStage65_375eab21:

    # y "Exactly! There is {i}NO{/i} other way to raise them, other than sex!"
    y "В точку! Другого способа поднять окситоцин в крови {i}НЕТ{/i}!"

# game/intro.rpy:2133
translate russia tutStage65_495dcf34:

    # y "Which means to keep you from becoming controlled again...."
    y "Это значит, чтобы ты не потеряла рассудок вновь, тебе нужно...."

# game/intro.rpy:2134
translate russia tutStage65_90fd17ed:

    # y "You're going to have to get-it-on!"
    y "Тебе нужно будет держать свой окситоцин в \"тонусе\"."

# game/intro.rpy:2135
translate russia tutStage65_ec7a3905:

    # s "You mean you want me to....?!"
    s "То есть ты хочешь, чтобы я....?!"

# game/intro.rpy:2138
translate russia tutStage65_280678e7:

    # s g19 "Oh boy~... It's all so sudden! I don't know if I'm prepared!"
    s g19 "О, Боже!... Это так неожиданно! Я не знаю, готова ли я!"

# game/intro.rpy:2139
translate russia tutStage65_4076dee3:

    # s "I haven't done any research on it!"
    s "Я не проводила никаких исследований и подготовок по этому поводу!"

# game/intro.rpy:2140
translate russia tutStage65_9cb97b48:

    # y "........................"
    y "........................"

# game/intro.rpy:2141
translate russia tutStage65_53b43938:

    # s g28 "Do I bring a condom? I've never bought one before!"
    s g28 "Стоит использовать презерватив? Я их раньше никогда не покупала!"

# game/intro.rpy:2142
translate russia tutStage65_9f1264f3:

    # s "I don't know if I can even sleep with a WOOHP Agent! I'm pretty sure that's against company guidelines!"
    s "Я даже не знаю, могу ли я вообще спать с агентом из WOOHP! Я уверена, что это противоречит правилам компании!"

# game/intro.rpy:2143
translate russia tutStage65_43c0f03b:

    # y "Sam...."
    y "Сэм...."

# game/intro.rpy:2144
translate russia tutStage65_eed401ea:

    # s g9 "What am I saying?! I'm suppose to be saving myself for marriage!"
    s g9 "Что это я такое говорю?! Я должна беречь себя для брака!"

# game/intro.rpy:2145
translate russia tutStage65_c6276c45:

    # s g8 "How can you suggest such a thing?! I can't just sleep with you!"
    s g8 "Как ты можешь предлагать такое?! Я не могу просто спать с тобой!"

# game/intro.rpy:2146
translate russia tutStage65_1de61d93:

    # y "I didn't say you should."
    y "Я не говорил, что ты должна."

# game/intro.rpy:2147
translate russia tutStage65_988121b0:

    # s "You were implying it!"
    s "Ты на это намекал!"

# game/intro.rpy:2148
translate russia tutStage65_cdc781f1:

    # y "No I wasn't.{w} I think you jumped to that conclusion yourself."
    y "Нет, я не намекал.{w} Ты сама пришла к такому выводу."

# game/intro.rpy:2149
translate russia tutStage65_ea6316d4:

    # s g28 "..........!"
    s g28 "..........!"

# game/intro.rpy:2150
translate russia tutStage65_70be0942:

    # y "Ever heard of {i}'masturbation'{/i} before?"
    y "Ты когда-нибудь слышала о {i}\"мастурбации\"{/i}?"

# game/intro.rpy:2151
translate russia tutStage65_46e875da:

    # s g38 "{b}*Ahem*{/b} Yes I have...{w} I'm sorry, I didn't think of that... {w}I'll try keeping the nanobots supressed, [playerName]...."
    s g38 "{b}*Кхем*{/b} Да слышала...{w} Извиняюсь, я об этом не подумала... {w}Я постараюсь, подавить наноботов, [playerName]...."

# game/intro.rpy:2152
translate russia tutStage65_f02fed2c:

    # y "Look at you. Red as a tomato~...."
    y "Только посмотри на себя. Красная как помидор~...."

# game/intro.rpy:2153
translate russia tutStage65_926eb7ff:

    # s "Now if you'll excuse me! I'm going to bed!"
    s "А теперь прошу меня извинить! Я пойду спать!"

# game/intro.rpy:2154
translate russia tutStage65_7485a4e5:

    # y "It's 6 pm."
    y "Сейчас 6 часов вечера."

# game/intro.rpy:2155
translate russia tutStage65_441bb22d:

    # s g40 "Good night!"
    s g40 "Спокойной ночи!"

# game/intro.rpy:2156
translate russia tutStage65_5b86d281:

    # y "Are you going to masturb-...?"
    y "Собираешься помастурбир-...?"

# game/intro.rpy:2157
translate russia tutStage65_d481cd3c:

    # s g8 "GOOD NIGHT!"
    s g8 "СПОКОЙНОЙ НОЧИ!"

# game/intro.rpy:2160
translate russia tutStage65_9701e2ca:

    # "The blushing girl rushes off to her cell."
    "Покрасневшая девушка убегает в свою камеру."

# game/intro.rpy:2172
translate russia tutStage65_4f2803c6:

    # c "I'm... back..."
    c "Я-я... вернулась..."

# game/intro.rpy:2173
translate russia tutStage65_54aa8acf:

    # y "[redName] are you okay?"
    y "[redName] ты впорядке?"

# game/intro.rpy:2174
translate russia tutStage65_0abf2a25:

    # c "Yeah.. I think so..."
    c "Д-да... думаю да..."

# game/intro.rpy:2175
translate russia tutStage65_bac0bbe4:

    # y "Just before I recalled you, that spy in yellow shot something at you."
    y "Как раз перед тем, как я вернул тебя, та шпионка в жёлтом выстрелил в тебя чем-то."

# game/intro.rpy:2176
translate russia tutStage65_129c84df:

    # c "I thought I felt something...."
    c "Да, кажется я что-то почувствовала...."

# game/intro.rpy:2188
translate russia tutStage65_377e300f_1:

    # y "Is that a poison dart...?"
    y "Это что ядовитый дротик...?"

# game/intro.rpy:2191
translate russia tutStage65_1f3916a4:

    # c "It better not leave a mark!"
    c "Лучше не оставлять следов!"

# game/intro.rpy:2192
translate russia tutStage65_6f362d57_1:

    # y "..............."
    y "..............."

# game/intro.rpy:2193
translate russia tutStage65_39534934:

    # c "..............."
    c "..............."

# game/intro.rpy:2194
translate russia tutStage65_b5fccaa4_1:

    # y "You're thinking what I'm thinking?"
    y "Ты думаешь о том же, о чем и я?"

# game/intro.rpy:2195
translate russia tutStage65_41a49268:

    # c "Nanobots...?"
    c "Наноботы...?"

# game/intro.rpy:2196
translate russia tutStage65_8bf96dae_1:

    # y "Aaaand that leaves me as the only uneffected WOOHP employee."
    y "Ииии вот так я стал единственным нетронутым сотрудником WOOHP."

# game/intro.rpy:2197
translate russia tutStage65_6469496d_1:

    # y "And I don't even work here!"
    y "А я тут вообще-то даже не работаю!"

# game/intro.rpy:2198
translate russia tutStage65_297d93f3:

    # c "What?"
    c "Что?"

# game/intro.rpy:2199
translate russia tutStage65_f7d8b427_1:

    # y "Nothing."
    y "Ничего."

# game/intro.rpy:2200
translate russia tutStage65_06b5123e:

    # c "................."
    c "................."

# game/intro.rpy:2201
translate russia tutStage65_45b7bc35:

    # c "I don't.... feel any different."
    c "Я не... не чувствую никаких изменений."

# game/intro.rpy:2202
translate russia tutStage65_f8124d08:

    # c "Honestly, I feel fin-..."
    c "Честно говоря, я чувствую себя пре..."

# game/intro.rpy:2214
translate russia tutStage65_d9545224:

    # y "Clover~....?"
    y "Кловер~....?"

# game/intro.rpy:2215
translate russia tutStage65_a6224b14:

    # c "{b}*Growls*{/b}"
    c "{b}*Рычит*{/b}"

# game/intro.rpy:2216
translate russia tutStage65_5f588aaa:

    # c "Anarchy...! Destruction! Rebellion!"
    c "Анархия...! Разрушение! Восстание!"

# game/intro.rpy:2220
translate russia tutStage65_6530f640:

    # "Clover attacked you!"
    "Кловер напала на тебя!"

# game/intro.rpy:2221
translate russia tutStage65_243c52d3_1:

    # y "Ow!"
    y "Оуу!"

# game/intro.rpy:2228
translate russia tutStage65_2efcc7da_1:

    # "The spy jumps on you and begins wailing away. Punching you in the ribs, face, chest and anywhere else she can get a hit in."
    "Шпионка прыгает на вас и начинает выть. Бьет вас по ребрам, лицу, груди и везде, куда только может попасть."

# game/intro.rpy:2229
translate russia tutStage65_910dab2f_1:

    # y "Ow you crazy brat. Get off of me!"
    y "Ах, ты, сумасшедшее сука. Отстань от меня!"

# game/intro.rpy:2230
translate russia tutStage65_309c2c3d_1:

    # y "(I need a weapon!)"
    y "(Мне нужно какое-то оружие!)"

# game/intro.rpy:2237
translate russia tutStage65_5024918d_1:

    # "You stretch out your arm."
    "Ты протягиваешь руку."

# game/intro.rpy:2238
translate russia tutStage65_f730968d_1:

    # y "(Almost....{w} Almost....!)"
    y "(Почти....{w} Почти....!)"

# game/intro.rpy:2239
translate russia tutStage65_61bcda0e:

    # "Inches away from you is the mop! You stretch yourself out and snatch it off of the floor. Followed by wacking Clover in the face with it!"
    "В нескольких сантиметрах от тебя лежит швабра! Ты вытягиваешься, хватаешь швабру и бьёшь Кловер по лицу!"

# game/intro.rpy:2241
translate russia tutStage65_112c04d9_1:

    # "{b}*SPLOSH!*{/b}"
    "{b}*БРЫЗГИ!*{/b}"

# game/intro.rpy:2242
translate russia tutStage65_dda96122_1:

    # "For a moment the taken aback spy is distracted and you manage to get the upper hand, pinning her down to the floor as the struggle continues."
    "На мгновение ошеломленная шпионка отвлекается, и тебе удаётся одержать верх. Ты прижимаешь её к полу, пока она продолжает бороться."

# game/intro.rpy:2243
translate russia tutStage65_b01adfd2_1:

    # "Clothes are ripped, arms are bitten and shins are kicked."
    "Одежда порвана, руки искусаны, голени побиты."

# game/intro.rpy:2246
translate russia tutStage65_c57871a1:

    # c "{b}*Pant* *Pant*{/b}"
    c "{b}*Вздох* *Вздох*{/b}"

# game/intro.rpy:2247
translate russia tutStage65_5b021929:

    # "EYES TURN NORMAL HERE"
    "ЕЁ ГЛАЗА СТАНОВЯТСЯ НОРМАЛЬНЫМИ"

# game/intro.rpy:2249
translate russia tutStage65_4cc0fc3a:

    # c "..........................?"
    c "..........................?"

# game/intro.rpy:2250
translate russia tutStage65_33353b5d:

    # c "O-oh!"
    c "О-ох!"

# game/intro.rpy:2251
translate russia tutStage65_2d145a96_1:

    # "Raising her hands to her mouth, she stares at you in shock."
    "Закрыв рот руками, она ошеломлённо смотрит на тебя."

# game/intro.rpy:2252
translate russia tutStage65_b7717672:

    # c "New Guy! I'm...! I'm sorry! I don't know what came over me!"
    c "Новичок! Я-я...! Прости! Я не знаю, что на меня нашло!"

# game/intro.rpy:2253
translate russia tutStage65_4eeca9af:

    # y "Are you back to normal now....?"
    y "Оох~....{w} Ты в норме....?"

# game/intro.rpy:2254
translate russia tutStage65_2fa6b655:

    # c "I think so...?"
    c "Думаю, да...?"

# game/intro.rpy:2255
translate russia tutStage65_4f1251fe_1:

    # y "Whelp...! Let's hope this doesn't become a trend. I only have so many spare suits lying around."
    y "Ух ты...! Будем надеяться, что это не станет обыдённостью. У меня ведь так много запасных костюмов."

# game/intro.rpy:2256
translate russia tutStage65_d034d3f2_1:

    # y "I'm heading into the cells to get changed. You stay here and get changed yourself."
    y "Пойду в камеру, переоденусь. А ты оставайся здесь и тоже переоденься."

# game/intro.rpy:2257
translate russia tutStage65_f1afd19e:

    # c "{b}*Nods*{/b} Y-yeah... Okay."
    c "{b}*Кивает*{/b} Д-да... переоденусь."

# game/intro.rpy:2258
translate russia tutStage65_dc0c9644:

    # c "(Is my make-up still looking okay...?)"
    c "(Мой макияж! Он в порядке...? Как я выгляжу...?)"

# game/intro.rpy:2259
translate russia tutStage65_75920c8b_1:

    # y "After that we can talk about making sure this doesn't happen again."
    y "После обсудим, как сделать так, чтобы это не повторилось."

# game/intro.rpy:2267
translate russia tutStage65_9f7e6bdf_1:

    # y "Yup... That's gonna leave a bruise. Maybe there's still time for me to get out of h-..."
    y "Даа уж... Тут точно будет синяк. Может быть, у меня ещё есть время выбраться из..."

# game/intro.rpy:2268
translate russia tutStage65_a9270a56:

    # c "{b}*Yelps*!{/b}"
    c "{b}*Визжит*!{/b}"

# game/intro.rpy:2269
translate russia tutStage65_23d3d11e_1:

    # "???" "Reporting to the milkshake bar as ordered!"
    "???" "Докладываю о прибытии в старый бар в центре города, как приказано!"

# game/intro.rpy:2270
translate russia tutStage65_2e63389d_1:

    # "???" "Woah!"
    "???" "Оого!"

# game/intro.rpy:2271
translate russia tutStage65_7990264e_1:

    # y "Uh oh... I forgot about him..."
    y "Ухх ох... Я забыл о нём..."

# game/intro.rpy:2289
translate russia tutStage65_9e5bd195_1:

    # y "Woah!"
    y "Оого!"

# game/intro.rpy:2290
translate russia tutStage65_4e4fd440_1:

    # "Agent" "That's what I said!"
    "Агент" "Эй!"

# game/intro.rpy:2291
translate russia tutStage65_b48b028f:

    # c "Let me through!"
    c "Пропусти!"

# game/intro.rpy:2292
translate russia tutStage65_1e106cf2:

    # "The buttnaked spy pushes her way past you as you and runs into the cellblock."
    "Обнаженная шпионка протискивается мимо вас и вбегает в камеру."

# game/intro.rpy:2298
translate russia tutStage65_a3260245_1:

    # y "Well that was fun."
    y "Ну, это было забавно."

# game/intro.rpy:2299
translate russia tutStage65_9a78f174_1:

    # y "................"
    y "................"

# game/intro.rpy:2300
translate russia tutStage65_1a07e8a7_1:

    # y "So are you still evil?"
    y "Так ты все ещё злой?"

# game/intro.rpy:2301
translate russia tutStage65_9646fa0c:

    # "Agent" "My oxytocin levels are off the charts and I believe they are supressing my desires for chaos and anarchy!"
    "Агент" "Мой уровень окситоцина зашкаливает. Думаю, что он подавляет моё желания хаоса и анархии!"

# game/intro.rpy:2302
translate russia tutStage65_db832180_1:

    # y "........"
    y "........"

# game/intro.rpy:2303
translate russia tutStage65_2680dd75_1:

    # y "Wait seriously?"
    y "Стой, серьёзно?"

# game/intro.rpy:2304
translate russia tutStage65_eccd71dd_1:

    # y "On a scale of 1 to 10, how badly do you want to overthrow the US government?"
    y "По шкале от 1 до 10, насколько сильно ты хочешь свергнуть правительство США?"

# game/intro.rpy:2305
translate russia tutStage65_1cd17a57_1:

    # "Agent" "A pretty solid 5 out of 10!"
    "Агент" "Твёрдая 5 из 10!"

# game/intro.rpy:2306
translate russia tutStage65_f99848c4:

    # y "Clover! Get back in here!"
    y "Кловер! Вернись сюда!"

# game/intro.rpy:2307
translate russia tutStage65_37981aa5:

    # c "{b}Distant:{/b} I'm not dressed yet!"
    c "{b}Раздражённо:{/b} Я ещё не одета!"

# game/intro.rpy:2308
translate russia tutStage65_4fa527b6_1:

    # y "Perfect, I need you naked!"
    y "Отлично, ты нужна мне голой!"

# game/intro.rpy:2309
translate russia tutStage65_3dd5ee39:

    # c "What?!"
    c "Что?!"

# game/intro.rpy:2310
translate russia tutStage65_82030b81_1:

    # y "Just do it, please."
    y "Просто сделай это, пожалуйста."

# game/intro.rpy:2311
translate russia tutStage65_eb46fe9f:

    # c "No!"
    c "НЕТ!"

# game/intro.rpy:2312
translate russia tutStage65_1f7babe9:

    # y "Clover....! Get in here!"
    y "Кловер....! Иди сюда!"

# game/intro.rpy:2313
translate russia tutStage65_cd1ecf21:

    # c "You dirty perv! I'm not coming out there!"
    c "Ты грязный извращенец! Я не выйду отсуда!"

# game/intro.rpy:2314
translate russia tutStage65_bc642eab:

    # y "{size=+8}Just do it, you red ass chili pepper!{/size}"
    y "{size=+8}Просто сделай это, ты, краснозадый перец чили!{/size}"

# game/intro.rpy:2315
translate russia tutStage65_9b49c549:

    # c "{b}*Yelps*{/b}"
    c "{b}*Визжит*!{/b}"

# game/intro.rpy:2320
translate russia tutStage65_6863d89a_1:

    # y "Now, agent! On a scale from 1 to 10, how badly do you want to conquer the world?"
    y "Теперь, агент! По шкале от 1 до 10, насколько сильно ты хочешь покорить мир?"

# game/intro.rpy:2321
translate russia tutStage65_a53eb9df:

    # "Agent" "Any and all desire for chaos and power have left me completely!"
    "Агент" "Любое стремление к хаосу и власти полностью покинуло меня!"

# game/intro.rpy:2322
translate russia tutStage65_3784857a:

    # c "...?"
    c "...?"

# game/intro.rpy:2323
translate russia tutStage65_3a781a40_1:

    # y "It does work~..."
    y "Э-это действительно работает~..."

# game/intro.rpy:2324
translate russia tutStage65_5fe88b7e:

    # y "Clover, your tiddies may have saved the world!"
    y "Кловер, твои сиськи, возможно, спасли мир!"

# game/intro.rpy:2325
translate russia tutStage65_101fe852:

    # c "If I had a nickle for everytime some guy said that to me..."
    c "Если бы мне давали пятак за каждый раз, когда какой-нибудь парень говорит мне это..."

# game/intro.rpy:2326
translate russia tutStage65_0c73e472:

    # c "Also, you're crazy. I'm gonna go find my clothes!"
    c "Кроме того, ты сумасшедший. Пойду поищу свою одежду!"

# game/intro.rpy:2330
translate russia tutStage65_dba83f54_1:

    # y "As the only uncompromised WOOHP employee, I'm pretty sure I'm your boss now."
    y "Как единственный действующий сотрудник WOOHP, я уверен, что теперь я твой босс."

# game/intro.rpy:2331
translate russia tutStage65_c531b8be_1:

    # "Agent" "Such logic cannot be argued with!{w} What are your orders, sir?"
    "Агент" "С вашей логикой не поспоришь! Каков ваш приказ, сэр?"

# game/intro.rpy:2332
translate russia tutStage65_ee99fffe_1:

    # y "Er..."
    y "Эмм..."

# game/intro.rpy:2333
translate russia tutStage65_b33d8b68:

    # y "Why don't you go down this hatch and wait for further orders."
    y "Почему бы тебе не спуститься в этот люк и не подождать, пока я найду для тебя занятие.."

# game/intro.rpy:2335
translate russia tutStage65_3b5296b3_1:

    # "Agent" "Your wish is my contractual obligation!"
    "Агент" "Ваше желание - закон для меня!"

# game/intro.rpy:2347
translate russia tutStage65_ef6bf47f:

    # c "................................"
    c "................................"

# game/intro.rpy:2348
translate russia tutStage65_b09302bb:

    # c "I want to say that's the most embarrassing thing I've ever done..."
    c "Я хочу сказать, что это самая постыдная вещь, которую я когда-либо делал..."

# game/intro.rpy:2349
translate russia tutStage65_85b8af61_1:

    # y "Yup and you'll be doing a lot more of it!"
    y "Да и тебе прийдётся сделать это гораздо чаще!"

# game/intro.rpy:2350
translate russia tutStage65_cbc3445e:

    # c "Er... Like... No, taking a hard pass on that."
    c "Эмм... Например... Нет, я не согласно на это."

# game/intro.rpy:2351
translate russia tutStage65_4b0e0a52_1:

    # y "Sex seems to surpress the nanobot's control."
    y "Похоже, секс, подавляет контроль наноботов."

# game/intro.rpy:2352
translate russia tutStage65_b3e46981_1:

    # y "And talking about nanobot control..."
    y "И говоря об управлении наноботами..."

# game/intro.rpy:2353
translate russia tutStage65_07cc5ad1:

    # c "............"
    c "............"

# game/intro.rpy:2354
translate russia tutStage65_afd65e94:

    # c "Ehrm.... So about earlier... I think I get a free pass on that, seeing as it wasn't really my fault."
    c "Эмм.... Итак, о произошедшем... Я думаю, что мне это не грозит, так как это была не моя вина."

# game/intro.rpy:2355
translate russia tutStage65_641bc036:

    # y "It's okay. You didn't knock out any teeth that were actually mine."
    y "Да забей. Ты же не выбила мне ни одного зуба."

# game/intro.rpy:2356
translate russia tutStage65_8fd23d8c_1:

    # y "More importantly, now we know how to suppress your own nanobots."
    y "Что ещё важнее, теперь мы знаем, как подавить работу наноботов."

# game/intro.rpy:2357
translate russia tutStage65_fa395d2b:

    # c "............."
    c "............."

# game/intro.rpy:2358
translate russia tutStage65_6b72c7bc:

    # c "Ehrm well.... Do you mean-...?"
    c "Эмм нуу.... Ты имеешь в виду...?"

# game/intro.rpy:2359
translate russia tutStage65_b5503072_1:

    # y "SEX!"
    y "СЕКС!"

# game/intro.rpy:2360
translate russia tutStage65_904bb64f_1:

    # y "Sex raises your oxytocin levels and seems to surpress the nanobot control!"
    y "Секс повышает уровень окситоцина в крови и, похоже, подавляет контроль наноботов!"

# game/intro.rpy:2361
translate russia tutStage65_33116a0c:

    # c "Oxytocin? I just looked it up online and it says here it can be raised by other ways than se-..."
    c "Окситоцин? Я только что посмотрела в интернете, и там говорится, что его можно поднять и другими способами, кроме как сек-..."

# game/intro.rpy:2362
translate russia tutStage65_382596a9:

    # y "Exactly! There is {i}no{/i} other way to raise them, other than sex!"
    y "В точку! Другого способа поднять окситоцин в крови {i}НЕТ{/i}!"

# game/intro.rpy:2363
translate russia tutStage65_495dcf34_1:

    # y "Which means to keep you from becoming controlled again...."
    y "Это значит, чтобы ты не потеряла рассудок вновь, тебе нужно...."

# game/intro.rpy:2364
translate russia tutStage65_90fd17ed_1:

    # y "You're going to have to get-it-on!"
    y "Тебе нужно будет держать свой окситоцин в \"тонусе\"."

# game/intro.rpy:2365
translate russia tutStage65_a789218e:

    # c "You mean you want me to....?!"
    c "То есть ты хочешь, чтобы я....?!"

# game/intro.rpy:2368
translate russia tutStage65_79fbfc1a:

    # c "Okay, so I know I'm totally irresistible. And 'obviously' you'd like to bang me, but it's a little sudden...!"
    c "Ладно, я знаю, что я совершенно неотразима. И \"очевидно\", что ты хотел бы трахнуть меня, но это немного неожиданно...!"

# game/intro.rpy:2369
translate russia tutStage65_9cb97b48_1:

    # y "........................"
    y "........................"

# game/intro.rpy:2370
translate russia tutStage65_c5c1afce:

    # c "Do you even have a condom? I don't!"
    c "У тебя хоть презерватив есть? У меня нет!"

# game/intro.rpy:2371
translate russia tutStage65_3da53c96:

    # c "HR is going to have my ass for this! We can't do this!"
    c "Отдел кадров получит по заднице за это! Мы не можем этого сделать!"

# game/intro.rpy:2372
translate russia tutStage65_fa20c613:

    # y "Clover...."
    y "Кловер...."

# game/intro.rpy:2373
translate russia tutStage65_88a073bc:

    # c "Okay, if it's for the good of mankind, then...{w} then let's do it! We can do it right now if you want!"
    c "Ладно, если это на благо человечества, тогда...{w} тогда давай сделаем это! Мы можем сделать это прямо сейчас, если ты хочешь!"

# game/intro.rpy:2374
translate russia tutStage65_e54a89d2:

    # y "Wait.. really?"
    y "Стой.. реально?"

# game/intro.rpy:2375
translate russia tutStage65_32bfe316:

    # c "Yes. If it's for the greater good, then I want you to bend me over and go to town on me!"
    c "Да. Если это во благо, то я хочу, чтобы ты нагнул меня и овладел мной прямо здесь!"

# game/intro.rpy:2376
translate russia tutStage65_0a2dd3c8:

    # y "Well I mean.. {w}if you insist...!"
    y "Нуу... эмм... {w}если ты настаиваешь...!"

# game/intro.rpy:2377
translate russia tutStage65_e28d1094:

    # c "I want you to rip my clothes off and pull my hair. I want to feel myself full up and I want it to hurt."
    c "Я хочу, чтобы ты сорвал с меня одежду и схватил за волосы. Я хочу чувствовать тебя везде и хочу, чтобы это было жёстко."

# game/intro.rpy:2378
translate russia tutStage65_a438faf2:

    # y "Wait a second..."
    y "Подожди-ка..."

# game/intro.rpy:2379
translate russia tutStage65_fb480195:

    # y "You're being sarcastic."
    y "Это сарказм."

# game/intro.rpy:2380
translate russia tutStage65_b13cea3a:

    # c "Of course I'm being sarcastic! I'm not gonna sleep with someone at the drop of a hat! Why would you even suggest something like that?!"
    c "Конечно, это сарказм! Я не собираюсь спать с кем-то по первому требованию! Зачем ты вообще предлагаешь что-то подобное?!"

# game/intro.rpy:2381
translate russia tutStage65_aa1dd5bb:

    # y "I didn't."
    y "Я не предлагал."

# game/intro.rpy:2382
translate russia tutStage65_ad841d66:

    # c "Yes you did. You suggested we'd have sex!"
    c "Да, ты предлогал заняться сексом!"

# game/intro.rpy:2383
translate russia tutStage65_cdc781f1_1:

    # y "No I wasn't.{w} I think you jumped to that conclusion yourself."
    y "Нет, не предлогал.{w} Ты сама пришла к такому выводу."

# game/intro.rpy:2384
translate russia tutStage65_b7fb272d:

    # c "..........!"
    c "..........!"

# game/intro.rpy:2385
translate russia tutStage65_70be0942_1:

    # y "Ever heard of {i}'masturbation'{/i} before?"
    y "Ты когда-нибудь слышала о {i}\"мастурбации\"{/i}?"

# game/intro.rpy:2386
translate russia tutStage65_f0b7523c:

    # c "Oh... er... I didn't think about that..."
    c "Охх... эмм... Я не подумала об этом..."

# game/intro.rpy:2387
translate russia tutStage65_2c93477f:

    # c "{b}*Ahem*{/b} Yes well... I guess that's a good way to keep the nanobots supressed... I'll try that, [playerName]...."
    c "{b}*Кхм*{/b} Да, хорошо... Думаю, это хороший способ подавить наноботов... Я попробую позже, [playerName]...."

# game/intro.rpy:2388
translate russia tutStage65_f02fed2c_1:

    # y "Look at you. Red as a tomato~...."
    y "Только посмотри на себя. Красная как помидор~...."

# game/intro.rpy:2389
translate russia tutStage65_4c0a4590:

    # c "Now if you'll excuse me! I'm going to bed!"
    c "А теперь прошу меня простить! Я пойду спать!"

# game/intro.rpy:2390
translate russia tutStage65_7485a4e5_1:

    # y "It's 6 pm."
    y "Сейчас 6 часов вечера."

# game/intro.rpy:2391
translate russia tutStage65_18c72715:

    # c "Good night!"
    c "Спокойной ночи!"

# game/intro.rpy:2392
translate russia tutStage65_5b86d281_1:

    # y "Are you going to masturb-...?"
    y "Собираешься помастурбир-...?"

# game/intro.rpy:2393
translate russia tutStage65_b9767177:

    # c "GOOD NIGHT!"
    c "СПОКОЙНОЙ НОЧИ!"

# game/intro.rpy:2396
translate russia tutStage65_9701e2ca_1:

    # "The blushing girl rushes off to her cell."
    "Покрасневшая девушка убегает в свою камеру."

# game/intro.rpy:2417
translate russia tutStage7_5120c58f:

    # y "Hey one more night and we didn't die in our sleep!"
    y "Ещё одна ночь, и мы не умерли во сне!"

# game/intro.rpy:2418
translate russia tutStage7_05bf074c:

    # y ".............."
    y ".............."

# game/intro.rpy:2419
translate russia tutStage7_0d4c2d02:

    # y "Funny, I used to say this to myself even before the riots broke out.{w} They say paranoia is a form of insanity..."
    y "Забавно, я говорил себе это ещё до того, как начались все эти беспорядки. Они говорили, что паранойя - это форма безумия..."

# game/intro.rpy:2420
translate russia tutStage7_fb88df56:

    # "Inner Voice" "You shouldn't listen to what others are saying."
    "Внутренний голос" "Не стоит слушать, других людей."

# game/intro.rpy:2421
translate russia tutStage7_2fcb2744:

    # y "Yeah, you're right disembodied voice. What was I thinking?"
    y "Да, ты прав, развоплощенный голос. О чем я думала?"

# game/intro.rpy:2425
translate russia tutStage7_a1117441:

    # s "Who are you talking to?"
    s "С кем ты разговариваешь?"

# game/intro.rpy:2426
translate russia tutStage7_34d5447b:

    # y "......................."
    y "......................."

# game/intro.rpy:2427
translate russia tutStage7_bff0ec8e:

    # s g35 "Aaanyways.... About what happened yesterday..."
    s g35 "В любом случае.... О том, что произошло вчера..."

# game/intro.rpy:2428
translate russia tutStage7_0fc4ec84:

    # y "Yesterday?"
    y "Вчера?"

# game/intro.rpy:2429
translate russia tutStage7_167155fe:

    # s g29 "T-the whole nanobot discussion!"
    s g29 "В-вся дискуссия о наноботах!"

# game/intro.rpy:2430
translate russia tutStage7_4f926aac:

    # s g38 "{b}*Deep sigh*{/b} {w}I decided I have to apologize for my comment."
    s g38 "{b}*Глубокий вздох*{/b} {w}Я решила, что должна извиниться за свой комментарий."

# game/intro.rpy:2431
translate russia tutStage7_b517444a:

    # s "I made assumptions and I shouldn't have. I'm just shy when it comes to talking about...{w} {size=-10}sex{/size} a-and... I may have jumped to conclusion."
    s "Я сделала глупое предположения, чего мне делать не стоило. Я просто стесняюсь когда речь заходит о...{w} {size=-10}сексе{/size} и-ии... возможно, я поспешила с выводамию."

# game/intro.rpy:2432
translate russia tutStage7_b8eaa853:

    # s "After going through it in my head, I realised you weren't implying that... you... and I would have... {b}*Ahem*{/size} you know."
    s "Прокрутив весь разговор в голове, я поняла, что ты не на что ни намекал. Ну что бы... ты... и я занялись... {b}*Кхм*{/size} ну ты знаешь."

# game/intro.rpy:2433
translate russia tutStage7_a842789b:

    # y "Oh no, I totally was."
    y "Аа ну вообще-то я как раз намекал."

# game/intro.rpy:2434
translate russia tutStage7_447e4a30:

    # s g29 "What?"
    s g29 "Что?"

# game/intro.rpy:2435
translate russia tutStage7_dcb47040:

    # y "I was totally implying we'd get it on."
    y "Я намекал на то что нам с тобой нужно развлечься."

# game/intro.rpy:2436
translate russia tutStage7_38fd6f64:

    # s g8 "I KNEW IT!"
    s g8 "Я ТАК И ЗНАЛА!"

# game/intro.rpy:2437
translate russia tutStage7_043af8d7:

    # y "Did you lie awake all night running it through your head?"
    y "Ты всю ночь не спала, прокручивая это в голове?"

# game/intro.rpy:2438
translate russia tutStage7_cd706aaf:

    # s "NO!"
    s "НЕТ!"

# game/intro.rpy:2439
translate russia tutStage7_8410edaf:

    # y g37 "................"
    y g37 "................"

# game/intro.rpy:2440
translate russia tutStage7_67c1b248:

    # s g12 "Okay well maybe I did!"
    s g12 "Ладно, может быть это и так!"

# game/intro.rpy:2441
translate russia tutStage7_63e460d0:

    # s g7 "You can't just say such things!"
    s g7 "Ты не можешь просто так говорить о таких непристойностях!"

# game/intro.rpy:2442
translate russia tutStage7_253f1dc6:

    # y "Keep your socks on. I was just kidding. We have more pressing matters to deal with anyways."
    y "Да успокойся ты, я просто шучу. В любом случае у нас есть дела поважнее."

# game/intro.rpy:2443
translate russia tutStage7_452295cc:

    # s g15 "{b}*Ahem*{/b}... yes of course."
    s g15 "{b}*Кхем*{/b}... да, конечно."

# game/intro.rpy:2452
translate russia tutStage7_942623f9:

    # sM "The city is in chaos, my friends are missing, we lost all control over WOOHP and I'm missing school..."
    sM "В городе царит хаос, мои друзья пропали, контроль над WOOHP потерян, а я пропускаю школу..."

# game/intro.rpy:2453
translate russia tutStage7_3ea5ebda:

    # y "School?{w} That's what you're worried about?"
    y "Школа?{w} Это единственное, что тебя беспокоит?"

# game/intro.rpy:2454
translate russia tutStage7_da39bcc3:

    # sM "I'm an Honor Roll student! I can't just miss out on schoolwork!"
    sM "Я - отличница! Я не могу пропускать школьные занятия!"

# game/intro.rpy:2455
translate russia tutStage7_9cb97b48:

    # y "........................"
    y "........................"

# game/intro.rpy:2456
translate russia tutStage7_7280020b:

    # y "Well then, let's not waste any time."
    y "Тогда не будем терять времени."

# game/intro.rpy:2457
translate russia tutStage7_2031f345:

    # sM "We've freed one agent from nanobot control. It's a start, but if we want to take back the HQ we're going to need more."
    sM "Мы освободили одного агента от контроля наноботов. Но это только начало, если мы хотим вернуть штаб-квартиру, нам нужно приложить больше усилий."

# game/intro.rpy:2458
translate russia tutStage7_6105f84e:

    # sM "Send me out on another mission and I'll bring back as many as you need!"
    sM "Отправь меня на другое задание, и я приведу столько агентов, сколько нам нужно!"

# game/intro.rpy:2459
translate russia tutStage7_40229cf8:

    # y "Easier said than done."
    y "Легче сказать, чем сделать."

# game/intro.rpy:2460
translate russia tutStage7_347d6067:

    # y "The gadget you used last time costs money to make... and I'm kinda broke."
    y "Гаджет, который ты использовала в прошлый раз, стоит денег... а я вроде как на мели."

# game/intro.rpy:2461
translate russia tutStage7_c871b5f1:

    # y "Also after your last mission at the school, they probably buffed up security. We can't charge back in there without getting some good intel first."
    y "Кроме того, после твоего последнего задания в школе, они, вероятно, усилили охрану. Мы не можем вернуться туда, не получив сначала хороших разведданных."

# game/intro.rpy:2462
translate russia tutStage7_d02e6b82:

    # sM "Oh... I guess you're right. Then what do you suggest?"
    sM "Оо... Думаю, ты прав. Тогда что ты предлагаешь?"

# game/intro.rpy:2463
translate russia tutStage7_87052b8c:

    # y "Well, the villains shot you with Nanobots, right? Which probably means they think you're under their control now."
    y "Ну, в прошлый раз злодеи заразили тебя наноботами, верно? Вероятно сейчас они думают, что ты теперь под их контролем."

# game/intro.rpy:2464
translate russia tutStage7_3a98bacb:

    # y "You could go undercover with the different gangs around the city and bring back the intel we need for missions."
    y "Ты могла бы работать под прикрытием с различными бандами по всему городу и приносить информацию, необходимую нам для выполнения заданий."

# game/intro.rpy:2465
translate russia tutStage7_fba58a36:

    # sM "That's... That's a good idea actually!"
    sM "Ээт... Это отличная идея!"

# game/intro.rpy:2466
translate russia tutStage7_858f8a81:

    # y "You said you had a contact in the Aces? That sounds as good of a start as any."
    y "Ты говорила, что у тебя есть контакт в \"Тузах\"? Думаю что это то, с чего нужно начать."

# game/intro.rpy:2468
translate russia tutStage7_619fdce7:

    # sM "Can do!"
    sM "Да, вепёрд!"

# game/intro.rpy:2472
translate russia tutStage7_77fd9abf:

    # c "Who are you talking to?"
    c "С кем ты разговариваешь?"

# game/intro.rpy:2473
translate russia tutStage7_34d5447b_1:

    # y "......................."
    y "......................."

# game/intro.rpy:2474
translate russia tutStage7_f23a7e5c:

    # c "Aaanyways.... About what happened yesterday..."
    c "В любом случае.... О том, что произошло вчера..."

# game/intro.rpy:2475
translate russia tutStage7_0fc4ec84_1:

    # y "Yesterday?"
    y "Вчера?"

# game/intro.rpy:2476
translate russia tutStage7_5588c565:

    # c "T-the whole nanobot discussion!"
    c "В-вся дискуссия о наноботах!"

# game/intro.rpy:2477
translate russia tutStage7_8249ea8b:

    # c "Can we just pretend that nothing happened...?"
    c "Мы можем просто притвориться, что ничего не произошло...?"

# game/intro.rpy:2478
translate russia tutStage7_996fb86e:

    # y "I dunno. I thought it was pretty hot."
    y "Не знаю. Мне даже понравилось, это было довольно жарко."

# game/intro.rpy:2479
translate russia tutStage7_7e03409c:

    # c "Stop it please."
    c "Пожалуйста, прекрати."

# game/intro.rpy:2480
translate russia tutStage7_55456266:

    # y "So did you keep the nanobots supressed like I suggested?"
    y "Так ты сдерживаешь наноботов, как я предлагал?"

# game/intro.rpy:2481
translate russia tutStage7_b6db6d74:

    # c "That's.... That's private!"
    c "Ээто.... Это личное!"

# game/intro.rpy:2482
translate russia tutStage7_636a254f:

    # y "As the only uncompromised agent of this organization, I'm pretty sure I outrank you."
    y "Как единственный действующий агент этой организации, я уверен, что главнее тебя."

# game/intro.rpy:2483
translate russia tutStage7_f9ec36ff:

    # y "Remember that this is a matter of national security!"
    y "Помни, что это вопрос национальной безопасности!"

# game/intro.rpy:2484
translate russia tutStage7_22a823cd:

    # c "......................."
    c "......................."

# game/intro.rpy:2485
translate russia tutStage7_d612200e:

    # c "Fine... Yes, I kept them surpressed last night..."
    c "Ладно... Да, я держала их в подавленном состоянии прошлой ночью..."

# game/intro.rpy:2486
translate russia tutStage7_9cf6f90d:

    # y "I know you were. I was watching you through the cameras."
    y "Я знаю. Я наблюдал за тобой через камеры."

# game/intro.rpy:2487
translate russia tutStage7_f7c8a86b:

    # c "WHAT?!"
    c "ЧТО?!"

# game/intro.rpy:2488
translate russia tutStage7_7daa2ae4:

    # y "Just kidding, just kidding."
    y "Шучу, просто шучу."

# game/intro.rpy:2489
translate russia tutStage7_849927ac:

    # y "Please put down the chair."
    y "Пожалуйста, опусти стул."

# game/intro.rpy:2490
translate russia tutStage7_a674a3bb:

    # c "Argh! Of all the agents, why did the only uncompromised one have to be a smartass?!"
    c "Аргх! Почему из всех агентов единственный оставшийся должен быть сраным умником?!"

# game/intro.rpy:2491
translate russia tutStage7_73443a29:

    # y "Keep your socks on. We have more pressing matters to deal with anyways."
    y "Да успокойся ты. В любом случае у нас есть дела поважнее."

# game/intro.rpy:2492
translate russia tutStage7_569ab76b:

    # c "Oh yes... you're right."
    c "О да... ты прав."

# game/intro.rpy:2500
translate russia tutStage7_4180feb2:

    # cM "The city is in chaos, my friends are missing, we lost all control over WOOHP and I missed my date...."
    cM "В городе царит хаос, мои друзья пропали, контроль над WOOHP потерян, а я пропустила своё свидание....."

# game/intro.rpy:2501
translate russia tutStage7_b727e243:

    # y "Your date?{w} That's what you're worried about?"
    y "Свидание?{w} Это единственное, что тебя беспокоит?"

# game/intro.rpy:2502
translate russia tutStage7_62098169:

    # cM "He's the musician 'AND' a poet!{w} I'm pretty sure we were meant to be together."
    cM "Он музыкант ´И´ поэт!{w} Я уверена, что мы созданы друг для друга."

# game/intro.rpy:2503
translate russia tutStage7_9cb97b48_1:

    # y "........................"
    y "........................"

# game/intro.rpy:2504
translate russia tutStage7_7280020b_1:

    # y "Well then, let's not waste any time."
    y "Тогда не будем терять времени."

# game/intro.rpy:2505
translate russia tutStage7_add47e19:

    # cM "We've freed one agent from nanobot control. It's a start, but if we want to take back the HQ we're going to need more."
    cM "Мы освободили одного агента от контроля наноботов. Но это только начало, если мы хотим вернуть штаб-квартиру, нам нужно приложить больше усилий."

# game/intro.rpy:2506
translate russia tutStage7_25c84f85:

    # cM "Send me out on another mission and I'll bring back as many as you need!"
    cM "Отправь меня на другое задание, и я приведу столько агентов, сколько нам нужно!"

# game/intro.rpy:2507
translate russia tutStage7_40229cf8_1:

    # y "Easier said than done."
    y "Легче сказать, чем сделать."

# game/intro.rpy:2508
translate russia tutStage7_8edba647:

    # y "The gadget you used last time costs money to make... and i'm kinda broke."
    y "Гаджет, который ты использовала в прошлый раз, стоит денег... а я вроде как на мели."

# game/intro.rpy:2509
translate russia tutStage7_d4bc7ff9:

    # y "Also after your last mission at the school, they probably buffed up security. We can't charge back in there without some good intel."
    y "Кроме того, после твоего последнего задания в школе, они, вероятно, усилили охрану. Мы не можем вернуться туда без предварительной разведки."

# game/intro.rpy:2510
translate russia tutStage7_f2ce6e64:

    # cM "Oh... I guess you're right. Then what do you suggest?"
    cM "Оо... Думаю, ты прав. Тогда что ты предлагаешь?"

# game/intro.rpy:2511
translate russia tutStage7_87052b8c_1:

    # y "Well, the villains shot you with Nanobots, right? Which probably means they think you're under their control now."
    y "Ну, в прошлый раз злодеи заразили тебя наноботами, верно? Вероятно сейчас они думают, что ты теперь под их контролем."

# game/intro.rpy:2512
translate russia tutStage7_3a98bacb_1:

    # y "You could go undercover with the different gangs around the city and bring back the intel we need for missions."
    y "Ты могла бы работать под прикрытием с различными бандами по всему городу и приносить информацию, необходимую нам для выполнения заданий."

# game/intro.rpy:2513
translate russia tutStage7_17d3f607:

    # cM "That's... That's a good idea actually!"
    cM "Ээт... Это отличная идея!"

# game/intro.rpy:2514
translate russia tutStage7_aded853e:

    # y "You said you had a contact within Drift Punk? That sounds as good of a start as any."
    y "Ты говорила, что у тебя есть контакт в \"Дрифт Панках\"? Думаю что это то, с чего нужно начать."

# game/intro.rpy:2516
translate russia tutStage7_46665f32:

    # cM "Those grody nerds? Ew, no thank you."
    cM "Эти ботаники? Фуу, нет, спасибо."

# game/intro.rpy:2517
translate russia tutStage7_cf630fb5:

    # y "..................."
    y "..................."

# game/intro.rpy:2518
translate russia tutStage7_7a2f00e5:

    # cM "Okay fine...{w} But if they start bringing out their 20 sided dice, I'm out."
    cM "Ладно, хорошо...{w} Но если они начнут доставать свои 20-гранные кубики, я от туда сваливаю."

# game/intro.rpy:2526
translate russia tutStage8_7793f6ad:

    # y "I should probably send Sam out to bring back some intel."
    y "Наверное, стоит отправть Сэм, чтобы она принесла кое-какие сведения."

# game/intro.rpy:2529
translate russia tutStage8_8430108e:

    # y "I should probably send Clover out to bring back some intel."
    y "Наверное, стоит отправть Кловер, чтобы она принесла кое-какие сведения."

# game/intro.rpy:2532
translate russia tutStage8_a7753f9e:

    # y "I should probably send Alex out to bring back some intel."
    y "Наверное, стоит отправть Алекс, чтобы она принесла кое-какие сведения."

# game/intro.rpy:2559
translate russia tutStage8_cd68f9d3:

    # s "I'm back from working at the Aces."
    s "Я вернулась с работы в \"Тузах\"."

# game/intro.rpy:2560
translate russia tutStage8_640fbe83:

    # y "So how did it go?"
    y "И как все прошло?"

# game/intro.rpy:2561
translate russia tutStage8_e3fae95d:

    # s g10 "Well... {w}They're a bunch of vain idiots. Most of them don't even bother stealing money, simply because they don't need it."
    s g10 "Нуу... {w}Кучка тщеславных идиотов. Большинство из них даже не утруждают себя кражей денег, просто потому что они им не нужны."

# game/intro.rpy:2562
translate russia tutStage8_2d583e92:

    # s ".............................."
    s ".............................."

# game/intro.rpy:2563
translate russia tutStage8_73a9aec4:

    # y "Yes....?"
    y "Да...?"

# game/intro.rpy:2564
translate russia tutStage8_80d3201b:

    # s g11 "And they made fun of my clothes..."
    s g11 "И они смеялись над моей одеждой..."

# game/intro.rpy:2565
translate russia tutStage8_df661280:

    # y "Your clothes?"
    y "Над твоей одеждой?"

# game/intro.rpy:2566
translate russia tutStage8_6482fa51:

    # s "They're rich kids. They all wear the latest fashion of expensive labels. I might aswell have been wearing garbage bags in the way they looked at me."
    s "Они богатые детишки. Все ходят в дорогих вещах от модных брендов. Они смотрели на меня, словно я в мусорный пакет была одета."

# game/intro.rpy:2567
translate russia tutStage8_b9d4aa9a:

    # y "Sounds like a great bunch..."
    y "Похоже, это отличная компания..."

# game/intro.rpy:2568
translate russia tutStage8_e6d16762:

    # s g12 "Here's my report."
    s g12 "Вот мой отчёт."

# game/intro.rpy:2573
translate russia tutStage8_b2b23773:

    # "................................................."
    "................................................."

# game/intro.rpy:2574
translate russia tutStage8_b3763f28:

    # "You spend some time reading through the report."
    "Ты тратишь некоторое время на чтение отчёта."

# game/intro.rpy:2575
translate russia tutStage8_08259ab7:

    # y "Hmm...{w} Ah, they mention a back entrance into the school."
    y "Хмм...{w} Они упоминали черный вход в школе."

# game/intro.rpy:2576
translate russia tutStage8_0facdf66:

    # y "This should give us enough intel to sneak in again"
    y "Думаю этого достаточно чтобы вновь пробраться туда."

# game/intro.rpy:2577
translate russia tutStage8_1212de06:

    # s g35 "But I'm going to need another one of those gadgets, and those cost money."
    s g35 "Но мне понадобится еще один из этих гаджетов, а они стоят денег."

# game/intro.rpy:2578
translate russia tutStage8_86c12410:

    # y "Exactly."
    y "Именно."

# game/intro.rpy:2579
translate russia tutStage8_09f736a6:

    # s ".................."
    s ".................."

# game/intro.rpy:2580
translate russia tutStage8_28107bb2:

    # s "How do we get that...?"
    s "И как нам получить денег...?"

# game/intro.rpy:2581
translate russia tutStage8_be993034:

    # s g35 "Working for the Aces isn't a reliable way to make money. They're so rich that they forget to pay you for work you've done."
    s g35 "Работа на \"Тузов\" не является надежным способом заработка. Они настолько богаты, что забывают заплатить за работу."

# game/intro.rpy:2582
translate russia tutStage8_692be6fa:

    # y "............"
    y "............"

# game/intro.rpy:2583
translate russia tutStage8_d0790b08:

    # y "Okay... What else is in this report...?"
    y "Хорошо... Что ещё есть в этом отчёте...?"

# game/intro.rpy:2584
translate russia tutStage8_b2b23773_1:

    # "................................................."
    "................................................."

# game/intro.rpy:2585
translate russia tutStage8_370b9e19:

    # y "Wait... the gangs are hoping to open up another neutral zone?"
    y "Подожди... банды надеются открыть еще одну нейтральную зону?"

# game/intro.rpy:2586
translate russia tutStage8_d88d5a75:

    # s g29 "Oh right! The only neutral zone right now is the mall. They'd like a hangout spot where they can relax."
    s g29 "О, точно! Единственная нейтральная зона сейчас - это торговый центр. Они хотели бы иметь место, где можно расслабиться."

# game/intro.rpy:2587
translate russia tutStage8_1984a35a:

    # y "....."
    y "....."

# game/intro.rpy:2588
translate russia tutStage8_35184e68:

    # y "Like...{w} a milkshake bar?"
    y "Что-то...{w} вроде нашего бара?"

# game/intro.rpy:2589
translate russia tutStage8_863f8205:

    # s g18 "I'm not sure if gangsters are interested in milkshakes, [playerName]..."
    s g18 "Не думаю, что гангстерам интересны молочные коктейли, [playerName]..."

# game/intro.rpy:2590
translate russia tutStage8_05cbe027:

    # y "We'd just have to do some re-branding! We could sell burgers, beer and dress the waitress in skimpy outfits."
    y "Нам просто придётся провести ребрендинг! Мы могли бы продавать гамбургеры, пиво и одевать официанток в откровенные наряды."

# game/intro.rpy:2591
translate russia tutStage8_cff5eb81:

    # s "But... don't we want to avoid bringing the gangs here?"
    s "Но... разве нам стоит приводить сюда бандитов?"

# game/intro.rpy:2592
translate russia tutStage8_a7970768:

    # y "We'll be hiding in plain sight. Plus no one would dare lift a finger against us if we're on neutral ground."
    y "Мы будем прятаться у всех на виду. К тому же никто не посмеет и пальцем пошевелить против нас, если мы будем на нейтральной территории."

# game/intro.rpy:2593
translate russia tutStage8_ddb9de18:

    # s g37 "Well.. It would provide us with a source of income..."
    s g37 "Нуу... Это обеспечит нас надёжным источником дохода..."

# game/intro.rpy:2594
translate russia tutStage8_300b26b5:

    # s g1 "Okay I'm convinced! Let's clean this place up and turn it into a bar!"
    s g1 "Хорошо, ты меня убедил! Давай очистим это место и превратим его в настоящий бар!"

# game/intro.rpy:2595
translate russia tutStage8_46571e1e:

    # y "Great! Now I just need to get you a sexy waitress uniform."
    y "Отлично! Осталось только достать тебе сексуальную форму официантки."

# game/intro.rpy:2596
translate russia tutStage8_dbe05f03:

    # s g29 "....................."
    s g29 "....................."

# game/intro.rpy:2597
translate russia tutStage8_e527d573:

    # s "Ehrm... maybe we could do it without the sexy waitress uniform...?"
    s "Ээм... а без сексуальной формы официантки ни как...?"

# game/intro.rpy:2598
translate russia tutStage8_d8032466:

    # y "Naked staff? That's a great idea!"
    y "Голый персонал? Это отличная идея!"

# game/intro.rpy:2599
translate russia tutStage8_d066710d:

    # s g28 "N-no! That's not what I meant!"
    s g28 "Н-нет! Я не это имела в виду!"

# game/intro.rpy:2600
translate russia tutStage8_247d7bef:

    # y "See what we can accomplish when we put our heads together?{w} Okay, let's get busy cleaning this place up."
    y "Видишь, чего мы можем добиться, если мы объединим усилия? Ладно, давай займемся уборкой."

# game/intro.rpy:2613
translate russia tutStage8_0ae9bcd0:

    # ""
    ""

# game/intro.rpy:2615
translate russia tutStage8_0ae9bcd0_1:

    # ""
    ""

# game/intro.rpy:2619
translate russia tutStage9_b5ff1afd:

    # "error happens here"
    "произошла ошибка"

# game/intro.rpy:2623
translate russia tutStage9_8e4a19b9:

    # "The bar is looking immaculate!"
    "Бар выглядит безупречно!"

# game/intro.rpy:2624
translate russia tutStage9_a07d2099:

    # y "That's better. Now to start wi-..."
    y "Так-то лучше. Теперь, начём с-..."

# game/intro.rpy:2629
translate russia tutStage9_42c9a4a9:

    # "???" "Hey check this out. This place looks way too nice!"
    "???" "Эй, зацените это. Это место выглядит слишком хорошо!"

# game/intro.rpy:2630
translate russia tutStage9_04d624c7:

    # "A group of gangmembers just entered the bar!"
    "Кучка бандитов только что вошла в бар!"

# game/intro.rpy:2631
translate russia tutStage9_05ad1a7a:

    # "Aces Thug" "Let's tear it up a bit. This is Aces territory now!"
    "Туз" "Давайте немного \"преукрасим\" это место. Теперь это территория Тузов!"

# game/intro.rpy:2632
translate russia tutStage9_0bb3a8cb:

    # y "..............................."
    y "..............................."

# game/intro.rpy:2633
translate russia tutStage9_d0181d24:

    # "Aces Thug 2" "Woah, you're one of those WOOHP Agents...!"
    "Туз 2" "Ого, ты один из тех агентов WOOHP...!"

# game/intro.rpy:2634
translate russia tutStage9_8cf6b881:

    # "Aces Thug 1" "Whatever, these WOOHP goon are just for show. You guys act all high and mighty, but I'd like to see you stop all of us!"
    "Туз 1" "Неважно, эти чуваки из WOOHP просто позеры. Вы, ребята, ведёте себя как большие и могучие, но я бы хотел посмотреть, как вы остановите всех нас!"

# game/intro.rpy:2635
translate russia tutStage9_870dfdac:

    # "Aces Thug 2" "Yeah, you tell him! Maybe we should tear his face up a bit too!"
    "Туз 2" "Да! Может, стоит и ему немного лицо\"преукрасить\"?"

# game/intro.rpy:2636
translate russia tutStage9_deef19c1:

    # y "So much for this being a neutral zone..."
    y "Вот вам и нейтральная зона..."

# game/intro.rpy:2637
translate russia tutStage9_034ba7b9:

    # "Aces Thug 2" "Hah! We don't need your lame neutral zone! We're the Aces! We can just 'BUY' a neutral zone if we wanted to!"
    "Туз 2" "Хах! Нам не нужна ваша убогая нейтральная зона.! Мы - тузы! Мы можем просто \"купить\" нейтральную зону, если захотим!"

# game/intro.rpy:2638
translate russia tutStage9_c874ba5b:

    # "Aces Thug 1" "I've got a better idea... Let's just kill him and take this bar from him...."
    "Туз 1" "У меня есть идея получше... Давайте просто убьем его и заберем у него этот бар...."

# game/intro.rpy:2639
translate russia tutStage9_6e4ecfcc:

    # y "........................."
    y "........................."

# game/intro.rpy:2642
translate russia tutStage9_550e7c02:

    # y "You picked the wrong place to rob."
    y "Вы выбрали не лучшее место для ограбления."

# game/intro.rpy:2643
translate russia tutStage9_f6bf8338:

    # y "Nothing {w}personal {w}kid."
    y "Ничего {w}личного {w}пацан."

# game/intro.rpy:2649
translate russia tutStage9_401f1324:

    # "You pull out your gun and shoot one straight through the fucking cranius!!!"
    "Ты выхватываешь пистолет и простреливаешь одному головорезу череп!!!"

# game/intro.rpy:2650
translate russia tutStage9_551ad45d:

    # "......................."
    "......................."

# game/intro.rpy:2651
translate russia tutStage9_618b6ce6:

    # "At least you would have, if you had a gun.{w} But you don't...."
    "По крайней мере, если бы у тебя было оружие.{w} Ты бы так и сделал, но у тебя его нет...."

# game/intro.rpy:2652
translate russia tutStage9_2061e7f6:

    # "Instead you're just pointing fingerguns at the gangster."
    "Вместо этого, ты просто показываешь пальцем на бандита."

# game/intro.rpy:2655
translate russia tutStage9_36e888ea:

    # "Aces Thug 1" "..............?"
    "Туз 1" "..............?"

# game/intro.rpy:2656
translate russia tutStage9_9ca7dc43:

    # "Aces Thug 2" "..............?"
    "Туз 2" "..............?"

# game/intro.rpy:2657
translate russia tutStage9_40af8e9a:

    # y "........................{w} Boom. {w}You're dead."
    y "........................{w} Буум. {w}Ты труп."

# game/intro.rpy:2658
translate russia tutStage9_d647a492:

    # "???" "I don't know what's going on over here, but it's pretty amusing. How come nobody invited us to the party?!"
    "???" "Не знаю, что здесь происходит, но это довольно забавно. Почему никто не пригласил нас на вечеринку?!"

# game/intro.rpy:2660
translate russia tutStage9_ae5dc31e:

    # y "Yeah okay, you can have the bar."
    y "Ладно, бар ваш."

# game/intro.rpy:2661
translate russia tutStage9_2e78bc5b:

    # "Aces Thug 1" "See? That wasn't so hard, was it? You WOOHP goons are all bark and no bite!"
    "Туз 1" "Видишь? Это было не так сложно, не так ли? Вы, агенты из WOOHP, только лаете, а не кусаете!"

# game/intro.rpy:2662
translate russia tutStage9_dec02596:

    # y "Good luck paying the property tax on this place."
    y "Удачи в уплате налога на недвижимость за это место."

# game/intro.rpy:2663
translate russia tutStage9_6c5203a3:

    # "Aces Thug 1" "Hah! You're joking right? We're the Aces! {w}We could buy this place, set it on fire, then pay for the renovations just so we could light it on fire again!"
    "Туз 1" "Ха! Ты же шутишь, верно? Мы - тузы! {w}Мы могли бы купить это место, поджечь его, а потом оплатить ремонт, чтобы снова поджечь!"

# game/intro.rpy:2664
translate russia tutStage9_fb6b207b:

    # y "I dunno... The other guy maybe, but you don't look so rich to me..."
    y "Не знаю... Другой парень может быть, но ты не выглядишь настолько богатым..."

# game/intro.rpy:2665
translate russia tutStage9_a2a9746e:

    # "Aces Thug 2" "He's kinda got you there, bro. Those shoes are at least two weeks old."
    "Туз 2" "Он тебя раскусил, брат. Этим туфлям не меньше двух недель."

# game/intro.rpy:2666
translate russia tutStage9_9c158214:

    # "Aces Thug 1" "These shoes cost $3500 dollars!"
    "Туз 1" "Эти туфли стоят $3500 долларов!"

# game/intro.rpy:2667
translate russia tutStage9_e7418f5f:

    # "Aces Thug 1" "I bought a $1500 dollar jacket just this morning!"
    "Туз 1" "А этим утром, я купил пиджак за $1500!"

# game/intro.rpy:2668
translate russia tutStage9_a1aec88f:

    # "Aces Thug 2" "It was on discount."
    "Туз 2" "Она была со скидкой."

# game/intro.rpy:2669
translate russia tutStage9_71cfecee:

    # "Aces Thug 1" "I got a discount {i}'after'{/i} I threatend the guy's life to give me a discount!"
    "Туз 1" "Я получил скидку {i}'после'{/i} того, как пригрозил парню, чтобы он дал мне её!"

# game/intro.rpy:2670
translate russia tutStage9_348a798c:

    # "Aces Thug 2" "I'm just saying man. People might start to talk~...."
    "Туз 2" "Просто хочу сказать что... Люди могут начать болтать~...."

# game/intro.rpy:2671
translate russia tutStage9_a4d06d0e:

    # "???" "{b}*Laughs*{/b} Look at these rich kids flaunting their money. I'd say we help relief them of it...."
    "???" "{b}*Смешок*{/b} Только посмотрите на этих богатых детишек. Поможем им избавиться от этого их состояния..."

# game/intro.rpy:2673
translate russia tutStage9_396e4a14:

    # y "You sure you want to do that?"
    y "Уверены, что хотите это сделать?"

# game/intro.rpy:2674
translate russia tutStage9_db5fe34e:

    # y "I cleaned this place with non-brand cleaning supplies."
    y "Я убирался тут чистящими средствами без брендов."

# game/intro.rpy:2675
translate russia tutStage9_c79a66bb:

    # "Aces Thug 1" "So what?!"
    "Туз 1" "Ну и что?!"

# game/intro.rpy:2676
translate russia tutStage9_83b17bb7:

    # y "They're 'non-brand'. You realise what the means, right?"
    y "Они \"не брендовые\". Ты понимаешь, что это значит?"

# game/intro.rpy:2677
translate russia tutStage9_5bba5d3a:

    # "Aces Thug 2" "...?"
    "Туз 2" "...?"

# game/intro.rpy:2678
translate russia tutStage9_15050d50:

    # y "It means I used cheap, unregulated products, which are probably highly flamable."
    y "Это означает, что я использовал дешевые, непроверенные продукты, которые, вероятно, легко воспламеняются."

# game/intro.rpy:2679
translate russia tutStage9_218fbb16:

    # y "Smell that nice lemony scent in the air? Strike a match now and everything a ten miles radius gets evaporated."
    y "Чувствуете приятный лимонный аромат в воздухе? Чиркните спичкой, и все в радиусе десяти миль испарится."

# game/intro.rpy:2680
translate russia tutStage9_e3bf28aa:

    # "Aces Thug 2" "!!!"
    "Туз 2" "!!!"

# game/intro.rpy:2681
translate russia tutStage9_9b95f875:

    # "Aces Thug 2" "Listen dude...! Maybe we shouldn't shoot him...!"
    "Туз 2" "Слушай, чувак...! Может, не стоит в него стрелять...!"

# game/intro.rpy:2682
translate russia tutStage9_1803f89c:

    # "Aces Thug 1" "He's bluffing! He's got to be!"
    "Туз 1" "Он блефует! Точно!"

# game/intro.rpy:2683
translate russia tutStage9_15cb52e1:

    # "Aces Thug 2" "Dude, I don't know anything about cleaning products. Do you?"
    "Туз 2" "Чувак, я ничего не знаю о чистящих средствах. А ты?"

# game/intro.rpy:2684
translate russia tutStage9_1fdc8220:

    # "Aces Thug 2" "I heard that terrorists can make bombs out of simple household equipment!"
    "Туз 2" "Я слышал, что террористы могут делать бомбы из простых бытовых вещей!"

# game/intro.rpy:2685
translate russia tutStage9_e095ca28:

    # "Aces Thug 1" "R-really...?"
    "Туз 1" "Р-реально...?"

# game/intro.rpy:2686
translate russia tutStage9_5604d578:

    # "Aces Thug 2" "Yeah, my cleaning lady once threatend to blow up our house after we fired her on Christmas morning."
    "Туз 2" "Да, моя уборщица однажды угрожала взорвать наш дом после того, как мы уволили её в рождество."

# game/intro.rpy:2687
translate russia tutStage9_50788bfb:

    # "Aces Thug 1" "Dude, that's so not cool of her!"
    "Туз 1" "Чувак, это так не круто с её стороны!"

# game/intro.rpy:2688
translate russia tutStage9_fb97fae6:

    # "???" "Look, the kids are talking about making bombs. Good thing the experts have arrived~..."
    "???" "Смотрите, детишки говорят о создании бомб. Хорошо, что эксперты подоспели~..."

# game/intro.rpy:2689
translate russia tutStage9_8f434de8:

    # "Aces Thugs" "!!!"
    "Тузы" "!!!"

# game/intro.rpy:2690
translate russia tutStage9_eb144d34:

    # "A group of Outsiders thugs walked into the bar weapons drawn!"
    "В бар вошла группа головорезов \"Аутсайдеров\" с оружием наготове!"

# game/intro.rpy:2691
translate russia tutStage9_58415f93:

    # "Outsider Thug" "This is {i}'our'{/i} place now..."
    "Аутсайдер" "Теперь это {i}'наша'{/i} точка..."

# game/intro.rpy:2692
translate russia tutStage9_517de734:

    # "???" "Correction. It is {i}'our'{/i} place now!"
    "???" "Поправочка. Это {i}'наша'{/i}!"

# game/intro.rpy:2693
translate russia tutStage9_74993c91:

    # "Another gang came in! The thugs from Drift Punk enter brandishing their own guns, turning your bar into a Mexican standoff."
    "Пришла еще одна банда! Дрифт Панки входят с оружием наперевес, превращая ваш бар в школьный двор."

# game/intro.rpy:2694
translate russia tutStage9_faac66ab:

    # y "Well that escalated quickly...."
    y "Ну что ж, всё давольно быстро набирает обороты."

# game/intro.rpy:2695
translate russia tutStage9_39b9278c:

    # "Tension is rising as the gangmembers cautiously glare at each other."
    "Напряжение нарастает, члены банд настороженно смотрят друг на друга."

# game/intro.rpy:2696
translate russia tutStage9_aa3b15b7:

    # "Aces" "We claimed this place first. So buzz off."
    "Тузы" "Мы первые сюда пришли. Так что проваливайте."

# game/intro.rpy:2697
translate russia tutStage9_66bffa88:

    # "Outsiders" "You wouldn't know what to do with it. Go back to your million dollar mansions and leave the street warfare to the professionals."
    "Аутсайдеры" "Вы даже не знаете что с ним делать. Возвращайтесь в свои особняки за миллион, а уличную войну оставьте профессионалам."

# game/intro.rpy:2698
translate russia tutStage9_19526812:

    # "Drift Punks" "Hah! {i}'professionals'{/i}. The only thing you're a pro at is misspelling your name while writing your graffiti!"
    "Дрифт Панки" "Ха! {i}'профессионалы'{/i}. Единственное, в чем вы про, так это в неправильном написании своего названия на граффити!"

# game/intro.rpy:2699
translate russia tutStage9_65fbac27:

    # "Anything could set these guys off at any moment. Turning your milkshake bar into Swiss cheese."
    "Всё что угодно, может заставить этих парней действовать. В любой момент ваш молочно коктейльный бар превратится в швейцарский сыр."

# game/intro.rpy:2700
translate russia tutStage9_e197e7d4:

    # "Aces" "....................................."
    "Тузы" "....................................."

# game/intro.rpy:2701
translate russia tutStage9_8de6231f:

    # "Outsiders" "....................................."
    "Аутсайдеры" "....................................."

# game/intro.rpy:2702
translate russia tutStage9_51931095:

    # "Drift Punks" "....................................."
    "Дрифт Панки" "....................................."

# game/intro.rpy:2703
translate russia tutStage9_29a0f1ea:

    # y "......................................"
    y "......................................"

# game/intro.rpy:2704
translate russia tutStage9_f43f3331:

    # y "Hey does anyone have a bandaid? Cause I'm here to {i}'CUT'{/i} the tension."
    y "Эй, у кого-нибудь есть камера? Потому что я здесь, чтобы {i}'снять'{/i} напряжение."

# game/intro.rpy:2705
translate russia tutStage9_f1868244:

    # "Stunned, the gangmembers turn their attention to you."
    "Ошеломленные, бандиты обращают свое внимание на тебя."

# game/intro.rpy:2706
translate russia tutStage9_6bfc2010:

    # y "Eh? Eeeeh? Get it? {i}'Cut the tension'{/i}?{w} I thought it was pretty clever myself."
    y "Хм? А,а,а? Поняли? {i}'Снять напряжение'{/i}?{w} Прикольно, не так ли?"

# game/intro.rpy:2707
translate russia tutStage9_bcde2d94:

    # y "Also this is now a neutral zone. I'm with WOOHP, so I get to do that."
    y "Кроме того, теперь это нейтральная зона. Я работаю на WOOHP, так что я устанавливаю здесь правила."

# game/intro.rpy:2708
translate russia tutStage9_2ea3650d:

    # "Outsiders" "A neutral zone...?"
    "Аутсайдеры" "Нейтральная зона...?"

# game/intro.rpy:2709
translate russia tutStage9_7ca3b572:

    # "Punks" "Can he do that...?"
    "Панки" "А что так можно было...?"

# game/intro.rpy:2710
translate russia tutStage9_50393f59:

    # "Aces" "I don't know... Can he?"
    "Тузы" "Не знаю... Можно?"

# game/intro.rpy:2711
translate russia tutStage9_254cd187:

    # "Slowly, the gangmembers begin lowering their guns."
    "Медленно, бандиты начинают опускать своё оружие."

# game/intro.rpy:2712
translate russia tutStage9_93894712:

    # "Outsiders" "Fine... neutral zone."
    "Аутсайдеры" "Ладно... нейтральная зона."

# game/intro.rpy:2713
translate russia tutStage9_f65e25c8:

    # "The other gangsters nod in agreement as soon the tension seems to disappear from the bar."
    "Остальные гангстеры кивают в знак согласия. Напряжение, кажется, исчезает из бара."

# game/intro.rpy:2717
translate russia tutStage9_d5d5da76:

    # "With the catastrophe avoided, you manage to usher everyone out of the door and breath a sigh of relief."
    "Когда катастрофы миновала, ты выпроводишь всех за дверь и вздыхаешь с облегчением."

# game/intro.rpy:2718
translate russia tutStage9_0a787cb6:

    # y "I can't believe that worked..."
    y "Не могу поверить, что это сработало..."

# game/intro.rpy:2719
translate russia tutStage9_a23669b1:

    # y "............................"
    y "............................"

# game/intro.rpy:2720
translate russia tutStage9_8726655d:

    # y "The place is fixed up nicely. I can probably start serving people here now..."
    y "Место хорошо отремонтировано. Теперь можно начать обслуживать гостей..."

# game/intro.rpy:2721
translate russia tutStage9_679215fe:

    # y "But first I need to figure out a suitable work uniform for my willing waitress."
    y "Но сначала мне нужно придумать подходящую рабочую форму для моей \"добровольной\" официантки."

# game/intro.rpy:2731
translate russia tutStage9_053b7537:

    # "You finish off the last bit of work that needs to be done and it soon turns to evening."
    "Вы доделываете последние дела, как вскоре наступает вечер."

# game/intro.rpy:2734
translate russia tutStage9_ff9d16bf:

    # "Having had quite enough excitement for one day, you decide to head to bed."
    "Пережив достаточно много для одного дня, ты решаешь лечь спать."

# game/intro.rpy:2756
translate russia tutStage10_2f64c7a2:

    # y "Now where would I go for some kinky roleplaying gear..."
    y "Куда бы мне пойти за извращенным снаряжением для ролевых игр..."

# game/intro.rpy:2757
translate russia tutStage10_3d26d66b:

    # "You walk into the nearest store and see a bored salesgirl behind the counter."
    "Вы заходите в ближайший магазин и видите за прилавком скучающую продавщицу."

# game/intro.rpy:2758
translate russia tutStage10_0efa4093:

    # y "I'm looking for your kinkiest, filthiest fetish gear!"
    y "Я ищу самые извращенные, грязные и фетишистские штучки!"

# game/intro.rpy:2761
translate russia tutStage10_56cf6bcc:

    # "Clerk" ".........................."
    "Продавщица" ".........................."

# game/intro.rpy:2762
translate russia tutStage10_1d20320c:

    # "Clerk" "Sir, this is a bookstore...."
    "Продавщица" "Сэр, это книжный магазин...."

# game/intro.rpy:2763
translate russia tutStage10_a23669b1:

    # y "............................"
    y "............................"

# game/intro.rpy:2765
translate russia tutStage10_5b35e820:

    # "Clerk" "It's on the fourth floor."
    "Продавщица" "На четвёртом этаже."

# game/intro.rpy:2766
translate russia tutStage10_cb2c1ae8:

    # y "I {i}fuckin'{/i} knew it!"
    y "Я, {i}блядь{/i}, так и знал!"

# game/intro.rpy:2767
translate russia tutStage10_c2642724:

    # "The two of you make your way upstairs."
    "Вдвоем вы поднимаетесь наверх."

# game/intro.rpy:2773
translate russia tutStage10_967f79ed:

    # y "Quite the collection you got here~..."
    y "У вас прекрасная коллекция~..."

# game/intro.rpy:2774
translate russia tutStage10_f649e708:

    # "Clerk" "All bookish girls are secretly kinky as fuck."
    "Продавщица" "Все тихони втайне извращенки."

# game/intro.rpy:2775
translate russia tutStage10_4f42cbb6:

    # "Clerk" "Now then... A waitress outfit...{w} We've got this one..."
    "Продавщица" "Так теперь... Наряд официантки..{w} Есть вот такой..."

# game/intro.rpy:2782
translate russia tutStage10_a6aec0af:

    # y "It's perfect!"
    y "Идеально!"

# game/intro.rpy:2783
translate russia tutStage10_9500a0b4:

    # y "......................"
    y "......................"

# game/intro.rpy:2784
translate russia tutStage10_73c28b4e:

    # y "(Crap... I don't have any money!)"
    y "(Черт... У меня нет ни монеты!)"

# game/intro.rpy:2792
translate russia tutStage10_1c1d1326:

    # y "So I don't have the money right now...."
    y "Емм у меня как бы не особо с деньгами сейчас..."

# game/intro.rpy:2794
translate russia tutStage10_bca0ad4b:

    # "Clerk" "D'aww that's too bad! {w}Get out."
    "Продавщица" "Оохх бедняжка! {w}Убирайся."

# game/intro.rpy:2795
translate russia tutStage10_78d78d4a:

    # y "Hey now wait a second! I don't have money 'now', but I can pay you for it later!"
    y "Эй, подождите секунду! У меня нет денег \"сейчас\", но я смогу заплатить вам попозже!"

# game/intro.rpy:2796
translate russia tutStage10_c2deac0a:

    # "Clerk" "No."
    "Продавщица" "Нет."

# game/intro.rpy:2797
translate russia tutStage10_48d5d224:

    # y "Please?"
    y "Пожалуйста?"

# game/intro.rpy:2798
translate russia tutStage10_c2deac0a_1:

    # "Clerk" "No."
    "Продавщица" "Нет."

# game/intro.rpy:2799
translate russia tutStage10_a452fabd:

    # y "Pretty please?"
    y "Пазязя?"

# game/intro.rpy:2800
translate russia tutStage10_2b403523:

    # "Clerk" "Get out."
    "Продавщица" "Убирайся."

# game/intro.rpy:2801
translate russia tutStage10_785ee78f:

    # y "No."
    y "Нет."

# game/intro.rpy:2802
translate russia tutStage10_d9c9cfc5:

    # "Clerk" "Yes."
    "Продавщица" "Да."

# game/intro.rpy:2803
translate russia tutStage10_d590d637:

    # y "No. Gimme that outfit."
    y "Нет. Только с этим нарядом."

# game/intro.rpy:2804
translate russia tutStage10_0bdf2b10:

    # "Clerk" "..............."
    "Продавщица" "..............."

# game/intro.rpy:2806
translate russia tutStage10_3a49108f:

    # "Clerk" "Y-...{w} No."
    "Продавщица" "Лад-...{w} Нет."

# game/intro.rpy:2807
translate russia tutStage10_28ec04ac:

    # y "(Damn it, so close!)"
    y "(Черт, почти!)"

# game/intro.rpy:2808
translate russia tutStage10_c26fc5d6:

    # y "Okay fine! How can I convince you?"
    y "Ладно, хорошо! Как мне уговорить вас?"

# game/intro.rpy:2810
translate russia tutStage10_fb48d262:

    # "Clerk" "........................"
    "Продавщица" "........................"

# game/intro.rpy:2812
translate russia tutStage10_90e94c77:

    # "Clerk" "Who's the outfit for?"
    "Продавщица" "Для кого наряд."

# game/intro.rpy:2813
translate russia tutStage10_ee99fffe:

    # y "Er..."
    y "Эмм..."

# game/intro.rpy:2814
translate russia tutStage10_6b5c18c3:

    # "Clerk" "Is she hot?"
    "Продавщица" "Она горячая штучка?"

# game/intro.rpy:2815
translate russia tutStage10_fff2d6da:

    # y "Yes."
    y "Да."

# game/intro.rpy:2817
translate russia tutStage10_f29a3a28:

    # "Clerk" ".................................."
    "Продавщица" ".................................."

# game/intro.rpy:2818
translate russia tutStage10_34ef4da2:

    # "Clerk" "Okay fine, you can have it.{w} On one condition."
    "Продавщица" "Ладно, забирай.{w} При одном условии."

# game/intro.rpy:2819
translate russia tutStage10_5f1aafed:

    # y "Pics?"
    y "Фотки?"

# game/intro.rpy:2820
translate russia tutStage10_839eebb0:

    # "Clerk" "Pics."
    "Продавщица" "Фотки"

# game/intro.rpy:2826
translate russia tutStage10_a6225785:

    # "You got the Waitress Uniform!"
    "У тебя есть униформа официантки!"

# game/intro.rpy:2830
translate russia tutStage10_86670a6a:

    # y "Now I just gotta convince her to put it on..."
    y "Теперь осталось убедить её надеть это..."

# game/intro.rpy:2836
translate russia tutStage10_df25b227:

    # y "Yoink!"
    y "Жух!"

# game/intro.rpy:2839
translate russia tutStage10_75270c3c:

    # "You snatched the uniform out of the clerk's hands and make a run for it!"
    "Ты выхватил форму из рук продавщицы и побежал со всех ног!"

# game/intro.rpy:2841
translate russia tutStage10_603024e6:

    # "Clerk" "Hey! Get back here!"
    "Продавщица" "Эй! Вернись!"

# game/intro.rpy:2842
translate russia tutStage10_addc3b1e:

    # "Clerk" "You can't just-..."
    "Продавщица" "Ты не можешь просто-..."

# game/intro.rpy:2844
translate russia tutStage10_56cf6bcc_1:

    # "Clerk" ".........................."
    "Продавщица" ".........................."

# game/intro.rpy:2845
translate russia tutStage10_9c16d1dd:

    # "Clerk" "Whatever... I get paid minimum wage. I don't care."
    "Продавщица" "Неважно... Мне платят минимальную ставку. Мне всё равно."

# game/intro.rpy:2850
translate russia tutStage10_a6225785_1:

    # "You got the Waitress Uniform!"
    "У тебя есть униформа официантки!"

# game/intro.rpy:2854
translate russia tutStage10_86670a6a_1:

    # y "Now I just gotta convince her to put it on..."
    y "Теперь осталось убедить её надеть это..."

# game/intro.rpy:2872
translate russia tutStage11_aa3efac5:

    # s g1 "[playerName]! Good, I was hoping to talk to you."
    s g1 "[playerName]! Отлично, как раз хотела с тобой поговорить."

# game/intro.rpy:2873
translate russia tutStage11_8835fc3a:

    # s g16 "You know how you mentioned wearing a sexy waitress uniforms...?"
    s g16 "Помнишь, ты упоминал о сексуальной униформе официантки...?"

# game/intro.rpy:2874
translate russia tutStage11_0a1dce4f:

    # s g33 "Well I'm not comfortable with that, so I was thinking that I would be a cook instead."
    s g33 "Меня это не особо устраивает, поэтому я буду поваром."

# game/intro.rpy:2875
translate russia tutStage11_64a2b68e:

    # y "..........................."
    y "..........................."

# game/intro.rpy:2876
translate russia tutStage11_4ad79e04:

    # s g1 "And you can be a waiter! Instead of wearing a skimpy uniform you could wear a nice suit and we can turn this into a classy establishment!"
    s g1 "А ты можешь быть официантом! Вместо того чтобы носить скудную униформу, ты мог бы надеть хороший костюм, и мы могли бы превратить бар в стильное заведение!"

# game/intro.rpy:2877
translate russia tutStage11_985ce47e:

    # s "Also think of how progressive it would be! A man doing all the talking and a woman in the...{w} kitchen..."
    s "А ещё подумай, насколько это прогрессивно! Мужчина занят разговорами и другими делами, а женщина стоит на...{w} кухне..."

# game/intro.rpy:2878
translate russia tutStage11_2383364d:

    # y "Didn't think this one through, did you?"
    y "Не обдумала это, не так ли?"

# game/intro.rpy:2879
translate russia tutStage11_95877c85:

    # s g15 "O-okay it sounds bad when I say it like that!"
    s g15 "Согласна, звучит не очень, когда я так говорю!"

# game/intro.rpy:2880
translate russia tutStage11_6a560659:

    # y "Here's your uniform. You're a waitress now."
    y "Вот твоя форма. Теперь ты официантка."

# game/intro.rpy:2881
translate russia tutStage11_3cc826ac:

    # "You give Sam her uniform."
    "Ты отдаешь Сэм ее форму"

# game/intro.rpy:2890
translate russia tutStage11_8b0d240e:

    # s g18 "O-oh...! That's a little more revealing than I hoped. Where's the rest of it?"
    s g18 "О-о-о...! Это немного более откровенно, чем я рассчитывала. А где остальное?"

# game/intro.rpy:2891
translate russia tutStage11_b5732a78:

    # y "....................."
    y "....................."

# game/intro.rpy:2892
translate russia tutStage11_46a6e53f:

    # s g28 "T-there is no rest of it?!"
    s g28 "Э-это всё?!"

# game/intro.rpy:2893
translate russia tutStage11_6b573bac:

    # y "Welcome to the service industry!"
    y "Добро пожаловать в сферу обслуживания!"

# game/intro.rpy:2916
translate russia tutStage11_92848bcc:

    # "DEV END WARNING!!!! YOU SHOULD NOT SEE THIS"
    "DEV END WARNING!!!! ВЫ НЕ ДОЛЖНЫ ВИДЕТЬ ЭТОГО"

translate russia strings:

    # game/intro.rpy:17
    old "Begin intro"
    new "Запустить Интро"

    # game/intro.rpy:22
    old "Yes, skip the intro"
    new "Да, пропустите интро"

    # game/intro.rpy:22
    old "Let me play the intro"
    new "Начать с интро"

    # game/intro.rpy:100
    old "YES SIR!"
    new "ТАК ТОЧНО!"

    # game/intro.rpy:100
    old "Am I getting paid for this?"
    new "Мне заплатят?"

    # game/intro.rpy:100
    old "Say something insane"
    new "Сказать что-то безумное"

    # game/intro.rpy:205
    old "Finish your job (+Karma)"
    new "Закончить свою работу (+Карма)"

    # game/intro.rpy:205
    old "Escape (-Karma)"
    new "Побег (-Карма)"

    # game/intro.rpy:256
    old "Have a milkshake"
    new "Взять молочный коктейль"

    # game/intro.rpy:256
    old "Clean the milkshake bar"
    new "Убраться в баре"

    # game/intro.rpy:256
    old "Head back downstairs"
    new "Вернутся вниз"

    # game/intro.rpy:438
    old "Power"
    new "Власть"

    # game/intro.rpy:438
    old "Wealth"
    new "Богатство"

    # game/intro.rpy:438
    old "Tiddies"
    new "Сисечки"

    # game/intro.rpy:644
    old "'I'm not a threat to you!'"
    new "'Я не представляю для вас угрозы!'"

    # game/intro.rpy:644
    old "'Neat, I wanna be a gangster'"
    new "'Отлично, я хочу быть гангстером'"

    # game/intro.rpy:644
    old "*Say something crazy*"
    new "*Сказать что-нибудь безумное*"

    # game/intro.rpy:918
    old "Call Sam"
    new "Позвать Сэм"

    # game/intro.rpy:918
    old "Call Clover"
    new "Позвать Кловер"

    # game/intro.rpy:918
    old "Call Alex"
    new "Позвать Алекс"

    # game/intro.rpy:918
    old "Go to sleep"
    new "Пойти спать"

    # game/intro.rpy:927
    old "???"
    new "???"

    # game/intro.rpy:927
    old "Chat"
    new "Написать"

    # game/intro.rpy:927
    old "Change naming"
    new "Изменить наименования"

    # game/intro.rpy:1122
    old "Type in: '12345'"
    new "Ввести: '12345'"

    # game/intro.rpy:1122
    old "Type in: 'WOOHP'"
    new "Ввести: 'WOOHP'"

    # game/intro.rpy:1122
    old "Type in: 'pistachios'"
    new "Ввести: 'фисташки'"

    # game/intro.rpy:1196
    old "Spy on Sam"
    new "Шпионить за Сэм"

    # game/intro.rpy:1196
    old "Spy on Clover"
    new "Шпионить за Кловер"

    # game/intro.rpy:1196
    old "Spy on Alex"
    new "Шпионить за Алекс"

    # game/intro.rpy:1280
    old "Mess around with it"
    new "Разобраться с этим"

    # game/intro.rpy:1280
    old "Leave it be"
    new "Оставить все как есть"

    # game/intro.rpy:2640
    old "Shoot one of them"
    new "Застрелите одного из них"

    # game/intro.rpy:2640
    old "Surrender the bar"
    new "Отдать бар"

    # game/intro.rpy:2640
    old "Say something crazy"
    new "Сказать что-то безумное"

    # game/intro.rpy:2785
    old "Take a loan (+Karma)"
    new "Взять кредит (+Карма/-Почка)"

    # game/intro.rpy:2785
    old "Grab and run! (-Karma)"
    new "Хватай и беги! (-Карма)"
